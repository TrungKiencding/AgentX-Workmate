#!/usr/bin/env bash
# Canonical test runner for agentx-agent. Run this instead of calling
...
#   * Proper venv activation (probes .venv, venv, then ~/.agentx/...)
#
# Usage:
...
# (AGENTX_PYTHON is exported by the devShell hook and ships [dev] extras:
# pytest, pytest-asyncio, pytest-timeout, ruff, ty).
#
# A candidate must have pytest INSTALLED, not merely exist. The release venv
# at ~/.agentx/agentx-agent/venv has bin/activate but no pytest, so an
...
VENV_PYTHON=""
SKIPPED_VENVS=""
for candidate in "$REPO_ROOT/.venv" "$REPO_ROOT/venv" "$HOME/.agentx/agentx-agent/venv"; do
...
if [ -n "$VENV" ]; then
  ...
  echo "       and AGENTX_PYTHON is not a python with pytest (enter the Nix devShell or create a venv)" >&2
  if [ -n "$SKIPPED_VENVS" ]; then
...
if [ -f "$HOME/.agentx/pytest_live_guard.py" ]; then
  EXTRA_PYTHONPATH="$HOME/.agentx"
...
exec env -i \
  ...
  PYTHONHASHSEED=0 \
  PYTHONUTF8=1 \
  ${AGENTX_RUN_SLOW_PET_TESTS:+AGENTX_RUN_SLOW_PET_TESTS="$AGENTX_RUN_SLOW_PET_TESTS"} \
  ${AGENTX_E2E_BROWSER:+AGENTX_E2E_BROWSER="$AGENTX_E2E_BROWSER"} \
