#!/usr/bin/env node
/**
 * AgentX Workmate WhatsApp Bridge
 *
 ...
 *
 * Usage:
 *   node bridge.js --port 3000 --session ~/.agentx/whatsapp/session
 */
...
const SESSION_DIR = getArg('session', path.join(process.env.HOME || '~', '.agentx', 'whatsapp', 'session'));
// Cache directories: the Python gateway passes the profile-aware paths via
// env (AGENTX_HOME-aware, new cache/ layout).  Fall back to the legacy
// hardcoded locations for bridges launched outside the gateway.
const IMAGE_CACHE_DIR = process.env.AGENTX_IMAGE_CACHE_DIR
  || path.join(process.env.HOME || '~', '.agentx', 'image_cache');
const DOCUMENT_CACHE_DIR = process.env.AGENTX_DOCUMENT_CACHE_DIR
  || path.join(process.env.HOME || '~', '.agentx', 'document_cache');
const AUDIO_CACHE_DIR = process.env.AGENTX_AUDIO_CACHE_DIR
  || path.join(process.env.HOME || '~', '.agentx', 'audio_cache');
...
// `agentx update` updates bridge.js on disk but a long-lived bridge process
...
const DEFAULT_REPLY_PREFIX = '⬡ *AgentX Workmate*\n────────────\n';
...
function enqueuePollUpdateEvent({ key, update, selectedOptions, aggregation }) {
  ...
  const pollId = key?.id
    ...
    || '';
  // Only surface votes on polls AgentX itself created (tracked when
...
async function startSocket() {
  ...
  sock = makeWASocket({
    ...
    browser: ['AgentX Workmate', 'Chrome', '120.0'],
    ...
    for (const msg of messages) {
      ...
      if (msg.key.fromMe) {
        ...
        if (WHATSAPP_MODE === 'bot') {
          ...
          // the owner replied to would leak into AgentX and trigger
      ...
      // Ignore AgentX' own reply messages in self-chat mode to avoid loops.
