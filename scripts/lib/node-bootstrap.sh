...
#   2. ~/.agentx/node/ from a prior AgentX-managed install
#   3. fnm, proto, nvm (in that order) if the user already uses a version manager
#   4. Termux `pkg`, macOS Homebrew
#   5. pinned nodejs.org tarball into ~/.agentx/node/ (always works, zero shell rc edits)
#
# Usage:
#   source scripts/lib/node-bootstrap.sh
#   ensure_node   # returns 0 on success, non-zero on failure
#   if [ "$AGENTX_NODE_AVAILABLE" = true ]; then ...; fi
#
# Env inputs (set before sourcing to override defaults):
#   AGENTX_NODE_MIN_VERSION   (default: 20)   — accepted on PATH
#   AGENTX_NODE_TARGET_MAJOR  (default: 22)   — installed when we install
#   AGENTX_HOME               (default: $HOME/.agentx)
# ============================================================================

AGENTX_NODE_MIN_VERSION="${AGENTX_NODE_MIN_VERSION:-20}"
AGENTX_NODE_TARGET_MAJOR="${AGENTX_NODE_TARGET_MAJOR:-22}"
AGENTX_HOME="${AGENTX_HOME:-$HOME/.agentx}"
AGENTX_NODE_AVAILABLE=false
...
_nb_configure_npm_prefix() {
    ...
    printf 'prefix=%s\n' "$(dirname "$_link_dir")" > "$AGENTX_HOME/node/etc/npmrc"
}
...
_nb_npm_range() {
    if [ -n "${AGENTX_NPM_TARGET_RANGE:-}" ]; then
        printf '%s\n' "$AGENTX_NPM_TARGET_RANGE"
        return 0
    fi
...
_nb_ensure_bundled_npm_range() {
    local npm_bin="$AGENTX_HOME/node/bin/npm"
    [ -x "$npm_bin" ] || return 0
    ...
    _nb_warn "Fix manually: npm install -g --prefix \"$AGENTX_HOME/node\" npm@\"$range\""
    return 1
}

_nb_have_modern_node() {
    command -v node >/dev/null 2>&1 || return 1
    [ "$(_nb_node_major)" -ge "$AGENTX_NODE_MIN_VERSION" ]
}
...
_nb_try_fnm() {
    ...
    _nb_log "fnm detected — installing Node $AGENTX_NODE_TARGET_MAJOR..."
    eval "$(fnm env 2>/dev/null)" || true
    fnm install "$AGENTX_NODE_TARGET_MAJOR" >/dev/null 2>&1 || return 1
    fnm use     "$AGENTX_NODE_TARGET_MAJOR" >/dev/null 2>&1 || return 1
...
_nb_try_proto() {
    ...
    _nb_log "proto detected — installing Node $AGENTX_NODE_TARGET_MAJOR..."
    proto install node "$AGENTX_NODE_TARGET_MAJOR" >/dev/null 2>&1 || return 1
    _nb_have_modern_node || return 1
...
_nb_try_nvm() {
    ...
    _nb_log "nvm detected — installing Node $AGENTX_NODE_TARGET_MAJOR..."
    nvm install "$AGENTX_NODE_TARGET_MAJOR" >/dev/null 2>&1 || return 1
    nvm use     "$AGENTX_NODE_TARGET_MAJOR" >/dev/null 2>&1 || return 1
    _nb_have_modern_node || return 1
...
_nb_try_brew() {
    ...
    brew install "node@${AGENTX_NODE_TARGET_MAJOR}" >/dev/null 2>&1 \
        || brew install node >/dev/null 2>&1 \
        || return 1
    brew link --overwrite --force "node@${AGENTX_NODE_TARGET_MAJOR}" >/dev/null 2>&1 || true
    _nb_have_modern_node || return 1
...
_nb_install_bundled_node() {
    ...
    local index_url="https://nodejs.org/dist/latest-v${AGENTX_NODE_TARGET_MAJOR}.x/"
    local tarball
    tarball=$(curl -fsSL "$index_url" \
        | grep -oE "node-v${AGENTX_NODE_TARGET_MAJOR}\.[0-9]+\.[0-9]+-${node_os}-${node_arch}\.tar\.xz" \
        | head -1)
    if [ -z "$tarball" ]; then
        tarball=$(curl -fsSL "$index_url" \
            | grep -oE "node-v${AGENTX_NODE_TARGET_MAJOR}\.[0-9]+\.[0-9]+-${node_os}-${node_arch}\.tar\.gz" \
            | head -1)
    fi
    if [ -z "$tarball" ]; then
        _nb_warn "Could not resolve Node $AGENTX_NODE_TARGET_MAJOR binary for $node_os-$node_arch"
        return 1
    fi
    ...
    _nb_log "Extracting to $AGENTX_HOME/node/..."
    ...
    mv "$extracted" "$AGENTX_HOME/node"
    rm -rf "$tmp"

    local _link_dir
    _link_dir="$(_nb_get_link_dir)"
    # AGENTX_NODE_SKIP_LINKS=1: the caller only wants the private managed tree
    ...
    if [ "${AGENTX_NODE_SKIP_LINKS:-0}" != "1" ]; then
        mkdir -p "$_link_dir"
        ln -sf "$AGENTX_HOME/node/bin/node" "$_link_dir/node"
        ln -sf "$AGENTX_HOME/node/bin/npm"  "$_link_dir/npm"
        ln -sf "$AGENTX_HOME/node/bin/npx"  "$_link_dir/npx"
    fi
    ...
    _nb_have_modern_node || return 1
    _nb_ok "Node $(node --version) installed to $AGENTX_HOME/node/"
...
# ---------------------------------------------------------------------------
# Heal a broken AgentX-managed Node tree (partial upgrade / missing lib/)
# ---------------------------------------------------------------------------
...
_nb_managed_tool_broken() {
    ...
    local probe
    for probe in \
        "$AGENTX_HOME/node/bin/$tool" \
        "$AGENTX_HOME/node/${tool}.exe" \
        "$AGENTX_HOME/node/$tool"; do
...
# The managed node runs but is below AGENTX_NODE_TARGET_MAJOR — an old tree
...
_nb_managed_node_outdated() {
    ...
    for probe in "$AGENTX_HOME/node/bin/node" "$AGENTX_HOME/node/node"; do
        [ -x "$probe" ] || continue
        ver="$("$probe" --version 2>/dev/null)" || return 1
        major="${ver#v}"; major="${major%%.*}"
        case "$major" in ''|*[!0-9]*) return 1 ;; esac
        [ "$major" -lt "$AGENTX_NODE_TARGET_MAJOR" ] && return 0
        return 1
    done
    return 1
}
...
heal_managed_node() {
    [ -d "$AGENTX_HOME/node" ] || return 1
    if ! _nb_managed_node_needs_heal; then
        return 0
    fi
    _nb_log "AgentX-managed Node is broken — redownloading to $AGENTX_HOME/node/..."
    _nb_install_bundled_node
}
...
ensure_node() {
    ...
    if _nb_have_modern_node; then
        ...
        AGENTX_NODE_AVAILABLE=true
        return 0
    fi

    if [ -x "$AGENTX_HOME/node/bin/node" ]; then
        export PATH="$AGENTX_HOME/node/bin:$PATH"
        if _nb_have_modern_node; then
            _nb_ok "Node $(node --version) found (AgentX-managed)"
            AGENTX_NODE_AVAILABLE=true
    ...
    _nb_try_fnm   && { AGENTX_NODE_AVAILABLE=true; return 0; }
    _nb_try_proto && { AGENTX_NODE_AVAILABLE=true; return 0; }
    _nb_try_nvm   && { AGENTX_NODE_AVAILABLE=true; return 0; }

    # Platform package managers.
    _nb_try_termux_pkg && { AGENTX_NODE_AVAILABLE=true; return 0; }
    _nb_try_brew       && { AGENTX_NODE_AVAILABLE=true; return 0; }

    # Last resort: pinned nodejs.org tarball.
    _nb_install_bundled_node && { AGENTX_NODE_AVAILABLE=true; return 0; }

    _nb_warn "Node.js install failed — TUI and browser tools will be unavailable."
    _nb_warn "Install manually: https://nodejs.org/en/download/  (or: \`brew install node\`, \`fnm install $AGENTX_NODE_TARGET_MAJOR\`, etc.)"
    return 1
}
