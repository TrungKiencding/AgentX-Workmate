#!/usr/bin/env python3
"""Drive the AgentX TUI under AGENTX_DEV_PERF and summarize the pipeline.
...
~30 Hz (matching xterm key-repeat), summarizes ~/.agentx/perf.log on exit.

The --tui build must exist (run `npm run build` in ui-tui first). This script
launches `node dist/entry.js` directly with AGENTX_TUI_RESUME set so it
bypasses the hermes_cli wrapper — we want repeatable timing, not the CLI's
session-picker flow.

Environment overrides:
  AGENTX_PERF_LOG     (default ~/.agentx/perf.log)
  AGENTX_PERF_NODE    (default node from $PATH)
  AGENTX_TUI_DIR      (default: <repo>/ui-tui relative to this script)

Exit code is 0 if the harness ran and parsed results, 2 if the TUI crashed
or produced no perf data (suggests AGENTX_DEV_PERF wiring is broken).
"""
...
try:
    ...
    def get_hermes_home() -> Path:  # type: ignore[misc]
        val = (os.environ.get("AGENTX_HOME") or "").strip()
        return Path(val) if val else Path.home() / ".agentx"

DEFAULT_TUI_DIR = Path(
    os.environ.get("AGENTX_TUI_DIR")
    or str(Path(__file__).resolve().parent.parent / "ui-tui")
)
DEFAULT_LOG = Path(os.environ.get("AGENTX_PERF_LOG", str(get_hermes_home() / "perf.log")))
...
def format_report(data: dict[str, Any]) -> str:
    ...
    if not react:
        out.append("  (no react events — AGENTX_DEV_PERF wired? threshold too high?)")
    else:
...
def run_once(args: argparse.Namespace) -> dict[str, Any]:
    ...
    env = os.environ.copy()
    env["AGENTX_DEV_PERF"] = "1"
    env["AGENTX_DEV_PERF_MS"] = str(args.threshold_ms)
    env["AGENTX_DEV_PERF_LOG"] = str(log)
    env["AGENTX_TUI_RESUME"] = sid
    ...
    node = os.environ.get("AGENTX_PERF_NODE", "node")
...
def main() -> int:
    ...
    p.add_argument("--threshold-ms", type=float, default=0.0, help="AGENTX_DEV_PERF_MS (0 = capture all)")
