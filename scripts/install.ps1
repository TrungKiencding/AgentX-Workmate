# ============================================================================
# AgentX Workmate Installer for Windows
# ============================================================================
...
param(
    ...
    [switch]$ForceCommit,
    [string]$Tag = "",
    [string]$AgentXHome = $(if ($env:AGENTX_HOME) { $env:AGENTX_HOME } else { "$env:LOCALAPPDATA\agentx" }),
    [string]$InstallDir = $(if ($env:AGENTX_HOME) { "$env:AGENTX_HOME\agentx-agent" } else { "$env:LOCALAPPDATA\agentx\agentx-agent" }),
    ...
    # builds apps/desktop into a launchable AgentX Workmate.exe.
    #
    # Why opt-in:
    #   * AgentX-Setup.exe (the signed Tauri bootstrap installer) passes
    ...
    #     from inside an already-launched AgentX Workmate.exe; if THAT recursively
    #     built apps/desktop it would try to overwrite the live AgentX Workmate.exe
    ...
    #     `agentx desktop` already builds on demand.
...
function Write-Banner {
    ...
    Write-Host "|             * AgentX Workmate Installer                    |" -ForegroundColor Magenta
    Write-Host "+---------------------------------------------------------+" -ForegroundColor Magenta
    Write-Host "|  An open source AI agent by AstralX Technology.              |" -ForegroundColor Magenta
    Write-Host "+---------------------------------------------------------+" -ForegroundColor Magenta
    Write-Host ""
}
...
function Write-BrowserEnv {
    ...
    if (-not (Test-Path $AgentXHome)) {
        New-Item -ItemType Directory -Force -Path $AgentXHome | Out-Null
    }
    $envFile = Join-Path $AgentXHome ".env"
...
function Install-AgentBrowser {
    ...
    $prefixDir = Join-Path $AgentXHome "node"
...
function Install-Uv {
    # AgentX owns its own uv at $AgentXHome\bin\uv.exe.  Always install there --
    # no PATH probing, no conda guards, no multi-location resolution chains.
    # The runtime update path (hermes_cli/managed_uv.py) looks in the same
    # place, so install.ps1 and `agentx update` stay in sync.
    $managedUv = Join-Path $AgentXHome "bin\uv.exe"
    ...
    Write-Info "Installing managed uv into $AgentXHome\bin ..."
    New-Item -ItemType Directory -Path (Join-Path $AgentXHome "bin") -Force | Out-Null

    # UV_INSTALL_DIR tells the astral installer to place the binary
    # directly into $AgentXHome\bin instead of ~/.local/bin.
    $prevEAP = $ErrorActionPreference
    try {
        $ErrorActionPreference = "Continue"
        $env:UV_INSTALL_DIR = Join-Path $AgentXHome "bin"
        ...
        # (hermes_cli/managed_uv.py looks only at $AgentXHome\bin\uv.exe).
...
# Put the AgentX-managed Node dir at the FRONT of the persisted User PATH.
#
...
# environment (a standalone agentx-setup.exe run, a user typing `npm`) silently
...
# Upgrade the AgentX-managed Node tree's bundled npm into $NpmRange.
#
...
function Update-ManagedNpm {
    ...
    $tmpCwd = Join-Path $env:TEMP ("agentx-npm-upgrade-" + [Guid]::NewGuid().ToString("N"))
...
function Resolve-UvCmd {
    ...
    # sibling process and AGENTX_HOME wasn't propagated).
...
function Resolve-AvailablePythonVersion {
    ...
    # ``$script:PythonVersion = $fallbackVer`` mutation.  Under AgentX-Setup.exe
...
function New-GitBashAslrFailureReason {
    ...
    return @(
        ...
        "Then rerun AgentX setup. If the override is blocked or later re-applied, ask your Windows administrator to allow this per-program exception."
...
function Install-Git {
    ...
    which AgentX uses to run shell commands.
    ...
    package manager):
      ...
      2. Download **PortableGit** from the official git-for-windows GitHub
         ...
         ``%LOCALAPPDATA%\agentx\git`` -- never touches system Git, never
    ...
    **Why PortableGit, not MinGit:**  MinGit is the minimal-automation
    ...
    AgentX needs ``bash.exe`` to run shell commands.  PortableGit is the
    ...
    blocked).  Owning the AgentX copy of Git ourselves is predictable and
    recoverable: if it ever breaks, ``Remove-Item %LOCALAPPDATA%\agentx\git``
    and re-running this installer fully recovers.

    After install we locate ``bash.exe`` and persist the path in
    ``AGENTX_GIT_BASH_PATH`` (User scope) so AgentX can find it in a fresh
    ...
    Write-Info "Git not found -- downloading PortableGit to $AgentXHome\git\ ..."
    ...
    try {
        ...
        if ($arch -eq "32-bit-mingit") {
            Write-Warn "32-bit Windows detected -- PortableGit is 64-bit only.  Installing MinGit 32-bit as a last resort; bash-dependent AgentX features (terminal tool, agent-browser) will not work on this machine."
        ...
        Write-Info "then re-run this installer.  AgentX needs Git Bash on Windows to run"
...
function Set-GitBashEnvVar {
    ...
    ``AGENTX_GIT_BASH_PATH`` (User env scope) so AgentX can find it even before
    ...
    #
    # Layouts:
    #   PortableGit (our default): $AgentXHome\git\bin\bash.exe
    #   MinGit (32-bit fallback):  $AgentXHome\git\usr\bin\bash.exe
    $candidates += "$AgentXHome\git\bin\bash.exe"       # PortableGit layout (primary)
    $candidates += "$AgentXHome\git\usr\bin\bash.exe"   # MinGit / PortableGit usr\bin fallback
    ...
    foreach ($candidate in $candidates) {
        if ($candidate -and (Test-Path $candidate)) {
            [Environment]::SetEnvironmentVariable("AGENTX_GIT_BASH_PATH", $candidate, "User")
            $env:AGENTX_GIT_BASH_PATH = $candidate
            $script:GitBashPath = $candidate
            Write-Info "Set AGENTX_GIT_BASH_PATH=$candidate"
            return
        }
    }

    Write-Warn "Could not locate bash.exe -- AgentX may not find Git Bash."
    Write-Info "If needed, set AGENTX_GIT_BASH_PATH manually to your bash.exe path."
}
...
function Test-Node {
    ...
    if ((Test-Path $managedNode) -and (Test-NodeVersionOk (& $managedNode --version))) {
        ...
        $env:Path = "$AgentXHome\node;$env:Path"
        Set-ManagedNodeFirstOnUserPath "$AgentXHome\node"
        Write-Success "Node.js $version found (AgentX-managed)"
        ...
        Update-ManagedNpm "$AgentXHome\node" | Out-Null
        $script:HasNode = $true
        return $true
    }

    Write-Info "Installing AgentX-managed Node.js $NodeVersion LTS..."
    ...
    # The portable zip path drops node.exe + npm into $AgentXHome\node\
    ...
    Write-Info "Downloading portable Node.js $NodeVersion to $AgentXHome\node\ ..."
    ...
    try {
        ...
        if ($zipName) {
            ...
            $tmpDir = "$env:TEMP\agentx-node-extract"
            ...
            if ($extractedDir) {
                if (Test-Path "$AgentXHome\node") { Remove-Item -Recurse -Force "$AgentXHome\node" }
                Move-Item $extractedDir.FullName "$AgentXHome\node"
                ...
                $version = & "$AgentXHome\node\node.exe" --version
                Write-Success "Node.js $version installed to $AgentXHome\node\ (portable, user-scoped)"
...
function Install-SystemPackages {
    ...
    if ($hasWinget) {
        ...
        foreach ($pkg in $wingetPkgs) {
            $log = "$env:TEMP\agentx-winget-$($pkg -replace '[^A-Za-z0-9]','_')-$(Get-Random).log"
            $pkgLogs[$pkg] = $log
...
function Install-Repository {
        ...
        if ($repoValid) {
            ...
            try {
                ...
                if (-not [string]::IsNullOrWhiteSpace(($statusOut -join "`n"))) {
                    ...
                    # the index conflict state is dropped. Mirrors the `agentx
                    # update` path (#4735).
                    ...
                    $stashName = "agentx-install-autostash-" + (Get-Date -Format "yyyyMMdd-HHmmss")
                ...
                if ($Commit) {
                    ...
                    # ff-only pull cannot succeed -- mirror ``agentx update`` and
                ...
                if ($autostashRef) {
                    ...
                    if ($restoreNow) {
                        ...
                        if (($restoreExit -eq 0) -and ($conflictedFiles.Count -eq 0)) {
                            ...
                            Write-Warn "Review git diff / git status if AgentX behaves unexpectedly."
                        } else {
            ...
            try {
                ...
                Write-Info "terminals, running agentx processes) and try again."
                throw
            }
        }
    }
    ...
    if (-not $didUpdate) {
        ...
        if (-not $cloneSuccess) {
            ...
            try {
                ...
                $extractPath = "$env:TEMP\agentx-agent-extract"
                ...
                if ($extractedDir) {
                    ...
                    # `agentx update` (see the notes at the shared clone-path
    ...
    # next `agentx update` checkout aborts on a "dirty" tree the user never
...
function Install-Venv {
    ...
    # Re-resolve the interpreter before creating the venv.  Under AgentX-Setup.exe
    ...
    if (Test-Path -LiteralPath "venv") {
        ...
        # speedups.pyd) are loaded as DLLs by any running agentx process.
        ...
        if ($env:OS -eq "Windows_NT") {
            $myPid = $PID
            Write-Info "Stopping any running agentx processes before recreating venv..."
            ...
            try {
                schtasks /Query /FO CSV 2>$null | ConvertFrom-Csv | Where-Object { $_.TaskName -like '*AgentX_Gateway*' } | ForEach-Object {
                    $tn = $_.TaskName
            ...
            # The launcher CLI (agentx.exe) plus its child tree.
            & taskkill /F /T /IM agentx.exe /FI "PID ne $myPid" 2>$null | Out-Null
            # taskkill /IM agentx.exe is NOT enough: the gateway/agent that a
            # scheduled task or watchdog autostarts runs as
            # `pythonw.exe -m hermes_cli.main gateway run` straight out of
            # venv\Scripts\, so its image name is python/pythonw, not agentx.exe.
            ...
            # AgentX children re-exec through .agentx-runtime, so killing only
        ...
        try {
            ...
            throw (
                ...
                "A process still has the install directory open (often a non-AgentX " +
    ...
    $env:UV_PYTHON = $venvPythonExe
        ...
        # the task fires normally on next logon and `agentx update` / the
...
function Install-Dependencies {
    ...
    if (Test-Path "uv.lock") {
        ...
        # empty and producing the broken state where `agentx.exe` exists
...
"@ 2>$null
    ...
    if (-not $installed) {
        throw "Failed to install agentx-agent package even with no extras. Inspect the uv pip install output above."
    }
    ...
    # isn't set, leaving venv\ empty and agentx.exe broken with
    ...
    # here, not 30 seconds later when the user runs `agentx`.
    if (-not $NoVenv) {
        $venvPython = "$InstallDir\venv\Scripts\python.exe"
        if (-not (Test-Path $venvPython)) {
            throw "Install reported success but $venvPython does not exist. The dependency sync likely landed in a sibling .venv\ directory. Re-run the installer; if it persists, close AgentX processes and preserve existing venv directories before retrying. Do not delete venv in place."
        }
        ...
        if ($importExitCode -ne 0) {
            ...
            $hint = if (Test-Path $sibling) {
                "Detected sibling .venv\ at $sibling -- uv synced there instead of venv\. Close AgentX processes, preserve the existing venv, and rerun the installer so the transactional recovery path can move directories safely."
            } else {
    ...
    if (-not $NoVenv) {
        # uv on Windows can register agentx.exe in dist-info/RECORD but fail to
        ...
        # `agentx` command while agentx-agent.exe / agentx-acp.exe exist
...
"@ 2>$null
    ...
    # users hit and lazy-import errors from `agentx dashboard` are confusing.
    ...
    if (Test-Path $pythonExe) {
        ...
        $ErrorActionPreference = $prevEAP
        if (-not $webOk) {
            Write-Warn "fastapi/uvicorn not importable -- `agentx dashboard` will not work."
            Write-Info "Attempting targeted install of [web] extra as last resort..."
            & $UvCmd pip install -e ".[web]"
            if ($LASTEXITCODE -eq 0) {
                Write-Success "[web] extra installed; `agentx dashboard` should now work."
            } else {
...
function Set-PathVariable {
    ...
    $currentHermesHome = [Environment]::GetEnvironmentVariable("AGENTX_HOME", "User")
    if (-not $currentHermesHome -or $currentHermesHome -ne $AgentXHome) {
...
function Write-BootstrapMarker {
    # Writes $InstallDir\.agentx-bootstrap-complete which tells the AgentX
    ...
    # AgentX-Setup.exe) or fall back to whatever git resolves in the
    # checkout. The desktop validates schemaVersion + pinnedCommit
    # length but doesn't enforce that HEAD matches the pin (users
    # update via `agentx update` which moves HEAD legitimately).
    ...
    $markerPath = Join-Path $InstallDir ".agentx-bootstrap-complete"
    $marker = [ordered]@{
        schemaVersion = 1
...
function Copy-ConfigTemplates {
    ...
    # Create the AGENTX_HOME directory structure ($AgentXHome, default %LOCALAPPDATA%\agentx)
    New-Item -ItemType Directory -Force -Path "$AgentXHome\cron" | Out-Null
    New-Item -ItemType Directory -Force -Path "$AgentXHome\sessions" | Out-Null
    New-Item -ItemType Directory -Force -Path "$AgentXHome\logs" | Out-Null
    New-Item -ItemType Directory -Force -Path "$AgentXHome\pairing" | Out-Null
    New-Item -ItemType Directory -Force -Path "$AgentXHome\hooks" | Out-Null
    New-Item -ItemType Directory -Force -Path "$AgentXHome\image_cache" | Out-Null
    New-Item -ItemType Directory -Force -Path "$AgentXHome\audio_cache" | Out-Null
    New-Item -ItemType Directory -Force -Path "$AgentXHome\memories" | Out-Null
    New-Item -ItemType Directory -Force -Path "$AgentXHome\skills" | Out-Null

    
    # Create .env
    $envPath = "$AgentXHome\.env"
    ...
    # Create config.yaml
    $configPath = "$AgentXHome\config.yaml"
    ...
    # (the default PS5 behaviour), and AgentX's prompt-injection scanner
    ...
    if (-not (Test-Path $soulPath)) {
        ...
        $soulContent = @"
You are AgentX Workmate, an intelligent AI assistant created by AstralX Technology. You are helpful, knowledgeable, and direct. You assist users with a wide range of tasks including answering questions, writing and editing code, analyzing information, creative work, and executing actions via your tools. You communicate clearly, admit uncertainty when appropriate, and prioritize being genuinely useful over being verbose unless otherwise directed below. Be targeted and efficient in your exploration and investigations.
"@
        $utf8NoBom = New-Object System.Text.UTF8Encoding($false)
    ...
    Write-Success "Configuration directory ready: $AgentXHome"
    
    # Seed bundled skills into $AgentXHome\skills (manifest-based, one-time per skill)
    Write-Info "Syncing bundled skills to $AgentXHome\skills ..."
    ...
    if (Test-Path $pythonExe) {
        try {
            ...
            Write-Success "Skills synced to $AgentXHome\skills"
        } catch {
            # Fallback: simple directory copy
            $bundledSkills = "$InstallDir\skills"
            $userSkills = "$AgentXHome\skills"
            if ((Test-Path $bundledSkills) -and -not (Get-ChildItem $userSkills -Exclude '.bundled_manifest' -ErrorAction SilentlyContinue)) {
                Copy-Item -Path "$bundledSkills\*" -Destination $userSkills -Recurse -Force -ErrorAction SilentlyContinue
                Write-Success "Skills copied to $AgentXHome\skills"
            }
        }
    }
}
...
function Install-NodeDeps {
    if (-not $HasNode) {
        # Cross-process driver mode (AgentX-Setup.exe runs each -Stage NAME
    ...
    if (-not $npmCmd) {
        ...
        Write-Info "Open a new PowerShell window and re-run 'agentx setup tools' later."
        return
    }
    $npmExe = $npmCmd.Source
    if ($npmExe -like "*.ps1") {
    ...
    if (Test-Path "$InstallDir\package.json") {
        ...
        $browserLog = "$env:TEMP\agentx-npm-browser-$(Get-Random).log"
        ...
        if ($browserNpmOk) {
            ...
            if (-not $npxExe) {
                ...
                $pwLog = "$env:TEMP\agentx-playwright-install-$(Get-Random).log"
                Push-Location $InstallDir
    ...
    if (Test-Path "$tuiDir\package.json") {
        ...
        $tuiLog = "$env:TEMP\agentx-npm-tui-$(Get-Random).log"
...
# the final `electron` -> `AgentX` rename dies with ENOENT and every re-run
...
function Install-Desktop {
    # Build apps/desktop into a launchable AgentX Workmate.exe. Only called from
    ...
    # resolves apps/desktop/release/win-unpacked/AgentX Workmate.exe directly,
    ...
    # enforces the build floor (Node >=26) and prepends the AgentX-managed
    ...
    # The --dir mode produces an unpacked AgentX Workmate.exe in
    ...
    $buildLog = "$env:TEMP\agentx-desktop-build-$(Get-Random).log"
    ...
    $exeCandidates = @(
        "$desktopDir\release\win-unpacked\AgentX Workmate.exe",
        "$desktopDir\release\win-arm64-unpacked\AgentX Workmate.exe"
    )
    $found = $false
    $desktopExe = $null
    ...
    #     SIDs under %LOCALAPPDATA% (electron/electron#51761, agentx-agent#38216).
    ...
    #    AgentX Workmate.exe. We deliberately do NOT point them at `agentx desktop`: that
    ...
    try {
        ...
        foreach ($lnkPath in $targets) {
            try {
                ...
                $sc.Description = 'AgentX Workmate'
                $sc.Save()
        ...
        # with the AgentX icon, but without this the shortcut can keep drawing
...
function Install-PlatformSdks {
    ...
    # ~/.agentx/.env are importable.  Two problems this solves:
    #
...
function Invoke-SetupWizard {
    ...
    if ($NonInteractive) {
        ...
        Write-Info "Skipping setup wizard (non-interactive). Configure via the GUI or 'agentx setup'."
        return
    }
    ...
    # Run agentx setup using the venv Python directly (no activation needed)
    if (-not $NoVenv) {
...
function Start-GatewayIfConfigured {
    ...
    $hermesCmd = "$InstallDir\venv\Scripts\agentx.exe"
    ...
    $whatsappSession = "$AgentXHome\whatsapp\session\creds.json"
    if ($whatsappEnabled -and -not (Test-Path $whatsappSession)) {
        Write-Host ""
        Write-Info "WhatsApp is enabled but not yet paired."
        Write-Info "Running 'agentx whatsapp' to pair via QR code..."
        Write-Host ""
    ...
    if ($NonInteractive) {
        ...
        Write-Info "Start the gateway later with: agentx gateway"
        return
    }
    ...
    if ($response -eq "" -or $response -match "^[Yy]") {
        ...
        try {
            $logFile = "$AgentXHome\logs\gateway.log"
            Start-Process -FilePath $hermesCmd -ArgumentList "gateway" `
                -RedirectStandardOutput $logFile `
                -RedirectStandardError "$AgentXHome\logs\gateway-error.log" `
                -WindowStyle Hidden
            Write-Success "Gateway started! Your bot is now online."
            Write-Info "Logs: $logFile"
            Write-Info "To stop: close the gateway process from Task Manager"
        } catch {
            Write-Warn "Failed to start gateway. Run manually: agentx gateway"
        }
    } else {
        Write-Info "Skipped. Start the gateway later with: agentx gateway"
    }
}
...
function Write-Completion {
    ...
    Write-Host "$AgentXHome\config.yaml"
    Write-Host "   API Keys:  " -NoNewline -ForegroundColor Yellow
    Write-Host "$AgentXHome\.env"
    Write-Host "   Data:      " -NoNewline -ForegroundColor Yellow
    Write-Host "$AgentXHome\cron\, sessions\, logs\"
    Write-Host "   Code:      " -NoNewline -ForegroundColor Yellow
    Write-Host "$AgentXHome\agentx-agent\"
    Write-Host ""
    ...
    Write-Host ""
    Write-Host "   agentx              " -NoNewline -ForegroundColor Green
    Write-Host "Start chatting"
    Write-Host "   agentx setup        " -NoNewline -ForegroundColor Green
    Write-Host "Configure API keys & settings"
    Write-Host "   agentx config       " -NoNewline -ForegroundColor Green
    Write-Host "View/edit configuration"
    Write-Host "   agentx config edit  " -NoNewline -ForegroundColor Green
    Write-Host "Open config in editor"
    Write-Host "   agentx gateway      " -NoNewline -ForegroundColor Green
    Write-Host "Start messaging gateway (Telegram, Discord, etc.)"
    Write-Host "   agentx update       " -NoNewline -ForegroundColor Green
...
$InstallStages = @(
    ...
    @{ Name = "repository";       Title = "Cloning AgentX repository";            Category = "install";      NeedsUserInput = $false; Worker = "Stage-Repository" }
...
if ($IncludeDesktop) {
    ...
    # (AgentX-Setup.exe), never via the irm|iex CLI one-liner.
    $InstallStages += @{ Name = "desktop"; Title = "Building desktop app"; Category = "install"; NeedsUserInput = $false; Worker = "Stage-Desktop" }
}
$InstallStages += @(
    @{ Name = "path";             Title = "Adding AgentX to PATH";                Category = "finalize";     NeedsUserInput = $false; Worker = "Stage-Path" }
