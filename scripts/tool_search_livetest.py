#!/usr/bin/env python3
"""Live test harness for AgentX Workmate's Tool Search feature.
...
# Force-isolate the test environment BEFORE any agentx imports.
ORIGINAL_HOME = os.environ.get("AGENTX_HOME")
ORIGINAL_AUTH = Path.home() / ".agentx" / "auth.json"
...
def setup_isolated_home(enabled: bool, listing: str = "off",
    ...
    """Create a fresh ~/.agentx/ for one test, copying minimal credentials.

    Also reads OPENROUTER_API_KEY from the user's real ``~/.agentx/.env`` so
    ...
    real_env_file = Path.home() / ".agentx" / ".env"
    if real_env_file.exists():
        ...
        load_hermes_dotenv(hermes_home=str(Path.home() / ".agentx"))
...
def reset_module_state():
    """Drop cached modules so the new AGENTX_HOME takes effect."""
...
def main():
    ...
    # Restore original AGENTX_HOME
    if ORIGINAL_HOME is not None:
        os.environ["AGENTX_HOME"] = ORIGINAL_HOME
    else:
