...
- **Two arms, one variable.** `baseline` and `fixes` runs differ ONLY by
  ...
  AgentX home, same model, same tasks, same reps.
...
1. Create a dedicated AgentX home with credentials for the models under test:
   ...
   AGENTX_HOME=$ABEVAL_HOME agentx plugins enable observability/nemo_relay
   ```
