"""
AgentX MCP Server — expose messaging conversations as MCP tools.
...
Plus: channels_list (AgentX-specific extra)

Usage:
    agentx mcp serve
    agentx mcp serve --verbose
...
def _get_sessions_dir() -> Path:
    """Return the sessions directory using AGENTX_HOME."""
    try:
        from hermes_constants import get_hermes_home
        return get_hermes_home() / "sessions"
    except ImportError:
        return Path(os.environ.get("AGENTX_HOME", Path.home() / ".agentx")) / "sessions"
...
def _load_channel_directory() -> dict:
    ...
    try:
        ...
        directory_file = Path(
            os.environ.get("AGENTX_HOME", Path.home() / ".agentx")
...
class EventBridge:
    ...
    This is the AgentX equivalent of OpenClaw's WebSocket gateway bridge.
    ...
    def _establish_baseline(self) -> None:
        ...
        try:
            ...
            db_file = Path(os.environ.get("AGENTX_HOME", Path.home() / ".agentx")) / "state.db"
        try:
...
def create_mcp_server(event_bridge: Optional[EventBridge] = None) -> "FastMCP":
    """Create and return the AgentX MCP server with all tools registered."""
    ...
    mcp = FastMCP(
        "agentx",
        instructions=(
            "AgentX Workmate messaging bridge. Use these tools to interact with "
...
def run_mcp_server(verbose: bool = False) -> None:
    """Start the AgentX MCP server on stdio."""
    if not _MCP_SERVER_AVAILABLE:
        print(
