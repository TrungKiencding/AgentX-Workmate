...
Plugin 元数据。`requires_env` 和 `optional_env` 块会自动填充 `agentx config` UI 条目（参见下方[在 agentx config 中暴露环境变量](#surfacing-env-vars-in-agentx-config)）。
...
## 在 `agentx config` 中暴露环境变量 {#surfacing-env-vars-in-agentx-config}
