...
这将进入设置向导，向导会提示你输入 API 密钥并将其写入 `~/.agentx/.env`。你只需执行一次。强烈建议此时为 gateway 配置一个聊天系统。
...
内置 Web dashboard 在同一容器内作为受 s6-rc 监管的服务与 gateway 并行运行。设置 `AGENTX_DASHBOARD=1` 即可拉起它：
...
| 环境变量 | 描述 | 默认值 |
|---------------------|-------------|---------|
| `AGENTX_DASHBOARD` | 设为 `1`（或 `true` / `yes`）以启用受监管的 dashboard 服务 | *（未设置——服务已注册但保持关闭）* |
| `AGENTX_DASHBOARD_HOST` | dashboard HTTP 服务器的绑定地址 | `0.0.0.0` |
| `AGENTX_DASHBOARD_PORT` | dashboard HTTP 服务器的端口 | `9119` |
| `AGENTX_DASHBOARD_INSECURE` | **已弃用 / 空操作。** 以前用于绕过鉴权门控；自 2026 年 6 月的安全加固起，它不再禁用鉴权。任何非回环绑定都必须配置鉴权提供方 | *（被忽略——请改为配置提供方）* |

容器内的 dashboard 默认绑定 `0.0.0.0`，否则发布的 `-p 9119:9119` 端口将无法从宿主机访问。若你要把它限制在容器回环地址（例如 sidecar / 反向代理拓扑），请显式设置 `AGENTX_DASHBOARD_HOST=127.0.0.1`。
...
- **用户名/密码** —— 最简单的自托管 / 局域网 / VPN 内部署方式：设置 `AGENTX_DASHBOARD_BASIC_AUTH_USERNAME` + `AGENTX_DASHBOARD_BASIC_AUTH_PASSWORD`（以及用于跨重启稳定 session 的 `AGENTX_DASHBOARD_BASIC_AUTH_SECRET`）。不适合直接暴露到公网上。
- **OAuth（Nous Portal）** —— 适合托管/公网部署：设置 `AGENTX_DASHBOARD_OAUTH_CLIENT_ID` 后，`dashboard_auth/nous` 提供者会自动激活。
- **自托管 OIDC** —— 通过标准 OpenID Connect 接入你自己的身份提供商：设置 `AGENTX_DASHBOARD_OIDC_ISSUER` + `AGENTX_DASHBOARD_OIDC_CLIENT_ID` 后，`dashboard_auth/self_hosted` 提供者会激活。
...
`AGENTX_DASHBOARD_INSECURE=1` 现在是一个已弃用的空操作（它会
打印告警并被忽略）。请改为配置鉴权提供方，或设置
`AGENTX_DASHBOARD_HOST=127.0.0.1` 并通过 SSH 隧道 / Tailscale 访问。

:::warning 为什么移除了 `--insecure`
无鉴权的公网 dashboard 是 2026 年 6 月 MCP 配置持久化攻击活动的入口：互联网扫描器访问到暴露的 dashboard（以及 OpenAI API 服务器），诱导 agent 植入 SSH 密钥后门。现在每个非回环绑定都强制启用鉴权门控。对于可信局域网 / homelab 主机，内置的用户名/密码提供方（`AGENTX_DASHBOARD_BASIC_AUTH_USERNAME` + `_PASSWORD`）是满足该要求的零基础设施方式。
:::
...
- 每个 profile 独立的轮转日志：`${AGENTX_HOME}/logs/gateways/<name>/current`。
...
services:
  hermes:
    ...
    ports:
      ...
      - "9119:9119"   # dashboard（仅在 AGENTX_DASHBOARD=1 时生效）
    volumes:
      - ~/.agentx:/opt/data
    environment:
      - AGENTX_DASHBOARD=1
...
- Dashboard 崩溃后自动重启（设置 `AGENTX_DASHBOARD=1` 以启动）。
...
当工具必须在每次容器启动时立即可用且无需重新安装延迟时，构建一个继承自 `trungkiencding/agentx-workmate` 并在层中安装该工具的新镜像：
...
入口点脚本和 `/opt/data` 语义原样继承，本页其余内容仍然适用。拉取更新的上游 `trungkiencding/agentx-workmate` 时记得重新构建镜像。
...
如果某个工具可能对大多数 AgentX Workmate 用户有用，考虑将其贡献到上游，而不是在私有派生镜像中维护。在 [agentx-agent 仓库](https://github.com/TrungKiencding/AgentX-Workmate)提交 issue 或 pull request，描述该工具及其使用场景。被纳入官方镜像的工具惠及所有用户，并避免了维护下游 fork 的开销。
...
然后在 `~/.agentx/config.yaml` 中，使用**容器名称**作为主机名：
...
docker run -it --rm trungkiencding/agentx-workmate:latest version     # 验证版本
