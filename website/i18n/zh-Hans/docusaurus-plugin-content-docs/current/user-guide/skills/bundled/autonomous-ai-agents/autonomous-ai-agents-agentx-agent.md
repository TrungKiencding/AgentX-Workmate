...
AgentX Workmate 是 AstralX Technology 开发的开源 AI agent 框架，可在终端、消息平台和 IDE 中运行。它与 Claude Code（Anthropic）、Codex（OpenAI）和 OpenClaw 同属一类——使用工具调用（tool calling）与系统交互的自主编码和任务执行 agent。AgentX 支持任意 LLM 提供商（OpenRouter、Anthropic、OpenAI、DeepSeek、本地模型及 15+ 其他提供商），可在 Linux、macOS 和 WSL 上运行。
...
在交互式聊天会话中输入这些命令。新命令会不定期上线；如果以下内容看起来过时，请在会话内运行 `/help` 获取权威列表，或查看[实时斜杠命令参考](/reference/slash-commands)。命令注册表的权威来源是 `hermes_cli/commands.py` — 每个消费方（自动补全、Telegram 菜单、Slack 映射、`/help`）均从中派生。
...
完整配置参考：/user-guide/configuration
...
完整提供商文档：/integrations/providers
...
用户文档：/user-guide/features/curator
...
用户文档：/user-guide/features/kanban
...
| 查找内容... | 位置 |
|----------------|----------|
| 配置选项 | `agentx config edit` 或[配置文档](/user-guide/configuration) |
| 可用工具 | `agentx tools list` 或[工具参考](/reference/tools-reference) |
| 斜杠命令 | 会话内 `/help` 或[斜杠命令参考](/reference/slash-commands) |
| Skill 目录 | `agentx skills browse` 或[Skill 目录](/reference/skills-catalog) |
| 提供商设置 | `agentx model` 或[提供商指南](/integrations/providers) |
| 平台设置 | `agentx gateway setup` 或[消息文档](/user-guide/messaging/) |
| MCP 服务器 | `agentx mcp list` 或[MCP 指南](/user-guide/features/mcp) |
| Profiles | `agentx profile list` 或[Profiles 文档](/user-guide/profiles) |
| Cron 任务 | `agentx cron list` 或[Cron 文档](/user-guide/features/cron) |
| 记忆 | `agentx memory status` 或[记忆文档](/user-guide/features/memory) |
| 环境变量 | `agentx config env-path` 或[环境变量参考](/reference/environment-variables) |
| CLI 命令 | `agentx --help` 或[CLI 参考](/reference/cli-commands) |
...
面向偶尔贡献者和 PR 作者。完整开发者文档：/developer-guide/
