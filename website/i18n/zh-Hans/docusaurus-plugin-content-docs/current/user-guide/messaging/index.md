...
sidebar_position: 1
title: "消息网关"
description: "通过 Telegram、Discord、Slack、WhatsApp、Signal、SMS、Email、Home Assistant、Mattermost、Matrix、DingTalk、Yuanbao、Microsoft Teams、LINE、Webhooks 或任何兼容 OpenAI 的前端与 AgentX 对话 — 架构与配置概览"
---

# 消息网关

通过 Telegram、Discord、Slack、WhatsApp、Signal、SMS、Email、Home Assistant、Mattermost、Matrix、DingTalk、Feishu/Lark、WeCom、Weixin、BlueBubbles（iMessage）、QQ、Yuanbao、Microsoft Teams、LINE、ntfy 或浏览器与 AgentX 对话。网关是一个单一后台进程，连接所有已配置的平台，管理会话，运行 cron 任务，并传递语音消息。

完整的语音功能集——包括 CLI 麦克风模式、消息中的语音回复以及 Discord 语音频道对话——请参阅 [Voice Mode](/user-guide/features/voice-mode) 和 [Use Voice Mode with AgentX](/guides/use-voice-mode-with-agentx)。
...
```bash
agentx gateway setup        # 交互式配置所有消息平台
```
...
```bash
agentx gateway              # 在前台运行
agentx gateway setup        # 交互式配置消息平台
agentx gateway install      # 安装为用户服务（Linux）/ launchd 服务（macOS）
sudo agentx gateway install --system   # 仅 Linux：安装开机启动的系统服务
agentx gateway start        # 启动默认服务
agentx gateway stop         # 停止默认服务
agentx gateway status       # 检查默认服务状态
agentx gateway status --system         # 仅 Linux：显式检查系统服务
```
...
| `/update` | 将 AgentX Workmate 更新至最新版本 |
| `/help` | 显示可用命令 |
...
# 其他配对命令：
agentx pairing list          # 查看待审核和已批准的用户
agentx pairing revoke telegram 123456789  # 撤销访问权限
```
...
第一次在任意平台向繁忙的 agent 发送消息时，AgentX 会在繁忙确认中附加一行提示，说明该配置项（`"💡 First-time tip — …"`）。该提示每次安装只触发一次——由 `onboarding.seen.busy_input_prompt` 下的标志锁定。删除该键可再次看到提示。
...
agentx gateway start                 # 启动服务
agentx gateway stop                  # 停止服务
agentx gateway status                # 检查状态
journalctl --user -u agentx-gateway -f  # 查看日志
...
除非你确实有此需要，否则避免同时安装用户和系统网关单元。AgentX 检测到两者同时存在时会发出警告，因为 start/stop/status 行为会变得不明确。

:::info 多个安装
如果你在同一台机器上运行多个 AgentX 安装（使用不同的 `AGENTX_HOME` 目录），每个安装都有自己的 systemd 服务名称。默认的 `~/.agentx` 使用 `agentx-gateway`；其他安装使用 `agentx-gateway-<hash>`。`agentx gateway` 命令会自动针对当前 `AGENTX_HOME` 对应的正确服务。
:::

### macOS（launchd）

```bash
agentx gateway install               # 安装为 launchd agent
agentx gateway start                 # 启动服务
agentx gateway stop                  # 停止服务
agentx gateway status                # 检查状态
...
生成的 plist 文件位于 `~/Library/LaunchAgents/ai.agentx.gateway.plist`。它包含三个环境变量：
...
- **AGENTX_HOME** — 将网关限定到你的 AgentX 安装。

:::tip 安装后 PATH 变更
launchd plist 是静态的——如果你在配置网关后安装了新工具（例如通过 nvm 安装新版 Node.js，或通过 Homebrew 安装 ffmpeg），请重新运行 `agentx gateway install` 以捕获更新后的 PATH。网关会检测到过时的 plist 并自动重新加载。
:::

:::info 多个安装
与 Linux systemd 服务类似，每个 `AGENTX_HOME` 目录都有自己的 launchd 标签。默认的 `~/.agentx` 使用 `ai.agentx.gateway`；其他安装使用 `ai.agentx.gateway-<suffix>`。
:::
...
|----------|---------|--------------|
| CLI | `agentx-cli` | 完全访问 |
| Telegram | `agentx-telegram` | 完整工具，包括终端 |
| Discord | `agentx-discord` | 完整工具，包括终端 |
| WhatsApp | `agentx-whatsapp` | 完整工具，包括终端 |
| Slack | `agentx-slack` | 完整工具，包括终端 |
| Google Chat | `agentx-google_chat` | 完整工具，包括终端 |
| Signal | `agentx-signal` | 完整工具，包括终端 |
| SMS | `agentx-sms` | 完整工具，包括终端 |
| Email | `agentx-email` | 完整工具，包括终端 |
| Home Assistant | `agentx-homeassistant` | 完整工具 + HA 设备控制（ha_list_entities、ha_get_state、ha_call_service、ha_list_services） |
| Mattermost | `agentx-mattermost` | 完整工具，包括终端 |
| Matrix | `agentx-matrix` | 完整工具，包括终端 |
| DingTalk | `agentx-dingtalk` | 完整工具，包括终端 |
| Feishu/Lark | `agentx-feishu` | 完整工具，包括终端 |
| WeCom | `agentx-wecom` | 完整工具，包括终端 |
| WeCom Callback | `agentx-wecom-callback` | 完整工具，包括终端 |
| Weixin | `agentx-weixin` | 完整工具，包括终端 |
| BlueBubbles | `agentx-bluebubbles` | 完整工具，包括终端 |
| QQBot | `agentx-qqbot` | 完整工具，包括终端 |
| Yuanbao | `agentx-yuanbao` | 完整工具，包括终端 |
| Microsoft Teams | `agentx-teams` | 完整工具，包括终端 |
| API Server | `agentx-api-server` | 完整工具（去除 `clarify`、`send_message`、`text_to_speech`——程序化访问没有交互用户） |
| Webhooks | `agentx-webhook` | 完整工具，包括终端 |
