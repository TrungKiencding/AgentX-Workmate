...
使用 `hermes model` 设置提供商，或直接编辑 `~/.agentx/.env`。所有提供商 key 请参阅[环境变量](./environment-variables.md)参考文档。
...
API 调用**仅发送至您配置的 LLM 提供商**（例如 OpenRouter、您本地的 Ollama 实例）。Hermes Agent 不收集遥测数据、使用数据或分析数据。您的对话、记忆和技能均存储在本地 `~/.agentx/` 目录中。
...
:::tip 本地模型超时问题
Hermes 会自动检测本地端点并放宽流式传输超时（读取超时从 120s 提升至 1800s，禁用停滞流检测）。如果在非常大的上下文下仍然超时，请在 `.env` 中设置 `AGENTX_STREAM_READ_TIMEOUT=1800`。详情请参阅[本地 LLM 指南](../guides/local-llm-on-mac.md#timeouts)。
:::
...
**解决方案：** Hermes 默认自动 source `~/.bashrc`。如果这还不够 — 例如您是 zsh 用户，PATH 在 `~/.zshrc` 中，或者您从独立文件初始化 `nvm` — 请在 `~/.agentx/config.yaml` 中列出需要额外 source 的文件：
...
```bash
# 安装核心消息网关依赖项
cd ~/.agentx/hermes-agent && uv pip install -e ".[messaging]"  # Telegram、Discord、Slack 及共享网关依赖
...
Profiles 是构建在 `AGENTX_HOME` 之上的托管层。您*可以*在每次命令前手动设置 `AGENTX_HOME=/some/path`，但 profiles 会为您处理所有底层工作：创建目录结构、生成 shell 别名（`hermes-work`）、在 `~/.agentx/active_profile` 中跟踪活动 profile，以及自动跨所有 profiles 同步技能更新。它们还与 tab 补全集成，让您无需记忆路径。
...
**解决方案：委托配置。** Hermes 可以自动将子智能体路由到不同的模型。在 `~/.agentx/config.yaml` 中设置：
...
2. 在**源机器**上创建完整备份：
   ...
   hermes backup
   ```
   这会将您整个 `~/.agentx/` 目录（配置、API key、记忆、技能、会话和 profiles）打包为 zip 文件，保存到主目录 `~/hermes-backup-<timestamp>.zip`。
