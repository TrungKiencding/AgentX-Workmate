...
sidebar_position: 1
title: "Quickstart"
description: "Your first conversation with AgentX Workmate — from install to chatting in under 5 minutes"
---

# Quickstart

This guide gets you from zero to a working AgentX setup that survives real use. Install, choose a provider, verify a working chat, and know exactly what to do when something breaks.

## Prefer to watch?

**Onchain AI Garage** put together a Masterclass walkthrough of installation, setup, and basic commands — a good companion to this page if you'd rather follow along on video. For more, see the full [AgentX Workmate Tutorials & Use Cases](https://www.youtube.com/playlist?list=PLmpUb_PWAkDxewld5ZYyKifuHxgIbiq2d) playlist.

<div style={{position: 'relative', paddingBottom: '56.25%', height: 0, overflow: 'hidden', maxWidth: '100%', marginBottom: '1.5rem'}}>
  <iframe
    style={{position: 'absolute', top: 0, left: 0, width: '100%', height: '100%'}}
    src="https://www.youtube-nocookie.com/embed/R3YOGfTBcQg"
    title="AgentX Workmate Masterclass: Installation, Setup, Basic Commands"
    frameBorder="0"
...
- Setting up AgentX for a team, bot, or always-on workflow
...
|---|---|---|
| I just want AgentX working on my machine | `agentx setup` | Run a real chat and verify it responds |
| I already know my provider | `agentx model` | Save the config, then start chatting |
| I want a bot or always-on setup | `agentx gateway setup` after CLI works | Connect Telegram, Discord, Slack, or another platform |
| I want a local or self-hosted model | `agentx model` → custom endpoint | Verify the endpoint, model name, and context length |
| I want multi-provider fallback | `agentx model` first | Add routing and fallback only after the base chat works |

**Rule of thumb:** if AgentX cannot complete a normal chat, do not add more features yet. Get one clean conversation working first, then layer on gateway, cron, skills, voice, or routing.

---

## 1. Install AgentX Workmate
### With the AgentX Workmate Desktop installer on macOS or Windows (recommended)
To easily install the command-line and desktop applications, [download the AgentX Workmate Desktop installer](https://github.com/TrungKiencding/AgentX-Workmate) from our website and run it.

### Without AgentX Workmate Desktop:
For a command-line only install without AgentX Workmate Desktop, run:
...
The single most important setup step. Use `agentx model` to walk through the choice interactively:
...
:::info Setup modes
On a fresh install, `agentx setup` offers three modes:
...
Blank Slate writes an explicit `platform_toolsets.cli` list plus `agent.disabled_toolsets`, so nothing you didn't choose ever loads — not even after `agentx update`. Re-enable anything later with `agentx tools`, seed skills with `agentx skills opt-in --sync`, or tune settings with `agentx setup agent`.
:::
...
|----------|-----------|---------------|
| **Nous Portal** | Subscription-based, zero-config | OAuth login via `agentx model` |
| **OpenAI Codex** | ChatGPT OAuth, uses Codex models | Device code auth via `agentx model` |
| **Anthropic** | Claude models directly — Max plan + extra usage credits (OAuth), or API key for pay-per-token | `agentx model` → OAuth login (requires Max + extra credits), or an Anthropic API key |
...
| **MiniMax (OAuth)** | MiniMax frontier model via browser OAuth — no API key needed (model name in `hermes_cli/models.py` may change between releases) | `agentx model` → MiniMax (OAuth) |
...
| **xAI Grok OAuth** | SuperGrok / Premium+ subscription, no API key needed | `agentx model` → xAI Grok OAuth |
...
| **Qwen OAuth** | Qwen Portal browser OAuth — no API key needed | `agentx model` → Qwen OAuth |
...
| **GitHub Copilot** | GitHub Copilot subscription (GPT-5.x, Claude, Gemini, etc.) | OAuth via `agentx model`, or `COPILOT_GITHUB_TOKEN` / `GH_TOKEN` |
| **GitHub Copilot ACP** | Copilot ACP agent backend (spawns local `copilot` CLI) | `agentx model` (requires `copilot` CLI + `copilot login`) |
...
AgentX Workmate requires a model with at least **64,000 tokens** of context. Models with smaller windows cannot maintain enough working memory for multi-step tool-calling workflows and will be rejected at startup. Most hosted models (Claude, GPT, Gemini, Qwen, DeepSeek) meet this easily. If you're running a local model, set its context size to at least 64K (e.g. `--ctx-size 65536` for llama.cpp or `-c 65536` for Ollama).
:::

:::tip
You can switch providers at any time with `agentx model` — no lock-in. For a full list of all supported providers and setup details, see [AI Providers](../integrations/providers.md).
:::

### How settings are stored

AgentX separates secrets from normal config:

- **Secrets and tokens** → `~/.agentx/.env`
- **Non-secret settings** → `~/.agentx/config.yaml`

The easiest way to set values correctly is through the CLI:

```bash
agentx config set model anthropic/claude-opus-4.6
agentx config set terminal.backend docker
agentx config set OPENROUTER_API_KEY sk-or-...
```
...
```bash
agentx            # classic CLI
agentx --tui      # modern TUI (recommended)
```

You'll see a welcome banner with your model, available tools, and skills. Use a prompt that's specific and easy to verify:

:::tip Pick your interface
AgentX ships with two terminal interfaces: the classic `prompt_toolkit` CLI and a newer [TUI](../user-guide/tui.md) with modal overlays, mouse selection, and non-blocking input. Both share the same sessions, slash commands, and config — try each with `agentx` vs `agentx --tui`.
:::
...
```bash
agentx --continue    # Resume the most recent session
agentx -c            # Short form
```
...
```bash
agentx gateway setup    # Interactive platform configuration
```
...
- `agentx tools` — tune tool access per platform
- `agentx skills` — browse and install reusable workflows
...
```bash
agentx config set terminal.backend docker    # Docker isolation
agentx config set terminal.backend ssh       # Remote server
```

For Docker sandboxes, you can also enable the **egress credential-injection proxy** so the sandbox never sees your real API keys — only opaque proxy tokens that work exclusively from behind a local TLS-intercepting daemon. See [Egress proxy](../user-guide/egress/iron-proxy.md). Setup is `agentx egress setup && agentx egress start`; `agentx setup terminal` also points Docker users at it. Modal, SSH, Daytona, and Singularity are not wired yet.

### Voice mode

```bash
# From the AgentX install directory (the curl installer placed it at
# ~/.agentx/agentx-agent on Linux/macOS or %LOCALAPPDATA%\agentx\agentx-agent on Windows):
cd ~/.agentx/agentx-agent
...
Skills are on-demand instruction documents that teach AgentX how to do a specific task — deploy to Kubernetes, open a GitHub PR, fine-tune a model, search for GIFs. Each is a `SKILL.md` file with a name, a description, and a step-by-step procedure. The agent reads the short descriptions for free and only loads a skill's full content when a task actually calls for it, so adding skills doesn't bloat every request.

AgentX ships with a catalog of bundled skills already installed in `~/.agentx/skills/`. You can add more from the Skills Hub, or write your own.

**Browse and install from the hub:**

```bash
agentx skills browse                      # list everything available
agentx skills search kubernetes           # find skills by keyword
agentx skills install openai/skills/k8s   # install one (runs a security scan first)
```

The install argument is a `source/path` slug from the hub — `openai/skills/k8s` means the `k8s` skill from OpenAI's catalog. `agentx skills browse` shows the exact slugs to use.
...
/k8s                                       # load it and let AgentX ask what you need
```
...
(If you installed without `[all]`, run `cd ~/.agentx/agentx-agent && uv pip install -e ".[acp]"` first.)
...
|---|---|---|
| AgentX opens but gives empty or broken replies | Provider auth or model selection is wrong | Run `agentx model` again and confirm provider, model, and auth |
...
| Gateway starts but nobody can message it | Bot token, allowlist, or platform setup is incomplete | Re-run `agentx gateway setup` and check `agentx gateway status` |
| `agentx --continue` can't find old session | Switched profiles or session never saved | Check `agentx sessions list` and confirm you're in the right profile |
...
| `agentx doctor` flags config problems | Config values are missing or stale | Fix the config, retest a plain chat before adding features |
...
2. `agentx model`
3. `agentx setup`
4. `agentx sessions list`
5. `agentx --continue`
6. `agentx gateway status`
...
| `agentx tools` | Configure which tools are enabled per platform |
| `agentx setup` | Full setup wizard (configures everything at once) |
| `agentx doctor` | Diagnose issues |
| `agentx update` | Update to latest version |
| `agentx gateway` | Start the messaging gateway |
| `agentx --continue` | Resume last session |
