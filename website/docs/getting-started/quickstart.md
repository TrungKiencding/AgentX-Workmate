...
# ~/.agentx/hermes-agent on Linux/macOS or %LOCALAPPDATA%\agentx\hermes-agent on Windows):
...
Hermes ships with a catalog of bundled skills already installed in `~/.agentx/skills/`. You can add more from the Skills Hub, or write your own.
...
(If you installed without `[all]`, run `cd ~/.agentx/hermes-agent && uv pip install -e ".[acp]"` first.)
