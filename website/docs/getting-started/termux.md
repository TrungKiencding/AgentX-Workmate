...
description: "Run AgentX Workmate directly on an Android phone with Termux"
---

# AgentX on Android with Termux
...
AgentX Workmate can run directly on an Android phone through [Termux](https://termux.dev/).
...
That does not stop AgentX from working well as a phone-native CLI agent — it just means the recommended mobile install is intentionally narrower than the desktop/server install.
...
- links `agentx` into `$PREFIX/bin` so it stays on your Termux PATH
...
### 5. Put `agentx` on your Termux PATH

```bash
ln -sf "$PWD/venv/bin/agentx" "$PREFIX/bin/agentx"
```

`$PREFIX/bin` is already on PATH in Termux, so this makes the `agentx` command persist across new shells without re-activating the venv every time.
...
Or set keys directly in `~/.agentx/.env`.
...
### `agentx doctor` says ripgrep or Node is missing
