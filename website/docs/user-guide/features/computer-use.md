...
AgentX Workmate can drive your desktop — clicking, typing, scrolling,
...
```
agentx computer-use install
```

This fetches and runs the upstream cua-driver installer — `install.sh`
on macOS/Linux, `install.ps1` on Windows. Use `agentx computer-use
status` to verify the install.

**Option 2: enable the toolset interactively.**

1. Run `agentx tools`, pick `🖱️  Computer Use (macOS/Windows/Linux)`.
...
| Platform | Prereqs |
|---|---|
| **macOS** | System Settings → Privacy & Security → **Accessibility** + **Screen Recording** → allow your terminal (or AgentX app). `agentx computer-use doctor` will tell you which permission is missing. |
...
AgentX maps its existing approval UX onto cua-driver 0.10's immutable daemon
modes. There is no second permission toggle to keep in sync:

| AgentX session | cua-driver mode | Human intervention | `existing_profile` |
|---|---|---|---|
| Manual or smart approvals (default) | `standard` | Normal AgentX approvals; Cua stops at its protected boundary | Refuses unless a certified protected host is available; AgentX does not claim one today |
| `--yolo`, `/yolo`, or `approvals.mode: off` | private `unrestricted` daemon | One explicit AgentX risk acceptance; no runtime Cua prompts | Allowed within Cua's built-in, managed, and user policy ceilings |

The unrestricted daemon is private to that AgentX session. Turning `/yolo`
...
daemon's mode or grants another AgentX conversation the same authority.
...
## `agentx computer-use doctor` — your first triage stop

`agentx computer-use doctor` runs cua-driver's structured
...
agent is acting here." Each AgentX run declares its own cua-driver
...
AgentX intentionally keeps its skill (`skills/autonomous-ai-agents/computer-use/SKILL.md`)
focused on the AgentX-side `computer_use` action vocabulary — the
...
These are **platform deep dives, not duplicates of the AgentX skill** —
...
Code, Codex, OpenCode, OpenClaw, and Antigravity; **AgentX
...
AgentX applies multi-layer guardrails:
...
Screenshots are expensive. AgentX applies four layers of optimisation:
...
- **Windows: elevated (admin) windows can't be driven from a normal
  ...
  AgentX agent) cannot enumerate the UIA tree of, or inject mouse input
  ...
  run the AgentX agent itself at High integrity (launch from an
...
- **Platform-specific deployment gotchas:**
  - **macOS** uses private SkyLight SPIs. Apple can change them in any
    OS update. AgentX warns when the installed cua-driver is older than
    the version it was tested against.
  - **Windows** SSH sessions run in **Session 0**, which has no
    interactive desktop. Drive AgentX from inside the RDP / console
...
upstream. **AgentX disables it for you** — on every cua-driver invocation
(the MCP backend, `status`, `doctor`, and install) AgentX sets
...
When it's on, `agentx computer-use doctor` reports `telemetry: enabled`;
...
unreleased fix — point AgentX at a binary you built from source instead
of the published release. AgentX resolves the driver with
...
- `-NoAutoStart` skips registering the `cua-driver-serve` logon daemon
  — you don't need it for AgentX testing (see notes).
...
### Option B — point AgentX straight at the built binary (fastest loop)
...
### Confirm AgentX is using your build

- `agentx computer-use status` prints the resolved binary path and
  version.
- `agentx computer-use doctor` confirms the binary is reachable and
...
- **AgentX spawns a `cua-driver mcp` stdio proxy.** In a normal session the
  proxy connects to (and may start) the standard machine daemon. In explicit
  AgentX YOLO, AgentX instead owns a private `cua-driver serve --embedded`
...
- **Rebuild loop.** After editing cua-driver source, re-run
  ...
  for Option A, or just re-`cargo build` for Option B — no AgentX
  change needed either way.
- **Local builds skip the version check.** AgentX warns when the
...
**First action when anything's off: run `agentx computer-use doctor`.**
...
Run `agentx computer-use install` to fetch the cua-driver binary, or
run `agentx tools` and enable the Computer Use toolset.
...
Wayland without an XWayland bridge. `agentx computer-use doctor` will
...
- **AgentX-side skill** — `skills/autonomous-ai-agents/computer-use/SKILL.md` — teaches the
  AgentX `computer_use` action vocabulary; this is what the agent loads.
- **cua-driver skill pack** — for platform-specific deep dives
  ...
  install` autodetects AgentX (planned follow-up), this happens
...
- **cua.ai/docs** — the cua-driver project's documentation:
  ...
  - [Connect your agent](https://cua.ai/docs/how-to-guides/driver/connect-your-agent) — register cua-driver with various harnesses (AgentX among them)
