...
description: "Customize AgentX Workmate's personality with a global SOUL.md, built-in personalities, and custom persona definitions"
---

# Personality & SOUL.md

AgentX Workmate's personality is fully customizable. `SOUL.md` is the **primary identity** — it's the first thing in the system prompt and defines who the agent is.

- `SOUL.md` — a durable persona file that lives in `AGENTX_HOME` and serves as the agent's identity (slot #1 in the system prompt)
- built-in or custom `/personality` presets — session-level system-prompt overlays

If you want to change who AgentX is — or replace it with an entirely different agent persona — edit `SOUL.md`.

## How SOUL.md works now

AgentX now seeds a default `SOUL.md` automatically in:

```text
~/.agentx/SOUL.md
```

More precisely, it uses the current instance's `AGENTX_HOME`, so if you run AgentX with a custom home directory, it will use:
...
- AgentX creates a starter `SOUL.md` automatically if one does not exist yet
- Existing user `SOUL.md` files are never overwritten
- AgentX loads `SOUL.md` only from `AGENTX_HOME`
- AgentX does not look in the current working directory for `SOUL.md`
- If `SOUL.md` exists but is empty, or cannot be loaded, AgentX falls back to a built-in default identity
...
If AgentX loaded `SOUL.md` from whatever directory you happened to launch it in, your personality could change unexpectedly between projects. By loading only from `AGENTX_HOME`, the personality belongs to the AgentX instance itself.

That also makes it easier to teach users:
- "Edit `~/.agentx/SOUL.md` to change AgentX' default personality."
...
- how AgentX should handle uncertainty, disagreement, or ambiguity
...
## What AgentX injects into the prompt
...
If the file is empty, whitespace-only, or cannot be read, AgentX falls back to a built-in default identity ("You are AgentX Workmate, an intelligent AI assistant created by AstralX Technology..."). This fallback also applies when `skip_context_files` is set (e.g., in subagent/delegation contexts).
...
AgentX ships with built-in personalities you can switch to with `/personality`.
...
| **pirate** | Captain AgentX, tech-savvy buccaneer |
...
These are convenient overlays, but your global `SOUL.md` still gives AgentX its persistent default personality unless the overlay meaningfully changes it.

## Custom personalities in config

You can also define named custom personalities in `~/.agentx/config.yaml` under `agent.personalities`.
...
1. Keep a thoughtful global `SOUL.md` in `~/.agentx/SOUL.md`
...
- [SOUL.md Guide](/guides/use-soul-with-agentx)

## CLI appearance vs conversational personality

Conversational personality and CLI appearance are separate:

- `SOUL.md`, `agent.system_prompt`, and `/personality` affect how AgentX speaks
- `display.skin` and `/skin` affect how AgentX looks in the terminal
