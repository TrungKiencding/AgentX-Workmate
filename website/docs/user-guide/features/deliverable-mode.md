...
When AgentX Workmate runs inside a messaging gateway (Slack, Discord, Telegram,
...
persona in `~/.agentx/SOUL.md`, or as a named preset under
`agent.personalities` in `~/.agentx/config.yaml` (switchable per session
via `/personality`).
...
If you use AgentX' kanban multi-agent workflow, workers can attach
...
Install MCP servers via `~/.agentx/config.yaml` under the `mcp_servers`
...
back into the thread as a native attachment. AgentX Workmate's deliverable
