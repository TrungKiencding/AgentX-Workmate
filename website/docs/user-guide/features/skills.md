...
All skills live in **`~/.agentx/skills/`** — the primary directory and source of truth. On fresh install, bundled skills are copied from the repo. Hub-installed and agent-created skills also go here. The agent can modify or delete any skill.

You can also point AgentX at **external skill directories** — additional folders scanned alongside the local one. See [External Skill Directories](#external-skill-directories) below.
...
By default every profile is seeded with the bundled skill catalog, and each `agentx update` adds any newly bundled skills. If you want a profile with **no bundled skills** — and that stays empty across updates — you have two paths:
...
```bash
agentx profile create research --no-skills
```

**On an already-installed profile** (default or named), toggle it at runtime:

```bash
agentx skills opt-out            # stop future seeding — nothing on disk is touched
agentx skills opt-out --remove   # also delete UNMODIFIED bundled skills (confirms first)
agentx skills opt-in --sync      # undo: remove the marker and re-seed now
```

All three paths write a `.no-bundled-skills` marker into the profile directory. While the marker is present, the installer, `agentx update`, and any skill sync all skip bundled-skill seeding for that profile. Delete the marker (or run `agentx skills opt-in`) to re-enable.

:::note Safe by default
`agentx skills opt-out` only stops *future* seeding — it never deletes anything already on disk. The optional `--remove` flag deletes bundled skills **only** when they are unmodified (byte-identical to the version AgentX installed). Skills you have edited, skills installed from the hub, and skills you wrote yourself are always kept.
:::
...
The bundled `plan` skill is a good example. Running `/plan [request]` loads the skill's instructions, telling AgentX to inspect context if needed, write a markdown implementation plan instead of executing the task, and save the result under `.agentx/plans/` relative to the active workspace/backend working directory.

You can also interact with skills through natural conversation:

```bash
agentx chat --toolsets skills -q "What skills do you have?"
agentx chat --toolsets skills -q "Show me the axolotl skill"
```
...
description, the standard section order, AgentX-tool framing, no invented
commands).
...
When a missing value is encountered, AgentX asks for it securely only when the skill is actually loaded in the local CLI. You can skip setup and keep using the skill. Messaging surfaces never ask for secrets in chat — they tell you to use `agentx setup` or `~/.agentx/.env` locally instead.
...
Settings are stored under `skills.config` in your config.yaml. `agentx config migrate` prompts for unconfigured settings, and `agentx config show` displays them. When a skill loads, its resolved config values are injected into the context so the agent knows the configured values automatically.
...
and `examples/`. Unreferenced repository files are not copied. AgentX scans the
...
If you maintain skills outside of AgentX — for example, a shared `~/.agents/skills/` directory used by multiple AI tools — you can tell AgentX to scan those directories too.

Add `external_dirs` under the `skills` section in `~/.agentx/config.yaml`:
...
- **Create locally, update in place**: New agent-created skills are written to `~/.agentx/skills/`. Existing skills are modified where they are found, including skills under `external_dirs`, when the agent uses `skill_manage` actions such as `patch`, `edit`, `write_file`, `remove_file`, or `delete`.
- **External dirs are not a write-protection boundary**: If an external skill directory is writable by the AgentX process, agent-managed skill updates can change files in that directory. Use filesystem permissions or a separate profile/toolset setup if shared external skills must stay read-only.
- **Local precedence**: If the same skill name exists in both the local dir and an external dir, the local version wins.
- **Full integration**: External skills appear in the system prompt index, `skills_list`, `skill_view`, and as `/skill-name` slash commands — no different from local skills.
- **Non-existent paths are silently skipped**: If a configured directory doesn't exist, AgentX ignores it without errors. Useful for optional shared directories that may not be present on every machine.
...
Bundles live in **`~/.agentx/skill-bundles/<slug>.yaml`** and look like this:
...
- `description` (optional) — short text shown in `/bundles` and `agentx bundles list`.
...
agentx bundles create research

# Overwrite an existing bundle
agentx bundles create backend-dev --skill ... --force

# Delete a bundle
agentx bundles delete backend-dev

# Re-scan ~/.agentx/skill-bundles/ and report changes
agentx bundles reload
```
...
- You want to ship a team-wide "task profile" by checking the bundle YAML into a shared dotfiles repo and symlinking it into `~/.agentx/skill-bundles/`.

A bundle is just a YAML alias — it doesn't install skills for you. The skills themselves must already be present (in `~/.agentx/skills/` or an external skill directory). Otherwise the bundle invocation just skips the missing ones.
...
Staged writes survive restarts under `~/.agentx/pending/skills/` and are
...
```bash
agentx skills browse                              # Browse all hub skills (official first)
agentx skills browse --source official            # Browse only official optional skills
agentx skills search kubernetes                   # Search all sources
agentx skills search react --source skills-sh     # Search the skills.sh directory
agentx skills search https://mintlify.com/docs --source well-known
agentx skills inspect openai/skills/k8s           # Preview before installing
agentx skills install openai/skills/k8s           # Install with security scan
agentx skills install official/security/1password
agentx skills install skills-sh/vercel-labs/json-render/json-render-react --force
agentx skills install well-known:https://mintlify.com/docs/.well-known/skills/mintlify
...
agentx skills list --source hub                   # List hub-installed skills
agentx skills check                               # Check installed hub skills for upstream updates
agentx skills update                              # Reinstall hub skills with upstream changes when needed
agentx skills audit                               # Re-scan all hub skills for security
agentx skills uninstall k8s                       # Remove a hub skill
agentx skills reset google-workspace              # Un-stick a bundled skill from "user-modified" (see below)
agentx skills reset google-workspace --restore    # Also restore the bundled version, deleting your local edits
agentx skills publish skills/my-skill --to github --repo owner/repo
agentx skills snapshot export setup.json          # Export skill config
agentx skills tap add myorg/skills-repo           # Add a custom GitHub source
```
...
|--------|---------|-------|
| `official` | `official/security/1password` | Optional skills shipped with AgentX. |
| `skills-sh` | `skills-sh/vercel-labs/agent-skills/vercel-react-best-practices` | Searchable via `agentx skills search <query> --source skills-sh`. AgentX resolves alias-style skills when the skills.sh slug differs from the repo folder. |
...
AgentX currently integrates with these skills ecosystems and discovery sources:

#### 1. Official optional skills (`official`)

These are maintained in the AgentX repository itself and install with built-in trust.
...
This is Vercel's public skills directory. AgentX can search it directly, inspect skill detail pages, resolve alias-style slugs, and install from the underlying source repo.
...
```bash
agentx skills search https://mintlify.com/docs --source well-known
agentx skills inspect well-known:https://mintlify.com/docs/.well-known/skills/mintlify
agentx skills install well-known:https://mintlify.com/docs/.well-known/skills/mintlify
```

#### 4. Direct GitHub skills (`github`)

AgentX can install directly from GitHub repositories and GitHub-based taps. This is useful when you already know the repo/path or want to add your own custom source repo.
...
```bash
agentx skills install openai/skills/k8s
...
- AgentX source id: `clawhub`

#### 6. LobeHub (`lobehub`)

AgentX can search and convert agent entries from LobeHub's public catalog into installable AgentX skills.

- Site: [LobeHub](https://lobehub.com/)
- Public agents index: [chat-agents.lobehub.com](https://chat-agents.lobehub.com/)
- Backing repo: [lobehub/lobe-chat-agents](https://github.com/lobehub/lobe-chat-agents)
- AgentX source id: `lobehub`

#### 7. browse.sh (`browse-sh`)

AgentX integrates with [browse.sh](https://browse.sh), Browserbase's catalog of 200+ site-specific browser-automation SKILL.md files (Airbnb, Amazon, arXiv, 12306.cn, Etsy, Xero, and many more). Each skill describes how to drive one website end-to-end and is suitable for use with AgentX' browser tools and any browser-automation skills you already have installed.

- Site: [browse.sh](https://browse.sh/)
- Catalog API: `https://browse.sh/api/skills`
- AgentX source id: `browse-sh`
- Trust level: `community`

```bash
agentx skills search airbnb --source browse-sh
agentx skills inspect browse-sh/airbnb.com/search-listings-ddgioa
agentx skills install browse-sh/airbnb.com/search-listings-ddgioa
```
...
Install `SKILL.md` directly from any HTTP(S) URL — useful when an author hosts a skill on their own site (no hub listing, no GitHub path to type). AgentX also fetches explicitly referenced files under `references/`, `templates/`, `scripts/`, `assets/`, and `examples/`, then scans and installs the complete bundle.

- AgentX source id: `url`
- Identifier: the URL itself (no prefix needed)
- Scope: `SKILL.md` plus exact referenced support files in the allowlisted directories. AgentX does not enumerate or copy unrelated files from the host.

```bash
agentx skills install https://sharethis.chat/SKILL.md
agentx skills install https://example.com/my-skill/SKILL.md --category productivity
```
...
agentx skills install https://example.com/SKILL.md --name sharethis-chat
...
`agentx skills inspect ...` now also surfaces upstream metadata when available:
- repo URL
...
| Level | Source | Policy |
|-------|--------|--------|
| `builtin` | Ships with AgentX | Always trusted |
...
If you want to share a curated set of skills — for your team, your org, or publicly — you can publish them as a **tap**: a GitHub repository other AgentX users add with `agentx skills tap add <owner/repo>`. No server, no registry sign-up, no release pipeline. Just a directory of `SKILL.md` files.
...
AgentX discovers skills by listing every subdirectory of the tap path and probing each for `SKILL.md`.

#### Minimal tap example

```
my-org/agentx-skills
└── skills/
    └── deploy-runbook/
        └── SKILL.md
```
...
After pushing that to GitHub, any AgentX user can subscribe and install:

```bash
agentx skills tap add my-org/agentx-skills
agentx skills search deploy
agentx skills install my-org/agentx-skills/deploy-runbook
```

#### Non-default paths

If your skills don't live under `skills/` (common when you're adding a `skills/` subtree to an existing project), edit the tap entry in `~/.agentx/skills/.hub/taps.json`:
...
The `agentx skills tap add` CLI defaults new taps to `path: "skills/"`; edit the file directly if you need a different path. `agentx skills tap list` shows the effective path per tap.
...
```bash
agentx skills install owner/repo/skills/my-workflow
```
...
Taps are stored in `~/.agentx/skills/.hub/taps.json` (created on demand).

## Bundled skill updates (`agentx skills reset`)

AgentX ships with a set of bundled skills in `skills/` inside the repo. On install and on every `agentx update`, a sync pass copies those into `~/.agentx/skills/` and records a manifest at `~/.agentx/skills/.bundled_manifest` mapping each skill name to the content hash at the time it was synced (the **origin hash**).

On each sync, AgentX recomputes the hash of your local copy and compares it to the origin hash:
...
agentx skills reset google-workspace --restore
...
:::note Profiles
Each profile has its own `.bundled_manifest` under its own `AGENTX_HOME`, so `agentx -p coder skills reset <name>` only affects that profile.
:::
