...
Themes are YAML files stored in `~/.agentx/dashboard-themes/`. The file name doesn't matter (the theme's `name:` field is what the system uses), but convention is `<name>.yaml`. Every field is optional — missing keys fall back to the built-in `default` theme, so a theme can be as small as one color.
...
Everything a plugin needs is on `window.__AGENTX_PLUGIN_SDK__`. Plugins should never import React directly.
...
|----------|-----------|--------------|
| 1 (wins on conflict) | `~/.agentx/plugins/<name>/dashboard/` | `user` |
| 2 | `<repo>/plugins/memory/<name>/dashboard/` | `bundled` |
| 2 | `<repo>/plugins/<name>/dashboard/` | `bundled` |
| 3 | `./.agentx/plugins/<name>/dashboard/` | `project` — only when `AGENTX_ENABLE_PROJECT_PLUGINS` is set |
...
1. Dashboard loads. `main.tsx` exposes the SDK on `window.__AGENTX_PLUGIN_SDK__` and the registry on `window.__AGENTX_PLUGINS__`.
...
4. The plugin's IIFE runs and calls `window.__AGENTX_PLUGINS__.register(name, Component)` — and optionally `.registerSlot(name, slot, Component)` for each slot.
...
|--------|------|----------|
| `window.__AGENTX_PLUGIN_SDK__` | object | `registry.ts` — React, hooks, UI components, API client, utils. |
| `window.__AGENTX_PLUGINS__.register(name, Component)` | function | Register a plugin's main component. |
| `window.__AGENTX_PLUGINS__.registerSlot(name, slot, Component)` | function | Register into a named shell slot. |
...
Check that the file is in `~/.agentx/dashboard-themes/` and ends in `.yaml` or `.yml`. Refresh the page. Run `curl http://127.0.0.1:9119/api/dashboard/themes` — your theme should be in the response. If the YAML has a parse error, the dashboard logs to `errors.log` under `~/.agentx/logs/`.

**My plugin's tab doesn't show up.**
1. Check the manifest is at `~/.agentx/plugins/<name>/dashboard/manifest.json` (note the `dashboard/` subdirectory).
...
4. Open browser dev tools → Console — look for errors during the IIFE or `window.__AGENTX_PLUGINS__ is undefined` (indicates the SDK didn't initialize, usually a React render crash earlier).
5. Verify your bundle calls `window.__AGENTX_PLUGINS__.register(...)` with the **same name** as `manifest.json:name`.
...
Dashboard plugins are installed by directory layout, not by pip entry point. The cleanest distribution path today is a git repo the user clones into `~/.agentx/plugins/`. A pip-based installer for dashboard plugins is not currently wired up.
