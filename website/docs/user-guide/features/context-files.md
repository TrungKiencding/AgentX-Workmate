...
Hermes Agent automatically discovers and loads context files that shape how it behaves. Some are project-local and discovered from your working directory. `SOUL.md` is now global to the Hermes instance and is loaded from `AGENTX_HOME` only.
...
| **SOUL.md** | Global personality and tone customization for this Hermes instance | `AGENTX_HOME/SOUL.md` only |
...
- `~/.agentx/SOUL.md`
- or `$AGENTX_HOME/SOUL.md` if you run Hermes with a custom home directory

Important details:

- Hermes seeds a default `SOUL.md` automatically if one does not exist yet
- Hermes loads `SOUL.md` only from `AGENTX_HOME`
...
Hermes is compatible with Cursor IDE's `.cursorrules` file and `.cursor/rules/*.mdc` rule modules. If these files exist in your project root and no higher-priority context file (`.agentx.md`, `AGENTS.md`, or `CLAUDE.md`) is found, they're loaded as the project context.
