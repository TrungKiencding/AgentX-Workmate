...
On a brand-new install (or the first time a pre-curator install ticks after `agentx update`), the curator **does not run immediately**. The first observation seeds `last_run_at` to "now" and defers the first real pass by one full `interval_hours`. This gives you a full interval to review your skill library, pin anything important, or opt out entirely before the curator ever touches it.

If you want to see what the curator *would* do before it runs for real, run `agentx curator run --dry-run` — it produces the same review report without mutating the library.
:::

A run has two phases:

1. **Automatic transitions** (deterministic, no LLM). Skills unused for `stale_after_days` (30) become `stale`; skills unused for `archive_after_days` (90) are moved to `~/.agentx/skills/.archive/`. This is the always-on pruning behavior — it runs whenever the curator is enabled, with no aux-model cost.
...
**Easiest — `agentx model`:**

```bash
agentx model                   # → "Auxiliary models — side-task routing"
...
:::note Legacy config
Earlier releases used a one-off `curator.auxiliary.{provider,model}` block. That path still works but emits a deprecation log line — please migrate to `auxiliary.curator` above so the curator shares the same plumbing (`agentx model`, dashboard Models tab, `base_url`, `api_key`, `timeout`, `extra_body`) as every other aux task.
:::

## CLI

```bash
agentx curator status         # last run, counts, pinned list, LRU top 5
agentx curator run            # trigger a run now (blocks until done). Prune-only unless curator.consolidate: true
agentx curator run --consolidate # force the LLM consolidation pass on for this run, overriding the config default
agentx curator run --background  # fire-and-forget: start the run in a background thread
agentx curator run --dry-run  # preview only — report without any mutations
agentx curator backup         # take a manual snapshot of ~/.agentx/skills/
agentx curator rollback       # restore from the newest snapshot
agentx curator rollback --list     # list available snapshots
agentx curator rollback --id <ts>  # restore a specific snapshot
agentx curator rollback -y         # skip the confirmation prompt
agentx curator pause          # stop runs until resumed
agentx curator resume
agentx curator pin <skill>    # never auto-transition this skill
agentx curator unpin <skill>
agentx curator adopt <skill>    # hand an unmanaged skill to the curator
agentx curator adopt --all-unmanaged   # hand over every unmanaged skill
agentx curator list-unmanaged   # itemize skills with no provenance marker
agentx curator restore <skill>  # move an archived skill back to active
agentx curator list-archived    # list skills currently in ~/.agentx/skills/.archive/
agentx curator archive <skill>  # manually archive a single skill now
agentx curator prune [--days N] # bulk-archive agent-created skills idle >= N days (default 90)
```

## Backups and rollback

Before every real curator pass, AgentX takes a tar.gz snapshot of `~/.agentx/skills/` at `~/.agentx/skills/.curator_backups/<utc-iso>/skills.tar.gz`. If a pass archives or consolidates something you didn't want touched, you can undo the whole run with one command:

```bash
agentx curator rollback        # restore newest snapshot (with confirmation)
agentx curator rollback -y     # skip the prompt
agentx curator rollback --list # see all snapshots with reason + size
```

The rollback itself is reversible: before replacing the skills tree, AgentX takes another snapshot tagged `pre-rollback to <target-id>`, so a mistaken rollback can be undone by rolling forward to that one with `--id`.

You can also take manual snapshots at any time with `agentx curator backup --reason "before-refactor"`. The `--reason` string lands in the snapshot's `manifest.json` and is shown in `--list`.
...
Set `curator.backup.enabled: false` to disable automatic snapshotting. The manual `agentx curator backup` command still works when backups are disabled only if you set `enabled: true` first — the flag gates both paths symmetrically so there's no way to accidentally skip the pre-run snapshot on mutating runs.

`agentx curator status` also lists the five least-recently-used skills — a quick way to see what's likely to become stale next.
...
`~/.agentx/skills/.usage.json`. A skill qualifies when ALL of the following
are true:

1. Its name is **not** in `~/.agentx/skills/.bundled_manifest` (bundled skills shipped with the repo).
2. Its name is **not** in `~/.agentx/skills/.hub/lock.json` (hub-installed skills).
...
If you manually created a `SKILL.md` or pointed AgentX at an external skill
...
**To see which skills the curator actually manages**, run `agentx curator status`.
...
`agentx curator status` reports an **unmanaged** count alongside the managed
one:
...
unmanaged (no provenance marker): 112 total
  ...
  never auto-staled or archived — `agentx curator adopt <name>` hands one over
```
...
```bash
agentx curator list-unmanaged                    # itemize them, with reasons
agentx curator adopt <name> [<name> ...]         # hand specific skills over
agentx curator adopt --all-unmanaged --dry-run   # preview the full list
agentx curator adopt --all-unmanaged             # hand over everything (prompts)
agentx curator adopt --all-unmanaged --yes       # skip the prompt
```
...
`.usage.json`; read it as policy. `agentx curator adopt` changes the policy, and
...
**wrote** it — AgentX edits user-authored skills on your behalf constantly. An
...
- Archives are recoverable via `agentx curator restore <name>`

If you want to protect a specific skill from ever being touched — for example a
hand-authored skill you rely on — use `agentx curator pin <name>`. See the next
section.
...
- The **agent's `skill_manage` tool** refuses `delete` on it, pointing the user at `agentx curator unpin <name>`. Patches and edits still go through, so the agent can improve a pinned skill's content as pitfalls come up without a pin/unpin/re-pin dance.

Pin and unpin with:

```bash
agentx curator pin <skill>
agentx curator unpin <skill>
```

The flag is stored as `"pinned": true` on the skill's entry in `~/.agentx/skills/.usage.json`, so it survives across sessions.
...
If you want a stronger guarantee than "no deletion" — for instance, freezing a skill's content entirely while the agent still reads it — edit `~/.agentx/skills/<name>/SKILL.md` directly with your editor. The pin guards tool-driven deletion, not your own filesystem access.

## Usage telemetry

The curator maintains a sidecar at `~/.agentx/skills/.usage.json` with one entry per skill:
...
Every curator run writes a timestamped directory under `~/.agentx/logs/curator/`:
...
```bash
agentx curator restore <skill-name>
```

This moves the skill back from `~/.agentx/skills/.archive/` to the active tree and resets its state to `active`. The restore refuses if a bundled or hub-installed skill has since been installed under the same name (would shadow upstream).

## Disabling per environment

The curator is on by default. To turn it off:

- **For one profile only:** edit `~/.agentx/config.yaml` (or the active profile's config) and set `curator.enabled: false`.
- **For just one run:** `agentx curator pause` — the pause persists across sessions; use `resume` to re-enable.
