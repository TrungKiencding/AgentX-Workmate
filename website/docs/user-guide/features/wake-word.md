...
sidebar_position: 11
title: "Wake Word"
description: "Hands-free 'Hey AgentX' wake word — start a voice session by speaking, the 'Hey Siri' way"
---

# Wake Word ("Hey AgentX")

The wake word turns AgentX into a hands-free assistant across the CLI, TUI, and
desktop app: with one setting on, AgentX listens in the background for a spoken
trigger phrase. Say it, and AgentX starts a fresh session, opens the microphone,
...
|--------|------|---------|-------|
| **openWakeWord** (default) | Free | None | Local ONNX models. Ships a bundled **"hey agentx"** model (default); also supports `hey_jarvis`, `alexa`, `hey_mycroft`, … and custom models |
...
By default the phrase is **"hey agentx"** — a model for it ships with AgentX, so
...
```bash
cd ~/.agentx/agentx-agent && uv pip install -e ".[wake]"
```

## Quick start

```bash
# In an interactive `agentx` session:
...
wake_word:
  ...
  phrase: "hey agentx"        # cosmetic label only — detection is keyed by the model/keyword below
...
- **`confirmation_frames`** (default `3`, openWakeWord only) — how many
  *consecutive* over-threshold frames are required before the wake fires. A real
  "hey agentx" holds a high score across several frames; an ambient blip spikes
...
- **`sensitivity`** (default `0.6`) — the detection threshold, `0.0`–`1.0`.
  ...
  **all** engines — for openWakeWord it's the raw per-frame score threshold, for
  ...
  if real "hey agentx" utterances are missed.
...
default) to let AgentX choose per platform: **tflite on Apple Silicon**, onnx
...
fire. The tflite backend needs `ai-edge-litert` on macOS, which AgentX installs
on demand alongside the other wake-word deps.

### Surfaces (CLI, TUI, GUI)

The wake word works in all three AgentX surfaces, and `surface` picks which one
...
| `cli` | Only the classic `agentx` CLI. |
| `tui` | Only `agentx --tui`. |
| `gui` | Only the desktop app. |

The detector is on-device and single-mic, so only one surface listens at a time,
including when AgentX surfaces run in separate processes. Ownership is sticky:
the first eligible claimant keeps the listener until it stops, disconnects, or
its process exits. AgentX does not silently fail over to another open surface.
...
"Hey AgentX" works out of the box — the bundled openWakeWord model
...
command (`agentx -p <profile>`) instead of routing.
...
syllables with an uncommon word ("agentx" qualifies) beat common words like
"hello" or "stop".
:::

### Option C — Porcupine (custom keyword in seconds)

Create a "Hey AgentX" keyword in the [Picovoice Console](https://console.picovoice.ai/),
...
- A TTS provider for speaking the reply (the default `edge-tts` works with no
  ...
  both STT and TTS are ready — `agentx tools` (Voice section) sets them up.
- The wake engine deps (auto-installed, or `agentx-workmate[wake]`).
...
but the phrase never fires. AgentX detects this (`/wake status` shows
"mic delivers only silence"; the desktop ear tooltip carries the same hint).
Fix: System Settings → Privacy & Security → Microphone → enable the AgentX
backend (it may appear as your terminal, `python`, or AgentX), then toggle the
...
```bash
agentx config set wake_word.input_device "Microphone Array"
```

Use `null` to return to the process default:

```bash
agentx config set wake_word.input_device null
```
