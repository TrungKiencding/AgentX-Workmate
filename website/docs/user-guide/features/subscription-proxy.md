...
chat completions — use your AgentX-managed provider subscription as their
...
| Use case | "Use AgentX as a chat backend" | "Use my Portal sub from another app" |
...
This opens your browser for the Nous Portal OAuth flow. AgentX stores
the refresh token in `~/.agentx/auth.json` — the same place all AgentX
...
If you see `not logged in`, run `agentx portal`. If you see
