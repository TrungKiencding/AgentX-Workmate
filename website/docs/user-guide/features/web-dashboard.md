...
The web dashboard is a browser-based UI for managing your AgentX Workmate installation. Instead of editing YAML files or running CLI commands, you can configure settings, manage API keys, and monitor sessions from a clean web interface.

:::tip
Hosted-mode auth uses Nous Portal OAuth; if you also want the dashboard to talk to a real backend, `agentx setup --portal` wires up the model and tool gateway too. See [Nous Portal](/integrations/nous-portal).
:::
...
```bash
# Custom port
agentx dashboard --port 8080

# Bind to all interfaces (use with caution on shared networks)
agentx dashboard --host 0.0.0.0

# Start without opening browser
agentx dashboard --no-open
```
...
child with the selected profile's `AGENTX_HOME`, so the conversation runs
...
processes (manage them via `agentx -p <name> gateway …`), each profile's
session database, and cron schedulers (the Cron page already aggregates
across profiles with its own filter).

## Prerequisites

The default `agentx-agent` install does not ship the HTTP stack or PTY helper — those are optional extras. The **web dashboard** needs FastAPI and Uvicorn (`web` extra). The **Chat** tab also needs `ptyprocess` to spawn the embedded TUI behind a pseudo-terminal (`pty` extra on POSIX). Install both with:

```bash
cd ~/.agentx/agentx-agent && uv pip install -e ".[web,pty]"
```

The `web` extra pulls in FastAPI/Uvicorn; `pty` pulls in `ptyprocess` (POSIX) or `pywinpty` (native Windows — note that the embedded TUI itself still requires WSL). `cd ~/.agentx/agentx-agent && uv pip install -e ".[all]"` includes both extras and is the easiest path if you also want messaging/voice/etc.

When you run `agentx dashboard` without the dependencies, it will tell you what to install. If the frontend hasn't been built yet and `npm` is available, it builds automatically on first launch.

The Chat tab is part of every `agentx dashboard` launch — the embedded browser chat pane (running the TUI over PTY/WebSocket) is always available, with no extra flag required.
...
- Node.js (same requirement as `agentx --tui`; the TUI bundle is built on first launch)
- `ptyprocess` — installed by the `pty` extra (`cd ~/.agentx/agentx-agent && uv pip install -e ".[web,pty]"`, or `[all]` covers both)
...
```ini
[Service]
EnvironmentFile=%h/.agentx/.env
ExecStart=/path/to/venv/bin/python -m hermes_cli.main dashboard \
    --host 0.0.0.0 --port 9119 --no-open
```

with `~/.agentx/.env` containing:

```bash
AGENTX_DASHBOARD_BASIC_AUTH_USERNAME=admin
AGENTX_DASHBOARD_BASIC_AUTH_PASSWORD=choose-a-strong-password
AGENTX_DASHBOARD_BASIC_AUTH_SECRET=<32+ random bytes; openssl rand -base64 32>
```
...
:::tip
Config changes take effect on the next agent session or gateway restart. The web dashboard edits the same `config.yaml` file that `agentx config set` and the gateway read from.
:::
...
Create and manage [profiles](../profiles.md) — isolated AgentX instances with their own config, skills, and sessions.
...
- **Set as active** — flips the sticky default that **future CLI/gateway runs** pick up (same as `agentx profile use`). This does *not* change what the dashboard manages — that's the profile switcher's job
- **Edit model / description / SOUL** — inline editors writing into that profile
- **Rename / Delete** — named profiles only

### Skills

Browse, search, and toggle installed skills and toolsets, and install new ones from the hub. Skills are loaded from `~/.agentx/skills/` and grouped by category.
...
- **Browse hub** — a third view searches the skill hub across all sources (the same as `agentx skills search`), installs any result by identifier with a live install log, and offers an "Update all" button to refresh installed skills.
...
block in `config.yaml` that `agentx mcp` reads from.
...
`agentx mcp catalog` / `agentx mcp install` use.
...
Connect AgentX to any messaging platform from the browser — full parity with
`agentx setup gateway`. The page lists every supported channel (Telegram,
...
- **Restart gateway** — credentials are written to `~/.agentx/.env` and the enabled flag to `config.yaml`; the gateway connects each enabled channel on its next restart, which you can trigger right from the page.
...
- **Host** — live system stats: OS / kernel, architecture, hostname, Python and AgentX versions, CPU core count + utilization, memory, disk usage of the AgentX home, uptime, and load average. (CPU/memory/disk come from `psutil` when installed; identity fields are always shown.) The AgentX version shows an **update-status badge** (up to date / N commits behind) and a **Check for updates** button. When an update is available on a git install, an **Update now** button opens a confirmation dialog — showing how many commits you'll pull — before running `agentx update` in the background. On Docker/Nix installs the dashboard can't apply the update in place, so it shows the correct out-of-band command instead.
- **Nous Portal** — login status, the active inference provider, and the Tool Gateway routing table (which tools run via the Portal vs. locally), with a link to manage your subscription. Read-only mirror of `agentx portal`.
- **Skill curator** — the background skill-maintenance status (active / paused, interval, last run) with pause/resume and a run-now button. Mirrors `agentx curator`.
...
This re-reads `~/.agentx/.env` into the running process's environment. Useful when you've added a new provider key via the dashboard and want to use it immediately.
...
`AGENTX_HOME`. Omitted = the dashboard's own profile. Unknown profile names
...
| `GET /api/agentx/update/check` | Report update availability (commits behind, install method) without applying. For git installs that are behind, also returns a `commits` list (`sha`, `summary`, `author`, `at`) of what's changed. `?force=1` busts the 6h cache |
...
- **CLI — `agentx dashboard register`.** Run it on the host where the dashboard lives. It resolves your existing Nous login (run `agentx setup` first if you're not logged in), registers a self-hosted OAuth client with the Portal, and writes `AGENTX_DASHBOARD_OAUTH_CLIENT_ID` into `~/.agentx/.env` for you. Optional flags: `--name` (a human-readable label, otherwise auto-generated) and `--redirect-uri` (a public HTTPS callback URL for an internet-facing host).

  ```bash
  agentx dashboard register
  # ✓ Registered dashboard "swift_falcon"
  # …writes AGENTX_DASHBOARD_OAUTH_CLIENT_ID to ~/.agentx/.env
  ```

- **GUI — the Local Dashboards page.** Open [`/local-dashboards`](https://portal.nousresearch.com/local-dashboards) in the Nous Portal to register, name, manage, and revoke self-hosted dashboards from the browser. Copy the resulting `agent:{id}` client ID into `AGENTX_DASHBOARD_OAUTH_CLIENT_ID` (env) or `dashboard.oauth.client_id` (config.yaml). This is also where you revoke a dashboard registered via the CLI.
...
|---------|-----------|--------|----------------|
| `AGENTX_DASHBOARD_OAUTH_CLIENT_ID` | `dashboard.oauth.client_id` | `agent:{instance_id}` | `agentx dashboard register` |

Per the AgentX Workmate convention (`~/.agentx/.env` is for API keys / secrets only), **`config.yaml` is the recommended place to set these values** for local dev, on-prem, and any deployment you control directly. The environment-variable path exists so a hosting platform's secret injection can push per-deploy `client_id`s without anyone having to edit `config.yaml` inside the image — that's its primary purpose.
...
Bundled providers reported these issues:
  • nous: AGENTX_DASHBOARD_OAUTH_CLIENT_ID is not set (and
...
Configure an auth provider before exposing the dashboard:
  ...
  • OAuth: run `agentx dashboard register` (Nous Portal) or install a
...
From a logged-in AgentX install to a Nous-gated dashboard in three steps.

**1. Log in and register the dashboard.** `agentx dashboard register` uses your existing Nous login to provision an OAuth client and writes `AGENTX_DASHBOARD_OAUTH_CLIENT_ID` into `~/.agentx/.env` for you:

```bash
agentx setup            # if you're not already logged into Nous Portal
agentx dashboard register
# ✓ Registered dashboard "swift_falcon"
# …writes AGENTX_DASHBOARD_OAUTH_CLIENT_ID to ~/.agentx/.env
```
...
|---------|-----------|-------|
| `AGENTX_DASHBOARD_BASIC_AUTH_USERNAME` | `dashboard.basic_auth.username` | required to activate |
| `AGENTX_DASHBOARD_BASIC_AUTH_PASSWORD_HASH` | `dashboard.basic_auth.password_hash` | preferred (no plaintext at rest) |
| `AGENTX_DASHBOARD_BASIC_AUTH_PASSWORD` | `dashboard.basic_auth.password` | plaintext; **wins over a config `password_hash`** so you can rotate via env |
| `AGENTX_DASHBOARD_BASIC_AUTH_SECRET` | `dashboard.basic_auth.secret` | token-signing key |
| `AGENTX_DASHBOARD_BASIC_AUTH_TTL_SECONDS` | `dashboard.basic_auth.session_ttl_seconds` | access-token lifetime |
...
**1. Set credentials in `~/.agentx/.env`.** Hash the password so no plaintext sits at rest, and set a stable signing secret so sessions survive restarts:
...
cat >> ~/.agentx/.env <<EOF
AGENTX_DASHBOARD_BASIC_AUTH_USERNAME=admin
AGENTX_DASHBOARD_BASIC_AUTH_PASSWORD_HASH=$HASH
AGENTX_DASHBOARD_BASIC_AUTH_SECRET=$(openssl rand -base64 32)
EOF
chmod 600 ~/.agentx/.env
```
...
| Env var | Overrides | Notes |
|---------|-----------|-------|
| `AGENTX_DASHBOARD_OIDC_ISSUER` | `dashboard.oauth.self_hosted.issuer` | OIDC issuer URL — required |
| `AGENTX_DASHBOARD_OIDC_CLIENT_ID` | `dashboard.oauth.self_hosted.client_id` | Public client id — required |
| `AGENTX_DASHBOARD_OIDC_SCOPES` | `dashboard.oauth.self_hosted.scopes` | Defaults to `openid profile email` |
...
```json
{
  ...
  "clients": [
    {
      "clientId": "agentx-dashboard",
      "name": "AgentX Workmate Dashboard",
      "enabled": true,
      "publicClient": true,
...
`AGENTX_DASHBOARD_PUBLIC_URL` tells the dashboard its OAuth callback is
...
> admin console (Clients → agentx-dashboard → Settings). The same pattern works
...
For deploys behind reverse proxies that don't reliably forward those headers (manual nginx setups, on-prem ingresses, custom-domain deploys with partial proxy chains), set `dashboard.public_url` (or `AGENTX_DASHBOARD_PUBLIC_URL`) to the **complete public URL** the dashboard is reached at:
...
|---------|---------------|-------------|
| `dashboard.public_url` in `config.yaml` | `AGENTX_DASHBOARD_PUBLIC_URL` | Local dev / on-prem (canonical) |
| `AGENTX_DASHBOARD_PUBLIC_URL` env var | — | Hosting-platform secrets / CI |
...
Every login start, success, failure, and session-verify failure is written as a JSON line to `$AGENTX_HOME/logs/dashboard-auth.log`. Sensitive fields (`access_token`, `refresh_token`, `code`, `code_verifier`, `state`, `Authorization` header) are redacted before logging.
...
```python
# ~/.agentx/plugins/dashboard-auth-myidp/__init__.py
...
```bash
# Quick env-var path.
AGENTX_DASHBOARD_OAUTH_CLIENT_ID=agent:test \
  agentx dashboard --host 0.0.0.0
...
```bash
# 1. Set the dashboard login credentials in ~/.agentx/.env (secrets file, 0600).
cat >> ~/.agentx/.env <<'EOF'
AGENTX_DASHBOARD_BASIC_AUTH_USERNAME=admin
AGENTX_DASHBOARD_BASIC_AUTH_PASSWORD=choose-a-strong-password
...
agentx dashboard --no-open --host 0.0.0.0 --port 9119
```

Prefer no plaintext at rest? Use `AGENTX_DASHBOARD_BASIC_AUTH_PASSWORD_HASH` with a scrypt hash instead — see [Username/password provider](#usernamepassword-provider-no-oauth-idp) for the full surface.

If you run the dashboard as a systemd service, `~/.agentx/.env` is picked up automatically when the unit has `EnvironmentFile=%h/.agentx/.env`, so the credentials are in the environment at boot.
...
The session refreshes automatically and survives restarts when `AGENTX_DASHBOARD_BASIC_AUTH_SECRET` is set on the backend.

### Environment-variable override

Instead of the in-app setting, you can point the desktop at a backend with an env var before launching it. When `AGENTX_DESKTOP_REMOTE_URL` is set, it overrides the saved in-app URL (the Gateway settings panel shows an "env override" badge and disables editing); you still **Sign in** with your username and password from the panel.

| Env var | Value |
|---------|-------|
| `AGENTX_DESKTOP_REMOTE_URL` | `http://<backend-host>:9119` |

### Troubleshooting

- **"Remote gateway incomplete"** — you haven't entered a remote URL.
- **Sign-in fails with 401 / "Invalid credentials"** — the username or password doesn't match the backend's `AGENTX_DASHBOARD_BASIC_AUTH_USERNAME` / `AGENTX_DASHBOARD_BASIC_AUTH_PASSWORD`. The backend returns the same generic error for unknown user and wrong password, so check both. Confirm the gate with `curl -s http://<host>:9119/api/status | jq '.auth_required, .auth_providers'` — it should report `true` and include `"basic"`.
...
- **Signed out on every restart** — set `AGENTX_DASHBOARD_BASIC_AUTH_SECRET` to a stable value; otherwise the signing key is regenerated per boot.
...
When you run `agentx update`, the web frontend is automatically rebuilt if `npm` is available. This keeps the dashboard in sync with code updates. If `npm` isn't installed, the update skips the frontend build and `agentx dashboard` will build it on first launch.
...
| Theme | Character |
|-------|-----------|
| **AgentX Teal** (`default`) | Dark teal + cream, system fonts, comfortable spacing |
| **AgentX Teal (Large)** (`default-large`) | Same as default with 18px text and roomier spacing |
