...
- **Plugin / integration authors** wanting to add a new lane shape (a CLI worker that wraps Codex / Claude Code / OpenCode, a containerised review worker, a non-AgentX service that pulls tasks via the API).

If you're writing the worker code itself — the agent that runs *inside* a lane — the kanban lifecycle and reference details are injected into the worker's system prompt automatically (the `KANBAN_GUIDANCE` block in [`agent/prompt_builder.py`](https://github.com/TrungKiencding/AgentX-Workmate/blob/main/agent/prompt_builder.py)).

## The hierarchy

```text
AgentX Kanban  =  canonical task lifecycle + audit trail
...
AgentX Kanban owns lifecycle truth — `ready` → `running` → `blocked` / `done` / `archived`. Worker lanes execute work but never own that truth; everything they do flows back through the kanban kernel via the `kanban_*` tools (or, for non-AgentX external workers, via the API). Reviewers gate the transition from "code change written" to "task done."
...
The dispatcher matches `task.assignee` against either an AgentX profile name (the default lane shape) or a registered non-spawnable identifier (the plugin lane shape — see [Adding an external CLI worker lane](#adding-an-external-cli-worker-lane) below). Tasks whose assignee doesn't resolve are left on `ready` with a `skipped_nonspawnable` event so a board operator can fix them; they are not silently dropped or executed by an arbitrary fallback.

### 2. A spawn mechanism

For AgentX profile lanes, the dispatcher's `_default_spawn` runs `agentx -p <assignee> chat -q <prompt>` (or the equivalent module form when the `agentx` shim isn't on `$PATH`) inside the task's pinned workspace, with these env vars set:

| Variable | Carries |
|---|---|
| `AGENTX_KANBAN_TASK` | the task id the worker is operating on |
| `AGENTX_KANBAN_DB` | absolute path to the per-board SQLite file |
| `AGENTX_KANBAN_BOARD` | board slug |
| `AGENTX_KANBAN_WORKSPACES_ROOT` | root of the board's workspace tree |
| `AGENTX_KANBAN_WORKSPACE` | absolute path to *this* task's workspace |
| `AGENTX_KANBAN_RUN_ID` | the current run's id (for the lifecycle gate) |
| `AGENTX_KANBAN_CLAIM_LOCK` | the claim lock string (`<host>:<pid>:<uuid>`) |
| `AGENTX_PROFILE` | the worker's own profile name (for `kanban_comment` author attribution) |
| `AGENTX_TENANT` | tenant namespace, if the task has one |

For non-AgentX lanes (registered via a plugin), the plugin supplies its own `spawn_fn` callable that gets `task`, `workspace`, and `board` and returns an optional pid for crash detection.
...
- **Block instead of complete**, with `reason` prefixed `review-required: ` so the dashboard / `agentx kanban show` surfaces the row as awaiting review.
...
The dashboard renders run history with summaries, metadata blocks, and exit-status badges. CLI users can run `agentx kanban tail <task_id>` to follow live, or `agentx kanban runs <task_id>` for the historical attempt list.

## Existing lane shapes

### AgentX profile lane (default)

The shape every kanban worker takes today: the assignee is a profile name, the dispatcher spawns `agentx -p <profile>`, the worker gets the `KANBAN_GUIDANCE` system-prompt block injected automatically, and uses the `kanban_*` tools to terminate the run. No setup beyond defining the profile.
