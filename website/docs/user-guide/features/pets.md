...
AgentX can show an animated **pet** — a small mascot sprite that reacts to what
...
- Pets are installed into your profile's `pets/` directory
  (`<AGENTX_HOME>/pets/<slug>/`), so each [profile](../profiles.md) keeps its
  own set.
...
In the terminal (CLI/TUI), AgentX renders the sprite at full fidelity when your
...
agentx pets list
agentx pets list cat

# Install a pet and make it active in one step
agentx pets install boba --select
...
## `agentx pets` commands

| Goal | Command |
| --- | --- |
| Browse the gallery | `agentx pets list [query] [--limit N]` |
| List installed pets | `agentx pets list --installed` |
| Install a pet | `agentx pets install <slug> [--select] [--force]` |
| Set the active pet | `agentx pets select [slug]` (omit slug for a picker) |
| Resize the pet everywhere | `agentx pets scale <factor>` (e.g. `0.5`, clamped 0.1–3.0) |
| Preview/animate | `agentx pets show [slug] [--state <s>] [--cycle] [--once] [--mode <m>] [--scale <f>]` |
| Disable the pet | `agentx pets off` |
| Remove an installed pet | `agentx pets remove <slug>` |
| Diagnose setup | `agentx pets doctor` |

`agentx pets show` flags:
...
Beyond installing pre-made pets from the gallery, AgentX can **generate a brand-new pet** from a text description — its own AI sprite-generation pipeline.

- CLI/TUI: `/hatch <description>` (alias `/generate-pet`), or `agentx pets` → the generate flow.
...
- If no reference-capable backend is configured, generation surfaces an actionable error pointing you to `agentx tools` → Image Generation. (Installing/adopting existing gallery pets needs no image backend.)
- Override the backend with the `AGENTX_PET_IMAGE_PROVIDER` env var (e.g. `AGENTX_PET_IMAGE_PROVIDER=openrouter`).
...
always-on-top desktop window. Out there it stays visible while AgentX is
...
Run `agentx pets doctor` — it reports:
...
- The petdex npm CLI installs to `~/.codex/pets`; AgentX uses its own
  profile-scoped `<AGENTX_HOME>/pets/` instead — install through `agentx pets`.

## See also

- The [`agentx-agent` skill](../skills/bundled/autonomous-ai-agents/autonomous-ai-agents-agentx-agent.md)
