...
AgentX Workmate ships with 8 external memory provider plugins that give the agent persistent, cross-session knowledge beyond the built-in MEMORY.md and USER.md. Only **one** external provider can be active at a time — the built-in memory is always active alongside it.

## Quick Start

```bash
agentx memory setup      # interactive picker + configuration
agentx memory status     # check what's active
agentx memory off        # disable external provider
```

You can also select the active memory provider via `agentx plugins` → Provider Plugins → Memory Provider.

Or set manually in `~/.agentx/config.yaml`:
...
When a memory provider is active, AgentX automatically:
...
**Setup Wizard:**
```bash
agentx memory setup        # select "honcho" — runs the Honcho-specific post-setup
```

The legacy `agentx honcho setup` command still works (it now redirects to `agentx memory setup`), but is only registered after Honcho is selected as the active memory provider.
...
**Config:** `$AGENTX_HOME/honcho.json` (profile-local) or `~/.honcho/config.json` (global). Resolution order: `$AGENTX_HOME/honcho.json` > `~/.agentx/honcho.json` > `~/.honcho/config.json`. See the [config reference](https://github.com/TrungKiencding/AgentX-Workmate/blob/main/plugins/memory/honcho/README.md) and the [Honcho integration guide](https://docs.honcho.dev/v3/guides/integrations/hermes).
...
:::tip Migrating from `agentx honcho`
If you previously used `agentx honcho setup`, your config and all server-side data are intact. Just re-enable through the setup wizard again or manually set `memory.provider: honcho` to reactivate via the new system.
:::

**Multi-peer setup:**

Honcho models conversations as peers exchanging messages — one user peer plus one AI peer per AgentX profile, all sharing a workspace. The workspace is the shared environment: the user peer is global across profiles, each AI peer is its own identity. Every AI peer builds an independent representation / card from its own observations, so a `coder` profile stays code-oriented while a `writer` profile stays editorial against the same user.
...
```bash
agentx profile create coder --clone
```

`--clone` creates a `agentx.coder` host block in `honcho.json` with `aiPeer: "coder"`, shared `workspace`, inherited `peerName`, `recallMode`, `writeFrequency`, `observation`, etc. The AI peer is eagerly created in Honcho so it exists before the first message.
...
Scans every AgentX profile, creates host blocks for any profile without one, inherits settings from the default `agentx` block, and creates the new AI peers eagerly. Idempotent — skips profiles that already have a host block.
...
Off-gateway these keys do nothing. `agentx memory setup` only prompts for them when it detects a connected gateway platform. See the [Honcho page](./honcho.md#gateway-identity-mapping) for the resolver ladder and the setup flow.
...
See the [config reference](https://github.com/TrungKiencding/AgentX-Workmate/blob/main/plugins/memory/honcho/README.md) and [Honcho integration guide](https://docs.honcho.dev/v3/guides/integrations/hermes).
...
agentx memory setup    # select "openviking"
# Or manually:
agentx config set memory.provider openviking
```

`agentx memory setup` can reuse or copy connection values from
`~/.openviking/ovcli.conf`. Manual setup uses the active profile's `.env` file;
for the default profile that is `~/.agentx/.env`, and for named profiles use
...
`OPENVIKING_AGENT` is AgentX' peer ID in OpenViking for peer-scoped memories.
...
**Setup (Platform):**
```bash
agentx memory setup    # select "mem0" → "Platform"
# Or manually:
...
**Setup (OSS):**
```bash
agentx memory setup    # select "mem0" → "Open Source (self-hosted)"
# Or via flags:
agentx memory setup mem0 --mode oss --oss-llm openai --oss-llm-key sk-... --oss-vector qdrant
```
...
```bash
echo "MEM0_HOST=http://localhost:8888" >> ~/.agentx/.env
echo "MEM0_API_KEY=your-admin-api-key" >> ~/.agentx/.env
```
...
**Config:** `$AGENTX_HOME/mem0.json` (behavioral settings). Only the secret `MEM0_API_KEY` belongs in `~/.agentx/.env`.
...
| `user_id` | `agentx-user` | User identifier |
...
**Switching modes:** Re-run `agentx memory setup mem0 --mode <platform|selfhosted|oss>` or edit `mem0.json` directly.
...
**Setup:**
```bash
agentx memory setup    # select "hindsight"
# Or manually:
agentx config set memory.provider hindsight
echo "HINDSIGHT_API_KEY=your-key" >> ~/.agentx/.env
```

The setup wizard installs dependencies automatically and only installs what's needed for the selected mode (`hindsight-client` for cloud, `hindsight-all` for local). Requires `hindsight-client >= 0.4.22` (auto-upgraded on session start if outdated).

**Local mode UI:** `hindsight-embed -p agentx ui start`

**Config:** `$AGENTX_HOME/hindsight/config.json`

| Key | Default | Description |
|-----|---------|-------------|
| `mode` | `cloud` | `cloud` or `local` |
| `bank_id` | `agentx` | Memory bank identifier |
| `recall_budget` | `mid` | Recall thoroughness: `low` / `mid` / `high` |
| `memory_mode` | `hybrid` | `hybrid` (context + tools), `context` (auto-inject only), `tools` (tools only) |
| `auto_retain` | `true` | Automatically retain conversation turns |
| `auto_recall` | `true` | Automatically recall memories before each turn |
| `retain_async` | `true` | Process retain asynchronously on the server |
| `retain_context` | `conversation between AgentX Workmate and the User` | Context label for retained memories |
...
See [plugin README](https://github.com/TrungKiencding/AgentX-Workmate/blob/main/plugins/memory/hindsight/README.md) for the full configuration reference.
...
**Setup:**
```bash
agentx memory setup    # select "holographic"
# Or manually:
agentx config set memory.provider holographic
```

**Config:** `config.yaml` under `plugins.agentx-memory-store`

| Key | Default | Description |
|-----|---------|-------------|
| `db_path` | `$AGENTX_HOME/memory_store.db` | SQLite database path |
...
**Setup:**
```bash
agentx memory setup    # select "retaindb"
# Or manually:
agentx config set memory.provider retaindb
echo "RETAINDB_API_KEY=your-key" >> ~/.agentx/.env
```
...
# Then configure AgentX
agentx memory setup    # select "byterover"
# Or manually:
agentx config set memory.provider byterover
```

**Key features:**
- Automatic pre-compression extraction (saves insights before context compression discards them)
- Knowledge tree stored at `$AGENTX_HOME/byterover/` (profile-scoped)
...
| **Requires** | `pip install supermemory` + [cloud API key](http://app.supermemory.ai/integrations?connect=agentx), or a [self-hosted server](https://supermemory.ai/docs/self-hosting/overview) |
...
**Setup:**
```bash
agentx memory setup    # select "supermemory"
# Or manually:
agentx config set memory.provider supermemory
echo 'SUPERMEMORY_API_KEY=***' >> ~/.agentx/.env
```
...
Before running `agentx memory setup`, set `base_url` in
`$AGENTX_HOME/supermemory.json`:
...
Then run `agentx memory setup` and enter the API key printed by the local
server. Configuring the endpoint first ensures the setup connection probe also
stays local.

**Config:** `$AGENTX_HOME/supermemory.json`
...
| `container_tag` | `agentx` | Container tag used for search and writes. Supports `{identity}` template for profile-scoped tags. |
...
- **Profile-scoped containers** — use `{identity}` in `container_tag` (e.g. `agentx-{identity}` → `agentx-coder`) to isolate memories per AgentX profile
...
| **Requires** | `pip install agentx-memori` + `agentx-memori install` + [Memori API key](https://app.memorilabs.ai/signup) |
...
**Setup:**
```bash
pip install agentx-memori
agentx-memori install
agentx config set memory.provider memori
agentx memory setup
```
...
| **Memori** | Cloud | Free/Paid | 5 | `agentx-memori` | Tool-aware memory + structured recall |

## Profile Isolation

Each provider's data is isolated per [profile](/user-guide/profiles):

- **Local storage providers** (Holographic, ByteRover) use `$AGENTX_HOME/` paths which differ per profile
- **Config file providers** (Honcho, Mem0, Hindsight, Supermemory) store config in `$AGENTX_HOME/` so each profile has its own credentials
