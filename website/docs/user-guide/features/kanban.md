...
description: "Durable SQLite-backed task board for coordinating multiple AgentX profiles"
---
...
AgentX Kanban is a durable task board, shared across all your AgentX profiles, that lets multiple named agents collaborate on work without fragile in-process subagent swarms. Every task is a row in `~/.agentx/kanban.db`; every handoff is a row anyone can read and write; every worker is a full OS process with its own identity.

### Two surfaces: the model talks through tools, you talk through the CLI

The board has two front doors, both backed by the same `~/.agentx/kanban.db`:

- **Agents drive the board through a dedicated `kanban_*` toolset** — `kanban_show`, `kanban_list`, `kanban_complete`, `kanban_block`, `kanban_heartbeat`, `kanban_comment`, `kanban_attach`, `kanban_attach_url`, `kanban_attachments`, `kanban_create`, `kanban_link`, `kanban_unblock`. The dispatcher spawns each worker with these tools already in its schema; orchestrator profiles can also enable the `kanban` toolset explicitly. The model reads and routes tasks by calling tools directly, *not* by shelling out to `agentx kanban`. See [How workers interact with the board](#how-workers-interact-with-the-board) below.
- **You (and scripts, and cron) drive the board through `agentx kanban …`** on the CLI, `/kanban …` as a slash command, or the dashboard. These are for humans and automation — the places without a tool-calling model behind them.
...
For the full design rationale, comparative analysis against Cline Kanban / Paperclip / NanoClaw / Google Gemini Enterprise, and the eight canonical collaboration patterns, see `docs/agentx-kanban-v1-spec.pdf` in the repository.
...
- **Dispatcher** — a long-lived loop that, every N seconds (default 60): reclaims stale claims, reclaims crashed workers (PID gone but TTL not yet expired), promotes ready tasks, atomically claims, spawns assigned profiles. Runs **inside the gateway** by default (`kanban.dispatch_in_gateway: true`). One dispatcher sweeps all boards per tick; workers are spawned with `AGENTX_KANBAN_BOARD` pinned so they can't see other boards. After `kanban.failure_limit` consecutive spawn failures on the same task (default: 2) the dispatcher auto-blocks it with the last error as the reason — prevents thrashing on tasks whose profile doesn't exist, workspace can't mount, etc.
...
agentx kanban boards list

# Create a new board.
agentx kanban boards create atm10-server \
    --name "ATM10 Server" \
...
agentx kanban --board atm10-server list
agentx kanban --board atm10-server create "Restart ATM server" --assignee ops

# Change which board is "current" for subsequent calls.
agentx kanban boards switch atm10-server
agentx kanban boards show             # who's active right now?

# Rename the display name (the slug is immutable — it's the directory name).
agentx kanban boards rename atm10-server "ATM10 (Prod)"
...
2. `AGENTX_KANBAN_BOARD` env var (set by the dispatcher when spawning a
   worker, so workers can't see other boards).
3. `~/.agentx/kanban/current` — the slug persisted by `agentx kanban
   boards switch`.
4. `default`.
...
`agentx dashboard` → Kanban tab shows a board switcher at the top as soon
...
- **Storage** — files land under
  `<agentx-home>/kanban/attachments/<task_id>/` for the default board, or
  `<agentx-home>/kanban/boards/<slug>/attachments/<task_id>/` for a named
  board. Set `AGENTX_KANBAN_ATTACHMENTS_ROOT` to pin a custom location.
...
agentx kanban create "research AI funding landscape" --assignee researcher
...
When the dispatcher picks up `t_abcd` and spawns the `researcher` profile, the very first thing that worker's model does is call `kanban_show()` to read its task. It doesn't run `agentx kanban show t_abcd`.
...
Override the config flag at runtime via `AGENTX_KANBAN_DISPATCH_IN_GATEWAY=0`
for debugging. Standard gateway supervision applies: run `agentx gateway
...
until one comes up — `agentx kanban create` warns about this at creation
time.

Running `agentx kanban daemon` as a separate process is **deprecated**;
...
agentx kanban create "nightly ops review" \
    --assignee ops \
...
```bash
agentx kanban complete t_abc t_def t_hij --result "batch wrap"
agentx kanban archive  t_abc t_def t_hij
agentx kanban unblock  t_abc t_def
agentx kanban block    t_abc "need input" --ids t_def t_hij
```
...
**Workers do not shell out to `agentx kanban`.** When the dispatcher spawns a worker it sets `AGENTX_KANBAN_TASK=t_abcd` in the child's env, and that env var flips on a dedicated **kanban toolset** in the model's schema. The same toolset is also available to orchestrator profiles that enable `kanban` in their toolsets config. These tools read and mutate the board directly via the Python `kanban_db` layer, same as the CLI does. A running worker calls these like any other tool; it never sees or needs the `agentx kanban` CLI.
...
kanban_show()                                     # no args — uses AGENTX_KANBAN_TASK
...
### Why tools instead of shelling to `agentx kanban`

Three reasons:

1. **Backend portability.** Workers whose terminal tool points at a remote backend (Docker / Modal / Singularity / SSH) would run `agentx kanban complete` *inside* the container, where `agentx` isn't installed and `~/.agentx/kanban.db` isn't mounted. The kanban tools run in the agent's own Python process and always reach `~/.agentx/kanban.db` regardless of terminal backend.
...
**Zero schema footprint on normal sessions.** A regular `agentx chat` session has zero `kanban_*` tools in its schema unless the active profile explicitly enables the `kanban` toolset for orchestrator work. Dispatcher-spawned task workers get task-scoped tools because `AGENTX_KANBAN_TASK` is set; orchestrator profiles get the broader routing surface through config. No tool bloat for users who never touch kanban.
...
2. `cd $AGENTX_KANBAN_WORKSPACE` (via the terminal tool) and do the work there.
...
**Agent-side prevention:** Before the worker exits, AgentX injects up to two
...
guard is active only for dispatcher-spawned workers (`AGENTX_KANBAN_TASK` is
set) and can be disabled with `AGENTX_KANBAN_STOP_NUDGE=0`.
...
```bash
agentx kanban create "translate README to Japanese" \
    --assignee linguist \
    --skill translation

agentx kanban create "audit auth flow" \
    --assignee reviewer \
...
The dispatcher emits one `--skills <name>` flag per skill listed, so the worker spawns with all of them loaded on top of the auto-injected kanban guidance. The skill names must match skills that are actually installed on the assignee's profile (run `agentx skills list` to see what's available); there's no runtime install.
...
```bash
# At creation
agentx kanban create "hard refactor" --assignee coder \
    --model claude-opus-4.6 --provider anthropic

# Or later — takes effect on the next dispatch
agentx kanban set-model t_abcd claude-opus-4.6 --provider anthropic
agentx kanban set-model t_abcd none    # clear the override
```
...
```bash
agentx kanban create "Translate the docs site to French" \
...
**Manual** — `kanban.auto_decompose: false`. Triage tasks stay in triage until you act. Click the **⚗ Decompose** button on a card, run `agentx kanban decompose <id>` (or `--all`), or use `/kanban decompose <id>` from a chat. This matches the pre-decomposer behavior of the board, useful when you want full control over what runs when.

Flip between the two modes from the **Orchestration: Auto/Manual** pill at the top of the kanban page (emerald = Auto, muted gray = Manual), or by editing `config.yaml` directly. Both modes coexist with `agentx kanban specify` — that's still available as a single-task spec rewrite when you don't want fan-out.

The decomposer's routing decisions depend on profile descriptions, which is a per-profile labeling primitive you set with `agentx profile create --description "..."`, `agentx profile describe <name> --text "..."`, `agentx profile describe <name> --auto` (LLM-generates from the profile's installed skills + model), or the dashboard's per-profile editor in the expanded **Orchestration settings** panel. Profiles without a description still appear in the roster — they're routable by name, just less precisely. The decomposer NEVER lands a child task with `assignee=None`: when the LLM picks an unknown profile, the child gets routed to `kanban.default_assignee` (or the active default profile if that's unset).
...
Config knobs (all under `kanban:` in `~/.agentx/config.yaml`):
...
| `auxiliary.profile_describer` | Model that auto-generates profile descriptions (called by `agentx profile describe --auto`). |
...
└──────────┬─────────────┘                                    │
           ...
           ▼                                                  │
┌────────────────────────┐                                    │
│  ~/.agentx/kanban.db   │ ───── append task_events ──────────┘
│  (WAL, shared)         │
└────────────────────────┘
```
...
The plugin uses the standard AgentX dashboard plugin contract — see [Extending the Dashboard](./extending-the-dashboard) for the full manifest reference, shell slots, page-scoped slots, and the Plugin SDK. Extra columns, custom card chrome, tenant-filtered layouts, or full `tab.override` replacements are all expressible without forking this plugin.
...
```
agentx kanban init                                     # create kanban.db + print daemon hint
agentx kanban create "<title>" [--body ...] [--assignee <profile>]
                                ...
                                [--skill <name>]...
                                [--json]
agentx kanban list [--mine] [--assignee P] [--status S] [--tenant T] [--archived]
        ...
        [--json]
agentx kanban show <id> [--json]
agentx kanban assign <id> <profile>                    # or 'none' to unassign
agentx kanban reassign <id>... <profile>               # bulk re-assign tasks to a profile
agentx kanban edit <id> [--title ...] [--body ...]     # edit task title / body / priority in place
        [--priority N]
agentx kanban promote <id>...                          # move todo/blocked tasks to ready (recovery)
agentx kanban schedule <id> --at <ISO8601>             # set/clear a task's scheduled_at start time
agentx kanban diagnostics [--json]                     # board health snapshot (alias: diag)
agentx kanban link <parent_id> <child_id>
agentx kanban unlink <parent_id> <child_id>
agentx kanban claim <id> [--ttl SECONDS]
agentx kanban comment <id> "<text>" [--author NAME]

# Bulk verbs — accept multiple ids:
agentx kanban complete <id>... [--result "..."]
agentx kanban block <id> "<reason>" [--ids <id>...]
agentx kanban unblock <id>...
agentx kanban archive <id>...

agentx kanban tail <id>                                # follow a single task's event stream
agentx kanban watch [--assignee P] [--tenant T]        # live stream ALL events to the terminal
        [--kinds completed,blocked,…] [--interval SECS]
agentx kanban heartbeat <id> [--note "..."]            # worker liveness signal for long ops
agentx kanban runs <id> [--json]                       # attempt history (one row per run)
agentx kanban assignees [--json]                       # profiles on disk + per-assignee task counts
agentx kanban dispatch [--dry-run] [--max N]           # one-shot pass
        [--failure-limit N] [--json]
agentx kanban daemon --force                           # DEPRECATED — standalone dispatcher (use `agentx gateway start` instead)
        [--failure-limit N] [--pidfile PATH] [-v]
agentx kanban stats [--json]                           # per-status + per-assignee counts
agentx kanban log <id> [--tail BYTES]                  # worker log from ~/.agentx/kanban/logs/
agentx kanban notify-subscribe <id>                    # gateway bridge hook (used by /kanban in the gateway)
        --platform <name> --chat-id <id> [--thread-id <id>] [--user-id <id>]
agentx kanban notify-list [<id>] [--json]
agentx kanban notify-unsubscribe <id>
        --platform <name> --chat-id <id> [--thread-id <id>]
agentx kanban context <id>                             # what a worker sees
agentx kanban specify [<id> | --all] [--tenant T]      # flesh out a triage-column idea
        [--author NAME] [--json]                       #   into a full spec and promote to todo
agentx kanban gc [--event-retention-days N]            # workspaces + old events + old logs
...
```bash
agentx kanban create "nightly backup audit" \
...
Every `agentx kanban <action>` verb is also reachable as `/kanban <action>` — from inside an interactive `agentx chat` session **and** from any gateway platform (Telegram, Discord, Slack, WhatsApp, Signal, Matrix, Mattermost, email, SMS). Both surfaces call the exact same `hermes_cli.kanban.run_slash()` entry point that reuses the `agentx kanban` argparse tree, so the argument surface, flags, and output format are identical across CLI, `/kanban`, and `agentx kanban`. You don't have to leave the chat to drive the board.
...
The gateway normally queues slash commands and user messages while an agent is still thinking — that's what stops you from accidentally starting a second turn while the first is in flight. **`/kanban` is explicitly exempted from this guard.** The board lives in `~/.agentx/kanban.db`, not in the running agent's state, so reads (`list`, `show`, `context`, `tail`, `watch`, `stats`, `runs`) and writes (`comment`, `unblock`, `block`, `assign`, `archive`, `create`, `link`, …) all go through immediately, even mid-turn.
...
Gateway platforms have practical message-length caps. If `/kanban list`, `/kanban show`, or `/kanban tail` produce more than ~3800 characters of output, the response is truncated with a `… (truncated; use \`agentx kanban …\` in your terminal for full output)` footer. The CLI surface has no such cap.
...
| **P9 Triage specifier** | rough idea → `triage` → `agentx kanban specify` expands body → `todo` | "turn this one-liner into a spec'd task" |

For worked examples of each, see `docs/agentx-kanban-v1-spec.pdf`.
...
[`merge-reconciler` skill](https://github.com/TrungKiencding/AgentX-Workmate/blob/main/skills/autonomous-ai-agents/merge-reconciler/SKILL.md)
...
```bash
agentx kanban create "monthly report" \
    --assignee researcher \
    --tenant business-a \
    --workspace dir:~/tenants/business-a/data/
```

Workers receive `$AGENTX_TENANT` and namespace their memory writes by prefix. The board, the dispatcher, and the profile definitions are all shared; only the data is scoped.
...
```bash
agentx kanban notify-subscribe t_abcd \
    --platform telegram --chat-id 12345678 --thread-id 7
agentx kanban notify-list
agentx kanban notify-unsubscribe t_abcd \
    --platform telegram --chat-id 12345678 --thread-id 7
```
...
guide](https://github.com/TrungKiencding/AgentX-Workmate/blob/main/docs/kanban/multi-gateway.md)),
...
```bash
agentx kanban complete t_abcd \
    --result "rate limiter shipped" \
    --summary "implemented token bucket, keys on user_id with IP fallback, all tests pass" \
    --metadata '{"changed_files": ["limiter.py", "tests/test_limiter.py"], "tests_run": 14}'

# Review the attempt history on a retried task:
agentx kanban runs t_abcd
...
**Bulk close caveat.** `agentx kanban complete a b c --summary X` is refused — structured handoff is per-run, so copy-pasting the same summary to N tasks is almost always wrong. Bulk close *without* `--summary` / `--metadata` still works for the common "I finished a pile of admin tasks" case.
...
Every transition appends a row to `task_events`. Each row carries an optional `run_id` so UIs can group events by attempt. Kinds group into three clusters so filtering is easy (`agentx kanban watch --kinds completed,gave_up,timed_out`):
...
| `heartbeat` | `{note?}` | Worker called `agentx kanban heartbeat $TASK` to signal liveness during long operations. |
...
`agentx kanban tail <id>` shows these for a single task. `agentx kanban watch` streams them board-wide.

## Out of scope

Kanban is deliberately single-host. `~/.agentx/kanban.db` is a local SQLite file and the dispatcher spawns workers on the same machine. Running a shared board across two hosts is not supported — there's no coordination primitive for "worker X on host A, worker Y on host B," and the crash-detection path assumes PIDs are host-local. If you need multi-host, run an independent board per host and use `delegate_task` / a message queue to bridge them.

## Design spec

The complete design — architecture, concurrency correctness, comparison with other systems, implementation plan, risks, open questions — lives in `docs/agentx-kanban-v1-spec.pdf`. Read that before filing any behavior-change PR.
