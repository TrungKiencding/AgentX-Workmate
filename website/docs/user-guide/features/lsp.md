...
AgentX runs full language servers — pyright, gopls, rust-analyzer,
...
This is the same architecture top-tier coding agents use. AgentX
...
1. AgentX captures a baseline of current diagnostics for the file.
...
…). AgentX auto-detects the binary on PATH or in
`<AGENTX_HOME>/lsp/bin/`.
...
2. Download the latest release zip from
   ...
   and extract it.
3. Point AgentX at the extracted bundle — the directory that contains
   `PowerShellEditorServices/Start-EditorServices.ps1`. Either:
   - set `lsp.servers.powershell.command: ["/path/to/bundle"]` in
     `config.yaml`, or
   - extract it to `<AGENTX_HOME>/lsp/PowerShellEditorServices`, or
   - export `PSES_BUNDLE_PATH=/path/to/bundle`.

`agentx lsp status` reports `installed` once `pwsh` is found; if the
...
`node_modules` tree — AgentX installs both packages together when you
run `agentx lsp install typescript` or auto-install fires on first
use.

## CLI

```
agentx lsp status          # service state + per-server install status
agentx lsp list            # registry, optionally --installed-only
agentx lsp install <id>    # eagerly install one server
agentx lsp install-all     # try every server with a known recipe
agentx lsp restart         # tear down running clients
agentx lsp which <id>      # print resolved binary path
```

`agentx lsp status` is the best starting point — it shows which
...
lsp:
  ...
  #   auto    — install via npm/pip/go install into <AGENTX_HOME>/lsp/bin
...
When `install_strategy: auto`, AgentX installs binaries into
`<AGENTX_HOME>/lsp/bin/`. NPM packages land in
`<AGENTX_HOME>/lsp/node_modules/` with bin symlinks one level up.
Go binaries come from `go install` with `GOBIN` pointed at the
staging dir.

Nothing is ever installed to `/usr/local/`, `~/.local/`, or any other
shared location — the staging dir is fully AgentX-owned and is
...
**`agentx lsp status` shows a server as "missing"**

The binary isn't on PATH and isn't in `<AGENTX_HOME>/lsp/bin/`. Run
`agentx lsp install <server_id>` to attempt an auto-install, or
install the binary manually through the language's normal toolchain.

**`Backend warnings` section in `agentx lsp status`**
...
When `agentx lsp status` shows a `Backend warnings` section, install
...
`~/.agentx/logs/agent.log`.

**Server starts but never returns diagnostics**

Check `~/.agentx/logs/agent.log` for `[agent.lsp.client]` entries —
...
the rest of the session. Run `agentx lsp restart` to clear the set;
