...
- Configure Hermes provider credentials first with `hermes model`, or set them in `~/.agentx/.env` / `~/.agentx/config.yaml`.
...
- The ACP extra is installed (`cd ~/.agentx/hermes-agent && uv pip install -e '.[acp]'`).
...
or by editing `~/.agentx/.env`. The terminal auth flow (`hermes acp --setup`) can also trigger the interactive provider/model setup.
