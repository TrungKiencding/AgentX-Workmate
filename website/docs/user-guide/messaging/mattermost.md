...
sidebar_position: 8
title: "Mattermost"
description: "Set up AgentX Workmate as a Mattermost bot"
---
...
| **Public/private channels** | AgentX responds when you `@mention` it. Without a mention, AgentX ignores the message. |
| **Threads** | If `MATTERMOST_REPLY_MODE=thread`, AgentX replies in a thread under your message. Thread context stays isolated from the parent channel. |
| **Shared channels with multiple users** | By default, AgentX isolates session history per user inside the channel. Two people talking in the same channel do not share one transcript unless you explicitly disable that. |

:::tip
If you want AgentX to reply as threaded conversations (nested under your original message), set `MATTERMOST_REPLY_MODE=thread`. The default is `off`, which sends flat messages in the channel.
:::
...
3. Fill in the details:
   - **Username**: e.g., `agentx`
   - **Display Name**: e.g., `AgentX Workmate`
...
:::tip
You can also use a **personal access token** instead of a bot account. Go to **Profile** → **Security** → **Personal Access Tokens** → **Create Token**. This is useful if you want AgentX to post as your own user rather than a separate bot user.
:::
...
3. Search for your bot username (e.g., `agentx`) and add it.
...
AgentX Workmate uses your Mattermost User ID to control who can interact with the bot. To find it:
...
## Step 5: Configure AgentX Workmate
...
The `MATTERMOST_REPLY_MODE` setting controls how AgentX posts responses:

| Mode | Behavior |
|------|----------|
| `off` (default) | AgentX posts flat messages in the channel, like a normal user. |
| `thread` | AgentX replies in a thread under your original message. Keeps channels clean when there's lots of back-and-forth. |

Set it in your `~/.agentx/.env`:
...
**Fix**: Check that `agentx gateway` is running. Look at the terminal output for error messages. Common issues: wrong URL, expired token, Mattermost server unreachable.

### "User not allowed" / Bot ignores you

**Cause**: Your User ID isn't in `MATTERMOST_ALLOWED_USERS`.

**Fix**: Add your User ID to `MATTERMOST_ALLOWED_USERS` in `~/.agentx/.env` and restart the gateway. Remember: the User ID is a 26-character alphanumeric string, not your `@username`.
...
- **No extra dependencies**: The adapter uses `aiohttp` for HTTP and WebSocket, which is already included with AgentX Workmate.
