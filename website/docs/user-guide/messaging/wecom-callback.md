...
Connect AgentX to WeCom (Enterprise WeChat) as a self-built enterprise application using the callback/webhook model.

:::info WeCom Bot vs WeCom Callback
AgentX supports two WeCom integration modes:
...
> Run `agentx gateway setup` and pick **WeCom Callback** for a guided walk-through.
...
3. AgentX decrypts the message, queues it for the agent
...
(Use `agentx gateway start` only after `agentx gateway install` has registered the systemd/launchd service.)
...
console. A mismatch between the token configured in AgentX and the token the
...
in `~/.agentx/.env` values around `=` will also break signature checks. After
fixing, restart `agentx gateway run`.
...
Check `agentx gateway run` logs for the bound host/port. If the adapter bound to
