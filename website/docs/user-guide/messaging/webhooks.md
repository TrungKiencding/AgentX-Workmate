...
1. Enable via `agentx gateway setup` or environment variables
2. Define routes in `config.yaml` **or** create them dynamically with `agentx webhook subscribe`
...
| `script` | No | Filter/transform script under `~/.agentx/scripts/`. The webhook payload is passed as JSON on stdin. JSON object stdout replaces the payload before templating; text stdout is exposed as `script_output`; empty stdout, `[SILENT]`, or a nonzero exit code ignores the webhook. |
...
Use `script` when declarative filters are not enough. Scripts must live under `~/.agentx/scripts/` for the active profile; relative paths resolve there, and path traversal outside that directory is blocked. `.sh` and `.bash` scripts run with bash, and all other extensions run with the current Python interpreter.
...
Add the `github-pr` route to your `~/.agentx/config.yaml` as shown in the example above.
...
Open a pull request on the repository. The webhook fires, AgentX processes the event, and posts a review comment on the PR.
...
In addition to static routes in `config.yaml`, you can create webhook subscriptions dynamically using the `agentx webhook` CLI command. This is especially useful when the agent itself needs to set up event-driven triggers.
...
- Subscriptions are stored in `~/.agentx/webhook_subscriptions.json`
...
The agent can create subscriptions via the terminal tool when guided by the `webhook-subscriptions` skill. Ask the agent to "set up a webhook for GitHub issues" and it will run the appropriate `agentx webhook subscribe` command.
...
- Run the gateway in foreground to see logs: `agentx gateway run`
