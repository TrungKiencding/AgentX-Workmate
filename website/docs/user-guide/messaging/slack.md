...
sidebar_position: 4
title: "Slack"
description: "Set up AgentX Workmate as a Slack bot using Socket Mode"
---

# Slack Setup

Connect AgentX Workmate to Slack as a bot using Socket Mode. Socket Mode uses WebSockets instead of
public HTTP endpoints, so your AgentX instance doesn't need to be publicly accessible — it works
behind firewalls, on your laptop, or on a private server.

:::warning Classic Slack Apps Deprecated
Classic Slack apps (using RTM API) were **fully deprecated in March 2025**. AgentX uses the modern
...
The fastest path is to paste a manifest AgentX generates for you. It
...
1. Generate the manifest. New Slack apps must use Agent view:
   ```bash
   agentx slack manifest --agent-view --write
   ```
   This writes `~/.agentx/slack-manifest.json` and prints paste-in
   ...
   ```bash
   agentx slack manifest --agent-view \
...
4. Enter an app name (e.g., "AgentX Workmate") and select your workspace
5. Click **Create App**
...
it will only work in DMs. Without `files:read`, AgentX can chat but **cannot reliably read user-uploaded attachments**.
...
3. You'll be prompted to create an **App-Level Token**:
   - Name it something like `agentx-socket` (the name doesn't matter)
...
AgentX uses Slack **Member IDs** (not usernames or display names) for the allowlist.
...
```bash
agentx gateway setup    # Select Slack when prompted
```

Then start the gateway:

```bash
agentx gateway              # Foreground
agentx gateway install      # Install as a user service
sudo agentx gateway install --system   # Linux only: boot-time system service
```
...
can spend the entire turn in hidden reasoning and never produce visible assistant text; AgentX now
...
Every AgentX command (`/btw`, `/stop`, `/new`, `/model`, `/help`, ...)
...
AgentX command with its description.

Under the hood: AgentX ships with a generated Slack app manifest (see
Step 1, Option A) that declares every command in
[`COMMAND_REGISTRY`](https://github.com/TrungKiencding/AgentX-Workmate/blob/main/hermes_cli/commands.py)
...
New Slack apps use Slack's **Agent** messaging experience. Existing AgentX
...
to `message.im`, `app_home_opened`, and `app_context_changed`, so AgentX can
identify a Messages-tab DM and receive the user's active Slack context with a
turn. AgentX only supplies that context as a label; it does not read the viewed
channel's history.

### Refreshing slash commands after updates

When AgentX adds new commands (e.g. after `agentx update`), regenerate
the manifest and update your Slack app:

```bash
agentx slack manifest --write
```
...
3. Paste the new contents of `~/.agentx/slack-manifest.json`
...
Slack never delivers them to AgentX.

As a workaround, AgentX recognises a leading `!` as an alternate
command prefix that works in threads (and anywhere else). Type
`!queue`, `!stop`, `!model gpt-5.4`, etc. as a regular thread reply —
AgentX treats it identically to the slash form and replies in the same
thread.
...
The bang form also works behind a mention (`@AgentX !stop`) and with
...
AgentX falls back to a text prompt, the prompt instructs you to reply
...
fails, AgentX retries via a second ephemeral API path — a slash reply is never
posted publicly to the channel as a fallback. (Commands typed as regular
messages — `!cmd` in threads, `@AgentX /cmd` — reply as normal visible
messages instead.)
...
```bash
agentx slack manifest --slashes-only > /tmp/slashes.json
```
...
Understanding how AgentX behaves in different contexts:

| Context | Behavior |
|---------|----------|
| **DMs** | Bot responds to every message — no @mention needed |
| **Channels** | Bot **only responds when @mentioned** (e.g., `@AgentX Workmate what time is it?`). In channels, AgentX replies in a thread attached to that message. |
| **Threads** | If you @mention AgentX inside an existing thread, it replies in that same thread. Once the bot has an active session in a thread, **subsequent replies in that thread do not require @mention** — the bot follows the conversation naturally. |
...
Beyond the required environment variables from Step 8, you can customize Slack bot behavior through `~/.agentx/config.yaml`.
...
platforms:
  slack:
    ...
    extra:
      ...
      # that message itself @mentions AgentX, and "all" accepts every
      # other bot. AgentX always ignores its own bot user to prevent
      # self-echoes.
      allow_bots: "none"
...
| `platforms.slack.extra.allow_bots` | `"none"` | Controls messages from other Slack bots: `"none"` ignores them, `"mentions"` accepts a bot message only when **that message itself** @mentions AgentX, and `"all"` accepts all of them. Use `"mentions"` for the safest bot-to-bot collaboration mode. See [Accepting messages from other bots](#accepting-messages-from-other-bots-allow_bots). |
...
name in the thread. By default AgentX sets it to `is thinking...`; customize it
...
(`platforms.google_chat.typing_status_text`, default `"AgentX is thinking…"`) —
...
When `true` (the default), each user in a shared channel gets their own isolated conversation session. Two people talking to AgentX in `#general` will have separate histories and contexts.
...
slack:
  ...
  # must @mention the bot before AgentX will respond.
...
By default AgentX ignores every message authored by another Slack bot or app (including Workflow Builder posts). For multi-agent workspaces — several AgentX instances or peer bots collaborating in one channel — opt in with `allow_bots`:
...
- AgentX always ignores its own messages, in every mode, to prevent self-echo loops.

`mentions` is the recommended mode for bot-to-bot collaboration: each agent must explicitly summon the other per turn. Avoid `all` unless every peer bot's own reply policy is loop-safe — two bots that answer everything will answer each other forever. Detection covers labeled bot messages (`bot_id`, `subtype: bot_message`), app-originated events, and unlabeled bot *users* (probed via `users.info`), so peer AgentX agents are filtered consistently across workspaces.
...
manifest — regenerate with `agentx slack manifest`):
...
Set `SLACK_HOME_CHANNEL` to a channel ID where AgentX will deliver scheduled messages,
...
Make sure the bot has been **invited to the channel** (`/invite @AgentX Workmate`).
...
Delivery works even when the cron process isn't co-located with the gateway — AgentX falls back to a standalone Web API sender using `SLACK_BOT_TOKEN`. `MEDIA:` attachments in the cron output are uploaded as native Slack file shares to the same target.
...
AgentX can connect to **multiple Slack workspaces** simultaneously using a single gateway instance. Each workspace is authenticated independently with its own bot user ID.
...
In addition to tokens in the environment or config, AgentX also loads tokens from an **OAuth token file** at:
...
- When a message arrives, AgentX uses the correct workspace-specific client to respond.
...
AgentX supports voice on Slack:
...
| Bot works in DMs but not in channels | **Most common issue.** Add `message.channels` and `message.groups` to event subscriptions, reinstall the app, and invite the bot to the channel with `/invite @AgentX Workmate` |
...
| Bot responds but can't post in a channel | Invite the bot to the channel with `/invite @AgentX Workmate` |
| Bot can chat but can't read uploaded images/files | Add `files:read`, then **reinstall** the app. AgentX now surfaces attachment access diagnostics in-chat when Slack returns scope/auth/permission failures. |
...
7. ✅ Bot was **invited** to the channel (`/invite @AgentX Workmate`)
...
- Tokens should be stored in `~/.agentx/.env` (file permissions `600`)
- Rotate tokens periodically via the Slack app settings
- Audit who has access to your AgentX config directory
