...
Or set environment variables directly in `~/.agentx/.env`:
...
**Pre-authorize specific users** (in `~/.agentx/.env`):
```bash
BLUEBUBBLES_ALLOWED_USERS=user@icloud.com,+15551234567
```

**Open access** (in `~/.agentx/.env`):
```bash
...
Auto-marking messages as read is controlled by the `send_read_receipts` key under `platforms.bluebubbles.extra` in `~/.agentx/config.yaml` (default: `true`). There is no corresponding environment variable.
