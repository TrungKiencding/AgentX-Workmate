...
description: Control your smart home with AgentX Workmate via Home Assistant integration.
...
AgentX Workmate integrates with [Home Assistant](https://www.home-assistant.io/) in two ways:
...
4. Click **Create Token**, give it a name like "AgentX Workmate"
5. Copy the token
...
AgentX Workmate registers four tools for smart home control:
...
Configure which events the agent sees in `~/.agentx/config.yaml` under the Home Assistant platform's `extra` section:
...
Outbound messages from the agent are delivered as **Home Assistant persistent notifications** (via `persistent_notification.create`). These appear in the HA notification panel with the title "AgentX Workmate".
...
The adapter reads credentials from `~/.agentx/.env` (auto-merged at startup) or
from `config.yaml`. Double-check the file lives under the active AgentX profile
...
running AgentX — `curl -H "Authorization: Bearer <token>" <url>/api/` should
