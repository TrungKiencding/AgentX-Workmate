...
`~/.agentx/config.yaml`, `~/.agentx/.env`, and even the shell environment — for
...
```text
/etc/hermes/
├── config.yaml     # managed config layer (wins over ~/.agentx/config.yaml)
└── .env            # managed env layer (wins over ~/.agentx/.env + shell)
```
...
The location can be relocated with the `AGENTX_MANAGED_DIR` environment variable
(for containers or non-`/etc` deployments). This is a deployment/bootstrap path
knob — like `AGENTX_HOME` — set by the same administrator who owns the managed
...
export AGENTX_MANAGED_DIR=/opt/org/hermes-policy
```

:::warning
A user who can set `AGENTX_MANAGED_DIR` can repoint managed scope at a directory
...
| 2 | `~/.agentx/config.yaml` (user) | `~/.agentx/.env` (user) |
