...
title: "Git Worktrees"
description: "Run multiple AgentX agents safely on the same repository using git worktrees and isolated checkouts"
---

# Git Worktrees

AgentX Workmate is often used on large, long‑lived repositories. When you want to:
...
This page shows how to combine worktrees with AgentX so each session has a clean, isolated working directory.

## Why Use Worktrees with AgentX?

AgentX treats the **current working directory** as the project root:

- CLI: the directory where you run `agentx` or `agentx chat`
- Messaging gateways: the directory set by `terminal.cwd` in `~/.agentx/config.yaml`
...
git worktree add ../repo-feature feature/agentx-experiment
```

This creates:

- A new directory: `../repo-feature`
- A new branch: `feature/agentx-experiment` checked out in that directory

Now you can `cd` into the new worktree and run AgentX there:

```bash
cd ../repo-feature

# Start AgentX in the worktree
agentx
```
...
git worktree add ../repo-experiment-a feature/agentx-a
git worktree add ../repo-experiment-b feature/agentx-b
```
...
- Works on its own branch (`feature/agentx-a` vs `feature/agentx-b`).
...
- AgentX checkpoint data under `~/.agentx/checkpoints/` is not automatically pruned when you remove a worktree, but it is usually very small.

## Best Practices

- **One worktree per AgentX experiment**
...
- **Name branches after the experiment**
  - e.g. `feature/agentx-checkpoints-docs`, `feature/agentx-refactor-tests`.
- **Commit frequently**
...
- **Avoid running AgentX from the bare repo root when using worktrees**
  - Prefer the worktree directories instead, so each agent has a clear scope.

## Using `agentx -w` (Automatic Worktree Mode)

AgentX has a built‑in `-w` flag that **automatically creates a disposable git worktree** with its own branch. You don't need to set up worktrees manually — just `cd` into your repo and run:
...
```bash
agentx -w -z "Fix issue #123"
```

For parallel agents, open multiple terminals and run `agentx -w` in each — every invocation gets its own worktree and branch automatically.

## Putting It All Together

- Use **git worktrees** to give each AgentX session its own clean checkout.
