...
| Related skills | [`claude-code`](/docs/user-guide/skills/bundled/autonomous-ai-agents/autonomous-ai-agents-claude-code), [`agentx-agent`](/docs/user-guide/skills/bundled/autonomous-ai-agents/autonomous-ai-agents-agentx-agent) |
...
Delegate coding tasks to [Codex](https://github.com/openai/codex) via the AgentX terminal. Codex is OpenAI's autonomous coding agent CLI.
...
For AgentX itself, `model.provider: openai-codex` uses AgentX-managed Codex
OAuth from `~/.agentx/auth.json` after `agentx auth add openai-codex`. For the
...
## AgentX Gateway Caveat

When invoking the Codex CLI from an AgentX gateway/service context (for example,
