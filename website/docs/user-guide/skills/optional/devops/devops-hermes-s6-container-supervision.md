...
# AgentX s6-overlay Container Supervision
...
If you're just running the AgentX Workmate and want to use Docker, see `website/docs/user-guide/docker.md` instead.
...
│   │   ├── type        ("longrun")
│   │   ├── run         ("#!/command/with-contenv sh ... exec s6-setuidgid agentx agentx -p coder gateway run")
...
| `hermes_cli/gateway.py::_dispatch_via_service_manager_if_s6` | Intercepts `agentx gateway start/stop/restart` and routes to s6 when running in a container. |

## Why Architecture B (CMD as main program, not s6-supervised)

The original plan (v1–v3) called for main agentx to run as a supervised s6-rc service. Two real s6-overlay v3 mechanics blocked that:
...
2. Drop to agentx via `s6-setuidgid agentx` at the top of run (unless you specifically need root).
...
The cont-init reconciler runs as agentx (`s6-setuidgid agentx` in `02-reconcile-profiles`). If a profile dir ends up root-owned (e.g. because `docker exec <c> agentx profile create …` ran as root by default), the reconciler can't read SOUL.md and fails with `PermissionError`. Mitigation: `stage2-hook.sh` chowns `$AGENTX_HOME/profiles` to agentx on **every** boot, idempotently. Don't remove that block.

### Files written by `docker exec` are root-owned

`docker exec` defaults to root. Either pass `--user agentx` or rely on the stage2 chown sweep next reboot. Don't write files under `$AGENTX_HOME/profiles/<name>/` as root manually — the next reconcile pass will sweep them but in-flight operations may hit perm errors.
...
Most likely the profile has no model or auth configured. The service slot is correct — the gateway itself is unconfigured. Run `agentx -p <profile> setup` first. The s6 supervisor will keep restarting it; that's the desired behavior (when you fix the config, the next attempt succeeds and stays up).

### Reconciler skipped a profile

The reconciler keys on the **presence of `SOUL.md`** as the "real profile" marker. `agentx profile create` always seeds it. If a profile dir is missing SOUL.md (stray directory, partial restore, backup-in-progress), the reconciler skips it intentionally. Add a `SOUL.md` (even empty) to opt back in.
