...
| | |
|---|---|
| Source | Optional — install with `agentx skills install official/creative/unreal-mcp` |
...
Companion skill for the `unreal-engine` entry in the AgentX MCP catalog. The
...
Two halves, in this order: the editor side must be up before AgentX connects.
...
### One-time, AgentX side

    agentx mcp install unreal-engine
...
`~/.agentx/config.yaml` under `mcp_servers.unreal-engine` to match.

Do NOT use `ModelContextProtocol.GenerateClientConfig` for AgentX — that
writes `.mcp.json`-style files for Claude Code/Cursor/etc. AgentX connects
...
2. Start the AgentX session. Tools register as `mcp_unreal_engine_*`. If
...
three meta-tools, and every real tool is reached through them. Through AgentX
they appear as:
...
then happens at `agentx mcp install`/`configure` time instead. Tool-search
...
3. **NEVER issue overlapping calls.** Do not batch multiple
   `mcp_unreal_engine_*` calls in one turn — AgentX runs batched calls
...
- **Start order matters.** Editor + server up first, then the AgentX
...
- **Timeouts on long operations.** AgentX' per-call default is 120 s; asset
  imports, big level saves, and renders can exceed it. Raise
  `mcp_servers.unreal-engine.timeout` in `~/.agentx/config.yaml` for
