...
| | |
|---|---|
| Source | Optional — install with `agentx skills install official/blockchain/hyperliquid` |
...
The script reads `${AGENTX_HOME:-~/.agentx}/.env` for two optional defaults:
...
Helper script: `~/.agentx/skills/blockchain/hyperliquid/scripts/hyperliquid_client.py`
...
optional when `HYPERLIQUID_USER_ADDRESS` is set in `${AGENTX_HOME:-~/.agentx}/.env`.
