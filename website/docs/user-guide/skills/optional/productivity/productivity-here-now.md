...
| | |
|---|---|
| Source | Optional — install with `agentx skills install official/productivity/here-now` |
...
- Skill helper paths:
  - `${AGENTX_SKILL_DIR}/scripts/publish.sh` for publishing sites
  - `${AGENTX_SKILL_DIR}/scripts/drive.sh` for private Drive storage

## Create a site

```bash
PUBLISH="${AGENTX_SKILL_DIR}/scripts/publish.sh"
bash "$PUBLISH" {file-or-dir} --client agentx
```
...
```bash
PUBLISH="${AGENTX_SKILL_DIR}/scripts/publish.sh"
bash "$PUBLISH" {file-or-dir} --slug {slug} --client agentx
```
...
```bash
DRIVE="${AGENTX_SKILL_DIR}/scripts/drive.sh"
bash "$DRIVE" default
...
| `--client {name}`      | Agent name for attribution (e.g. `agentx`)    |
