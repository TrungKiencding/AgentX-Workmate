---
title: "Openclaw Migration — Import an OpenClaw setup (memories, skills) into AgentX"
sidebar_label: "Openclaw Migration"
description: "Import an OpenClaw setup (memories, skills) into AgentX"
---
...
Import an OpenClaw setup (memories, skills) into AgentX.
...
Use this skill when a user wants to move their OpenClaw setup into AgentX Workmate with minimal manual cleanup.
...
```bash
agentx claw migrate              # Full interactive migration
...
agentx claw migrate --source /custom/path/.openclaw  # Custom source
```
...
**First-time setup:** The `agentx setup` wizard automatically detects `~/.openclaw` and offers migration before configuration begins.

## What this skill does

It uses `scripts/openclaw_to_hermes.py` to:

- import `SOUL.md` into the AgentX home directory as `SOUL.md`
- transform OpenClaw `MEMORY.md` and `USER.md` into AgentX memory entries
- merge OpenClaw command approval patterns into AgentX `command_allowlist`
- migrate AgentX-compatible messaging settings such as `TELEGRAM_ALLOWED_USERS`, and map OpenClaw workspace settings to AgentX working-directory configuration
- copy OpenClaw skills into `~/.agentx/skills/openclaw-imports/`
- optionally copy the OpenClaw workspace instructions file into a chosen AgentX workspace
- mirror compatible workspace assets such as `workspace/tts/` into `~/.agentx/tts/`
- archive non-secret docs that do not have a direct AgentX destination
...
Do not guess a shorter path like `~/.agentx/skills/openclaw-migration/...`.

Before running the helper:

1. Prefer the installed path under `~/.agentx/skills/migration/openclaw-migration/`.
...
With `--migrate-secrets`, it will also import a small allowlisted set of AgentX-compatible secrets, currently:
...
AgentX CLI supports the `clarify` tool for interactive prompts, but it is limited to:
...
13. If a renamed imported skill has an empty `details.backup`, do not imply the existing AgentX skill was renamed or backed up. Say only that the imported copy was placed in the new destination and reference `details.renamed_from` as the pre-existing folder that remained in place.
...
2. Do not migrate secrets by default. Tokens, auth blobs, device credentials, and raw gateway config should stay out of AgentX unless the user explicitly asks for secret migration.
3. Do not silently overwrite non-empty AgentX targets unless the user explicitly wants that. The helper script will preserve backups when overwriting is enabled.
...
6. Even in secret-migration mode, only migrate secrets with a clean AgentX destination. Unsupported auth blobs must still be reported as skipped.
...
- AgentX persona state imported
- AgentX memory files populated with converted OpenClaw knowledge
- OpenClaw skills available under `~/.agentx/skills/openclaw-imports/`
