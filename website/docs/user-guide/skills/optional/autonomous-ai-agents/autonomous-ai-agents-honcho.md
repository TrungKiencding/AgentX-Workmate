---
title: "Honcho — Configure and troubleshoot Honcho memory for AgentX"
sidebar_label: "Honcho"
description: "Configure and troubleshoot Honcho memory for AgentX"
---
...
Configure and troubleshoot Honcho memory for AgentX.

## Skill metadata

| | |
|---|---|
| Source | Optional — install with `agentx skills install official/autonomous-ai-agents/honcho` |
...
# Honcho Memory for AgentX

Honcho provides AI-native cross-session user modeling. It learns who the user is across conversations and gives every AgentX profile its own peer identity while sharing a unified view of the user.
...
```bash
agentx honcho status    # shows resolved config, connection test, peer info
```
...
3. **AI peer card** -- the identity card for this AgentX profile's AI peer
...
Honcho models conversations as interactions between **peers**. AgentX creates two peers per session:

- **User peer** (`peerName`): represents the human. Honcho builds a user representation from observed messages.
- **AI peer** (`aiPeer`): represents this AgentX instance. Each profile gets its own AI peer so agents develop independent views.
...
| `per-session` | New Honcho session each AgentX run |
| `global` | Single session across all directories |

Manual override: `agentx honcho map my-project-name`
...
Each AgentX profile gets its own Honcho AI peer while sharing the same workspace (user context). This means:
...
# creates host block agentx.coder, AI peer "coder", inherits config from default
```

What `--clone` does for Honcho:
1. Creates a `agentx.coder` host block in `honcho.json`
...
```bash
agentx honcho sync    # creates host blocks for all profiles that don't have one yet
```
...
Guidelines for AgentX when Honcho memory is active.
...
Config file: `$AGENTX_HOME/honcho.json` (profile-local) or `~/.honcho/config.json` (global).
...
Run `agentx honcho setup`. Ensure `memory.provider: honcho` is in `~/.agentx/config.yaml`.

### Memory not persisting across sessions
Check `agentx honcho status` -- verify `saveMessages: true` and `writeFrequency` isn't `session` (which only writes on exit).

### Profile not getting its own peer
Use `--clone` when creating: `agentx profile create <name> --clone`. For existing profiles: `agentx honcho sync`.
...
| Command | Description |
|---------|-------------|
| `agentx honcho setup` | Interactive setup wizard (cloud/local, identity, observation, recall, sessions) |
| `agentx honcho status` | Show resolved config, connection test, peer info for active profile |
| `agentx honcho enable` | Enable Honcho for the active profile (creates host block if needed) |
| `agentx honcho disable` | Disable Honcho for the active profile |
| `agentx honcho peer` | Show or update peer names (`--user <name>`, `--ai <name>`, `--reasoning <level>`) |
| `agentx honcho peers` | Show peer identities across all profiles |
| `agentx honcho mode` | Show or set recall mode (`hybrid`, `context`, `tools`) |
| `agentx honcho tokens` | Show or set token budgets (`--context <N>`, `--dialectic <N>`) |
| `agentx honcho sessions` | List known directory-to-session-name mappings |
| `agentx honcho map <name>` | Map current working directory to a Honcho session name |
| `agentx honcho identity` | Seed AI peer identity or show both peer representations |
| `agentx honcho sync` | Create host blocks for all AgentX profiles that don't have one yet |
| `agentx honcho migrate` | Step-by-step migration guide from OpenClaw native memory to AgentX + Honcho |
| `agentx memory setup` | Generic memory provider picker (selecting "honcho" runs the same wizard) |
| `agentx memory status` | Show active memory provider and config |
| `agentx memory off` | Disable external memory provider |
