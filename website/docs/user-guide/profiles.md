...
:::note
You cannot delete the default profile (`~/.agentx`). To remove everything, use `hermes uninstall`.
:::
...
- `AGENTX_HOME` is the profile boundary. It controls Hermes config, `.env`,
...
Hermes also exposes `AGENTX_REAL_HOME` to subprocesses so scripts can still find
