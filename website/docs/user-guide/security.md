...
| Hermes credential stores | `auth.json`, `.env`, `.anthropic_oauth.json`, `mcp-tokens/`, `pairing/` under AGENTX_HOME (active profile and global root) |
...
- **Do not add to `~/.agentx/.env` casually.** If you set it to a project directory, the agent cannot write to `~/.agentx/cron/jobs.json`, profile skills, or other Hermes state outside that prefix
...
Do not ask the agent to `patch` `~/.agentx/cron/jobs.json` directly. Use the `cronjob` tool, [`hermes cron`](./features/cron.md), or `/cron` — they update the job store through the supported API. The same applies to other Hermes control files when write safety blocks direct edits.
...
When loaded, Hermes checks if these files exist in the active profile's `AGENTX_HOME` and registers them for mounting:
