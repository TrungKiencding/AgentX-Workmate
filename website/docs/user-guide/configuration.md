...
In that mode tool subprocesses use `{AGENTX_HOME}/home` as `HOME`. Hermes also
...
prefer `AGENTX_HOME` for Hermes data and `AGENTX_REAL_HOME` for the account home:
...
**Requirements:** Docker Desktop or Docker Engine installed and running. Hermes probes `$PATH` plus common macOS install locations (`/usr/local/bin/docker`, `/opt/homebrew/bin/docker`, Docker Desktop app bundle). Podman is supported out of the box: set `AGENTX_DOCKER_BINARY=podman` (or the full path) to force it when both are installed.
...
**Scratch directory:** Resolved in order: `TERMINAL_SCRATCH_DIR` → `TERMINAL_SANDBOX_DIR/singularity` → `/scratch/$USER/hermes-agent` (HPC convention) → `~/.agentx/sandboxes/singularity`.
...
- This covers Hermes state (`~/.agentx/`), **not** arbitrary working-tree files inside the sandbox — have the agent copy important artifacts out explicitly (e.g. `scp`, `modal volume put`) before the sandbox is destroyed.
...
Hermes resolves each listed variable from your current shell first, then falls back to `~/.agentx/.env` if it was saved with `hermes config set`.
...
Controls how much content Hermes loads from each automatic context file before applying head/tail truncation. This applies to files injected into the system prompt such as `SOUL.md`, `.agentx.md`, `AGENTS.md`, `CLAUDE.md`, and `.cursorrules`. It does **not** affect the `read_file` tool.
...
The **socket read timeout** controls how long httpx waits for the next chunk of data from the provider. Local LLMs can take minutes for prefill on large contexts before producing the first token, so Hermes raises this to 30 minutes when it detects a local endpoint. If you explicitly set `AGENTX_STREAM_READ_TIMEOUT`, that value is always used regardless of endpoint detection.
...
The **stale non-stream detection** kills non-streaming calls that produce no response for too long. By default Hermes disables this on local endpoints to avoid false positives during long prefills. If you explicitly set `providers.<id>.stale_timeout_seconds`, `providers.<id>.models.<model>.stale_timeout_seconds`, or `AGENTX_API_CALL_STALE_TIMEOUT`, that explicit value is honored even on local endpoints.
...
If writes to Hermes state (cron jobs, skills, scripts under `~/.agentx/`) are failing, check whether `AGENTX_WRITE_SAFE_ROOT` is set in your environment. For cron changes, use the `cronjob` tool or `hermes cron edit` instead of patching `jobs.json` directly.
...
quick_commands:
  ...
  update:
    type: exec
    command: cd ~/.agentx/hermes-agent && git pull && uv pip install -e .
  gpu:
    type: exec
