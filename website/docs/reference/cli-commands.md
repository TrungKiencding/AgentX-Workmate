...
| `hermes secrets` | Manage external secret sources (currently Bitwarden Secrets Manager) for pulling API keys at process startup instead of from `~/.agentx/.env`. |
...
| `hermes checkpoints` | Inspect / prune / clear `~/.agentx/checkpoints/` (the shadow store used by `/rollback`). Run with no args for a status overview. |
...
| `hermes update` | Pull latest code and reinstall dependencies. `--check` previews without installing; `--backup` takes a pre-pull `AGENTX_HOME` snapshot. |
...
# or:
AGENTX_INFERENCE_MODEL=anthropic/claude-sonnet-4.6 hermes -z "…"
```
...
| Option | Description |
|--------|-------------|
| `--all` | On `start` / `restart` / `stop`: act on **every profile's** gateway, not just the active `AGENTX_HOME`. Useful if you run multiple profiles side-by-side and want to restart them all after `hermes update`. |
...
hermes slack manifest --write      # write to ~/.agentx/slack-manifest.json
...
On-demand vulnerability scan against [OSV.dev](https://osv.dev). Covers the Hermes venv (installed PyPI distributions), Python dependencies declared by plugins under `~/.agentx/plugins/`, and pinned `npx`/`uvx` MCP servers in `config.yaml`. Does NOT scan globally-installed packages or editor/browser extensions.
...
View, tail, and filter Hermes log files. All logs are stored in `~/.agentx/logs/` (or `<profile>/logs/` for non-default profiles).
...
Migrate your OpenClaw setup to Hermes. Reads from `~/.openclaw` (or a custom path) and writes to `~/.agentx`. Automatically detects legacy directory names (`~/.clawdbot`, `~/.moltbot`) and config filenames (`clawdbot.json`, `moltbot.json`).
...
| `--no-backup` | Skip the pre-migration zip snapshot of `~/.agentx/` (by default a single restore-point archive is written to `~/.agentx/backups/pre-migration-*.zip` before apply; restorable with `hermes import`). |
...
Import a **Claude Code** (`~/.claude`) or **OpenAI Codex CLI** (`~/.codex`) setup into Hermes. Maps `CLAUDE.md`/`AGENTS.md` instructions to memory entries, `Bash(...)` permission allow/deny rules to `command_allowlist`/`approvals.deny`, MCP servers to `mcp_servers` in `config.yaml`, and skill directories into `~/.agentx/skills/`. Always previews before applying; API keys and credentials are never imported.
...
Launch the web dashboard — a browser-based UI for managing configuration, API keys, and monitoring sessions. (For a headless backend with no browser UI — e.g. what the desktop app spawns — use [`hermes serve`](#hermes-serve) above.) Requires `cd ~/.agentx/hermes-agent && uv pip install -e ".[web]"` (FastAPI + Uvicorn). The embedded browser Chat tab is always available and additionally needs the `pty` extra (`cd ~/.agentx/hermes-agent && uv pip install -e ".[web,pty]"`) plus a POSIX PTY environment such as Linux, macOS, or WSL2. See [Web Dashboard](/user-guide/features/web-dashboard) for full documentation.
...
Register this install as a self-hosted dashboard with your Nous Portal account. Creates an OAuth client, writes `AGENTX_DASHBOARD_OAUTH_CLIENT_ID` into `~/.agentx/.env`, and prints how to engage the login gate. Requires being logged in (`hermes setup`).
...
- **Pairing data snapshot.** Even when `--backup` is off, `hermes update` takes a lightweight snapshot of `~/.agentx/pairing/` and the Feishu comment rules before `git pull`. You can roll it back with `hermes backup restore --state pre-update` if a pull rewrites a file you were editing.
