...
This page covers all commands related to [AgentX profiles](../user-guide/profiles.md). For general CLI commands, see [CLI Commands Reference](./cli-commands.md).
...
Top-level command for managing profiles. Running `agentx profile` without a subcommand shows help.
...
```bash
agentx profile use <name>
```

Sets `<name>` as the active profile. All subsequent `agentx` commands (without `-p`) will use this profile.
...
agentx profile use default
```

## `agentx profile create`

```bash
agentx profile create <name> [options]
```
...
| `--description "<text>"` | One- or two-sentence description of what this profile is good at. Used by the kanban orchestrator to route tasks based on role instead of profile name alone. Skip and add later via `agentx profile describe`. Persisted in `<profile_dir>/profile.yaml`. |
| `--no-skills` | Create an **empty** profile with zero bundled skills enabled. Writes a `.no-bundled-skills` marker into the profile so future `agentx update` runs won't re-seed the bundled set, and refuses to combine with `--clone`, `--clone-from`, or `--clone-all` (which would copy skills in anyway). Useful for narrow orchestrator profiles or sandbox profiles that should not inherit the full skill catalog. To toggle this on an already-created profile (including the default `~/.agentx`), use `agentx skills opt-out` / `agentx skills opt-in`. |
...
agentx profile create work2 --clone-from work

# Clone everything from a specific profile
agentx profile create work2-backup --clone-from work --clone-all
```

## `agentx profile describe`

```bash
agentx profile describe [<name>] [options]
```
...
agentx profile describe researcher

# Set it explicitly
agentx profile describe researcher --text "Reads source code and writes findings."

# Let the LLM generate one
agentx profile describe researcher --auto

# Fill in descriptions for every profile that doesn't have one
agentx profile describe --all --auto
```

## `agentx profile delete`

```bash
agentx profile delete <name> [options]
```
...
```bash
agentx profile delete mybot
agentx profile delete mybot --yes
```
...
```bash
agentx profile show <name>
```

Displays details about a profile including its home directory, configured model, gateway status, skills count, and configuration file status.

This shows the profile's AgentX home directory, not the terminal working directory. Terminal commands start from `terminal.cwd` (or the launch directory on the local backend when `cwd: "."`).
...
```bash
$ agentx profile show work
Profile: work
Path:    ~/.agentx/profiles/work
...
## `agentx profile alias`

```bash
agentx profile alias <name> [options]
```

Regenerates the shell alias script at `~/.local/bin/<name>`. Useful if the alias was accidentally deleted or if you need to update it after moving your AgentX installation.
...
```bash
agentx profile alias work
# Creates/updates ~/.local/bin/work

agentx profile alias work --name mywork
# Creates ~/.local/bin/mywork

agentx profile alias work --remove
# Removes the wrapper script
```

## `agentx profile rename`

```bash
agentx profile rename <old-name> <new-name>
```
...
```bash
agentx profile rename mybot assistant
# ~/.agentx/profiles/mybot → ~/.agentx/profiles/assistant
# ~/.local/bin/mybot → ~/.local/bin/assistant
```

## `agentx profile export`

```bash
agentx profile export <name> [options]
```
...
```bash
agentx profile export work
# Creates work.tar.gz in the current directory

agentx profile export work -o ./work-2026-03-29.tar.gz
```

## `agentx profile import`

```bash
agentx profile import <archive> [options]
```
...
```bash
agentx profile import ./work-2026-03-29.tar.gz
# Infers profile name from the archive

agentx profile import ./work-2026-03-29.tar.gz --name work-restored
```
...
:::info
`agentx profile export` / `import` are still the right commands for
...
### `agentx profile install`

```bash
agentx profile install <source> [--name <name>] [--alias] [--force] [--yes]
```
...
| `--alias` | Also create a shell wrapper (e.g. `telemetry` → `agentx -p telemetry`). |
...
agentx profile install github.com/kyle/telemetry-distribution --alias

# Install from a full HTTPS git URL
agentx profile install https://github.com/kyle/telemetry-distribution.git

# Install from SSH
agentx profile install git@github.com:kyle/telemetry-distribution.git

# Install from a local directory during development
agentx profile install ./telemetry/
```

### `agentx profile update`

```bash
agentx profile update <name> [--force-config] [--yes]
```
...
```bash
agentx profile info <name>
```

Prints the profile's distribution manifest — name, version, required
AgentX version, author, env var requirements, the source URL/path, and
...
`agentx profile list` also shows the distribution name and version in a
`Distribution` column, and `agentx profile show <name>` / `delete <name>`
...
agentx profile install git@github.com:your-org/internal-assistant.git

# Uses your git credential helper
agentx profile install https://github.com/your-org/internal-assistant.git
```
...
AgentX version doesn't satisfy the spec.
...
2. Initialize a git repo (or use an existing one) and push to GitHub /
   GitLab / any host AgentX can clone from.
3. Tell recipients to run `agentx profile install <your-repo-url>`.
...
## `agentx -p` / `agentx --profile`

```bash
agentx -p <name> <command> [options]
agentx --profile <name> <command> [options]
```

Global flag to run any AgentX command under a specific profile without changing the sticky default. This overrides the active profile for the duration of the command.
...
```bash
agentx -p work chat -q "Check the server status"
agentx --profile dev gateway start
agentx -p personal skills list
agentx -p work config edit
```

## `agentx completion`

```bash
agentx completion <shell>
```
...
- `agentx profile <TAB>` — subcommands (list, use, create, etc.)
- `agentx profile use <TAB>` — profile names
- `agentx -p <TAB>` — profile names
