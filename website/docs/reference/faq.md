...
Set your provider with `hermes model` or by editing `~/.agentx/.env`. See the [Environment Variables](./environment-variables.md) reference for all provider keys.
...
API calls go **only to the LLM provider you configure** (e.g., OpenRouter, your local Ollama instance). Hermes Agent does not collect telemetry, usage data, or analytics. Your conversations, memory, and skills are stored locally in `~/.agentx/`.
...
**Solution:** Hermes auto-sources `~/.bashrc` by default. If that's not enough — e.g. you're a zsh user whose PATH lives in `~/.zshrc`, or you init `nvm` from a standalone file — list the extra files to source in `~/.agentx/config.yaml`:
...
# Set a valid model
hermes config set AGENTX_MODEL anthropic/claude-opus-4.7
...
cd ~/.agentx/hermes-agent && uv pip install -e ".[messaging]"  # Telegram, Discord, Slack, and shared gateway deps
...
nohup hermes gateway run > ~/.agentx/logs/gateway.log 2>&1 &
```
...
cd ~/.agentx/hermes-agent && uv pip install -e ".[mcp]"
...
**Solution: Delegation config.** Hermes can route subagents to a different model automatically. Set this in `~/.agentx/config.yaml`:
...
2. On the **source machine**, create a full backup:
   ...
   hermes backup
   ```
   This creates a zip of your entire `~/.agentx/` directory — config, API keys, memories, skills, sessions, and profiles — saved to your home directory as `~/hermes-backup-<timestamp>.zip`.
...
```bash
rsync -av --exclude='hermes-agent' ~/.agentx/ newmachine:~/.agentx/
```
