...
AgentX has two slash-command surfaces, both driven by a central `COMMAND_REGISTRY` in `hermes_cli/commands.py`:
...
| `/export [md\|json] [filename]` | Export the current session history to a file in the working directory — Markdown by default (`session_<id>.md`), or the raw JSON export with `json`. Pass a filename to choose the name. Also works on messaging platforms, where the file comes back as a document. |
...
| `/diff [staged\|all\|session] [--stat] [path...]` | Show git changes in the working directory. Default: unstaged changes plus untracked files. `staged` shows what's staged for commit, `all` everything since HEAD, and `session` the cumulative diff of everything AgentX changed here (from the earliest retained checkpoint baseline — requires checkpoints to be enabled; complements `/rollback diff <N>`). `--stat` prints just the changed-file summary; path arguments restrict the diff. |
| `/snapshot [create\|restore <id>\|prune]` (alias: `/snap`) | Create or restore state snapshots of AgentX config/state. `create [label]` saves a snapshot, `restore <id>` reverts to it, `prune [N]` removes old snapshots, or list all with no args. |
...
| `/goal <text>` | Set a standing goal AgentX works toward across turns — our take on the Ralph loop. After each turn an auxiliary judge model decides whether the goal is done; if not, AgentX auto-continues. Subcommands: `/goal status`, `/goal pause`, `/goal resume`, `/goal clear`. Budget defaults to 20 turns (`goals.max_turns`); any real user message preempts the continuation loop, and state survives `/resume`. See [Persistent Goals](/user-guide/features/goals) for the full walkthrough. |
...
| `/busy [queue\|steer\|interrupt\|status]` | CLI-only: control what pressing Enter does while AgentX is working — queue the new message, steer mid-turn, or interrupt immediately. |
...
| `/wake [on\|off\|status]` | CLI-only: toggle the "Hey AgentX" wake word listener. |
| `/export-profile [profile] [-o output.tar.gz]` | CLI-only: export a profile (config, skills, theme) to a shareable `.tar.gz` archive — the active profile and `<profile>.tar.gz` by default. Credentials (`.env`, `auth.json`) are left out, but memories and sessions go with it. Same as [`agentx profile export`](./profile-commands.md#agentx-profile-export). |
| `/import <archive.tar.gz> [--name <name>]` | CLI-only: import a shared profile archive as a new profile. Same as [`agentx profile import`](./profile-commands.md#agentx-profile-import). |
...
| `/bundles` | List configured skill bundles — `/<name>` slash aliases that preload several skills at once. Configure under `bundles:` in `~/.agentx/config.yaml`. See [Skill Bundles](/user-guide/features/skills#skill-bundles). |
...
| `/version` | Show AgentX Workmate version, build, and environment info. |
...
User-defined quick commands map a short slash command to either a shell command or another slash command. Configure them in `~/.agentx/config.yaml`:
...
**Full form** — pin an exact model, provider, and optionally a base URL. Put this in `~/.agentx/config.yaml`:
...
```bash
agentx config set model.aliases.fav anthropic/claude-opus-4.6
agentx config set model.aliases.grok x-ai/grok-4
```
...
> Slack itself blocks native slash commands inside message threads ("/queue is not supported in threads. Sorry!") and never delivers them to AgentX. Inside a Slack thread, use the `!` prefix instead — `!stop`, `!new`, `!status` — and the gateway dispatches it exactly like the slash form. `@AgentX !stop` and `@AgentX /stop` work in threads too. Only the first token is checked against the known command list, so messages like `!nice work` pass through to the agent unchanged. See [Using commands inside threads](/user-guide/messaging/slack#using-commands-inside-threads-the-cmd-prefix) for details.
...
| `/export [md\|json] [filename]` | Export this chat's session history and send it back as a document — Markdown by default, raw JSON with `json`. Only the file name of `filename` is used. |
...
| `/diff [staged\|all\|session] [--stat]` | Show git changes in the working directory (fenced and truncated to platform message limits). `session` shows the cumulative diff of everything AgentX changed; `--stat` shows just the summary. |
...
| `/skills [pending\|approve\|reject\|diff\|approval]` | Review pending **skill** writes staged by the write-approval gate (`skills.write_approval`). Shows a one-line gist per staged write; `/skills diff <id>` is truncated for chat — read the full diff on the CLI or in `~/.agentx/pending/skills/<id>.json`. Only appears when the gate is on (or staged writes remain); search/install stay CLI-only. |
...
- `/skin`, `/snapshot`, `/reload`, `/tools`, `/toolsets`, `/browser`, `/config`, `/cron`, `/platforms`, `/paste`, `/image`, `/statusbar`, `/battery`, `/focus`, `/plugins`, `/busy`, `/indicator`, `/wake`, `/journey`, `/redraw`, `/clear`, `/history`, `/save`, `/copy`, `/handoff`, `/prompt`, `/pet`, `/hatch`, `/timestamps`, `/subscription`, `/export-profile`, `/import`, and `/quit` are **CLI-only** commands.
...
- `/status`, `/egress`, `/version`, `/whoami`, `/background`, `/queue`, `/steer`, `/voice`, `/reload-mcp`, `/reload-skills`, `/rollback`, `/diff`, `/debug`, `/fast`, `/approvals`, `/footer`, `/curator`, `/kanban`, `/topup`, `/suggestions`, `/blueprint`, `/learn`, `/init`, `/sessions`, `/export`, and `/yolo` work in **both** the CLI and the messaging gateway.
- `/voice join`, `/voice channel`, and `/voice leave` are only meaningful on Discord.
- In the TUI, `/sessions` shows live sessions in the current TUI process. Use `/resume [name]` or `agentx --tui --resume <id-or-title>` for saved or closed transcripts.
...
Set `approvals.destructive_slash_confirm: false` in `~/.agentx/config.yaml` to disable the prompts globally; set it back to `true` to re-enable. See [Security — Destructive slash command confirmation](../user-guide/security.md#dangerous-command-approval) for context.
