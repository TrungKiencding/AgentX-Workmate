...
description: "Reference for AgentX Workmate MCP configuration keys, filtering semantics, and utility-tool policy"
---
...
Values resolve from the active profile's secret scope (falling back to the process environment), so put the secret in `~/.agentx/.env`. An unset variable keeps its literal placeholder.
...
AgentX may register these utility wrappers per MCP server:
...
Even when `resources: true` or `prompts: true`, AgentX only registers those utility tools if the MCP session actually exposes the corresponding capability.
...
If filtering removes all server-native tools and no utility tools are registered, AgentX does not create an empty MCP runtime toolset for that server.
...
Behavior:
- AgentX uses the MCP SDK's OAuth 2.1 PKCE flow (metadata discovery, dynamic client registration, token exchange, and refresh)
- On first connect, a browser window opens for authorization
- Tokens are persisted to `~/.agentx/mcp-tokens/<server>.json` and reused across sessions
