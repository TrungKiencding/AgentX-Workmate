...
| An **STT backend** (custom whisper / ASR CLI) | [Voice Message Transcription](/user-guide/features/tts#voice-message-transcription-stt) — set `AGENTX_LOCAL_STT_COMMAND` to an argv-tokenized template |
...
**Key properties:**
- Plugin skills are **read-only** — they don't enter `~/.agentx/skills/` and can't be edited via `skill_manage`.
...
:::tip Legacy pattern
The old `shutil.copy2` pattern (copying a skill into `~/.agentx/skills/`) still works but creates name collision risk with built-in skills. Prefer `ctx.register_skill()` for new plugins.
:::
...
auditable in `~/.agentx/logs/agent.log`. Plugins load after built-in
...
hooks:
  output_spill:
    ...
    # directory: null      # default: $AGENTX_HOME/hook_outputs
```
...
Drop a manifest + handler into `~/.agentx/hooks/<name>/`:
