...
AgentX Workmate uses a dual compression system and Anthropic prompt caching to
...
Configure via `agentx plugins` → Provider Plugins → Context Engine, or edit `config.yaml` directly.
...
AgentX has two separate compression layers that operate independently:
...
compression:
  ...
  codex_app_server_auto: native  # native|agentx|off for Codex app-server thread compaction
...
| `codex_app_server_auto` | `native` | `native`, `agentx`, `off` | Thread-compaction mode for Codex app-server sessions (see below) |
...
OAuth (`provider: openai-codex`) and the model is gpt-5.5, AgentX raises the
trigger to **85%** (~231K) and shows a notice with the opt-out command. The
notice is shown once per profile — a marker under `$AGENTX_HOME`
...
```bash
agentx config set compression.codex_gpt55_autoraise false
```

To keep the 85% autoraise but hide only the one-time notice:

```bash
agentx config set compression.codex_gpt55_autoraise_notice false
```
...
thread context, so AgentX' auxiliary summarizer cannot shrink it — rewriting the
...
- Automatic compaction is controlled by `compression.codex_app_server_auto`:
  the default `native` lets the app-server decide when to compact and AgentX
  records the resulting compaction events (compression counters, session
  events). Set `agentx` to let AgentX' compression threshold initiate
  app-server compaction, or `off` to disable AgentX-initiated automatic
  compaction entirely (codex may still compact natively).

AgentX' local transcript is never rewritten on this runtime — state.db records
the compaction boundary while the visible transcript stays intact. All other
routes (including Codex OAuth chat sessions) keep AgentX' summary compressor.
...
Anthropic allows a maximum of 4 `cache_control` breakpoints per request. AgentX
...
5. **Model identity is part of the cache key**: Provider-side caches are scoped
   ...
   AgentX can avoid; user-facing docs for `/model`, fallback providers, and
