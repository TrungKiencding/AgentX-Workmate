...
Most users set up a platform by dropping env vars into `~/.agentx/.env` rather than editing `config.yaml`. The `env_enablement_fn` hook lets your plugin pick those env vars up **before** the adapter is constructed, so `hermes gateway status`, `get_connected_platforms()`, and cron delivery see the correct state without instantiating the platform SDK.
...
1. **`toolsets.py`** — Add `"hermes-newplat"` toolset definition with `_AGENTX_CORE_TOOLS`
