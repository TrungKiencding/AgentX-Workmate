...
Plugin metadata. The `requires_env` and `optional_env` blocks auto-populate `agentx config` UI entries (see [Surfacing Env Vars](#surfacing-env-vars-in-agentx-config) below).
