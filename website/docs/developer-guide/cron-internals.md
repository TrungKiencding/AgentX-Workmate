...
Jobs are stored in `~/.agentx/cron/jobs.json` with atomic write semantics (write to temp file, then rename). Each job record contains:
...
| Local file | `local` | Save to `~/.agentx/cron/output/` |
