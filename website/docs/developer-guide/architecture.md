...
sidebar_position: 1
title: "Architecture"
description: "AgentX Workmate internals — major subsystems, execution paths, data flow, and where to read next"
---

# Architecture

This page is the top-level map of AgentX Workmate internals. Use it to orient yourself in the codebase, then dive into subsystem-specific docs for implementation details.
...
├── hermes_constants.py       # AGENTX_HOME, profile-aware paths
...
│   ├── main.py               # Entry point — all `agentx` subcommands (large file)
...
│   ├── skills_config.py      # agentx skills — enable/disable per platform
│   ├── skills_hub.py         # /skills slash command
│   ├── tools_config.py       # agentx tools — enable/disable per platform
│   ├── plugins.py            # PluginManager — discovery, loading, hooks
│   ├── callbacks.py          # Terminal callbacks (clarify, sudo, approval)
│   └── gateway.py            # agentx gateway start/stop
│
...
Three discovery sources: `~/.agentx/plugins/` (user), `.agentx/plugins/` (project), and pip entry points. Plugins register tools, hooks, and CLI commands through a context API. Two specialized plugin types exist: memory providers (`plugins/memory/`) and context engines (`plugins/context_engine/`). Both are single-select — only one of each can be active at a time, configured via `agentx plugins` or `config.yaml`.
...
Exposes AgentX as an editor-native agent over stdio/JSON-RPC for VS Code, Zed, and JetBrains.
...
| **Profile isolation** | Each profile (`agentx -p <name>`) gets its own AGENTX_HOME, config, memory, sessions, and gateway PID. Multiple profiles run concurrently. |
