...
`AGENTX_TUI_DIR` points Hermes at a *prebuilt* bundle (Nix, system packages), which has no source to hot-reload. If it's set in your shell, `hermes --tui --dev` exits with an error. Run `unset AGENTX_TUI_DIR` before `htui`.
:::
...
|----------|----------------|
| `AGENTX_DESKTOP_AGENTX_ROOT` | Runs the backend from **this worktree**, not the packaged/PATH `hermes`. |
| `AGENTX_DESKTOP_PYTHON` | Reuses the deps checkout's venv instead of re-resolving a Python. |
| `AGENTX_DESKTOP_IGNORE_EXISTING` | Ignores any `hermes` on `PATH` so it can't shadow the worktree. |
...
- [TUI](../user-guide/tui.md) — `hermes --tui --dev` and the `AGENTX_TUI_DIR` prebuild path
...
- [Environment Variables](../reference/environment-variables.md) — every `AGENTX_*` variable Hermes reads
