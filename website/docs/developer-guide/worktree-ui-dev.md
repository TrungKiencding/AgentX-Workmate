...
| Variable | Meaning |
|----------|---------|
| `AGENTX_MAIN_CHECKOUT` | The deps checkout — where `node_modules` really lives, and whose `.venv/bin/python` runs the backend. |
| `AGENTX_GUI_DEPS_CHECKOUT` | Where the desktop deps (`apps/desktop/node_modules`) live. Defaults to `AGENTX_MAIN_CHECKOUT`; override only if you keep desktop deps elsewhere. |

Neither is read by AgentX itself — they're private to these helpers. The variables AgentX *does* read are covered in [Environment Variables](../reference/environment-variables.md).

## `htui` — TUI from the worktree

The Ink TUI has a dev path already: `agentx --tui --dev` runs the TypeScript sources via `tsx` instead of the prebuilt bundle. `htui` is a one-liner over it that also points the run at the current worktree's `ui-tui/`:
...
`--dev` compiles from source, so it links `ui-tui/node_modules` from `AGENTX_MAIN_CHECKOUT` when the root lockfile matches and installs locally otherwise (see [`_hermes_root` / linking helpers](#shared-helpers)).

:::warning `--dev` and `AGENTX_TUI_DIR` are mutually exclusive
`AGENTX_TUI_DIR` points AgentX at a *prebuilt* bundle (Nix, system packages), which has no source to hot-reload. If it's set in your shell, `agentx --tui --dev` exits with an error. Run `unset AGENTX_TUI_DIR` before `htui`.
:::
...
hgui() {
  ...
  ( cd "$desktop"
    ...
    AGENTX_DESKTOP_AGENTX_ROOT="$root" \
    AGENTX_DESKTOP_PYTHON="$AGENTX_MAIN_CHECKOUT/.venv/bin/python" \
    AGENTX_DESKTOP_IGNORE_EXISTING=1 \
    AGENTX_DESKTOP_CWD="$root" \
    npm run dev )
}
```
...
|----------|----------------|
| `AGENTX_DESKTOP_AGENTX_ROOT` | Runs the backend from **this worktree**, not the packaged/PATH `agentx`. |
| `AGENTX_DESKTOP_PYTHON` | Reuses the deps checkout's venv instead of re-resolving a Python. |
| `AGENTX_DESKTOP_IGNORE_EXISTING` | Ignores any `agentx` on `PATH` so it can't shadow the worktree. |
| `AGENTX_DESKTOP_CWD` | Opens the desktop chat rooted at the worktree. |
...
```bash
# The enclosing worktree, verified as a real AgentX checkout.
_hermes_root() {
  local root
...
- [TUI](../user-guide/tui.md) — `agentx --tui --dev` and the `AGENTX_TUI_DIR` prebuild path
- [Desktop App](../user-guide/desktop.md) — building from source and the backend resolution ladder
- [`apps/desktop/README.md`](https://github.com/TrungKiencding/AgentX-Workmate/blob/main/apps/desktop/README.md) — dev server, sandbox script, and packaging
- [Environment Variables](../reference/environment-variables.md) — every `AGENTX_*` variable AgentX reads
