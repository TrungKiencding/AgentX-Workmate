...
description: "How AgentX detects and responds to native JS dialogs and interacts with cross-origin iframes via a persistent CDP connection."
---

# Browser CDP Supervisor

The CDP supervisor closes two long-standing gaps in AgentX' browser tooling:
...
(`agentx-dialog-bridge.invalid`). `Fetch.enable` intercepts those XHRs before
...
One `asyncio.Task` running in a background daemon thread per AgentX `task_id`.
...
- `toolsets.py` — registers `browser_dialog` in `browser`, `agentx-acp`, `agentx-api-server`, and core toolsets (gated on CDP reachability)
