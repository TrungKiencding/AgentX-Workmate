...
Plugins can launch and supervise fresh AgentX child sessions without importing
...
active agent turn fails closed with `No active AgentX parent session`.
...
overrides, and per-launch timeouts are explicitly rejected until AgentX can
support them without weakening isolation. Use `allowed_toolsets` to narrow a
child; AgentX's existing unsafe-tool block remains enforced.
