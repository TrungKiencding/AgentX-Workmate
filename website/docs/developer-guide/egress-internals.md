...
description: "How the iron-proxy egress firewall integrates with AgentX — module layout, lifecycle, security invariants, and extension points"
---

# Egress proxy internals

This page covers the architecture of the egress credential-injection firewall (`agentx egress` / iron-proxy) from a contributor / plugin author's perspective. End-user setup + usage docs live at [Egress proxy](../user-guide/egress/iron-proxy.md).
...
hermes_cli/proxy_cli.py               Wizard + slash command handlers.
                                       `agentx egress {install,setup,start,stop,
...
hermes_cli/main.py:_dispatch_egress   Top-level subparser dispatcher.
                                       ...
                                       `agentx proxy` subparser, which uses
                                       dest='proxy_command').
...
agentx egress setup [--from-bitwarden | --no-bitwarden] [--rotate-tokens]
  -> proxy_cli.cmd_setup
...
`merge_mappings(existing, discovered, rotate=False)` MUST return prior tokens for providers that overlap.  Re-running `agentx egress setup` cannot silently 401 running sandboxes.  `--rotate-tokens` is the explicit opt-in.
...
`cmd_setup` MUST NOT downgrade `credential_source: bitwarden` to `env` on re-run without an explicit `--no-bitwarden` flag.  Running `agentx egress setup` (no flag) preserves whatever was previously configured.
...
If the provider uses SigV4 / SDK-minted OAuth / request signatures, a static header swap cannot cover it.  Add the env var to `_NON_BEARER_PROVIDERS` so the wizard and `agentx egress status` warn about it:
...
5. Distributes the CA cert into the sandbox at a path the runtime will trust (typically `/etc/ssl/certs/agentx-egress-ca.crt`).
...
# Live PTY smoke against `agentx egress`
AGENTX_HOME=/tmp/agentx-egress-test python3 -m hermes_cli.main egress --help
AGENTX_HOME=/tmp/agentx-egress-test python3 -m hermes_cli.main egress setup --help
```
