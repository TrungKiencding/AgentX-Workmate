...
* **Host-owned credentials.** OAuth tokens, refresh flows, the
  ...
  concept AgentX already has applies. The plugin never sees a
...
[`agentx-example-plugins`](https://github.com/NousResearch/agentx-example-plugins/tree/main/plugin-llm-example)
repo (companion repo for reference plugins — not bundled with
agentx-agent itself). For the async surface (`acomplete()` /
`acomplete_structured()` with `asyncio.gather()`), see
[`plugin-llm-async-example`](https://github.com/NousResearch/agentx-example-plugins/tree/main/plugin-llm-async-example)
in the same repo.
...
result = ctx.llm.complete(
    ...
    provider=None,         # optional, gated — AgentX provider id (e.g. "openrouter")
...
plugins:
  entries:
    my-plugin:
      llm:
        # Allow this plugin to choose a different AgentX provider
        # (must be one AgentX already knows about — same names as
        # `agentx model` and config.yaml model.provider).
        ...
        # plugin sends — AgentX does not look anything up.
        allowed_models:
          - openai/gpt-4o-mini
...
* **Auth.** Pulls API keys, OAuth tokens, or refresh tokens from
  `~/.agentx/auth.json` / env, including the credential pool when
...
* **Fallback chain.** If the user's primary provider 5xxs or 429s,
  the request goes through AgentX' usual aggregator-aware fallback
...
Existing `ctx.*` methods extend an existing AgentX subsystem:
...
* Reference plugins (companion repo):
  * [`plugin-llm-example`](https://github.com/NousResearch/agentx-example-plugins/tree/main/plugin-llm-example) — sync structured extraction with image input
  * [`plugin-llm-async-example`](https://github.com/NousResearch/agentx-example-plugins/tree/main/plugin-llm-async-example) — async with `asyncio.gather()`
