...
AgentX Workmate uses a SQLite database (`~/.agentx/state.db`) to persist session
...
Multiple agentx processes (gateway + CLI sessions + worktree agents) share one
...
db = SessionDB()                           # Default: ~/.agentx/state.db
...
Default path: `~/.agentx/state.db`

This is derived from `hermes_constants.get_hermes_home()` which resolves to
`~/.agentx/` by default, or the value of `AGENTX_HOME` environment variable.
