---
sidebar_position: 7
title: "Use SOUL.md with AgentX"
description: "How to use SOUL.md to shape AgentX Workmate's default voice, what belongs there, and how it differs from AGENTS.md and /personality"
---

# Use SOUL.md with AgentX

`SOUL.md` is the **primary identity** for your AgentX instance. It's the first thing in the system prompt — it defines who the agent is, how it speaks, and what it avoids.

If you want AgentX to feel like the same assistant every time you talk to it — or if you want to replace the AgentX persona entirely with your own — this is the file to use.
...
- personality
- communication style
- how direct or warm AgentX should be
- what AgentX should avoid stylistically
- how AgentX should relate to uncertainty, disagreement, and ambiguity

In short:
- `SOUL.md` is about who AgentX is and how AgentX speaks
...
AgentX now uses only the global SOUL file for the current instance:

```text
~/.agentx/SOUL.md
```

If you run AgentX with a custom home directory, it becomes:
...
AgentX automatically seeds a starter `SOUL.md` for you if one does not already exist.

That means most users now begin with a real file they can read and edit immediately.

Important:
- if you already have a `SOUL.md`, AgentX does not overwrite it
- if the file exists but is empty, AgentX adds nothing from it to the prompt

## How AgentX uses it

When AgentX starts a session, it reads `SOUL.md` from `AGENTX_HOME`, scans it for prompt-injection patterns, truncates it if needed, and uses it as the **agent identity** — slot #1 in the system prompt. This means SOUL.md completely replaces the built-in default identity text.
...
That alone can noticeably change how AgentX feels.
...
AgentX already tries to be helpful and clear. `SOUL.md` should add real personality and style, not restate obvious defaults.
...
# Style
How AgentX should sound.

# Avoid
What AgentX should not do.

# Defaults
How AgentX should behave when ambiguity appears.
```
...
Then restart AgentX or start a new session.
...
### I edited SOUL.md but AgentX still sounds the same

Check:
- you edited `~/.agentx/SOUL.md` or `$AGENTX_HOME/SOUL.md`
...
### AgentX is ignoring parts of my SOUL.md
