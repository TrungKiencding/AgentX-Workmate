...
This tutorial walks you through setting up a Telegram bot powered by AgentX Workmate that multiple team members can use. By the end, your team will have a shared AI assistant they can message for help with code, research, system administration, and anything else — secured with per-user authorization.
...
- **AgentX Workmate installed** on a server or VPS (not your laptop — the bot needs to stay running). Follow the [installation guide](/getting-started/installation) if you haven't yet.
- **A Telegram account** for yourself (the bot owner)
- **An LLM provider configured** — at minimum, an API key for OpenAI, Anthropic, or another supported provider in `~/.agentx/.env`

:::tip
A $5/month VPS is plenty for running the gateway. AgentX itself is lightweight — the LLM API calls are what cost money, and those happen remotely.
:::
...
2. **Send `/newbot`** — BotFather will ask you two things:
   - **Display name** — what users see (e.g., `Team AgentX Assistant`)
...
4. **Set a description** (optional but recommended):
   ...
   ```
   Team AI assistant powered by AgentX Workmate. DM me for help with code, research, debugging, and more.
   ```
...
```
[Gateway] Starting AgentX Gateway...
...
# View live logs
journalctl --user -u agentx-gateway -f
...
:::tip macOS PATH
The launchd plist captures your shell PATH at install time so gateway subprocesses can find tools like Node.js and ffmpeg. If you install new tools later, re-run `agentx gateway install` to update the plist.
:::
...
```bash
agentx gateway stop && agentx gateway start
```
...
agentx pairing revoke telegram 987654321
...
**Option 2:** Set it manually in `~/.agentx/.env`:
...
Control how much detail the bot shows when using tools. In `~/.agentx/config.yaml`:
...
Customize how the bot communicates by editing `~/.agentx/SOUL.md`:

For a full guide, see [Use SOUL.md with AgentX](/guides/use-soul-with-agentx).
...
```bash
# From the CLI
agentx cron list          # View all scheduled jobs
agentx cron status        # Check if scheduler is running
...
| What | Location |
|------|----------|
| Gateway logs | `journalctl --user -u agentx-gateway` (Linux) or `~/.agentx/logs/gateway.log` (macOS) |
| Cron job output | `~/.agentx/cron/output/{job_id}/{timestamp}.md` |
| Cron job definitions | `~/.agentx/cron/jobs.json` |
| Pairing data | `~/.agentx/pairing/` |
| Session history | `~/.agentx/sessions/` |
