...
description: "Diagnose and fix common AgentX cron issues — jobs not firing, delivery failures, skill loading errors, and performance problems"
---
...
If you're expecting jobs to fire automatically, you need a running gateway (`agentx gateway` for foreground, or `agentx gateway start` for the installed service). For one-off debugging, you can manually trigger a tick with `agentx cron tick`.
...
```bash
date
agentx cron list   # Compare next_run times with local time
```
...
| Target | Requires |
|--------|----------|
| `telegram` | `TELEGRAM_BOT_TOKEN` in `~/.agentx/.env` |
| `discord` | `DISCORD_BOT_TOKEN` in `~/.agentx/.env` |
| `slack` | `SLACK_BOT_TOKEN` in `~/.agentx/.env` |
...
| `local` | Write access to `~/.agentx/cron/output/` |
...
If delivery fails, the job still runs — it just won't send anywhere. Check `agentx cron list` for updated `last_error` field (if available).
...
Skills must be installed before they can be attached to cron jobs. If a skill is missing, install it first with `agentx skills install <skill-name>` or via `/skills` in the CLI.

### Check 2: Check skill name vs. skill folder name

Skill names are case-sensitive and must match the installed skill's folder name. If your job specifies `ai-funding-report` but the skill folder is `ai-funding-daily-report`, confirm the exact name from `agentx skills list`.
...
2. `~/.agentx/logs/agent.log` for scheduler messages (or `errors.log` for warnings)
3. The job's `last_run` metadata via `agentx cron list`

### Check 2: Common error patterns

**"No such file or directory" for scripts**
The `script` path must be an absolute path (or relative to the AgentX config directory). Verify:
```bash
ls ~/.agentx/scripts/your-script.py   # Must exist
agentx cron edit <job_id> --script ~/.agentx/scripts/your-script.py
```

**"Skill not found" at job execution**
The skill must be installed on the machine running the scheduler. If you move between machines, skills don't automatically sync — reinstall them with `agentx skills install <skill-name>`.
...
Jobs are stored in `~/.agentx/cron/jobs.json`. If this file is not readable/writable by your user, the scheduler will fail silently:

```bash
ls -la ~/.agentx/cron/jobs.json
chmod 600 ~/.agentx/cron/jobs.json   # Your user should own it
```
...
```bash
agentx cron list                    # Show all jobs, states, next_run times
agentx cron run <job_id>            # Schedule for next tick (for testing)
agentx cron edit <job_id>           # Fix configuration issues
agentx logs                         # View recent AgentX logs
agentx skills list                  # Verify installed skills
```
...
3. Open an issue at [github.com/TrungKiencding/AgentX-Workmate](https://github.com/TrungKiencding/AgentX-Workmate) with:
