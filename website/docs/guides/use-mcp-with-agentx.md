---
sidebar_position: 6
title: "Use MCP with AgentX"
description: "A practical guide to connecting MCP servers to AgentX Workmate, filtering their tools, and using them safely in real workflows"
---

# Use MCP with AgentX

This guide shows how to actually use MCP with AgentX Workmate in day-to-day workflows.
...
Use MCP when:
- a tool already exists in MCP form and you do not want to build a native AgentX tool
- you want AgentX to operate against a local or remote system through a clean RPC layer
- you want fine-grained per-server exposure control
- you want to connect AgentX to internal APIs, databases, or company systems without modifying AgentX core

Do not use MCP when:
- a built-in AgentX tool already solves the job well
...
- AgentX remains the agent
- MCP servers contribute tools
- AgentX discovers those tools at startup or reload time
...
If you installed AgentX with the standard install script, MCP support is already included (the installer runs `uv pip install -e ".[all]"`).
...
- AgentX banner/status should show MCP integration when configured
- ask AgentX what tools it has available
...
## WSL2: bridge AgentX in WSL to Windows Chrome

This is the practical setup when:

- AgentX runs inside WSL2
...
In this setup, AgentX does **not** connect to Chrome directly. Instead:

- AgentX runs in WSL
- AgentX starts a local stdio MCP server
...
```text
AgentX (WSL) -> MCP stdio bridge -> Windows Chrome
```

### Why this mode is useful

- you keep your real Windows browser profile, cookies, and logins
- AgentX stays in its supported Unix environment (WSL2)
- browser control is exposed as MCP tools instead of relying on AgentX core browser transport
...
```bash
agentx mcp add chrome-devtools-win --command cmd.exe --args /c npx -y chrome-devtools-mcp@latest --autoConnect --no-usage-statistics
```

After saving the server:

```bash
agentx mcp test chrome-devtools-win
```

Then start a fresh AgentX session or run:
...
Once loaded, AgentX can use the MCP-prefixed browser tools directly. For example:
...
If AgentX runs in WSL and Chrome runs on Windows, `/browser connect` may fail even though Chrome is open and debuggable.
...
- Start AgentX from a Windows-mounted path like `/mnt/c/Users/<you>` or `/mnt/c/workspace/...` when using Windows stdio executables through MCP.
- If you start AgentX from `/root` or `/home/...`, Windows may emit a `UNC` current-directory warning before the MCP server starts.
...
2. AgentX-added utility wrappers
- filtered with:
  - `tools.resources`
  - `tools.prompts`
...
So AgentX will not pretend a server has resources/prompts if it does not.

## Common patterns

### Pattern 1: local project assistant

Use MCP for a repo-local filesystem or git server when you want AgentX to reason over a bounded workspace.
...
Use [Open Scaffold](https://github.com/graphanov/open-scaffold) when you want AgentX to read a repository's durable AI-work record: mission, plans, evidence notes, handoff packets, and review/gate results. AgentX remains the agent; Open Scaffold remains the repo-local record.

Add the server for one scaffolded repository:

```bash
agentx mcp add open_scaffold --command npx --args -y open-scaffold@latest mcp serve --repo /absolute/path/to/repo
agentx mcp test open_scaffold
```

Then keep the exposed surface read-oriented. Choose `select` in the `agentx mcp add` prompt, or edit `config.yaml` afterward:
...
- Its write tools require the server to be started with `--allow-write`; do not enable that until you explicitly want AgentX to mutate `.osc` files.
- Open Scaffold records and gates work; it does not authorize AgentX to merge, publish, deploy, or spawn runtimes.
...
Now AgentX can combine them:

```text
Inspect the local project files, then create a GitHub issue summarizing the bug you find.
```

That is where MCP gets powerful: multi-system workflows without changing AgentX core.
...
Because AgentX now respects your per-server policy and capability-aware registration. That is expected, and usually desirable.
