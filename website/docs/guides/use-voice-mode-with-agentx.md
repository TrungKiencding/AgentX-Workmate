---
sidebar_position: 8
title: "Use Voice Mode with AgentX"
description: "A practical guide to setting up and using AgentX voice mode across CLI, Telegram, Discord, and Discord voice channels"
---

# Use Voice Mode with AgentX
...
- you want AgentX sitting in a Discord voice channel for live conversation
...
There are really three different voice experiences in AgentX.
...
## Step 1: make sure normal AgentX works first
...
AgentX supports both local and cloud speech stacks.
...
### If you use `agentx setup`

If you choose NeuTTS in the setup wizard, AgentX checks whether `neutts` is already installed. If it is missing, the wizard tells you NeuTTS needs the Python package `neutts` and the system package `espeak-ng`, offers to install them for you, installs `espeak-ng` with your platform package manager, and then runs:
...
4. AgentX transcribes and responds
...
- asking AgentX to structure your thoughts in real time

#### Accessibility / low-typing sessions

If typing is inconvenient, voice mode is one of the fastest ways to stay in the full AgentX loop.

## Tuning CLI behavior

### Silence threshold

If AgentX starts/stops too aggressively, tune:
...
AgentX stays a normal chat bot, but can speak replies.
...
- you want AgentX to function like a portable research or ops assistant
...
AgentX joins a Discord VC, listens to user speech, transcribes it, runs the normal agent pipeline, and speaks replies back into the channel.
...
- AgentX detects speech boundaries
- transcripts are posted in the associated text channel
- AgentX responds in text and audio
...
1. get text AgentX working
2. run `agentx setup tts` to enable voice support
