...
```bash
agentx teams-pipeline validate
```
...
```bash
agentx teams-pipeline token-health
agentx teams-pipeline token-health --force-refresh
```
...
```bash
agentx teams-pipeline subscriptions
```

### Renew near-expiry subscriptions

```bash
agentx teams-pipeline maintain-subscriptions
agentx teams-pipeline maintain-subscriptions --dry-run
```
...
#### Option 1: AgentX cron (recommended if you already run the AgentX gateway)

AgentX ships a built-in cron scheduler. The `--no-agent` mode runs a script as the job (rather than using an LLM), and `--script` must point at a file under `~/.agentx/scripts/`. First create the script:

```bash
mkdir -p ~/.agentx/scripts
cat > ~/.agentx/scripts/maintain-teams-subscriptions.sh <<'EOF'
#!/usr/bin/env bash
exec agentx teams-pipeline maintain-subscriptions
EOF
chmod +x ~/.agentx/scripts/maintain-teams-subscriptions.sh
```

Then register a script-only cron job that runs every 12 hours (gives 6x headroom against the 72h expiry window):

```bash
agentx cron create "0 */12 * * *" \
...
Create `/etc/systemd/system/agentx-teams-pipeline-maintain.service`:

```ini
[Unit]
Description=AgentX Teams pipeline subscription maintenance
After=network-online.target

[Service]
Type=oneshot
User=agentx
EnvironmentFile=/etc/agentx/env
ExecStart=/usr/local/bin/agentx teams-pipeline maintain-subscriptions
```

And `/etc/systemd/system/agentx-teams-pipeline-maintain.timer`:

```ini
[Unit]
Description=Run AgentX Teams pipeline subscription maintenance every 12 hours
...
sudo systemctl enable --now agentx-teams-pipeline-maintain.timer
systemctl list-timers agentx-teams-pipeline-maintain.timer
```
...
```bash
agentx teams-pipeline subscriptions   # should show expirationDateTime advanced
agentx teams-pipeline maintain-subscriptions --dry-run   # should show "0 expiring soon" most of the time
```
...
```bash
agentx teams-pipeline list
agentx teams-pipeline list --status failed
agentx teams-pipeline show <job-id>
```

### Replay a stored job

```bash
agentx teams-pipeline run <job-id>
```

### Dry-run meeting artifact fetches

```bash
agentx teams-pipeline fetch --meeting-id <meeting-id>
agentx teams-pipeline fetch --join-web-url "<join-url>"
```
...
- run `agentx teams-pipeline maintain-subscriptions --dry-run`
- inspect `agentx teams-pipeline list --status failed`
...
- run `agentx teams-pipeline validate`
...
Check:
- whether you manually replayed a job with `agentx teams-pipeline run`
...
- [ ] `agentx teams-pipeline validate` returns an OK snapshot
- [ ] `agentx teams-pipeline token-health --force-refresh` succeeds
- [ ] **`maintain-subscriptions` is scheduled** (AgentX cron, systemd timer, or crontab — see [Automating subscription renewal](#automating-subscription-renewal-required-for-production)). Without this, Graph subscriptions silently expire within 72 hours.
