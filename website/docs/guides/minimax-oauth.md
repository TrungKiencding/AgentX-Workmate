...
sidebar_position: 15
title: "MiniMax OAuth"
description: "Log into MiniMax via browser OAuth and use MiniMax-M2.7 models in AgentX Workmate — no API key required"
---

# MiniMax OAuth

AgentX Workmate supports **MiniMax** through a browser-based OAuth login flow, using the same credentials as the [MiniMax portal](https://www.minimax.io). No API key or credit card is required — log in once and AgentX automatically refreshes your session.
...
# → AgentX opens your browser to the MiniMax authorization page
...
After the first login, credentials are stored under `~/.agentx/auth.json` and are refreshed automatically before each session.
...
```bash
agentx auth add minimax-oauth --no-browser
```

AgentX will print the verification URL and user code — open the URL on any device and enter the code when prompted.

## The OAuth Flow

AgentX implements a PKCE browser OAuth flow against the MiniMax OAuth endpoints:

1. AgentX generates a PKCE verifier / challenge pair and a random state value.
...
4. AgentX polls `{base_url}/oauth/token` until the token arrives (or the deadline passes).
5. Tokens (`access_token`, `refresh_token`, expiry) are saved to `~/.agentx/auth.json` under the `minimax-oauth` key.
...
```bash
agentx config set model.default MiniMax-M2.7
agentx config set model.provider minimax-oauth
```

## Configuration Reference

After login, `~/.agentx/config.yaml` will contain entries similar to:
...
```bash
agentx --provider minimax-oauth    # canonical
agentx --provider minimax-portal   # alias
agentx --provider minimax-global   # alias
agentx --provider minimax_oauth    # alias (underscore form)
```
...
To use `minimax-oauth` as the active provider, set `model.provider: minimax-oauth` in `config.yaml` (use `agentx setup` for the guided flow), or pass `--provider minimax-oauth` for a single invocation:

```bash
agentx --provider minimax-oauth
```
...
AgentX refreshes the token on every session start if it is within 60 seconds of expiry. If the access token is already expired (for example, after a long offline period), the refresh happens automatically on the next request. If refresh fails with `refresh_token_reused` or `invalid_grant`, AgentX marks the session as requiring re-login.

When the refresh failure is terminal (HTTP 4xx, `invalid_grant`, revoked grant, etc.), AgentX marks the refresh token as dead and quarantines it locally so it doesn't keep replaying the doomed exchange. The agent surfaces a single "re-authentication required" message and stays out of the way until you log in again.

**Fix:** run `agentx auth add minimax-oauth` again to start a fresh login. The quarantine clears on the next successful exchange.

### Authorization timed out

The device-code flow has a finite expiry window. If you don't approve the login in time, AgentX raises a timeout error.

**Fix:** re-run `agentx auth add minimax-oauth` (or `agentx model`). The flow starts fresh.

### State mismatch (possible CSRF)

AgentX detected that the `state` value returned by the authorization server does not match what it sent.
...
If `agentx` cannot open a browser window, use `--no-browser`:

```bash
agentx auth add minimax-oauth --no-browser
```

AgentX prints the URL and code. Open the URL on any device and complete the flow there.
...
**Fix:** run `agentx model` and select MiniMax (OAuth), or run `agentx auth add minimax-oauth`.
...
```bash
agentx auth logout minimax-oauth
```
...
- [agentx doctor](../reference/cli-commands.md)
