...
You need **tenant admin rights** (or an admin to grant consent on your behalf) to finish this. Bookmark the values you collect — they go into `~/.agentx/.env` at the end.
...
Put the three values you collected into `~/.agentx/.env`:
...
2. Update `MSGRAPH_CLIENT_SECRET` in `~/.agentx/.env` with the new value.
