...
Copy-paste blueprints for common automation patterns. Each blueprint uses AgentX's built-in [cron scheduler](/user-guide/features/cron) for time-based triggers and [webhook platform](/user-guide/messaging/webhooks) for event-driven triggers.
...
| **GitHub Event** | Fires on PR opens, pushes, issues, CI results | Webhook platform (`agentx webhook subscribe`) |
| **API Call** | External service POSTs JSON to your endpoint | Webhook platform (config.yaml routes or `agentx webhook subscribe`) |
...
```bash
agentx cron create "0 2 * * *" \
  "You are a project manager triaging the TrungKiencding/AgentX-Workmate GitHub repo.

1. Run: gh issue list --repo TrungKiencding/AgentX-Workmate --state open --json number,title,labels,author,createdAt --limit 30
...
```bash
agentx webhook subscribe github-pr-review \
  --events "pull_request" \
...
```bash
agentx cron create "0 9 * * 1" \
  "Scan the TrungKiencding/AgentX-Workmate repo for documentation drift.

1. Run: gh pr list --repo TrungKiencding/AgentX-Workmate --state merged --json number,title,files,mergedAt --limit 30
...
```bash
agentx cron create "0 6 * * *" \
  "Run a dependency security audit on the agentx-agent project.

1. cd ~/.agentx/agentx-agent && source .venv/bin/activate
...
```bash
agentx webhook subscribe deploy-verify \
  --events "deployment" \
...
```bash
agentx webhook subscribe alert-triage \
...
```python title="~/.agentx/scripts/check-uptime.py"
...
for ep in ENDPOINTS:
    try:
        start = time.time()
        req = urllib.request.Request(ep["url"], headers={"User-Agent": "AgentX-Monitor/1.0"})
...
agentx cron create "every 30m" \
  ...
  --script ~/.agentx/scripts/check-uptime.py \
  --name "Uptime monitor" \
  --deliver telegram
```
...
```bash
agentx cron create "0 8 * * *" \
  "Search arXiv for the 3 most interesting papers on 'language model reasoning' OR 'tool-use agents' from the past day. For each paper, create an Obsidian note with the title, authors, abstract summary, key contribution, and potential relevance to AgentX Workmate development." \
...
```bash
agentx webhook subscribe auto-port \
  --events "pull_request" \
...
```bash
agentx webhook subscribe stripe-payments \
...
```bash
agentx cron create "0 3 * * 0" \
  "Run a comprehensive security audit of the agentx-agent codebase.
