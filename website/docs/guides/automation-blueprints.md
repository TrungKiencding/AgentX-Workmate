...
agentx cron create "0 2 * * *" \
  "You are a project manager triaging the AstralX/agentx-workmate GitHub repo.

1. Run: gh issue list --repo AstralX/agentx-workmate --state open --json number,title,labels,author,createdAt --limit 30
...
agentx cron create "0 9 * * 1" \
  "Scan the AstralX/agentx-workmate repo for documentation drift.

1. Run: gh pr list --repo AstralX/agentx-workmate --state merged --json number,title,files,mergedAt --limit 30
