...
| `--no-backup` | Skip the pre-migration zip snapshot of `~/.agentx/` (by default a single restore-point archive is written before apply, under `~/.agentx/backups/pre-migration-*.zip`; restorable with `hermes import`). |
