# Contributing to AgentX Workmate

Thank you for contributing to AgentX Workmate! This guide covers everything you need: setting up your dev environment, understanding the architecture, deciding what to build, and getting your PR merged.
...
2. **Cross-platform compatibility** — macOS, different Linux distros, and WSL2 on Windows. We want AgentX to work everywhere.
...
- **Search both open *and* merged PRs and issues** for your topic or error symptom — the duplicate-check in the PR template fires at review time, after you've already done the work:
  ```bash
  gh search issues --repo TrungKiencding/AgentX-Workmate "<your terms>"
  gh search prs --repo TrungKiencding/AgentX-Workmate --state all "<your terms>"
  ```
  Or use the web UI: [issues](https://github.com/TrungKiencding/AgentX-Workmate/issues?q=) · [PRs (all states)](https://github.com/TrungKiencding/AgentX-Workmate/pulls?q=is%3Apr).
...
Related: #38284 covers the agent-side analog — AgentX itself checking existing issues and PRs before deep self-troubleshooting. This section is the human-contributor complement.
...
Bundled skills (in `skills/`) ship with every AgentX install. They should be **broadly useful to most users**:

- Document handling, web research, common dev workflows, system administration
- Used regularly by a wide range of people

If your skill is official and useful but not universally needed (e.g., a paid service integration, a heavyweight dependency), put it in **`optional-skills/`** — it ships with the repo but isn't activated by default. Users can discover it via `agentx skills browse` (labeled "official") and install it with `agentx skills install` (no third-party warning, built-in trust).

If your skill is specialized, community-contributed, or niche, it's better suited for a **Skills Hub** — upload it to a skills registry and share it in the [GitHub Discussions](https://github.com/TrungKiencding/AgentX-Workmate/discussions). Users can install it with `agentx skills install`.

---

## Memory Providers: Ship as a Standalone Plugin

**We are no longer accepting new memory providers into this repo.** The set of built-in providers under `plugins/memory/` (honcho, mem0, supermemory, byterover, hindsight, holographic, openviking, retaindb) is closed. If you want to add a new memory backend, publish it as a **standalone plugin repo** that users install into `~/.agentx/plugins/` (or via a pip entry point).
...
- Integrate with `agentx memory setup` via `post_setup()` — no need to touch core code
...
The reason is maintenance load, not quality. Every external product absorbed into the core tree becomes ours to keep working against a fast-moving codebase, for a backend we don't own and can't control. AgentX ships a lot and the core moves quickly; coupling third-party products into it creates an open-ended burden on the maintainers.

Publish these as a **standalone plugin repo** instead:

- Implement the relevant ABC and use the existing plugin discovery path (`~/.agentx/plugins/`, project `.agentx/plugins/`, or a pip entry point) — see [Build an AgentX Plugin](https://github.com/TrungKiencding/AgentX-Workmate/tree/main/website/docs)
- Register lifecycle hooks (`pre_tool_call`, `post_tool_call`, `pre_llm_call`, `post_llm_call`, `on_session_start`, `on_session_end`), tools (`ctx.register_tool`), and CLI subcommands (`ctx.register_cli_command`) through the surface we already expose — no core changes needed
- If your plugin needs a capability the framework doesn't expose, that's a feature request to **widen the generic plugin surface** (a new hook or `ctx` method) — never special-case your plugin in core
- Promote it in the [GitHub Discussions](https://github.com/TrungKiencding/AgentX-Workmate/discussions) `#plugins-skills-and-skins` channel so users can find and install it
...
Use this only if you intentionally do not want AgentX' managed install layout
(for example, a throwaway clone inside a container or CI job). If you install
this way, make sure you run the `agentx` entrypoint from this venv; running the
...
```bash
mkdir -p ~/.agentx/{cron,sessions,logs,memories,skills}
cp cli-config.yaml.example ~/.agentx/config.yaml
touch ~/.agentx/.env

# Add at minimum an LLM provider key:
echo "OPENROUTER_API_KEY=***" >> ~/.agentx/.env
```

### Run

```bash
# The standard installer already put `agentx` on PATH.
agentx doctor
agentx chat -q "Hello"
```

If you used the manual clone fallback, run `./agentx` from the checkout or
symlink this clone's venv explicitly:

```bash
mkdir -p ~/.local/bin
ln -sf "$(pwd)/venv/bin/agentx" ~/.local/bin/agentx
```
...
├── toolsets.py               # Tool groupings and presets (agentx-cli, agentx-telegram, etc.)
...
### User configuration (stored in `~/.agentx/`)

| Path | Purpose |
|------|---------|
| `~/.agentx/config.yaml` | Settings (model, terminal, toolsets, compression, etc.) |
| `~/.agentx/.env` | API keys and secrets |
| `~/.agentx/auth.json` | OAuth credentials (Nous Portal) |
| `~/.agentx/skills/` | All active skills (bundled + hub-installed + agent-created) |
| `~/.agentx/memories/` | Persistent memory (MEMORY.md, USER.md) |
| `~/.agentx/state.db` | SQLite session database |
| `~/.agentx/sessions/` | Gateway routing index (`sessions.json`), request-dump breadcrumbs, gateway `*.jsonl` transcripts, and (optionally) per-session JSON snapshots when `sessions.write_json_snapshots: true` is set. The per-session snapshots are off by default; state.db is canonical. |
| `~/.agentx/cron/` | Scheduled job data |
| `~/.agentx/whatsapp/session/` | WhatsApp bridge credentials |
...
- **Session persistence**: All conversations are stored in SQLite (`hermes_state.py`) with full-text search and unique session titles. Per-session JSON snapshots in `~/.agentx/sessions/` were superseded by the SQLite store and are off by default; opt back in with `sessions.write_json_snapshots: true` if you have external tooling that consumes the JSON files directly.
...
(for example `_AGENTX_CORE_TOOLS` or a dedicated toolset); otherwise the tool
...
The user may skip setup and keep loading the skill. AgentX only exposes metadata (`stored_as`, `skipped`, `validated`) to the model — never the secret value.
...
Gateway and messaging sessions never collect secrets in-band; they instruct the user to run `agentx setup` or update `~/.agentx/.env` locally.
...
4. **`author` credits the human contributor first.** For external contributions, the contributor's real name + GitHub handle goes first (`Jane Doe (jane-doe)`); "AgentX Workmate" is the secondary collaborator. If the contributor's commit shows "AgentX Workmate" as author because they used AgentX to draft the skill, replace it with their actual name — credit the human, not the tool.
...
- **No external dependencies unless absolutely necessary.** Prefer stdlib Python, curl, and existing AgentX tools (`web_extract`, `terminal`, `read_file`).
...
- **Test it.** Run `agentx --toolsets skills -q "Use the X skill to do Y"` and verify the agent follows the instructions correctly.

---

## Adding a Skin / Theme

AgentX uses a data-driven skin system — no code changes needed to add a new skin.

**Option A: User skin (YAML file)**

Create `~/.agentx/skins/<name>.yaml`:
...
AgentX runs on Linux, macOS, and native Windows (plus WSL2). When writing code
...
1. **Never call `os.kill(pid, 0)` for liveness checks.** `os.kill(pid, 0)`
   ...
   **Preferred:** use `psutil` (a core dependency — always available):
   ...
   If you specifically need the agentx wrapper (it has a stdlib fallback
...
AgentX has terminal access. Security matters.
...
2. **Test manually**: Run `agentx` and exercise the code path you changed
...
- Use [GitHub Issues](https://github.com/TrungKiencding/AgentX-Workmate/issues)
- Include: OS, Python version, AgentX version (`agentx version`), full error traceback
...
- **Discussions**: [GitHub Discussions](https://github.com/TrungKiencding/AgentX-Workmate/discussions) — for questions, showcasing projects, and sharing skills
