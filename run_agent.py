...
def _launch_cwd_for_session(source: str) -> Optional[str]:
    ...
    launched from is meaningful for ``agentx -c`` / ``--resume`` (relaunch
...
def _safe_session_filename_component(session_id: str) -> str:
    ...
    ``X-Agentx-Session-Id`` API header) and are otherwise interpolated raw
    into on-disk artifact filenames under ``~/.agentx/sessions/``.  Without
...
class AIAgent:
    ...
    _TOOL_CALL_ARGUMENTS_CORRUPTION_MARKER = (
        "[agentx-agent: tool call arguments were corrupted in this session and "
    ...
    def _ensure_db_session(self) -> None:
        ...
        try:
            ...
            # `agentx --resume`. set_session_yolo() no-ops on a missing row, so
    ...
    def _vprint(self, *args, force: bool = False, **kwargs):
        ...
        parseable single-query flows such as ``agentx chat -q``. In that mode,
    ...
    def _resolved_api_call_timeout(self) -> float:
        ...
        Priority:
          ...
          3. ``AGENTX_API_TIMEOUT`` env var (legacy escape hatch)
          4. 1800.0s default
        ...
        helper, the hardcoded ``AGENTX_API_TIMEOUT`` fallback would always be
        ...
        if cfg is not None:
            return cfg
        return env_float("AGENTX_API_TIMEOUT", 1800.0)
    ...
    def _resolved_api_call_stale_timeout_base(self) -> tuple[float, bool]:
        ...
        Priority:
          ...
          3. ``AGENTX_API_CALL_STALE_TIMEOUT`` env var
        ...
        env_timeout = os.getenv("AGENTX_API_CALL_STALE_TIMEOUT")
    ...
    def _codex_silent_hang_hint(self, model: Optional[str] = None) -> Optional[str]:
        ...
        agentx-agent #21444 for the symptom history.  The upstream backend
        ...
        return (
            ...
            "See agentx-agent#21444 for symptom history."
        )
    ...
    def _decorate_xai_entitlement_error(detail: str) -> str:
        ...
        SuperGrok subscribers can use AgentX against xai-oauth.  Lots of
    ...
    def _summarize_api_error(error: Exception) -> str:
        ...
        while current is not None and id(current) not in seen:
            ...
            if any(
                ...
                return (
                    "AgentX can't reach the model provider. You may be offline. "
    ...
    def _hook_payload_max_chars() -> int:
        raw = os.getenv("AGENTX_PLUGIN_PAYLOAD_MAX_CHARS", "50000")
        try:
    ...
    def _redact_message_content(content):
        ...
        Respects ``AGENTX_REDACT_SECRETS`` via ``redact_sensitive_text`` —
    ...
    def _save_session_log(self, messages: List[Dict[str, Any]] = None):
        ...
        whose external tooling consumes ``~/.agentx/sessions/session_{sid}.json``
        ...
        # untrusted input (X-Agentx-Session-Id header) and must not escape
        ...
        try:
            ...
            for msg in messages:
                ...
                # output, or user paste. Respects AGENTX_REDACT_SECRETS via
    ...
    def interrupt(self, message: Optional[str] = None, *, hard_cancel: bool = False) -> None:
        ...
        # interrupt event rather than AgentX' per-thread flag.
    ...
    def redirect(self, text: str) -> bool:
        ...
        During a normal AgentX model request this cancels only that request;
    ...
    def _file_mutation_verifier_enabled(self) -> bool:
        ...
        ``AGENTX_FILE_MUTATION_VERIFIER`` env var overrides config.  Exposed
        ...
        try:
            import os as _os
            env = _os.environ.get("AGENTX_FILE_MUTATION_VERIFIER")
            if env is not None:
    ...
    def _format_file_mutation_failure_footer(cls, failed: Dict[str, Dict[str, Any]]) -> str:
        ...
        (e.g. ``~/.agentx/config.yaml``) to a messaging channel (#35584).
        """
        if not failed:
            return ""
        lines = [
    ...
    def _turn_completion_explainer_enabled(self) -> bool:
        ...
        True).  ``AGENTX_TURN_COMPLETION_EXPLAINER`` env var overrides
        ...
        try:
            import os as _os
            env = _os.environ.get("AGENTX_TURN_COMPLETION_EXPLAINER")
            if env is not None:
                return env.strip().lower() not in {"0", "false", "no", "off"}
        ...
        if reason == "session_persistence_failed":
            ...
            if cause == "locked":
                return (
                    ...
                    "(another AgentX process was writing to the state "
            ...
            return (
                ...
                "Check the state database health (`agentx doctor`), then "
    ...
    def _touch_activity(
        ...
        *,
        ...
        process is a dispatcher-spawned worker (AGENTX_KANBAN_TASK set),
    ...
    def _capture_credits(self, http_response: Any) -> None:
        ...
        """
        # Dev test fixture (AGENTX_DEV_CREDITS_FIXTURE): inject a chosen notice state
        ...
        if _fixture is not None:
            ...
            logger.info(
                ...
                "(real headers bypassed — `echo clear` / unset AGENTX_DEV_CREDITS_FIXTURE to restore)",
        ...
        if not headers:
            return
        _dev = is_truthy_value(os.environ.get("AGENTX_DEV_CREDITS"))
        ...
        if _dev:
            # AGENTX_DEV_CREDITS: stream each capture to agent.log — watch live with
            # `agentx logs -f` (grep 'credits ▸'). Dev-only; silent for normal users.
    ...
    def close(self) -> None:
        ...
        # a private embedded daemon when AgentX YOLO selected unrestricted
    ...
    def _build_keepalive_http_client(base_url: str = "", *, verify: Any = True) -> Any:
        ...
        ``AGENTX_CA_BUNDLE`` settings.  It is passed on the client AND on
    ...
    def _try_refresh_codex_client_credentials(self, *, force: bool = True) -> bool:
        ...
        # pool entry (``agentx auth add xai-oauth``) whose tokens belong to
        ...
        *,
        ...
        try:
            ...
            creds = resolve_nous_runtime_credentials(
                timeout_seconds=env_float("AGENTX_NOUS_TIMEOUT_SECONDS", 15),
                force_refresh=force,
            )
        except Exception as exc:
    ...
    def _try_refresh_env_client_credentials(self) -> bool:
        """Adopt ~/.agentx/.env credential/base-url edits at the turn boundary.

        A Settings save (desktop ``PUT /api/env``, ``agentx setup``) updates
    ...
    def _compress_context(
        ...
        *,
        ...
        try:
            ...
            # AGENTX_SESSION_ID to the child id after an out-of-place
