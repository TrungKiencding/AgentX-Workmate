...
def test_ensure_management_token_persists_and_is_stable(hermes_home):
    ...
    assert t1 == t2
    assert t1.startswith("agentx-mgmt-")
...
def test_start_proxy_injects_management_key_env(hermes_home, monkeypatch):
    ...
    assert captured_env[ip._MGMT_API_KEY_ENV].startswith("agentx-mgmt-")
...
def test_docker_egress_node_options_uses_sentinel(hermes_home, monkeypatch):
    ...
    ``_AGENTX_EGRESS_NODE_OPTIONS_APPEND`` so DockerEnvironment can
    ...
    assert env.get("_AGENTX_EGRESS_NODE_OPTIONS_APPEND") == "--use-openssl-ca"
