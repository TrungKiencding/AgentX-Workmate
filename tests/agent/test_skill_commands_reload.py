...
def hermes_home(monkeypatch):
    """Isolate AGENTX_HOME for ``reload_skills`` tests.

    Rather than popping cache-bearing modules from ``sys.modules``,
    we monkeypatch the module-level ``AGENTX_HOME`` / ``SKILLS_DIR``
    constants in place so the isolation is local to this fixture's scope.
    """
    td = tempfile.mkdtemp(prefix="agentx-reload-skills-")
    monkeypatch.setenv("AGENTX_HOME", td)
    home = Path(td)
    ...
    monkeypatch.setattr(_st, "AGENTX_HOME", home, raising=False)
