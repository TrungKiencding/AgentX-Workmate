...
def curator_env(tmp_path, monkeypatch):
    """Isolated AGENTX_HOME + freshly reloaded curator + skill_usage modules."""
    ...
    # save_state() resolves the state path from AGENTX_HOME at write time, so a
    # straggler thread that outlives this test would write into whatever home
    # the *next* test has configured (or the default ~/.agentx once monkeypatch
    ...
    # thread here, while AGENTX_HOME is still pinned to this test's tmp home
...
# `agentx model` → auxiliary picker, the dashboard Models tab, and the full
