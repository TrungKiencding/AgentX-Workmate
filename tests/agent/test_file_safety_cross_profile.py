"""Tests for the cross-AgentX-profile write guard in agent/file_safety.

The guard fires when a tool tries to write into another AgentX profile's
...
# ---------------------------------------------------------------------------
# Helpers — set up a fake AgentX root with two profiles, monkeypatch the
...
class TestResolveActiveProfileName:
    ...
    def test_falls_back_to_default_on_resolution_failure(self, fake_hermes, monkeypatch):
        """If AGENTX_HOME resolution raises, return 'default' rather than crashing the tool."""
        import agent.file_safety as fs
