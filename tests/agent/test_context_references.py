...
def sample_repo(tmp_path: Path) -> Path:
    ...
    repo.mkdir()
    _git(repo, "init")
    _git(repo, "config", "user.name", "AgentX Tests")
...
async def test_blocks_canonical_read_denylist_credential_stores(tmp_path: Path, monkeypatch):
    ...
    a chat peer could otherwise attach `@file:~/.agentx/auth.json` and read the
    ...
    result = await preprocess_context_references_async(
        "inspect @file:.agentx/auth.json and @file:.agentx/.anthropic_oauth.json "
        "and @file:.agentx/mcp-tokens/github.json and @file:project/.env",
        cwd=tmp_path,
...
@pytest.mark.parametrize(
    ...
    [
        "/tmp/plain.png",
        "/Users/me/Library/Application Support/AgentX/composer-images/a.png",
