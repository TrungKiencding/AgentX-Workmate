...
def test_coding_prompt_preserves_legacy_workspace_order(monkeypatch):
    ...
    monkeypatch.setattr(system_prompt, "AGENTX_AGENT_HELP_GUIDANCE", "HELP")
    monkeypatch.setattr(system_prompt, "STEER_CHANNEL_NOTE", "STEER")
    monkeypatch.setattr(system_prompt, "get_hermes_home", lambda: Path("/agentx"))

    expected_profile = (
        "Active AgentX profile: default. Other profiles (if any) live "
        "under /agentx/profiles/<name>/. Each profile has its own skills/, "
...
class TestTelegramRichMessagesHint:
    ...
    def test_gateway_rich_messages_integration_via_real_config(self, tmp_path, monkeypatch):
        ...
        under AGENTX_HOME with ``gateway.platforms.telegram.extra.rich_messages``
