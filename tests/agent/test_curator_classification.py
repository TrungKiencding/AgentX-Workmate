...
def test_reconcile_absorbed_into_beats_everything_else(curator_env):
    ...
    out = curator_env._reconcile_classification(
        ...
        destinations={"agentx-agent-dev"},
        absorbed_declarations={
            "pr-review-format": {"into": "agentx-agent-dev", "declared": True},
        },
    )
    ...
    assert e["into"] == "agentx-agent-dev"
...
# `agentx curator status`). The full data has always been in REPORT.md on
...
# ---------------------------------------------------------------------------
# Pin hint — surfaces `agentx curator pin <umbrella>` in the rename block so
