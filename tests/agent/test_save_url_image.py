...
def http_server(tmp_path, monkeypatch):
    """Spin up a localhost HTTP server and isolate AGENTX_HOME under tmp_path."""
    monkeypatch.setenv("AGENTX_HOME", str(tmp_path / ".agentx"))
    (tmp_path / ".agentx").mkdir()

    # Force the constants/image cache helpers to re-read AGENTX_HOME.
    import sys
...
class TestSaveUrlImage:
    def test_writes_real_bytes_to_hermes_home_cache(self, http_server):
        ...
        # The cache directory must be under AGENTX_HOME — gateway cleanup
