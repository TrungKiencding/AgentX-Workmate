...
normal tests assume away:
  ...
  - Filesystem: AGENTX_HOME with spaces / unicode / symlinks.
...
def scenario(name):
    """Decorator: run `fn` in its own AGENTX_HOME, collect failures.
...
def _(home, kb):
    ...
    env = {**os.environ, "PYTHONPATH": str(WT), "AGENTX_HOME": home, "HOME": home}
    bad_metas = [
        "not-json",
...
def _(home, kb):
    ...
    try:
        ...
        try:
            ...
            home_abs = str(Path(os.environ["AGENTX_HOME"]).resolve())
...
def _(home, kb):
    """AGENTX_HOME at a path with spaces — should work but catches
    ...
    weird = tempfile.mkdtemp(prefix="agentx with spaces ")
    os.environ["AGENTX_HOME"] = weird
    ...
    try:
        ...
        print(f"  AGENTX_HOME with spaces: OK at {weird}")
    finally:
        conn.close()
...
def _(home, kb):
    """AGENTX_HOME with non-ASCII chars."""
    ...
    try:
        ...
        print(f"  AGENTX_HOME with unicode path: OK at {weird}")
    finally:
        conn.close()
        shutil.rmtree(weird, ignore_errors=True)
...
def _(home, kb):
    """AGENTX_HOME is a symlink to the real dir. _INITIALIZED_PATHS
    ...
    try:
        ...
        conn2.close()
        print("  symlinks to same AGENTX_HOME share DB correctly")
    finally:
