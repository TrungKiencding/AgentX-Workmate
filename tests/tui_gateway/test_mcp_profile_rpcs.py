...
``AGENTX_HOME`` with named profile dirs — no mocks of the config/mcp layer — and
...
def hermes_root(tmp_path, monkeypatch):
    """A temp AGENTX_HOME root with two named profiles: 'work' and 'other'.

    Pointing AGENTX_HOME at a dir outside ~/.agentx makes it the profile ROOT
...
def test_default_profile_add_when_profile_omitted(hermes_root):
    ...
    # Omitted profile → launch/default profile == AGENTX_HOME root config.yaml.
    default_cfg = _read_yaml(root / "config.yaml")
