...
AgentX's own top-level modules (``utils``, ``proxy``, ``ui``).  entry.py must
...
def test_guard_handles_absolute_cwd_path():
    ...
    path, ahead of the AgentX root.  harden_import_path must relocate the
    AgentX root to the front so ``from utils import ...`` resolves to AgentX."""
    import sys

    original = sys.path[:]
    try:
        sys.path[:] = ["/home/user/tg-ws-proxy", "/opt/agentx", "/usr/lib"]
        hermes_bootstrap.harden_import_path(src_root="/opt/agentx")
        assert sys.path[0] == "/opt/agentx"
        assert sys.path.index("/opt/agentx") < sys.path.index(
