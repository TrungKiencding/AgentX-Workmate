...
def test_disable_flush_env_var_actually_wires_to_module_constant(monkeypatch):
    """End-to-end: setting `AGENTX_TUI_GATEWAY_NO_FLUSH=1` and importing
    ...
    monkeypatch.setenv("AGENTX_TUI_GATEWAY_NO_FLUSH", "1")
    ...
    try:
        ...
        monkeypatch.delenv("AGENTX_TUI_GATEWAY_NO_FLUSH", raising=False)
...
def test_slash_exec_rejects_skill_commands(server):
    ...
    fake_skills = {"/agentx-agent-dev": {"name": "agentx-agent-dev", "description": "Dev workflow"}}

    with patch("agent.skill_commands.get_skill_commands", return_value=fake_skills):
        resp = server.handle_request({
            "id": "r1",
            "method": "slash.exec",
            "params": {"command": "agentx-agent-dev", "session_id": sid},
        })
...
def test_slash_exec_scopes_skill_lookup_to_session_profile(server, tmp_path):
    ...
    profile_home rather than the gateway process's ambient AGENTX_HOME
    ...
    # The gateway's own AGENTX_HOME (the test-isolation tempdir, no
...
def test_skin_live_switch_end_to_end(server, tmp_path, monkeypatch):
    """Real config + skin files: activating a skin (as `agentx config set` does)
    ...
    # Activate midnight, as `agentx config set display.skin midnight` would.
...
def test_peerless_global_broadcast_never_reaches_stdout_in_ws_backend(capture, monkeypatch):
    """`agentx serve` / dashboard speak JSON-RPC over WS only; Desktop captures their stdout
    into desktop.log. After the last WS client leaves, the change watcher keeps ticking and
    the orphan reaper announces reclaims — those frames must be dropped, not printed as raw
    `[agentx] {"jsonrpc": ...}` lines."""
    server, buf = capture
    ...
    server._broadcast_global_event("session.reclaimed", {"session_id": "gone"})

    assert [f["params"]["type"] for f in a.frames] == ["sessions.changed"]
    assert buf.getvalue() == ""
