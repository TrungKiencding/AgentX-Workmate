...
Override the image with ``AGENTX_TEST_IMAGE`` env var to point at a pre-built
...
IMAGE_TAG = os.environ.get("AGENTX_TEST_IMAGE", "agentx-agent-harness:latest")
...
def built_image() -> str:
    ...
    Override with ``AGENTX_TEST_IMAGE`` env var to point at a pre-built
    image (faster local iteration).
    """
    if os.environ.get("AGENTX_TEST_IMAGE"):
        return IMAGE_TAG
...
# ---------------------------------------------------------------------------
# docker_exec — default to the unprivileged agentx user
# ---------------------------------------------------------------------------
#
# Background: every AgentX runtime path inside the container drops to UID
# 10000 (the ``agentx`` user) via ``s6-setuidgid agentx``. ``docker exec``
...
# fails (PermissionError swallowed) for agentx, so a test that ran as root
...
# helpers below run every probe under ``-u agentx`` unless a specific
...
def docker_exec(
    ...
    *args: str,
    ...
    """Run a command inside ``container`` as ``user`` (default: agentx).
