...
Before the s6 migration, ``docker run trungkiencding/agentx-workmate gateway
...
crash and a supervised dashboard alongside (when ``AGENTX_DASHBOARD=1``).
...
These tests verify the three load-bearing properties of that redirect:
  ...
  2. ``--no-supervise`` / ``AGENTX_GATEWAY_NO_SUPERVISE=1`` opts out.
  3. The supervised process itself does NOT recurse — the
     ``AGENTX_S6_SUPERVISED_CHILD`` sentinel breaks the loop.

Every ``docker exec`` runs as ``agentx`` per the conftest module
...
def test_gateway_run_redirects_to_supervised(
    ...
    # process (main-wrapper.sh → python → agentx gateway run) can take
    ...
    # (main-wrapper.sh → python → agentx gateway run) can take well over
...
def test_supervised_gateway_does_not_recurse(
    ...
    """The AGENTX_S6_SUPERVISED_CHILD sentinel must prevent the
    supervised ``agentx gateway run`` from re-entering the redirect.
    ...
    gateway slot would never actually run a python ``agentx gateway
    ...
    # (main-wrapper.sh → python → agentx gateway run) can take well
    ...
    # started either — so we'd see the CMD's `agentx gateway run`
    ...
    # `agentx gateway run`. If the recursion guard fails, s6 would
    ...
    assert n <= 1, (
        f"expected at most one supervised python `agentx gateway run` "
...
def test_dashboard_supervised_when_env_set(
    ...
    """When ``AGENTX_DASHBOARD=1`` is set, ``docker run <image> gateway
