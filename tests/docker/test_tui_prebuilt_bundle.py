...
package-lock.json. Without AGENTX_TUI_DIR set, ``_make_tui_argv`` falls
...
The fix is ``ENV AGENTX_TUI_DIR=/opt/agentx/ui-tui`` in the Dockerfile, which
...
def _exec_py(image: str, py: str) -> str:
    ...
    inner = (
        "source /opt/agentx/.venv/bin/activate && "
        "cd /opt/agentx && "
...
def test_hermes_tui_dir_env_is_set(built_image: str) -> None:
    """AGENTX_TUI_DIR must point at the prebuilt bundle dir in the image."""
    r = subprocess.run(
        ["docker", "run", "--rm", "--entrypoint", "sh", built_image,
         "-c", 'printf "%s" "$AGENTX_TUI_DIR"'],
        capture_output=True, text=True, timeout=60,
    )
    assert r.returncode == 0, r.stderr[-2000:]
    assert r.stdout.strip() == "/opt/agentx/ui-tui", (
        f"AGENTX_TUI_DIR={r.stdout.strip()!r} (expected /opt/agentx/ui-tui)"
    )
...
def test_prebuilt_bundle_present_and_no_runtime_install(built_image: str) -> None:
    ...
    # With AGENTX_TUI_DIR set, _make_tui_argv returns the prebuilt path BEFORE
