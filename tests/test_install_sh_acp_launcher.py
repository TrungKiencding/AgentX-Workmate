"""`setup_path()` must also install a `agentx-acp` launcher.

ACP hosts (Zed, JetBrains, Buzz Desktop) spawn the agent by resolving a command
name against the login-shell PATH. The `agentx-acp` console script generated by
the packaging metadata lives inside the install's venv, which is not on that
PATH, so those hosts report AgentX as not installed. `setup_path()` therefore
writes a `agentx-acp` launcher next to the `agentx` one.
...
ACP_BLOCK = re.compile(
    r'(    rm -f "\$command_link_dir/agentx-acp".*?'
    r'log_success "Installed agentx-acp launcher[^\n]*\n)',
    re.S,
)
...
def _extract_acp_shim_block() -> str:
    ...
    assert match, (
        "could not locate the agentx-acp launcher block in scripts/install.sh — "
...
def _run_block(tmp_path: Path, use_venv: str) -> Path:
    ...
    entrypoint = tmp_path / "agentx"
    entrypoint.write_text("#!/bin/sh\n", encoding="utf-8")

    script = (
        "set -e\n"
        f"AGENTX_BIN={hermes_bin}\n"
        f"AGENTX_ENTRYPOINT={entrypoint}\n"
    ...
    return command_link_dir / "agentx-acp"
...
def test_acp_launcher_does_not_follow_a_symlink_into_the_venv(tmp_path):
    ...
    An older install could leave `agentx-acp` as a symlink to the venv console
    ...
    shim_path = command_link_dir / "agentx-acp"
    ...
    assert console_script.read_text(encoding="utf-8") == marker, (
        "venv/bin/agentx-acp was overwritten — symlink-stomp regression (#21454)"
    )
    assert not shim_path.is_symlink(), (
        "command_link_dir/agentx-acp must be replaced with a regular file"
    )
