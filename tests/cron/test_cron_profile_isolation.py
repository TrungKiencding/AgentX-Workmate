...
profile's AGENTX_HOME and EXECUTE under it.

- Storage: a job created under profile ``coder`` writes to
  ``~/.agentx/profiles/coder/cron/jobs.json`` — NOT the shared default root.
- Execution: the profile-scoped gateway's in-process ticker resolves the
  active AGENTX_HOME (profile home) at call time, so jobs run with that
...
one store and runs them under whatever AGENTX_HOME the ticker happens to have —
...
def _set_profile_env(monkeypatch, root: Path, profile_home: Path) -> None:
    ...
    AGENTX_HOME is a profile under it (``<root>/profiles/<name>``)."""
...
def test_cron_storage_anchors_at_profile_home(tmp_path, monkeypatch):
    """Under a profile AGENTX_HOME (<root>/profiles/<name>), the cron store
    ...
    # cron/jobs.py computes AGENTX_DIR from get_hermes_home() at import, so a
    # fresh import under this env anchors the store at <profile>/cron.
    import cron.jobs as jobs

    importlib.reload(jobs)
    try:
        assert jobs.AGENTX_DIR.resolve() == profile_home.resolve()
        assert (
