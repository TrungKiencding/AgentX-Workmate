...
class TestRunJobConfigEnvVarExpansion:
    ...
    def test_model_env_ref_in_config_yaml_is_expanded(self, tmp_path, monkeypatch):
        ...
        (tmp_path / "config.yaml").write_text("model: ${_AGENTX_TEST_CRON_MODEL}\n")
        monkeypatch.setenv("_AGENTX_TEST_CRON_MODEL", "gpt-4o-mini-cron-test")
    ...
    def test_unexpanded_ref_passthrough_when_var_unset(self, tmp_path, monkeypatch):
        ...
        (tmp_path / "config.yaml").write_text("model: ${_AGENTX_TEST_CRON_UNSET_VAR}\n")
        monkeypatch.delenv("_AGENTX_TEST_CRON_UNSET_VAR", raising=False)
        ...
        assert kwargs["model"] == "${_AGENTX_TEST_CRON_UNSET_VAR}"
...
class TestRunJobModelResolution:
    ...
    ``model: null``. At fire time the scheduler must:
      1. fall back to ``AGENTX_MODEL`` env if set,
    ...
    def test_null_job_model_falls_back_to_env(self, tmp_path, monkeypatch):
        """``model: null`` on the job uses AGENTX_MODEL when set."""
        (tmp_path / "config.yaml").write_text("")
        monkeypatch.setenv("AGENTX_MODEL", "env-model")
...
class TestHomeTargetEnvVarRegistry:
    ...
    entry means ``agentx cron create --deliver=<platform>`` silently
