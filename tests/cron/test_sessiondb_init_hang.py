...
which is already bounded by AGENTX_CRON_TIMEOUT — the worker thread submitted
...
timeout (AGENTX_CRON_SESSION_DB_TIMEOUT, default 10s) so a hang there can
...
class TestSessionDbInitTimeout:
    ...
    def test_invalid_timeout_env_falls_back_to_default(self, tmp_path, monkeypatch, caplog):
        """A malformed AGENTX_CRON_SESSION_DB_TIMEOUT logs a warning and still
        bounds the call (mirrors AGENTX_CRON_TIMEOUT's own fallback)."""
        monkeypatch.setenv("AGENTX_CRON_SESSION_DB_TIMEOUT", "not-a-number")
        fake_db = MagicMock()
        ...
        assert any(
            "AGENTX_CRON_SESSION_DB_TIMEOUT" in rec.message
    ...
    def test_timeout_resolved_from_config_yaml(self, tmp_path, monkeypatch):
        ...
        monkeypatch.delenv("AGENTX_CRON_SESSION_DB_TIMEOUT", raising=False)
