...
Guards against the 60-vs-500 bug where a stale `.env AGENTX_MAX_ITERATIONS=60`
...
def _run_gateway_import(hermes_home: Path, initial_env: dict[str, str]) -> dict[str, str]:
    ...
    script = textwrap.dedent(
        ...
        for k in (
            ...
            "AGENTX_AGENT_TIMEOUT",
            "AGENTX_AGENT_TIMEOUT_WARNING",
            "AGENTX_SESSION_STALL_TIMEOUT",
            "AGENTX_GATEWAY_BUSY_INPUT_MODE",
            "AGENTX_GATEWAY_BUSY_TEXT_MODE",
...
def test_config_gateway_timeout_wins_over_stale_env(hermes_home: Path) -> None:
    ...
    _write_env(hermes_home, {
        "AGENTX_AGENT_TIMEOUT": "60",
        "AGENTX_AGENT_TIMEOUT_WARNING": "30",
        "AGENTX_SESSION_STALL_TIMEOUT": "15",
    })

    env = _run_gateway_import(hermes_home, initial_env={})

    assert env.get("AGENTX_AGENT_TIMEOUT") == "1800"
    assert env.get("AGENTX_AGENT_TIMEOUT_WARNING") == "900"
    assert env.get("AGENTX_SESSION_STALL_TIMEOUT") == "300"
...
def test_config_platform_connect_timeout_supplies_env_when_unset(hermes_home: Path) -> None:
    ...
    assert env.get("AGENTX_GATEWAY_PLATFORM_CONNECT_TIMEOUT") == "90"
...
def test_env_platform_connect_timeout_wins_over_config(hermes_home: Path) -> None:
    ...
    AGENTX_GATEWAY_PLATFORM_CONNECT_TIMEOUT is the manual-override escape hatch:
    ...
    env = _run_gateway_import(
        hermes_home,
        initial_env={"AGENTX_GATEWAY_PLATFORM_CONNECT_TIMEOUT": "120"},
    )

    assert env.get("AGENTX_GATEWAY_PLATFORM_CONNECT_TIMEOUT") == "120"
