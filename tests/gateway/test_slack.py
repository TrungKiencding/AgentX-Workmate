...
class TestAppMentionHandler:
    ...
    def test_app_mention_registered_on_connect(self):
        ...
        # covering every COMMAND_REGISTRY entry (e.g. /agentx, /btw, /stop,
        ...
        for expected in ("/agentx", "/btw", "/stop", "/model", "/help"):
...
class TestSlackConnectCleanup:
    ...
    async def test_disconnect_closes_workspace_clients_and_clears_runtime_state(self):
        ...
        ``agentx gateway run --replace`` takes the old process through the
...
class TestSlashCommands:
    ...
    # instead of as /agentx subcommands. This is the Discord/Telegram parity
    ...
    @pytest.mark.asyncio
    async def test_legacy_agentx_prefix_still_works(self, adapter):
        """Backward compat: /agentx btw foo must still route to /btw foo.

        Old workspace manifests only declared /agentx as the single slash.
...
class TestMissingCredentials:
    ...
    async def test_missing_app_token_sets_fatal_error(self):
        ...
        assert "agentx gateway setup" in fatal_errors[0]["message"].lower() or ".env" in fatal_errors[0]["message"]
...
class TestThreadImageContext:
    ...
    def _prep(self, adapter_with_session_store):
        ...
        a._download_slack_file = AsyncMock(return_value="/tmp/agentx-cached.png")
        return a
    ...
    async def test_cold_start_delivers_thread_root_image(
        ...
        assert msg_event.media_urls == ["/tmp/agentx-cached.png"]
...
class TestSlackUserAgent:
    ...
    ``user_agent_prefix=_AGENTX_SLACK_USER_AGENT_PREFIX`` on every
