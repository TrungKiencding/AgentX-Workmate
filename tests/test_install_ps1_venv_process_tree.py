"""Windows installer regression for AgentX children outside the venv.

The venv sweep deliberately selects process roots by executable path so it
does not kill unrelated Python processes. A selected AgentX process can spawn
...
def test_venv_sweep_stops_managed_runtime_children_but_not_unrelated_processes(
    ...
    install_dir = hermes_home / "agentx-agent"
    venv_scripts = install_dir / "venv" / "Scripts"
    runtime_dir = hermes_home / ".agentx-runtime" / "python" / "generation-test"
    ...
    # Keep the test away from real gateway tasks and real AgentX launchers
