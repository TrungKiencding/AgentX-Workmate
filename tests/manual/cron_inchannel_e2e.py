...
def _fresh_home():
    """Point AGENTX_HOME at a throwaway dir BEFORE importing gateway modules
    (mirror.py binds _SESSIONS_INDEX from get_hermes_home() at import time)."""
    d = tempfile.mkdtemp(prefix="cron_inchannel_e2e_")
    os.environ["AGENTX_HOME"] = d
    return Path(d)


HOME = _fresh_home()

# Import AFTER AGENTX_HOME is set.
...
def main():
    ...
    print(f"AGENTX_HOME (throwaway): {HOME}")
