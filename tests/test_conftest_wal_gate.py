...
``~/.agentx`` before the per-test ``AGENTX_HOME`` redirect, which makes tests
...
def test_conftest_does_not_import_hermes_state_at_collection():
    ...
    ~/.agentx, so tests read live production sessions instead of a tempdir.
    ...
    def guard(name, *args, **kwargs):
        if name == "hermes_state" or name.startswith("hermes_state."):
            ...
            raise AssertionError(
                ...
                "DEFAULT_DB_PATH from the real ~/.agentx during collection"
            )
