...
Key invariants covered by these tests:
  ...
  5. Load order: every AgentX entry point imports hermes_bootstrap as its
...
class TestWindowsBehavior:
    ...
    def test_stdout_reconfigured_to_utf8_on_windows(self):
        # The live process's stdout should now be UTF-8 (the AgentX CLI
...
class TestEntryPointsImportBootstrap:
    """Every AgentX entry point must import hermes_bootstrap as its
    ...
    # Entry points that invoke AgentX as a process.  Each one must
    ...
    def test_entry_point_imports_bootstrap(self, path):
        ...
        half-finished ``agentx update`` (git-reset landed new code but
        ``uv pip install -e .`` didn't finish re-registering
        ``hermes_bootstrap`` as a top-level module) leaves agentx
        ...
        for node in ast.iter_child_nodes(tree):
            ...
            # agentx start even when hermes_bootstrap hasn't been
...
class TestHardenImportPath:
    ...
    directory from shadowing AgentX's own top-level modules — covering both
