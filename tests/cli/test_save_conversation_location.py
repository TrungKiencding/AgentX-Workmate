...
file to be discoverable via ``agentx sessions browse``, but CWD-resident
snapshots are not indexed in the state DB and are generally invisible.
The fix writes snapshots under ``~/.agentx/sessions/saved/`` and prints
...
def test_save_conversation_writes_under_hermes_home(hermes_home, tmp_path, monkeypatch, capsys):
    ...
    # File MUST be under ~/.agentx/sessions/saved/
    ...
    assert "agentx --resume 20260101_120000_abc123" in out, out
