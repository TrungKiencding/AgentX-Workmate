...
class TestSaveConfigValueAtomic:
    ...
    def config_env(self, tmp_path, monkeypatch):
        ...
        # point AGENTX_HOME at the temp dir (the _hermes_home import-time
...
class TestSaveConfigValueTargetsUserConfig:
    """Regression: persisted runtime settings must land in AGENTX_HOME/config.yaml
    ...
    install whose AGENTX_HOME/config.yaml did not exist yet, save_config_value
    fell back to the checked-in cli-config.yaml. The toggle reported success, but
    startup read AGENTX_HOME/config.yaml and never saw the setting."""

    def test_creates_user_config_when_absent(self, tmp_path, monkeypatch):
        # Fresh AGENTX_HOME with NO config.yaml (managed/desktop first launch).
        hermes_home = tmp_path / ".agentx"
        hermes_home.mkdir()
        monkeypatch.setenv("AGENTX_HOME", str(hermes_home))
