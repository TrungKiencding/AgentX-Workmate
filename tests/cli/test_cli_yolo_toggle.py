...
``os.environ["AGENTX_YOLO_MODE"]``. That env var is captured once at
...
prompt. Only ``agentx --yolo`` (process-start env), ``AGENTX_YOLO_MODE=1``,
and ``agentx config set approvals.mode off`` actually bypassed.
...
def _clear_approval_state(monkeypatch):
    ...
    # AgentX-driven test runs may inherit AGENTX_YOLO_MODE=1 from the parent
...
class TestIsSessionYoloActiveHelper:
    ...
    def test_helper_honors_frozen_yolo_mode(self):
        """``agentx --yolo`` sets ``AGENTX_YOLO_MODE`` before tool imports, so
