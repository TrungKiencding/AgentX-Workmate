...
prompt. Only ``hermes --yolo`` (process-start env), ``AGENTX_YOLO_MODE=1``,
...
def _clear_approval_state(monkeypatch):
    ...
    # Hermes-driven test runs may inherit AGENTX_YOLO_MODE=1 from the parent
...
class TestIsSessionYoloActiveHelper:
    ...
    def test_helper_honors_frozen_yolo_mode(self):
        """``hermes --yolo`` sets ``AGENTX_YOLO_MODE`` before tool imports, so
