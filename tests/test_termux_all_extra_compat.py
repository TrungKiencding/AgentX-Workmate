...
def test_pyproject_defines_termux_all_without_known_blockers() -> None:
    ...
    assert '"agentx-workmate[termux]"' in text
    assert '"agentx-workmate[matrix]"' not in text.split("termux-all = [", 1)[1].split("]", 1)[0]
    assert '"agentx-workmate[voice]"' not in text.split("termux-all = [", 1)[1].split("]", 1)[0]
