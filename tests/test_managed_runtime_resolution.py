"""Guard: AgentX-owned subprocesses must not resolve managed runtimes by bare PATH.

AgentX installs runtimes for itself — ``uv`` at ``$AGENTX_HOME/bin/uv``, Node at
``$AGENTX_HOME/node``. Neither directory is on the ambient PATH of an arbitrary
process, so ``shutil.which("uv")`` / ``shutil.which("node")`` in AgentX's own
...
* when a system copy also exists, the one AgentX does not own wins — which is
...
# Runtimes AgentX provisions into AGENTX_HOME and must therefore resolve
# through a managed-aware helper rather than PATH.
_MANAGED_COMMANDS = frozenset({"uv", "node", "npm", "npx"})

# Directories that are not AgentX-owned subprocess code: plugins ship their own
