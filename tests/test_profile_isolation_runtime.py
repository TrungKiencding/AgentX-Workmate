...
``_AGENTX_HOME_OVERRIDE`` ContextVar, not the process environment.  State that
...
with real temp AGENTX_HOME directories (no mocks) and assert the *active*
...
def two_profiles(tmp_path):
    """Two distinct profile AGENTX_HOME dirs with the dir skeleton created."""
...
class TestRichSentStorePathResolution:
    ...
    def test_store_path_follows_override(self, two_profiles, monkeypatch):
        prof_a, prof_b = two_profiles
        # Ensure no ambient AGENTX_HOME env masks the test.
