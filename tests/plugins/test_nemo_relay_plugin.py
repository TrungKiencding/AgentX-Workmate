...
""",
    ...
    monkeypatch.setenv("AGENTX_NEMO_RELAY_PLUGINS_TOML", str(plugins_toml))
    return plugins_toml
...
def test_shared_metrics_and_rich_plugin_share_one_core_session(
    ...
    monkeypatch.setenv("AGENTX_NEMO_RELAY_ATIF_ENABLED", "1")
    monkeypatch.setenv(
        "AGENTX_NEMO_RELAY_ATIF_OUTPUT_DIRECTORY", str(tmp_path / "atif")
    )
    monkeypatch.setattr(
    ...
    register_metrics = next(
        ...
        and item[1].startswith("agentx.nemo_relay.shared_metrics.")
    )
    register_atif = next(
    ...
    assert package["metrics"][0]["name"] == "agentx.model_route.count"
    assert package["metrics"][0]["value"] == 1
    assert (tmp_path / "atif" / "agentx-atif-s1.json").exists()
