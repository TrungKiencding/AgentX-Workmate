"""Tests for the `agentx memory reset` CLI command.
...
- Profile-scoped reset (uses AGENTX_HOME)
"""
...
def memory_env(tmp_path, monkeypatch):
    """Set up a fake AGENTX_HOME with memory files."""
    ...
    (memories / "MEMORY.md").write_text(
        "§\nHermes repo is at ~/.agentx/hermes-agent\n§\nUser prefers dark themes",
        encoding="utf-8",
    )
...
def _run_memory_reset(target="all", yes=False, monkeypatch=None, confirm_input="no"):
    ...
    Simulates what happens when `agentx memory reset` is run.
    """
...
class TestMemoryReset:
    """Tests for `agentx memory reset` subcommand."""
