...
silently ignores administrator-pinned values while `agentx config`/`doctor`
...
def _load_cli_config(home):
    ...
    singleton), so monkeypatching AGENTX_HOME after import doesn't move it.
    ...
    this only matters for tests that swap AGENTX_HOME.)
    """
    import cli
