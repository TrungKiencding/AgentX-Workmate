...
This saves 500-650ms on ``agentx --help``, ``agentx version``,
``agentx logs``, etc., by not importing ``google.cloud.pubsub_v1``,
...
def _live_subcommand_names() -> set[str]:
    """Run ``agentx --help`` in-process and parse the subcommand block.
...
def test_deferred_platform_cli_resolution_targets_matching_platform():
    ...
    Photon registers ``agentx photon`` only when its adapter module is
...
def test_deferred_platform_loader_registers_cli_command_before_parser_table():
    ...
    def _fake_loader():
        ...
        # top-level agentx <name> CLI command via PluginContext.
