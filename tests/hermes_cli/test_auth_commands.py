...
def test_auth_add_api_key_persists_manual_entry(tmp_path, monkeypatch):
    ...
    payload = json.loads((tmp_path / "agentx" / "auth.json").read_text())
...
def test_auth_add_nous_oauth_persists_pool_entry(tmp_path, monkeypatch):
    ...
    # `agentx auth add nous` must also populate providers.nous so the
    ...
    # raises "AgentX is not logged into Nous Portal" and the agent dies.
...
def test_auth_add_nous_oauth_honors_custom_label(tmp_path, monkeypatch):
    """`agentx auth add nous --type oauth --label <name>` must preserve the
...
def test_auth_add_codex_oauth_keeps_distinct_pool_accounts(tmp_path, monkeypatch):
    """Two ``agentx auth add openai-codex`` runs for different ChatGPT
    ...
    second independent one. ``agentx auth list`` showed two labels sharing
...
def test_auth_add_xai_oauth_sets_active_provider(tmp_path, monkeypatch):
    """agentx auth add xai-oauth must set active_provider and write a pool entry.
...
def test_auth_add_xai_oauth_keeps_distinct_pool_accounts(tmp_path, monkeypatch):
    """Two ``agentx auth add xai-oauth`` runs must produce independent pool entries.
...
def test_logout_resets_codex_config_when_auth_state_already_cleared(tmp_path, monkeypatch, capsys):
    """`agentx logout --provider openai-codex` must still clear model.provider.
...
# =============================================================================
# Unified credential-source stickiness — every source AgentX reads from has a
...
def test_seed_from_singletons_respects_hermes_pkce_suppression(tmp_path, monkeypatch):
    """anthropic hermes_pkce must not re-seed from ~/.agentx/.anthropic_oauth.json when suppressed."""
    hermes_home = tmp_path / "agentx"
    hermes_home.mkdir(parents=True, exist_ok=True)
    monkeypatch.setenv("AGENTX_HOME", str(hermes_home))
