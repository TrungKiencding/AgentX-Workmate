"""Tests for the multi-board kanban layer (``agentx kanban boards …``).
...
* Current-board persistence via ``<root>/kanban/current`` and
  ``AGENTX_KANBAN_BOARD`` env var.
* ``connect(board=)`` isolation — writes on one board don't leak.
* ``create_board`` / ``list_boards`` / ``remove_board`` round trip.
* CLI surface: ``agentx kanban boards list/create/switch/rm``.
* ``_default_spawn`` injects ``AGENTX_KANBAN_BOARD`` into worker env.
"""
...
def fresh_home(tmp_path, monkeypatch):
    """Isolated AGENTX_HOME with no prior kanban state.

    The autouse hermetic conftest already nukes credentials + TZ; this
    fixture layers a per-test AGENTX_HOME plus a path-init cache reset
...
class TestSlugValidation:
    @pytest.mark.parametrize("good", [
        "default", "atm10-server", "agentx-agent", "proj_1", "a",
...
class TestPathResolution:
    ...
    def test_env_var_db_override_still_wins(self, fresh_home, tmp_path, monkeypatch):
        """``AGENTX_KANBAN_DB`` pins the file regardless of board= arg."""
        forced = tmp_path / "custom.db"
        monkeypatch.setenv("AGENTX_KANBAN_DB", str(forced))
...
class TestConnectionIsolation:
    ...
    def test_connect_env_var_overrides_current(self, fresh_home, monkeypatch):
        ...
        monkeypatch.setenv("AGENTX_KANBAN_BOARD", "envwin")
        with kb.connect() as conn:
...
class TestWorkerSpawnEnv:
    """Ensure the dispatcher pins ``AGENTX_KANBAN_BOARD`` / DB / workspaces on spawn.
    ...
    def test_default_spawn_sets_env_vars(self, fresh_home, monkeypatch):
        ...
        env = captured["env"]
        assert env["AGENTX_KANBAN_BOARD"] == "spawntest"
        assert env["AGENTX_KANBAN_TASK"] == "t_abc"
        # DB path should match the per-board DB, not the legacy default.
        expected_db = fresh_home / "kanban" / "boards" / "spawntest" / "kanban.db"
        assert env["AGENTX_KANBAN_DB"] == str(expected_db)
        expected_ws = fresh_home / "kanban" / "boards" / "spawntest" / "workspaces"
        assert env["AGENTX_KANBAN_WORKSPACES_ROOT"] == str(expected_ws)
...
def _cli(args: list[str], env_extra: dict | None = None) -> subprocess.CompletedProcess:
    """Run ``agentx kanban …`` with PYTHONPATH pinned to the worktree."""
    env = dict(os.environ)
