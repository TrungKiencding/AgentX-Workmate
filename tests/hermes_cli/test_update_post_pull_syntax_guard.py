"""Tests for the post-pull syntax guard in ``agentx update``.
...
becomes unbootable — every ``agentx`` invocation imports those files at
...
markers in ``hermes_cli/config.py``; users who ran ``agentx update`` in
the 7-minute window before #28458 landed could not run any ``agentx``
command afterward.
"""
...
# release; if a future ``agentx update`` would brick users, this test fails
# in CI first.
# ---------------------------------------------------------------------------
