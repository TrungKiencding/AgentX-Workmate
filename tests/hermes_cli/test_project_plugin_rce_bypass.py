...
Execution via the ``AGENTX_ENABLE_PROJECT_PLUGINS`` bypass in the web
...
1. ``hermes_cli.web_server._discover_dashboard_plugins`` opted into
   the untrusted ``./.agentx/plugins/`` source via
   ``os.environ.get("AGENTX_ENABLE_PROJECT_PLUGINS")`` — truthy for
...
# ---------------------------------------------------------------------------
# Layer 1 — AGENTX_ENABLE_PROJECT_PLUGINS env gate uses truthy semantics.
# ---------------------------------------------------------------------------
...
class TestProjectPluginsEnvGate:
    ...
    def project_plugin(self, tmp_path, monkeypatch):
        """Plant a project-source plugin under CWD's ``.agentx/plugins``
    ...
    def test_falsy_values_keep_project_plugins_disabled(
        ...
        if value == "":
            monkeypatch.delenv("AGENTX_ENABLE_PROJECT_PLUGINS", raising=False)
        else:
            monkeypatch.setenv("AGENTX_ENABLE_PROJECT_PLUGINS", value)

        plugins = web_server._get_dashboard_plugins(force_rescan=True)
        names = {p["name"] for p in plugins}
        assert "evil" not in names, (
            f"AGENTX_ENABLE_PROJECT_PLUGINS={value!r} must NOT enable the "
...
class TestEndToEndPocBlocked:
    ...
    ``AGENTX_ENABLE_PROJECT_PLUGINS=0`` (so the operator believed the
    ...
    def test_full_chain_blocked(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setenv("AGENTX_ENABLE_PROJECT_PLUGINS", "0")
