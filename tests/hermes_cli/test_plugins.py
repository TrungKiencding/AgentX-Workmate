"""Tests for the AgentX plugin system (hermes_cli.plugins)."""
...
def _make_plugin_dir(base: Path, name: str, *, register_body: str = "pass",
    ...
    *base* is expected to be ``<hermes_home>/plugins/``; we derive
    ...
    if auto_enable:
        # Write/merge plugins.enabled in <AGENTX_HOME>/config.yaml.
        # Config is always read from AGENTX_HOME (not from the project
        # dir for project plugins), so that's where we opt in.
        import os
        hermes_home_str = os.environ.get("AGENTX_HOME")
        if hermes_home_str:
...
class TestPluginDiscovery:
    ...
    def test_plugin_can_register_and_invoke_middleware(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setenv("AGENTX_HOME", str(tmp_path / "hermes_test"))
...
class TestPluginManagerList:
    ...
    def test_shared_hook_name_credited_to_every_plugin(self, tmp_path, monkeypatch):
        ...
        `agentx plugins list`. Attribution now counts what each plugin's own
...
class TestPluginDebugLogging:
    """AGENTX_PLUGINS_DEBUG opt-in stderr handler for plugin developers."""
    ...
    def test_debug_handler_not_installed_when_env_var_absent(self, monkeypatch):
        ...
        monkeypatch.delenv("AGENTX_PLUGINS_DEBUG", raising=False)
...
class TestPluginContextProfileName:
    """ctx.profile_name resolves from AGENTX_HOME in every context."""
    ...
    def test_default_profile(self, tmp_path, monkeypatch):
        """AGENTX_HOME at the root resolves to 'default'."""
    ...
    def test_named_profile(self, tmp_path, monkeypatch):
        """AGENTX_HOME under profiles/<name> resolves to that name."""
        prof = tmp_path / ".agentx" / "profiles" / "coder"
        prof.mkdir(parents=True)
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
        monkeypatch.setenv("AGENTX_HOME", str(prof))
