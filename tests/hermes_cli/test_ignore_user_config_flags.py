"""Tests for --ignore-user-config and --ignore-rules flags on `agentx chat`.
...
files. In AgentX the equivalent isolation is:

* ``--ignore-user-config`` → skip ``~/.agentx/config.yaml`` in ``load_cli_config()``
...
def _clean_env(monkeypatch):
    ...
    """
    for var in ("AGENTX_IGNORE_USER_CONFIG", "AGENTX_IGNORE_RULES"):
...
class TestIgnoreUserConfigEnvGate:
    """``load_cli_config()`` must honour ``AGENTX_IGNORE_USER_CONFIG=1``.
    ...
    def _write_user_config(self, tmp_path, model_default):
        ...
        # config. With AGENTX_IGNORE_USER_CONFIG=1, load_cli_config() falls
    ...
    def test_flag_ignored_when_set_to_other_value(self, tmp_path, monkeypatch):
        ...
        self._write_user_config(tmp_path, "test-vendor/ignore-user-config-sentinel")
        monkeypatch.setenv("AGENTX_IGNORE_USER_CONFIG", "true")  # not "1"
...
class TestIgnoreRulesEnvGate:
    ...
    def test_env_var_enables_ignore_rules(self, monkeypatch):
        """Setting AGENTX_IGNORE_RULES=1 flips HermesCLI.ignore_rules True."""
        monkeypatch.setenv("AGENTX_IGNORE_RULES", "1")
        ...
        obj.ignore_rules = ignore_rules or os.environ.get("AGENTX_IGNORE_RULES") == "1"
...
class TestCmdChatWiring:
    ...
    def test_both_flags_set_both_env_vars(self, monkeypatch):
        monkeypatch.delenv("AGENTX_IGNORE_USER_CONFIG", raising=False)
        monkeypatch.delenv("AGENTX_IGNORE_RULES", raising=False)
        ...
        assert os.environ.get("AGENTX_IGNORE_USER_CONFIG") == "1"
        assert os.environ.get("AGENTX_IGNORE_RULES") == "1"
    ...
    def test_flags_absent_sets_nothing(self, monkeypatch):
        ...
        assert "AGENTX_IGNORE_USER_CONFIG" not in os.environ
        assert "AGENTX_IGNORE_RULES" not in os.environ
