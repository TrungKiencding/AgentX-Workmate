...
def _client():
    ...
    # Keep the state DB under the isolated AGENTX_HOME for any handler that
    # touches it.
...
class TestCredentialPoolEndpoints:
    ...
    def test_env_seeded_delete_stays_deleted(self):
        ...
        load_pool() re-seeds from ~/.agentx/.env on every call, so removing
        just the pool row silently reverts on the next dashboard refresh.
        The endpoint must mirror `agentx auth remove`: clean up the backing
    ...
    def test_post_readd_lifts_suppression(self):
        ...
        Mirrors `agentx auth add`, which clears every suppression for the
    ...
    def test_sources_lists_configured_hubs(self, monkeypatch):
        ...
        class _Src:
            ...
            def search(self, q, limit=10):
                return [_FakeMeta("agentx-index/featured-skill", "trusted")]

        def _fake_router():
            srcs = [_Src("official"), _Src("github")]
            # agentx-index source advertises availability + featured search.
            idx = _Src("agentx-index")
        ...
        assert {"official", "github", "agentx-index"} <= ids
...
class TestSkillsHubPreviewEndpoint:
    ...
    def test_preview_returns_skill_md_text(self, monkeypatch):
        monkeypatch.setattr(
            "tools.skills_hub.create_source_router", lambda **kwargs: []
        )
...
class TestUpdateCheckEndpoint:
    """``GET /api/agentx/update/check`` reports availability without applying.

    Powers the dashboard's check-before-you-update flow: the System page
    shows the commit-behind count and asks the user to confirm before
    ``POST /api/agentx/update`` runs ``agentx update``.
    """
    ...
    def test_git_install_reports_behind_count(self, monkeypatch):
        ...
        r = self.client.get("/api/agentx/update/check")
        assert r.status_code == 200
        body = r.json()
        assert {
            "install_method",
    ...
    def test_managed_runtime_dashboard_is_not_applyable(self, monkeypatch):
        ...
        body = self.client.get("/api/agentx/update/check").json()
...
class TestToolsConfigEndpoints:
    ...
    the dashboard surface that replicates the `agentx tools` configurator."""
...
def test_spawn_hermes_action_scrubs_gateway_loop_guard_env(monkeypatch, tmp_path):
    ...
    _AGENTX_GATEWAY=1. Spawned actions (e.g. `gateway restart`) must NOT inherit
    ...
    assert "_AGENTX_GATEWAY" not in captured["env"]
    assert captured["env"]["AGENTX_NONINTERACTIVE"] == "1"
