...
def _install_fake_gateway_run(monkeypatch, start_gateway):
    ...
    # /agentx-gateway.service"`` and runs ``systemctl --user daemon-reload``
    # — both target the *real* user environment because the conftest only
    # sandboxes ``AGENTX_HOME``, not ``HOME``. Tests that drive
    ...
    # unit (baking in the test's pytest-tmp ``AGENTX_HOME`` value, which
...
def test_gateway_run_subprocess_preserves_daemon_exit_codes(
    ...
    script = textwrap.dedent(
        ...
        outcome = os.environ["AGENTX_TEST_GATEWAY_OUTCOME"]
    ...
    env = {
        **os.environ,
        "AGENTX_HOME": str(tmp_path),
        "AGENTX_GATEWAY_EXIT_DIAG": "0",
        "AGENTX_TEST_GATEWAY_OUTCOME": outcome,
...
def _clear_supervisor_markers(monkeypatch):
    ...
    monkeypatch.delenv("AGENTX_S6_SUPERVISED_CHILD", raising=False)
...
def test_systemd_install_checks_linger_status(monkeypatch, tmp_path, capsys):
    unit_path = tmp_path / "systemd" / "user" / "agentx-gateway.service"

    monkeypatch.setattr(gateway, "get_systemd_unit_path", lambda system=False: unit_path)
    # Synthetic unit with a non-temp home: the real generator bakes the
    # hermetic test AGENTX_HOME (a tmp dir), which the temp-home write
    # guard correctly refuses.
    monkeypatch.setattr(
        gateway,
        "generate_systemd_unit",
        lambda system=False, run_as_user=None: (
            '[Service]\nEnvironment="AGENTX_HOME=/home/alice/.agentx"\n'
        ),
    )
...
def test_gateway_install_noninteractive_skips_legacy_unit_prompt(monkeypatch, tmp_path):
    ...
    fake_path = tmp_path / "agentx-gateway.service"
