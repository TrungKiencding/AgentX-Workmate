...
Runs a real subprocess with a temp AGENTX_HOME whose config.yaml contains a
``${ENV_VAR}`` reference, plus a managed-scope overlay dir (AGENTX_MANAGED_DIR).
...
production: ``AGENTX_MANAGED_DIR`` is set explicitly, which bypasses the
...
def test_behavioral_read_gets_expansion_and_overlay_while_writeback_stays_raw(
    ...
    out = _run_py(
        code,
        {
            ...
            "AGENTX_MANAGED_DIR": str(managed_dir),
...
def test_writeback_roundtrip_byte_identical_when_unchanged(tmp_path):
    ...
    code = textwrap.dedent(
        ...
        p = Path(__import__('os').environ['AGENTX_HOME']) / 'config.yaml'
    ...
    out = _run_py(code, {"AGENTX_HOME": str(home)}, tmp_path)
