...
class TestCmdUpdateNpmLockfileCache:
    ...
    def test_package_json_only_edit_defeats_skip(self, tmp_path, monkeypatch):
        ...
        npm — lockfile unchanged. `agentx update` must still install (the
    ...
    def test_update_uses_one_shared_npm_cache_across_profiles(
        ...
        with patch.object(
            ...
            monkeypatch.setenv("AGENTX_HOME", str(shared_root))
            hm._update_node_dependencies()

            monkeypatch.setenv("AGENTX_HOME", str(named_profile))
            hm._update_node_dependencies()
...
class TestCmdUpdateBranchFallback:
    ...
    def test_update_on_fork_checks_upstream_when_origin_up_to_date(
        ...
        origin but behind TrungKiencding/AgentX-Workmate silently misses updates.
        """
        from hermes_cli import main as hm
        ...
        with patch.object(
            hm,
            "_get_origin_url",
            return_value="https://github.com/example/agentx-agent.git",
...
class TestCmdUpdateProfileSkillSync:
    ...
    def test_active_profile_included_in_skill_sync(
        ...
        default_p = SimpleNamespace(name="default", path=Path("/fake/.agentx"))
        active_p = SimpleNamespace(name="bit", path=Path("/fake/.agentx/profiles/bit"))
        other_p = SimpleNamespace(name="work", path=Path("/fake/.agentx/profiles/work"))
...
class TestCmdUpdateBranchFlag:
    """``agentx update --branch <name>`` targets the requested branch.
...
class TestCmdUpdateCheckBranchFlag:
    """``agentx update --check --branch <name>`` honors the branch override.
...
class TestCmdUpdateZipBranchRefusal:
    """``agentx update --branch=<non-main>`` must refuse on the ZIP fallback path.
