"""Tests for IRC gateway configuration via `agentx setup gateway` UI.

Covers the full plugin-platform discovery → status → configure flow so that
a fresh AgentX install (no state, no env vars) can set up IRC through the
...
class TestIRCFreshInstallDiscovery:
    """IRC appears in the setup menu on a brand-new AgentX install."""
    ...
    def test_irc_status_configured_when_env_set(self, monkeypatch):
        ...
        try:
            ...
            monkeypatch.setenv("IRC_CHANNEL", "#agentx")
            monkeypatch.setenv("IRC_NICKNAME", "agentx-bot")
...
class TestIRCGatewaySetupFreshInstall:
    """Simulate the full `agentx setup gateway` experience with IRC present."""
