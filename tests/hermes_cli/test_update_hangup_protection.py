"""Tests for SIGHUP protection and stdout mirroring in ``agentx update``.

Covers ``_UpdateOutputStream``, ``_install_hangup_protection``, and
``_finalize_update_output`` in ``hermes_cli/main.py``.  These exist so
that ``agentx update`` survives a terminal disconnect mid-install
...
class TestInstallHangupProtection:
    ...
    def test_wraps_stdout_and_stderr_with_mirror(self, tmp_path, monkeypatch):
        ...
        if hasattr(_cfg, "_AGENTX_HOME_CACHE"):
            _cfg._AGENTX_HOME_CACHE = None  # type: ignore[attr-defined]
        ...
        try:
            ...
            assert "agentx update started" in contents
        finally:
