"""Tests for Bug #12905 fix — stale OAuth token detection in agentx model flow.

Bug 3: `agentx model` with `provider=anthropic` skips OAuth re-authentication
when a stale ANTHROPIC_TOKEN exists in ~/.agentx/.env but no valid
