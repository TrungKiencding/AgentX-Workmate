...
def test_all_invalid_platform_toolsets_logs_runtime_warning(caplog):
    ...
    (e.g. 'agentx' instead of 'agentx-cli') must warn at resolve time so an
...
def test_valid_platform_toolsets_no_runtime_warning(caplog):
    ...
    config = {"platform_toolsets": {"cli": ["agentx-cli"]}}
...
def test_partially_valid_platform_toolsets_no_runtime_warning(caplog):
    ...
    config = {"platform_toolsets": {"cli": ["agentx-cli", "bogus"]}}
...
# via the platform's native composite. The composite ``agentx-discord``
...
def test_discord_toolsets_do_not_leak_to_other_platforms():
    ...
    config = {"platform_toolsets": {"telegram": ["agentx-telegram", "discord"]}}
...
class TestPlatformToolsetConsistency:
    ...
    def test_gateway_toolset_includes_all_messaging_platforms(self):
        """agentx-gateway includes list should cover all messaging platforms."""
        from hermes_cli.tools_config import PLATFORMS
        from toolsets import TOOLSETS

        gateway_includes = set(TOOLSETS["agentx-gateway"]["includes"])
...
def test_get_effective_configurable_toolsets_dedupes_bundled_plugins():
    ...
    them twice — otherwise `agentx tools` → "reconfigure existing" shows
...
#    reported as added/removed by `agentx tools` ──────────────────────────




def test_kanban_not_reported_as_removed_in_diff():
    """Reproduces the false-signal bug: `agentx tools` printed ``- kanban``
...
# `agentx tools post-setup <key>` child. On Windows each console child (npm,
...
# ── Toolsets that shipped after a platform's last `agentx tools` save ────────
#
# Saving the picker (or one toggle in the desktop Toolsets UI) replaces a
# platform's composite (``[agentx-cli]``) with a frozen explicit list, and
...
def test_saved_list_gains_toolsets_that_shipped_after_it_was_written():
    ...
    on_composite = _get_platform_tools(
        {"platform_toolsets": {"cli": ["agentx-cli"]}},
        "cli",
...
def test_platforms_whose_composite_excludes_it_are_left_narrow():
    ...
    composite (agentx-acp, agentx-webhook) that never carried the toolset."""
    from toolsets import TOOLSETS, resolve_toolset

    narrow = [
        platform
        for platform in ("acp", "webhook")
        if f"agentx-{platform}" in TOOLSETS
        and not any(
            set(resolve_toolset(ts, include_registry=False))
            <= set(resolve_toolset(f"agentx-{platform}"))
