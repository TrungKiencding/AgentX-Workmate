...
On hosts with many profile-backed ``agentx-gateway*.service`` units,
``agentx update`` used to wrap the entire per-scope unit loop in a single
...
class TestFleetRestartTimeoutIsolation:
    def test_timeout_on_middle_unit_continues_remaining_units(self):
        units = [
            "agentx-gateway-xiaomo1",
            "agentx-gateway-xiaomo2",
            "agentx-gateway-xiaomo3",
            "agentx-gateway-xiaomo4",
            "agentx-gateway-xiaomo5",
            "agentx-gateway-xiaomo6",
            "agentx-gateway-xiaomo7",
            "agentx-gateway",
        ]
        restarted: list[str] = []
        failed: list[str] = []
        ...
        def process_unit(svc_name: str) -> None:
            if svc_name == "agentx-gateway-xiaomo5":
        ...
        assert failed == ["agentx-gateway-xiaomo5"]
        assert restarted == [
            "agentx-gateway-xiaomo1",
            "agentx-gateway-xiaomo2",
            "agentx-gateway-xiaomo3",
        ...
        assert timeout_cmds == [
            ["systemctl", "--user", "--no-ask-password", "restart", "agentx-gateway-xiaomo5"]
        ]
    ...
    def test_non_gateway_units_in_list_output_are_ignored(self):
        ...
        _for_each_systemd_gateway_unit(
            "\n".join(
                [
                    ...
                    "agentx-gateway-coder.service loaded active running",
        ...
        assert seen == ["agentx-gateway-coder"]
    ...
    def test_process_errors_other_than_timeout_still_propagate(self):
        ...
        with pytest.raises(RuntimeError, match="not a timeout"):
            _for_each_systemd_gateway_unit(
                _list_units_stdout(["agentx-gateway"]),
                process_unit=process_unit,
...
class TestIncompleteFleetRestartWarning:
    def test_warns_with_exact_unrestarted_units(self, capsys):
        _warn_incomplete_gateway_fleet_restart(
            ["agentx-gateway-xiaomo5", "agentx-gateway-xiaomo6", "agentx-gateway-xiaomo5"]
        )
        out = capsys.readouterr().out
        assert "Update incomplete" in out
        assert out.count("agentx-gateway-xiaomo5") == 1
        assert "agentx-gateway-xiaomo6" in out
