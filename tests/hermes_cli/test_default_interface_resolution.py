...
``agentx … chat -q`` on a pipe, and the TUI's no-TTY bail-out exits 0
...
def _reset_early_cache(monkeypatch):
    ...
    # a fresh value, and make sure no stray AGENTX_TUI leaks in.
    monkeypatch.setattr(m, "_EARLY_INTERFACE_CACHE", None)
    monkeypatch.delenv("AGENTX_TUI", raising=False)
    yield
    monkeypatch.setattr(m, "_EARLY_INTERFACE_CACHE", None)
...
class TestWantsTuiEarly:
    ...
    def test_missing_config_defaults_to_cli(self, tmp_path, monkeypatch):
        # AGENTX_HOME points at an empty dir — no config.yaml.
        monkeypatch.setenv("AGENTX_HOME", str(tmp_path))
        monkeypatch.setattr(m, "_EARLY_INTERFACE_CACHE", None)
        assert m._wants_tui_early([]) is False
