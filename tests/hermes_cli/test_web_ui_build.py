...
(mirroring the desktop build), recorded in a stamp file under $AGENTX_HOME,
NOT mtime comparison — so ``git pull`` / ``agentx update`` that rewrite
...
def _isolated_hermes_home(tmp_path, monkeypatch):
    ...
    monkeypatch.setenv("AGENTX_HOME", str(tmp_path / "_hermes_home"))
...
class TestWebUIBuildNeeded:
    ...
    def test_mtime_only_change_is_not_stale(self, tmp_path):
        ...
        ``git pull`` / ``agentx update`` do) must NOT report stale."""
