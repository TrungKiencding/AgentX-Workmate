...
def test_bootstrap_marker_not_autostashed_by_update(tmp_path):
    ...
    ``agentx update``'s ``git stash push --include-untracked`` does not sweep it
    ...
    marker = tmp_path / ".agentx-bootstrap-complete"
    marker.write_text("")
    ...
    assert ".agentx-bootstrap-complete" not in status
...
def test_update_autostash_survives_undeletable_untracked_dir(tmp_path):
    ...
    (pkg / "agentx-agent.rb").write_text("formula\n")
    ...
    try:
        ...
        assert (pkg / "agentx-agent.rb").read_text() == "formula\n"
    finally:
        os.chmod(pkg, 0o755)
