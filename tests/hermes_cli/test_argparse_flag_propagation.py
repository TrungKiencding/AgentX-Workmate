...
subparser, placing the flag BEFORE the subcommand (e.g. 'agentx --yolo chat')
...
def _build_parser():
    """Build the agentx argument parser from the real code.
...
class TestYoloEnvVar:
    """Verify --yolo sets AGENTX_YOLO_MODE regardless of flag position.
...
class TestAcceptHooksOnAgentSubparsers:
    ...
    parser and `chat`, so `agentx gateway run --accept-hooks` failed
...
"""
    ...
    def test_accepted_at_every_position(self):
        """Every `agentx <argv>` must exit 0 (help) rather than failing
        with `unrecognized arguments`."""
        import json
        import subprocess
...
class TestChatSubparserInheritedValueFlags:
    ...
    Regression test for #28780: `agentx -t web chat` silently dropped the
    ...
    def test_chat_subparser_inherited_value_flags_use_suppress(self):
        ...
        assert not offenders, (
            ...
            "top-level value when used as `agentx <flag> <value> chat`:\n  "
