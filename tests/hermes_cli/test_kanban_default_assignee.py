...
def isolated_kanban_home(monkeypatch):
    """Spin up a fresh AGENTX_HOME with a clean kanban DB."""
    test_home = tempfile.mkdtemp(prefix="kanban_default_assignee_test_")
    monkeypatch.setenv("AGENTX_HOME", test_home)
    # Force-reimport so the fresh AGENTX_HOME is picked up.
    ...
    # gives each test its own monkeypatched AGENTX_HOME so no cross-test
    # contamination.
