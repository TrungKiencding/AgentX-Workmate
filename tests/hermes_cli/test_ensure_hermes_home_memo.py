...
def test_distinct_home_paths_each_get_the_skeleton(tmp_path, monkeypatch):
    first = tmp_path / "a" / ".agentx"
    second = tmp_path / "b" / ".agentx"

    monkeypatch.setenv("AGENTX_HOME", str(first))
    cfg.ensure_hermes_home()

    # Profile switch: AGENTX_HOME moves → the new path is ensured too.
    monkeypatch.setenv("AGENTX_HOME", str(second))
    cfg.ensure_hermes_home()
