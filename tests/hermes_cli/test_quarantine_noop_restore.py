...
``agentx*.exe`` shims aside (``agentx.exe.old.<ms>``) before invoking the
...
quarantined shims stayed renamed aside and ``agentx`` vanished from PATH
...
def _make_scripts_dir(tmp_path: Path) -> Path:
    ...
    for name in ("agentx", "agentx-agent", "agentx-acp", "agentx-gateway"):
...
def test_main_noop_success_restores_shims(tmp_path):
    ...
    assert "agentx.exe" in names, "agentx.exe must be restored after a no-op install"
    assert "agentx-acp.exe" in names
    assert "agentx-gateway.exe" in names
    assert (scripts / "agentx.exe").read_bytes() == b"MZ-old-agentx"
...
def test_main_rewriting_success_keeps_fresh_shims(tmp_path):
    ...
    assert (scripts / "agentx.exe").read_bytes() == b"MZ-new-agentx"
...
def test_repair_noop_success_restores_shims(tmp_path):
    ...
    names = _shim_names(scripts)
    assert "agentx.exe" in names, "agentx.exe must be restored after a no-op recovery install"
    assert (scripts / "agentx.exe").read_bytes() == b"MZ-old-agentx"
...
def test_repair_rewriting_success_keeps_fresh_shims(tmp_path):
    ...
    def fake_run(cmd, cwd=None, check=None, env=None):
        (scripts / "agentx.exe").write_bytes(b"MZ-new-agentx")
