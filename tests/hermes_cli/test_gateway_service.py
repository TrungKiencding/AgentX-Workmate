...
class TestSystemdServiceRefresh:
    ...
    def test_systemd_restart_timeout_prints_status_guidance(self, monkeypatch, capsys):
        """`agentx gateway restart` must not surface a raw TimeoutExpired traceback.

        The dashboard spawns `agentx gateway restart` in the background; when a
        ...
        assert "agentx gateway status" in output
    ...
    def test_refresh_refuses_to_bake_pytest_tmpdir_into_real_user_unit(
        ...
        ``generate_systemd_unit()`` bakes ``AGENTX_HOME`` into the unit's
        ...
        ``/tmp/pytest-of-.../hermes_test`` AGENTX_HOME — silently breaking
        ...
        carry those markers.
        """
        unit_path = tmp_path / "agentx-gateway.service"
        unit_path.write_text("old unit\n", encoding="utf-8")

        monkeypatch.setattr(
            gateway_cli, "get_systemd_unit_path", lambda system=False: unit_path
        )
        # Realistic generated unit referencing a pytest tmpdir AGENTX_HOME
        polluted_unit = (
            "[Service]\n"
            'Environment="AGENTX_HOME=/tmp/pytest-of-alice/pytest-42/'
...
class TestRequireServiceInstalled:
    def test_exits_with_install_hint_when_unit_missing(self, tmp_path, monkeypatch, capsys):
        ...
        assert "agentx gateway install" in out
...
class TestGeneratedSystemdUnits:
    ...
    def test_user_unit_does_not_leak_profile_node_symlink_target(self, tmp_path, monkeypatch):
        ...
        profile_node_bin = tmp_path / ".agentx" / "profiles" / "jarvis" / "node" / "bin"
    ...
    def test_gateway_restart_does_not_fallback_to_foreground_when_launchd_restart_fails(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setattr(
            ...
            lambda: (_ for _ in ()).throw(
                gateway_cli.subprocess.CalledProcessError(5, ["launchctl", "kickstart", "-k", "gui/501/ai.agentx.gateway"])
            ),
        )
...
class TestSystemUnitHermesHome:
    """AGENTX_HOME in system units must reference the target user, not root."""
    ...
    def test_system_unit_uses_target_user_home_not_calling_user(self, monkeypatch):
        ...
        assert 'AGENTX_HOME=/home/alice/.agentx' in unit
        assert '/root/.agentx' not in unit


    def test_user_unit_unaffected_by_change(self):
        # User-scope units should still use the calling user's AGENTX_HOME
        unit = gateway_cli.generate_systemd_unit(system=False)

        hermes_home = str(gateway_cli.get_hermes_home().resolve())
        assert f'AGENTX_HOME={hermes_home}' in unit
...
class TestSystemUnitRefreshSyncsHermesHome:
    ...
    def test_refresh_adopts_unit_hermes_home_before_rewriting(self, tmp_path, monkeypatch):
        ...
        alice_hermes = alice_home / ".agentx"
        ...
        monkeypatch.delenv("AGENTX_RESTART_DRAIN_TIMEOUT", raising=False)

        # Correct installed unit (operator's AGENTX_HOME + drain timeout).
        monkeypatch.setenv("AGENTX_HOME", str(alice_hermes))
        ...
        # Simulate sudo without inherited AGENTX_HOME (falls back to root).
        ...
        assert os.environ["AGENTX_HOME"] == str(alice_hermes)
    ...
    def test_is_current_syncs_before_reading_unit(self, tmp_path, monkeypatch):
        ...
        unit's pinned AGENTX_HOME *before* it reads/compares the unit.
...
class TestHermesHomeForTargetUser:
    ...
    def test_remaps_default_home(self, monkeypatch):
        ...
        assert result == "/home/alice/.agentx"
...
class TestPreflightUserSystemd:
    ...
    def test_raises_when_linger_disabled_and_loginctl_denied(self, monkeypatch):
        ...
        assert "agentx gateway run" in msg  # foreground fallback mentioned
...
class TestProfileArg:
    ...
    def test_systemd_unit_for_target_user_includes_named_profile(self, tmp_path, monkeypatch):
        ...
        root_profile = root_home / ".agentx" / "profiles" / "mybot"
        root_profile.mkdir(parents=True)

        monkeypatch.setattr(Path, "home", lambda: root_home)
        monkeypatch.setenv("AGENTX_HOME", str(root_profile))
        ...
        assert f'AGENTX_HOME={target_home / ".agentx" / "profiles" / "mybot"}' in unit



    def test_launchd_plist_path_uses_real_user_home_not_profile_home(self, tmp_path, monkeypatch):
        profile_dir = tmp_path / ".agentx" / "profiles" / "orcha"
        ...
        assert plist_path == machine_home / "Library" / "LaunchAgents" / "ai.agentx.gateway-orcha.plist"
...
class TestSystemUnitPathRemapping:
    ...
    def test_system_unit_has_no_root_paths(self, monkeypatch, tmp_path):
        ...
        monkeypatch.setattr(Path, "home", lambda: root_home)
        monkeypatch.setenv("AGENTX_HOME", str(root_home / ".agentx"))
        monkeypatch.setattr(gateway_cli, "get_hermes_home", lambda: root_home / ".agentx")
        ...
        # WorkingDirectory is anchored at the target user's AGENTX_HOME (stable,
...
class TestDockerAwareGateway:
    ...
    def test_run_systemctl_raises_runtimeerror_when_missing(self, monkeypatch):
        ...
        with pytest.raises(RuntimeError, match="systemctl is not available"):
            gateway_cli._run_systemctl(["start", "agentx-gateway"])
    ...
    def test_run_systemctl_passes_through_on_success(self, monkeypatch):
        ...
        result = gateway_cli._run_systemctl(["status", "agentx-gateway"])
        assert result.returncode == 0
        assert len(calls) == 1
        assert "status" in calls[0]

    def test_install_in_container_prints_docker_guidance(self, monkeypatch, capsys):
        """'agentx gateway install' inside Docker exits 0 with container guidance."""
        import pytest
...
class TestLegacyHermesUnitDetection:
    ...
    renamed to ``agentx-gateway.service``. After PR #5646 (signal recovery
    ...
    (``agentx-gateway-<profile>.service``) or unrelated third-party services.
    """
    ...
    def test_accepts_alternate_execstart_formats(self, tmp_path, monkeypatch):
        ...
        ExecStart variants we've seen in the wild:
          ...
          - agentx gateway run   (direct binary)
    ...
    def test_print_legacy_unit_warning_shows_migration_hint(self, tmp_path, monkeypatch, capsys):
        ...
        assert "agentx gateway migrate-legacy" in out
...
class TestRemoveLegacyHermesUnits:
    ...
    def test_does_not_touch_profile_units_during_migration(
        ...
        """Teknium's constraint: profile units (agentx-gateway-coder.service)
        ...
        profile_unit = user_dir / "agentx-gateway-coder.service"
        profile_unit.write_text(self._OUR_UNIT_TEXT, encoding="utf-8")
        default_unit = user_dir / "agentx-gateway.service"
...
class TestMigrateLegacyCommand:
    """Tests for the `agentx gateway migrate-legacy` subcommand dispatch."""
    ...
    def test_migrate_legacy_subparser_accepts_dry_run_and_yes(self):
        ...
        # public, just confirm that `agentx gateway --help` shows it.
        import subprocess
        import sys
...
class TestSystemScopeWizardPreCheck:
    ...
    def _setup_units(tmp_path, monkeypatch, system_present: bool, user_present: bool):
        ...
        usr_dir.mkdir()
        if system_present:
            (sys_dir / "agentx-gateway.service").write_text("[Unit]\n")
        if user_present:
            (usr_dir / "agentx-gateway.service").write_text("[Unit]\n")
        monkeypatch.setattr(
            gateway_cli,
            "get_systemd_unit_path",
            lambda system=False: (sys_dir if system else usr_dir) / "agentx-gateway.service",
        )
    ...
    def test_non_root_with_explicit_system_arg_returns_true(self, tmp_path, monkeypatch):
        # Caller passed system=True explicitly (e.g. ``agentx gateway start --system``).
...
class TestSystemScopeRemediationOutput:
    ...
    def test_start_remediation_mentions_sudo_systemctl_and_uninstall(self, capsys, monkeypatch):
        monkeypatch.setattr(gateway_cli, "get_service_name", lambda: "agentx-gateway")
        ...
        assert "sudo systemctl start agentx-gateway" in out
        assert "sudo agentx gateway uninstall --system" in out
        assert "agentx gateway install" in out


class TestGatewayCommandCatchesSystemScopeError:
    """The direct CLI path (``agentx gateway start --system`` etc.) must
...
class TestServiceWorkingDirIsStable:
    ...
    (AGENTX_HOME), never the source checkout / worktree, so a relocated or
...
class TestLaunchctlBootstrapEioRetry:
    ...
    PLIST = "/tmp/ai.agentx.gateway.plist"
    DOMAIN = "gui/501"
    LABEL = "ai.agentx.gateway"
