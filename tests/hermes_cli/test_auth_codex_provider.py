"""Tests for Codex auth — tokens stored in AgentX auth store (~/.agentx/auth.json)."""
...
def _setup_hermes_auth(hermes_home: Path, *, access_token: str = "access", refresh_token: str = "refresh"):
    """Write Codex tokens into the AgentX auth store."""
...
def test_save_codex_tokens_syncs_manual_device_code_entries(tmp_path, monkeypatch):
    ...
    fix landed would have run ``agentx auth add openai-codex`` as a
    ...
    clobbered independent accounts added via ``agentx auth add openai-codex``.
    ...
    (hermes_home / "auth.json").write_text(json.dumps({
        ...
        "credential_pool": {
            "openai-codex": [
                ...
                # Independent account from `agentx auth add openai-codex` —
...
def test_save_codex_tokens_does_not_overwrite_independent_manual_entries(tmp_path, monkeypatch):
    ...
    Regression for #39236: ``agentx auth add openai-codex`` for accounts B and C
    ...
    (hermes_home / "auth.json").write_text(json.dumps({
        ...
        "credential_pool": {
            "openai-codex": [
                ...
                # ``agentx auth add openai-codex`` (account B and account C).
...
def test_codex_tokens_not_written_to_shared_file(tmp_path, monkeypatch):
    """Verify _save_codex_tokens writes only to AgentX auth store, not ~/.codex/."""
    hermes_home = tmp_path / "agentx"
...
def test_refresh_429_classified_as_quota_not_auth_failure(monkeypatch):
    ...
    than a misleading "run agentx auth".
    """
    from hermes_cli.auth import (
    ...
    assert "agentx auth" not in rendered
