...
def _install_example_plugin(_isolate_hermes_home):
    """Drop the example-dashboard fixture into the per-test AGENTX_HOME
    ...
    isolated ``AGENTX_HOME``.

    The user-plugin source is preferred over a transient
    ``AGENTX_BUNDLED_PLUGINS`` override because the bundled dir is
    ...
    # plugin in exactly as a real operator would with `agentx plugins
    # enable example`.
    ...
    #      and start serving requests against a torn-down AGENTX_HOME.
    app = web_server.app
...
class TestReloadEnv:
    ...
    def test_removes_deleted_known_vars(self, tmp_path):
        """reload_env() removes known AgentX vars not present in .env."""
        env_file = tmp_path / ".env"
...
class TestSessionTokenInjection:
    """The desktop shell mints AGENTX_DASHBOARD_SESSION_TOKEN and signs its
    ...
    def test_honors_injected_token(self, monkeypatch):
        ...
        monkeypatch.setenv("AGENTX_DASHBOARD_SESSION_TOKEN", "desktop-seeded-token")
    ...
    def test_falls_back_to_random_token(self, monkeypatch):
        ...
        monkeypatch.delenv("AGENTX_DASHBOARD_SESSION_TOKEN", raising=False)
        with patch.object(
...
class TestWebServerEndpoints:
    ...
    def _setup_test_client(self, monkeypatch, _isolate_hermes_home):
        """Create a TestClient and isolate the state DB under the test AGENTX_HOME."""
        try:
            from starlette.testclient import TestClient
        except ImportError:
    ...
    def test_messaging_platforms_profile_scopes_gateway_reads(self, monkeypatch):
        ...
        AGENTX_HOME contextvar override (#56986), so /api/messaging/platforms
    ...
    def _isolate_honcho_config(self):
        # Honcho tests write the suite-wide AGENTX_HOME honcho.json; snapshot and
    ...
    def test_put_honcho_writes_host_block_root_and_secret(self, monkeypatch, tmp_path):
        ...
        assert cfg["hosts"]["agentx"]["workspace"] == "myws"
        assert cfg["hosts"]["agentx"]["peerName"] == "eri"
        assert cfg["hosts"]["agentx"]["environment"] == "local"
        assert cfg["hosts"]["agentx"]["sessionStrategy"] == "per-repo"
        # The key lands where the client reads first; GET keeps it write-only.
        assert cfg["hosts"]["agentx"]["apiKey"] == "hch-test-key"
    ...
    def test_update_hermes_returns_docker_guidance_without_spawning(self, monkeypatch):
        ...
        def fail_spawn(*_args, **_kwargs):
            nonlocal spawned
            spawned = True
            raise AssertionError("docker update guard should not spawn agentx update")
        ...
        web_server._ACTION_PROCS.pop("agentx-update", None)
        web_server._ACTION_RESULTS.pop("agentx-update", None)
        ...
        assert data["name"] == "agentx-update"
        assert data["pid"] is None
        assert data["error"] == "docker_update_unsupported"
        assert "docker pull trungkiencding/agentx-workmate:latest" in data["message"]
        assert spawned is False

        status = self.client.get("/api/actions/agentx-update/status")
        assert status.status_code == 200
        status_data = status.json()
        assert status_data["running"] is False
        assert status_data["exit_code"] == 1
        assert status_data["pid"] is None
        assert any("docker pull trungkiencding/agentx-workmate:latest" in line for line in status_data["lines"])
    ...
    def test_set_model_main_custom_persists_api_key_and_registers_provider(self):
        ...
        ``agentx model`` custom flow. Regression for the desktop loop where a
...
class TestNewEndpoints:
    ...
    def test_terminal_ssh_probe_ready_when_configured(self, monkeypatch):
        ...
        config["terminal"]["ssh_user"] = "agentx"
        save_config(config)
        ...
        assert "agentx@devbox.example.com" in ssh["detail"]
...
class TestDiscoverUserThemes:
    """Tests for _discover_user_themes() — scans ~/.agentx/dashboard-themes/."""
...
class TestThemeBootstrapCSS:
    ...
    def test_user_theme_renders_bundle_vars(self, tmp_path, monkeypatch):
        ...
        assert css.startswith('<style id="agentx-theme-bootstrap">')
...
class TestDeleteSessionEndpoint:
    ...
    def test_delete_gateway_conversation_removes_compression_lineage(self):
        ...
        assert resp.status_code == 200
        assert resp.json()["deleted_ids"] == ["telegram_root", "telegram_tip"]
    ...
    def test_delete_gateway_tip_takes_stale_sibling_continuation(self):
        """A stale closed sibling off the root-to-tip path belongs to the same
        conversation. Leaving it behind resurfaced it as a ghost row."""
        from hermes_state import SessionDB

        db = SessionDB()
        try:
            db.create_session(session_id="stale_root", source="telegram")
            db.end_session("stale_root", "compression")
            db.create_session(
                session_id="stale_sibling",
                source="telegram",
                parent_session_id="stale_root",
            )
            db.end_session("stale_sibling", "ws_orphan_reap")
            db.create_session(
                session_id="stale_tip",
                source="telegram",
                parent_session_id="stale_root",
            )
            for offset, sid in enumerate(("stale_root", "stale_sibling", "stale_tip")):
                db._conn.execute(
                    "UPDATE sessions SET started_at=? WHERE id=?",
                    (1_000_000.0 + offset, sid),
                )
            db._conn.commit()
            assert db.get_compression_tip("stale_root") == "stale_tip"
        finally:
            db.close()

        resp = self.auth_client.delete("/api/sessions/stale_tip")
        assert resp.status_code == 200
        assert resp.json()["deleted_ids"] == ["stale_root", "stale_sibling", "stale_tip"]

        db = SessionDB()
        try:
            for sid in ("stale_root", "stale_sibling", "stale_tip"):
                assert db.get_session(sid) is None
        finally:
            db.close()

    def test_delete_echoes_cascaded_delegate_children(self):
        from hermes_state import SessionDB

        db = SessionDB()
        try:
            db.create_session(session_id="with_delegate", source="desktop")
            db.create_session(
                session_id="its_delegate",
                source="delegate",
                parent_session_id="with_delegate",
                model_config={"_delegate_from": "with_delegate"},
            )
        finally:
            db.close()

        resp = self.auth_client.delete("/api/sessions/with_delegate")
        assert resp.status_code == 200
        assert resp.json()["deleted_ids"] == ["with_delegate", "its_delegate"]
        assert not self._exists("its_delegate")
    ...
    def test_delete_local_compression_tip_keeps_its_ancestors(self):
        ...
        assert resp.status_code == 200
        assert resp.json() == {"ok": True, "deleted_ids": ["desktop_tip"]}
...
class TestSessionDetailLineage:
    """``GET /api/sessions/{id}`` names the compression chain a row belongs to.

    The messaging gateway can compress a platform chat again after the
    desktop opened it, so the desktop may hold a middle id that no list row
    names. The detail row carries the chain so it can still find that row.
    """
    ...
    def test_middle_segment_names_its_chain(self):
        from hermes_state import SessionDB

        db = SessionDB()
        try:
            db.create_session(
                session_id="root-R", source="telegram", session_key="telegram:dm:peer"
            )
        ...
        resp = self.auth_client.get("/api/sessions/mid-M")
        assert resp.status_code == 200
        body = resp.json()
        assert body["id"] == "mid-M"
        assert body["_lineage_root_id"] == "root-R"
        assert body["_lineage_ids"] == ["root-R", "mid-M", "tip-T"]
        assert body["profile"] == "default"

    def test_uncompressed_session_has_no_lineage_keys(self):
        from hermes_state import SessionDB

        db = SessionDB()
        try:
            db.create_session(session_id="plain", source="telegram")
        finally:
            db.close()

        resp = self.auth_client.get("/api/sessions/plain")
        assert resp.status_code == 200
        body = resp.json()
        assert body["id"] == "plain"
...
class TestBulkDeleteSessionsEndpoint:
    ...
    def test_bulk_delete_gateway_tip_counts_stale_sibling_continuation(self):
        from hermes_state import SessionDB
        ...
        resp = self.auth_client.post(
            "/api/sessions/bulk-delete", json={"ids": ["stale_tip"]}
        )
        assert resp.status_code == 200
        ...
        db = SessionDB()
        try:
            assert db.get_session("stale_sibling") is None
        finally:
            db.close()
...
class TestDeleteEmptySessionsEndpoint:
    ...
    def _setup_test_client(self, monkeypatch, _isolate_hermes_home):
        ...
        # Pin the SessionDB to the isolated AGENTX_HOME so each test
...
class TestPluginAPIAuth:
    ...
    def _setup_test_client(self, monkeypatch, _isolate_hermes_home, _install_example_plugin):
        ...
        installs it into the per-test ``AGENTX_HOME``.
        """
        try:
            from starlette.testclient import TestClient
        except ImportError:
    ...
    def test_plugin_route_allows_auth(self):
        ...
        test fixture (installed into AGENTX_HOME by the class-level
...
class TestDashboardPluginManifestExtensions:
    ...
    def test_user_plugins_ignore_profile_home_override(self, tmp_path, monkeypatch):
        ...
        AGENTX_HOME override scopes a request to another profile."""
        from hermes_constants import (
            reset_hermes_home_override,
            set_hermes_home_override,
        )
        ...
        monkeypatch.setenv("AGENTX_HOME", str(launch_home))
        from hermes_cli import web_server
        token = set_hermes_home_override(str(other))
        try:
...
# or ``sh -c 'printf …'``) instead of the real ``agentx --tui`` binary.  The
...
class TestPtyWebSocket:
    ...
    def test_tui_python_command_uses_child_path(self, tmp_path):
        ...
        command = f"agentx-review-python{Path(sys.executable).suffix}"
        ...
        env = {
            "AGENTX_CWD": str(tmp_path),
            "AGENTX_PYTHON": command,
            "PATH": str(bin_dir),
        }

        main_mod._apply_tui_python_env(env)

        assert env["AGENTX_PYTHON"] == command
...
def test_resolve_chat_argv_injects_gateway_ws_url(monkeypatch):
    ...
    gateway_url = env.get("AGENTX_TUI_GATEWAY_URL", "")
...
class TestDashboardPluginStaticAssetAllowlist:
    ...
    def _setup_test_client(self, monkeypatch, _isolate_hermes_home, _install_example_plugin):
        ...
        the per-test ``AGENTX_HOME`` user-plugins dir.
        """
        try:
            from starlette.testclient import TestClient
        except ImportError:
...
class TestDesktopCronTicker:
    ...
    def test_ticker_runs_when_desktop(self, monkeypatch, _isolate_hermes_home):
        ...
        with self._client():
            assert called.wait(3.0), "expected cron tick under AGENTX_DESKTOP=1"
...
class TestServeIndexMissingIndex:
    ...
    def _client_with_dist(tmp_path, monkeypatch, *, write_index: bool):
        ...
        monkeypatch.setattr(ws, "WEB_DIST", dist)
        monkeypatch.delenv("AGENTX_SERVE_HEADLESS", raising=False)
        spa_app = FastAPI()
        ws.mount_spa(spa_app)
