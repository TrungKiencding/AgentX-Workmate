...
credentials to ``~/.agentx/.env``. It previously called ``Path.write_text``
...
other subprocess-influencing or AgentX-runtime-location name) could
otherwise plant a value into ``.env`` via the interactive memory-setup
wizard. The next AgentX process would load it through the
...
def _env_file_keys() -> set[str]:
    """Parse ``~/.agentx/.env`` directly and return the set of keys present.
...
def test_denylisted_key_does_not_block_other_writes(capsys):
    ...
    _write_env_vars({
        ...
        "AGENTX_LANGFUSE_PUBLIC_KEY": "pk-test-123",
        "OPENROUTER_API_KEY": "sk-or-test-456",
    })

    assert "LD_PRELOAD" not in _env_file_keys()
    env = load_env()
    assert env["AGENTX_LANGFUSE_PUBLIC_KEY"] == "pk-test-123"
...
def test_legitimate_hermes_integration_key_still_writable():
    ...
    following the ``AGENTX_*`` convention (AGENTX_LANGFUSE_*,
    AGENTX_SPOTIFY_*, AGENTX_QWEN_BASE_URL, ...) must keep working or
    the memory-setup wizard regresses for every plugin that follows
    the convention."""
    _write_env_vars({
        "AGENTX_LANGFUSE_PUBLIC_KEY": "pk-lf-789",
        "AGENTX_QWEN_BASE_URL": "https://example.com/v1",
    })

    env = load_env()
    assert env["AGENTX_LANGFUSE_PUBLIC_KEY"] == "pk-lf-789"
    assert env["AGENTX_QWEN_BASE_URL"] == "https://example.com/v1"
...
def test_explicit_hermes_home_writes_to_that_env_file(tmp_path):
    ...
    AgentX home; keep that target while still routing through
