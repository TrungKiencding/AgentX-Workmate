"""Tests for ``agentx debug`` CLI command and debug utilities."""
...
def hermes_home(tmp_path, monkeypatch):
    """Set up an isolated AGENTX_HOME with minimal logs."""
...
class TestMissingLogNote:
    ...
    `agentx debug share` runs on the backend, so a desktop connected to a
    ...
    def test_client_written_log_names_its_writer_and_path(self, hermes_home):
        ...
        assert "AgentX Workmate Desktop" in snap.tail_text
...
# ---------------------------------------------------------------------------
# Capture log redaction (force=True applies regardless of AGENTX_REDACT_SECRETS)
# ---------------------------------------------------------------------------
...
class TestCaptureLogSnapshotRedaction:
    ...
    def hermes_home_with_secret(self, tmp_path, monkeypatch):
        """Isolated AGENTX_HOME whose agent.log contains a vendor-prefixed token."""
        home = tmp_path / ".agentx"
        home.mkdir()
        monkeypatch.setenv("AGENTX_HOME", str(home))
        ...
        monkeypatch.delenv("AGENTX_REDACT_SECRETS", raising=False)
    ...
    def test_force_true_works_when_redaction_disabled(
        ...
        input unchanged when AGENTX_REDACT_SECRETS=false, and the share-time
        ...
        monkeypatch.setenv("AGENTX_REDACT_SECRETS", "false")

        from hermes_cli.debug import _capture_log_snapshot

        assert os.environ.get("AGENTX_REDACT_SECRETS", "") == "false"
...
class TestRunDebugShare:
    ...
    def test_share_uploads_five_pastes(self, hermes_home, capsys):
        ...
        with patch("hermes_cli.dump.run_dump") as mock_dump, \
            ...
            mock_dump.side_effect = lambda a: print("--- agentx dump ---\nversion: test\n--- end dump ---")
            run_debug_share(args)
...
class TestScheduleAutoDelete:
    ...
    ``~/.agentx/pastes/pending.json`` and lets ``_sweep_expired_pastes``
    handle the DELETE requests synchronously on the next ``agentx debug``
    invocation.
    """
...
class TestBuildDebugShare:
    ...
    Backs both ``agentx debug share`` (CLI) and ``POST /api/ops/debug-share``
...
class TestShareConsentGate:
    """`agentx debug share` requires explicit consent before uploading.
