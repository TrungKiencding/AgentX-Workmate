"""Tests for ``agentx update`` / ``--check`` inside the Docker container.
...
fix, ``agentx update`` would fall through to ``"✗ Not a git repository.
Please reinstall: curl ... install.sh"`` — that script installs a *new*
host-side AgentX, not an update to the running container, so the message
...
def test_cmd_update_in_docker_prints_guidance_and_exits(
    ...
    """``agentx update`` inside Docker → friendly message + exit 1, no git calls."""
    ...
    assert "docker pull trungkiencding/agentx-workmate:latest" in out
...
def test_format_docker_update_message_contents():
    ...
    assert "docker pull trungkiencding/agentx-workmate:latest" in msg
    ...
    assert "AGENTX_HOME" in msg or "/opt/data" in msg, (
