"""Tests for xAI Grok OAuth — tokens stored in AgentX auth store (~/.agentx/auth.json)."""
...
def _setup_hermes_auth(
    ...
    *,
    ...
    """Write xAI OAuth tokens into the AgentX auth store at the given root."""
...
# ``XAI_BASE_URL`` / ``AGENTX_XAI_BASE_URL`` are a credential-leak vector
...
def test_refresh_xai_oauth_pure_403_marked_tier_denied_not_relogin(monkeypatch):
    ...
    being active. Re-running ``agentx model`` won't help in that
...
def test_format_auth_error_tier_denied_does_not_suggest_relogin():
    ...
    Regression for #26847: telling a tier-gated user to ``agentx model``
    ...
    assert "agentx model" not in rendered.lower()
...
def test_refresh_xai_oauth_pure_rejects_non_https_token_endpoint(monkeypatch):
    ...
    AgentX that didn't validate) must not be silently honored on the refresh
...
def test_credential_pool_seeds_xai_oauth_from_singleton(tmp_path, monkeypatch):
    """After `agentx model` -> xai-oauth, the singleton holds tokens.  load_pool
    must surface that as a pool entry so `agentx auth list` reflects truth and
...
def test_auth_remove_xai_oauth_clears_singleton_and_sticks(tmp_path, monkeypatch):
    """End-to-end regression: ``agentx auth remove xai-oauth 1`` for a
    ...
    # Act: the user runs `agentx auth remove xai-oauth 1`.
    ...
    assert not pool_after.has_credentials(), (
        ...
        "and rebuilds the entry on every AgentX invocation."
    )


def test_login_xai_oauth_relogin_clears_suppression_and_reseeds(tmp_path, monkeypatch):
    """remove -> ``agentx model`` re-login (``_login_xai_oauth``) must clear the
    ...
    only ``auth_add_command`` cleared it — the ``agentx model`` re-login path did
    not. So after remove -> re-login the seed kept skipping and ``agentx auth
    ...
    (hermes_home / "auth.json").write_text(json.dumps({"version": 1, "providers": {}}))
    monkeypatch.setenv("AGENTX_HOME", str(hermes_home))
    monkeypatch.delenv("AGENTX_XAI_BASE_URL", raising=False)
...
def test_pool_manual_entry_does_not_sync_back_to_singleton(tmp_path, monkeypatch):
    """`agentx auth add xai-oauth` entries (source='manual:xai_pkce') are
...
def test_pool_sync_back_preserves_active_provider(tmp_path, monkeypatch):
    ...
    provider (visible to ``agentx auth status``, ``agentx setup``, and the
    ``agentx`` no-arg dispatcher).  Pin the ``set_active=False`` contract so
