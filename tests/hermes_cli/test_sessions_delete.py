...
def test_sessions_delete_accepts_unique_id_prefix(monkeypatch, capsys):
    ...
    monkeypatch.setattr(
        sys,
        "argv",
        ["agentx", "sessions", "delete", "20260315_092437_c9a6", "--yes"],
    )
...
def _run_prune(monkeypatch, capsys, argv_tail, candidates=None):
    """Run `agentx sessions prune <argv_tail>` against a FakeDB, capturing
    ...
    monkeypatch.setattr(hermes_state, "SessionDB", lambda: FakeDB())
    monkeypatch.setattr(
        sys, "argv", ["agentx", "sessions", "prune", *argv_tail]
    )
...
def test_sessions_prune_bare_keeps_90_day_default(monkeypatch, capsys):
    """A truly bare `agentx sessions prune` keeps the implicit 90-day cutoff."""
