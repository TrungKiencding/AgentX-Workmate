...
MUTATING_CONFIRMATION_SMOKE_COMMANDS = [
    ...
    "mcp picker",
    "backup --quick -o /tmp/agentx-console-test.zip",
    "import /tmp/agentx-console-test.zip",
...
def test_repl_runs_non_interactive_lines_without_prompts(_isolate_hermes_home):
    ...
    assert code == 0
    assert "AgentX Console" in stdout.getvalue()
    assert "agentx>" not in stdout.getvalue()
