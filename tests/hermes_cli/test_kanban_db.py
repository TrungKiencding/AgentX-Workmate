...
def kanban_home(tmp_path, monkeypatch):
    """Isolated AGENTX_HOME with an empty kanban DB."""
...
def test_rate_limit_exit_requeues_without_counting_failure(
    ...
    monkeypatch.setattr(_kb, "_pid_alive", lambda _pid: False)
    monkeypatch.setenv("AGENTX_KANBAN_CRASH_GRACE_SECONDS", "0")
...
def test_respawn_guard_defers_rate_limited_within_cooldown(
    ...
    monkeypatch.setenv("AGENTX_KANBAN_RATE_LIMIT_COOLDOWN_SECONDS", "300")
    now = 5_000_000
...
def test_is_managed_scratch_path_rejects_kanban_metadata_subtrees(kanban_home):
    """AgentX' own DB/metadata/log subtrees under ``<kanban_home>/kanban`` are NOT managed.
    ...
    ``shutil.rmtree`` AgentX' DB/metadata/logs on task completion.
    """
...
# spawned with `agentx -p <profile>` must read/write the same kanban.db
...
# where `kanban_db_path()` resolved to the active profile's AGENTX_HOME.
# ---------------------------------------------------------------------------

class TestSharedBoardPaths:
    """`kanban_home`/`kanban_db_path`/`workspaces_root`/`worker_log_path`
    must anchor at the **shared root**, not the active profile's AGENTX_HOME."""

    def _set_home(self, monkeypatch, tmp_path, hermes_home):
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
        monkeypatch.setenv("AGENTX_HOME", str(hermes_home))
        monkeypatch.delenv("AGENTX_KANBAN_HOME", raising=False)


    def test_profile_worker_resolves_to_shared_root(
        self, tmp_path, monkeypatch
    ):
        # Reproduces the bug: dispatcher uses ~/.agentx/kanban.db,
        # worker spawned with -p <profile> previously resolved to
        # ~/.agentx/profiles/<profile>/kanban.db. After the fix both
        # converge on ~/.agentx/kanban.db.
    ...
    def test_dispatcher_and_worker_share_a_real_database(
        ...
        # AGENTX_HOME perspectives via a real SQLite file. Without the
        ...
        # Worker switches to the profile AGENTX_HOME and reads.
        monkeypatch.setenv("AGENTX_HOME", str(profile_home))
        with kb.connect() as conn:
    ...
    def test_dispatcher_spawn_injects_kanban_paths_without_stale_session(
        ...
        # AGENTX_SESSION_* identity inherited from the long-lived gateway.
        # The one exception is AGENTX_SESSION_SOURCE, which the dispatcher
        ...
        env = captured["env"]
        assert env["AGENTX_KANBAN_DB"] == str(default_home / "kanban.db")
        assert env["AGENTX_KANBAN_WORKSPACES_ROOT"] == str(
            default_home / "kanban" / "workspaces"
        )
        assert env["AGENTX_KANBAN_TASK"] == "t_dispatch_env"
        assert env["AGENTX_KANBAN_BRANCH"] == "wt/t_dispatch_env"
        for key in sc._VAR_MAP:
            if key == "AGENTX_SESSION_SOURCE":
...
def test_unlink_tasks_triggers_recompute_ready(kanban_home):
    ...
    next dispatcher tick or a manual 'agentx kanban recompute' would promote it.
    """
    with kb.connect() as conn:
        # A is done.
...
#
# Workers spawned by the dispatcher must use a `agentx` invocation that does
...
# $PATH that doesn't include the venv's bin/, so a bare `["agentx", ...]`
...
def test_resolve_hermes_argv_falls_back_to_module_form_when_no_path_shim(monkeypatch):
    ...
    Pins the correct module name (NOT `agentx` — there is no top-level
    `agentx` package). Regression for #23198: the original PR shipped
    `python -m agentx` which fails with `No module named agentx` on every
    invocation.
    """
    import shutil
    import sys
    ...
    monkeypatch.delenv("AGENTX_BIN", raising=False)
...
def test_resolve_hermes_argv_module_actually_runs():
    ...
    with mock.patch.dict(os.environ, {}, clear=False):
        os.environ.pop("AGENTX_BIN", None)
    ...
    assert "AgentX Workmate" in r.stdout, f"unexpected output: {r.stdout[:200]!r}"
