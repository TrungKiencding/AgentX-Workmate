...
Covers the three paths changed by fix/bedrock-provider-model-ids-live-discovery:
  ...
  2. list_authenticated_providers() Section 2 (AGENTX_OVERLAYS) — bedrock
...
# ---------------------------------------------------------------------------
# 2. list_authenticated_providers() — bedrock via AGENTX_OVERLAYS (Section 2)
# ---------------------------------------------------------------------------

class TestListAuthenticatedProvidersBedrock:
    """Bedrock should appear in the /model picker when AWS creds are present."""

    def test_bedrock_shown_with_credentials(self, monkeypatch):
        """Bedrock must appear when AWS credentials resolve."""
...
class TestBedrockOverlayRegistration:
    """bedrock entry in AGENTX_OVERLAYS is correctly configured."""

    def test_bedrock_overlay_exists(self):
        from hermes_cli.providers import AGENTX_OVERLAYS
        assert "bedrock" in AGENTX_OVERLAYS
