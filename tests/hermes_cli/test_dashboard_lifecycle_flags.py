"""Tests for ``agentx dashboard --stop`` / ``--status`` flags.

These flags share the detection + kill path with the post-``agentx update``
...
class TestDashboardStatus:
    def test_status_no_processes(self, capsys):
        ...
        assert "No agentx dashboard processes running" in out

    def test_status_with_processes(self, capsys):
        processes = [
            (12345, "agentx dashboard --port 9119"),
        ...
        assert exc.value.code == 0
        out = capsys.readouterr().out
        assert "2 agentx dashboard process(es) running" in out
...
class TestDashboardStop:
    ...
    def test_stop_kills_and_exits_zero_when_all_killed(self, capsys):
        ...
        # for the post-`agentx update` path).
...
class TestLifecycleFlagsTakePrecedence:
    ...
    who typed ``agentx dashboard --stop`` must not end up ALSO starting
    a new server."""
...
class TestArgparseWiring:
    ...
    ``agentx dashboard --stop`` / ``--status`` actually parse."""
