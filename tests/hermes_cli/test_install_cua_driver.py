...
* Be supported-platform-only — no-op silently elsewhere so ``agentx update``
...
class TestRequireConfirmedUpdate:
    """`agentx update` passes require_confirmed_update=True: the full
    ...
    Explicit `agentx computer-use install --upgrade` keeps the old
    ...
    def test_explicit_upgrade_still_falls_through_on_indeterminate(self):
        # `agentx computer-use install --upgrade` (default flag): the old
...
class TestUpdateCheckTimeoutDefaults:
    ...
    full reinstall fall-through during `agentx update`.
    """
