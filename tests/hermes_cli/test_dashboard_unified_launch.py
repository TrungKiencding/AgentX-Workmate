...
class TestUnifiedDashboardRouting:
    ...
    def test_profile_launch_reexecs_machine_dashboard(self, main_mod, monkeypatch):
        ...
        # AGENTX_HOME.  For a standard install (AGENTX_HOME unset) that root is
        # the platform-native default (~/.agentx), NOT dropped — see the Docker
        # test below for why we resolve explicitly instead of popping.
        from hermes_constants import get_default_hermes_root
        assert env.get("AGENTX_HOME") == str(get_default_hermes_root())


    def test_desktop_profile_backend_skips_machine_dashboard_reroute(self, main_mod, monkeypatch):
        """A desktop-spawned named-profile backend (AGENTX_DESKTOP=1) must NOT
