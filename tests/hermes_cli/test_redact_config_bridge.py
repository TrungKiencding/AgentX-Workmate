...
`AGENTX_REDACT_SECRETS` at module-import time. `hermes_cli/main.py` at
...
in .env), the toggle was silently ignored in both `agentx chat` and
`agentx gateway run`.

Fix: bridge `security.redact_secrets` from config.yaml → `AGENTX_REDACT_SECRETS`
...
def test_dotenv_redact_secrets_beats_config_yaml(tmp_path):
    """.env AGENTX_REDACT_SECRETS takes precedence over config.yaml."""
    hermes_home = tmp_path / ".agentx"
    hermes_home.mkdir()
    (hermes_home / "config.yaml").write_text(
        textwrap.dedent(
    ...
    (hermes_home / ".env").write_text("AGENTX_REDACT_SECRETS=true\n")
