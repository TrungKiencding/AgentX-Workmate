"""Tests for agentx backup and import commands."""
...
def _make_hermes_tree(root: Path) -> None:
    """Create a realistic ~/.agentx directory structure for testing."""
    ...
    # agentx-agent repo (should be EXCLUDED)
    (root / "agentx-agent").mkdir(exist_ok=True)
    (root / "agentx-agent" / "run_agent.py").write_text("# big file\n")
    (root / "agentx-agent" / ".git").mkdir()
    (root / "agentx-agent" / ".git" / "HEAD").write_text("ref: refs/heads/main\n")
...
class TestShouldExclude:
    def test_excludes_hermes_agent(self):
        ...
        assert _should_exclude(Path("agentx-agent/run_agent.py"))
        assert _should_exclude(Path("agentx-agent/.git/HEAD"))
...
class TestBackup:
    ...
    def test_skips_symlinked_files(self, tmp_path, monkeypatch):
        """Backup must not dereference symlinks and leak files outside AGENTX_HOME."""
        hermes_home = tmp_path / ".agentx"
        hermes_home.mkdir()
        _make_hermes_tree(hermes_home)
...
class TestValidateBackupZip:
    ...
    def test_state_db_passes(self, tmp_path):
        """A zip containing state.db is accepted as a valid AgentX backup."""
...
class TestRoundTrip:
    def test_backup_then_import(self, tmp_path, monkeypatch):
        ...
        # Source
        src_home = tmp_path / "source" / ".agentx"
        src_home.mkdir(parents=True)
        _make_hermes_tree(src_home)

        monkeypatch.setenv("AGENTX_HOME", str(src_home))
        ...
        dst_home = tmp_path / "dest" / ".agentx"
        dst_home.mkdir(parents=True)
        monkeypatch.setenv("AGENTX_HOME", str(dst_home))
        ...
        # agentx-agent should NOT be present
        assert not (dst_home / "agentx-agent").exists()
...
class TestBackupEdgeCases:
    ...
    def test_empty_hermes_home(self, tmp_path, monkeypatch):
        """Backup handles empty agentx home (no files to back up)."""
        hermes_home = tmp_path / ".agentx"
        hermes_home.mkdir()
...
class TestQuickSnapshot:
    ...
    def hermes_home(self, tmp_path):
        """Create a fake AGENTX_HOME with critical state files."""
...
# ---------------------------------------------------------------------------
# Pre-update backup (agentx update safety net)
# ---------------------------------------------------------------------------
...
class TestPreUpdateBackup:
    """Tests for create_pre_update_backup — the auto-backup ``agentx update``
    ...
    def test_backup_contents_match_full_backup(self, hermes_home):
        ...
        ``agentx backup`` would, and should exclude the same directories."""
        ...
        # agentx-agent repo excluded
        assert not any(n.startswith("agentx-agent/") for n in names)
    ...
    def test_skips_symlinked_files(self, hermes_home, tmp_path):
        """Pre-update backups must not dereference symlinks outside AGENTX_HOME."""
        from hermes_cli.backup import create_pre_update_backup
...
class TestRunPreUpdateBackup:
    ...
    def hermes_home(self, tmp_path, monkeypatch):
        ...
        root.mkdir()
        _make_hermes_tree(root)
        # Point AGENTX_HOME at the temp dir so config + backup paths resolve here
        monkeypatch.setenv("AGENTX_HOME", str(root))
        # Make Path.home() point at tmp_path for anything that uses it
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
        # Config reads resolve AGENTX_HOME dynamically and their caches are
...
# ---------------------------------------------------------------------------
# Pre-migration backup (agentx claw migrate safety net)
# ---------------------------------------------------------------------------

class TestPreMigrationBackup:
    """Tests for create_pre_migration_backup — the auto-backup
    ``agentx claw migrate`` runs before mutating ~/.agentx/."""
    ...
    def test_restorable_with_hermes_import(self, hermes_home, tmp_path):
        """The zip produced by pre-migration backup must be a valid AgentX
        backup — `agentx import` should accept it."""
...
class TestRestoreCronJobsIfEmptied:
    """`agentx update` config migration can leave cron/jobs.json valid-but-empty,
...
# location, NOT under AGENTX_HOME. (backup/import cycle data-loss fix)
# ---------------------------------------------------------------------------
...
class TestMemoryProviderExternalPaths:
    ...
    def test_import_restores_external_to_home_relative_location(self, tmp_path, monkeypatch):
        """_external/ members restore to ~/<relpath>, not under AGENTX_HOME,
        and credential-shaped files get 0600."""
        dst_home = tmp_path / "dst"
        dst_home.mkdir()
        hermes_home = dst_home / ".agentx"
        hermes_home.mkdir()
        ...
        # External state did NOT leak into AGENTX_HOME.
