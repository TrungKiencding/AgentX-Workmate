"""Tests for the post-update *import* guard in ``agentx update``.
...
def test_hint_fires_for_first_party_import_error():
    ...
    assert any("agentx update" in line for line in hint)
...
def test_import_guard_prefers_the_project_venv_interpreter(monkeypatch, tmp_path):
    """``agentx update`` can run under a different Python than the install's.
...
@pytest.mark.parametrize("modname", ["agents", "agentops", "toolsets_x", "agentxx"])
...
def test_probe_and_hint_share_one_first_party_definition():
    ...
    `cli` was first-party to the hint but not the probe, and `agentxx`
    (third-party) matched the probe's loose `startswith("agentx")`. A user
    ...
    for lookalike in ("agents", "agentops", "toolsets_x", "agentxx"):
