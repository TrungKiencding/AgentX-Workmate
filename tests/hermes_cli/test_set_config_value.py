...
def _isolated_hermes_home(tmp_path):
    """Point AGENTX_HOME at a temp dir so tests never touch real config."""
...
class TestListNavigation:
    """agentx config set must preserve YAML list fields when using numeric
...
class TestSchemaValidation:
    """#34067: ``agentx config set`` must not report bare success for
    ...
    a post-write notice warns that AgentX may never read it and suggests the
    ...
    def test_desktop_macos_signing_identity_is_accepted(self, _isolated_hermes_home, capsys):
        ...
        set_config_value("desktop.macos_signing_identity", "AgentX Local Signing")
        import yaml
        saved = yaml.safe_load(_read_config(_isolated_hermes_home))
        assert saved["desktop"]["macos_signing_identity"] == "AgentX Local Signing"
        assert "not a recognized config key" not in capsys.readouterr().out
...
class TestDisplaySkinTouch:
    ...
    (`agentx config set display.skin X` while it is already X — the recovery
...
class TestMappingGuard:
    """``agentx config set <section> <scalar>`` must not silently destroy an
    ...
    def test_bare_model_shorthand_preserves_siblings(self, _isolated_hermes_home):
        """agentx config set model <id> → model.default, siblings survive."""
        self._write_config(_isolated_hermes_home, {
            "model": {
                "default": "gpt-4o",
                "provider": "openai-api",
    ...
    def test_non_model_mapping_is_refused(self, _isolated_hermes_home):
        """agentx config set terminal bash → refuse, terminal has sub-keys."""
        self._write_config(_isolated_hermes_home, {
            "terminal": {
                "backend": "docker",
    ...
    def test_non_model_mapping_force_overwrites(self, _isolated_hermes_home):
        """agentx config set --force terminal bash → proceed, section wiped."""
        self._write_config(_isolated_hermes_home, {
            "terminal": {
                "backend": "docker",
                "shell": "bash",
    ...
    def test_model_force_overwrites_entire_section(self, _isolated_hermes_home):
        """agentx config set --force model <id> → overwrite entire section."""
        self._write_config(_isolated_hermes_home, {
            "model": {
                "default": "gpt-4o",
                "provider": "openai-api",
