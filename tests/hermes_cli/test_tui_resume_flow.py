...
def test_oneshot_wires_session_db_for_recall(monkeypatch):
    """agentx -z bypasses HermesCLI, but recall still needs SessionDB."""
...
def test_launch_tui_exports_model_provider_and_toolsets(monkeypatch, main_mod):
    ...
    def fake_call(argv, cwd=None, env=None):
        ...
        active_path_during_call = Path(env["AGENTX_TUI_ACTIVE_SESSION_FILE"])
    ...
    active_path = Path(env["AGENTX_TUI_ACTIVE_SESSION_FILE"])
    assert active_path.name.startswith("agentx-tui-active-session-")
...
def test_make_tui_argv_dev_prebuilds_hermes_ink(monkeypatch, main_mod, tmp_path):
    ...
    monkeypatch.delenv("AGENTX_TUI_DIR", raising=False)
