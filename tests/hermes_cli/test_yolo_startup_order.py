"""Regression tests for #60328: --yolo must set AGENTX_YOLO_MODE in
...
def _run_main_and_capture_yolo_at_startup(monkeypatch, argv):
    """Run main() with *argv*, capturing AGENTX_YOLO_MODE at the
    ...
    def spy_prepare_startup(args):
        yolo_at_startup["value"] = os.environ.get("AGENTX_YOLO_MODE")
...
def test_top_level_yolo_flag_sets_env_before_startup(monkeypatch):
    """agentx --yolo must set AGENTX_YOLO_MODE before
    _prepare_agent_startup imports tools.approval."""
    result = _run_main_and_capture_yolo_at_startup(
        monkeypatch, ["agentx", "--yolo"]
    )
    assert result == "1", (
        "AGENTX_YOLO_MODE was not '1' when _prepare_agent_startup was "
