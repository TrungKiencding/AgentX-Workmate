"""Tests for issue #26670 — concurrent agentx.exe detection and improved
quarantine retry / reboot-deferred fallback during `agentx update` on Windows.
...
def _make_proc(pid: int, exe: str, name: str = "agentx.exe"):
...
def test_detect_concurrent_parents_call_robust_to_one_bad_hop(_winp, tmp_path):
    ...
    rows = [
        ...
        _make_proc(launcher_pid, str(shim), "agentx.exe"),
    ]
    ...
    assert result == [(launcher_pid, "agentx.exe")]
...
def test_quarantine_succeeds_first_attempt(_winp, tmp_path):
    ...
    assert orig == shim
    assert quarantine.name.startswith("agentx.exe.old.")
