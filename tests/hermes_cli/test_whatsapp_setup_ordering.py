...
Before the fix, ``agentx whatsapp`` wrote ``WHATSAPP_ENABLED=true`` at
...
``agentx gateway`` then paid a 30s bridge-bootstrap timeout and queued
...
def test_existing_pairing_skip_branch_enables_whatsapp(isolated_home, monkeypatch):
    """User runs ``agentx whatsapp`` with an existing paired session and
