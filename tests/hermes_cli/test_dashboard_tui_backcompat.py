"""Regression test: `agentx dashboard --tui` must not hard-crash.

Older AgentX desktop app shells (<= 0.15.x) spawn the backend as::

    agentx dashboard --no-open --tui --host 127.0.0.1 --port <PORT>
...
died before it became ready and the desktop GUI showed only "AgentX couldn't
