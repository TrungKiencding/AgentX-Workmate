...
Every aux picker (``agentx model`` → Configure auxiliary models, the
``agentx tools`` vision picker, and any future one) must route through
...
def configured_home(tmp_path, monkeypatch):
    """A AGENTX_HOME with one ``providers:`` entry and one legacy
