...
Covers the canonical fix for issues #4146, #27303, #30882, #33057:
  ...
  4. tools.code_execution_tool._scrub_child_env — broad AGENTX_ prefix dropped,
...
def gw_session(monkeypatch):
    """A clean gateway session: AGENTX_GATEWAY_SESSION set, a bound session
...
def test_env_scrub_hermes_allowlist_and_secret_blocks():
    ...
    env = {
        ...
        "AGENTX_HOME": "/h", "AGENTX_PROFILE": "p",
        "AGENTX_CONFIG": "/c.yaml", "AGENTX_ENV": "/e",
        "AGENTX_DELEGATED_CHILD_CONTEXT": "1",
        # other AGENTX_* → dropped (broad prefix removed)
        "AGENTX_BASE_URL": "https://x", "AGENTX_INTERACTIVE": "1",
        "AGENTX_KANBAN_DB": "postgres://u:p@h/db",
    ...
    for kept in (
        ...
        "AGENTX_DELEGATED_CHILD_CONTEXT", "PATH",
    ):
        assert kept in out, f"{kept} should be kept"
    for dropped in (
        "AGENTX_BASE_URL", "AGENTX_INTERACTIVE", "AGENTX_KANBAN_DB",
...
def test_env_scrub_no_log_when_nothing_dropped(caplog):
    """No diagnostic noise when there are no dropped AGENTX_* vars."""
    import logging
