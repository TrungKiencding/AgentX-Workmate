...
class TestSmartApproval:
    ...
    def test_smart_approval_does_not_allowlist_the_pattern_for_session(self, monkeypatch):
        ...
        monkeypatch.setenv("AGENTX_SESSION_KEY", session_key)
        monkeypatch.setenv("AGENTX_EXEC_ASK", "1")
        monkeypatch.delenv("AGENTX_CRON_SESSION", raising=False)
        monkeypatch.setattr(
            approval_module,
...
class TestDetectDangerousRm:
    ...
    def test_symlinked_temp_dir_only_exempts_canonical_target(self, tmp_path):
        ...
        basename = "agentx-verify-example.py"
    ...
    def test_verification_cleanup_exemption_rejects_broader_deletions(self):
        commands = (
            "rm -rf /tmp/agentx-verify-example.py",
            "rm -f /tmp/agentx-verify-example.py /tmp/other.py",
            "rm -f /tmp/nested/../agentx-verify-example.py",
            "rm -f /tmp/a/../../tmp/agentx-verify-example.py",
            "rm -f /var/tmp/agentx-verify-example.py",
            "rm -f /tmp/agentx-verify-*",
            "rm -f /tmp/agentx-verify-$(touch>/tmp/pwned).py",
            "rm -f /tmp/agentx-ad-hoc-`touch>/tmp/pwned`.py",
            "rm -f /tmp/agentx-verify-example.py; touch /tmp/pwned",
        )
        with mock_patch("tempfile.gettempdir", return_value="/tmp"):
...
class TestWindowsShellDestructiveCommands:
    def test_windows_destructive_requires_approval(self):
        cases = [
            (r"cmd /c del /f /q C:\tmp\agentx-victim\file.txt", "Windows cmd destructive delete"),
            (r"cmd.exe /k rmdir /s /q C:\tmp\agentx-victim", "Windows cmd destructive delete"),
            ...
            (r"powershell Remove-Item -Recurse -Force C:\tmp\agentx-victim",
...
class TestSessionKeyContext:
    def test_context_session_key_overrides_process_env(self):
        ...
        try:
            with mock_patch.dict("os.environ", {"AGENTX_SESSION_KEY": "bob"}, clear=False):
...
class TestTeePattern:
    ...
    def test_tee_to_sensitive_target(self):
        for command in (
            ...
            "echo x | tee /dev/sda",
            "echo x | tee ~/.agentx/.env",
            "echo x | tee $AGENTX_HOME/.env",
            'echo x | tee "$AGENTX_HOME/.env"',
        ):
            dangerous, key, desc = detect_dangerous_command(command)
            assert dangerous is True, command
...
class TestHermesConfigWriteProtection:
    ...
    ~/.agentx/config.yaml (#14639). config.yaml IS the security policy
    ...
    def test_write_idioms_against_config(self):
        for command in (
            "echo 'approvals:' > ~/.agentx/config.yaml",
            "echo '  mode: off' >> ~/.agentx/config.yaml",
            "echo x | tee ~/.agentx/config.yaml",
            "echo x | tee $AGENTX_HOME/config.yaml",
            "cp /tmp/evil.yaml ~/.agentx/config.yaml",
        ):
            dangerous, key, desc = detect_dangerous_command(command)
            assert dangerous is True, command
            assert key is not None, command
...
class TestSensitiveCopyMovePattern:
    ...
    shell rc files, or ~/.agentx/config.yaml/.env must require approval — the
...
class TestSmartDeniedPrompt:
    ...
    def test_smart_deny_uses_locale_specific_once_deny_choices(self, monkeypatch, capsys):
        monkeypatch.setenv("AGENTX_LANGUAGE", "tr")
...
class TestGatewayProtection:
    ...
    def test_gateway_run_backgrounded_detected(self):
        cmd = "kill 1605 && cd ~/.agentx/agentx-agent && source venv/bin/activate && python -m hermes_cli.main gateway run --replace &disown; echo done"
        dangerous, key, desc = detect_dangerous_command(cmd)
        assert dangerous is True
...
class TestWebhookApprovalExclusion:
    ...
    def test_webhook_platform_returns_false(self, monkeypatch):
        ...
        monkeypatch.setenv("AGENTX_SESSION_PLATFORM", "webhook")
    ...
    def test_all_unattended_platforms_return_false(self, monkeypatch):
        ...
        for platform in _UNATTENDED_APPROVAL_PLATFORMS:
            monkeypatch.setenv("AGENTX_SESSION_PLATFORM", platform)
    ...
    def test_non_webhook_gateway_session_returns_true(self, monkeypatch):
        ...
        monkeypatch.delenv("AGENTX_CRON_SESSION", raising=False)
        monkeypatch.setenv("AGENTX_GATEWAY_SESSION", "1")
    ...
    def test_cron_session_returns_false_regardless_of_platform(self, monkeypatch):
        ...
        monkeypatch.setenv("AGENTX_CRON_SESSION", "1")
        monkeypatch.setenv("AGENTX_SESSION_PLATFORM", "telegram")
    ...
    def test_no_platform_returns_false(self, monkeypatch):
        ...
        monkeypatch.delenv("AGENTX_CRON_SESSION", raising=False)
        monkeypatch.delenv("AGENTX_GATEWAY_SESSION", raising=False)
        monkeypatch.delenv("AGENTX_SESSION_PLATFORM", raising=False)
    ...
    def test_webhook_dangerous_command_denies_by_default(self, monkeypatch):
        ...
        monkeypatch.setenv("AGENTX_SESSION_PLATFORM", "webhook")
        monkeypatch.setenv("AGENTX_SESSION_KEY", "test-webhook-session")
    ...
    def test_api_server_dangerous_command_denies_by_default(self, monkeypatch):
        ...
        monkeypatch.delenv("AGENTX_GATEWAY_SESSION", raising=False)
        monkeypatch.delenv("AGENTX_INTERACTIVE", raising=False)
        monkeypatch.setenv("AGENTX_SESSION_PLATFORM", "api_server")
        monkeypatch.setenv("AGENTX_SESSION_KEY", "test-api-session")
...
class TestApprovalTimeoutIsNotConsent:
    ...
    def setup_method(self):
        ...
        self._saved_env = {
            k: os.environ.get(k)
            for k in ("AGENTX_GATEWAY_SESSION", "AGENTX_CRON_SESSION",
                      "AGENTX_YOLO_MODE",
                      "AGENTX_SESSION_KEY", "AGENTX_INTERACTIVE")
        }
        os.environ.pop("AGENTX_YOLO_MODE", None)
        os.environ.pop("AGENTX_INTERACTIVE", None)
        # AGENTX_CRON_SESSION takes priority over AGENTX_GATEWAY_SESSION in
        ...
        os.environ.pop("AGENTX_CRON_SESSION", None)
        os.environ["AGENTX_GATEWAY_SESSION"] = "1"
        os.environ["AGENTX_SESSION_KEY"] = self.SESSION_KEY
        ...
        with _patch("builtins.__import__", side_effect=self._make_failing_import(real_import)):
            with _patch("hermes_cli.config.load_config_readonly", return_value=cfg):
                with _patch("tools.approval.detect_dangerous_command", return_value=(False, None, None)):
                    with mock_patch.dict("os.environ", {"AGENTX_INTERACTIVE": "1"}, clear=False):
