...
def test_dockerfile_makes_opt_hermes_readonly_for_hermes_user() -> None:
    ...
    assert "chown -R root:root /opt/agentx" not in text
    assert "chmod -R a+rX /opt/agentx" not in text
    assert "chmod -R a-w /opt/agentx" not in text
...
def test_dockerfile_bakes_photon_sidecar_deps() -> None:
    ...
    assert not re.search(
        r"chown\s+-R\s+agentx:agentx\s+/opt/agentx/plugins", text
    )
