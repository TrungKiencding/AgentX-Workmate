"""Tests for tools.wake_word — the "Hey AgentX" hotword detector.
...
def test_config_defaults_and_clamping():
    ...
    assert ww.wake_phrase({"phrase": "hey agentx"}) == "hey agentx"
    assert ww.wake_phrase({}) == "hey agentx"
...
def test_requirements_openwakeword_available(monkeypatch):
    ...
    r = ww.check_wake_word_requirements(
        {"provider": "openwakeword", "phrase": "hey agentx"}
    )
    assert r["available"] is True
    assert r["provider"] == "openwakeword"
    assert r["phrase"] == "hey agentx"
...
def test_bundled_hey_hermes_model_ships_on_disk():
    # The "hey agentx" wake word works out of the box only if the model is
