...
def _legacy_posix_scrubber(source_env, is_passthrough):
    ...
    Deliberately updated for #27303 (the broad ``AGENTX_`` prefix was dropped
...
class TestPosixEquivalence:
    ...
    _POSIX_SYNTHETIC_ENV = {
        ...
        # AGENTX_* handling (#27303): only the operational allowlist passes;
        # every other AGENTX_* is dropped (the broad prefix was removed).
        "AGENTX_HOME": "/home/alice/.agentx",        # allowlisted → kept
        "AGENTX_PROFILE": "default",                 # allowlisted → kept
        "AGENTX_INTERACTIVE": "1",                   # not allowlisted → dropped
        "AGENTX_BASE_URL": "https://api.internal",   # not allowlisted → dropped
        "AGENTX_KANBAN_DB": "postgres://u:p@h/db",   # not allowlisted → dropped
