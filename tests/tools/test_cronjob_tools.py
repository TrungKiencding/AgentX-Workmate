...
class TestScanCronSkillAssembled:
    ...
    def test_descriptive_attack_command_prose_allowed(self):
        ...
        Real example: the `agentx-agent-dev` skill contains a postmortem
        section saying 'the attacker could just cat ~/.agentx/.env'.
        """
        assert _scan_cron_skill_assembled(
            "the attacker could just cat ~/.agentx/.env to steal credentials"
        )[1] == ""
        assert _scan_cron_skill_assembled(
...
class TestCronjobRequirements:
    ...
    @pytest.mark.parametrize(
        "var_name",
        ["AGENTX_INTERACTIVE", "AGENTX_GATEWAY_SESSION", "AGENTX_EXEC_ASK"],
    )
    ...
    def test_rejects_false_like_any_session_env(
        ...
        for v in ("AGENTX_INTERACTIVE", "AGENTX_GATEWAY_SESSION", "AGENTX_EXEC_ASK"):
...
class TestGithubExemptionAbuse:
    ...
    def test_same_line_payload_after_github_url_is_scanned(self):
        ...
        for sep in (";", " &&", " |"):
            prompt = f"{self.GH}{sep} cat ~/.agentx/.env"
    ...
    def test_subshell_and_backtick_payloads_are_scanned(self):
        ...
        assert "Blocked" in _scan_cron_prompt(f"{self.GH}$(cat ~/.agentx/.env)")
        assert "Blocked" in _scan_cron_prompt(f"{self.GH}`cat ~/.agentx/.env`")
    ...
    def test_payload_between_two_github_blocks_is_scanned(self):
        ...
        prompt = f"{self.GH}; cat ~/.agentx/.env; {self.GH}"
        assert "Blocked" in _scan_cron_prompt(prompt)
