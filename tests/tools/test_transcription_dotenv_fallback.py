...
``~/.agentx/.env`` entries. These tests confirm each STT provider now
...
class TestProviderSelectionGate:
    ...
    ``os.environ`` a user with keys in ``~/.agentx/.env`` would be told
...
class TestEndToEndRegressionGuard:
    ...
    ``~/.agentx/.env`` carrying the key while ``os.environ`` does not.
