...
code paths AgentX takes on native Windows.
...
class TestConfigureWindowsStdio:
    ...
    - respect AGENTX_DISABLE_WINDOWS_UTF8 opt-out
    """
...
class TestCodeExecutionTransportTcpFallback:
    ...
    AF_INET endpoints based on the AGENTX_RPC_SOCKET format.
    """
...
class TestGatewayDetachedWatcherWindowsFlags:
    ...
    def test_watcher_threads_hidden_console_spec_into_respawn(self):
        ...
        The spec supplies the stable cwd + env overlay (AGENTX_HOME,
        ...
        assert '_popen_kwargs["env"]' in block, (
            ...
            "AGENTX_HOME) from the restart spec."
        )
...
class TestGatewayRunRestartWatcherOuterPopenFallback:
    ...
    def test_outer_watcher_retries_without_breakaway_on_oserror(self, monkeypatch):
        ...
        monkeypatch.setattr(gr, "_resolve_hermes_bin", lambda: ["agentx"])
        ...
        assert "_AGENTX_GATEWAY" not in kw1["env"]
    ...
    def test_outer_watcher_dual_failure_warns_without_leaking_secrets(
        ...
        monkeypatch.setenv("AGENTX_TEST_SECRET", secret)
        ...
        assert secret in (kwargs_used.get("env") or {}).get("AGENTX_TEST_SECRET", "")
