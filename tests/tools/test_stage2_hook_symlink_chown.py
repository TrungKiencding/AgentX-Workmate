...
def test_chown_helper_refuses_target_under_symlinked_home(
    ...
    assert proc.returncode == 0, proc.stderr
    assert not log_path.exists(), "must not chown through a symlinked AGENTX_HOME"
    assert "refusing recursive chown through symlinked path" in proc.stdout
...
def test_stage2_skips_top_level_chown_for_symlinked_hermes_home(
    ...
    assert 'refuse_symlinked_path "chown" "$AGENTX_HOME"' in stage2_text
...
def test_stage2_skips_recursive_repairs_when_tree_is_already_owned(
    ...
    assert 'if [ -e "$AGENTX_HOME/$sub" ] && tree_has_non_hermes_owner "$AGENTX_HOME/$sub"; then' in stage2_text
    assert 'if [ -d "$AGENTX_HOME/profiles" ] && tree_has_non_hermes_owner "$AGENTX_HOME/profiles"; then' in stage2_text
    # Sibling every-boot chown blocks carry the same warm-boot gate.
    assert 'if [ -d "$AGENTX_HOME/cron" ] && tree_has_non_hermes_owner "$AGENTX_HOME/cron"; then' in stage2_text
    assert 'if [ -d "$AGENTX_HOME/platforms/pairing" ] && tree_has_non_hermes_owner "$AGENTX_HOME/platforms/pairing"; then' in stage2_text
    assert 'if [ -d "$AGENTX_HOME/pairing" ] && tree_has_non_hermes_owner "$AGENTX_HOME/pairing"; then' in stage2_text
