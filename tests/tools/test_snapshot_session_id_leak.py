"""Cross-session AGENTX_SESSION_ID leak via the shared bash snapshot.
...
``AGENTX_SESSION_ID`` into the snapshot, so every LATER session ``source``d that
stale value and its ``echo $AGENTX_SESSION_ID`` reported a FOREIGN session's id
— overriding the correct per-command Popen env injected by
``_inject_session_context_env``.

The fix strips the per-session bridged vars (AGENTX_SESSION_* / UI /
...
def test_export_snippet_shape():
    ...
    assert "${!AGENTX_SESSION_*}" in snippet
    assert "${!AGENTX_CRON_AUTO_DELIVER_*}" in snippet
    assert "AGENTX_UI_SESSION_ID" in snippet
    ...
    try:
        def run_as(sid):
            ...
            def worker():
                ...
                out["r"] = env.execute('echo "[$AGENTX_SESSION_ID]"')
        ...
        if os.path.exists(snap):
            with open(snap) as f:
                assert "AGENTX_SESSION_ID" not in f.read()
    finally:
        env.cleanup()
