...
class TestRegisterCredentialFiles:
    def test_dict_with_path_key(self, tmp_path):
        ...
        assert mounts[0]["container_path"] == "/root/.agentx/token.json"
...
class TestSkillsDirectoryMount:
    def test_returns_mount_when_skills_dir_exists(self, tmp_path):
        ...
        assert mounts[0]["container_path"] == "/root/.agentx/skills"
    ...
    def test_custom_container_base(self, tmp_path):
        ...
        with patch.dict(os.environ, {"AGENTX_HOME": str(hermes_home)}):
            mounts = get_skills_directory_mount(container_base="/home/user/.agentx")

        assert mounts[0]["container_path"] == "/home/user/.agentx/skills"
...
class TestIterSkillsFiles:
    def test_returns_files_skipping_symlinks(self, tmp_path):
        ...
        assert "/root/.agentx/skills/cat/myskill/SKILL.md" in paths
        assert "/root/.agentx/skills/cat/myskill/scripts/run.sh" in paths
    ...
    def test_skips_excluded_bookkeeping_dirs(self, tmp_path):
        ...
        paths = {f["container_path"] for f in files}
        assert "/root/.agentx/skills/cat/myskill/SKILL.md" in paths
        assert "/root/.agentx/skills/cat/myskill/references/api.md" in paths
        for excluded in (
            ".hub",
            ".archive",
            ".curator_backups",
...
class TestPathTraversalSecurity:
    ...
    def test_dotdot_traversal_rejected(self, tmp_path, monkeypatch):
        """'../sensitive' must not escape AGENTX_HOME."""
    ...
    def test_nested_subdir_inside_hermes_home_allowed(self, tmp_path, monkeypatch):
        """Files in subdirectories of AGENTX_HOME must be allowed."""
        hermes_home = tmp_path / ".agentx"
        hermes_home.mkdir()
    ...
    def test_symlink_traversal_rejected(self, tmp_path, monkeypatch):
        """A symlink inside AGENTX_HOME pointing outside must be rejected."""
        hermes_home = tmp_path / ".agentx"
        hermes_home.mkdir()
        monkeypatch.setenv("AGENTX_HOME", str(hermes_home))
        ...
        # The resolved path escapes AGENTX_HOME — must be rejected
        assert result is False
        assert get_credential_file_mounts() == []
...
class TestConfigPathTraversal:
    ...
    def test_config_traversal_rejected(self, tmp_path, monkeypatch):
        """'../secret' in config.yaml must not escape AGENTX_HOME."""
        hermes_home = tmp_path / ".agentx"
        hermes_home.mkdir()
        monkeypatch.setenv("AGENTX_HOME", str(hermes_home))
    ...
    def test_config_legitimate_file_works(self, tmp_path, monkeypatch):
        """Normal files inside AGENTX_HOME via config must still mount."""
        hermes_home = tmp_path / ".agentx"
        hermes_home.mkdir()
        monkeypatch.setenv("AGENTX_HOME", str(hermes_home))
...
class TestCacheDirectoryMounts:
    ...
    def test_returns_existing_cache_dirs(self, tmp_path, monkeypatch):
        ...
        assert "/root/.agentx/cache/documents" in paths
        assert "/root/.agentx/cache/audio" in paths
        assert "/root/.agentx/cache/videos" in paths
    ...
    def test_legacy_dir_names_resolved(self, tmp_path, monkeypatch):
        ...
        assert "/root/.agentx/cache/documents" in container_paths
        assert "/root/.agentx/cache/images" in container_paths
    ...
    def test_images_upload_dir_is_mounted(self, tmp_path, monkeypatch):
        ...
        Desktop / clipboard / PDF uploads land in ``AGENTX_HOME/images``, not
        ...
        assert "/root/.agentx/images" in by_container
        assert by_container["/root/.agentx/images"] == str(hermes_home / "images")
    ...
    def test_images_upload_file_maps_into_container(self, tmp_path, monkeypatch):
        ...
        assert (
            ...
            == "/root/.agentx/images/upload_20260722_181019_1.png"
        )
...
class TestMasterCredentialStoresAreNeverMountable:
    """Containment is not enough — AGENTX_HOME *is* where the keys live.
    ...
    ``../../.ssh/id_rsa`` from escaping AGENTX_HOME, but every master
    ...
    def test_master_credential_store_is_refused(self, tmp_path, rel_path):
        ...
        with patch.dict(os.environ, {"AGENTX_HOME": str(home)}):
    ...
    def test_refused_entry_does_not_block_the_rest_of_the_batch(self, tmp_path):
        ...
        paths = [m["container_path"] for m in mounts]
        assert "/root/.agentx/google_token.json" in paths
        assert "/root/.agentx/.env" not in paths
