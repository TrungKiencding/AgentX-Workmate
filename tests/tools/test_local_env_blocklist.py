...
from tools.environments.local import (
    LocalEnvironment,
    _AGENTX_PROVIDER_ENV_BLOCKLIST,
    _AGENTX_PROVIDER_ENV_FORCE_PREFIX,
)
...
class TestProviderEnvBlocklist:
    """Provider env vars loaded from ~/.agentx/.env must not leak."""
    ...
    def test_tool_and_gateway_vars_are_stripped(self):
        ...
        leaked_vars = {
            ...
            "AGENTX_DASHBOARD_SESSION_TOKEN": "dashboard-session-secret",
...
class TestForceEnvOptIn:
    """Callers can opt in to passing a blocked var via _AGENTX_FORCE_ prefix."""

    def test_force_prefix_passes_blocked_var(self):
        """_AGENTX_FORCE_OPENAI_API_KEY in self.env should inject OPENAI_API_KEY."""
        result_env = _run_with_env(self_env={
            f"{_AGENTX_PROVIDER_ENV_FORCE_PREFIX}OPENAI_API_KEY": "sk-explicit",
        })
        ...
        assert f"{_AGENTX_PROVIDER_ENV_FORCE_PREFIX}OPENAI_API_KEY" not in result_env
    ...
    def test_force_prefix_overrides_os_environ_block(self):
        ...
        result_env = _run_with_env(
            ...
            self_env={f"{_AGENTX_PROVIDER_ENV_FORCE_PREFIX}OPENAI_BASE_URL": "http://intended/v1"},
        )
...
class TestBlocklistCoverage:
    ...
    def test_issue_1002_offenders(self):
        ...
        assert must_block.issubset(_AGENTX_PROVIDER_ENV_BLOCKLIST)
    ...
    def test_registry_vars_are_in_blocklist(self):
        ...
        for pconfig in PROVIDER_REGISTRY.values():
            for var in pconfig.api_key_env_vars:
                if var in exempt:
                    continue
                assert var in _AGENTX_PROVIDER_ENV_BLOCKLIST, (
                    f"Registry var {var} (provider={pconfig.id}) missing from blocklist"
                )
            if pconfig.base_url_env_var:
                assert pconfig.base_url_env_var in _AGENTX_PROVIDER_ENV_BLOCKLIST, (
    ...
    def test_general_aws_chain_not_in_blocklist(self):
        ...
        leaked_block = general_chain & _AGENTX_PROVIDER_ENV_BLOCKLIST
    ...
    def test_extra_auth_vars_covered(self):
        ...
        assert extras.issubset(_AGENTX_PROVIDER_ENV_BLOCKLIST)
    ...
    def test_claude_code_oauth_token_is_inheritable(self):
        ...
        assert "CLAUDE_CODE_OAUTH_TOKEN" not in _AGENTX_PROVIDER_ENV_BLOCKLIST
    ...
    def test_optional_tool_and_messaging_vars_are_in_blocklist(self):
        ...
        for name, metadata in OPTIONAL_ENV_VARS.items():
            ...
            if category in {"tool", "messaging"}:
                assert name in _AGENTX_PROVIDER_ENV_BLOCKLIST, (
...
class TestSanePathIncludesHomebrew:
    ...
    def _disable_hermes_bin_injection(self):
        ...
        agentx-install-dir prepend (a separate concern, covered by
        ...
        local_mod._AGENTX_BIN_DIR = None  # resolved -> no dir to inject
        yield
        local_mod._AGENTX_BIN_DIR = saved
...
class TestHermesBinDirOnPath:
    ...
    def _reset_cache(self):
        from tools.environments import local as local_mod
        local_mod._AGENTX_BIN_DIR = local_mod._SENTINEL
    ...
    def test_prepend_noop_when_unresolved(self, monkeypatch):
        from tools.environments import local as local_mod
        self._reset_cache()
        local_mod._AGENTX_BIN_DIR = None
...
class TestHermesInternalDynamicSecrets:
    ...
    def test_gateway_relay_static_names_in_blocklist(self):
        ...
        assert "GATEWAY_RELAY_SECRET" in _AGENTX_PROVIDER_ENV_BLOCKLIST
        assert "GATEWAY_RELAY_DELIVERY_KEY" in _AGENTX_PROVIDER_ENV_BLOCKLIST
        assert "GATEWAY_RELAY_ID" in _AGENTX_PROVIDER_ENV_BLOCKLIST
