...
def _make_backend(session: _FakeSession):
    ...
    backend._session_id = "agentx-session"
...
def _browser_route(driver: _BrowserDriver, session_id: str = "agentx-a"):
...
def test_exact_browser_binding_injects_hermes_session_capability():
    ...
    route = _browser_route(driver, session_id="agentx-owned-session")
    ...
    assert driver.calls == [
        (
            "get_browser_state",
            {"pid": 101, "window_id": 202, "session": "agentx-owned-session"},
        )
    ]
...
def test_typed_browser_refs_do_not_cross_route_sessions():
    driver = _BrowserDriver()
    first = _browser_route(driver, session_id="agentx-a")
    second = _browser_route(driver, session_id="agentx-b")
