...
class TestXAIProviderIsAvailable:
    ...
    auth-store lock. It runs on every ``agentx tools`` repaint and at
...
class TestXAIBackendWiring:
    ...
    def test_is_backend_available_does_not_call_resolver(self, monkeypatch):
        ...
        dispatch and every `agentx tools` repaint. It must not invoke the
    ...
    def test_xai_not_in_legacy_backend_candidate_chain(self, monkeypatch):
        ...
        chain is for users who set env vars but never ran ``agentx tools``,
...
class TestXAIProviderOAuthPath:
    ...
    credential pool (``agentx auth`` sign-in) rather than an env-var key.
    ...
    def test_search_uses_oauth_bearer_token_and_base_url(self, monkeypatch, tmp_path):
        ...
        monkeypatch.delenv("XAI_API_KEY", raising=False)
        monkeypatch.setenv("AGENTX_HOME", str(tmp_path))
        monkeypatch.setenv("AGENTX_XAI_BASE_URL", "https://proxy.x.ai/v1/")
