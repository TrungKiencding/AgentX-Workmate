...
The marker at ``$INSTALL_DIR/.agentx-bootstrap-complete`` is what the desktop
...
def make_checkout(tmp_path):
    install_dir = tmp_path / "agentx-agent"
    install_dir.mkdir()
...
def test_marker_matches_the_schema_the_desktop_validates(tmp_path):
    ...
    marker = install_dir / ".agentx-bootstrap-complete"
...
def test_marker_publish_leaves_no_temp_sibling(tmp_path):
    ...
    assert (install_dir / ".agentx-bootstrap-complete").is_file()
