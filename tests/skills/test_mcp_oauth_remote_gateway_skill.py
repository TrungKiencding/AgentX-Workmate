...
SESSION_REVOKED / REFRESH_DEAD), the AGENTX_HOME resolution fallback, the
...
def _run_main(mod, tokens_dir, argv, responses):
    ...
    with patch.object(mod.os, "environ", dict(mod.os.environ, AGENTX_HOME=str(tokens_dir.parent))), \
...
def test_skill_md_frontmatter_invariants():
    ...
    assert fm["author"].split(",")[0].strip() != "AgentX Workmate"  # human credited first
