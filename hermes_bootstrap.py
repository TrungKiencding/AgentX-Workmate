"""Windows UTF-8 bootstrap for AgentX entry points.
...
should be imported at the very top of every AgentX entry point
(``agentx``, ``agentx-agent``, ``agentx-acp``, ``python -m gateway.run``,
...
def harden_import_path(src_root: str | None = None) -> None:
    """Stop a package in the current directory from shadowing AgentX modules.

    AgentX ships top-level modules with common names (``utils``, ``proxy``,
    ...
    We drop the relative forms outright, then force the real AgentX source root
    ...
    """
    root = src_root or os.environ.get("AGENTX_PYTHON_SRC_ROOT") or os.path.dirname(
...
def activate_durable_lazy_target() -> None:
    ...
    (``AGENTX_LAZY_INSTALL_TARGET``, e.g. ``/opt/data/lazy-packages``).
    ...
    """
    if not os.environ.get("AGENTX_LAZY_INSTALL_TARGET", "").strip():
        return
    try:
