#!/usr/bin/env python3
"""Google Workspace API CLI for AgentX Workmate.

Uses the Google Workspace CLI (`gws`) when available, but preserves the
existing AgentX-facing JSON contract and falls back to the Python client
...
def _gws_binary() -> str | None:
    override = os.getenv("AGENTX_GWS_BIN")
    if override:
        return override
...
def main():
    parser = argparse.ArgumentParser(description="Google Workspace API for AgentX Workmate")
