...
AgentX runs natively on Windows (PowerShell, cmd, Windows Terminal, git-bash
...
`agentx config edit` writes correctly.

### `execute_code` / Sandbox

**WinError 10106** from the sandbox child process — it can't create an
`AF_INET` socket. Root cause is usually AgentX's env scrubber dropping
...
AgentX-installed `venv/Scripts/` has no pip/pytest (stripped for size).
...
every AgentX tool and most Windows APIs. Prefer forward slashes in code
