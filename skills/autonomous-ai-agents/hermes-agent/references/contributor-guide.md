...
Config: `~/.agentx/config.yaml` (settings), `~/.agentx/.env` (API keys) — both under `$AGENTX_HOME` when it is set.
...
`_AGENTX_CORE_TOOLS` (every platform) or to a specific toolset.

All handlers must return JSON strings. Use `get_hermes_home()` for paths,
never hardcode `~/.agentx`. For custom/local-only tools, write a plugin in
`~/.agentx/plugins/` instead of editing core — see the developer docs.
...
- Tests auto-redirect `AGENTX_HOME` to temp dirs — never touch real `~/.agentx/`.
