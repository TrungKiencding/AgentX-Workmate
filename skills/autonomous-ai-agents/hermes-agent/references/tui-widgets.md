...
Author widget apps for the AgentX TUI (`agentx --tui`): glanceable ambient
...
- The TUI must be in use (`agentx --tui`). Widgets do not render in the
...
1. Use `write_file` to create `~/.agentx/tui-widgets/<name>.mjs` (see
...
- A thrown `register()` is logged and skipped; check
  `~/.agentx/logs/tui_gateway_crash.log` if a widget never appears.
