...
## Layer 1 — Is this thing an AgentX plugin, or a separate app?
...
|---|---|---|
| **OpenViking memory plugin** (`plugins/memory/openviking/`) | Code that runs **inside the AgentX process**. Its LLM calls go through AgentX's already-configured provider. | Already uses Portal if user's AgentX is configured for Portal. Nothing extra needed. `OPENVIKING_API_KEY` is the OpenViking *server's* own auth, not LLM auth. |
...
plugin that already runs inside AgentX. That LLM call is already authenticated
via AgentX's provider config. The plugin's own server auth (e.g.
...
What AgentX Workmate has that *feels* like OAuth — `agentx login --provider nous`
opening a browser, user signs in, token lands in `~/.agentx/auth.json` — is a
**AgentX-specific browser flow**. Under the hood it produces a credential
AgentX uses as a bearer. It is not a public OAuth provider that Karakeep et al.
...
1. Read AgentX's existing Portal credential out of `~/.agentx/auth.json`.
...
rides on the credential AgentX already holds.

Where this could live in AgentX:
...
- **Per-key rate limits** so a noisy app can be capped without eating the
  user's headroom for AgentX itself.
...
1. First decide: AgentX plugin (runs inside AgentX) or separate app? If plugin,
   it already uses Portal via AgentX's provider config — done.
...
4. Mixed setup ("Portal for some things, OpenRouter/Ollama Cloud for the
   AgentX agent itself") is fully supported. AgentX treats agent
...
pitch in the Tool Gateway announcement is specifically about AgentX Workmate's
*tools* (web search, browser, image gen, TTS) flowing through the Portal
subscription when AgentX is configured to use Portal as its provider. It is
