...
`agentx config check` reports sections missing from an older config.

### Toolsets

Enable/disable via `agentx tools` (interactive) or `agentx tools enable/disable NAME`.
Full enumeration: `TOOLSETS` dict in `toolsets.py` (`_AGENTX_CORE_TOOLS` is the default bundle most platforms inherit).
