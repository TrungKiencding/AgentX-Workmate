...
| nous | OAuth device code | `agentx auth add nous` (or `NOUS_API_KEY`) |
| openai-codex | OAuth | `agentx auth add openai-codex` |
| qwen-oauth | OAuth | `agentx auth add qwen-oauth` |
| minimax-oauth | OAuth | `agentx auth add minimax-oauth` |
...
Multiple credentials per provider pool and rotate automatically (`agentx auth`).
Fallback chain when the primary fails: `agentx fallback add|remove|list`.
...
#   agentx config set model.aliases.fav openrouter/anthropic/claude-sonnet-4.6
model:
  aliases:
