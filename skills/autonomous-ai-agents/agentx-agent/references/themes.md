# Themes / Skins — Author an AgentX Color Theme

Author an AgentX **skin** — one YAML file that themes the CLI, the TUI, and the
...
`~/.agentx/skins/` is the theme analogue of a plugin: no code, all surfaces. This
...
- The user asks for a custom look ("make me a synthwave theme", "dark forest
  vibes", "match my brand colors") for AgentX itself.
...
3. **Apply it yourself — never hand-edit `config.yaml`.** Run the safe writer via
   `terminal`:
   ```
   agentx config set display.skin <name>
   ```
...
4. **Confirm the new look landed** and tell the user how to revert: run
   `agentx config set display.skin default` (or they can `/skin default`).
...
```
agentx skin set <key> <hex>      # e.g. agentx skin set ui_tool "#00FFFF"
```
...
palette and resets the background. `agentx skin set background "#08201f"` changes
only the background; `agentx skin use <name>` / `agentx skin list` switch and
enumerate.

## Pitfalls

- **Don't hardcode `~/.agentx`** when a profile is active — resolve the real home
  from `$AGENTX_HOME` first, falling back to `~/.agentx`.
...
- **Never hand-edit `config.yaml` to activate.** Use `agentx config set
...
- **You apply it, not the user.** `agentx config set display.skin <name>` is
