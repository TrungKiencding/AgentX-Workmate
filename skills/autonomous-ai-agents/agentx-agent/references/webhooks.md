...
Create dynamic webhook subscriptions so external services (GitHub, GitLab, Stripe, CI/CD, IoT sensors, monitoring tools) can trigger AgentX agent runs by POSTing events to a URL.
...
All management is via the `agentx webhook` CLI command:

### Create a subscription
```bash
agentx webhook subscribe <name> \
...
Full filter syntax: https://github.com/TrungKiencding/AgentX-Workmate/blob/main/website/docs/user-guide/messaging/webhooks.md#payload-filters
...
```bash
agentx webhook remove <name>
```

### Test a subscription
```bash
agentx webhook test <name>
agentx webhook test <name> --payload '{"key": "value"}'
```
...
### GitHub: PR reviews
```bash
agentx webhook subscribe github-prs \
  --events "pull_request" \
...
```bash
agentx webhook subscribe ci-builds \
  --events "pipeline" \
...
```bash
agentx webhook subscribe alerts \
...
- Subscriptions persist to `~/.agentx/webhook_subscriptions.json`

## How It Works

1. `agentx webhook subscribe` writes to `~/.agentx/webhook_subscriptions.json`
...
1. **Is the gateway running?** Check with `systemctl --user status agentx-gateway` or `ps aux | grep gateway`
2. **Is the webhook server listening?** `curl http://localhost:8644/health` should return `{"status": "ok"}`
3. **Check gateway logs:** `grep webhook ~/.agentx/logs/gateway.log | tail -20`
4. **Signature mismatch?** Verify the secret in your service matches the one from `agentx webhook list`. GitHub sends `X-Hub-Signature-256`, GitLab sends `X-Gitlab-Token`.
5. **Firewall/NAT?** The webhook URL must be reachable from the service. For local development, use a tunnel (ngrok, cloudflared).
6. **Wrong event type?** Check `--events` filter matches what the service sends. Use `agentx webhook test <name>` to verify the route works.
