...
Common "why is AgentX doing X to my output / tool calls / commands?" toggles — and the exact commands to change them. Most of these need a fresh session (`/reset` in chat, or start a new `agentx` invocation) because they're read once at startup.
...
```bash
agentx config set security.redact_secrets true       # keep enabled globally
```
...
```bash
agentx config set privacy.redact_pii true    # enable
agentx config set privacy.redact_pii false   # disable (default)
```

### Command approval prompts

By default (`approvals.mode: smart`), AgentX asks an auxiliary LLM to assess shell commands flagged as destructive (`rm -rf`, `git reset --hard`, etc.). The modes are:
...
```bash
agentx config set approvals.mode smart       # recommended middle ground
agentx config set approvals.mode off         # bypass everything (not recommended)
```
...
### "Reset permissions" / "make AgentX ask again"
...
1. Shell-command allowlist: `agentx config set command_allowlist '[]'`
2. Shell-hook consent (only if present): `rm -f ~/.agentx/shell-hooks-allowlist.json`

Then sanity-check `agentx config get approvals.mode` (should not be `off`)
...
To keep the model away from network or media tools entirely, open `agentx tools` and toggle per-platform. Takes effect on next session (`/reset`). See `references/configuration.md` for the toolset list.
