...
**three** code paths in AgentX that cap a batch. If none of them fired, the
cap came from the **model itself** — not from AgentX — and the user's
narration of "the runtime caps at N" is the model rationalising its own
choice.

## The three real caps in AgentX
...
3. **Cost-warning** — same `_get_max_concurrent_children()`. When the
   ...
   log line** — it does not cap anything. Easy to mis-read as "AgentX is
   refusing my value."
...
agentx config get delegation.max_concurrent_children

# 2. Did AgentX' truncator or rejector actually fire?
grep -E "Truncated.*delegate_task|Too many tasks" ~/.agentx/logs/agent.log | tail
# If neither line appears, neither cap path executed.

# 3. Confirm the resolver returns what config says (in venv with agentx on path)
...
appears, **the cap is the model**, not AgentX.
