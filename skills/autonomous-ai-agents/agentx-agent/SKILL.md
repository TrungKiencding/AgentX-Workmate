...
- **Self-improving through skills** — AgentX learns from experience by saving reusable procedures as skills that load into future sessions.
...
- **Profiles** — run multiple independent AgentX instances with isolated configs, sessions, skills, and memory.
...
# Other surfaces
agentx desktop                 # launch the native desktop app (alias: agentx gui)
agentx dashboard               # web admin panel + embedded chat
agentx proxy                   # OpenAI-compatible local proxy backed by your OAuth provider
```
...
| MCP servers (add, catalog, `agentx mcp`) | `references/native-mcp.md` |
...
Two theming rules that hold even without loading the reference: **you apply skins yourself** (`agentx config set display.skin <name>` — every surface repaints live within ~a second; don't tell the user to run `/skin`), and **to tweak one color, edit the ACTIVE skin** (`agentx skin set <key> <hex>`) — never fork `default`, which drops the palette and resets the background.

## Spawning Additional AgentX Instances

Run additional AgentX processes as fully independent subprocesses — separate sessions, tools, and environments.

### When to Use This vs delegate_task

| | `delegate_task` | Spawning `agentx` process |
|-|-----------------|--------------------------|
...
AgentX uses prompt_toolkit, which requires a real terminal. Use tmux for interactive spawning:
...
- **Use `agentx chat -q` for fire-and-forget** — no PTY needed
- **Use tmux for interactive sessions** — raw PTY mode has `\r` vs `\n` issues with prompt_toolkit
- **For scheduled tasks**, use the `cronjob` tool instead of spawning — handles delivery and retry
- **"delegate_task is capped at N" reports** — see `references/delegate-task-concurrency-diagnosis.md`. Three real cap paths in AgentX; if none fired, the model is self-limiting and rationalising it as "the runtime caps."
...
- **Desktop app** (`agentx desktop` / `agentx gui`) — native Electron app for macOS/Linux/Windows: streaming chat, session list, Cmd+K palette, drag-and-drop files, native notifications, per-profile remote-gateway login. Extend it with UI plugins — `references/desktop-plugins.md`.
- **Web dashboard** (`agentx dashboard`) — full admin panel: messaging channels, MCP catalog, webhooks, memory, profile builder, plus an embedded `agentx --tui` chat. Secured behind an OAuth/token gate.
- **Ink TUI** (`agentx --tui` or `display.interface: tui`) — terminal UI with docked widget apps — `references/tui-widgets.md`.
- **OpenAI-compatible proxy** (`agentx proxy`) — a local OpenAI API backed by whichever OAuth provider you're signed into. Point Codex CLI, Aider, Cline, or any script at it — no API key.
...
- **Never hand-edit `config.yaml` for the user** — use `agentx config set KEY VAL`; a stray indent can corrupt the file and break the live gateway.
