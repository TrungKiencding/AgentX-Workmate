...
AGENTX_HOME_DIR="${AGENTX_HOME:-$HOME/.agentx}"
AGENTX_CFG="${AGENTX_HOME_DIR}/config.yaml"
MCP_PORT=40404
...
if [[ ! -f "$AGENTX_CFG" ]]; then
    echo -e " ${FAIL} Hermes config not found at ${AGENTX_CFG}"
    manual_steps+=("Create ${AGENTX_CFG} with twozero_td MCP server entry")
elif grep -q 'twozero_td' "$AGENTX_CFG" 2>/dev/null; then
...
" 2>/dev/null && echo -e " ${OK} twozero_td MCP entry added to config" \
              || { echo -e " ${FAIL} Could not update config (is PyYAML installed?)"; \
                   manual_steps+=("Add twozero_td MCP entry to ${AGENTX_CFG} manually"); }
