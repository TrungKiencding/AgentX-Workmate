---
name: agentx-agent-skill-authoring
...
# Authoring AgentX-Agent Skills (in-repo)
...
2. **Choose the right context load.** A model-invoked AgentX skill pays for its description every turn. Keep descriptions focused on trigger classes and the skill's distinctive behavior. Put details in the body or linked references.
...
- AgentX-specific recipes (tests via scripts/run_tests.sh, ui-tui paths, etc.)
...
`metadata.hermes.related_skills` unions both trees (`skills/` in-repo and `~/.agentx/skills/`) at load time. You CAN reference a user-local skill from an in-repo skill, but it won't resolve for other users who clone the repo fresh. Prefer referencing only in-repo skills from in-repo skills. If a frequently-referenced skill lives only in `~/.agentx/skills/`, consider promoting it to the repo.
...
1. **Using `skill_manage(action='create')` for an in-repo skill.** It writes to `~/.agentx/skills/`, not the repo tree. Use `write_file` for in-repo creation.
...
3. **Description too generic or trigger buried past char 57.** The system prompt
   ...
   - Good: `Use when debugging AgentX skill discovery failures.`
   - Bad: `This skill contains detailed guidance for agents working on AgentX skill discovery failures.`
...
- [ ] File is at `skills/<category>/<name>/SKILL.md` (not in `~/.agentx/skills/`)
