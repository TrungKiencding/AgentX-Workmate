# OpenAI-Compatible API Server for AgentX Workmate
...
SSE streaming. By exposing this endpoint, agentx-agent becomes instantly
...
│  AnythingLLM     │  │    {"messages": [...]}             │  agentx-agent   │
│  NextChat        │  │                                    │  gateway        │
...
A user would:
1. Set `API_SERVER_ENABLED=true` in `~/.agentx/.env`
2. Run `agentx gateway` (API server starts alongside Telegram/Discord/etc.)
3. Point Open WebUI (or any frontend) at `http://localhost:8642/v1`
4. Chat with agentx-agent through any OpenAI-compatible UI
...
| GET | `/v1/models` | List available "models" (returns agentx-agent as a model) |
...
Authorization: Bearer agentx-api-key-here
...
But agentx-agent sessions have persistent state (memory, skills, tool context).
...
Frontends send `"model": "agentx-agent"` (or whatever). The actual LLM model
used is configured server-side in config.yaml. The API server maps any
requested model name to the configured agentx-agent model.
...
1. `gateway/platforms/api_server.py` — new adapter
   ...
   - Frontend system prompt layered on top of agentx-agent's core prompt
...
Once implemented, agentx-agent works as a drop-in backend for:
