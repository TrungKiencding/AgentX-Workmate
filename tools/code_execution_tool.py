...
Lets the LLM write a Python script that calls AgentX tools via RPC,
...
# available on every platform AgentX itself runs on.
...
# match a safe prefix, the operational AGENTX_ allowlist, or (on Windows) an
# OS-essential name.  Delegate-task child context is also an exact-name
# operational marker: without it, a sandbox script that spawns/imports AgentX
# code can lose the DB-layer Kanban mutation guard while still inheriting
# AGENTX_HOME.
#
# NB: the broad ENV_PREFIX match was deliberately removed (#27303) — it leaked
# branded config that lacks a secret substring (e.g. AGENTX_BASE_URL,
# AGENTX_KANBAN_DB, AGENTX_*_WEBHOOK).  The child only needs the few
# location/profile vars in _AGENTX_CHILD_ALLOWED below; AGENTX_RPC_SOCKET /
# AGENTX_RPC_DIR / TZ / HOME are injected explicitly after scrubbing.
...
# Operational AGENTX_* vars the child legitimately needs by exact name — these
...
_AGENTX_CHILD_ALLOWED = frozenset({
    "AGENTX_HOME",
    "AGENTX_PROFILE",
    "AGENTX_CONFIG",
    "AGENTX_ENV",
    "AGENTX_DELEGATED_CHILD_CONTEXT",
})
...
def _scrub_child_env(source_env, is_passthrough=None, is_windows=None):
    ...
    Rules (order matters):
      ...
      4. Operational AGENTX_* vars (_AGENTX_CHILD_ALLOWED) pass by exact name.
    ...
    scrubbed = {}
    # Non-secret AGENTX_* vars dropped by the tightened allowlist (#27303). The
    # broad prefix used to pass these through; now only the operational set
    # does. The drop is intentional (those vars can carry config like
    # AGENTX_KANBAN_DB / AGENTX_BASE_URL), but a sandbox script that imports a
    # repo module reading one at import time would otherwise see it silently
    # unset. Surface the drop once so the behavior change is diagnosable and
    # points at the env_passthrough opt-in escape hatch.
    _dropped_branded = []
    for k, v in source_env.items():
        ...
        if k.startswith(ENV_PREFIX):
            ...
            # allowlist — a deliberately-dropped AGENTX_* var.
            _dropped_branded.append(k)
    if _dropped_branded:
        logger.debug(
            "execute_code: dropped %d non-allowlisted %s* var(s) from the "
            ...
            len(_dropped_branded),
            ENV_PREFIX,
            ", ".join(sorted(_dropped_branded)),
        )
...
_TOOL_STUBS = {
    ...
    "write_file": (
        ...
        '"""Write content to a file (always overwrites). Returns dict with status. cross_profile=True opts out of the cross-AgentX-profile soft guard."""',
    ...
    "patch": (
        ...
        '"""Targeted find-and-replace (mode="replace") or V4A multi-file patches (mode="patch"). Returns dict with status. cross_profile=True opts out of the cross-AgentX-profile soft guard."""',
...
"""Auto-generated AgentX tools RPC stubs."""
...
def _connect():
    ...
    AGENTX_RPC_SOCKET can be either:
    ...
    global _sock
    if _sock is None:
        endpoint = os.environ["AGENTX_RPC_SOCKET"]
...
def _call(tool_name, args):
    ...
    request = json.dumps({
        "tool": tool_name,
        "args": args,
        "token": os.environ.get("AGENTX_RPC_TOKEN", ""),
    }) + "\\n"
    with _call_lock:
        conn = _connect()
...
"""Auto-generated AgentX tools RPC stubs (file-based transport)."""
import json, os, shlex, tempfile, threading, time

_RPC_DIR = os.environ.get("AGENTX_RPC_DIR") or os.path.join(tempfile.gettempdir(), "hermes_rpc")
_seq = 0
...
def _execute_remote(
    ...
    try:
        ...
        env_prefix = (
            f"AGENTX_RPC_DIR={shlex.quote(f'{sandbox_dir}/rpc')} "
            f"AGENTX_RPC_TOKEN={shlex.quote(rpc_token)} "
            f"PYTHONDONTWRITEBYTECODE=1"
        )
        tz = os.getenv("AGENTX_TIMEZONE", "").strip()
        if tz:
...
def execute_code(
    ...
    # selector from AGENTX_RPC_SOCKET (path vs. ``tcp://host:port``).
    ...
    try:
        ...
        #   connect.  AGENTX_RPC_SOCKET is set to ``tcp://127.0.0.1:<port>``
        ...
        child_env["AGENTX_RPC_SOCKET"] = rpc_endpoint
        child_env["AGENTX_RPC_TOKEN"] = rpc_token
        ...
        # Ensure the agentx-agent root is importable in the sandbox so
        ...
        # AGENTX_TIMEZONE is an internal AgentX setting and must not leak
        # into child processes.
        _tz_name = os.getenv("AGENTX_TIMEZONE", "").strip()
        if _tz_name:
            child_env["TZ"] = _tz_name
        child_env.pop("AGENTX_TIMEZONE", None)
        ...
        # but scripts can still read secrets from disk (e.g. open('~/.agentx/.env')).
...
def _load_config() -> dict:
    ...
    ``agentx --tui`` where it is never used.  Read the lightweight raw config
...
def _get_execution_mode() -> str:
    ...
    Mode semantics:
      ...
      - ``strict``: scripts run in an isolated temp directory with
        ``sys.executable`` (agentx-agent's python). Reproducible and the
...
def build_execute_code_schema(enabled_sandbox_tools: set = None,
    ...
    When tools are disabled via ``agentx tools`` (e.g. web is turned off),
    ...
    # temp-dir staging and agentx-agent's own python.
    if mode == "strict":
        cwd_note = (
            "Scripts run in their own temp dir, not the session's CWD — use absolute paths "
            "(os.path.expanduser('~/.agentx/.env')) or terminal()/read_file() for user files."
        )
    else:
        cwd_note = (
    ...
    description = (
        "Run a Python script that calls AgentX tools programmatically. "
