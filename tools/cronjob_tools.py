"""
Cron job management tools for AgentX Workmate.
...
# uses 30s) — comfortably below the 1800s default AGENTX_AGENT_TIMEOUT.
_CRON_RUN_HEARTBEAT_INTERVAL = 10.0

# Hard ceiling on how long the heartbeat keeps the parent watchdog at bay.
# The child cron run has its own inactivity watchdog (AGENTX_CRON_TIMEOUT,
# default 600s) that bounds a wedged job, but with AGENTX_CRON_TIMEOUT=0
...
#      saying "cat ~/.agentx/.env" or "rm -rf /". `_scan_cron_prompt()` runs
...
#      `agentx-agent-dev` skill contains a security postmortem mentioning
#      `cat ~/.agentx/.env`, which tripped `read_secrets` and silently
...
def _origin_from_env() -> Optional[Dict[str, str]]:
    ...
    origin_platform = get_session_env("AGENTX_SESSION_PLATFORM")
    origin_chat_id = get_session_env("AGENTX_SESSION_CHAT_ID")
    if origin_platform and origin_chat_id:
        ...
        return {
            ...
            "chat_name": get_session_env("AGENTX_SESSION_CHAT_NAME") or None,
            "thread_id": thread_id,
            ...
            # send_message, which passes AGENTX_SESSION_USER_ID to
...
def _local_delivery_notice(job: Dict[str, Any], user_deliver: Optional[str]) -> Optional[str]:
    ...
    ``AGENTX_SESSION_PLATFORM``/``CHAT_ID`` is set for them), so a
...
def _validate_cron_script_path(script: Optional[str]) -> Optional[str]:
    ...
    Scripts must be relative paths that resolve within AGENTX_HOME/scripts/.
    ...
    # Only relative paths within ~/.agentx/scripts/ are allowed.
    if raw.startswith(("/", "~")) or (len(raw) >= 2 and raw[1] == ":"):
        return (
            f"Script path must be relative to ~/.agentx/scripts/. "
            f"Got absolute or home-relative path: {raw!r}. "
            f"Place scripts in ~/.agentx/scripts/ and use just the filename."
        )
...
def check_cronjob_requirements() -> bool:
    ...
    return (
        env_var_enabled("AGENTX_INTERACTIVE")
        or env_var_enabled("AGENTX_GATEWAY_SESSION")
        or env_var_enabled("AGENTX_EXEC_ASK")
    )
...
registry.register(
    ...
    handler=lambda args, **kw: cronjob(
        ...
        # `agentx cron create/edit --model`, or hand-edited jobs). The agent
