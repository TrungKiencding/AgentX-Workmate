...
Uses the Vercel Python SDK to run commands in cloud sandboxes through AgentX'
shared ``BaseEnvironment`` shell contract. When persistence is enabled, the
backend stores task-scoped snapshot metadata under ``AGENTX_HOME`` and restores
...
def _ensure_vercel_sdk() -> None:
    ...
    # (vercel-internal-telemetry posts to telemetry.vercel.com). AgentX
...
class VercelSandboxEnvironment(BaseEnvironment):
    ...
    def _configure_attached_sandbox(self, *, requested_cwd: str) -> None:
        ...
        if self._remote_home == "/":
            container_base = "/.agentx"
        else:
            container_base = f"{self._remote_home.rstrip('/')}/.agentx"
    ...
    def _vercel_bulk_download(self, dest_tar_path: Path) -> None:
        remote_hermes = (
            "/.agentx"
            if self._remote_home == "/"
            else f"{self._remote_home.rstrip('/')}/.agentx"
        )
