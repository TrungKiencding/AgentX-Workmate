"""Base class for all AgentX execution environment backends.
...
# AGENTX_DEBUG_INTERRUPT=1 to log loop entry/exit, periodic heartbeats, and
# every is_interrupted() state change from _wait_for_process.  Off by default
# to avoid flooding production gateway logs.
_DEBUG_INTERRUPT = bool(os.getenv("AGENTX_DEBUG_INTERRUPT"))
...
def get_sandbox_dir() -> Path:
    ...
    Configurable via TERMINAL_SANDBOX_DIR. Defaults to {AGENTX_HOME}/sandboxes/.
    """
...
def _cwd_marker(session_id: str) -> str:
    return f"__AGENTX_CWD_{session_id}__"
...
# the FIRST session's AGENTX_SESSION_ID into the snapshot makes every LATER
...
# AGENTX_SESSION_ID leak via the shared snapshot). Stripping them from the
...
# set), not AgentX' per-turn session identity.
#
# Kept in sync with gateway.session_context._VAR_MAP: every bridged name starts
# with one of these prefixes (or is AGENTX_UI_SESSION_ID). Used by unit tests
# as the Python-side contract for the exclusion set; the dump path unsets by
# name/prefix instead of grepping declare lines (see below / issue #71296).
_SNAPSHOT_EXCLUDED_ENV_REGEX = (
    "^declare -x (AGENTX_SESSION_|AGENTX_UI_SESSION_ID|AGENTX_CRON_AUTO_DELIVER_|AGENTX_CRON_SESSION)"
)
...
def _export_dump_excluding_session_vars(
    ...
    Matrix room/display name via ``AGENTX_SESSION_CHAT_NAME``) land in the
    ...
    return (
        "{ ( "
        "unset ${!AGENTX_SESSION_*} ${!AGENTX_CRON_AUTO_DELIVER_*} "
        f"AGENTX_UI_SESSION_ID{extra_unset} 2>/dev/null; "
        "export -p; "
        ") || true; } "
        f"> {tmp_path}"
    )
...
class BaseEnvironment(ABC):
    """Common interface and unified execution flow for all AgentX backends.
    ...
    def __init__(self, cwd: str, timeout: int, env: dict = None):
        ...
        self._snapshot_path = f"{temp_dir}/agentx-snap-{self._session_id}.sh"
        self._cwd_file = f"{temp_dir}/agentx-cwd-{self._session_id}.txt"
    ...
    def _wrap_command(self, command: str, cwd: str) -> str:
        ...
        for name in passthrough_names:
            marker = f"_AGENTX_RUNTIME_PASSTHROUGH_{name}"
        ...
        # Restrict AgentX metadata files without changing the user's command
    ...
    def _embed_stdin_heredoc(command: str, stdin_data: str) -> str:
        ...
        delimiter = f"AGENTX_STDIN_{uuid.uuid4().hex[:12]}"
    ...
    def _wait_for_process(
        ...
        # --- Debug tracing (opt-in via AGENTX_DEBUG_INTERRUPT=1) -------------
    ...
    def _extract_cwd_from_output(self, result: dict):
        """Parse the __AGENTX_CWD_{session}__ marker from stdout output.
