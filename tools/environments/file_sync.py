...
_FORCE_SYNC_ENV = "AGENTX_FORCE_FILE_SYNC"
...
def iter_sync_files(container_base: str = "/root/.agentx") -> list[tuple[str, str]]:
    ...
    the hardcoded /root/.agentx to *container_base* because the remote
...
class FileSyncManager:
    ...
    def sync(self, *, force: bool = False) -> None:
        ...
        or ``AGENTX_FORCE_FILE_SYNC=1`` is set.
    ...
    def sync_back(self, hermes_home: Path | None = None) -> None:
        ...
        Downloads the remote ``.agentx/`` directory as a tar archive,
    ...
    def _sync_back_transaction(self, hermes_home: Path | None = None) -> None:
        ...
        # against an uninitialized remote .agentx/ directory.
    ...
    def _sync_back_impl(self) -> None:
        ...
        with tempfile.NamedTemporaryFile(suffix=".tar") as tf:
            ...
            with tempfile.TemporaryDirectory(prefix="agentx-sync-back-") as staging:
    ...
    def _infer_host_path(self, remote_path: str,
        ...
        For example, if the mapping has ``/root/.agentx/skills/a.md`` →
        ``~/.agentx/skills/a.md``, a new remote file at
        ``/root/.agentx/skills/b.md`` maps to ``~/.agentx/skills/b.md``.
        """
        mapping = file_mapping if file_mapping is not None else []
