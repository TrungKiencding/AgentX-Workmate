...
etc.) surface as specific blocked checks via `agentx computer-use doctor`
...
# fetches the latest release, so a `AGENTX_CUA_DRIVER_VERSION` env var would
# only have *looked* like it pinned. For a reproducible version, point
# `AGENTX_CUA_DRIVER_CMD` at a specific binary instead.

_CUA_DRIVER_CMD_ENV = "AGENTX_CUA_DRIVER_CMD"
...
def _cua_no_overlay() -> bool:
    """True when AgentX should pass ``--no-overlay`` to cua-driver.
...
def _cua_telemetry_disabled() -> bool:
    """True when AgentX should disable cua-driver telemetry for this user.
...
def _wsl_windows_path_to_posix(path: str) -> str:
    """Translate a Windows absolute manifest command when AgentX runs in WSL.

    Windows cua-driver manifests can report ``C:\\Users\\...\\cua-driver.exe``
    even though the AgentX process uses POSIX subprocess spawning inside WSL.
...
class _EmbeddedCuaDaemon:
    ...
    machine-wide daemon would therefore let one AgentX session's YOLO choice
    ...
    def __init__(self, driver_cmd: str, permission_mode: str) -> None:
        ...
        if sys.platform == "win32":
            self.socket_path = rf"\\.\pipe\agentx-cua-{token}"
        else:
...
def _resolve_mcp_invocation(
    ...
    *,
    ...
    # A Windows-installed cua-driver can hand a WSL-hosted AgentX an absolute
...
def _candidate_cua_driver_commands(override: Optional[str] = None) -> List[str]:
    ...
    ``AGENTX_CUA_DRIVER_CMD`` is authoritative; only when neither is set do we
    ...
    AgentX Workmate Desktop/TUI session can otherwise filter out the `computer_use`
    tool even though `agentx computer-use doctor` succeeds from a login shell.
    """
...
def resolve_cua_driver_cmd(override: Optional[str] = None) -> Optional[str]:
    ...
    A supplied override (or ``AGENTX_CUA_DRIVER_CMD``) is never silently
...
def cua_driver_update_nudge() -> Optional[str]:
    ...
    return (
        ...
        f"update with `agentx computer-use install --upgrade`."
    )
...
def cua_driver_install_hint() -> str:
    ...
    return (
        ...
        f"{installer}\n"
        "Or run `agentx tools` and enable the Computer Use toolset to install it automatically."
    )
    ...
    async def _lifecycle_coro(self) -> None:
        ...
        try:
            ...
            params = StdioServerParameters(
                ...
                # sanitize AgentX-managed secrets out of the child env.
    ...
    def _start_lifecycle_locked(self) -> None:
        ...
        if not self._ready_event.wait(timeout=30.0):
            ...
            raise RuntimeError(
                ...
                "Run `agentx computer-use doctor` and check "
...
class CuaDriverBackend(ComputerUseBackend):
    ...
    def __init__(self, permission_mode: str = "standard") -> None:
        ...
        #   - Gets a distinct agent-cursor color per AgentX run, with
        ...
        #     concurrent AgentX runs / subagents don't step on each
        #     other.
        # We mint a UUID4-based id once per CuaDriverBackend instance —
        # one AgentX run = one backend = one session — and pass it as
        ...
        self._session_id: str = f"agentx-{uuid.uuid4().hex[:12]}"
    ...
    def start(self) -> None:
        ...
        # `computer-use` / `mcp` extras), not part of AgentX' minimal core.
    ...
    def capture(
        ...
        Maps agentx `capture(mode, app)` → cua-driver `list_windows` +
    ...
    def focus_app(self, app: str, raise_window: bool = False) -> ActionResult:
        ...
        separately approved by the AgentX adapter, and uses cua-driver's
