...
`health_report` (surfaced by `agentx computer-use doctor`) reports the
...
# regardless of approval level (e.g. logout kills the session AgentX runs in).
_BLOCKED_KEY_COMBOS = {
...
# Per-AgentX-session cached backends. Each backend owns its own cua-driver
...
def _cua_permission_mode(session_id: str) -> str:
    """Map AgentX's explicit approval bypass onto Cua's immutable mode.

    AgentX has TWO session-identity namespaces: the tool-dispatch path passes
        ...
        with _backend_lock:
            ...
            if cached is not None:
                ...
                backend_name = os.environ.get(
                    "AGENTX_COMPUTER_USE_BACKEND", "cua"
                ).lower()
                if backend_name in {"cua", "cua-driver", ""}:
                    ...
                    raise RuntimeError(
                        f"Unknown AGENTX_COMPUTER_USE_BACKEND={backend_name!r}"
                    )
...
def release_computer_use_session(session_id: str) -> bool:
    ...
    AgentX YOLO selected unrestricted mode. Approval state is cleared even
    ...
    try:
        ...
        # while waiting: unrelated AgentX sessions remain independent.
...
def _shutdown_backend_atexit() -> None:
    ...
    without this a driver can survive the AgentX process that spawned it
...
def handle_computer_use(args: Dict[str, Any], **kwargs) -> Any:
    ...
    try:
        ...
        return json.dumps({
            ...
            "hint": "If the cua-driver binary is missing, run `agentx computer-use install`. "
...
def _dispatch(backend: ComputerUseBackend, action: str, args: Dict[str, Any]) -> Any:
    ...
    # none of those capabilities can be supplied across AgentX sessions.
...
def _route_capture_through_aux_vision(
    ...
    The captured base64 PNG is materialised to ``$AGENTX_HOME/cache/vision/``
...
def check_computer_use_requirements() -> bool:
    ...
    `agentx computer-use doctor` if their session is incomplete (e.g. no
    DISPLAY set).
    """
