    ...
    mcp_servers:
      my_server:
        ...
        oauth:                                  # all fields optional
          ...
          client_name: "My Custom Client"       # default: "AgentX Workmate"
"""
...
def _get_token_dir(hermes_home: str | Path | None = None) -> Path:
    ...
    Uses AGENTX_HOME so each profile gets its own OAuth tokens.
    Layout: ``AGENTX_HOME/mcp-tokens/``
    """
...
def _cached_redirect_port(storage: "HermesTokenStorage | None") -> int | None:
    ...
    redirect URI that was registered with it. If AgentX restarts and chooses a
...
def _raise_if_non_interactive(lead: str) -> None:
    ...
    shared, actionable ``agentx mcp login`` next-step so the guidance wording
    lives in one place across every non-interactive OAuth boundary (#57836).
    """
    if not _is_interactive():
        raise OAuthNonInteractiveError(
            f"{lead} "
            "Run `agentx mcp login <server>` interactively to (re)authorize, "
...
class HermesTokenStorage:
    ...
    File layout::
        ...
        AGENTX_HOME/mcp-tokens/<server_name>.json         -- tokens
        AGENTX_HOME/mcp-tokens/<server_name>.client.json   -- client info
        AGENTX_HOME/mcp-tokens/<server_name>.meta.json     -- oauth server metadata
    """
    ...
    async def get_tokens(self) -> "OAuthToken | None":
        ...
        if OAuthToken is None and not _ensure_sdk_loaded():
            return None
        # AgentX records an absolute wall-clock ``expires_at`` alongside the
    ...
    class _Handler(BaseHTTPRequestHandler):
        def do_GET(self) -> None:  # noqa: N802
            ...
            body = (
                ...
                "<p>You can close this tab and return to AgentX.</p></body></html>"
            ) if code else (
    ...
    async def _redirect_handler(authorization_url: str) -> None:
        ...
        if on_ssh and redirect_uri:
            ...
            print(
                ...
                f"\n"
                f"  See: https://github.com/TrungKiencding/AgentX-Workmate/blob/main/website/docs/guides/oauth-over-ssh.md\n",
                file=sys.stderr,
            )
...
def _paste_callback_reader(result: dict) -> None:
    ...
    if line.lower() in _SKIP_TOKENS:
        ...
        print(
            "  OAuth skipped. Run `agentx mcp login <server>` later to "
...
#   "Claude Code" → 200
#   "Codex"       → 200
#   "AgentX Workmate" / "AgentX" / "Cursor" / "VS Code" / … → 403
...
def _build_client_metadata(cfg: dict) -> "OAuthClientMetadata":
    ...
    if OAuthClientMetadata is None:
        _ensure_sdk_loaded()
    client_name = cfg.get("client_name", "AgentX Workmate")
    scope = cfg.get("scope")
    ...
    metadata_kwargs: dict[str, Any] = {
        ...
        # rejecting loopback redirect_uris. AgentX is a CLI/desktop app
...
def _invalidate_tokens_on_client_change(
    ...
    if removed:
        logger.warning(
            ...
            "Re-authorize with: agentx mcp login %s",
...
def humanize_oauth_registration_error(
    ...
    *,
    ...
    Figma's remote MCP gates DCR on exact ``client_name``. AgentX auto-sets
    ``Claude Code`` (known-good); this message fires when the user overrode
    that with something Figma still rejects, or an older AgentX is running.
    """
    msg = str(exc)
    lowered = msg.lower()
...
def build_oauth_auth(
    ...
    if not _is_interactive() and not storage.has_cached_tokens():
        raise OAuthNonInteractiveError(
            ...
            f"`agentx mcp login {server_name}` interactively first to complete "
