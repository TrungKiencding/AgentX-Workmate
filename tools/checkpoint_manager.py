...
-----------------------------------------------------------------------------
    ...
    ~/.agentx/checkpoints/
        store/                          — single bare-ish git repo
            ...
            refs/agentx/<hash16>        — per-project branch tip
...
DEFAULT_EXCLUDES = [
    ...
    ".hg/",
    ".svn/",
    # Worktrees (AgentX convention — don't recursively snapshot siblings)
    ".worktrees/",
...
_GIT_TIMEOUT: int = max(10, min(60, env_int("AGENTX_CHECKPOINT_TIMEOUT", 30)))
...
def _git_env(
    ...
    The shared store is internal AgentX infrastructure — it must NOT inherit
...
def _migrate_legacy_store(base: Path) -> Optional[Path]:
    ...
    cleared with ``agentx checkpoints clear-legacy``.
    ...
    if legacy_root is not None:
        logger.info(
            ...
            "Clear with `agentx checkpoints clear-legacy` when safe.",
            legacy_root,
        )
    return legacy_root
...
def _init_store(store: Path, working_dir: str) -> Optional[str]:
    ...
    _run_git(["config", "user.name", "AgentX Checkpoint"], store, cfg_wd)
...
def _pre_v2_shadow_repos(base: Path) -> List[Dict]:
    ...
    for child in base.iterdir():
        ...
        wd_marker = child / "AGENTX_WORKDIR"
        if wd_marker.exists():
            try:
...
# look for ``HEAD``/``info/exclude``/``AGENTX_WORKDIR``.  In v2 we also write
...
def _init_shadow_repo(shadow_repo: Path, working_dir: str) -> Optional[str]:
    ...
    # Compat marker for tests that look at AGENTX_WORKDIR
    # (write in addition to the JSON metadata).
    try:
        (shadow_repo / "AGENTX_WORKDIR").write_text(
...
class CheckpointManager:
    ...
    def session_diff(self, working_dir: str) -> Dict:
        ...
        This powers ``/diff session``.  It answers "what has AgentX changed
        ...
        *retained* checkpoint may predate the current session (or, after
        ...
        AgentX changed", not an exact per-session ledger.
...
# ---------------------------------------------------------------------------
# Public helpers for `agentx checkpoints` CLI
# ---------------------------------------------------------------------------
