...
def _make_hermes_provider_class() -> Optional[type]:
    ...
    class HermesMCPOAuthProvider(OAuthClientProvider):
        ...
        async def _prefetch_oauth_metadata(self) -> None:
            ...
            """
            # The SDK's httpx flavour, not AgentX's — mcp 2.0 builds on httpx2,
        ...
        async def _maybe_flag_poisoned_client(self, response: Any) -> None:
            ...
            and is handled by ``agentx mcp reauth``.
...
_AGENTX_PROVIDER_CLS: Optional[type] = _make_hermes_provider_class()
...
class MCPOAuthManager:
    ...
    def _build_provider(
        ...
        Returns None if the MCP SDK's OAuth support is unavailable.
        """
        if _AGENTX_PROVIDER_CLS is None:
            logger.warning(
        ...
        if (
            ...
            raise OAuthNonInteractiveError(
                ...
                "cached tokens found. Run `agentx mcp login "
    ...
    def remove(
        ...
        *,
        ...
        Called by ``agentx mcp remove <name>`` and (indirectly) by
        ``agentx mcp login <name>`` during forced re-auth.
        """
        with self._entries_lock:
