...
def _is_hermes_provider_credential(name: str) -> bool:
    """True if ``name`` is a AgentX-managed provider credential (API key,
    ...
    let a skill tunnel a AgentX credential into the execute_code child.
    """
    try:
...
def register_env_passthrough(var_names: Iterable[str]) -> None:
    ...
    GHSA-rhgp-j443-p4rf. A skill that needs to talk to a AgentX-managed
