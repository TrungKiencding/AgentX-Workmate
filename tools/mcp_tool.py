...
# per-profile log file (~/.agentx/logs/mcp-stderr.log), tagged with the
...
def _check_logging_callback_support() -> bool:
    ...
    server emits, so server-side diagnostics never reach AgentX' logs.
    """
    if not _MCP_AVAILABLE:
        return False
    try:
...
# Recomputed by _ensure_mcp_sdk() once ClientSession is bound; at module
# level ClientSession is still None, so the probe could only ever say False.
...
def _build_safe_env(user_env: Optional[dict]) -> dict:
    ...
    AgentX explicitly tagged during dotenv loading, plus any variables
    ...
    an exception: users configured that backend specifically so AgentX and
...
# ---------------------------------------------------------------------------
# MCP ImageContent block → AgentX MEDIA tag
# ---------------------------------------------------------------------------
...
def _cache_mcp_image_block(block) -> str:
    ...
    return a ``MEDIA:<path>`` tag that AgentX gateways know how to render.
...
def _render_mcp_resource_block(block, server_name: str = "") -> str:
    ...
    - ``EmbeddedResource`` with blob contents → bytes are decoded (size-capped)
      and materialized into the AgentX document cache; returns a marker with
...
class ElicitationHandler:
    ...
    Form-mode elicitations are routed through AgentX' existing approval
    ...
    async def __call__(self, context, params):
        ...
        # the agent's contextvars (AGENTX_SESSION_PLATFORM etc.). When
...
class MCPServerTask:
    ...
    def __init__(self, name: str):
        ...
        # context doesn't inherit AGENTX_SESSION_PLATFORM, so the
    ...
    def _make_logging_callback(self):
        ...
        server into AgentX' logging (agent.log via hermes_logging), tagged
    ...
    async def _run_stdio(self, config: dict):
        ...
        # ungraceful exit of this AgentX process (kill -9, crash, force-quit)
    ...
    async def run(self, config: dict):
        ...
        # routes those requests through AgentX' approval system.
        ...
        while True:
            try:
                ...
                # and the MCP server would stay dead until AgentX is fully
...
def _handle_auth_error_and_retry(
    ...
    return tool_error(
        ...
        f"Run `agentx mcp login {server_name}` (or delete the tokens "
        f"file under ~/.agentx/mcp-tokens/ and restart). Do NOT retry "
...
# ---------------------------------------------------------------------------
# Advisory file lock that prevents N concurrent AgentX processes (e.g.
...
def _wrap_with_home_override(coro: "Coroutine") -> "Coroutine":
    """Carry the caller's context-local AGENTX_HOME override into ``coro``.
...
def _run_on_mcp_loop(coro_or_factory, timeout: float = 30):
    ...
    # Propagate the context-local AGENTX_HOME override onto the MCP loop.
...
def _load_mcp_config() -> Dict[str, dict]:
    """Read ``mcp_servers`` from the AgentX config file.
    ...
    ``os.environ`` (which includes ``~/.agentx/.env`` loaded at startup).
    """
    try:
        from hermes_cli.config import load_config
        from utils import env_var_enabled as _env_enabled

        if _env_enabled("AGENTX_SAFE_MODE"):
            return {}
        config = load_config()
...
def _make_tool_handler(server_name: str, tool_name: str, tool_timeout: float):
    ...
    def _handler(args: dict, **kwargs) -> str:
        ...
        async def _call():
            ...
            # flow through AgentX' MEDIA: tag convention and out to messaging
            ...
            # AgentX' MEDIA tag + cache_image_from_bytes) was the cleaner of
...
def sanitize_mcp_name_component(value: str) -> str:
    ...
    Preserves AgentX's historical behavior of converting hyphens to
...
# Native MCP tool-name prefix. AgentX uses the ``mcp__<server>__<tool>``
...
def _convert_mcp_schema(server_name: str, mcp_tool) -> dict:
    """Convert an MCP tool listing to the AgentX registry schema format.
...
def probe_mcp_server_tools() -> Dict[str, List[tuple]]:
    ...
    Designed for ``agentx tools`` interactive configuration — connects to each
    enabled server, grabs tool names and descriptions, then disconnects.
    Does NOT register tools in the AgentX registry.
...
def _kill_orphaned_mcp_children(
    ...
    survivors, avoiding shared-resource collisions when multiple agentx
