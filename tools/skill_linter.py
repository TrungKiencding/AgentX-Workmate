...
Design contract (matches the AgentX "no lazy-reading escape hatches / don't
...
def _check_metadata_block(frontmatter: Dict[str, Any]) -> List[LintFinding]:
    ...
    if author and author.strip().lower() in ("agentx", "agent", "agentx agent") and (
        ...
        findings.append(
            LintFinding(
                WARNING,
                "author-caps",
                f"author '{author}' should be 'AgentX Workmate' (proper caps) "
