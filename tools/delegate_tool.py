...
def _expand_parent_toolsets(parent_toolsets: set) -> set:
    ...
    When a parent uses a composite toolset like ``agentx-cli`` (which bundles
...
def _blocked_toolsets_for_role(role: str) -> List[str]:
    ...
    mixed platform bundles such as ``agentx-cli`` because those also contain
...
def _build_child_agent(
    ...
    if toolsets:
        ...
        # Expand composite toolsets (e.g. agentx-cli) so that individual
    ...
    # Blocked tools also live inside mixed platform bundles (agentx-cli,
    # agentx-telegram, etc.) that _strip_blocked_tools must keep because they
    ...
    # child AgentX/Qwen subagent onto the parent's Claude Messages wire (or the
...
def _dump_subagent_timeout_diagnostic(
    *,
    ...
    writes a dedicated log under ``~/.agentx/logs/subagent-<sid>-<ts>.log``
...
def delegate_task(
    ...
    # AGENTX_SESSION_ID ContextVar and os.environ with the subagent's internal
    # id before the background-dispatch code below would read it. The
    # request-scoped chat_id binding (the raw X-Agentx-Session-Id on
    ...
    if background:
        ...
        if not _async_ok:
            ...
            # with that id in X-Agentx-Session-Id once the batch completes.
            ...
            # AGENTX_SESSION_ID here would return the subagent's internal id.
        ...
        if not _async_ok:
            ...
            if isinstance(_sync_result, dict):
                _sync_result["note"] = (
                    ...
                    "one-shot runner such as `agentx -z`, a cron job, a Kanban "
        ...
        try:
            ...
            _source = get_session_env("AGENTX_SESSION_SOURCE", "")
            _origin_ui_session_id = get_session_env("AGENTX_UI_SESSION_ID", "")
        ...
        if not _session_key:
            ...
            # during gateway/TUI turns and AGENTX_SESSION_KEY is not in the CLI
...
def _resolve_delegation_credentials(cfg: dict, parent_agent) -> dict:
    ...
    if not api_key:
        raise ValueError(
            ...
            f"Set the appropriate environment variable or run 'agentx auth'."
        )
...
def _load_config() -> dict:
    """Load delegation config from the active AgentX config.

    Prefer the shared persistent loader because it follows the active
    AGENTX_HOME/profile. ``cli.CLI_CONFIG`` is a legacy fallback for entry
    ...
    ``AGENTX_IGNORE_USER_CONFIG=1`` (``agentx chat --ignore-user-config``) is
    ...
    """
    prefer_legacy = os.environ.get("AGENTX_IGNORE_USER_CONFIG") == "1"
