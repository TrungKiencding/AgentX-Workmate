...
Backend is selected during ``agentx tools`` setup (web.backend in config.yaml).
When available, AgentX can route Firecrawl calls through a Nous-hosted tool-gateway
...
def _env_value(name: str) -> str:
    """Resolve ``name`` via AgentX config-aware env, falling back to process env.

    Mirrors the SearXNG provider's ``_searxng_url()`` so that values set
    through AgentX' config/.env layer (``agentx config set``, ``agentx tools``)
...
def _load_web_config() -> dict:
    """Load the ``web:`` section from ~/.agentx/config.yaml."""
    try:
...
def _get_backend() -> str:
    ...
    Reads ``web.backend`` from config.yaml (set by ``agentx tools``).
...
def _is_backend_available(backend: str) -> bool:
    ...
    if backend == "xai":
        ...
        # runs on every web_search dispatch + every `agentx tools` repaint.
        try:
            ...
            if disabled_key:
                ...
                response_data = {
                    ...
                    "error": (
                        ...
                        f"Re-enable it with `agentx plugins enable {disabled_key}` "
                ...
                response_data = {
                    ...
                    "error": (
                        ...
                        "Run `agentx tools` to set one up."
                    ),
                }
        else:
            logger.info(
                        ...
                        return json.dumps(
                            {
                                ...
                                "error": (
                                    ...
                                    f"`agentx plugins enable {disabled_key}` "
                                    "(or remove it from plugins.disabled)."
                                ),
                            },
                            ensure_ascii=False,
