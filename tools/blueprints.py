...
`agentx skills publish` for sharing. No new source type, no new store, no new
...
def parse_blueprint(skill_md_text: str) -> Optional[BlueprintSpec]:
    ...
    blueprint = ns.get("blueprint") if isinstance(ns, dict) else None
...
def export_blueprint(job: Dict[str, Any], body: str, *, blueprint_name: Optional[str] = None) -> str:
    ...
    to ``agentx skills publish`` to share. ``body`` is the plain-language
