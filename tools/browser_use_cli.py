...
_PRIVATE_BROWSER_SENTINEL = "_AGENTX_BU_PRIVATE_BROWSER"
...
_OWN_TAB_PREAMBLE = """\
# agentx: pin this named session to its own tab (once per daemon process)
def _hermes_ensure_own_tab():
    ...
    _marker = _os.path.join(
        _tf.gettempdir(), "agentx-bu-owntab-%s-%s-%s" % (_uid, _name, _dpid)
    )
...
def default_downgrade_notice() -> Optional[str]:
    ...
    try:
        ...
        return (
            ...
            "Run `agentx tools` (Browser Automation → Browser Use) to install it, "
...
def _managed_bin_dir() -> Optional[str]:
    """AgentX's own bin dir ($AGENTX_HOME/bin) — where install.sh puts uv/uvx
...
def _find_cli() -> Optional[List[str]]:
    ...
    MANAGED-FIRST resolution: AgentX's own ``$AGENTX_HOME/bin`` copy — the
    one every browser backend selection installs and updates via
    ``install_cli()`` — always wins, so all sessions drive one canonical,
    AgentX-controlled binary. PATH and the user-level tool dir
...
def install_cli(timeout_s: int = 600) -> Tuple[bool, str]:
    ...
    Resolution order for uv: AgentX's managed uv (bootstrapped on demand via
    ``hermes_cli.managed_uv.ensure_uv``) → uv on PATH. The binary is linked
    into ``$AGENTX_HOME/bin`` (``UV_TOOL_BIN_DIR``) so ``_find_cli()``
    ...
    # prevent provisioning the canonical AgentX-managed copy, or resolution
    # stays pinned to a binary we don't control (version drift, no updates
    # through agentx tools).
...
def _resolve_backend_cdp(
    ...
    try:
        ...
        return (
            ...
            "switch backends via `agentx tools` → Browser Automation."
        )
