...
<url>``) is spawned as a direct child of the AgentX process. AgentX's own
...
AgentX process dies hard — ``kill -9``, an OS-level crash, a force-quit of
...
parent dies, so nothing reaps these until the next AgentX startup's opt-in
