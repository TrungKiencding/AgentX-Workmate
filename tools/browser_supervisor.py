...
One ``CDPSupervisor`` runs per AgentX ``task_id`` that has a reachable CDP
...
def _redact_cdp_error_text(exc: object) -> str:
    ...
    those credentials never reach AgentX logs or tracebacks. Falls back to a
...
DIALOG_BRIDGE_HOST = "agentx-dialog-bridge.invalid"
...
(() => {
  ...
  const ENDPOINT = "http://agentx-dialog-bridge.invalid/";
