"""
Lazy dependency installer for opt-in AgentX Workmate backends.

Many AgentX features (Mistral TTS, ElevenLabs TTS, Honcho memory, Bedrock,
...
(``agentx-workmate[all]``) and install them eagerly at setup time. That has
two problems:
...
remediation hint pointing at ``agentx tools`` or the manual pip command.
...
* **Durable-target mode (immutable images).** When the deployment seals the
  agent's own venv (the Docker image sets ``AGENTX_DISABLE_LAZY_INSTALLS=1``
  and makes ``/opt/agentx`` read-only), setting
  ``AGENTX_LAZY_INSTALL_TARGET`` redirects lazy installs to a writable
  ...
  installed package cannot brick AgentX, which is what made it safe to seal
...
LAZY_DEPS: dict[str, tuple[str, ...]] = {
    ...
    # (AGENTX_DISABLE_LAZY_INSTALLS=1) and lazy installs are redirected to the
    ...
    # HF Agent Trace Viewer upload (agentx trace upload / /upload-trace).
    #
    ...
    # presence, the `agentx update` lazy-refresh pass re-asserts THIS pin on
...
_LAZY_TARGET_ENV = "AGENTX_LAZY_INSTALL_TARGET"
...
def _allow_lazy_installs() -> bool:
    ...
    2. ``AGENTX_DISABLE_LAZY_INSTALLS=1`` seals the *agent venv* (set by the
    ...
    if os.environ.get("AGENTX_DISABLE_LAZY_INSTALLS") == "1":
...
def _unsupported_feature_reason(feature: str) -> Optional[str]:
    ...
    ``agentx update`` lazy-refresh pass.
    """
    if sys.platform == "win32" and feature == "platform.matrix":
        return (
            ...
            "from sdist. Run AgentX under WSL to use Matrix on Windows."
        )
    return None
...
def _is_satisfied(spec: str) -> bool:
    ...
    ``agentx update`` propagate pin bumps in :data:`LAZY_DEPS` to already-
...
def _core_constraints_file() -> Optional[Path]:
    ...
    * A backend that *requires* a version conflicting with core fails loudly
    ...
    try:
        ...
        if not lines:
            return None
        fd, path = tempfile.mkstemp(prefix="agentx-core-constraints-", suffix=".txt")
...
def _venv_pip_install(specs: tuple[str, ...], *, timeout: int = 300) -> _InstallResult:
    ...
    * **Durable-target.** When :data:`_LAZY_TARGET_ENV` is set, installs into
    ...
    try:
        ...
        # which() misses the uv AgentX installed and falls through to the
...
def ensure(feature: str, *, prompt: bool = True) -> None:
    ...
    # Package-manager installs (NixOS, and any other distro that ships AgentX
    ...
    # deployment sets AGENTX_MANAGED=true *and* AGENTX_LAZY_INSTALL_TARGET
    ...
    if _lazy_install_target() is None:
        ...
        if managed_by:
            raise FeatureUnavailable(
                ...
                f"packages come from {managed_by}, so AgentX cannot install "
                f"them at runtime. Add the dependencies for {feature!r} via "
                f"{managed_by} (or run a pip/uv install of AgentX instead)."
            )
...
def install_specs(specs: list[str] | tuple[str, ...], *, timeout: int = 300) -> InstallSpecsResult:
    ...
    * **Durable-target on immutable images** — when the deployment seals the
      agent venv (``AGENTX_DISABLE_LAZY_INSTALLS=1``) and sets
      ``AGENTX_LAZY_INSTALL_TARGET``, installs are redirected to the writable
...
def active_features() -> list[str]:
    ...
    Used by ``agentx update`` to figure out which lazy backends need a
...
def refresh_active_features(*, prompt: bool = False) -> dict[str, str]:
    ...
    Intended for ``agentx update``. Never raises; lazy-install failures
