...
LOCAL_STT_COMMAND_ENV = "AGENTX_LOCAL_STT_COMMAND"
LOCAL_STT_LANGUAGE_ENV = "AGENTX_LOCAL_STT_LANGUAGE"
...
def _resolve_stt_language(
    ...
    *,
    ...
    Resolution order (first non-empty wins):
      ...
      3. ``AGENTX_LOCAL_STT_LANGUAGE`` env var (legacy escape hatch)
...
#
# The single-env-var ``AGENTX_LOCAL_STT_COMMAND`` escape hatch is preserved
...
def _render_command_stt_template(
    ...
    def replace_match(match: "re.Match[str]") -> str:
        ...
        token = f"__AGENTX_STT_PLACEHOLDER_{len(replacements)}__"
        replacements.append((
            token,
...
def _transcribe_command_stt(
    ...
    try:
        with tempfile.TemporaryDirectory(prefix=f"agentx-cmd-stt-{provider_name}-") as tmpdir:
    ...
    if explicit:
        if provider == "local":
            ...
            logger.warning(
                ...
                "(install faster-whisper or set AGENTX_LOCAL_STT_COMMAND)"
            )
            return "none"
...
def _prepare_audio_for_transcription(
    ...
    temp_dir = tempfile.mkdtemp(prefix="agentx-silk-")
...
def _transcribe_local_command(file_path: str, model_name: str) -> Dict[str, Any]:
    ...
    try:
        with tempfile.TemporaryDirectory(prefix="agentx-local-stt-") as output_dir:
...
def _transcribe_groq(file_path: str, model_name: str) -> Dict[str, Any]:
    ...
    ``AGENTX_LOCAL_STT_LANGUAGE`` (env). When none is set, Groq
