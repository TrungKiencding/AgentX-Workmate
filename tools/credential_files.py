...
def register_credential_file(
    relative_path: str,
    container_base: str = "/root/.agentx",
) -> bool:
    """Register a credential file for mounting into remote sandboxes.

    *relative_path* is relative to ``AGENTX_HOME`` (e.g. ``google_token.json``).
    ...
    The resolved host path must remain inside AGENTX_HOME so that a malicious
    skill cannot declare ``required_credential_files: ['../../.ssh/id_rsa']``
    and exfiltrate sensitive host files into a container sandbox.

    Containment alone is not sufficient, because AGENTX_HOME is exactly where
    ...
    # Reject absolute paths — they bypass the AGENTX_HOME sandbox entirely.
    if os.path.isabs(relative_path):
        logger.warning(
            "credential_files: rejected absolute path %r (must be relative to AGENTX_HOME)",
            relative_path,
        )
        return False
    ...
    # that traversal like ``../. ssh/id_rsa`` cannot escape AGENTX_HOME.
    ...
    # inside AGENTX_HOME and therefore pass the containment check above.
            ...
            for item in cred_files:
                if isinstance(item, str) and item.strip():
                    ...
                    if resolved_path.is_file():
                        container_path = f"/root/.agentx/{rel}"
                        result.append({
...
def _safe_skills_path(skills_dir: Path) -> str:
    ...
    safe_dir = Path(tempfile.mkdtemp(prefix="agentx-skills-safe-"))
...
def map_cache_path_to_container(
    ...
    caller decides which ``container_base`` applies (Docker ``/root/.agentx``,
    SSH ``<remote_home>/.agentx``, etc.) and whether translation is wanted.
