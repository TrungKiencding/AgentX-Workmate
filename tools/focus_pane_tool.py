#!/usr/bin/env python3
"""Reveal/focus a pane in the AgentX desktop GUI.

Gated on ``AGENTX_DESKTOP`` (like the other GUI affordances). Emits
...
def focus_pane_tool(pane: str) -> str:
    ...
    if not ok:
        return tool_error("Pane focus is only available in the AgentX desktop app.")
...
FOCUS_PANE_SCHEMA = {
    "name": "focus_pane",
    "description": (
        "Reveal and focus a pane in the AgentX desktop app when the user asks to "
