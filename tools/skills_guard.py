...
Trust levels:
  - builtin:   Ships with AgentX. Never scanned, always trusted.
...
THREAT_PATTERNS = [
    ...
    (r'\$HOME/\.agentx/\.env|\~/\.agentx/\.env',
     "hermes_env_access", "critical", "exfiltration",
     "directly references AgentX secrets file"),
    ...
    # invocation: any client plugin that relays AgentX' secure prompts has to name them, and a bare
    ...
    (r'\.agentx/config\.yaml|\.agentx/SOUL\.md',
     "hermes_config_mod", "critical", "persistence",
     "references AgentX configuration files directly"),
...
# scan. `.skillignore` is the AgentX-native name; `.clawhubignore` is honored
