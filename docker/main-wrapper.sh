...
# with-contenv before touching AGENTX_HOME / PATH. On the non-PID-1
# fallback path the Dockerfile env is still intact, so we skip the
# re-exec and continue directly.
#
# Routing:
#   no args                       → exec `agentx` (the default)
#   first arg is an executable    → exec it directly (sleep, bash, sh, …)
#   first arg is anything else    → exec `agentx <args>` (subcommand passthrough)
#
# Drop to agentx via s6-setuidgid, but skip it when already non-root.
set -e

if [ -z "${AGENTX_MAIN_WRAPPER_ENV_READY:-}" ] && \
   [ -z "${AGENTX_HOME:-}" ] && \
   [ -x /command/with-contenv ]; then
    export AGENTX_MAIN_WRAPPER_ENV_READY=1
    exec /command/with-contenv sh "$0" "$@"
fi
unset AGENTX_MAIN_WRAPPER_ENV_READY

drop() { [ "$(id -u)" = 0 ] && set -- s6-setuidgid agentx "$@"; exec "$@"; }
...
# to the agentx user's home before dropping privileges so libraries that
...
_hermes_orig_cwd="${AGENTX_ORIG_CWD:-$PWD}"
...
# AgentX subcommand pass-through.
drop agentx "$@"
