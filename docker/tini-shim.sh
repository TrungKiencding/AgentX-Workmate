...
# AgentX WebUI, NAS "update" flows that preserve an old entrypoint, etc.)
...
#
# Test hook: AGENTX_TINI_SHIM_TARGET overrides the `/init` path (and
# AGENTX_TINI_SHIM_WRAPPER the main-wrapper path) so unit tests can
