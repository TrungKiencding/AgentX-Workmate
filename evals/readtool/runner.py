"""Run the read-tool eval through the REAL AgentX AIAgent.

For each task: fresh temp AGENTX_HOME, fresh fixture workspace, real
...
def run_task(task, model: str, provider: str, timeout_mult: float,
    ...
    hermes_home = Path(tempfile.mkdtemp(prefix="readtool-home-")) / ".agentx"
...
def main() -> int:
    ...
    if not os.environ.get("OPENROUTER_API_KEY"):
        raise SystemExit(
            ...
            "source ~/.agentx/.env; set +a  — then relaunch."
        )
