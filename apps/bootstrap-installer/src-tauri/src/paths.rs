...
//!   Windows: %LOCALAPPDATA%\agentx
//!   macOS:   ~/.agentx
//!   Linux:   ~/.agentx  (override via $AGENTX_HOME)
//!
//! NOTE (macOS): Python's get_hermes_home(), scripts/install.sh, and the
//! Electron desktop's resolveHermesHome() ALL use ~/.agentx on macOS — there
...
pub fn hermes_home() -> PathBuf {
    ...
    // macOS + Linux + fallback: ~/.agentx (matches Python get_hermes_home(),
    // install.sh, and the Electron desktop's resolveHermesHome()).
    if let Some(home) = dirs::home_dir() {
        return home.join(".agentx");
    }
...
///
/// Lives directly under AGENTX_HOME (same rationale as `installer_dest`) so the
...
pub fn copy_self_to_hermes_home() -> std::io::Result<()> {
    ...
    tracing::info!(?src, ?dest, "copied installer to AGENTX_HOME");
    Ok(())
}
...
pub fn init_logging() -> Option<WorkerGuard> {
    ...
    let env_filter = tracing_subscriber::EnvFilter::try_from_env("AGENTX_BOOTSTRAP_LOG")
