...
//!
//! Driven when the installer is launched as `AgentX-Setup.exe --update` (see
//! `AppMode` in lib.rs). The desktop app hands off to us — it exits, then we:
//!
//!   1. wait for the old AgentX desktop process to fully exit (so both the
//!      venv shim and packaged app.asar are free; otherwise `agentx update`
//!      or repair bootstrap can race locked files),
//!   2. run `agentx update --yes --gateway` (Python/repo update; this does NOT
//!      rebuild apps/desktop by design — see cmd_update in hermes_cli/main.py),
//!   3. run `agentx desktop --build-only` (the rebuild step update skips),
...
//!
//! Cross-platform note: `agentx update` already handles macOS/Linux (git/pip).
...
/// `agentx update` exit code meaning "another agentx process is holding the
/// venv shim open / dirty precondition" — see _cmd_update_impl in
/// hermes_cli/main.py (sys.exit(2)). We surface a targeted message for this.
const UPDATE_EXIT_CONCURRENT: i32 = 2;

/// How long to wait for the old desktop process to release files under the
/// install tree before giving up and letting `agentx update`'s own guard decide.
...
///
/// The marker is also the cross-process update lock: `agentx update` claims
...
/// overwriting — the pre-fix clobber is what let a dashboard `agentx update`
...
/// owner that is itself and aborts ("Another AgentX update is already
/// running"), then the desktop relaunches and retries forever. A foreign live
/// pid (e.g. a dashboard-spawned `agentx update`) still blocks.
...
async fn run_update(app: AppHandle) -> Result<()> {
    ...
    let install_root = hermes_home.join("agentx-agent");
    ...
    // updater — most often a dashboard-spawned `agentx update` — is already
    ...
    let _update_marker = match UpdateMarkerGuard::acquire(
        ...
        Err(owner) => {
            ...
            let msg = format!(
                "Another AgentX update is already running (PID {}, started {} ago). \
    ...
    let agentx = resolve_hermes(&install_root).ok_or_else(|| {
        let msg = format!(
            "Could not find the agentx CLI under {}. Is AgentX installed? \
    ...
    // async on Windows. If it still holds the venv shim, `agentx update`
    ...
    // ---- stage 2: agentx update -----------------------------------------
    // Pass --branch so `agentx update` targets the branch this installer was
    ...
    // without --branch, `agentx update` switches the checkout to `main` (a
    ...
    // --force skips `agentx update`'s Windows running-exe guard (which would
    ...
    // time `agentx update` runs there is no legitimate agentx.exe to protect,
    // and the guard would only produce a false "AgentX is still running" stop.
    //
    ...
    // probe only checks the agentx.exe shim and app.asar, so an external venv
    ...
    // below surfaces the correct "close all AgentX windows" message.
    ...
    // Retry-once for the update-boundary crash. `agentx update` lazily imports
    ...
    // second `agentx update` runs clean because the now-current module is loaded
    ...
    // for the concurrent-instance guard (exit 2) — that's a "close AgentX" state
    // a retry can't fix.
    ...
    match update.exit_code {
        ...
        Some(code) if code == UPDATE_EXIT_CONCURRENT => {
            let msg = "AgentX is still running. Close all AgentX windows and try \
                       the update again."
                .to_string();
            emit_stage(
                &app,
        ...
        other => {
            let msg = format!(
                "agentx update failed (exit {:?}). See {} for details.",
                other,
    ...
    // ---- stage 3: agentx desktop --build-only ----------------------------
    // `agentx update` deliberately does NOT build apps/desktop (it installs
    ...
    // retry-once `agentx update` already does above, and `agentx update`'s own
    ...
    if rebuild.exit_code != Some(0) {
        let msg = format!(
            "Rebuilding the desktop app failed (exit {:?}). The update was \
             applied but the app could not be rebuilt; run `agentx desktop` \
    ...
    if let Some(target_app) = launch_target {
        if let Err(err) = launch_macos_app_and_exit(&app, &target_app).await {
            emit_log(
                ...
                None,
                LogStream::Stderr,
                &format!("[update] could not auto-launch desktop: {err}. Launch AgentX manually."),
            );
        }
    } else if let Err(err) =
...
pub(crate) async fn wait_for_install_locks_free(install_root: &Path, app: &AppHandle, stage: &str) {
    ...
    emit_log(app, Some(stage), LogStream::Stdout, "[handoff] waiting for AgentX to exit…");
    ...
    loop {
        ...
        if Instant::now() >= deadline {
            // Last resort: a backend agentx.exe (or the desktop AgentX Workmate.exe
            ...
            // force-kill every AgentX Workmate.exe except ourselves, then give the OS a
            ...
            emit_log(
                ...
                LogStream::Stdout,
                &format!(
                    "[handoff] AgentX still holding install files ({}); force-killing stragglers…",
...
fn desktop_app_payload_paths(install_root: &Path) -> Vec<PathBuf> {
    ...
    if cfg!(target_os = "windows") {
        ...
        vec![
            release.join("mac").join("AgentX Workmate.app").join("Contents").join("Resources").join("app.asar"),
            release.join("mac-arm64").join("AgentX Workmate.app").join("Contents").join("Resources").join("app.asar"),
        ]
    } else {
...
/// Force-kill any `agentx.exe` other than this process. Windows-only; a no-op
...
/// knew its children — so we kill the whole `agentx.exe` image tree via
/// taskkill, excluding our own PID.
///
/// Safe w.r.t. our own update child: this runs inside the install-lock wait,
/// which completes BEFORE we spawn `venv\Scripts\agentx.exe update`. And a
...
/// agentx.exe images here are stragglers from the old desktop — exactly what
/// we want gone. (`/FI PID ne <self>` also spares this Tauri process, though it
/// isn't named agentx.exe.)
...
/// Spawn `agentx <args>` from `cwd`, stream stdout/stderr as Log events on the
...
/// Path to the venv agentx shim under an install root, regardless of existence.
fn venv_hermes(install_root: &Path) -> PathBuf {
    if cfg!(target_os = "windows") {
        install_root.join("venv").join("Scripts").join("agentx.exe")
    } else {
        install_root.join("venv").join("bin").join("agentx")
    }
}

/// Resolve the agentx CLI to drive. Prefer the venv shim in the install we
/// just updated; fall back to `agentx` on PATH.
fn resolve_hermes(install_root: &Path) -> Option<PathBuf> {
    ...
    let exe = if cfg!(target_os = "windows") { "agentx.exe" } else { "agentx" };
...
fn update_child_env(install_root: &Path) -> Vec<(String, OsString)> {
    let hermes_home = crate::paths::hermes_home();
    let mut envs = vec![(
        "AGENTX_HOME".to_string(),
        hermes_home.as_os_str().to_os_string(),
    )];
    // `agentx update` is a Python CLI writing to a pipe here, so CPython
    ...
    // `agentx update` child claims that SAME lock (hermes_cli/update_lock.py).
    ...
    // refuses its parent's marker with exit 2 ("AgentX is still running")
    ...
    envs.push((
        "AGENTX_UPDATE_HANDOFF_PID".to_string(),
...
async fn install_macos_app_update(
    ...
    let rebuilt_app = crate::bootstrap::resolve_hermes_desktop_app(install_root).ok_or_else(|| {
        anyhow!(
            "desktop rebuild succeeded but no AgentX Workmate.app was found under {}",
    ...
    let tmp = PathBuf::from(format!("{}.agentx-update-new", target_app.display()));
    let old = PathBuf::from(format!("{}.agentx-update-old", target_app.display()));
...
mod tests {
    ...
    fn venv_hermes_is_under_install_root() {
        let root = Path::new("/x/agentx-agent");
    ...
    fn update_child_env_forces_unbuffered_python() {
        let envs = update_child_env(Path::new("/x/agentx-agent"));
        assert!(
            envs.iter()
    ...
    fn update_child_env_names_our_pid_for_the_lock_handoff() {
        let envs = update_child_env(Path::new("/x/agentx-agent"));
        assert!(
            envs.iter().any(|(k, v)| k == "AGENTX_UPDATE_HANDOFF_PID"
                && v.to_str() == Some(std::process::id().to_string().as_str())),
            "the agentx update child claims the same marker we hold; without our pid \
    ...
    fn locked_paths_ignores_missing_payloads() {
        let root = Path::new("/nonexistent/agentx-agent");
        let probes = install_lock_probe_paths(root);
    ...
    fn update_marker_guard_writes_then_removes_on_drop() {
        ...
        let marker = dir.join(".agentx-update-in-progress");
    ...
    fn acquire_refuses_while_a_live_updater_owns_the_marker() {
        ...
        // dashboard `agentx update` and install-mode bootstrap mutate one tree
    ...
    fn parses_only_app_targets() {
        assert_eq!(
            target_app_from_args(["--update", "--target-app", "/Applications/AgentX Workmate.app"]),
            Some(PathBuf::from("/Applications/AgentX Workmate.app"))
        );
    ...
    async fn swap_installs_new_bundle_and_cleans_up() {
        ...
        let target = base.join("AgentX Workmate.app");
        let tmp = base.join("AgentX Workmate.app.agentx-update-new");
        let old = base.join("AgentX Workmate.app.agentx-update-old");
    ...
    async fn swap_failure_never_leaves_target_missing() {
        ...
        let target = base.join("AgentX Workmate.app");
        let tmp = base.join("AgentX Workmate.app.agentx-update-new"); // intentionally absent
        let old = base.join("AgentX Workmate.app.agentx-update-old");
        write_marker(&target, "OLD");
    ...
    async fn swap_rolls_back_when_install_step_fails() {
        ...
        let target = base.join("AgentX Workmate.app");
        let tmp = base.join("AgentX Workmate.app.agentx-update-new"); // absent
        let old = base.join("AgentX Workmate.app.agentx-update-old");
        write_marker(&target, "OLD");
