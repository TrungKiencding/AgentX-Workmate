...
fn main() {
    ...
    // local checkout hasn't pushed). Set AGENTX_BUILD_PIN_COMMIT to bake an
    // immutable commit pin for reproducible/release installers.
    //
    // Commit pin resolution:
    //   - AGENTX_BUILD_PIN_COMMIT, if set and non-empty. Accepts a SHA, tag,
    ...
    //   1. AGENTX_BUILD_PIN_BRANCH, if set and non-empty.
        ...
        match &commit {
            ...
            None => println!(
                "cargo:warning=hermes-bootstrap: following branch {b} HEAD (no commit pin; \
                 set AGENTX_BUILD_PIN_COMMIT for an immutable pin)"
            ),
        }
    }
    if commit.is_none() && branch.is_none() {
        ...
        // build environment.
        println!(
            "cargo:warning=hermes-bootstrap: no pin resolved at build time; binary will fail at runtime without AGENTX_SETUP_DEV_REPO_ROOT or runtime args"
        );
    }
    ...
    // AGENTX_BUILD_PIN_COMMIT resolves a moving ref (tag/branch) to a SHA, a
    ...
    println!("cargo:rerun-if-env-changed=AGENTX_BUILD_PIN_COMMIT");
    println!("cargo:rerun-if-env-changed=AGENTX_BUILD_PIN_BRANCH");
...
fn resolve_commit_pin() -> Option<String> {
    ...
    // asks for one via AGENTX_BUILD_PIN_COMMIT. With no env var, we return
    // None and the installer follows the branch HEAD at install time.
    let requested = std::env::var("AGENTX_BUILD_PIN_COMMIT").ok()?;
    ...
    panic!(
        "AGENTX_BUILD_PIN_COMMIT={requested:?} could not be resolved to a commit \
...
fn resolve_branch_pin() -> Option<String> {
    if let Ok(v) = std::env::var("AGENTX_BUILD_PIN_BRANCH") {
        if !v.trim().is_empty() {
