/**
 * Shared E2E fixtures for the AgentX desktop Playwright suite.
 *
 * Two fixture modes:
 *
 ...
 *     (electron → agentx serve → provider → inference → renderer) is
 ...
 * `electron-builder --dir` step and matches `agentx desktop --source`. The
...
export function createSandbox(prefix: string): Sandbox {
  const root = fs.mkdtempSync(path.join(os.tmpdir(), `agentx-e2e-${prefix}-${Math.random()}`))
  const hermesHome = path.join(root, 'agentx-home')
 ...
 *
 * Key env vars:
 *  - AGENTX_HOME → sandbox agentx-home (isolated config/sessions)
 *  - AGENTX_DESKTOP_USER_DATA_DIR → sandbox electron-user-data
 *  - AGENTX_DESKTOP_IGNORE_EXISTING=1 → don't pick up `agentx` from PATH
 *    (we want the dev checkout at REPO_ROOT)
 *  - AGENTX_DESKTOP_AGENTX_ROOT → REPO_ROOT (dev checkout resolution)
 *  - AGENTX_DESKTOP_APP_NAME → unique-ish per test (avoids single-instance lock)
...
export function buildAppEnv(sandbox: Sandbox, extra: Record<string, string> = {}): Record<string, string> {
  ...
  return {
    ...clean,
    AGENTX_HOME: sandbox.hermesHome,
    AGENTX_DESKTOP_USER_DATA_DIR: sandbox.userDataDir,
    AGENTX_DESKTOP_IGNORE_EXISTING: '1',
    AGENTX_DESKTOP_AGENTX_ROOT: REPO_ROOT,
    AGENTX_DESKTOP_APP_NAME: `HermesE2E-${Date.now()}`,
    ...
    AGENTX_DESKTOP_SKIP_QUIT_CONFIRM: '1',
 ...
 *
 * @param sandbox  - isolated AGENTX_HOME + userData
 * @param env      - the process environment (already has AGENTX_HOME etc.)
...
export interface DeadBackendOptions {
  /**
   * When true, inject a fake boot error via AGENTX_DESKTOP_BOOT_FAKE_ERROR
 ...
 * nothing listens on). By default the backend still boots (`agentx serve`
...
`,
  ...
  const env = buildAppEnv(sandbox, options.fakeError ? { AGENTX_DESKTOP_BOOT_FAKE_ERROR: 'Failed to connect to AgentX backend: connection refused' } : {})
  const { app, page } = await launchDesktop(env)
...
function resolvePackagedBinaryPath(): string {
  if (process.platform === 'win32') {
    return path.join(RELEASE_ROOT, 'win-unpacked', 'AgentX Workmate.exe')
  }

  if (process.platform === 'darwin') {
    const arch = process.arch === 'arm64' ? 'arm64' : 'x64'

    return path.join(RELEASE_ROOT, `mac-${arch}`, 'AgentX Workmate.app', 'Contents', 'MacOS', 'AgentX Workmate')
  }

  return path.join(RELEASE_ROOT, 'linux-unpacked', 'agentx')
}
...
export async function setupPackagedApp(): Promise<PackagedAppFixture> {
  ...
  const env = buildAppEnv(sandbox, {
    ...
    AGENTX_DESKTOP_BOOT_FAKE_STEP_MS: '120',
  })

  // Clear dev-server + agentx-root overrides — the packaged binary
  // should use its own bundled renderer, not the dev checkout.
  delete (env as Record<string, string | undefined>).AGENTX_DESKTOP_DEV_SERVER
