 ...
 *
 * Key env vars:
 *  - AGENTX_HOME → sandbox hermes-home (isolated config/sessions)
 *  - AGENTX_DESKTOP_USER_DATA_DIR → sandbox electron-user-data
 *  - AGENTX_DESKTOP_IGNORE_EXISTING=1 → don't pick up `hermes` from PATH
...
`,
  ...
  const env = buildAppEnv(sandbox, options.fakeError ? { AGENTX_DESKTOP_BOOT_FAKE_ERROR: 'Failed to connect to Hermes backend: connection refused' } : {})
  const { app, page } = await launchDesktop(env)
...
export async function setupPackagedApp(): Promise<PackagedAppFixture> {
  ...
  delete (env as Record<string, string | undefined>).AGENTX_DESKTOP_HERMES
