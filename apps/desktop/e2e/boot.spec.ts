 ...
 * packaged binary) with a real `agentx serve` backend pointed at a mock
 * inference server. The full chain is exercised:
 *
 *   electron → agentx serve (python) → mock provider → renderer
 *
...
test.describe('dev-mode boot with mock backend', () => {
  test('window opens with AgentX title', async () => {
    const title = await fixture!.page.title()
    expect(title).toContain('AgentX')
  })
  ...
  test('backend boots and app becomes ready', async () => {
    ...
    // electron starts → agentx serve is spawned → WS connects → config
