...
test('connected Telegram channel and its transcript stay visible but read-only', async ({
  ...
  try {
    ...
    // Reproduce the workspace-grouped sidebar shown in the bug report. A
    // Telegram transcript must stay in its platform section, not also appear
    // as a Home/project preview.
    await fixture.page.evaluate(() => localStorage.setItem('agentx.desktop.agentsGroupedByWorkspace', 'true'))
    ...
    await sessionRow
      .locator('..')
      .getByRole('button', { name: /Chat actions|Thao tác với cuộc trò chuyện/i })
      .click()
    await fixture.page.getByRole('menuitem', { name: /Delete from Workmate|Xoá khỏi Workmate/i }).click()
    await expect(sessionRow).toHaveCount(0)

    // The deletion must survive a full renderer restart, not just an
    // optimistic sidebar filter while the backend still holds the row.
    await fixture.page.reload()
    await waitForAppReady(fixture, 120_000)
    await expect(sessionRow).toHaveCount(0)
  } finally {
