...
first launch it can install the Hermes Agent runtime into `AGENTX_HOME`
...
4. `AGENTX_DESKTOP_HERMES`, or `hermes` on `PATH`
...
```powershell
# Force a clean first-launch setup
Remove-Item "$env:LOCALAPPDATA\agentx\hermes-agent\.hermes-bootstrap-complete"
# Rebuild a broken Python venv
Remove-Item -Recurse -Force "$env:LOCALAPPDATA\agentx\hermes-agent\venv"
```

> The default Hermes home on Windows is `%LOCALAPPDATA%\agentx`. Set the `AGENTX_HOME` env var if you've relocated it.
