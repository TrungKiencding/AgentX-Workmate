 ...
 * product name (`AgentX`). If a PREVIOUS `npm run pack` was interrupted
 ...
 *   '.../release/linux-unpacked/electron' -> '.../release/linux-unpacked/AgentX Workmate'
 *
 * This is a hard failure with no obvious cause for the user — `agentx desktop`
 ...
 * (the mac-unpacked AgentX Workmate.app bundle) and Windows (win-unpacked), so we
 ...
 * AgentX Workmate.exe that Windows can't load (truncated PE from a corrupt cached
 * Electron zip, wrong arch), the updater's integrity gate in
 * `agentx desktop --build-only` (hermes_cli/main.py
 ...
 */
export function preserveRollbackBackup(appOutDir, productExeName = 'AgentX Workmate.exe') {
...
export default async function beforePack(context) {
  ...
  try {
    ...
    // rename fails.
    const productExe = `${(context && context.packager?.appInfo?.productFilename) || 'AgentX Workmate'}.exe`
