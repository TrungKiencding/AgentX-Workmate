#!/usr/bin/env node
// set-exe-identity.mjs — stamp the AgentX icon + version metadata onto the
// built AgentX Workmate.exe using rcedit, completely decoupled from electron-builder's
// signing path.
//
// WHY THIS EXISTS
// ---------------
...
// skips rcedit, so the unpacked AgentX Workmate.exe keeps the stock Electron icon and
...
// so EVERY packed build — first install, `agentx desktop`, the installer's
...
// path (which rebuilds via `agentx desktop --build-only`, never install.ps1)
// shipped a stock "Electron" exe. Keeping it in afterPack closes that gap.
//
// Also runnable standalone for ad-hoc re-stamping:
//   node scripts/set-exe-identity.mjs <path-to-AgentX Workmate.exe>
//
...
// Stamp the AgentX icon + identity onto `exe`. Resolves on success, throws on
...
async function stampExeIdentity(exe, desktopRoot = resolve(import.meta.dirname, '..')) {
  ...
  console.log('[set-exe-identity] done — AgentX icon + identity stamped')
}
