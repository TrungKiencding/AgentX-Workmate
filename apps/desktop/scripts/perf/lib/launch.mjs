...
//   · its own AGENTX_HOME      → its own backend + sessions, no shared state.
//   · its own --remote-debugging-port → a private CDP endpoint.
//   · AGENTX_DESKTOP_BOOT_FAKE=1 → deterministic boot overlay.
...
// Seed an isolated AGENTX_HOME with just enough config (NOT sessions) so the
// spawned instance reaches an empty chat view instead of the onboarding wizard.
// A separate AGENTX_HOME dir means a separate gateway lock — no collision with
...
export async function startIsolatedInstance({
  ...
  try {
    ...
    // AGENTX_HOME (backend + sessions). No DEV_SERVER env in prod → dist load.
    const electronBin = require('electron')
    // NB: do NOT set AGENTX_DESKTOP_BOOT_FAKE here — it injects artificial
    ...
    if (devUrl) {
      env.AGENTX_DESKTOP_DEV_SERVER = devUrl
    }
