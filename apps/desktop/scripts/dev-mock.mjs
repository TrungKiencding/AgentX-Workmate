...
function createSandbox() {
  const root = fs.mkdtempSync(path.join(os.tmpdir(), `agentx-dev-mock-${Date.now()}`))
...
async function main() {
  ...
  console.log(`  AGENTX_HOME: ${sandbox.hermesHome}`)
  ...
  const env = {
    ...
    AGENTX_DESKTOP_APP_NAME: `HermesDevMock-${Date.now()}`,
  }
