// Backend subcommand routing for the desktop-managed AgentX process.
//
// The desktop app launches its own headless backend via `agentx serve` — it
...
// app hasn't updated yet, or an older `agentx` resolved from PATH) only knows
 ...
 * @param {string} [profile] optional AgentX profile to pin via `--profile`.
 */
