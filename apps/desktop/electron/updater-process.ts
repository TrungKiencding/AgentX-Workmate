...
export function resolveStagedUpdaterBinary(
  ...
  const candidate = path.join(hermesHome, 'agentx-setup.exe')
 ...
 * "Another AgentX update is already running (PID <itself>, started 1s ago)" —
 * the observed infinite "Install didn't finish" loop. Skipping the pre-write
 * for those binaries lets them acquire cleanly and run `agentx update`, which
 ...
 * the installer is written to AGENTX_HOME at install/repair time, making mtime
