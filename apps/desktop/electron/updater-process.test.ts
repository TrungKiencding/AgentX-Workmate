...
test('stagedUpdaterSupportsPrewrittenMarker rejects installers predating the self-adopt fix', () => {
  ...
  assert.equal(
    stagedUpdaterSupportsPrewrittenMarker('C:\\AgentX Workmate\\agentx-setup.exe', {
...
test('resolveStagedUpdaterBinary still returns a stale staged updater on Windows', () => {
  ...
  assert.equal(
    resolveStagedUpdaterBinary('C:\\AgentX Workmate', {
      fileExists: () => true,
      isWindows: true,
      stagedMtimeMs: () => MARKER_SELF_ADOPT_EPOCH_MS - 60 * DAY_MS
    }),
    path.join('C:\\AgentX Workmate', 'agentx-setup.exe')
  )
})
...
test('spawnUpdaterProcess hides the updater console and detaches the child on Windows', () => {
  ...
  const result = spawnUpdaterProcess(
    'agentx-setup.exe',
    ['--update', '--branch', 'main'],
    { cwd: 'C:\\AgentX Workmate', detached: true, stdio: 'ignore' },
    {
      isWindows: true,
  ...
  assert.deepEqual(calls, [
    {
      ...
      command: 'agentx-setup.exe',
      options: { cwd: 'C:\\AgentX Workmate', detached: true, stdio: 'ignore', windowsHide: true }
    }
  ])
})
...
test('resolveStagedUpdaterBinary hands Windows the staged installer it finds', () => {
  const home = 'C:\\Users\\agentx\\AppData\\Local\\agentx'
  const staged = path.join(home, 'agentx-setup.exe')
  const probed: string[] = []
...
test('resolveStagedUpdaterBinary returns null off Windows even when agentx-setup is staged (#74836)', () => {
  ...
  const resolved = resolveStagedUpdaterBinary(home, {
    // The installer stages agentx-setup on macOS/Linux too, so "it exists" is
...
test('resolveStagedUpdaterBinary returns null on Windows when nothing is staged', () => {
  const resolved = resolveStagedUpdaterBinary('C:\\Users\\agentx\\AppData\\Local\\agentx', {
    fileExists: () => false,
    isWindows: true
  })
