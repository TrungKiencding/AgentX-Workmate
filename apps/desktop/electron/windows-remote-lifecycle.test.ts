  ...
  await assert.rejects(
    detectRemotePlatform(
      sshWith(async command => {
        ...
        throw new Error('AgentX is not installed on the remote Windows host.')
      })
    ),
    (err: any) => err.kind === 'unsupported-platform' && /AgentX is not installed/.test(err.message)
  )
})

test('helper command uses the fixed remote Python entry point and quotes path data', () => {
  const command = helperCommand({ python: "C:\\Program Files\\AgentX Workmate's\\python.exe" }, 'inspect', [
    'C:\\x y\\agentx.exe'
  ])
  ...
  assert.match(script, /AgentX Workmate''s/)
  assert.match(script, /C:\\x y\\agentx\.exe/)
})
