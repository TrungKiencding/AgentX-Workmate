...
test('buildNativeAuthorizeUrl omits provider when not given and preserves prefix', () => {
  const url = buildNativeAuthorizeUrl('https://gw.example.com/agentx', {
    challenge: 'C',
  ...
  assert.equal(parsed.pathname, '/agentx/auth/native/authorize')
...
test('nativeTokenUrl / nativeRefreshUrl build the right endpoints', () => {
  ...
  assert.equal(nativeRefreshUrl('https://gw.example.com/agentx'), 'https://gw.example.com/agentx/auth/native/refresh')
})
