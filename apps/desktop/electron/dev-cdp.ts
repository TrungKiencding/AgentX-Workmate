 ...
 *  - no AGENTX_DESKTOP_DEV_SERVER → closed (an unpackaged `electron .` against
 *    dist/ is how the packaged app gets smoke tested; it should behave like
 *    the packaged app).
 *  - otherwise                    → open on 9222, or AGENTX_DESKTOP_CDP_PORT.
 *
 * `AGENTX_DESKTOP_CDP_PORT=off` (or `0` / `false`) opts out for anyone who
...
function resolveDevCdpPort({ env, isPackaged, devServer }: DevCdpInput): DevCdpDecision {
  ...
  const requested = (env.AGENTX_DESKTOP_CDP_PORT ?? '').trim()
