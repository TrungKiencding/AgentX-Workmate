 ...
 *     `X-Agentx-Session-Token` header; WS uses `?token=`.
...
//     reuse-detected — Portal NAS #293 / agentx #37247). When the AT cookie
...
// The Nous portal (NAS) does NOT use AgentX gateway session cookies — it is a
...
const RESERVED_REMOTE_PROFILES = new Set(['agentx', 'test', 'tmp', 'root', 'sudo'])
...
// AgentX Cloud connection (cloud-auto-discovery Q3/Q6): it carries a
...
function normalizeSshConfig(entry) {
  ...
  // name used by the remote AgentX installation. Preserve an explicit mapping
  // when it is a valid AgentX profile identifier; otherwise fall back to the
 ...
 *     `--profile`, so its `AGENTX_HOME` scopes it.
 *
 ...
 * the AgentX gateway session cookies, so cloud sign-in / discovery liveness
