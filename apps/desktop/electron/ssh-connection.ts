...
// root under a short per-user base (`~/.agentx/desktop-ssh`) so even worst case
...
function defaultControlDir() {
  ...
  if (process.platform === 'win32') {
    return path.join(os.tmpdir(), 'agentx-desktop-ssh')
  }

  return path.join(os.homedir(), '.agentx', 'desktop-ssh')
}
