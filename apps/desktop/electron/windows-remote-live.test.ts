...
//   AGENTX_WIN_SSH_HOST   ssh alias/host of the Windows box
//   AGENTX_WIN_SSH_USER   remote user
//   AGENTX_WIN_SSH_AGENTX absolute path to the remote agentx.exe under test
const liveHost = process.env.AGENTX_WIN_SSH_HOST || ''
const liveUser = process.env.AGENTX_WIN_SSH_USER || ''
const configuredHermes = process.env.AGENTX_WIN_SSH_AGENTX || ''
const ownershipId = '89abcdef0123456789abcdef01234567'

function fetchJson(url, token, path) {
  return fetch(`${url}${path}`, { headers: { 'X-Agentx-Session-Token': token } }).then(async response => {
    if (!response.ok) {
...
test.skipIf(!liveHost || !liveUser || !configuredHermes)(
  ...
  async () => {
    ...
    try {
      ...
      const runtimeScript = `& '${configuredHermes.replace('agentx.exe', 'python.exe')}' -m hermes_cli.windows_ssh_runtime read-lock '${ownershipId}'`
      ...
      if (lock) {
        const python = configuredHermes.replace('agentx.exe', 'python.exe')
