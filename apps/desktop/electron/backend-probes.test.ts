...
test('canImportHermesCli returns false when binary does not exist', () => {
  const ghost = path.join(os.tmpdir(), 'agentx-probes-ghost-' + Date.now() + '.exe')
  assert.equal(canImportHermesCli(ghost), false)
})

test('agentx runtime import probe checks config dependencies', () => {
...
test('explicit AgentX override is authoritative', () => {
  assert.equal(shouldTrustHermesOverride('/nix/store/abc/bin/agentx'), true)
})

test('empty AgentX override is not authoritative', () => {
...
test('verifyHermesCli returns true when --version exits 0', () => {
  ...
  // it through node. This stands in for a working agentx binary --
  // verifyHermesCli only cares about the exit code.
  const scriptPath = path.join(os.tmpdir(), `agentx-probes-ok-${Date.now()}-${process.pid}.cjs`)
...
test('default probe timeout is 15s (not the old 5s death-loop value)', () => {
  ...
  // matches the default (tests run without AGENTX_PROBE_TIMEOUT_MS).
  assert.equal(PROBE_TIMEOUT_MS, DEFAULT_PROBE_TIMEOUT_MS)
})

test('resolveProbeTimeoutMs honours AGENTX_PROBE_TIMEOUT_MS', () => {
  assert.equal(resolveProbeTimeoutMs({}), DEFAULT_PROBE_TIMEOUT_MS)
  assert.equal(resolveProbeTimeoutMs({ AGENTX_PROBE_TIMEOUT_MS: '30000' }), 30_000)
  assert.equal(resolveProbeTimeoutMs({ AGENTX_PROBE_TIMEOUT_MS: '0' }), DEFAULT_PROBE_TIMEOUT_MS)
  assert.equal(resolveProbeTimeoutMs({ AGENTX_PROBE_TIMEOUT_MS: 'nope' }), DEFAULT_PROBE_TIMEOUT_MS)
  // Cap runaway values
  assert.equal(resolveProbeTimeoutMs({ AGENTX_PROBE_TIMEOUT_MS: '999999' }), 120_000)
})
