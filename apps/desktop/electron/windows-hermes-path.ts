 ...
 *
 * Pure, dependency-injected pieces of Windows `agentx` resolution pulled out
 ...
 *      FIRST, so an extensionless Git-Bash `agentx` shim shadowed the real
 *      agentx.cmd/agentx.exe; the shim then failed the --version probe and
 ...
 *      venv\Scripts\agentx.exe (the console-script shim, written at the END
 ...
 * extensionless file (e.g. a Git-Bash shell-script shim named `agentx`) must
 * not shadow `agentx.cmd`/`agentx.exe`. The empty entry is kept LAST so
 ...
 * signals (venv python interpreter, venv agentx shim, bootstrap-complete
 * marker) — gating on just the agentx.exe console-script shim alone is the
 ...
 *
 * Returns null when `command` is not a venv agentx shim, the underlying
