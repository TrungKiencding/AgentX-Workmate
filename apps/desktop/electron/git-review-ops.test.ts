...
function makeRepo() {
  const dir = fs.mkdtempSync(path.join(os.tmpdir(), 'agentx-desktop-git-status-'))

  tempDirs.push(dir)
  execFileSync('git', ['init', '-q'], { cwd: dir })
  execFileSync('git', ['config', 'user.email', 'agentx-test@example.com'], { cwd: dir })
  execFileSync('git', ['config', 'user.name', 'AgentX Test'], { cwd: dir })
