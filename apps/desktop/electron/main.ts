...
const USER_DATA_OVERRIDE = process.env.AGENTX_DESKTOP_USER_DATA_DIR
...
const DEV_SERVER = process.env.AGENTX_DESKTOP_DEV_SERVER
const IS_PACKAGED = app.isPackaged || Boolean(process.env.AGENTX_DESKTOP_IS_PACKAGED)
...
// switches only apply pre-launch. Override with AGENTX_DESKTOP_DISABLE_GPU
...
if (DEV_CDP.port) {
  ...
  console.log(
    `[agentx] renderer debugging on http://127.0.0.1:${DEV_CDP.port} — anything that can reach it ` +
      'can run code in the renderer. AGENTX_DESKTOP_CDP_PORT=off to disable.'
  )
} else {
...
// never go through `agentx desktop`; it is version-scoped so an app update
...
if (IS_WINDOWS) {
  ...
  // expose AgentX sessions/config to every packaged app on the machine.
...
// always-on timers. A completely idle, minimized AgentX burned ~20% CPU
...
function loadInstallStamp() {
  ...
  for (const p of candidates) {
    try {
      ...
      console.warn(`[agentx] install-stamp.json found at ${p} , but parsing failed with ${e}`)
...
// AGENTX_HOME — the user-facing root for everything AgentX-related. Mirrors
// scripts/install.ps1's $AgentXHome and scripts/install.sh's $AGENTX_HOME.
//
// Defaults:
//   Windows: %LOCALAPPDATA%\agentx (matches install.ps1)
//   macOS / Linux: ~/.agentx (matches install.sh)
//
// Special case for Windows: if the user has a legacy ~/.agentx directory
// (e.g., from a prior pip install or a manual setup) AND no
// %LOCALAPPDATA%\agentx yet, prefer the legacy path so we don't orphan their
// existing config / sessions / .env. New installs go to %LOCALAPPDATA%.
//
// AGENTX_DESKTOP_USER_DATA_DIR (used by test:desktop:fresh) puts the sandbox
// AGENTX_HOME beneath the throwaway userData dir so a fresh-install run never
// touches the user's real ~/.agentx / %LOCALAPPDATA%\agentx.
function resolveHermesHome() {
  ...
  if (USER_DATA_OVERRIDE) {
    return path.join(path.resolve(USER_DATA_OVERRIDE), 'agentx-home')
  }

  if (IS_WINDOWS) {
    // A GUI app launched from Explorer inherits the environment block captured
    // at login, so a AGENTX_HOME set via `setx` AFTER login is invisible in
    // process.env even though the CLI (a fresh shell) sees it. Without this the
    // backend silently falls back to %LOCALAPPDATA%\agentx and reports "No
  ...
  if (IS_WINDOWS && process.env.LOCALAPPDATA) {
    const localappdata = path.join(process.env.LOCALAPPDATA, 'agentx')
    const legacy = path.join(app.getPath('home'), '.agentx')

    // Migrate transparently to LOCALAPPDATA, but honour an existing legacy
    // ~/.agentx setup (no LOCALAPPDATA install yet) so users don't lose state.
  ...
  return path.join(app.getPath('home'), '.agentx')
}
...
// ACTIVE_AGENTX_ROOT — the canonical mutable AgentX install. Same path
// install.ps1 / install.sh use, so a desktop-only user and a CLI-only user end
// up with identical layouts and can share one install.
const ACTIVE_AGENTX_ROOT = path.join(AGENTX_HOME, 'agentx-agent')
// VENV_ROOT — venv lives inside the repo, exactly like install.ps1 does it.
const VENV_ROOT = path.join(ACTIVE_AGENTX_ROOT, 'venv')
...
//
// We deliberately put the marker INSIDE ACTIVE_AGENTX_ROOT (not alongside)
...
// active-profile.json records which AgentX profile the desktop launches its
// local backend as. When set, startHermes() passes `agentx --profile <name>
// dashboard …`, which deterministically pins AGENTX_HOME (see
...
const DESKTOP_LOG_PATH = path.join(AGENTX_HOME, 'logs', 'desktop.log')
...
const BOOT_FAKE_MODE = process.env.AGENTX_DESKTOP_BOOT_FAKE === '1'
const BOOT_FAKE_ERROR = process.env.AGENTX_DESKTOP_BOOT_FAKE_ERROR || ''
...
const SKIP_QUIT_CONFIRM = process.env.AGENTX_DESKTOP_SKIP_QUIT_CONFIRM === '1'
...
const APP_NAME = process.env.AGENTX_DESKTOP_APP_NAME || 'AgentX'
...
// 'agentx:native-theme' ('dark' | 'light' | 'system'); we pin
...
// electron-builder derives from the build `appId` (com.agentx.workmate) —
...
const MEDIA_PROTOCOL = 'agentx-media'

// Only audio/video may be streamed. Without this the handler would read any
// non-blocklisted local file (no size cap) for any `fetch(agentx-media://…)`.
...
const POOL_MAX_BACKENDS = Math.max(1, Number(process.env.AGENTX_DESKTOP_POOL_MAX) || 3)
const POOL_IDLE_MS = Math.max(60_000, Number(process.env.AGENTX_DESKTOP_POOL_IDLE_MS) || 10 * 60_000)
...
// respawning agentx serve backend children in a tight loop while boot is broken.
...
//
// The snapshot is queryable via the agentx:bootstrap:get IPC handler so a
...
// The Tauri updater writes AGENTX_HOME/.agentx-update-in-progress for the whole
...
// (`force_kill_other_hermes`, taskkill /IM agentx.exe) kills it, the launch
...
function findOnPath(command) {
  ...
  // shell-script shim named `agentx` — must not shadow `agentx.cmd`/`agentx.exe`.
...
// spawns `agentx serve`; runtimes older than serve only have `dashboard`. We
...
// (covers a bare `agentx` resolved from PATH with no known source root). Result
...
function backendSupportsServe(backend) {
  ...
  if (supported === null) {
    try {
      ...
      // `agentx --version` (~10.5s measured cold), and a false negative here
...
function findSystemPython() {
  ...
  // CPython but several AgentX deps (notably pywinpty's Rust-built
...
// findGitBash — locate bash.exe on Windows. Resolves AGENTX_GIT_BASH_PATH
...
// normal AGENTX_DASHBOARD_READY stdout line and no ready-file side channel is
// needed.
...
// install only has PortableGit under %LOCALAPPDATA%\agentx\git (never on
...
function resolveGitBinary() {
  ...
  if (localAppData) {
    candidates.push(path.join(localAppData, 'agentx', 'git', 'cmd', 'git.exe'))
    candidates.push(path.join(localAppData, 'agentx', 'git', 'bin', 'git.exe'))
  }
...
// Dev → SOURCE_REPO_ROOT. Packaged/CLI install → ACTIVE_AGENTX_ROOT.
// AGENTX_DESKTOP_AGENTX_ROOT always wins so devs can pin a worktree.
function resolveUpdateRoot() {
  const candidates = [
    process.env.AGENTX_DESKTOP_AGENTX_ROOT && path.resolve(process.env.AGENTX_DESKTOP_AGENTX_ROOT),
  ...
  return candidates.find(c => directoryExists(path.join(c, '.git'))) || candidates[0] || ACTIVE_AGENTX_ROOT
}
...
// `agentx.exe` holds open; on POSIX it's never mandatory-locked.
function venvHermesShimPath(updateRoot) {
  return IS_WINDOWS
    ? path.join(updateRoot, 'venv', 'Scripts', 'agentx.exe')
    : path.join(updateRoot, 'venv', 'bin', 'agentx')
}
...
// only signals the direct child, so on Windows a backend `agentx.exe` that
// spawned its own grandchildren (a `agentx` REPL, a pty terminal session, the
...
// teardown, so the updater raced a still-locked `agentx.exe`, the quarantine
...
async function handOffWindowsBootstrapRecovery(reason) {
  ...
  const venvBin = path.join(updateRoot, 'venv', IS_WINDOWS ? 'Scripts' : 'bin')
  const venvHermes = path.join(venvBin, IS_WINDOWS ? 'agentx.exe' : 'agentx')
  ...
  // not just the `agentx.exe` console-script shim. That shim is generated at the
  ...
  const haveRealInstall =
    fileExists(venvPython) || fileExists(venvHermes) || fileExists(path.join(updateRoot, '.agentx-bootstrap-complete'))
...
// Marker-independent: is the canonical install at ACTIVE_AGENTX_ROOT actually
...
function isActiveRuntimeUsable() {
  ...
  return (
    isHermesSourceRoot(ACTIVE_AGENTX_ROOT) &&
    fileExists(venvPython) &&
    canImportHermesCli(venvPython, {
      env: {
        PYTHONPATH: [ACTIVE_AGENTX_ROOT, process.env.PYTHONPATH].filter(Boolean).join(path.delimiter)
      }
    })
  )
}
  ...
  if (IS_PACKAGED && /app\.asar(?=$|[\\/])/.test(fallback) && !directoryExists(fallback)) {
    rememberLog(
      `[web-dist] dashboard frontend dir resolved to an asar-internal path that ` +
        ...
        `Ensure dist/** is unpacked (asarUnpack) or set AGENTX_DESKTOP_WEB_DIST.`
    )
  }
...
function resolveRendererIndex() {
  ...
  rememberLog(
    `[renderer] index.html not found — the desktop app was packaged without a ` +
      ...
      `Rebuild with: agentx desktop --force-build`
  )
...
function resolveHermesCwd() {
  ...
  // `…/win-unpacked` on Windows or `/Applications/AgentX Workmate.app/Contents/...`
...
// createActiveBackend — build a backend pointing at ACTIVE_AGENTX_ROOT, the
...
function resolveHermesBackend(backendArgs) {
  // 1. Explicit override -- AGENTX_DESKTOP_AGENTX_ROOT points at a developer
  //    checkout. Honour it as-is (no bootstrap; the user is driving).
  const overrideRoot = process.env.AGENTX_DESKTOP_AGENTX_ROOT && path.resolve(process.env.AGENTX_DESKTOP_AGENTX_ROOT)
  ...
  //    installed `agentx` on PATH so local Python edits are actually exercised.
  ...
  // 3. ACTIVE_AGENTX_ROOT — the canonical install at
  //    %LOCALAPPDATA%\\agentx\\agentx-agent (Windows) or ~/.agentx/agentx-agent.
  ...
  // 4. Existing `agentx` on PATH -- installed via install.ps1 / install.sh from
  //    a previous tool-only setup, or pip-installed system-wide. Use it but
  //    do NOT write a bootstrap marker; the user did this themselves and we
  //    don't want to take ownership of an install we didn't perform.
  //    AGENTX_DESKTOP_IGNORE_EXISTING=1 forces the bootstrap path for testing.
  if (process.env.AGENTX_DESKTOP_IGNORE_EXISTING !== '1') {
    ...
    if (hermesCommand) {
      ...
      // Smoke-test the candidate before trusting it. A `agentx` shim
      ...
      // AGENTX_DESKTOP_AGENTX is an explicit deployment override (used by
  ...
  if (python) {
    ...
    // a uv-managed 3.11 into %LOCALAPPDATA%\agentx\agentx-agent\venv.
...
async function ensureRuntime(backend) {
  ...
  if (backend.kind === 'bootstrap-needed') {
    ...
    if (!bootstrapResult.ok) {
      const bootstrapError = new Error(
        `AgentX bootstrap failed${bootstrapResult.failedStage ? ` at stage '${bootstrapResult.failedStage}'` : ''}: ` +
          `${bootstrapResult.error || 'unknown error'}. ` +
          `Check ${path.join(AGENTX_HOME, 'logs', 'desktop.log')} for the full transcript.`
      ) as any
      ...
      // agentx:bootstrap:reset IPC (renderer's "Reload and retry").
  ...
  if (!isHermesSourceRoot(ACTIVE_AGENTX_ROOT)) {
    throw new Error(
      `AgentX install at ${ACTIVE_AGENTX_ROOT} is missing or incomplete. ` +
        'Reinstall via the desktop installer or scripts/install.ps1.'
    )
  }

  // On Windows, preflight Git Bash. AgentX' terminal tool calls bash.exe
  ...
  // %LOCALAPPDATA%\agentx\git\, which findGitBash() picks up, so for any
  // user who completed the bootstrap this is a no-op. For users who got
  // here via an external `agentx` on PATH, this check still helps.
  if (IS_WINDOWS && !findGitBash()) {
    throw new Error(
      'Git for Windows is required for AgentX on Windows (provides Git Bash, ' +
        "which the agent's terminal tool uses). Install it from " +
        'https://git-scm.com/download/win or run `winget install -e --id Git.Git`, ' +
        'then relaunch AgentX.'
    )
  }
            ...
            reject(
              new Error(
                `Expected JSON from ${url} but got HTML (status ${res.statusCode}). ` +
                  'The endpoint is likely missing on the AgentX backend.'
              )
            )
...
function fetchPublicJson(url, options: any = {}) {
  ...
  // NO ``X-Agentx-Session-Token`` header — used by the auth-mode probe before
...
function getLinkTitleSession() {
  ...
  linkTitleSession = session.fromPartition('agentx:link-titles', { cache: false })
...
// signal so the chat composer doesn't stay stuck on "Starting AgentX...".
...
// 'agentx:power-battery' push below.
...
//
// Hosted AgentX gateways gate the dashboard behind an OAuth provider (e.g.
...
//     alongside the ~15-min access token (Portal NAS #293 / agentx #37247).
...
const OAUTH_SESSION_PARTITION = 'persist:agentx-remote-oauth'
...
// single-shot boot() with no retry) surfaced as the "AgentX couldn't start"
...
async function hasLiveOauthSession(baseUrl) {
  ...
  // for a signed-in user — the exact race that produced the transient "AgentX
...
// --- AgentX Cloud discovery + silent per-agent sign-in (cloud-auto-discovery
// Phase 3) ---------------------------------------------------------------
//
...
// portal session cookie living in `persist:agentx-remote-oauth`:
...
// so a single override flips every AgentX surface to the same portal.
...
// authenticates via PRIVY, not the AgentX gateway session cookies, so this
...
// Discover the hosted (AgentX Cloud) agents the signed-in user can see. Calls
...
async function discoverCloudAgents(org?: string) {
  ...
  try {
    ...
    if (error && error.statusCode === 401) {
      const err = new Error('Your AgentX Cloud session has expired. Open Settings → Gateway and sign in again.') as any
      err.needsCloudLogin = true
      err.cause = error
      throw err
    }
...
async function cloudAgentSilentSignIn(dashboardUrl) {
  ...
  if (!(await hasLivePortalSession())) {
    const err = new Error('Your AgentX Cloud session has expired. Sign in to AgentX Cloud again.') as any
    err.needsCloudLogin = true
    throw err
  }
...
function sanitizeConnectionProfiles(raw: Record<string, any>) {
  ...
  for (const [name, entry] of Object.entries(raw)) {
    ...
    // Preserve the AgentX Cloud org tag on cloud-mode entries so Settings can
...
// a valid stored value (pins the root AGENTX_HOME explicitly); null means "no
...
// `org` (optional) is the AgentX Cloud org slug/id the instance was discovered
...
function coerceDesktopConnectionConfig(input: any = {}, existing = readDesktopConnectionConfig(), options: any = {}) {
  ...
  // describe a discovered AgentX Cloud instance, NOT a user-owned remote gateway,
...
async function buildRemoteConnection(
  ...
  if (authMode === 'oauth') {
    ...
    // refresh token (agentx #37247), and the gateway middleware transparently
    ...
    if (
      ...
      const err = new Error(
        'Remote AgentX gateway uses OAuth, but you are not signed in. ' +
  ...
  if (!token) {
    throw new Error(
      'Remote AgentX gateway is selected, but no session token is saved. ' +
...
async function bootstrapSshConnectionInner(profile, sshConfig, reuseToken, source, fingerprint, lease) {
  ...
  sshRememberLog(
    `[ssh] connection ${result.reused ? 'REUSED' : 'spawned'} dashboard: ` +
      `${result.hermesVersion || 'agentx (version unknown)'} at ${result.hermesPath || '?'}`
  )
...
//   2. env override (AGENTX_DESKTOP_REMOTE_URL/_TOKEN) — applies app-wide
...
async function resolveRemoteBackend(profile) {
  ...
  if (rawEnvUrl) {
    if (!rawEnvToken) {
      throw new Error(
        'AGENTX_DESKTOP_REMOTE_URL is set but AGENTX_DESKTOP_REMOTE_TOKEN is not. ' +
          'Both must be provided to connect to a remote AgentX backend.'
      )
    }
...
async function probeRemoteAuthMode(rawUrl) {
  ...
  // sending any credentials. ``/api/status`` is public on every AgentX
...
async function testDesktopConnectionConfig(input: any = {}) {
  ...
  // connect to AgentX gateway". Mirror the renderer's connect here so the test
...
async function startHermes() {
  ...
  const connectionPromise = (async () => {
    ...
    // deterministic (it wins over the sticky ~/.agentx/active_profile file) and
    // resolves AGENTX_HOME the same way `agentx -p <name>` does on the CLI. An
...
// here so it can leave the app's bounds and stay visible while AgentX is
...
// pushes pet state over IPC (agentx:pet-overlay:state); the overlay just renders
// it. Control flows back (pop-in, composer submit) via agentx:pet-overlay:control.
...
function spawnPetOverlayWindow(bounds) {
  ...
  try {
    ...
    // which briefly hides the Dock/cmd-tab presence. Keep AgentX in the normal
...
// "Starting AgentX…". Before the renderer's backoff loop reconnects, it asks us
...
ipcMain.handle('agentx:pet-overlay:open', async (_event, request) => {
...
ipcMain.handle('agentx:connection-config:get', async (_event, profile) =>
...
ipcMain.handle('agentx:profile:set', async (_event, name) => {
  ...
  // new AGENTX_HOME. Pool backends keep their own homes, so only the primary
  // is torn down.
  await teardownPrimaryBackendAndWait()
  mainWindow?.reload()
...
ipcMain.handle('agentx:api', async (_event, request) => {
...
ipcMain.handle('agentx:selectPaths', async (_event, options: any = {}) => {
...
ipcMain.handle('agentx:selectSavePath', async (_event, options: any = {}) => {
  const result = await dialog.showSaveDialog(mainWindow, {
...
ipcMain.handle('agentx:normalizePreviewTarget', (_event, target, baseDir) =>
...
// per-session secondary windows spawned via `agentx:window:openSession`).
...
function terminalShellCommand() {
  // AGENTX_DESKTOP_SHELL is the cross-platform escape hatch (a path or a bare
  ...
  const override = (process.env.AGENTX_DESKTOP_SHELL || (IS_WINDOWS ? '' : process.env.SHELL) || '').trim()
...
function terminalShellEnv() {
  ...
  // /TERM=dumb; some terminals set COLORFGBG which would flip AgentX' TUI into
  ...
  env.TERM_PROGRAM = 'AgentX'
  env.TERM_PROGRAM_VERSION = app.getVersion()

  // Let an agentx --tui launched in this pane know it's embedded in the desktop
  // GUI (build_environment_hints surfaces this). Distinct from AGENTX_DESKTOP,
  // which marks the agent *backend* and gates cron/gateway behavior.
  env.AGENTX_DESKTOP_TERMINAL = '1'
...
// The LOCAL Desktop runtime-plugin root: `<AGENTX_HOME>/desktop-plugins`,
// resolved from the main-process AGENTX_HOME (see resolveHermesHome) — NOT from
...
ipcMain.handle('agentx:git:worktreeAdd', async (_event, repoPath, options) =>
  addWorktree(repoPath, options || {}, resolveGitBinary())
)

ipcMain.handle('agentx:git:worktreeRemove', async (_event, repoPath, worktreePath, options) =>
  removeWorktree(repoPath, worktreePath, options || {}, resolveGitBinary())
)

ipcMain.handle('agentx:git:branchSwitch', async (_event, repoPath, branch) =>
...
ipcMain.handle('agentx:git:review:list', async (_event, repoPath, scope, baseRef) =>
  reviewList(repoPath, scope, baseRef, resolveGitBinary())
)
ipcMain.handle('agentx:git:review:diff', async (_event, repoPath, filePath, scope, baseRef, staged) =>
...
ipcMain.handle('agentx:git:fileDiff', async (_event, repoPath, filePath) =>
  fileDiffVsHead(repoPath, filePath, resolveGitBinary())
)
ipcMain.handle('agentx:git:review:stage', async (_event, repoPath, filePath) =>
  reviewStage(repoPath, filePath ?? null, resolveGitBinary())
)
ipcMain.handle('agentx:git:review:unstage', async (_event, repoPath, filePath) =>
  reviewUnstage(repoPath, filePath ?? null, resolveGitBinary())
)
ipcMain.handle('agentx:git:review:revert', async (_event, repoPath, filePath) =>
  reviewRevert(repoPath, filePath ?? null, resolveGitBinary())
)
ipcMain.handle('agentx:git:review:revParse', async (_event, repoPath, ref) =>
  reviewRevParse(repoPath, ref, resolveGitBinary())
)
ipcMain.handle('agentx:git:review:commit', async (_event, repoPath, message, push) =>
  reviewCommit(repoPath, message, Boolean(push), resolveGitBinary())
)
ipcMain.handle('agentx:git:review:commitContext', async (_event, repoPath) =>
...
ipcMain.handle('agentx:git:review:createPr', async (_event, repoPath) =>
...
// the actual removal via `agentx uninstall …` so the cross-platform PATH /
...
async function runDesktopUninstall(mode) {
  ...
  try {
    if (IS_WINDOWS) {
      scriptPath = path.join(app.getPath('temp'), `agentx-uninstall-${Date.now()}.cmd`)
      ...
      scriptPath = path.join(app.getPath('temp'), `agentx-uninstall-${Date.now()}.sh`)
...
// ---------------------------------------------------------------------------
// agentx:// deep links (e.g. agentx://blueprint/morning-brief?time=08:00).
// A docs/dashboard "Send to App" button opens this URL; we route it into the
// running app's chat composer. Three delivery paths: macOS 'open-url',
// Win/Linux running-app 'second-instance' (argv), Win/Linux cold-start argv.
// ---------------------------------------------------------------------------
const AGENTX_PROTOCOL = 'agentx'
...
function handleDeepLink(url) {
  ...
  // agentx://blueprint/<key>?slot=val  -> host="blueprint", path="/<key>"
...
ipcMain.handle('agentx:deep-link-ready', () => {
  ...
  if (_pendingDeepLink) {
    ...
    handleDeepLink(
      `${AGENTX_PROTOCOL}://${queued.kind}/${encodeURIComponent(queued.name)}` +
...
function registerDeepLinkProtocol() {
  try {
    if (process.defaultApp && process.argv.length >= 2) {
      ...
      app.setAsDefaultProtocolClient(AGENTX_PROTOCOL, process.execPath, [path.resolve(process.argv[1])])
    } else {
...
// second-instance argv. Without the lock a second `agentx://` launch spawns a
...
app.on('before-quit', event => {
  ...
  // quitting AgentX never keeps another app's chord hostage.
