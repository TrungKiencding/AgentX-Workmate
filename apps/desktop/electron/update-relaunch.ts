 ...
 * Background
 * ----------
 * After `agentx update` + `agentx desktop --build-only`, the freshly-rebuilt
...
// backend/profile/root it talks to). Anything AGENTX_DESKTOP_* is preserved
// plus AGENTX_HOME. We snapshot the values, not the live env, so the new
...
const PRESERVED_ENV_KEYS = ['AGENTX_HOME', 'ELECTRON_DISABLE_SANDBOX']
const PRESERVED_ENV_PREFIXES = ['AGENTX_DESKTOP_']
