 ...
 * the three user-facing uninstall modes to the `agentx uninstall` CLI flags,
 ...
 *
 *   macOS:   …/AgentX Workmate.app/Contents/MacOS/AgentX Workmate  → …/AgentX Workmate.app
 *   Windows: …\AgentX Workmate\AgentX Workmate.exe                 → …\AgentX Workmate  (install dir)
...
function resolveRemovableAppPath(execPath, platform, env: any = {}) {
  ...
  if (platform === 'darwin') {
    // …/AgentX Workmate.app/Contents/MacOS/AgentX Workmate → strip 3 segments to the .app
    const macOsDir = p.dirname(exe) // …/Contents/MacOS
    const contents = p.dirname(macOsDir) // …/Contents
    const appBundle = p.dirname(contents) // …/AgentX Workmate.app
  ...
  if (platform === 'win32') {
    // NSIS per-user installs AgentX Workmate.exe directly in the install dir.
    const dir = p.dirname(exe)

    if (/[\\/]AgentX Workmate$/i.test(dir) || /[\\/]agentx-desktop$/i.test(dir)) {
      return dir
    }
...
function buildPosixCleanupScript({ desktopPid, pythonExe, pythonPath, agentRoot, uninstallArgs, appPath, hermesHome }) {
  ...
  const lines = [
    ...
    '  done',
    'fi',
    `export AGENTX_HOME=${q(hermesHome)}`
  ]
...
function buildWindowsCleanupScript({
  ...
  // a problem, but AgentX install paths don't use them.
  const q = s => `"${String(s).replace(/"/g, '')}"`
