 ...
 *     issue #61764 death-loop) with AGENTX_PROBE_TIMEOUT_MS override
 ...
 * Honours AGENTX_PROBE_TIMEOUT_MS when it parses as a positive integer.
 */
function resolveProbeTimeoutMs(env: NodeJS.ProcessEnv = process.env): number {
  const raw = env.AGENTX_PROBE_TIMEOUT_MS
...
function execProbeSync(
  ...
  try {
    ...
    // One cold-cache / AV miss should not force agentx-setup --update (#61764).
    execFileSync(command, args, options)
  }
}

/**
 * Return the Python snippet used to verify AgentX can import far enough to
...
/**
 * Return true iff the AgentX runtime import probe exits 0.
 *
 ...
 * its immutable, matching AgentX package; it must never fall through to the
