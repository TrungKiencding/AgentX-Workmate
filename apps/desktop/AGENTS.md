...
How to build AgentX Workmate Desktop well. This is a judgment guide, not an inventory —
...
- The **backend** is authoritative for anything another AgentX surface can also
...
- A **runtime home change** (switching the underlying `AGENTX_HOME` profile) is
...
its shape. "Plugin" means several unrelated things across AgentX — do not assume
