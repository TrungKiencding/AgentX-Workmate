...
const STORAGE_KEY = 'agentx:native-notifications'
...
// "Backgrounded" = the user isn't on AgentX. `document.hidden` only flips when
...
export function dispatchNativeNotification(input: NativeNotificationInput): void {
  ...
  void window.agentxDesktop?.notify({
 ...
 *  user is away from AgentX — the in-app toast (`host.notify`) covers the
 *  foreground case. */
...
export async function sendTestNativeNotification(title: string, body: string): Promise<boolean> {
  const bridge = window.agentxDesktop
