 ...
 *
 * The archive is the CLI's own `agentx profile export` tar.gz (config, skills,
...
const ARCHIVE_FILTERS = [{ extensions: ['tar.gz', 'tgz'], name: 'AgentX profile' }]
...
export async function runExportProfileFlow(profile?: string): Promise<null | string> {
  ...
  const pick = window.agentxDesktop?.selectSavePath
...
export async function runImportProfileFlow(): Promise<null | string> {
  const paths = await window.agentxDesktop?.selectPaths?.({
