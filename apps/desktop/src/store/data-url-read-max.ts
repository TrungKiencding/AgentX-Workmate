 ...
 * (`agentx:data-url-read-max:*`) and the default/clamp in electron/hardening.ts.
 */
...
export async function refreshDataUrlReadMaxMb(): Promise<number> {
  const api = window.agentxDesktop?.dataUrlReadMax
...
if (typeof window !== 'undefined' && window.agentxDesktop?.dataUrlReadMax) {
