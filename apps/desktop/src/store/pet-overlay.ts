 ...
 * app's bounds and stays visible while AgentX is minimized. That window carries
...
const OVERLAY_ACTIVE_KEY = 'agentx.desktop.pet-overlay-active.v1'
const OVERLAY_BOUNDS_KEY = 'agentx.desktop.pet-overlay-bounds.v1'
...
function pushNow(): void {
  window.agentxDesktop?.petOverlay?.pushState(currentPayload())
}
...
function openOverlay(request: PetOverlayOpenRequest): void {
  const api = window.agentxDesktop?.petOverlay
...
export function restorePetOverlay(): void {
  if (!window.agentxDesktop?.petOverlay || !$petOverlayActive.get() || stateUnsubs.length) {
    return
  }
...
export function popInPet(): void {
  ...
  stateUnsubs = []
  $petOverlayActive.set(false)
  void window.agentxDesktop?.petOverlay?.close()
}
