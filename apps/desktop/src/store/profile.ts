...
// /api/profiles/active `current`). "default" is the root ~/.agentx. This is the
...
const PROFILE_ORDER_STORAGE_KEY = 'agentx.desktop.profileOrder'
...
const PROFILE_COLORS_STORAGE_KEY = 'agentx.desktop.profileColors'
...
export async function refreshActiveProfile(): Promise<void> {
  try {
    const res = await window.agentxDesktop.api<ActiveProfileResponse>({
...
// Persist the choice and relaunch the backend under the new AGENTX_HOME. The
...
export async function switchProfile(name: string): Promise<void> {
  ...
  await window.agentxDesktop.profile.set(name)
}
...
async function syncConnectionToActiveProfile(profile: string): Promise<void> {
  const getConnection = window.agentxDesktop?.getConnection
...
const SHOW_ALL_PROFILES_STORAGE_KEY = 'agentx.desktop.showAllProfiles'
...
// Switch to the default (root ~/.agentx) profile — bound to ⌘1.
...
export function touchActiveGatewayBackend(): void {
  ...
  void window.agentxDesktop?.touchBackend?.(target).catch(() => undefined)
}
