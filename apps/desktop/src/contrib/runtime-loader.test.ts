...
const getStatus = vi.fn(async () => ({ hermes_home: '/remote/box/.agentx' }))
...
describe('scanDiskPlugins (#66899)', () => {
  it('scans the Electron-resolved local root, never the backend hermes_home', async () => {
    desktopPluginsRoot.mockResolvedValue('/local/.agentx/desktop-plugins')
    readDir.mockResolvedValue({ entries: [] })

    await discoverRuntimePlugins()

    expect(desktopPluginsRoot).toHaveBeenCalled()
    expect(readDir).toHaveBeenCalledWith('/local/.agentx/desktop-plugins')
    // The remote backend's hermes_home must never feed the local plugin scan.
    expect(getStatus).not.toHaveBeenCalled()
    expect(readDir).not.toHaveBeenCalledWith('/remote/box/.agentx/desktop-plugins')
  })
...
describe('watchRuntimePlugins dir watch (#66899)', () => {
  it('watches the Electron-resolved local root, never the backend hermes_home', async () => {
    ...
    expect(watchDirectory).toHaveBeenCalledWith('/local/.agentx/desktop-plugins')
    expect(watchDirectory).not.toHaveBeenCalledWith('/remote/box/.agentx/desktop-plugins')
    expect(getStatus).not.toHaveBeenCalled()
  })
})
