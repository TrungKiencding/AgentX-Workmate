...
export const en: Translations = {
  ...
  settings: {
    ...
    mcp: {
      ...
      hubTrustPrivate: 'Yours',
      hubVerdictCaution: 'Scan: caution',
      hubUpdate: version => `Update to ${version}`,
      hubUpdateAvailable: version => `${version} available`,
      hubBlocked: (count, tools) => `${count} tool${count === 1 ? '' : 's'} blocked: ${tools}`,
      hubOpen: 'Open on the Hub',
      hubRemove: 'Remove',
      hubRemoved: name => `${name} removed. The AgentX Hub hears it on the next sync.`,
      hubRemoveFailed: name => `Failed to remove ${name}`,
      hubReplace: 'Replace with the Hub version…',
      hubReplaceConfirm: name =>
        `Replace ${name} with the version from the AgentX Hub? Your edits to its command are lost.`,
      hubModified: 'Edited on this machine: the Hub does not overwrite it.',
      hubNameTaken: 'Another server uses this name here.',
  ...
  messaging: {
    ...
    destinationTitle: 'Conversation address',
    destinationUnavailable: 'Connected, but this platform does not provide a public address.',
    openDestination: 'Open in app',
    viewOnly: 'View only',
    viewOnlyTitle: platform => `Following from ${platform}`,
    viewOnlyHint: platform => `Reply in ${platform}; Workmate mirrors the live progress here.`,
  ...
  assistant: {
    ...
    thread: {
      ...
      thinking: 'Thinking',
      startingAgent: 'Starting the assistant',
      thought: 'Thought',
