...
const AGENTX_REF_TYPES = WIRE_REFERENCE_KINDS
type HermesRefType = (typeof AGENTX_REF_TYPES)[number]
...
const AGENTX_DIRECTIVE_RE = referenceRe()
...
function parseDirectiveText(text: string): Unstable_DirectiveSegment[] {
  const matches = [
    ...
    ...Array.from(text.matchAll(AGENTX_DIRECTIVE_RE)).map(match => {
...
/**
 * Renders text containing AgentX directives (`@file:...`, `@image:...`) as
...
const DirectiveImage: FC<{ id: string; label: string }> = ({ id, label }) => {
  ...
  useEffect(() => {
    ...
    const load =
      window.agentxDesktop && isRemoteGateway() ? gatewayMediaDataUrl(id) : window.agentxDesktop?.readFileDataUrl(id)
