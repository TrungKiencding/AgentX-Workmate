...
const STORAGE_KEY = 'agentx.desktop.layoutTree.v2'

writeKey('agentx.desktop.layoutTree.v1', null)
 ...
 */
export const $activePresetId = atom<string>(readKey('agentx.desktop.layoutPreset.active') ?? 'default')

export function markActivePreset(id: string) {
  $activePresetId.set(id)
  writeKey('agentx.desktop.layoutPreset.active', id)
}
 ...
 */
const DISMISSED_KEY = 'agentx.desktop.dismissedPanes.v1'
...
const USER_PLACED_KEY = 'agentx.desktop.userPlacedPanes.v1'
...
if ((import.meta.env.DEV || import.meta.env.VITE_PERF_PROBE === '1') && typeof window !== 'undefined') {
  ;(window as unknown as Record<string, unknown>).__AGENTX_LAYOUT_TREE__ = {
    close: closeTreePane,
