...
const TAG_CLASSES = {
  ...
  h2: 'mt-3 mb-1.5 text-sm font-semibold tracking-tight text-foreground first:mt-0',
  h3: 'mt-2.5 mb-1 text-xs font-semibold text-foreground first:mt-0',
  h4: 'mt-2 mb-1 text-xs font-semibold text-foreground first:mt-0',
  ...
  pre: 'mb-2 overflow-x-auto rounded-md border border-(--ui-stroke-tertiary) bg-background/70 p-2 font-mono text-2xs leading-[1.55] last:mb-0',
  td: 'px-2 py-1 align-top leading-snug',
  th: 'px-2 py-1 text-left text-2xs font-semibold uppercase tracking-[0.08em] text-muted-foreground/80',
  thead: 'bg-muted/40',
...
function MarkdownCode({ className, ...rest }: ComponentProps<'code'>) {
  return (
    <code
      className={cn('rounded bg-muted/80 px-1 py-px font-mono text-[0.9em] text-muted-foreground', className)}
      {...rest}
    />
  )
}
...
function MarkdownTable({ className, ...rest }: ComponentProps<'table'>) {
  return (
    <div className="mb-2 max-w-full overflow-x-auto rounded-md border border-(--ui-stroke-tertiary) last:mb-0">
      <table
        className={cn(
          'w-full border-collapse text-xs [&_tr]:border-b [&_tr]:border-(--ui-stroke-tertiary) last:[&_tr]:border-0',
          className
        )}
        {...rest}
      />
