...
function reveal(file: string) {
  void window.agentxDesktop?.revealPath?.(file)?.catch(() => undefined)
}
...
async function revealPluginsDir() {
  try {
    ...
    const dir = await window.agentxDesktop?.desktopPluginsRoot?.()
    ...
    const result = await window.agentxDesktop?.openDir?.(dir)
