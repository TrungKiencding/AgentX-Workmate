...
const canRunInTerminal = () => typeof window !== 'undefined' && Boolean(window.agentxDesktop?.terminal)
...
//      `agentx model` uses. This is authoritative: a provider tagged by the
...
function ConnectedProviderRow({
  ...
  // AgentX can clear this provider's creds via the API.
  const canDisconnect = provider.disconnectable ?? provider.flow !== 'external'
  // External (CLI-managed) provider AgentX can't clear via the API, but ships a
...
export function ProvidersSettings({
  ...
  // AgentX never deletes creds another tool owns behind a silent API call.
