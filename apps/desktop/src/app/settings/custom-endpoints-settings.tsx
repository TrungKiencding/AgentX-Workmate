...
export function CustomEndpointsSettings({ onConfigSaved, onMainModelChanged }: CustomEndpointsSettingsProps) {
  ...
  useEffect(() => {
    ...
    async function load() {
      try {
        ...
        notifyError(err, copy.loadError)
      } finally {
        if (!cancelled) {
          setLoading(false)
        }
    ...
    // Boot-time fetch only — re-running on a locale switch would clobber an
    // in-progress edit; copy.loadError is read at failure time.
  ...
  async function handleSave() {
    try {
      ...
      notifyError(err, copy.saveError)
    } finally {
      setSaving(false)
    }
  }
  ...
  async function handleValidate() {
    try {
      ...
      notifyError(err, copy.validateError)
    } finally {
      setTesting(false)
    }
  }
  ...
  async function handleActivate(endpoint: CustomEndpoint) {
    try {
      ...
      notifyError(err, copy.activateError)
    } finally {
      setActivating(null)
    }
  }
  ...
  async function handleDelete(endpoint: CustomEndpoint) {
    ...
    try {
      ...
      notifyError(err, copy.deleteError)
    } finally {
      setDeleting(null)
    }
  }
  ...
  return (
    <SettingsContent>
      <div className="space-y-6">
                ...
                <div className="grid gap-3 p-3 sm:grid-cols-[minmax(0,1fr)_auto] sm:items-center" key={endpoint.id}>
                  ...
                  <div className="flex items-center gap-2 sm:justify-end">
                    ...
                    {endpoint.source !== 'direct-config' && (
                      <Button
                        aria-label={copy.deleteEndpoint}
        ...
        <section>
          <SectionHeading icon={Plus} title={form.id ? copy.editHeading : copy.addHeading} />
          <div className="grid gap-3 rounded-md border border-border/50 p-3">
            ...
            <label className="grid gap-1.5 text-xs text-muted-foreground">
              ...
              <Input
                ...
                placeholder={form.id ? copy.keepKeyPlaceholder : copy.optionalPlaceholder}
                type="password"
                value={form.apiKey}
              />
            </label>
