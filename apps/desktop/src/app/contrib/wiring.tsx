...
import { WebmatePromptCard } from '@/components/webmate-prompt-card'
...
export function ContribWiring({ children }: { children: ReactNode }) {
  ...
  useEffect(() => {
    ...
    window.addEventListener('agentx:open-keybinds', onOpenKeybinds)

    return () => window.removeEventListener('agentx:open-keybinds', onOpenKeybinds)
  }, [navigate])
  ...
  return (
    <ContribWiringContext.Provider value={api}>
      ...
      <RemoteDisplayBanner />
      {!isSecondaryWindow() && <WebmatePromptCard />}
