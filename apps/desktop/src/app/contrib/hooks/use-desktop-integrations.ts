...
export function useDesktopIntegrations({
  ...
  useEffect(() => {
    window.agentxDesktop?.setPreviewShortcutActive?.(true)
  }, [])
  ...
  useEffect(() => {
    const unsubscribe = window.agentxDesktop?.onFocusSession?.(sessionId => {
      if (sessionId) {
  ...
  useEffect(() => {
    const unsubscribe = window.agentxDesktop?.onNotificationAction?.(({ actionId, sessionId }) => {
  ...
  useEffect(() => {
    ...
    void window.agentxDesktop?.signalDeepLinkReady?.()
  ...
  useEffect(() => {
    const unsubscribe = window.agentxDesktop?.onClosePreviewRequested?.(
  ...
  useEffect(() => {
    const unsubscribe = window.agentxDesktop?.onOpenFolderRequested?.(() => void openFolderAsProject())
