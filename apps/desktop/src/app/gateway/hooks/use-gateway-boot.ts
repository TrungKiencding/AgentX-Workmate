...
interface GatewayBootOptions {
  ...
  onConnectionReady: (
    connection: Awaited<ReturnType<NonNullable<typeof window.agentxDesktop>['getConnection']>> | null
  ) => void
...
export function useGatewayBoot({
  ...
  useEffect(() => {
    ...
    // composer stayed disabled forever on "Starting AgentX...". Once the
    ...
    const attemptReconnect = async () => {
      ...
      try {
        ...
        // "Starting AgentX…". The probe is a no-op for a healthy or local backend.
        ...
        // as an opaque "Could not connect to AgentX gateway". resolveGatewayWsUrl
    ...
    async function boot() {
      try {
        ...
        await Promise.all([
          ...
          // retry it), not brick boot behind the "AgentX couldn't start"
