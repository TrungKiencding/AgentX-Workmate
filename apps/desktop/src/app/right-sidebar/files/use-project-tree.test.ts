...
describe('useProjectTree', () => {
  ...
  it('reads gitignore from the real path while caching per connection', async () => {
    ...
    ;(window as unknown as { agentxDesktop: unknown }).agentxDesktop = { gitRoot, readDir, readFileDataUrl }
  ...
  it('falls back to the sanitized workspace dir when the session cwd is gone', async () => {
    ...
    ;(window as unknown as { agentxDesktop: unknown }).agentxDesktop = { readDir, sanitizeWorkspaceCwd }
  ...
  it('returns no-bridge gracefully when window.agentxDesktop is missing', async () => {
    delete (window as unknown as { agentxDesktop?: unknown }).agentxDesktop
