...
beforeEach(() => {
  ...
  proposeSkillToOrg.mockResolvedValue({
    ok: true,
    status: 'ok',
    created: true,
    slug: 'vneb-report',
...
describe('PublishSkillDialog', () => {
  it('previews through the hub, uploads with the chosen visibility and shows where it went', async () => {
    ...
    await waitFor(() =>
      expect(validateSkillForHub).toHaveBeenCalledWith('vneb-report', { kind: undefined, visibility: 'org' })
    )
    ...
    await waitFor(() =>
      expect(publishSkillToHub).toHaveBeenCalledWith('vneb-report', { visibility: 'org', kind: undefined })
    )
