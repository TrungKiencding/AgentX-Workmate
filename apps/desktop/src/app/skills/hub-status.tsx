  ...
  return (
    <section className="mb-4 rounded-lg border border-(--ui-stroke-tertiary) bg-(--ui-bg-quinary) p-3" data-testid="hub-status">
      ...
      {data && data.workspaces.length > 0 && (
        <p className="mt-2 text-2xs text-(--ui-text-tertiary)" data-testid="hub-workspaces">
          {h.workspaceSkills(data.workspaces.reduce((n, w) => n + w.skills.length, 0), data.workspaces.length)}
        </p>
      )}
