        ...
        <>
          <a
            ...
            onClick={event => {
              ...
              void window.agentxDesktop?.openExternal(error.url)
            }}
          >
...
export function PreviewPane({
  ...
  const restartServer = useCallback(async () => {
    ...
    // "Ask AgentX to restart the server" looked like it did nothing —
  ...
  useEffect(() => {
    if (
      ...
      !window.agentxDesktop?.watchPreviewFile ||
      !window.agentxDesktop?.onPreviewFileChanged
    ) {
      return
    }
    ...
    const unsubscribe = window.agentxDesktop.onPreviewFileChanged(payload => {
    ...
    void window.agentxDesktop
      ...
      .then(watch => {
        if (!active) {
          void window.agentxDesktop?.stopPreviewFileWatch?.(watch.id)
    ...
    return () => {
      ...
      if (watchId) {
        void window.agentxDesktop?.stopPreviewFileWatch?.(watchId)
      }
    }
