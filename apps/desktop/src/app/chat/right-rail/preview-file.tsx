...
async function readTextPreview(filePath: string) {
  try {
    ...
    if (!message.includes("No handler registered for 'agentx:readFileText'")) {
      throw error
    }
  }
  ...
  const dataUrl = await window.agentxDesktop.readFileDataUrl(filePath)
...
function startLineDrag(event: ReactDragEvent<HTMLElement>, filePath: string, { end, start }: LineSelection) {
  ...
  event.dataTransfer.setData(AGENTX_PATHS_MIME, JSON.stringify([{ line: start, lineEnd, path: filePath }]))
