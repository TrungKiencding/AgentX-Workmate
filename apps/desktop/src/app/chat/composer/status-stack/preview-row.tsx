...
export const PreviewStatusRow = memo(function PreviewStatusRow({ item, onDismiss }: PreviewStatusRowProps) {
  ...
  const openInBrowser = async () => {
    try {
      const bridge = window.agentxDesktop?.openPreviewInBrowser
  ...
  return (
    ...
    >
      ...
      >
        <span className="min-w-0 max-w-[18rem] truncate text-xs leading-4 text-foreground/92">{item.label}</span>
      </Tip>
    </StatusRow>
  )
})
