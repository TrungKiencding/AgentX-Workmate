 ...
 * Returning `{}` is fine — the row then shows AgentX' defaults.
 */
          ...
          {groups.map(group => {
            ...
            return (
              <DropdownMenuGroup className="py-0.5" key={slug}>
                <DropdownMenuItem
                  className="group/label flex w-full items-center gap-1 px-2 pb-0.5 pt-0.5 text-2xs font-semibold uppercase tracking-label text-(--ui-text-tertiary) cursor-pointer !bg-transparent focus:!bg-transparent"
                  onSelect={event => {
                    event.preventDefault()
