...
//
// When someone is in a hands-free "Hey AgentX" voice chat, the natural way to
...
// phrase (optionally addressed to AgentX), so a real turn that merely contains
...
// Optional address prefixes so "agentx stop" / "ok stop" / "hey agentx, stop"
// still count. Stripped before matching the core phrase.
const ADDRESS_PREFIXES: readonly string[] = ['hey agentx', 'hey agentx,', 'agentx', 'agentx,', 'ok', 'okay', 'hey']
...
function stripAddress(text: string): string {
  for (const prefix of ADDRESS_PREFIXES) {
    if (text === prefix) {
      // Bare address ("agentx") is not a stop command on its own.
      continue
    }
 ...
 * to AgentX). Returns false for anything that merely contains "stop" as part of
