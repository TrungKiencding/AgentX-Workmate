...
/** Why a known AgentX command has no desktop UI surface. */
...
const DESKTOP_COMMAND_SPECS: readonly DesktopCommandSpec[] = [
  ...
  { name: '/profile', description: 'Switch the active AgentX profile', surface: action('profile') },
  {
    name: '/skin',
  ...
  { name: '/version', description: 'Show AgentX Workmate version', surface: exec() },
 ...
 * AgentX' built-in slash commands — i.e. skill commands (`/gif-search`,
 ...
 * activity — the ones that ship with AgentX and were never asked for. It is
