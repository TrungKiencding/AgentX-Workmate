...
export function interpretRuntimeReadiness(
  ...
  // Resolved per call rather than at module load: the locale is only known
  // once the provider has read `display.language`.
  const defaultReason = options.defaultReason ?? translateNow('desktop.providerCredentialRequired')
