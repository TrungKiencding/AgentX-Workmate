...
| 4 · Liền mạch | **Xong** (09/09/2026) | Nhánh `webmate-install/phase-4` (Workmate) và `feat/workmate-sso` (WebMate, đầu `1cb6e0a08`). **WebMate:** `auth_hint` (prompt=none + login_hint qua `chrome.identity.launchWebAuthFlow`, redirect `https://pfadeibckkgklmmjghiikadphihbpape.chromiumapp.org/`, PKCE, giữ 60 s khi `login_required`), `auth_open` (đăng nhập tương tác có sẵn email, trả `opened` ngay), manifest Chrome thêm `identity` (union qua `addPermissions`), `hello.instanceId` mỗi profile, khung `session` khi phiên đổi, gate đăng nhập tự mở khoá khi nền đăng nhập; MCP server 1.2.0 giữ nhiều kết nối theo instanceId (kết nối lại cùng instance chỉ thay socket của nó), lệnh đi tới trình duyệt sở hữu run, mặc định ưu tiên trình duyệt đã đăng nhập, `webmate_connection` liệt kê, `state.json.connections[]`, lệnh tệp `auth_hint`/`auth_open`. **Workmate:** `sso.ts` (planAuthHints, chooseLoginTarget), `service.ts` tự gửi `auth_hint` với email tài khoản sau mỗi hello chưa đăng nhập (cooldown 10 phút), `openLoginUrl` đưa trang Keycloak (đăng nhập lẫn đăng xuất) vào cửa sổ riêng (CDP `Target.createTarget`) hoặc trình duyệt/profile đã chọn (`open -na … --profile-directory=… URL`), rớt về trình duyệt hệ thống; Cài đặt → Trình duyệt có mục Đăng nhập chung (chọn trình duyệt, công tắc `ssoAutoSignIn`, nút Đăng nhập WebMate, pill từng kết nối); bước onboarding báo đã/đang/chưa đăng nhập; thẻ nhắc `WEBMATE_NOT_SIGNED_IN` mở đăng nhập thẳng; i18n 6 ngôn ngữ; bundle MCP 1.2.0 vendor lại. **Nghiệm thu tại máy (Edge 152 làm cửa sổ riêng):** hello mang instanceId → Workmate gửi `auth_hint` `kien.le@…` trong 2 s → Edge chạy launchWebAuthFlow và trả "Authorization page could not be loaded" vì Keycloak trả 400 `Invalid parameter: redirect_uri` (curl xác nhận; với redirect loopback đã đăng ký, prompt=none trả `error=login_required` đúng như bộ xử lý mong đợi) → **chủ dự án phải thêm `https://pfadeibckkgklmmjghiikadphihbpape.chromiumapp.org/*` vào Valid Redirect URIs của client `agentx-workmate`** thì đường im lặng mới hoàn tất; nút Đăng nhập WebMate → `auth_open` trả `opened` sau 2 s và cửa sổ riêng mở tab "Sign in to Agent X"; Chrome tạm (profile bỏ đi) nạp cùng thư mục cạnh cửa sổ Edge: `connections` = 2 trong 45 s, 0 supersede, `auth_hint` chỉ gửi tới trình duyệt mới, Edge còn nguyên khi Chrome đóng. **Chưa kiểm sống:** đăng nhập im lặng thành công (chờ đăng ký redirect), khung `session` sau khi đăng nhập thật, trang Keycloak mở trong cửa sổ riêng qua luồng đăng nhập thật (đã có unit test + dry-run chọn đích), Windows/Brave. |
...
- Cập nhật ở chế độ cửa sổ đi bằng relaunch (đóng cửa sổ → hoán đổi → mở lại) vì `chrome.runtime.reload()` trên
  ...
  lý do Chrome trả về.
- Phát hành thật 09/09/2026: secret `WEBMATE_RELEASE_SIGNING_KEY` đã thêm; tag `v1.0.3` đặt vào commit bump 1.0.3
  (`7a36fc7fc`) để ghi chú phát hành chỉ gồm commit từ 1.0.3 (không có tag `v1.0.x` thì workflow lấy từ tag upstream
  `v31.0.1`, 272 dòng); workflow `main.yml` từng bị tắt tay (`disabled_manually`) nên phải bật lại trước khi dispatch;
  kho chuyển sang public vì `raw.githubusercontent.com` và asset của release trả 404 khi kho private.
  Feed: `https://raw.githubusercontent.com/astralxkienlt/agentx-webmate/main/release.json` (v1.0.4, chữ ký xác minh được).
- Còn lại cho chủ dự án: kiểm trên Windows thật (spawn với `stdio` 5 phần tử, `windowsHide`, SendKeys của bước dẫn
  cài), thử Brave làm cửa sổ riêng (Edge đã nạp được extension qua cửa sổ riêng theo log 09/09), và Reload/tắt bản dev
  trong Chrome + Edge.

**Ghi chú sau giai đoạn 4:**
- Keycloak: client `agentx-workmate` cần thêm `https://pfadeibckkgklmmjghiikadphihbpape.chromiumapp.org/*` vào Valid
  Redirect URIs (ID cố định của extension; `chrome.identity.getRedirectURL()` trả đúng URL này). Chưa đăng ký thì
  `auth_hint` kết thúc bằng `silent_sign_in_failed` ("Authorization page could not be loaded") và mọi thứ khác vẫn chạy.
- Trả lời của background cho `auth_*` không được mang trường `error`: offscreen bridge coi mọi trả lời có `error` là
  lỗi giao thức và chỉ chuyển chuỗi lỗi lên server (mất `outcome`). Mã lỗi đi trong `code`; server đọc `code` (rớt về
  `error` cho bản cũ).
- Service worker của extension unpacked được trình duyệt **cache theo phiên bản**: hoán đổi thư mục mà giữ nguyên
  `version` (chỉ xảy ra khi thử nghiệm bản cục bộ) thì background vẫn chạy mã cũ tới khi extension được nạp lại với
  phiên bản khác hoặc profile mới; offscreen document thì đọc tệp mới. Bản phát hành thật luôn đổi phiên bản nên bộ cập
  nhật không gặp điều này; khi thử tay hãy xoá `<webmate>/profile` rồi mở lại cửa sổ riêng.
- `auth_open` trả `opened` ngay khi tab Keycloak mở; kết quả đăng nhập về Workmate qua khung `session` (storage
  `agentxAuthSessionV1` đổi → background relay → offscreen gửi `{type:'session'}`), không qua trả lời lệnh.
- Nhiều kết nối: server 1.2.0 giữ mọi socket có hello hợp lệ, khoá `instanceId` (`socket:<n>` khi extension cũ không
  gửi); `active` = đã đăng nhập trước, rồi hello mới nhất; lệnh nêu `runId` quay về trình duyệt đã sinh run đó.
