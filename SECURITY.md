# AgentX Workmate Security Policy

This document describes AgentX Workmate's trust model, names the one
...
- Environment details (`agentx version`, commit SHA, OS, Python
  version).
...
AgentX Workmate is a single-tenant personal agent. Its posture is
layered, and the layers are not equally load-bearing. Reporters and
operators should reason about them in the same terms.

### 2.1 Definitions

- **Agent process.** The Python interpreter running AgentX Workmate,
...
- **Trust envelope.** The set of resources an operator has implicitly
  granted AgentX Workmate access to by running it — typically, whatever
  the operator's own user account can reach on the host.
- **Stance.** An explicit statement in AgentX Workmate's documentation
...
AgentX Workmate supports two OS-level isolation postures. They address
...
AgentX Workmate supports this in two ways:

- **AgentX Workmate's own Docker image and Compose setup.** Lighter-
...
Under a whole-process wrapper, AgentX Workmate's in-process heuristics
...
AgentX Workmate filters the environment it passes to its lower-trust
...
A malicious or buggy plugin is not a vulnerability in AgentX Workmate
itself. Bugs in AgentX Workmate's plugin-install or plugin-discovery
...
4. **Within the authorized set, all callers are equally trusted.**
   AgentX Workmate does not model per-caller capabilities inside a
...
- Trust-model documentation violations: code behaving contrary to
  what this policy, AgentX Workmate's own documentation, or reasonable
  operator expectations would predict — including cases where
  AgentX Workmate has documented a stance about how its output should
...
- **Documented break-glass settings.** Operator-selected trade-offs
  ...
  agentx-home security, and similar. Reports against those
...
- **Community-contributed skills and plugins.** Third-party skills
  ...
  plugins are in the operator's review surface, not AgentX Workmate's
  trust surface (§2.4, §2.5). A skill or plugin doing something
  malicious is the expected failure mode of one that wasn't
  reviewed, not a vulnerability in AgentX Workmate. Bugs in AgentX
...
- AgentX Workmate includes supply-chain guards for MCP server
