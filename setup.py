...
AgentX Workmate (see website/docs/getting-started/platform-support.md). The
...
build sandbox. ``nix/python.nix`` sets ``AGENTX_NIX_BUILD=1`` on the
AgentX package derivation, so only that build may create an artifact.
...
_BLOCK_MESSAGE = (
    ...
    "See: https://github.com/TrungKiencding/AgentX-Workmate/blob/main/website/docs/getting-started/installation.md\n"
    "\n"
    "If you are developing, use an editable install instead:\n"
    "  uv sync          # or: uv pip install -e .\n"
    "\n"
    "If you are building with Nix (uv2nix), this error should not fire —\n"
    "the AgentX Nix derivation sets AGENTX_NIX_BUILD=1. If it does, file a bug."
)
