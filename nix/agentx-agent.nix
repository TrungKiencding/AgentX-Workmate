...
let
  ...
  # lib.nix) — the wrapper exposes them via AGENTX_OPTIONAL_SKILLS, the
  ...
  # to a dev checkout — the loader reads them from AGENTX_BUNDLED_PLUGINS.
  ...
  # at by AGENTX_BUNDLED_LOCALES so the wrapped binary always resolves human
  ...
  # exposed via AGENTX_OPTIONAL_MCPS.
...
stdenv.mkDerivation (finalAttrs: {
  ...
  passthru =
    ...
    in
    {
      ...
      devShellHook = ''
        export AGENTX_PYTHON=${devPython}/bin/python3
      '';
