...
let
    ...
    (callPackage pyproject-nix.build.packages {
      ...
      (
        lib.composeManyExtensions [
          ...
          # AgentX derivation. This is deliberately a derivation environment
          # variable, not a devShell variable: ``nix develop -c uv build``
          # must remain blocked.
          (final: prev: {
            agentx-agent = prev.agentx-agent.overrideAttrs (_old: {
              AGENTX_NIX_BUILD = "1";
            });
          })
        ]
  ...
  # $AGENTX_PYTHON_SRC_ROOT at runtime.
  workspaceRoot = ./..;
  ...
  editableOverlay = editableWorkspace.mkEditablePyprojectOverlay {
    root = "$AGENTX_PYTHON_SRC_ROOT"; # resolved at shellHook time
  };
  ...
  editableSet = pythonSet.overrideScope (
    lib.composeManyExtensions [
      editableOverlay
      (final: prev: {
        agentx-agent = prev.agentx-agent.overrideAttrs (old: {
...
in
{
  venv = pythonSet.mkVirtualEnv "agentx-agent-env" {
    agentx-agent = dependency-groups;
  };
  editableVenv = editableSet.mkVirtualEnv "agentx-agent-editable-env" {
    agentx-agent = dependency-groups;
  };
}
