# nix/nixosModules.nix — NixOS module for agentx-agent
#
# Two modes:
#   container.enable = false (default) → native systemd service
#   container.enable = true            → OCI container (persistent writable layer)
#
# Container mode: agentx runs from /nix/store bind-mounted read-only into a
...
# container recreation. Environment variables are written to $AGENTX_HOME/.env
# and read by agentx at startup — no container recreation needed for env changes.
#
# Tool resolution: the agentx wrapper uses --suffix PATH for nix store tools,
...
{ inputs, ... }: {
  ...
  let
    cfg = config.services.agentx-agent;
    effectivePackage =
    ...
    agentx-agent = inputs.self.packages.${pkgs.stdenv.hostPlatform.system}.default;

    # Deep-merge config type (from 0xrsydn/nix-agentx-agent)
    deepConfigType = lib.types.mkOptionType {
      name = "agentx-config-attrs";
      description = "AgentX YAML config (attrset), merged deeply via lib.recursiveUpdate.";
      check = builtins.isAttrs;
      merge = _loc: defs: lib.foldl' lib.recursiveUpdate { } (map (d: d.value) defs);
    };

    # Generate config.yaml from Nix attrset (YAML is a superset of JSON).
    # terminal.cwd replaces the deprecated MESSAGING_CWD env var — agentx
    ...
    generatedConfigFile = pkgs.writeText "agentx-config.yaml" configJson;
    configFile = if cfg.configFile != null then cfg.configFile else generatedConfigFile;

    configMergeScript = pkgs.callPackage ./configMergeScript.nix { };

    # config.yaml mode: group-writable (0660) when interactive users share this
    # AGENTX_HOME via addToSystemPackages, so they can save settings through the
    ...
    documentDerivation = pkgs.runCommand "agentx-documents" { } (
      ''
        mkdir -p $out
      '' + lib.concatStringsSep "\n" (
        lib.mapAttrsToList (name: value:
          ...
          else "cat > $out/${name} <<'AGENTX_DOC_EOF'\n${value}\nAGENTX_DOC_EOF"
        ) cfg.documents
      )
    );

    containerName = "agentx-agent";
    containerDataDir = "/data";     # stateDir mount point inside container
    containerHomeDir = "/home/agentx";
    ...
    # agentx user + sudo on first boot (writable layer persists), then
    # drops privileges. Supports arbitrary base images (Debian, Alpine, etc).
    containerEntrypoint = pkgs.writeShellScript "agentx-container-entrypoint" ''
      set -eu

      AGENTX_UID="''${AGENTX_UID:?AGENTX_UID must be set}"
      AGENTX_GID="''${AGENTX_GID:?AGENTX_GID must be set}"

      # ── Group: ensure a group with GID=$AGENTX_GID exists ──
      # Check by GID (not name) to avoid collisions with pre-existing groups
      # (e.g. GID 100 = "users" on Ubuntu)
      EXISTING_GROUP=$(getent group "$AGENTX_GID" 2>/dev/null | cut -d: -f1 || true)
      if [ -n "$EXISTING_GROUP" ]; then
        GROUP_NAME="$EXISTING_GROUP"
      else
        GROUP_NAME="agentx"
        if command -v groupadd >/dev/null 2>&1; then
          groupadd -g "$AGENTX_GID" "$GROUP_NAME"
        elif command -v addgroup >/dev/null 2>&1; then
          addgroup -g "$AGENTX_GID" "$GROUP_NAME" 2>/dev/null || true
        fi
      fi

      # ── User: ensure a user with UID=$AGENTX_UID exists ──
      PASSWD_ENTRY=$(getent passwd "$AGENTX_UID" 2>/dev/null || true)
      if [ -n "$PASSWD_ENTRY" ]; then
        TARGET_USER=$(echo "$PASSWD_ENTRY" | cut -d: -f1)
        TARGET_HOME=$(echo "$PASSWD_ENTRY" | cut -d: -f6)
      else
        TARGET_USER="agentx"
        TARGET_HOME="/home/agentx"
        if command -v useradd >/dev/null 2>&1; then
          useradd -u "$AGENTX_UID" -g "$AGENTX_GID" -m -d "$TARGET_HOME" -s /bin/bash "$TARGET_USER"
        elif command -v adduser >/dev/null 2>&1; then
          adduser -u "$AGENTX_UID" -D -h "$TARGET_HOME" -s /bin/sh -G "$GROUP_NAME" "$TARGET_USER" 2>/dev/null || true
        fi
      fi
      mkdir -p "$TARGET_HOME"
      chown "$AGENTX_UID:$AGENTX_GID" "$TARGET_HOME"
      chmod 0750 "$TARGET_HOME"

      # Ensure AGENTX_HOME is owned by the target user.
      ...
      if [ -n "''${AGENTX_HOME:-}" ] && [ -d "$AGENTX_HOME" ]; then
        find "$AGENTX_HOME" \! -user "$AGENTX_UID" -exec chown "$AGENTX_UID:$AGENTX_GID" {} +
      fi
      ...
      if [ ! -f /var/lib/agentx-tools-provisioned ] && command -v apt-get >/dev/null 2>&1; then
        ...
        touch /var/lib/agentx-tools-provisioned
      fi

      if command -v sudo >/dev/null 2>&1 && [ ! -f /etc/sudoers.d/agentx ]; then
        mkdir -p /etc/sudoers.d
        echo "$TARGET_USER ALL=(ALL) NOPASSWD:ALL" > /etc/sudoers.d/agentx
        chmod 0440 /etc/sudoers.d/agentx
      fi
      ...
      if command -v setpriv >/dev/null 2>&1; then
        exec setpriv --reuid="$AGENTX_UID" --regid="$AGENTX_GID" --init-groups "$@"
    ...
    # so they can update without recreation. Env vars go through $AGENTX_HOME/.env.
    ...
    # Default: /var/lib/agentx/workspace → /data/workspace.
  ...
  in {
    options.services.agentx-agent = with lib; {
      enable = mkEnableOption "AgentX Workmate gateway service";

      # ── Package ──────────────────────────────────────────────────────────
      package = mkOption {
        type = types.package;
        default = agentx-agent;
        description = "The agentx-agent package to use.";
      };
      ...
      stateDir = mkOption {
        type = types.str;
        default = "/var/lib/agentx";
        description = "State directory. Contains .agentx/ subdir (AGENTX_HOME).";
      };
      ...
      settings = mkOption {
        ...
        default = { };
        description = ''
          Declarative AgentX config (attrset). Deep-merged across module
      ...
      environmentFiles = mkOption {
        ...
        description = ''
          ...
          Contents are merged into $AGENTX_HOME/.env at activation time.
          AgentX reads this file on every startup via load_hermes_dotenv().
        '';
      };
      ...
      environment = mkOption {
        ...
        default = { };
        description = ''
          Non-secret environment variables. Merged into $AGENTX_HOME/.env
          ...
          options = {
            ...
            auth = mkOption {
              ...
              description = ''
                ...
                (remote MCP servers). Tokens are stored in $AGENTX_HOME/mcp-tokens/.
              '';
            };
      ...
      extraArgs = mkOption {
        type = types.listOf types.str;
        default = [ ];
        description = "Extra command-line arguments for `agentx gateway`.";
      };
      ...
      extraPackages = mkOption {
        ...
        description = ''
          ...
          Implemented via the agentx user's per-user profile
      ...
      extraPlugins = mkOption {
        ...
        default = [ ];
        description = ''
          Directory-based plugin packages to symlink into the agentx plugins
          directory. Each package should contain a plugin.yaml and __init__.py
          at its root. AgentX discovers these automatically on startup.
        '';
        example = literalExpression ''
          [
      ...
      extraPythonPackages = mkOption {
        ...
        description = ''
          ...
          with the same Python interpreter as agentx (python312).
        '';
        example = literalExpression ''
          [
      ...
      extraDependencyGroups = mkOption {
        ...
        description = ''
          ...
          Use this for optional extras already declared in agentx-agent's
      ...
      addToSystemPackages = mkOption {
        ...
        default = false;
        description = ''
          Add the agentx CLI to environment.systemPackages and export
          AGENTX_HOME system-wide (via environment.variables) so interactive
      ...
      container = {
        ...
        hostUsers = mkOption {
          ...
          default = [ ];
          description = ''
            Interactive users who get a ~/.agentx symlink to the service
            stateDir. These users are automatically added to the agentx group.
          '';
          example = [ "sidbin" ];
        };
      };
    ...
    config = lib.mkIf cfg.enable (lib.mkMerge [
      ...
      (lib.mkIf (cfg.mcpServers != { }) {
        services.agentx-agent.settings.mcp_servers = lib.mapAttrs (_name: srv:
          # Stdio transport
      ...
      # ── Host CLI ──────────────────────────────────────────────────────
      # Add the agentx CLI to system PATH and export AGENTX_HOME system-wide
      # so interactive shells share state (sessions, skills, cron) with the
      # gateway service instead of creating a separate ~/.agentx/.
      (lib.mkIf cfg.addToSystemPackages {
        environment.systemPackages = [ effectivePackage ];
        environment.variables.AGENTX_HOME = "${cfg.stateDir}/.agentx";
      })
      ...
      # ── Assertions ─────────────────────────────────────────────────────
      {
        ...
        in [{
          ...
          message = "services.agentx-agent.extraPlugins: duplicate plugin names detected: ${toString names}. If using fetchFromGitHub, set name = \"plugin-name\" to disambiguate.";
        }];
      }

      # ── Warnings ──────────────────────────────────────────────────────
      # ── Per-user profile for extraPackages ───────────────────────────
      # Wire extraPackages into the agentx user's per-user profile so the
      ...
      (lib.mkIf (cfg.container.enable && !cfg.addToSystemPackages && cfg.container.hostUsers != []) {
        warnings = [
          ''
            services.agentx-agent: container.enable is true and container.hostUsers
            is set, but addToSystemPackages is false. Without a host-installed agentx
            binary, container routing will not work for interactive users.
            Set addToSystemPackages = true or ensure agentx is on PATH.
          ''
        ];
      })
      ...
      # ── Directories ───────────────────────────────────────────────────
      {
        systemd.tmpfiles.rules = [
          ...
          "d ${cfg.stateDir}/.agentx        2770 ${cfg.user} ${cfg.group} - -"
          "d ${cfg.stateDir}/.agentx/cron   2770 ${cfg.user} ${cfg.group} - -"
          "d ${cfg.stateDir}/.agentx/sessions 2770 ${cfg.user} ${cfg.group} - -"
          "d ${cfg.stateDir}/.agentx/logs   2770 ${cfg.user} ${cfg.group} - -"
          "d ${cfg.stateDir}/.agentx/memories 2770 ${cfg.user} ${cfg.group} - -"
          "d ${cfg.stateDir}/.agentx/plugins 2770 ${cfg.user} ${cfg.group} - -"
      ...
      {
        system.activationScripts."agentx-agent-setup" = lib.stringAfter ([ "users" ] ++ lib.optional (config.system.activationScripts ? setupSecrets) "setupSecrets") ''
          # Ensure directories exist (activation runs before tmpfiles)
          mkdir -p ${cfg.stateDir}/.agentx
          mkdir -p ${cfg.stateDir}/home
          mkdir -p ${cfg.workingDirectory}
          chown ${cfg.user}:${cfg.group} ${cfg.stateDir} ${cfg.stateDir}/.agentx ${cfg.stateDir}/home ${cfg.workingDirectory}
          chmod 2770 ${cfg.stateDir} ${cfg.stateDir}/.agentx ${cfg.workingDirectory}
          ...
          find ${cfg.stateDir}/.agentx -maxdepth 1 \
            \( -name "*.db" -o -name "*.db-wal" -o -name "*.db-shm" -o -name "SOUL.md" \) \
            -exec chmod g+rw {} + 2>/dev/null || true
          for _subdir in cron sessions logs memories plugins; do
            mkdir -p "${cfg.stateDir}/.agentx/$_subdir"
            chown ${cfg.user}:${cfg.group} "${cfg.stateDir}/.agentx/$_subdir"
            chmod 2770 "${cfg.stateDir}/.agentx/$_subdir"
            find "${cfg.stateDir}/.agentx/$_subdir" -type f \
              -exec chmod g+rw {} + 2>/dev/null || true
          done
          ...
          # agentx-group users can save settings via the CLI/TUI, else 0640).
          ${if cfg.configFile != null then ''
            install -o ${cfg.user} -g ${cfg.group} -m ${configYamlMode} -D ${configFile} ${cfg.stateDir}/.agentx/config.yaml
          '' else ''
            ${configMergeScript} ${generatedConfigFile} ${cfg.stateDir}/.agentx/config.yaml
            chown ${cfg.user}:${cfg.group} ${cfg.stateDir}/.agentx/config.yaml
            chmod ${configYamlMode} ${cfg.stateDir}/.agentx/config.yaml
          ''}

          # Managed mode marker (so interactive shells also detect NixOS management)
          touch ${cfg.stateDir}/.agentx/.managed
          chown ${cfg.user}:${cfg.group} ${cfg.stateDir}/.agentx/.managed
          chmod 0644 ${cfg.stateDir}/.agentx/.managed
          ...
          ${if cfg.container.enable then ''
            cat > ${cfg.stateDir}/.agentx/.container-mode <<'AGENTX_CONTAINER_MODE_EOF'
    # Written by NixOS activation script. Do not edit manually.
    backend=${cfg.container.backend}
    container_name=${containerName}
    exec_user=${cfg.user}
    hermes_bin=${containerDataDir}/current-package/bin/agentx
    AGENTX_CONTAINER_MODE_EOF
            chown ${cfg.user}:${cfg.group} ${cfg.stateDir}/.agentx/.container-mode
            chmod 0644 ${cfg.stateDir}/.agentx/.container-mode
          '' else ''
            rm -f ${cfg.stateDir}/.agentx/.container-mode

            # Remove symlink bridge for hostUsers
            ${lib.concatStringsSep "\n" (map (user:
              let
                userHome = config.users.users.${user}.home;
                symlinkPath = "${userHome}/.agentx";
              in ''
                if [ -L "${symlinkPath}" ] && [ "$(readlink "${symlinkPath}")" = "${cfg.stateDir}/.agentx" ]; then
                  rm -f "${symlinkPath}"
                  echo "agentx-agent: removed symlink ${symlinkPath}"
                fi
              '') cfg.container.hostUsers)}
          ''}

          # ── Symlink bridge for interactive users ───────────────────────
          # Create ~/.agentx -> stateDir/.agentx for each hostUser so the
          ...
          ${lib.optionalString cfg.container.enable
            (lib.concatStringsSep "\n" (map (user:
              let
                userHome = config.users.users.${user}.home;
                symlinkPath = "${userHome}/.agentx";
                target = "${cfg.stateDir}/.agentx";
              in ''
                if [ -d "${symlinkPath}" ] && [ ! -L "${symlinkPath}" ]; then
                  # Real directory — back it up, then create symlink.
                  # (ln -sfn cannot atomically replace a directory.)
                  _backup="${symlinkPath}.bak.$(date +%s)"
                  echo "agentx-agent: backing up existing ${symlinkPath} to $_backup"
          ...
          ${lib.optionalString (cfg.authFile != null) ''
            ${if cfg.authFileForceOverwrite then ''
              install -o ${cfg.user} -g ${cfg.group} -m 0600 ${cfg.authFile} ${cfg.stateDir}/.agentx/auth.json
            '' else ''
              if [ ! -f ${cfg.stateDir}/.agentx/auth.json ]; then
                install -o ${cfg.user} -g ${cfg.group} -m 0600 ${cfg.authFile} ${cfg.stateDir}/.agentx/auth.json
              fi
            ''}
          ''}
          ...
          # AgentX reads $AGENTX_HOME/.env at startup via load_hermes_dotenv(),
          # so this is the single source of truth for both native and container mode.
          ${lib.optionalString (cfg.environment != {} || cfg.environmentFiles != []) ''
            ENV_FILE="${cfg.stateDir}/.agentx/.env"
            install -o ${cfg.user} -g ${cfg.group} -m 0640 /dev/null "$ENV_FILE"
            cat > "$ENV_FILE" <<'AGENTX_NIX_ENV_EOF'
    ${envFileContent}
    AGENTX_NIX_ENV_EOF
            ${lib.concatStringsSep "\n" (map (f: ''
              if [ -f "${f}" ]; then
        ...
        find ${cfg.stateDir}/.agentx/plugins -maxdepth 1 -type l -name 'nix-managed-*' -delete 2>/dev/null || true
        ...
        ${lib.concatStringsSep "\n" (map (plugin:
          ...
          in ''
            ...
            ln -sfn ${plugin} ${cfg.stateDir}/.agentx/plugins/nix-managed-${name}
            chown -h ${cfg.user}:${cfg.group} ${cfg.stateDir}/.agentx/plugins/nix-managed-${name}
          '') cfg.extraPlugins)}
        '';
      }
      ...
      (lib.mkIf (!cfg.container.enable) {
        systemd.services.agentx-agent = {
          description = "AgentX Workmate Gateway";
          ...
          environment = {
            HOME = cfg.stateDir;
            AGENTX_HOME = "${cfg.stateDir}/.agentx";
            AGENTX_MANAGED = "true";
          ...
          serviceConfig = {
            ...
            # $AGENTX_HOME/.env by the activation script. load_hermes_dotenv()
            # reads them at Python startup — no systemd EnvironmentFile needed.

            ExecStart = lib.concatStringsSep " " ([
              "${effectivePackage}/bin/agentx"
              "gateway"
            ] ++ cfg.extraArgs);
            ...
            # so interactive users in the agentx group can read/write them.
            UMask = "0007";
      ...
      (lib.mkIf cfg.container.enable {
        ...
        systemd.services.agentx-agent = {
          description = "AgentX Workmate Gateway (container)";
          wantedBy = [ "multi-user.target" ];
          after = [ "network-online.target" ]
          ...
          preStart = ''
            ...
            if [ "$NEED_CREATE" = "true" ]; then
              ...
              AGENTX_UID=$(${pkgs.coreutils}/bin/id -u ${cfg.user})
              AGENTX_GID=$(${pkgs.coreutils}/bin/id -g ${cfg.user})
              ...
              ${containerBin} create \
                ...
                --env AGENTX_UID="$AGENTX_UID" \
                --env AGENTX_GID="$AGENTX_GID" \
                --env AGENTX_HOME=${containerDataDir}/.agentx \
                --env AGENTX_MANAGED=true \
                --env HOME=${containerHomeDir} \
                ${lib.concatStringsSep " " cfg.container.extraOptions} \
                ${cfg.container.image} \
                ${containerDataDir}/current-package/bin/agentx gateway run --replace ${lib.concatStringsSep " " cfg.extraArgs}
