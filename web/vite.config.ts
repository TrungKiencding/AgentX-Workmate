...
const BACKEND = process.env.AGENTX_DASHBOARD_URL ?? "http://127.0.0.1:9119";

/**
 * In production the Python `agentx dashboard` server injects a one-shot
 * session token into `index.html` (see `hermes_cli/web_server.py`). The
 * Vite dev server serves its own `index.html`, so unless we forward that
 * token, every protected `/api/*` call 401s.
 *
 * This plugin fetches the running dashboard's `index.html` on each dev page
 * load, scrapes the `window.__AGENTX_SESSION_TOKEN__` assignment, and
 * re-injects it into the dev HTML. No-op in production builds.
 */
function hermesDevToken(): Plugin {
  const TOKEN_RE = /window\.__AGENTX_SESSION_TOKEN__\s*=\s*"([^"]+)"/;
  const EMBEDDED_RE =
  ...
  return {
    ...
    async transformIndexHtml() {
      try {
        ...
        if (!match) {
          console.warn(
            `[agentx] Could not find session token in ${BACKEND} — ` +
              `is \`agentx dashboard\` running? /api calls will 401.`,
          );
          return;
        }
        ...
        return [
          {
            ...
            injectTo: "head",
            children:
              `window.__AGENTX_SESSION_TOKEN__="${match[1]}";` +
              `window.__AGENTX_DASHBOARD_EMBEDDED_CHAT__=${embeddedJs};`,
          },
        ];
      } catch (err) {
        console.warn(
          `[agentx] Dashboard at ${BACKEND} unreachable — ` +
            `start it with \`agentx dashboard\` or set AGENTX_DASHBOARD_URL. ` +
...
export default defineConfig({
  ...
  resolve: {
    alias: {
      ...
      "@agentx/shared": path.resolve(__dirname, "../apps/shared/src"),
    },
  ...
  server: {
    proxy: {
      ...
      // Same host as `agentx dashboard` must serve these; Vite has no
