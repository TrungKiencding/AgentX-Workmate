...
import { api, AGENTX_BASE_PATH } from "@/lib/api";
...
export function usePlugins() {
  ...
  useEffect(() => {
    ...
    for (const manifest of manifests) {
      ...
      if (manifest.css) {
        const cssUrl = `${AGENTX_BASE_PATH}/dashboard-plugins/${manifest.name}/${manifest.css}`;
      ...
      const baseUrl = `${AGENTX_BASE_PATH}/dashboard-plugins/${manifest.name}/${manifest.entry}`;
      ...
      script.setAttribute("data-agentx-plugin", manifest.name);
