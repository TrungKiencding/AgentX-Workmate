/**
 * ChatPage — embeds `agentx --tui` inside the dashboard.
 *
...
const PTY_ATTACH_TOKEN_KEY = "agentx.pty.token.chat";
...
export default function ChatPage({ isActive = true }: { isActive?: boolean }) {
  ...
  const [banner, setBanner] = useState<string | null>(() =>
    ...
    !window.__AGENTX_SESSION_TOKEN__ &&
    !window.__AGENTX_AUTH_REQUIRED__
      ? "Session token unavailable. Open this page through `agentx dashboard`, not directly."
      : null,
  );
  ...
  useEffect(() => {
    ...
    const gated = !!window.__AGENTX_AUTH_REQUIRED__;
    ...
    const term = new Terminal({
      ...
      // even when the inner AgentX TUI has enabled xterm mouse-events
    ...
    //      payloads upload to AGENTX_HOME/images then drive `/image`.
    //
    ...
    // must upload browser bytes to AGENTX_HOME/images, then drive `/image`
    ...
    if (useWebgl) {
      try {
        ...
        console.warn(
          "[agentx-chat] WebGL renderer unavailable; falling back to default",
          err,
        );
      }
    }
    ...
    // WebSocket. In gated mode (``window.__AGENTX_AUTH_REQUIRED__``) this
    ...
    void (async () => {
      ...
      // Profile-scoped chat: the PTY child gets AGENTX_HOME pointed at the
    ...
    // forwarding them into AgentX. Keyboard input, paste, and resize still
    // behave normally.
    ...
    <div className="flex min-h-0 flex-1 flex-col gap-2">
      ...
      <div className="flex min-h-0 flex-1 flex-col gap-2 lg:flex-row lg:gap-3">
        ...
        >
          <div
            ref={hostRef}
            className="agentx-chat-xterm-host min-h-0 min-w-0 flex-1"
          />
