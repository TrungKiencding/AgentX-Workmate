...
export function HermesConsoleModal({ open, onClose }: HermesConsoleModalProps) {
  ...
  const promptRef = useRef("agentx> ");
  const inputPromptRef = useRef("agentx> ");
  ...
  const handleFrame = useCallback(
    (frame: ConsoleFrame) => {
      ...
      if (frame.type === "ready") {
        const nextPrompt = frame.prompt || "agentx> ";
  ...
  useEffect(() => {
    ...
    writeLine(term, "\x1b[2mConnecting to AgentX Console...\x1b[0m");
