...
class HostSupervisor:
    ...
    def _spawn_locked(self, *, reason: str) -> None:
        ...
        if self.env:
            env.update(self.env)
        env["AGENTX_COMPUTE_HOST_HEARTBEAT_SECS"] = str(self.heartbeat_secs)
    ...
    def _validate_hello(self) -> None:
        ...
        if got_home and got_home != self.expected_hermes_home:
            raise RuntimeError(f"compute host AGENTX_HOME mismatch: {got_home} != {self.expected_hermes_home}")
