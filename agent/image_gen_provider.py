...
as ``kind: backend``) or ``~/.agentx/plugins/image_gen/<name>/`` (user, opt-in
via ``plugins.enabled``).
...
class ImageGenProvider(abc.ABC):
    ...
    def capabilities(self) -> Dict[str, Any]:
        ...
        model knows when ``image_url`` is honored. Used by ``agentx tools``
...
def _images_cache_dir() -> Path:
    """Return ``$AGENTX_HOME/cache/images/``, creating parents as needed."""
...
def save_b64_image(
    ...
    *,
    ...
    """Decode base64 image data and write it under ``$AGENTX_HOME/cache/images/``.
...
def save_url_image(
    ...
    *,
    ...
    """Download an image URL and write it under ``$AGENTX_HOME/cache/images/``.
