...
- ``manual`` — never install; if a binary is missing, the server is
  silently skipped and the user is told about it via ``agentx lsp
  status``.
...
# ``<AGENTX_HOME>/lsp/bin/`` first, then on PATH.
#
# Optional fields:
...
INSTALL_RECIPES: Dict[str, Dict[str, Any]] = {
    ...
    # `agentx lsp status` reports the host's presence.
...
def hermes_lsp_bin_dir() -> Path:
    """Return the AgentX-owned bin staging dir for LSP servers."""
...
def _install_npm(
    ...
    """
    # Managed npm first: $AGENTX_HOME/node is not on an arbitrary process's
    # PATH, so a bare which() misses the Node that AgentX installed and
    ...
    if npm is None:
        ...
        return None
    staging = hermes_lsp_bin_dir().parent  # <AGENTX_HOME>/lsp/
...
def detect_status(pkg: str) -> str:
    ...
    Used by the ``agentx lsp status`` CLI to give users a quick
