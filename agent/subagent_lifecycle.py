"""Public, plugin-safe lifecycle API for delegated AgentX subagents.
...
_EXECUTOR = _DaemonExecutor(max_workers=8, thread_name_prefix="agentx-lifecycle")
...
class SubagentLifecycleService:
    ...
    def launch(self, request: SubagentLaunchRequest) -> SubagentHandle:
        ...
        if parent is None:
            raise SubagentLifecycleError(
                "No active AgentX parent session is available."
            )
        ...
        if not subagent_id:
            raise SubagentLifecycleError("AgentX failed to assign a child identity.")
        created = time.time()
        handle = SubagentHandle(
    ...
    def _validate_request(request: SubagentLaunchRequest, parent: Any) -> None:
        ...
        if request.working_directory is not None:
            raise SubagentLifecycleError(
                "working_directory is not supported because AgentX delegates use isolated task environments."
            )
        if request.blocked_tools:
            raise SubagentLifecycleError(
                "Per-tool blocking is not supported; use allowed_toolsets. AgentX always blocks unsafe child tools."
            )
        try:
            metadata_bytes = len(
