"""Preventive SSL CA certificate checks for AgentX Workmate.
...
def _skip_ssl_guard_enabled() -> bool:
    return os.getenv("AGENTX_SKIP_SSL_GUARD", "").strip().lower() in _SKIP_VALUES


def _repair_hint() -> str:
    return (
        "Repair: run `agentx doctor --fix` (auto-reinstalls certifi), or "
...
def verify_ca_bundle() -> None:
    ...
    if _skip_ssl_guard_enabled():
        logger.debug("SSL CA bundle guard skipped via AGENTX_SKIP_SSL_GUARD")
        return
