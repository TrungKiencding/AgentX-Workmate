...
* First-use consent is gated by the allowlist under
  ``~/.agentx/shell-hooks-allowlist.json``.  Non-TTY callers must pass
  ``accept_hooks=True`` (resolved from ``--accept-hooks``,
  ``AGENTX_ACCEPT_HOOKS``, or ``hooks_auto_accept: true`` in config)
...
**stdout** (JSON, optional — anything else is ignored)::
    ...
    {"action":   "block", "message": "Forbidden command"}   # AgentX-canonical
...
def register_from_config(
    ...
    *,
    ...
    setting.  ``AGENTX_ACCEPT_HOOKS=1`` and ``hooks_auto_accept: true`` are
    ...
    # Safe mode (--safe-mode / AGENTX_SAFE_MODE=1): shell hooks are user
    ...
    if env_var_enabled("AGENTX_SAFE_MODE"):
        logger.info("AGENTX_SAFE_MODE=1 — shell-hook registration skipped")
        return []
        ...
        if not already_allowlisted:
            if not _prompt_and_record(
                ...
                logger.warning(
                    ...
                    "Use --accept-hooks / AGENTX_ACCEPT_HOOKS=1 / "
...
def iter_configured_hooks(cfg: Optional[Dict[str, Any]]) -> List[ShellHookSpec]:
    ...
    registering anything.  Used by ``agentx hooks list`` and ``doctor``."""
    if not isinstance(cfg, dict):
        return []
...
def _parse_response(event: str, stdout: str) -> Optional[Dict[str, Any]]:
    ...
    if event == "pre_verify":
        # "continue" (AgentX) / "block" (Claude-Code Stop: block the stop) both
...
def save_allowlist(data: Dict[str, Any]) -> None:
    ...
    try:
        ...
        logger.warning(
            ...
            "runs without --accept-hooks / AGENTX_ACCEPT_HOOKS).",
            p, exc,
        )
...
def _prompt_and_record(
    ...
    print(
        f"\n⚠ AgentX is about to register a shell hook that will run a\n"
...
def _resolve_effective_accept(
    ...
    Precedence (any truthy source flips us on):
      ...
      2. ``AGENTX_ACCEPT_HOOKS`` env var
      3. ``hooks_auto_accept: true`` in ``cli-config.yaml``
    """
    if accept_hooks_arg:
        return True
    env = os.environ.get("AGENTX_ACCEPT_HOOKS", "").strip().lower()
...
# ---------------------------------------------------------------------------
# Introspection (used by `agentx hooks` CLI)
# ---------------------------------------------------------------------------
...
def run_once(
    ...
    Used by ``agentx hooks test`` and ``agentx hooks doctor``.
    ...
    produce — otherwise scripts tested via ``agentx hooks test`` could
    diverge silently from production behaviour.

    Returns the :func:`_spawn` diagnostic dict plus a ``parsed`` field
    holding the canonical AgentX-wire-shape response — including exit-code-2
    blocking and ``fail_closed`` semantics, so what ``agentx hooks test``
