"""Coding-context awareness — base AgentX, every interactive surface.

When the user runs AgentX inside a code workspace (CLI, TUI, desktop app, or an
editor over ACP), AgentX shifts into a **coding posture**. This module is the
...
# in _AGENTX_CORE_TOOLS, so they're present on every surface this fires on.
...
class RuntimeMode:
    ...
    def toolset_selection(self, config: Optional[dict[str, Any]] = None) -> Optional[list[str]]:
        ...
        selection (``--toolsets``, ``AGENTX_TUI_TOOLSETS``, …); they never
...
def is_coding_context(
    *,
    ...
    """Whether AgentX should operate in its coding posture right now."""
