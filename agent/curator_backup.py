...
A pre-run snapshot of ``~/.agentx/skills/`` (excluding ``.curator_backups/``
itself) is taken before any mutating curator pass. Snapshots are tar.gz
files under ``~/.agentx/skills/.curator_backups/<utc-iso>/`` with a
...
``~/.agentx/cron/jobs.json`` as ``cron-jobs.json`` when it exists. Cron
...
def _cron_jobs_file() -> Path:
    """Source path for the live cron jobs store (``~/.agentx/cron/jobs.json``)."""
...
def snapshot_skills(reason: str = "manual", *, protect_ids: Optional[Set[str]] = None) -> Optional[Path]:
    """Create a tar.gz snapshot of ``~/.agentx/skills/`` and prune old ones.
    ...
    if not skills.exists():
        logger.debug("No ~/.agentx/skills/ directory — nothing to back up")
        return None
...
def rollback(backup_id: Optional[str] = None) -> Tuple[bool, str, Optional[Path]]:
    """Restore ``~/.agentx/skills/`` from a snapshot.
    ...
    Strategy:
      ...
      4. Extract the chosen snapshot into ``~/.agentx/skills/``.
    ...
    if target is None:
        return (
            ...
            + " (use `agentx curator rollback --list` to see available snapshots)",
            None,
        )
