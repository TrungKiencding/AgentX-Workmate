...
* The ``iron-proxy`` binary is auto-installed into ``<hermes_home>/bin/iron-proxy``
  on first use.  AgentX pins one upstream version (``_IRON_PROXY_VERSION``)
...
* A long-lived CA at ``<hermes_home>/proxy/ca.{crt,key}`` is generated on
  first ``agentx egress setup``.  Sandboxes trust this CA so iron-proxy can
...
* The proxy runs as a managed subprocess (``agentx egress start``), pidfile
...
# request needs the bearer key below.  ``agentx egress reload`` is the
# client.
#
...
_MGMT_API_KEY_ENV = "AGENTX_IRON_PROXY_MGMT_KEY"
...
# credential (AgentX' auth.py keys Google on both GEMINI_API_KEY and
...
def _proxy_state_dir_ro() -> Path:
    ...
    this — there's no reason to materialize ``~/.agentx/proxy/`` just to
...
def install_iron_proxy(*, force: bool = False) -> Path:
    ...
    these; the user-facing ``agentx proxy install`` surface lets them
    ...
    with tempfile.TemporaryDirectory(prefix="agentx-iron-proxy-") as tmpdir:
        tmp = Path(tmpdir)
...
def _http_download(url: str, dest: Path) -> None:
    req = urllib.request.Request(url, headers={"User-Agent": "agentx-agent"})
    try:
...
def iron_proxy_version(binary: Path) -> str:
    ...
    # Don't cache empty output — that would poison ``agentx egress
...
def ensure_ca_cert(*, force: bool = False) -> Tuple[Path, Path]:
    ...
    with tempfile.TemporaryDirectory(prefix="agentx-proxy-ca-") as tmpdir:
        tmp = Path(tmpdir)
        tmp_key = tmp / "ca.key"
        tmp_crt = tmp / "ca.crt"
...
def mint_proxy_token(prefix: str = "agentx-proxy") -> str:
...
def ensure_management_token(*, force: bool = False) -> str:
    ...
    The daemon receives it via the ``AGENTX_IRON_PROXY_MGMT_KEY`` env var
    (named in the generated config's ``management.api_key_env``);
    ``agentx egress reload`` reads the same file to authenticate.
    """
    ...
    if not force and p.exists():
        try:
            ...
            pass
    token = mint_proxy_token(prefix="agentx-mgmt")
    fd = os.open(
        str(p),
...
def reload_proxy() -> bool:
    ...
    if not pid or not _pid_alive(pid):
        raise RuntimeError(
            ...
            "Run `agentx egress start`."
        )
    mgmt = _read_management_listen_from_config()
    if mgmt is None:
        raise RuntimeError(
            "The generated proxy.yaml has no management listener (written "
            "before reload support).  Re-run `agentx egress setup` and use "
            "`agentx egress restart` this one time."
        )
    token = _read_management_token()
    if not token:
        raise RuntimeError(
            "management.token is missing — re-run `agentx egress setup`, "
            "then `agentx egress restart`."
        )
...
def build_proxy_config(
    ...
    * ``secrets`` transform takes ``secrets:`` (plural), each with a
    ...
    return {
        ...
        # in ``ProxyStatus`` / ``agentx egress status``.
        "metrics": {
            "listen": "127.0.0.1:0",
        },
        ...
        # pipeline — `agentx egress reload` applies allowlist/token/mapping
...
def discover_uncovered_providers(
    *,
    ...
    The wizard and ``agentx egress status`` use this to print a warning.
...
def merge_mappings(
    *,
    ...
    re-running ``agentx egress setup`` should not invalidate the tokens
...
# arbitrary env value).
_AGENTX_IRON_PROXY_NONCE_ENV = "AGENTX_IRON_PROXY_NONCE"
    ...
    if nonce_candidates:
        try:
            ...
            for nonce in nonce_candidates:
                needle = f"{_AGENTX_IRON_PROXY_NONCE_ENV}={nonce}".encode()
...
def start_proxy(
    *,
    ...
    if bin_path is None:
        raise RuntimeError(
            "iron-proxy binary not available — run `agentx egress install`."
        )
    ...
    env[_AGENTX_IRON_PROXY_NONCE_ENV] = _proxy_nonce
    ...
    # close-on-success path, every restart leaked one fd in the AgentX
    # process.
    #
    ...
    # ``agentx egress stop`` can clean up the orphan.  Failure paths
    ...
    # hits Ctrl-C while waiting for ``agentx egress start`` doesn't leak
    ...
    try:
        ...
        if existing_pid and _pid_alive(existing_pid):
            raise RuntimeError(
                ...
                f"Run `agentx egress stop` if that proxy is stuck."
            )
            ...
            if access_token and project_id:
                ...
                if missing:
                    ...
                    if not (bitwarden_config or {}).get("allow_env_fallback"):
                        raise RuntimeError(
                            ...
                            f"`agentx egress setup --no-bitwarden`, or set "
                ...
                if warnings:
                    logger.warning(
                        ...
                        "run `agentx secrets bitwarden status` for detail.",
                        len(warnings),
                    )
            else:
