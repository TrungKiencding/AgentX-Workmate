"""AgentX-tools-as-MCP server for the codex_app_server runtime.

When the user runs `openai/*` turns through the codex app-server, codex
owns the loop and builds its own tool list. By default, that means
AgentX' richer tool surface — web search, browser automation,
delegate_task subagents, vision analysis, persistent memory, skills,
cross-session search, image generation, TTS — is unreachable.

This module exposes a curated subset of those AgentX tools to the
spawned codex subprocess via stdio MCP. Codex registers it as a normal
MCP server (per `~/.codex/config.toml [mcp_servers.hermes-tools]`) and
the user gets full AgentX capability inside a Codex turn.
...
Scope (what we expose):
  ...
  - skill_view, skills_list              — AgentX' skill library
  - text_to_speech                       — TTS
  - kanban_* (complete/block/comment/    — kanban worker + orchestrator
    heartbeat/show/list/create/            handoff (stateless: read env var,
    unblock/link)                          write ~/.agentx/kanban.db)
...
What we DO NOT expose:
  ...
  - delegate_task / memory /             — `_AGENT_LOOP_TOOLS` in AgentX
...
# Tools we expose. Each name MUST match a registered AgentX tool that
...
#     `_AGENT_LOOP_TOOLS` in AgentX (model_tools.py:493). They require
#     the running AIAgent context to dispatch (mid-loop state), so a
#     stateless MCP callback can't drive them. AgentX' default runtime
...
EXPOSED_TOOLS: tuple[str, ...] = (
    ...
    "skills_list",
    "text_to_speech",
    # Kanban worker handoff tools — gated on AGENTX_KANBAN_TASK env var
    # (set by the kanban dispatcher when spawning a worker). Without these
    # in the callback, a worker spawned with openai_runtime=codex_app_server
    # could do the work but couldn't report completion back to the kernel,
    # making it hang until timeout. Stateless dispatch — they just read
    # the env var and write to ~/.agentx/kanban.db.
    "kanban_complete",
    "kanban_block",
    "kanban_comment",
    "kanban_heartbeat",
    ...
    # only — the kanban tool gates them on AGENTX_KANBAN_TASK being unset.
...
def _build_server() -> Any:
    """Create the FastMCP server with AgentX tools attached. Lazy imports
    ...
    # Discover AgentX tools so dispatch works.
    ...
    mcp = FastMCP(
        "hermes-tools",
        instructions=(
            "AgentX Workmate's tool surface, exposed for use inside a Codex "
    ...
    # Pull authoritative AgentX tool schemas for the ones we expose, so
    # MCP clients see the same parameter docs AgentX gives the model.
    all_defs = {
    ...
    for name in EXPOSED_TOOLS:
        ...
        if spec is None:
            logger.debug(
                "skipping %s — not registered in this AgentX process", name
            )
            continue

        description = spec.get("description") or f"AgentX {name} tool"
...
def main(argv: Optional[list[str]] = None) -> int:
    ...
    # Quiet mode: keep AgentX' own banners off stdout (which is the MCP wire).
    os.environ.setdefault("AGENTX_QUIET", "1")
    os.environ.setdefault("AGENTX_REDACT_SECRETS", "true")
