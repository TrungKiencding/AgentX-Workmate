...
Owns one Codex thread per AgentX session. Drives `turn/start`, consumes
...
# AgentX' tools.terminal.security_mode → Codex's permissions profile id.
# Defaults if config is missing → workspace-write (matches Codex's own default).
_AGENTX_TO_CODEX_PERMISSION_PROFILE = {
...
def _coerce_turn_input_text(user_input: Any) -> str:
    """Collapse AgentX/OpenAI rich content into app-server text input.
...
class CodexAppServerSession:
    """One Codex thread per AgentX session, lifetime owned by AIAgent.
    ...
    def __init__(
        ...
        *,
        ...
        self._permission_profile = (
            permission_profile or _AGENTX_TO_CODEX_PERMISSION_PROFILE.get(
                os.environ.get("AGENTX_TERMINAL_SECURITY_MODE", "auto"),
                "workspace-write",
            )
        )
    ...
    def run_turn(
        ...
        *,
        ...
        # supports rich content but AgentX' text path is the common case).
        try:
    ...
    def _handle_server_request(self, req: dict) -> None:
        """Translate a codex server request (approval) into AgentX' approval
        ...
        if method == "item/commandExecution/requestApproval":
            ...
            # auto-accept — the user already approved AgentX' tools
    ...
    def _decide_exec_approval(self, params: dict) -> str:
        ...
        This is protocol-level routing only — it carries NO AgentX
        approval-mode/timeout logic. The AgentX-side resolution happens
    ...
    def _decide_apply_patch_approval(self, params: dict) -> str:
        ...
        Protocol-level routing only; AgentX approval-mode/timeout
...
def _approval_choice_to_codex_decision(choice: str) -> str:
    """Map AgentX approval choices onto codex's CommandExecutionApprovalDecision
    / FileChangeApprovalDecision wire values.

    AgentX returns 'once', 'session', 'always', or 'deny'.
    ...
    NOT in tools/approval.py: the AgentX approval mode/timeout resolution
    ...
    # "prompt expired"; the AgentX-side messaging already distinguishes them.
    return "decline"
...
def _get_hermes_version() -> str:
    """Best-effort AgentX version string for codex's userAgent line."""
    try:
