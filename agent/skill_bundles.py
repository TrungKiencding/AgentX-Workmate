...
Storage
-------
Bundles live in ``~/.agentx/skill-bundles/*.yaml`` (and the equivalent
profile-aware directory under ``AGENTX_HOME``). Each file looks like::
...
- :func:`list_bundles` — return rich info for display (``agentx bundles``)
...
def _bundles_dir() -> Path:
    """Return the canonical bundles directory under AGENTX_HOME.

    Honors ``AGENTX_BUNDLES_DIR`` for tests; falls back to
    ``<AGENTX_HOME>/skill-bundles``.
    """
    override = os.environ.get("AGENTX_BUNDLES_DIR")
    if override:
...
# ---------------------------------------------------------------------------
# File-level CRUD helpers — used by `agentx bundles` CLI subcommand.
# ---------------------------------------------------------------------------
