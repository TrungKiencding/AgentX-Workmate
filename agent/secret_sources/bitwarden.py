...
AgentX pulls API keys from Bitwarden Secrets Manager at process startup
so they don't have to live in plaintext in ``~/.agentx/.env``.

Design summary
--------------

* The ``bws`` binary is auto-installed into ``<hermes_home>/bin/bws`` on
  first use.  AgentX pins one version (``_BWS_VERSION``) and downloads
...
* The access token is stored in ``~/.agentx/.env`` as
...
* Pulling secrets is a single ``bws secret list <project_id>
  ...
  ``cache_ttl_seconds`` so back-to-back ``agentx`` invocations don't
  hammer the API.
* Failures NEVER block AgentX startup.  Missing binary, no network,
...
# Disk-persisted cache so back-to-back CLI invocations (e.g. `agentx chat -q ...`
...
# never the access token. It's plaintext-equivalent to ~/.agentx/.env (which
...
_ENCRYPTED_CACHE_INFO = b"agentx-bws-encrypted-cache-v1"
...
def _disk_cache_path(home_path: Optional[Path] = None) -> Path:
    ...
    callers; falls back to `$AGENTX_HOME` / `~/.agentx` when home is None.
    """
...
def _hermes_bin_dir() -> Path:
    """Where AgentX stores its managed binaries.  Profile-aware."""
...
def install_bws(*, force: bool = False) -> Path:
    ...
    path catch these; the user-facing ``agentx secrets bitwarden setup``
    ...
    with tempfile.TemporaryDirectory(prefix="agentx-bws-") as tmpdir:
        tmp = Path(tmpdir)
...
def _summarize_bws_stderr(raw: str) -> str:
    ...
    Everything from ``Location:`` on is diagnostic noise for an AgentX
    ...
    *,
    ...
    if not access_token:
        result.error = (
            ...
            "not set.  Run `agentx secrets bitwarden setup`."
        )
        return result
    ...
    if not project_id:
        result.error = (
            ...
            "Run `agentx secrets bitwarden setup`."
        )
        return result
    ...
    if binary is None:
        result.error = (
            ...
            "Run `agentx secrets bitwarden setup` to install."
        )
        return result
    ...
    *fetches* — precedence, override semantics, conflict warnings, and
    ...
    def remediation(self, kind, cfg: dict) -> str:
        if kind in (ErrorKind.AUTH_FAILED, ErrorKind.AUTH_EXPIRED):
            return (
                "Run `agentx secrets bitwarden token` to paste a fresh access "
                "token (create one in the Bitwarden web app: Secrets Manager → "
                "Machine accounts → Access tokens).  Wrong region?  Re-run "
                "`agentx secrets bitwarden setup` and pick EU/self-hosted."
            )
...
def clear_caches(home_path: Optional[Path] = None) -> None:
    ...
    Used after a token rotation (`agentx secrets bitwarden token`) so the
