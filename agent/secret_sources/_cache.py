...
cache problem must never block AgentX startup.
"""
...
def resolve_cache_home(home_path: Optional[Path] = None) -> Path:
    """Resolve the AgentX home used for cache paths.

    ``home_path`` is whatever ``load_hermes_dotenv()`` already resolved;
    falling back to ``$AGENTX_HOME`` / ``~/.agentx`` keeps direct callers
...
class DiskCache(Generic[K]):
    ...
    def write(
        ...
        try:
            ...
            # metadata isn't exposed if AGENTX_HOME is ever made traversable.
            try:
