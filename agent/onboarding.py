...
def openclaw_residue_hint_cli() -> str:
    """Banner shown the first time AgentX starts and finds ``~/.openclaw/``.

    Points users at ``agentx claw migrate`` (non-destructive port of config,
    memory, and skills) first. ``agentx claw cleanup`` is mentioned as the
    ...
    return (
        ...
        "To port your config, memory, and skills over to AgentX, run "
        "`agentx claw migrate`.\n"
        "If you've already migrated and want to archive the old directory, "
        "run `agentx claw cleanup` (renames it to ~/.openclaw.pre-migration — "
