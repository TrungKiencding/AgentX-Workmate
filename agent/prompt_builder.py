...
def _find_hermes_md(cwd: Path) -> Optional[Path]:
    """Discover the nearest ``.agentx.md`` or ``AGENTX.md``.
    ...
    # could pick up a .agentx.md planted in /tmp, /home, etc.
    search_dirs = [current, *current.parents] if stop_at else [current]

    for directory in search_dirs:
        for name in _AGENTX_MD_NAMES:
...
KANBAN_GUIDANCE = (
    ...
    "the shared board at `~/.agentx/kanban.db`. Your task id is in "
    "`$AGENTX_KANBAN_TASK`; your workspace is `$AGENTX_KANBAN_WORKSPACE`. "
    ...
    "ground truth.\n"
    "2. **Work inside the workspace.** `cd $AGENTX_KANBAN_WORKSPACE` before "
    ...
    "\n"
    "- **Workspace.** `cd $AGENTX_KANBAN_WORKSPACE` first. For a `worktree` kind "
    "with no `.git`, `git worktree add <path> "
    "${AGENTX_KANBAN_BRANCH:-wt/$AGENTX_KANBAN_TASK}` from the main repo, then "
    "cd there. For a project-linked task the workspace is a fresh "
    "`<repo>/.worktrees/<task-id>` and `$AGENTX_KANBAN_BRANCH` a deterministic "
    ...
    "every assignee in a real profile (`agentx profile list`, or ask the user), "
    "and express dependencies via `parents=[...]` on `kanban_create`, not prose.\n"
    "\n"
    "## Do NOT\n"
    "\n"
    "- Do not shell out to `agentx kanban <verb>` for board operations. Use "
...
#
# The agentx-agent runtime already executes a batch of tool calls
...
# from Cline's TypeScript tool-surface guidance to agentx-agent's Python
...
def computer_use_guidance(platform_name: Optional[str] = None) -> str:
    ...
    return (
        ...
        "when AgentX has none; explicit AgentX YOLO uses a private unrestricted "
        ...
        "run `agentx computer-use doctor` and share the output. That command "
...
STEER_CHANNEL_NOTE = (
    ...
    "While you work, the user can send an out-of-band message that AgentX "
...
PLATFORM_HINTS = {
    ...
    "tui": (
        "You are running in the AgentX terminal UI (TUI). "
        "Cron jobs scheduled from this session are LOCAL-ONLY: their output is "
        "saved (viewable via cronjob action='list') but is NOT delivered back "
    ...
    "desktop": (
        "You are chatting inside the AgentX desktop app — a graphical chat "
        ...
        "File delivery: when a task ends in a file the user asked for (a "
        "document, spreadsheet, presentation, PDF, image, archive, …), call the "
        "deliver_file tool with its ABSOLUTE path right after writing and "
        "verifying it — one call per file. The file then appears in the chat as "
        "a card the user can preview, save, or reveal in their file manager; "
        "after the call, describe the content briefly and do not paste the path "
        "again. Never leave a bare path as the only handover. As a fallback "
        "when the tool is unavailable, include MEDIA:/absolute/path/to/file in "
        "your response: images (.png, .jpg, .webp) appear inline, audio and "
        "video play inline, and other files arrive as file cards. You can also "
        "include image URLs in markdown format ![alt](url) and they render "
        "inline as photos."
    ),
    "sms": (
    ...
    "webui": (
        "You are in the AgentX WebUI, a browser-based chat interface. "
...
# container / remote host rather than on the machine where AgentX itself
...
def _probe_remote_backend(env_type: str) -> str | None:
    ...
    operate on a different machine than the host AgentX runs on.
    """
...
def build_environment_hints() -> str:
    ...
    if not is_remote_backend:
        ...
        if probe:
            hints.append(
                ...
                f"where AgentX itself is running. The host OS, home, and cwd "
                f"of the AgentX process are irrelevant; only the following "
            ...
            hints.append(
                f"Terminal backend: {backend}. Your `terminal`, `read_file`, "
                f"`write_file`, `patch`, and `search_files` tools all operate "
                f"inside {description} — NOT on the machine where AgentX "
    ...
    # Embedder-supplied environment description. Lets a host that wraps AgentX
    ...
    extra = (os.getenv("AGENTX_ENVIRONMENT_HINT") or "").strip()
    if not extra:
        try:
...
def _current_session_platform_hint() -> str:
    ...
    platform = os.environ.get("AGENTX_PLATFORM") or os.environ.get("AGENTX_SESSION_PLATFORM")
    if platform:
        return platform
...
def build_skills_system_prompt(
    ...
    scanned alongside the local ``~/.agentx/skills/`` directory.  External dirs
    ...
    if not skills_by_category:
        ...
        result = (
            ...
            "or troubleshoot AgentX Workmate itself — its CLI, config, models, providers, tools, "
            "skills, voice, gateway, plugins, or any feature — load the `agentx-agent` skill "
            "first. It has the actual commands (e.g. `agentx config set …`, `agentx tools`, "
            "`agentx setup`) so you don't have to guess or invent workarounds.\n"
...
def build_nous_subscription_prompt(valid_tool_names: "set[str] | None" = None) -> str:
    ...
    lines.extend(
        [
            ...
            "Useful commands: agentx setup, agentx setup tools, agentx setup terminal, agentx status.",
        ]
    )
    return "\n".join(lines)
...
def load_soul_md(context_length: Optional[int] = None) -> Optional[str]:
    """Load SOUL.md from AGENTX_HOME and return its content, or None.
    ...
    try:
        ...
        logger.debug("Could not ensure AGENTX_HOME before loading SOUL.md: %s", e)
...
def _load_hermes_md(cwd_path: Path, context_length: Optional[int] = None) -> str:
    """.agentx.md / AGENTX.md — walk to git root."""
    ...
    try:
        ...
        return _truncate_content(
            result, ".agentx.md", context_length=context_length,
...
def build_context_files_prompt(
    ...
    Priority (first found wins — only ONE project context type is loaded):
      1. .agentx.md / AGENTX.md  (walk to git root)
      2. AGENTS.md / agents.md   (cwd only)
      3. CLAUDE.md / claude.md   (cwd only)
      4. .cursorrules / .cursor/rules/*.mdc  (cwd only)

    SOUL.md from AGENTX_HOME is independent and always included when present.
    ...
    # Never let a FALLBACK-picked directory inside the AgentX install/source
    ...
    # explicitly configured cwd is honored verbatim — the AgentX tree is a
    ...
    # their launch dir IS the user's shell cwd (developing AgentX in-tree).
    ...
    if (
        ...
        logger.warning(
            ...
            "fell back to the AgentX install tree (%s) — set terminal.cwd to "
    ...
    # SOUL.md from AGENTX_HOME only — skip when already loaded as identity
    if not skip_soul:
