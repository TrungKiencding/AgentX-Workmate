"""Lightweight internationalization (i18n) for AgentX static user-facing messages.

Scope (thin slice, by design): only the highest-impact static strings shown
to the user by AgentX itself -- approval prompts, a handful of gateway slash
...
Language resolution order:
    ...
    2. ``AGENTX_LANGUAGE`` environment variable (for tests / quick override)
...
def _locales_dir() -> Path:
    ...
    1. ``AGENTX_BUNDLED_LOCALES`` env var -- set by the Nix wrapper (or any
    ...
    """
    override = os.getenv("AGENTX_BUNDLED_LOCALES", "").strip()
    if override:
        candidate = Path(override)
        if candidate.is_dir():
            return candidate
        logger.warning(
            "AGENTX_BUNDLED_LOCALES points to a non-directory path (%s); "
...
def _load_catalog(lang: str) -> dict[str, str]:
    ...
    try:
        import yaml  # PyYAML is already an agentx dependency
...
def get_language() -> str:
    ...
    env_lang = os.environ.get("AGENTX_LANGUAGE")
    if env_lang:
