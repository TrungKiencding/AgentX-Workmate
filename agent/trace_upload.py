...
* **Zero LLM turn.** This is a deterministic export — it never spends a
  model call. The ``agentx trace upload`` subcommand calls
...
* **Private by default.** Traces can contain prompts, tool output, local
  ...
  is passed through AgentX' secret redactor (``force=True``) unless the
...
# ---------------------------------------------------------------------------
# Conversion: AgentX OpenAI-format messages -> Claude Code JSONL
# ---------------------------------------------------------------------------
...
def _redact(text: Any, enabled: bool) -> Any:
    ...
    Non-strings pass through untouched. Uses AgentX' shared redactor with
...
def build_trace_jsonl(
    ...
    *,
    ...
    """Render AgentX conversation messages as Claude Code JSONL text.
...
_NO_TOKEN_MESSAGE = (
    ...
    "2. Add it to your environment as HF_TOKEN (e.g. in ~/.agentx/.env):\n"
    "     HF_TOKEN=hf_xxxxxxxxxxxxxxxxxxxx\n"
    "3. Run /upload-trace again (or `agentx trace upload`)."
)
