"""Profile-scoped NeMo Relay runtimes owned by the AgentX agent core."""
...
class RelayRuntime:
    ...
    def get_session(self, session_id: str) -> RelaySession | None:
        """Return an active AgentX Relay session without creating one."""
        with self._sessions_lock:
        ...
        **kwargs: Any,
        ...
        with session.lock:
            if session.closing and not allow_closing:
                raise RuntimeError("AgentX Relay session is closing")
            if session.context is None or session.handle is None:
                raise RuntimeError("AgentX Relay session context is unavailable")
    ...
    def emit_mark(
        ...
        *,
        ...
        """Emit a mark parented to the AgentX session identified by ``event``."""
    ...
    def apply_tool_request_intercepts(
        ...
        *,
        ...
        """Apply Relay request rewriting before AgentX authorizes a tool call."""
    ...
    def close_session(self, event: dict[str, Any]) -> None:
        ...
        if failures:
            logger.warning(
                "AgentX Relay session %s closed with errors: %s",
                session_id,
                "; ".join(failures),
            )
    ...
    def _safe(callback: Callable[..., Any], *args: Any, **kwargs: Any) -> Any:
        try:
            ...
            logger.warning("AgentX Relay runtime operation failed", exc_info=True)
            return None
...
class RelayHostRegistry:
    """Own exactly one Relay host for each canonical AgentX profile."""
        ...
        *,
        ...
        with self._lock:
            ...
            try:
                ...
                logger.warning(
                    "AgentX Relay runtime initialization failed", exc_info=True
                )
...
class RelayTurnContext:
    """Runtime-only context for one AgentX turn or top-level task."""
...
class RelaySessionCoordinator:
    """Own semantic conversation and turn lifetimes for AgentX core."""
    ...
    def _prepare_session(
        ...
        for name, callback in initializers:
            try:
                ...
                logger.warning(
                    "AgentX Relay session initializer failed: %s",
                    name,
                    exc_info=True,
                )
        ...
        *,
        ...
        if isinstance(host, RelayRuntime):
            try:
                ...
                metadata = {"agentx.execution_surface": platform or "unknown"}
                ...
                logger.warning(
                    "AgentX Relay conversation initialization failed",
                    exc_info=True,
                )
    ...
    def begin_turn(
        ...
        *,
        ...
        if lease.released:
            raise RuntimeError("AgentX Relay conversation lease is released")
        ...
        with self._active_turns_lock:
            ...
            if active:
                ...
                # AgentX turns would create sibling scopes on that stack, but
                # their completion order is not guaranteed to be LIFO.
                turn.relay_enabled = False
                logger.warning(
                    "Skipping Relay instrumentation for concurrent AgentX turn "
                    "%s in session %s",
                    turn_id,
                    lease.session_id,
                )
            ...
            try:
                turn.handle = lease.host.run_in_session(
                    ...
                    metadata={
                        ...
                        "agentx.execution_surface": lease.platform or "unknown",
                    },
                )
            except Exception:
                logger.warning("AgentX Relay turn initialization failed", exc_info=True)
                ...
                if isinstance(lease.host, RelayRuntime) and lease.session is not None:
                    ...
                    if turn.handle is not None:
                        try:
                            ...
                            logger.warning(
                                "AgentX Relay turn finalization failed", exc_info=True
                            )
            finally:
                try:
                    ...
                    logger.warning(
                        "AgentX Relay child conversation finalization failed",
                        exc_info=True,
                    )
                finally:
        ...
        *,
        ...
        for index in range(len(logical_calls) - 1, -1, -1):
            ...
            try:
                ...
                logger.warning(
                    "AgentX Relay logical LLM finalization failed",
                    exc_info=True,
                )
                break
...
def emit_mark(
    ...
    *,
    ...
    try:
        ...
        logger.warning("AgentX Relay mark failed: %s", name, exc_info=True)
        return False
...
def apply_tool_request_intercepts(
    *,
    ...
    """Return Relay-rewritten arguments at AgentX's authorization boundary."""
    if not session_id:
        return args
    runtime = get_runtime(create=False)
    if runtime is None:
...
def ensure_session(*, session_id: str, **context: Any) -> RelaySession | None:
    """Create or return the shared Relay session used by AgentX core."""
    runtime = get_runtime()
    if runtime is None:
        return None
    try:
        ...
        logger.warning("AgentX Relay session initialization failed", exc_info=True)
        return None
...
def run_in_session(
    ...
    **kwargs: Any,
    ...
    """Run a scope, LLM, or tool API against a shared AgentX session."""
    runtime = get_runtime()
    if runtime is None:
        raise RuntimeError("AgentX Relay runtime is unavailable")
    session = runtime.get_session(session_id)
    if session is None:
        session = runtime.ensure_session({"session_id": session_id})
    if session is None:
        raise RuntimeError("AgentX Relay session is unavailable")
...
async def run_in_session_async(
    ...
    **kwargs: Any,
    ...
    """Await a Relay operation inside a shared AgentX session context."""
    runtime = get_runtime()
    if runtime is None:
        raise RuntimeError("AgentX Relay runtime is unavailable")
    session = runtime.get_session(session_id)
...
def get_runtime(
    *,
    ...
    """Return the Relay host for the active AgentX profile."""
