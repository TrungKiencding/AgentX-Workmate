...
tool calls. AgentX treats that as a clean exit → ``rc=0`` → dispatcher
``protocol_violation``.
...
def kanban_stop_nudge_enabled() -> bool:
    ...
    On when ``AGENTX_KANBAN_TASK`` is set (dispatcher-spawned worker), unless
    ``AGENTX_KANBAN_STOP_NUDGE`` explicitly disables it.
    """
    env = os.environ.get("AGENTX_KANBAN_STOP_NUDGE")
    if env is not None and env.strip().lower() in {"0", "false", "no", "off"}:
        return False
    task = (os.environ.get("AGENTX_KANBAN_TASK") or "").strip()
    return bool(task)
