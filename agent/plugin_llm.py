...
Plugins built on AgentX Workmate often need to make their own LLM calls
...
Today the only stable plugin surfaces extend an existing AgentX
...
embedded slugs, no shorthands. This mirrors AgentX' main config
...
handles every provider, fallback chain, and per-task override AgentX
supports.
"""
