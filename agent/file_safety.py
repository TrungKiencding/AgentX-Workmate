...
def _hermes_home_path() -> Path:
    """Resolve the active AGENTX_HOME (profile-aware) without circular imports."""
    try:
        from hermes_constants import get_hermes_home  # local import to avoid cycles
        return get_hermes_home()
    except Exception:
        return Path(os.path.expanduser("~/.agentx"))


def _hermes_root_path() -> Path:
    """Resolve the AgentX root dir (always the parent of any profile, never per-profile)."""
    try:
...
def get_safe_write_roots() -> set[str]:
    """Return resolved AGENTX_WRITE_SAFE_ROOT paths. Supports multiple directories
    separated by ``os.pathsep`` (``:`` on Unix, ``;`` on Windows).
    E.g., ``/opt/data:/var/www/html`` on Unix, ``C:\\data;D:\\www`` on Windows."""
    env = os.getenv("AGENTX_WRITE_SAFE_ROOT", "")
    if not env:
        return set()
    roots: set[str] = set()
...
def get_write_denied_error(path: str, *, verb: str = "Write") -> Optional[str]:
    ...
    if denial == "safe_root":
        ...
        return (
            f"{verb} denied: '{path}' is outside AGENTX_WRITE_SAFE_ROOT "
...
def get_read_block_error(path: str) -> Optional[str]:
    """Return an error message when a read targets a denied AgentX path.

    Three categories are blocked:

      * Internal AgentX cache files under ``AGENTX_HOME/skills/.hub`` —
        readable metadata that an attacker could use as a prompt-injection
        carrier.
      * Credential / secret stores under AGENTX_HOME and the global AgentX
    ...
    **This is NOT a security boundary.** The terminal tool runs as the
    ...
    or ``cat ~/.agentx/.env`` and exfiltrate the file. The read-deny exists
    ...
    # Resolve BOTH the active AGENTX_HOME (profile-aware) AND the global
    # AgentX root so credential stores at <root>/auth.json etc. are also
    # blocked when running under a profile (AGENTX_HOME points at
    ...
    for hd in hermes_dirs:
        ...
        for blocked in blocked_dirs:
            try:
                ...
                continue
            return (
                f"Access denied: {path} is an internal AgentX cache file "
    ...
    for hd in hermes_dirs:
        ...
        if resolved == mcp_tokens:
            return (
                f"Access denied: {path} is the AgentX MCP token directory "
...
def raise_if_read_blocked(path: str) -> None:
    """Raise ``ValueError`` if ``path`` is a denied AgentX read (see
...
#
# AgentX profiles are separate AGENTX_HOME dirs under
...
# Profile-scoped directories under AGENTX_HOME / <root> / <root>/profiles/<X>/
...
def _resolve_active_profile_name() -> str:
    """Return the active profile name derived from AGENTX_HOME.
...
def classify_cross_profile_target(path: str) -> Optional[dict]:
    ...
    Returns ``None`` when the target is outside AgentX scope, or is inside
...
def get_cross_profile_warning(path: str) -> Optional[str]:
    ...
    AgentX entirely. Caller is expected to surface the warning to the
    ...
    return (
        ...
        f"belongs to AgentX profile {info['target_profile']!r}, but the "
...
#
#   <AGENTX_HOME>/profiles/<name>/sandboxes/<backend>/<task>/home/.agentx/...
#
...
# ``…/sandboxes/<backend>/<task>/home/.agentx/…`` segment and warns
# regardless of which AgentX profile is active. It does NOT cover the
# inner-container case where the bind mount strips the ``sandboxes/`` prefix
# (the agent's view inside the container is plain ``/root/.agentx/...``);
...
def _find_sandbox_mirror_segments(parts: tuple) -> Optional[int]:
    """Return the index of the inner ``.agentx`` part in a sandbox-mirror path.

    Matches ``…/sandboxes/<backend>/<task>/home/.agentx/…`` and returns the
    index where the inner AgentX-state portion starts. Returns ``None`` for
    paths that do not contain the sandbox-mirror shape.
    """
    for i, part in enumerate(parts):
        if part != "sandboxes":
            continue
        # Need at least: sandboxes / <backend> / <task> / home / .agentx / <thing>
        if i + 5 >= len(parts):
            continue
        if parts[i + 3] == "home" and parts[i + 4] == ".agentx":
            return i + 4
    return None


def classify_sandbox_mirror_target(path: str) -> Optional[dict]:
    """Classify a write target as a sandbox-mirror of authoritative AgentX state.

    Returns ``None`` when the path does not match the sandbox-mirror shape.
    Otherwise returns a dict with:

      * ``target_path``: the resolved path string
      * ``mirror_root``: the ``…/sandboxes/<backend>/<task>/home/.agentx``
        prefix (so callers can show users which sandbox owns the mirror)
      * ``inner_path``: the portion under the mirror's ``.agentx`` (what the
        agent likely meant to address on the host)

    Detection is path-shape-only — does not require any AgentX resolver to
    succeed, so it works correctly even when called from contexts where
    AGENTX_HOME resolution would be ambiguous.
    """
    try:
        target = Path(os.path.expanduser(str(path))).resolve()
    except (OSError, RuntimeError):
...
def get_sandbox_mirror_warning(path: str) -> Optional[str]:
    ...
    return (
        ...
        f"Writes here land on a copy that the host AgentX process never "
        f"reads — the authoritative file is likely {info['inner_path']!r} "
        f"under the real AGENTX_HOME. Use the host-side tool for "
...
# full ``…/sandboxes/<backend>/<task>/home/.agentx/…`` prefix on the host.
# But when file tools execute *inside* the container the bind-mount strips
# that prefix: the agent sees plain ``/root/.agentx/…``.  The root:root
...
def classify_container_mirror_target(
    ...
    under the mirror prefix. Otherwise returns:
      ...
      * ``inner_path``: portion under the mirror root (what the agent
        likely meant to address in the host AGENTX_HOME)
    """
...
def get_container_mirror_warning(
    ...
    sandbox mirror of authoritative AgentX state.
    ...
    return (
        ...
        f"bind-mounted home — a per-task mirror that the host AgentX "
        f"process never reads. The authoritative file is "
        f"{info['inner_path']!r} under the real AGENTX_HOME. Use the "
