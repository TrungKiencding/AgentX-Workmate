# Política de Seguridad de AgentX Workmate

Este documento describe el modelo de confianza de AgentX Workmate, identifica el
...
- Detalles del entorno (`agentx version`, SHA del commit, SO, versión de Python).
...
AgentX Workmate es un agente personal de un solo inquilino. Su postura es
...
- **Proceso del agente.** El intérprete Python que ejecuta AgentX Workmate,
...
- **Envolvente de confianza.** El conjunto de recursos a los que un operador ha otorgado
  implícitamente acceso an AgentX Workmate al ejecutarlo — típicamente, todo lo que
  la propia cuenta de usuario del operador puede alcanzar en el host.
- **Postura.** Una declaración explícita en la documentación o código de AgentX Workmate
...
AgentX Workmate admite dos posturas de aislamiento a nivel de SO. Abordan
...
AgentX Workmate admite esto de dos maneras:

- **La propia imagen Docker de AgentX Workmate y la configuración de Compose.** Más
...
Bajo una envoltura de proceso completo, las heurísticas en proceso de AgentX Workmate
...
AgentX Workmate filtra el entorno que pasa a sus componentes en proceso de
...
Un plugin malicioso o con errores no es una vulnerabilidad en AgentX Workmate
en sí mismo. Los errores en la ruta de instalación o descubrimiento de plugins de AgentX Workmate
...
**Superficies en AgentX Workmate:**
...
4. **Dentro del conjunto autorizado, todos los llamadores tienen la misma confianza.**
   AgentX Workmate no modela capacidades por llamador dentro de un único adaptador.
...
- Violaciones de la documentación del modelo de confianza: código que se comporta
  contrariamente a lo que esta política, la propia documentación de AgentX Workmate o
  las expectativas razonables del operador predecirían — incluyendo casos donde
  AgentX Workmate ha documentado una postura sobre cómo su salida debe ser
...
- **Habilidades y plugins contribuidos por la comunidad.** Las habilidades de terceros
  ...
  están en la superficie de revisión del operador, no en la superficie de confianza de AgentX Workmate
  (§2.4, §2.5). Una habilidad o plugin que haga algo
  malicioso es el modo de falla esperado de uno que no fue
  revisado, no una vulnerabilidad en AgentX Workmate. Los errores en la ruta de
  instalación de habilidades o plugins de AgentX Workmate que impidan al
...
- AgentX Workmate incluye guardias de cadena de suministro para lanzamientos de servidores
