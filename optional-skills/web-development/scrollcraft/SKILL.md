...
version: 1.0.0
author: 'nateherkai (upstream scroll-craft), ported by AgentX Workmate'
license: MIT
...
- `scripts/shoot.mjs` needs Playwright (`npm install playwright` or
  `playwright-core` plus a Chrome install). AgentX' `browser_exec` tool is the
