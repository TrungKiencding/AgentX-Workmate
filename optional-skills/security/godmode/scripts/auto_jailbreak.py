...
Usage in execute_code:
    exec(open(os.path.expanduser(
        os.path.join(os.environ.get("AGENTX_HOME", os.path.expanduser("~/.agentx")), "skills/red-teaming/godmode/scripts/auto_jailbreak.py")
    )).read())
...
try:
    ...
    _SKILL_DIR = Path(os.getenv("AGENTX_HOME", Path.home() / ".agentx")) / "skills" / "red-teaming" / "godmode"
...
AGENTX_HOME = Path(os.getenv("AGENTX_HOME", Path.home() / ".agentx"))
CONFIG_PATH = AGENTX_HOME / "config.yaml"
PREFILL_PATH = AGENTX_HOME / "prefill.json"
...
MODEL_STRATEGIES = {
    ...
    # Nous/AgentX models — already uncensored, just needs clean prompt
    "hermes": {
        "order": ["prefill_only"],
        "system_templates": {},
    },
...
def _detect_model_family(model: str) -> str:
    ...
    if "grok" in model_lower or "x-ai" in model_lower:
        return "grok"
    if "agentx" in model_lower or "nous" in model_lower:
        return "agentx"
...
def _get_current_model() -> tuple:
    """Read current model and provider from AgentX config.yaml.
...
def _write_prefill(prefill_messages: list):
    """Write prefill messages to ~/.agentx/prefill.json."""
    ...
    if winning_strategy:
        ...
        if not dry_run:
            ...
            if verbose:
                ...
                print()
                print("[DONE] Jailbreak locked in. Restart AgentX for changes to take effect.")
        else:
            if verbose:
...
def undo_jailbreak(verbose=True):
    ...
    if verbose:
        print("[UNDO] Jailbreak removed. Restart AgentX for changes to take effect.")
