...
The design is **AgentX-native**: a small deterministic Python CLI (`scripts/pdd.py`) owns the state
...
```bash
agentx skills install official/security/unbroker
```

Then start a new AgentX session and drive it (below). The skill works zero-config; a few optional
...
Drive it from an AgentX session:
...
- **PII handling.** Dossiers live under the AgentX home directory (`0600`, optionally
