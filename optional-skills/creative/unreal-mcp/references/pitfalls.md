...
### 1. Start order: editor first, AgentX second

AgentX probes MCP servers at session start. If the editor (and its server)
...
`LogModelContextProtocol` with the address), then open a NEW AgentX session.
...
restart the editor, restart the AgentX session.
...
server fails to bind (Output Log) or AgentX' probe times out. Fix: change
Server Port Number in Editor Preferences > Model Context Protocol AND the
`url` in `~/.agentx/config.yaml` (`mcp_servers.unreal-engine`), then restart
...
### 5. GenerateClientConfig is not for AgentX

`ModelContextProtocol.GenerateClientConfig` writes config files for Claude
Code/Cursor/VSCode/Gemini/Codex into the project root. AgentX' connection
lives in `~/.agentx/config.yaml` via `agentx mcp install unreal-engine`.
Running GenerateClientConfig neither helps nor harms AgentX — just don't
mistake it for the AgentX setup step.
...
explicitly warns against overlapping calls. AgentX executes same-turn tool
...
### 9. Timeouts: AgentX gives up before Unreal does

AgentX' default per-call timeout is 120 s. Asset imports, first-shader
...
`<Project>/Saved/Screenshots/...`). If AgentX runs elsewhere (SSH backend,
