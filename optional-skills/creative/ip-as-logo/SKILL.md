...
version: 1.0.0
author: s1dashu (https://github.com/s1dashu/ip-as-logo-skill), ported by AgentX Workmate
license: MIT
...
> **AgentX adaptation notes** (the rest of this document is the upstream
...
8. Determine the available image-generation path before promising output. In AgentX this is the `image_generate` tool; if it reports no configured backend, ask the user to enable one (`agentx tools`) instead of fabricating results.
