...
Maps each role archetype to the AgentX skills it should `always_load` and the
toolsets it needs. Only references skills that ship in the public agentx-agent
...
## AgentX skills relevant to video production

### Visual / rendering skills (`agentx-agent/skills/creative/`)
...
### Design / pre-production skills (`agentx-agent/skills/creative/`)
...
### Audio / media skills (`agentx-agent/skills/creative/` + `skills/media/`)
...
These are **not** AgentX skills but external CLIs / APIs that profiles invoke.
...
## Built-in AgentX tools for media review

These are native AgentX tools — not invoked via terminal but through their own
...
key is present in `${AGENTX_HOME:-~/.agentx}/.env` (or macOS Keychain) before firing the kanban.
...
| Anthropic | `ANTHROPIC_API_KEY` | every AgentX profile (Claude) |

If a key is missing, prompt the user to add it. Storage methods, in order of
preference: macOS Keychain → `${AGENTX_HOME:-~/.agentx}/.env` → environment variable.
...
When a new AgentX-public video skill ships:
