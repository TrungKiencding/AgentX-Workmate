...
the actual `setup.sh` that creates the project workspace, configures AgentX
...
2. **Create profiles** — `agentx profile create <name> --clone`
3. **Configure profiles** — patch each profile's
   `~/.agentx/profiles/<name>/config.yaml` to set toolsets, always_load skills,
   and `cwd`
4. **Write SOUL.md per profile** — the personality + role definition
5. **Copy any provided assets + write `brief.md`, `TEAM.md`, and `taste/`**
6. **Fire the initial kanban task** — `agentx kanban create` assigned to the director
...
```bash
agentx profile create director --clone 2>/dev/null || true
```
...
Each profile has a YAML config at `~/.agentx/profiles/<name>/config.yaml`. The
...
p = os.path.expanduser(f"~/.agentx/profiles/{profile}/config.yaml")
with open(p) as f:
...
PyYAML must be installed in the user's Python (it ships with most AgentX
...
Each profile gets a `SOUL.md` at `~/.agentx/profiles/<name>/SOUL.md` that
...
```bash
agentx kanban create "Direct production of <video title>" \
    --assignee director \
...
the AgentX `.env` (`${AGENTX_HOME:-$HOME/.agentx}/.env`) and macOS Keychain
(if on macOS):
...
check_key() {
    ...
    local kc_account="$2"
    local kc_service="$3"
    local _hermes_env="${AGENTX_HOME:-$HOME/.agentx}/.env"
...
check_key ELEVENLABS_API_KEY agentx ELEVENLABS_API_KEY || exit 1
check_key OPENROUTER_API_KEY agentx OPENROUTER_API_KEY || exit 1
# ...
```
