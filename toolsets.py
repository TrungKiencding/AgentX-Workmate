...
_AGENTX_CORE_TOOLS = [
    ...
    # spawned as a kanban worker (AGENTX_KANBAN_TASK env set) or the current
...
TOOLSETS = {
    ...
    "kanban": {
        "description": (
            ...
            "is spawned by the kanban dispatcher (AGENTX_KANBAN_TASK env "
...
def bundle_non_core_tools(toolset_name: str) -> Set[str]:
    """Return a ``agentx-*`` bundle's platform-specific tools, excluding core.

    Platform bundles are defined as ``_AGENTX_CORE_TOOLS + [platform extras]``.
...
def resolve_toolset(name: str, visited: Set[str] = None, *, include_registry: bool = True) -> List[str]:
    ...
    if not toolset:
        # Auto-generate a toolset for plugin platforms (agentx-<name>).
        # Gives them _AGENTX_CORE_TOOLS plus any tools the plugin registered
        ...
        if include_registry and name.startswith("agentx-"):
            platform_name = name[len("agentx-"):]
            try:
                from gateway.platform_registry import platform_registry
                if platform_registry.is_registered(platform_name):
                    plugin_tools = set(_AGENTX_CORE_TOOLS)
                    try:
                        from tools.registry import registry
                        plugin_tools.update(
