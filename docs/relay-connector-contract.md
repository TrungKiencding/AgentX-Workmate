...
This document is the formal interface between the **AgentX gateway** (Python,
...
- **Registration.** The gateway registers a **wake URL** at enroll/provision —
  ...
  a dashboard host). Self-hosted: `agentx gateway enroll --wake-url <url>` (or
...
**per-gateway secret** (`agentx gateway enroll` → connector `/relay/enroll`, or
...
driven by the managed Portal / `agentx` CLI.
