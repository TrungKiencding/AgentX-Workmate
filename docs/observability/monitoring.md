...
AgentX gateway daemon, exported over OTLP/HTTP to an operator-configured
...
(`plugins/observability/nemo_relay/`) and its AgentX-owned subscribers.
...
```bash
agentx monitoring status
```

The OpenTelemetry SDK is an optional extra (`pip install 'agentx-workmate[otlp]'`),
...
a plain `agentx gateway run`: the exporter lives in the gateway process, so
...
AgentX Workmate-owned Relay transport health remains in scope. A separate gateway
...
```bash
agentx monitoring status                 # posture
python scripts/observability/gateway_health_export_probe.py \
  --endpoint http://127.0.0.1:4318/v1/traces \
...
The `agentx monitoring` CLI intentionally exposes `status` only. This first
release covers only AgentX Workmate-owned service-health and operational-diagnostic
signals, including AgentX Workmate-owned Relay transport health. Team Gateway's
