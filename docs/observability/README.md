# AgentX Observer Hooks

AgentX observer hooks are the read-only telemetry contract for plugins that
...
They do not replace AgentX' planner, model providers, memory, tool registry,
...
AgentX also has a first-party NeMo Relay shared-metrics path. It uses these
...
```text
telemetry_schema_version = "agentx.observer.v1"
```

Hook callbacks are fail-open. AgentX catches callback exceptions, logs a
...
payload construction is gated behind `has_hook(...)`, so AgentX only builds
