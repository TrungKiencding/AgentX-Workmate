...
AgentX middleware is the behavior-changing companion to observer hooks.
...
selection, or handoff to runtimes such as NeMo Relay without changing AgentX'
planner, model provider adapters, tool registry, memory, or CLI UX.

With middleware enabled, plugins can:

- Rewrite LLM provider request kwargs before AgentX calls the provider.
- Rewrite tool arguments before guardrails, approval checks, hooks, and tool
  execution see them.
- Wrap the actual LLM execution callback while preserving AgentX retry,
  streaming, interrupt, and hook behavior.
- Wrap the actual tool execution callback while preserving AgentX guardrails,
...
AgentX stores those trace entries in later observer hook payloads as
`middleware_trace`.
...
If multiple plugins register the same execution middleware kind, AgentX runs
them as a nested chain in registration order. Middleware failures are fail-open:
AgentX logs a warning and continues with the next middleware or the base
runtime path.
...
For each provider request, AgentX applies middleware in this order:
...
For each tool call, AgentX applies middleware in this order:

1. Parse and coerce model-provided tool arguments.
2. Apply `tool_request` middleware.
3. Run the normal AgentX pre-execution path against the effective arguments:
...
For isolated local testing, use one `AGENTX_HOME` for plugin enablement and the
agent run:
...
```bash
uv sync
uv run agentx plugins enable <plugin-name>
uv run agentx chat --query 'Reply exactly ok'
```
...
Return the same response shape AgentX expects from the provider adapter. Do not
...
- If execution middleware raises before calling `next_call(...)`, AgentX treats
...
- If execution middleware calls `next_call(...)` successfully and then raises
  during post-processing, AgentX preserves the downstream result and does not
  run the provider or tool a second time.
- If downstream provider or tool execution fails, middleware may let that error
  propagate or translate it deliberately. AgentX does not convert downstream
