...
When running AgentX inside Docker, the default `network_mode: host` gives the
...
The AgentX [SECURITY.md](../../SECURITY.md) §2 defines the trust model. The
...
#
# Usage:
#   AGENTX_UID=$(id -u) AGENTX_GID=$(id -g) docker compose up -d
#
...
services:
  gateway:
    ...
    environment:
      ...
      - NO_PROXY=agentx,agentx-dashboard,localhost
...
docker compose exec gateway \
  curl -sf --max-time 5 http://agentx-dashboard:9119/health && echo "OK: internal reachable" || echo "FAIL"
...
- [SECURITY.md](../../SECURITY.md) — AgentX trust model and vulnerability reporting
