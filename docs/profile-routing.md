...
`agentx config set gateway.profile_routes ...` writes).
...
| `profile` | yes | Target profile name (must exist under `~/.agentx/profiles/<name>`). |
...
activates the per-profile runtime scope (per-profile `AGENTX_HOME`, secret scope, and
