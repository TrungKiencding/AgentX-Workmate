...
has to be thrown away or summarized. AgentX has always done this in one batch:
...
AgentX folds the single oldest un-absorbed exchange into a running summary. The
...
The in-memory splice alone isn't enough. AgentX's normal session flush is
...
With `micro_compact` unset or `false` AgentX behaves exactly as it always has:
...
Defaults to `$AGENTX_HOME/logs/agent.log`. It reports passes, outcome mix, net
