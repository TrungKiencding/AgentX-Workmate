...
Counts consecutive restarts via a JSON file (`{AGENTX_HOME}/restart_counts.json`). If a
...
This prevents unwanted auto-resets after `agentx update`, `agentx gateway restart`,
or `/restart`.
