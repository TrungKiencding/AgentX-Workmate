...
Supersedes: PR #31781 (prompt_toolkit `agentx profile wizard`)

## Why this, not the CLI wizard

PR #31781 added a keyboard-driven `agentx profile wizard` in the terminal.
...
needs, and a profile is just a AGENTX_HOME directory — so the dashboard is the
right home for a full-featured builder, and it can reuse everything that
already exists.

A profile = a full `~/.agentx/profiles/<name>/` directory with its own:
...
| Model + Provider | ModelsPage | `_write_profile_model(profile_dir, …)` | yes — AGENTX_HOME override, already wired into create endpoint |
| MCPs | McpPage | `mcp_config._save_mcp_server` + `/api/mcp/catalog` | yes — wrap with AGENTX_HOME override |
...
### Seam #1 — hub-skill install cannot use the AGENTX_HOME override

`tools/skills_hub.py` binds `SKILLS_DIR = AGENTX_HOME / "skills"` at **module
...
A fresh subprocess re-imports `skills_hub` with the profile's AGENTX_HOME bound
...
2. write model (synchronous, AGENTX_HOME override)
3. write selected MCP servers (synchronous, AGENTX_HOME override)
4. seed/enable selected built-in + optional skills (synchronous)
5. spawn `agentx -p <profile> skills install <id>` per hub skill (async, returns PIDs)
...
class ProfileCreate(BaseModel):
    ...
    mcp_servers: List[MCPServerCreate] = []      # synchronous, AGENTX_HOME override
...
page afterward. Mirror `_write_profile_model`'s AGENTX_HOME-override helper for
...
- Backend E2E with isolated AGENTX_HOME: POST a full create body
