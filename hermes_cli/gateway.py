...
def get_service_name() -> str:
    ...
    Default ``~/.agentx`` returns ``hermes-gateway`` (backward compatible).
    Profile ``~/.agentx/profiles/coder`` returns ``hermes-gateway-coder``.
...
def get_launchd_plist_path() -> Path:
    ...
    Default ``~/.agentx`` → ``ai.hermes.gateway.plist`` (backward compatible).
    Profile ``~/.agentx/profiles/coder`` → ``ai.hermes.gateway-coder.plist``.
    """
    suffix = _profile_suffix()
...
def _append_node_dir_for_service(
    ...
    The Hermes-managed Node under ``$AGENTX_HOME/node`` goes first when it
...
def systemd_unit_is_current(system: bool = False) -> bool:
    ...
    #
    # Under ``sudo hermes gateway … --system``, AGENTX_HOME is often stripped
...
def systemd_install(
    ...
    # ``sudo hermes gateway install --system --force`` would bake /root/.agentx
...
def _setup_weixin():
    ...
    print_info(
        "  3. Hermes will store the returned account_id/token in ~/.agentx/.env."
    )
    print_info(
...
def _gateway_command_inner(args):
    ...
    if subcmd == "install":
        ...
        if supports_systemd_services():
            ...
            print(
                "  nohup hermes gateway run > ~/.agentx/logs/gateway.log 2>&1 &  # background"
            )
            sys.exit(1)
        elif is_container():
        ...
        if supports_systemd_services() and (
            ...
            if pids:
                ...
                if is_termux():
                    print(
                        "  nohup hermes gateway run > ~/.agentx/logs/gateway.log 2>&1 &  # Best-effort background start"
                    )
                elif is_wsl():
                    print(
                        "  tmux new -s hermes 'hermes gateway run'         # persistent via tmux"
