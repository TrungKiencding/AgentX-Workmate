"""Pydantic request/response models for the AgentX dashboard web server.
...
class ModelAssignment(BaseModel):
    ...
    # ``agentx model`` custom flow collects.
    api_key: str = ""
...
class ImportRequest(BaseModel):
    archive: str
    # Pass --force to `agentx import`. The spawned action runs with
...
class ProfileCreate(BaseModel):
    ...
    # via a subprocess scoped to the profile (`agentx -p <name> skills install`)
    # because skills_hub.SKILLS_DIR is import-time-bound and the AGENTX_HOME
...
class ProfileExport(BaseModel):
    ...
    # Where to write the archive. Empty → a staging path under AGENTX_HOME.
    output: str = ""
