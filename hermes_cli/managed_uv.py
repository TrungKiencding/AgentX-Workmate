"""AgentX-managed uv and Python runtime repair.

AgentX owns its own uv binary at ``$AGENTX_HOME/bin/uv`` (or ``uv.exe`` on
Windows).  Every code path that needs uv resolves it from that single location.
If the binary is missing, ``ensure_uv()`` bootstraps it via the official
standalone installer with ``UV_UNMANAGED_INSTALL`` / ``UV_INSTALL_DIR`` pointed
at ``$AGENTX_HOME/bin`` so the installer writes directly there — no PATH
...
def managed_uv_path() -> Path:
    """Return the path where AgentX keeps *its* uv binary.

    ``$AGENTX_HOME/bin/uv`` on POSIX, ``$AGENTX_HOME\\bin\\uv.exe`` on
...
def managed_python_env(
    ...
    *,
    ...
    """Return a sanitized environment for AgentX-private uv Python commands."""
    target = (
        Path(install_dir)
...
def _report_runtime_repair_failure(repair: RuntimeRepairResult) -> None:
    if repair.backup_venv is None:
        ...
        print(
            "    Sessions stay protected meanwhile: AgentX keeps databases "
            "out of WAL mode on this SQLite build. The next `agentx update` "
            "will retry."
        )
        return
...
class _UvResult(str):
    ...
    ``(path, fresh_bootstrap)`` tuple across releases. ``agentx update`` runs
...
def _uv_self_update_is_fresh(now: float | None = None) -> bool:
    ...
    uv releases roughly weekly while many users run ``agentx update`` daily;
    re-running a blocking network self-update on every invocation is waste
    and, offline, an unbounded hang risk. A stamp file under AGENTX_HOME
...
def update_managed_uv(
    *,
    ...
    Call this during ``agentx update`` so the managed copy stays current.
...
def _reload_hermes_constants():
    ...
    ``agentx update`` imports ``hermes_constants`` from the OLD checkout,
...
def _attempt_install_generation(
    ...
    *,
    ...
    try:
        ...
        logger.warning("uv resolved Python outside the AgentX generation: %s", python)
        _remove_tree(generation, boundary=python_root)
        return None
...
def _windows_runtime_holders() -> tuple[bool, str]:
    ...
    if holders:
        ...
        return True, f"other AgentX processes still hold the venv (PID {pids})"
    return False, ""
...
def _refresh_managed_uv_catalog(uv_bin: str) -> bool:
    ...
    unmanaged installs.  Only the AgentX-managed binary is ever refreshed;
...
def _default_live_venv(root: Path) -> Path:
    ...
    ``not-applicable`` on every ``agentx update`` and stayed on
    journal_mode=DELETE forever — even though the WAL fallback warning
    promises that ``agentx update`` repairs the runtime (issue class:
...
def _install_uv(target: Path) -> None:
    ...
    ``$AGENTX_HOME/bin/`` instead of ``~/.local/bin/``.
    """
