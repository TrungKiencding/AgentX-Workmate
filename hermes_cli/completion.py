"""Shell completion script generation for agentx CLI.
...
def generate_bash(parser: argparse.ArgumentParser) -> str:
    ...
    return f"""# AgentX Workmate bash completion
# Add to ~/.bashrc:
#   eval "$(agentx completion bash)"

_hermes_profiles() {{
    local profiles_dir="$HOME/.agentx/profiles"
...
complete -F _hermes_completion agentx
"""
...
def generate_zsh(parser: argparse.ArgumentParser) -> str:
    ...
    return f"""#compdef agentx
# AgentX Workmate zsh completion
# Add to ~/.zshrc:
...
_hermes_profiles() {{
    local -a profiles
    profiles=(default)
    if [[ -d "$HOME/.agentx/profiles" ]]; then
        profiles+=($HOME/.agentx/profiles/*(N/:t))
    fi
...
{top_cmds_str}
            ...
            _describe 'agentx command' subcmds
            ;;
        args)
            case ${{line[1]}} in
{sub_cases_str}
...
def generate_fish(parser: argparse.ArgumentParser) -> str:
    ...
    lines: list[str] = [
        "# AgentX Workmate fish completion",
        "# Add to your config:",
        "#   agentx completion fish | source",
        "",
        "# Helper: list available profiles",
        "function __hermes_profiles",
        "    echo default",
        "    if test -d $HOME/.agentx/profiles",
        "        for d in $HOME/.agentx/profiles/*/",
        "            basename $d",
        "        end",
        "    end",
        "end",
        ...
        "",
        "# Complete profile names after -p / --profile",
        "complete -c agentx -f -s p -l profile"
