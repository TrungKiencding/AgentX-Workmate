"""CLI handlers for the ``agentx proxy`` subcommand."""
...
def _print_aiohttp_missing() -> None:
    print(
        "agentx proxy requires aiohttp. Run `agentx setup` to install it.",
        file=sys.stderr,
    )
...
def cmd_proxy_start(args: Any) -> int:
    ...
    if not adapter.is_authenticated():
        auth_hint = getattr(adapter, "auth_hint", f"agentx auth add {adapter.name}")
        print(
    ...
    print(
        f"Starting AgentX proxy for {adapter.display_name}\n"
...
def cmd_proxy_status(args: Any) -> int:
    ...
    print("AgentX proxy upstream adapters\n")
    ...
    print(
        "\nStart the proxy with: agentx proxy start [--provider <name>]"
    )
    return 0
...
def cmd_proxy(args: Any) -> int:
    """Dispatch ``agentx proxy <subcommand>``."""
    ...
    print(
        "agentx proxy — local OpenAI-compatible proxy that attaches your\n"
        "OAuth-authenticated provider credentials to outbound requests.\n"
        "\n"
        "Subcommands:\n"
        "  agentx proxy start [--provider nous|xai] [--host 127.0.0.1] [--port 8645]\n"
