...
# xAI's public API is OpenAI-compatible for the endpoints AgentX commonly
# uses. The Responses endpoint is included because AgentX' native xAI runtime
...
class XAIGrokAdapter(UpstreamAdapter):
    """Proxy upstream for xAI Grok via AgentX-managed OAuth credentials."""

    auth_hint = "agentx auth add xai-oauth --type oauth"
        ...
        with self._lock:
            ...
            if pool is None or not pool.has_credentials():
                raise RuntimeError(
                    ...
                    "`agentx auth add xai-oauth --type oauth` first."
                )
            ...
            if entry is None:
                raise RuntimeError(
                    ...
                    "`agentx auth reset xai-oauth` or re-authenticate with "
                    "`agentx auth add xai-oauth --type oauth`."
                )
    ...
    def _credential_from_entry(self, entry: PooledCredential) -> UpstreamCredential:
        ...
        if not bearer:
            raise RuntimeError(
                ...
                "Re-authenticate with `agentx auth add xai-oauth --type oauth`."
            )
