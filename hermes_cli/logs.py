"""``agentx logs`` — view and filter AgentX log files.
...
Usage examples::
    ...
    agentx logs                    # last 50 lines of agent.log
    agentx logs -f                 # follow agent.log in real time
    agentx logs errors             # last 50 lines of errors.log
    agentx logs gateway -n 100    # last 100 lines of gateway.log
    agentx logs gui -f            # follow gui.log (dashboard/pty/ws)
    agentx logs desktop -f        # follow desktop.log (Electron app boot/backend)
    agentx logs --level WARNING    # only WARNING+ lines
    agentx logs --session abc123   # filter by session ID substring
    agentx logs --component tools  # only tool-related lines
    agentx logs --since 1h         # lines from the last hour
    agentx logs --since 30m -f     # follow, starting 30 min ago
"""
...
def tail_log(
    ...
    *,
    ...
    if not log_path.exists():
        ...
        print("(Logs are created when AgentX runs — try 'agentx chat' first)")
        sys.exit(1)
...
def list_logs() -> None:
    ...
    if not found:
        print("  (no log files yet — run 'agentx chat' to generate logs)")
