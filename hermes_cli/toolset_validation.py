...
the tool registry or launching AgentX (mirrors the decoupled-helper pattern used
elsewhere in the CLI).

Motivated by #38798: a config migration silently rewrote the valid toolset name
``agentx-cli`` to the non-existent ``agentx``. ``resolve_toolset('agentx')``
...
def validate_platform_toolsets(
    ...
    if valid_count == 0:
        warnings.append(
            ...
            "have no tools. Run `agentx tools` to reconfigure."
        )
    return warnings
