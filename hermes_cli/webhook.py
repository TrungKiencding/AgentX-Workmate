"""agentx webhook — manage dynamic webhook subscriptions from the CLI.
...
Subscriptions persist to ~/.agentx/webhook_subscriptions.json and are
...
def _setup_hint() -> str:
  ...
  Then start the gateway: agentx gateway run
"""
...
def webhook_command(args):
    """Entry point for 'agentx webhook' subcommand."""
    sub = getattr(args, "webhook_action", None)

    if not sub:
        print("Usage: agentx webhook {subscribe|list|remove|test}")
        print("Run 'agentx webhook --help' for details.")
        return
...
def _cmd_subscribe(args):
    ...
    print("  The gateway must be running to receive events (agentx gateway run).\n")
...
def _cmd_list(args):
    ...
    if not subs:
        ...
        print("  Create one with: agentx webhook subscribe <name>")
        return
...
def _cmd_test(args):
    ...
    payload = args.payload or '{"test": true, "event_type": "test", "message": "Hello from agentx webhook test"}'
    ...
    try:
        ...
        print(f"  Error: {e}")
        print("  Is the gateway running? (agentx gateway run)")
