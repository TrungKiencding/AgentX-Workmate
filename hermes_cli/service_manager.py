...
``agentx gateway start/stop/restart`` when running inside a container.
"""
...
def detect_service_manager() -> ServiceManagerKind:
    ...
    in ``agentx gateway start/stop/restart``).
    """
    ...
    # `agentx gateway start/stop/restart` fell through to host code that spawns
...
def _s6_running() -> bool:
    ...
    Detection has to work for **both** root and the unprivileged agentx
    ...
    detection always fails. Since every AgentX runtime call inside the
    container drops to agentx via ``s6-setuidgid``, that silent failure
...
# active (set via the ``agentx -p <profile>`` flag before the call). This
...
def _profile_dir_for_gateway_service(name: str) -> Path:
    ...
    ``gateway stop --all``. Do not write the caller's AGENTX_HOME blindly;
    derive the shared profile root from the current AGENTX_HOME and map the
    ...
    hermes_home = Path(os.environ.get("AGENTX_HOME", "/opt/data"))
...
# UID/GID of the in-image ``agentx`` user. Hardcoded to match what
...
def _seed_supervise_skeleton(svc_dir: Path) -> None:
    ...
    inside a service directory, owned by the agentx user.
    ...
    (the ``agentx`` user — UID 10000 — running every AgentX runtime
    ...
    is dispatched as the agentx user.
    ...
    with agentx ownership before triggering ``s6-svscanctl -a``,
    ...
    if not control.exists():
        ...
        control.chmod(0o660)
        try:
            os.chown(control, _AGENTX_UID, _AGENTX_GID)
        except PermissionError:
            pass
    ...
    if log_dir.is_dir():
        ...
        if not log_control.exists():
            ...
            try:
                os.chown(log_control, _AGENTX_UID, _AGENTX_GID)
            except PermissionError:
                pass
...
class GatewayNotRegisteredError(S6Error):
    ...
    Most commonly: ``agentx -p typo gateway start`` when no profile
    ...
    def __init__(self, profile: str) -> None:
        ...
        super().__init__(
            ...
            f"`agentx profile create {profile}` first, or pass "
...
class S6ServiceManager:
    ...
    def _render_run_script(
        ...
        The script:
          ...
          4. Drops to the agentx user and exec's
             ``agentx -p <profile> gateway run`` (or just ``agentx
             gateway run`` for the default profile — see below).

        Special case: ``profile == "default"`` emits ``agentx gateway
        run`` with **no** ``-p`` flag. This is the sentinel for "the
        root AGENTX_HOME profile" (the implicit profile that exists at
        the top of $AGENTX_HOME, not under profiles/). It must be
        ...
        would instead look up ``$AGENTX_HOME/profiles/default/`` — a
        ...
        each supervised profile gateway loads its own ``AGENTX_HOME``,
        ...
        # guard.
        lines.append("export AGENTX_S6_SUPERVISED_CHILD=1")
        # ``--replace`` makes the supervised gateway authoritative for its
        # profile's AGENTX_HOME. Without it, a gateway started OUTSIDE s6
        # (a stray ``agentx gateway run`` from a shell, an agent action, or
        # the Open WebUI helper) grabs the per-AGENTX_HOME PID lock first;
        ...
        # gateway/run.py) so s6 always wins. The AGENTX_S6_SUPERVISED_CHILD
        # sentinel above prevents the run→start→run redirect recursion.
        # Each profile is scoped to its own AGENTX_HOME and s6 guarantees a
        # single supervised instance per slot, so there is no legitimate
        # supervised sibling for ``--replace`` to clobber.
        if profile == "default":
            gateway_cmd = "agentx gateway run --replace"
        else:
            gateway_cmd = f"agentx -p {shlex.quote(profile)} gateway run --replace"
        # Skip the drop when already non-root (setgroups() lacks CAP_SETGID →
        # s6 boot-loop).
        lines.append(f'[ "$(id -u)" = 0 ] || exec {gateway_cmd}')
        lines.append(f"exec s6-setuidgid agentx {gateway_cmd}")
        return "\n".join(lines) + "\n"
    ...
    def _render_log_run(profile: str) -> str:
        ...
        per line, in order:
          ...
          2. ``T <log_dir>`` — also write a timestamped copy to the
             ...
             files). This is what ``agentx logs`` reads and what
        ...
        return (
            ...
            f'  s6-setuidgid agentx rm -f "$log_dir/lock"\n'
            f'else\n'
            f'  mkdir -p "$log_dir"\n'
            f'  rm -f "$log_dir/lock"\n'
            f'fi\n'
            ...
            f'exec s6-setuidgid agentx s6-log 1 n10 s1000000 T "$log_dir"\n'
        )
    ...
    def stop(self, name: str) -> None:
        ...
        intent). Without the marker, an intentional ``agentx gateway
    ...
    def register_profile_gateway(
        ...
        *,
        ...
        user explicitly runs ``agentx -p <profile> gateway start``.
        ...
        try:
            ...
            # Pre-create the supervise/ skeleton with agentx ownership
            ...
            # the agentx user) won't hit EACCES on root-owned 0700
