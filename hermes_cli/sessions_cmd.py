"""``agentx sessions`` command — extracted from ``hermes_cli/main.py``.
...
def cmd_sessions(args, sessions_parser=None):
    ...
    if action == "repair":
        ...
        if report.get("repaired"):
            ...
            print(f"    agentx sessions recover --source {source_hint} \\")
            print("        --inspect-only")
    ...
    if action == "list":
        ...
        # `agentx sessions prune`: no time window and no filters at all
        ...
        # Launch agentx --resume <id> by replacing the current process
        ...
        if result.get("vacuumed") is False:
            print("  (VACUUM was skipped or failed — run "
                  "`agentx sessions optimize` later to reclaim freed space.)")
