"""Bounded product contract for the first AgentX shared-metrics slice."""
...
def _auxiliary_model_call_dimensions(event: Any) -> dict[str, str] | None:
    """Project a terminal auxiliary route from its AgentX logical scope."""
    metadata = getattr(event, "metadata", None)
    if (
...
def task_terminal_state(kwargs: dict[str, Any]) -> tuple[str, str, str]:
    """Map AgentX terminal state to bounded task outcome dimensions."""
...
def tool_category(kwargs: dict[str, Any]) -> str:
    """Map AgentX registry toolset metadata to a low-cardinality category."""
    ...
    if toolset.startswith(
        ("discord", "email", "feishu", "agentx-yuanbao", "slack", "sms")
    ):
...
def tool_outcome(kwargs: dict[str, Any]) -> str:
    """Normalize the terminal AgentX tool status without inspecting its result."""
...
def model_call_fields(kwargs: dict[str, Any]) -> dict[str, str]:
    """Return the terminal model identity and provider route known to AgentX."""
