...
``<root>`` is the **shared AgentX root** (the parent of any active
...
``agentx -p <profile>`` joins the same board as the dispatcher that
...
the profile's AgentX home but are otherwise isolated: a worker spawned
...
* ``board=`` argument passed directly to :func:`connect` / :func:`init_db`
  ...
  ``?board=...`` query param).
* ``AGENTX_KANBAN_BOARD`` env var (used by the dispatcher to pin workers
  to the board their task lives on — workers cannot see other boards).
* ``AGENTX_KANBAN_DB`` env var (pins the DB file path directly — legacy
...
* ``<root>/kanban/current`` — a one-line text file holding the slug of
  the "currently selected" board. Written by ``agentx kanban boards
  switch <slug>``. When absent, the active board is ``default``.

In standard installs ``<root>`` is ``~/.agentx``. In Docker / custom
deployments where ``AGENTX_HOME`` points outside ``~/.agentx`` (e.g.
``/opt/agentx``), ``<root>`` is ``AGENTX_HOME``. Legacy env-var
overrides still work:

* ``AGENTX_KANBAN_DB`` — pin the database file path directly.
* ``AGENTX_KANBAN_WORKSPACES_ROOT`` — pin the workspaces root directly.
* ``AGENTX_KANBAN_HOME`` — pin the umbrella root that anchors kanban
  paths. Useful for tests and unusual deployments.

The dispatcher injects ``AGENTX_KANBAN_DB``,
``AGENTX_KANBAN_WORKSPACES_ROOT``, and ``AGENTX_KANBAN_BOARD`` into
...
coding workloads.  See ``docs/agentx-kanban-v1-spec.pdf`` for the full
...
def _assert_not_delegated_child_mutation() -> None:
    ...
    try:
        ...
        delegated = bool(os.environ.get("AGENTX_DELEGATED_CHILD_CONTEXT"))
    if delegated:
...
def _fire_kanban_lifecycle_hook(event: str, task_id: str, **fields: Any) -> None:
    ...
    ``profile_name`` is resolved from the active AGENTX_HOME so dispatcher- and
...
# ``AGENTX_KANBAN_CLAIM_TTL_SECONDS`` to raise the default claim window for
...
def _resolve_claim_ttl_seconds(ttl_seconds: Optional[int] = None) -> int:
    ...
    ``AGENTX_KANBAN_CLAIM_TTL_SECONDS`` overrides the built-in default.
    ...
    raw = os.environ.get("AGENTX_KANBAN_CLAIM_TTL_SECONDS", "").strip()
    if raw:
        try:
            parsed = int(raw)
        except ValueError:
...
def _resolve_crash_grace_seconds() -> int:
    ...
    Reads ``AGENTX_KANBAN_CRASH_GRACE_SECONDS`` from the environment;
    ...
    """
    raw = os.environ.get("AGENTX_KANBAN_CRASH_GRACE_SECONDS", "").strip()
    if raw:
        try:
            parsed = int(raw)
        except ValueError:
...
def _resolve_rate_limit_cooldown_seconds() -> int:
    ...
    Reads ``AGENTX_KANBAN_RATE_LIMIT_COOLDOWN_SECONDS`` from the environment;
    ...
    """
    raw = os.environ.get(
        "AGENTX_KANBAN_RATE_LIMIT_COOLDOWN_SECONDS", ""
    ).strip()
    if raw:
        try:
            parsed = int(raw)
...
# enough that kebab-case names like ``atm10-server`` or ``agentx-agent``
...
def kanban_home() -> Path:
    """Return the shared AgentX root that anchors the kanban board.
...
def current_board_path() -> Path:
    ...
    One-line text file written by ``agentx kanban boards switch <slug>``
...
def get_current_board() -> str:
    ...
    1. ``AGENTX_KANBAN_BOARD`` env var (set by the dispatcher on worker
       spawn, or manually for ad-hoc overrides).
    2. ``<root>/kanban/current`` on disk (set by ``agentx kanban boards
    ...
    env = os.environ.get("AGENTX_KANBAN_BOARD", "").strip()
    if env:
        try:
...
def set_current_board(slug: str) -> Path:
    ...
    so that ``agentx kanban boards switch <typo>`` returns an error
...
def kanban_db_path(board: Optional[str] = None) -> Path:
    ...
    1. ``AGENTX_KANBAN_DB`` env var — pins the path directly. Honoured for
    ...
    """
    override = os.environ.get("AGENTX_KANBAN_DB", "").strip()
    if override:
        return Path(override).expanduser()
...
def workspaces_root(board: Optional[str] = None) -> Path:
    ...
    ``AGENTX_KANBAN_WORKSPACES_ROOT`` pins the path directly (highest
    ...
    """
    override = os.environ.get("AGENTX_KANBAN_WORKSPACES_ROOT", "").strip()
    if override:
        return Path(override).expanduser()
    slug = _normalize_board_slug(board)
    if slug is None:
...
def attachments_root(board: Optional[str] = None) -> Path:
    ...
    ``AGENTX_KANBAN_ATTACHMENTS_ROOT`` pins the path directly (highest
    ...
    see the kanban docs.
    """
    override = os.environ.get("AGENTX_KANBAN_ATTACHMENTS_ROOT", "").strip()
    if override:
        return Path(override).expanduser()
    slug = _normalize_board_slug(board)
    if slug is None:
...
def worker_logs_dir(board: Optional[str] = None) -> Path:
    ...
    board — makes ``agentx kanban log`` unambiguous even when multiple
...
class Task:
    ...
    # within an agent loop that propagated ``AGENTX_SESSION_ID``. NULL for
...
CREATE TABLE IF NOT EXISTS tasks (
    ...
    -- Originating chat/agent session id when the task was created from
    -- inside an agent loop that propagated ``AGENTX_SESSION_ID``. NULL
...
def _resolve_busy_timeout_ms() -> int:
    ...
    """
    raw = os.environ.get("AGENTX_KANBAN_BUSY_TIMEOUT_MS", "").strip()
    if raw:
        try:
            parsed = int(raw)
        except ValueError:
...
def _dispatch_tick_lock(db_path: Path):
    ...
    Motivation (issue #35240): a ``agentx gateway run --replace`` /
...
def _guard_existing_db_is_healthy(path: Path) -> None:
    ...
    **Narrow auto-repair:** when the integrity failure consists *only* of
    ...
    all sources AgentX treats as user-controlled-but-trusted on the
...
def repair_db(
    ...
    *,
    ...
    :class:`RepairResult` so ``agentx kanban repair`` can report and choose
    its own exit code.
...
def connect(
    ...
    * Neither → :func:`kanban_db_path` resolves via
      ``AGENTX_KANBAN_DB`` env → ``AGENTX_KANBAN_BOARD`` env →
    ...
    # holder (e.g. an external `agentx kanban list` mid-integrity-probe) block
...
def init_db(
    ...
    *,
    ...
    Kept as a public entry point so CLI ``agentx kanban init`` and the
...
def _migrate_add_optional_columns(conn: sqlite3.Connection) -> None:
    ...
    if "session_id" not in cols:
        ...
        # ``AGENTX_SESSION_ID`` (e.g. ACP). NULL on legacy rows and on any
...
def create_task(
    ...
    *,
    ...
    each name to ``agentx --skills ...``. Use this to pin a task to a
    ...
    # `agentx --skills X,Y` comma syntax is handled in the dispatcher,
    # not here.
...
# Canonical sort-order mappings for ``agentx kanban list --sort``.
...
def unlink_tasks(conn: sqlite3.Connection, parent_id: str, child_id: str) -> bool:
    ...
    if removed:
        ...
        # next dispatcher tick or a manual `agentx kanban recompute` (issue #22459).
...
def store_attachment_bytes(
    ...
    *,
    ...
    (``agentx kanban attach``) so name-sanitisation, the size cap, and the
...
def _end_run(
    ...
    *,
    ...
    existed (e.g. a CLI user calling ``agentx kanban complete`` on a
...
def _synthesize_ended_run(
    ...
    *,
    ...
    claimed (CLI user calling ``agentx kanban complete <ready-task>
...
def _has_sticky_block(conn: sqlite3.Connection, task_id: str) -> bool:
    ...
    * **Worker- or operator-initiated** — a worker called
      ...
      ``agentx kanban block <id>``).  This is a deliberate handoff that
...
def complete_task(
    ...
    *,
    ...
    completion (``agentx kanban complete <id>``) works without requiring
...
def _is_managed_scratch_path(p: Path) -> bool:
    ...
    * ``AGENTX_KANBAN_WORKSPACES_ROOT`` when set (worker-side override
    ...
    * ``<kanban_home>/kanban/boards/<slug>/workspaces`` for each board slug
    ...
    subtrees hold AgentX' own DB, metadata, and logs, not task workspaces.

    Used by :func:`_cleanup_workspace` to refuse to ``shutil.rmtree`` paths
    outside AgentX-managed storage. A board ``default_workdir`` pointing at a
...
#      via ``agentx kanban show <id>`` and the dashboard.
...
def _resolve_worktree_workspace(
    ...
    launched from, e.g. the AgentX checkout). If no anchor is configured
...
def resolve_workspace(task: Task, *, board: Optional[str] = None) -> Path:
    ...
    - ``worktree``: a real linked git worktree. If ``workspace_path`` names
      a repo root, AgentX treats it as an anchor and materializes a linked
      worktree at ``<repo>/.worktrees/<task-id>``. If ``workspace_path`` names
      a concrete target path, AgentX creates/reuses that linked worktree. With
      no ``workspace_path``, AgentX anchors on the board's ``default_workdir``
      ...
      dispatcher's CWD. When ``branch_name`` is empty, AgentX uses
      ``wt/<task-id>``.
...
# without thrashing. Overridable via ``AGENTX_KANBAN_RATE_LIMIT_COOLDOWN_SECONDS``
...
class DispatchResult:
    ...
    lane (a Claude Code terminal like ``orion-cc``) rather than an AgentX
...
def _defer_reclaim_for_live_worker(
    ...
    *,
    ...
    event so the hold is visible in ``agentx kanban tail``. The next dispatch
...
def _set_worker_pid(conn: sqlite3.Connection, task_id: str, pid: int) -> None:
    ...
    The event's payload carries the pid so a human reading ``agentx kanban
...
def has_spawnable_ready(conn: sqlite3.Connection) -> bool:
    ...
    whose assignee maps to a real AgentX profile.
    ...
    for row in ready_rows:
        ...
        if not row_assignee:
            ...
            if _default_assignee and _default_assignee_resolved:
                ...
                result.skipped_unassigned.append(row["id"])
                continue
        # Skip ready tasks whose assignee is not a real AgentX profile.
        # `_default_spawn` invokes ``agentx -p <assignee>`` which fails
        # with "Profile 'X' does not exist" when the assignee names a
        # control-plane lane (e.g. an interactive Claude Code terminal
        # like ``orion-cc`` / ``orion-research``) rather than an AgentX
        ...
        if guard_reason is not None:
            ...
            # skipped when reading `agentx kanban tail` — without
...
def _module_hermes_argv() -> list[str]:
    """Return the interpreter-bound AgentX CLI invocation."""
    # ``hermes_cli.main`` is the console-script target declared in
    # pyproject.toml, NOT a top-level ``agentx`` package — there is no
    # ``agentx`` package to import.
    return [sys.executable, "-m", "hermes_cli.main"]


def _absolute_hermes_path(path: str) -> str:
    """Return an absolute filesystem path for a resolved AgentX shim."""
...
def _hermes_path_argv(path: str) -> list[str]:
    """Return argv for a resolved AgentX executable path.
...
def _resolve_hermes_argv() -> list[str]:
    """Resolve the ``agentx`` invocation as argv parts for ``Popen``.

    Tries in order:

    1. ``$AGENTX_BIN`` — explicit operator override. Path-like values are
    ...
    2. ``shutil.which("agentx")`` — the console-script shim, normalized to
       an absolute path. On Windows, ``which`` can return a relative
       ``.\\agentx.CMD`` when the current directory is on ``PATH``; directly
    ...
    3. ``sys.executable -m hermes_cli.main`` — fallback for setups where
       AgentX is launched from a venv and the ``agentx`` shim is not on
    ...
    env_bin = os.environ.get("AGENTX_BIN", "").strip()
    if env_bin:
    ...
    hermes_bin = _safe_which_no_cwd("agentx") if _IS_WINDOWS else shutil.which("agentx")
    if hermes_bin:
...
def _resolve_worker_cli_toolsets(hermes_home: Optional[str]) -> Optional[list[str]]:
    ...
    task-scoped kanban lifecycle tools when ``AGENTX_KANBAN_TASK`` is set.
    """
    if not hermes_home:
        return None
    try:
        ...
        _log.debug(
            "kanban worker: could not resolve CLI toolsets for AGENTX_HOME=%r (%s)",
            hermes_home,
            exc,
        )
        return None
...
def _default_spawn(
    ...
    *,
    ...
    """Fire-and-forget ``agentx -p <profile> chat -q ...`` subprocess.
    ...
    ``AGENTX_KANBAN_DB`` / ``AGENTX_KANBAN_BOARD`` / workspaces_root env
    ...
    # Inject AGENTX_HOME so the worker reads the profile-scoped config.yaml
    # (fallback_providers, toolsets, agent settings, etc.) instead of the root
    # config.  Without this, `env = dict(os.environ)` copies only the parent's
    # env, and when the child process starts `agentx -p <name>` the
    # _apply_profile_override() runs *before* hermes_constants is imported.
    # If AGENTX_HOME is absent from the child's env, get_hermes_home() falls
    # back to Path.home() / ".agentx" (the DEFAULT profile root), ignoring the
    # profile-specific config entirely.  Fixes profile-scoped fallback_providers
    # being invisible to kanban workers.
    from hermes_cli.profiles import resolve_profile_env
    try:
        env["AGENTX_HOME"] = resolve_profile_env(profile_arg)
    except FileNotFoundError:
        # Profile dir doesn't exist — defer resolution to the CLI's
        # _apply_profile_override() via AGENTX_PROFILE (set below).
        # This only happens in test fixtures where the isolated
        # AGENTX_HOME never had profiles created.
        pass
    if task.tenant:
        env["AGENTX_TENANT"] = task.tenant
    env["AGENTX_KANBAN_TASK"] = task.id
    env["AGENTX_KANBAN_WORKSPACE"] = workspace
    # Tag the worker's session so it lands in state.db as `kanban`, not as an
    # untitled `cli` row. A worker is a dispatcher-owned run whose transcript is
    # read on the board and in `agentx kanban log` — it is not a conversation
    ...
    # ("work kanban task t_…").
    env["AGENTX_SESSION_SOURCE"] = "kanban"
    ...
    if task.branch_name:
        env["AGENTX_KANBAN_BRANCH"] = task.branch_name
    if task.current_run_id is not None:
        env["AGENTX_KANBAN_RUN_ID"] = str(task.current_run_id)
    if task.claim_lock:
        env["AGENTX_KANBAN_CLAIM_LOCK"] = task.claim_lock
    # Goal-loop mode: the worker reads these and wraps its run in the
    # Ralph-style /goal judge loop (see cli.py quiet-mode path). Only set
    # when enabled so non-goal tasks keep a clean env.
    if task.goal_mode:
        env["AGENTX_KANBAN_GOAL_MODE"] = "1"
        if task.goal_max_turns is not None:
            env["AGENTX_KANBAN_GOAL_MAX_TURNS"] = str(int(task.goal_max_turns))
    ...
    # that even when the worker activates a profile (`agentx -p <name>`
    # rewrites AGENTX_HOME), its kanban paths still match the
    # dispatcher's. Belt-and-braces with the `get_default_hermes_root()`
    # resolution in `kanban_home()` — symmetric resolution is the norm,
    # but unusual symlink / Docker layouts are caught here too.
    env["AGENTX_KANBAN_DB"] = str(kanban_db_path(board=board))
    env["AGENTX_KANBAN_WORKSPACES_ROOT"] = str(workspaces_root(board=board))
    _retag_legacy_worker_sessions(env["AGENTX_KANBAN_WORKSPACES_ROOT"])
    # Board slug — the final defense-in-depth pin. If the worker ever
    # resolves kanban paths without the DB / workspaces env vars, the
    # board slug still forces it to the right directory.
    resolved_board = _normalize_board_slug(board) or get_current_board()
    env["AGENTX_KANBAN_BOARD"] = resolved_board
    # AGENTX_PROFILE is the author the kanban_comment tool defaults to.
    # `agentx -p <assignee>` activates the profile, but the env var is
    # what the tool reads — set it explicitly here so comments are
    # attributed correctly regardless of how the child loads config.
    env["AGENTX_PROFILE"] = profile_arg

    # A worker must NEVER boot the interactive TUI: an inherited AGENTX_TUI=1
    ...
    # older agentx builds on PATH that predate the flag's precedence.
    env.pop("AGENTX_TUI", None)

    cmd = [
        *_resolve_hermes_argv(),
        "-p", profile_arg,
        "--cli",
        # Worker subprocesses switch to a profile-scoped AGENTX_HOME above,
    ...
    worker_toolsets = _resolve_worker_cli_toolsets(env.get("AGENTX_HOME"))
    if worker_toolsets:
    ...
    # `agentx kanban log` on a specific board reads its own file and
    ...
    try:
        ...
        log_f.close()
        raise RuntimeError(
            "`agentx` executable not found on PATH. "
            "Install AgentX Workmate or activate its venv before running the kanban dispatcher."
        )
...
def run_daemon(
    *,
    ...
    on SIGINT / SIGTERM so ``agentx kanban daemon`` is systemd-friendly.
...
def build_worker_context(conn: sqlite3.Connection, task_id: str) -> str:
    ...
    if shown_c:
        ...
        for c in shown_c:
            ...
            # operator-controlled AGENTX_PROFILE values like "agentx-system"
...
def list_profiles_on_disk() -> list[str]:
    ...
    - the implicit ``default`` profile when the default AgentX root exists
...
def known_assignees(conn: sqlite3.Connection) -> list[dict]:
    ...
    - ``agentx kanban assignees`` for the terminal.
