"""Default configuration data for AgentX Workmate.
...
DEFAULT_CONFIG = {
    ...
    "toolsets": ["agentx-cli"],
    # SQLite journal mode used by every AgentX database opener. WAL is the
    ...
    "agent": {
        ...
        # the AgentX-level retry loop that wraps the whole call.  Lower
        ...
        # prompt's environment-hints block. Lets a host that wraps AgentX
        ...
        # identity slot (SOUL.md). Empty by default. The AGENTX_ENVIRONMENT_HINT
        # env var overrides this (build-time/container mechanism).
        "environment_hint": "",
        # Coding posture — on interactive coding surfaces (CLI, TUI, desktop
        # app, ACP) in a code workspace, AgentX adds a coding operating brief
        ...
        # ``AGENTX_LOCAL_STREAM_STALE_TIMEOUT`` overrides for escape-hatch use.
    ...
    "terminal": {
        ...
        #             AGENTX_HOME/home for persistent state (default)
        #   real    — force the real OS-user HOME
        #   profile — force AGENTX_HOME/home when it exists (old strict
        ...
        # skipped. When empty, AgentX auto-sources ``~/.profile``,
        # ``~/.bash_profile``, and ``~/.bashrc`` (in that order) if the
        # snapshot shell is bash (this is the ``auto_source_bashrc``
        # behaviour — disable with that key if you want strict login-only
        # semantics).
        "shell_init_files": [],
        # When true (default), AgentX sources the user's shell rc files
        ...
        # docker_env lets you specify exact key-value pairs — useful when AgentX
        ...
        # bundled AgentX image, which drops to the `agentx` user via
    ...
    "browser": {
        ...
        "camofox": {
            # When true, AgentX sends a stable profile-scoped userId to Camofox
            # so the server maps it to a persistent Firefox profile automatically.
            # When false (default), each session gets a random userId (ephemeral).
            "managed_persistence": False,
            # Optional externally managed Camofox identity. Useful when another
            # app owns the visible browser and AgentX should operate in it.
            "user_id": "",
            "session_key": "",
    ...
    # Opt in via ``agentx chat --checkpoints`` or set enabled=True here.
    "checkpoints": {
        "enabled": False,
        ...
        "max_snapshots": 20,
        # Hard ceiling on total ``~/.agentx/checkpoints/`` size (MB).  When
        ...
        "max_file_size_mb": 10,
        # Auto-maintenance: agentx sweeps the checkpoint base at startup
        ...
        # ``agentx checkpoints prune`` command (add ``--keep-orphans`` to
    ...
    # AGENTS.md, CLAUDE.md, .agentx.md, or .cursorrules before AgentX applies
    ...
    # Single-query (``agentx -q/-z "..."``) variant of mcp_discovery_timeout.
    ...
    # single read_file page exceeds these limits, AgentX truncates the
    ...
    "compression": {
        ...
        "codex_app_server_auto": "native",  # Codex app-server (codex CLI runtime) thread
                                      ...
                                      # thread context, so AgentX' summarizer cannot
                                      # shrink it (#36801). native = codex decides when
                                      # to compact its own thread (default); agentx =
                                      # AgentX' compression threshold triggers
    ...
    "auxiliary": {
        ...
        # Invoked by ``agentx kanban specify`` (single id or --all). Set a
        ...
        # Invoked by ``agentx kanban decompose`` and the kanban
        ...
        # ``agentx profile describe <name> --auto`` and the dashboard's
        ...
        # model; override via `agentx model` → auxiliary → Curator to route
    ...
    "display": {
        ...
        # Which interface bare `agentx` (and `agentx chat`) launches by default:
        ...
        # REPL and `--tui` (or AGENTX_TUI=1) forces the TUI regardless of config.
        "interface": "cli",
        # When true, `agentx --tui` auto-resumes the most recent human-
        # facing session on launch instead of forging a fresh one.
        # Mirrors `agentx -c` muscle memory.  Default off so existing
        # users aren't surprised.  AGENTX_TUI_RESUME=<id> always wins.
        "tui_auto_resume_recent": False,
        # When true (default), `agentx --tui` drops a one-time hint
        ...
        # can set `path`/`paths` to AGENTX_HOME-relative YAML files/directories
        ...
        # e.g. `model · 68% · ~/projects/agentx`. Per-platform overrides go under
        ...
        # CLI, TUI, and desktop app. Manage with `agentx pets`. Disabled until
    ...
    "dashboard": {
        ...
        # ``AGENTX_DASHBOARD_OAUTH_CLIENT_ID`` and
        # ``AGENTX_DASHBOARD_PORTAL_URL`` respectively — and the env var
        ...
        # (``AGENTX_DASHBOARD_BASIC_AUTH_USERNAME`` /
        ...
        # ``AGENTX_DASHBOARD_DRAIN_SECRET`` env var (the .env-is-for-secrets
        ...
        # Public URL override (env: ``AGENTX_DASHBOARD_PUBLIC_URL``).
        # When set, this is the complete authority — scheme + host +
        # optional path prefix (e.g. ``https://example.com/agentx``) —
    ...
    "tts": {
        ...
        "gemini": {
            ...
            # transcript section; AgentX appends the live transcript when absent.
            "persona_prompt_file": "",
        },
        "xai": {
        ...
        "piper": {
            ...
            # "voices_dir": "",        # Override voice cache dir; default = ~/.agentx/cache/piper-voices/
    ...
    # "Hey AgentX" hands-free wake word. Always-on, on-device hotword
    ...
    "wake_word": {
        ...
        "phrase": "hey agentx",       # for "sherpa" this IS the detected phrase (any text works); for other engines it's a cosmetic label — detection is keyed by the model/keyword below
    ...
    # a plugin in plugins/context_engine/<name>/ or ~/.agentx/plugins/.
    "context": {
        "engine": "compressor",
    ...
    "delegation": {
        ...
        # trim, the full text is spilled to ~/.agentx/cache/delegation/
    ...
    # response. If not, AgentX feeds a continuation prompt back into the
    ...
    "goals": {
        # Max continuation turns before AgentX auto-pauses the goal and
    ...
    # cron daemon.
    #
    # See `agentx curator status` for the last run summary.
    "curator": {
        "enabled": True,
        ...
        # class-level umbrellas. `agentx curator run --consolidate` overrides
        ...
        # normally restored on every `agentx update`, so pruning them only
        ...
        # skipped), snapshot ~/.agentx/skills/ into
        # ~/.agentx/skills/.curator_backups/<utc-iso>/skills.tar.gz so the
        # user can roll back with `agentx curator rollback`.
        "backup": {
            "enabled": True,
    ...
    # This section is only needed for agentx-specific overrides; everything else
    ...
    "whatsapp": {
        ...
        # Default (None) uses the built-in "⬡ *AgentX Workmate*" header.
    ...
    "approvals": {
        ...
        # false.  TUI has its own modal overlay (AGENTX_TUI_NO_CONFIRM=1 to
        # opt out there).
    ...
    # replace AgentX' built-in platform hint for a single messaging platform
    ...
    # stored approval from ~/.agentx/shell-hooks-allowlist.json.
    # See `website/docs/user-guide/features/hooks.md` for schema + examples.
    "hooks": {},

    # Auto-accept shell-hook registrations without a TTY prompt.  Also
    # toggleable per-invocation via --accept-hooks or AGENTX_ACCEPT_HOOKS=1.
    ...
    "security": {
        ...
        # longer trigger the startup banner. Add via `agentx doctor --ack
        # <id>`; remove by editing the list directly. See
        # ``hermes_cli/security_advisories.py`` for the catalog.
        "acked_advisories": [],
        # Allow AgentX to lazy-install opt-in backend packages from PyPI
    ...
    "cron": {
        ...
        # $AGENTX_HOME/plugins/<name>/) to relocate the trigger — e.g. "chronos",
        ...
        # Also overridable via AGENTX_CRON_MAX_PARALLEL env var.
        ...
        # AGENTX_CRON_SESSION_DB_TIMEOUT env var. 0 = unlimited (skip the bound).
    ...
    # todos to ready, and fires `agentx -p <assignee> chat -q ...` for
    ...
    "kanban": {
        ...
        # one `agentx` launches with no -p flag). This does not control the
        ...
        # decomposition is manual via `agentx kanban decompose <id>` or
    ...
    "code_execution": {
        ...
        #     agentx-agent's own python (sys.executable). Maximum isolation
    ...
    #
    # Core AgentX tools (terminal, read_file, write_file, patch,
    ...
    # Logging — controls file logging to ~/.agentx/logs/.
    ...
    "gateway": {
        ...
        # internal AGENTX_GATEWAY_PLATFORM_CONNECT_TIMEOUT env var, which still
        ...
        # producing ~/.agentx/sessions/sessions.json entirely.
        ...
        # the AGENTX_SCALE_TO_ZERO env stamp) AND messaging is relay-only/absent
        ...
        # `launchctl kickstart ai.agentx.gateway` that defenses 1-2 don't
        ...
        # ``AGENTX_GATEWAY_MAX_STARTS`` / ``AGENTX_GATEWAY_START_WINDOW_S``
        ...
        # system-path denylist (/etc, /proc, ~/.ssh, ~/.aws, ~/.agentx/.env,
        ...
        # behavior: files must live under the AgentX cache, under
        ...
        # user. Bridged to AGENTX_MEDIA_DELIVERY_STRICT.
        "strict": False,
        # Extra directories from which model-emitted bare file paths may be
        # uploaded as native gateway attachments. Files inside the AgentX
        # cache (~/.agentx/cache/{documents,images,audio,video,screenshots})
        ...
        # to AGENTX_MEDIA_ALLOW_DIRS at gateway startup. Tilde paths are
        ...
        # AGENTX_MEDIA_TRUST_RECENT_FILES. Only consulted when ``strict``
        # is true; in default mode the denylist alone gates delivery.
        "trust_recent_files": True,
        # Recency window in seconds. 600 (10 min) comfortably covers a
        # multi-tool agent turn. Bridged to AGENTX_MEDIA_TRUST_RECENT_SECONDS.
    ...
    # Session storage — controls automatic cleanup of ~/.agentx/state.db.
    ...
    "sessions": {
        ...
        # the default of ``agentx sessions prune``.
        "retention_days": 90,
        ...
        # rewrites ``~/.agentx/sessions/session_{sid}.json`` on every turn
        ...
        # user runs `agentx sessions optimize-storage`, because the rebuild is
        # disk-heavy and long on large DBs (see that command's disk preflight).
        #
        #   "advise" (default): `agentx update` prints a one-line notice with
        ...
        # ~/.agentx/lib/libfts5_cjk.so), 1-2 char CJK terms (일본, 项目, ...)
        ...
        # serve the cjk index. Bridged to AGENTX_CJK_FTS (internal carrier).
        "cjk_fts": True,
        ...
        # AGENTX_SEARCH_SLOW_MS (internal carrier).
        "search_slow_ms": 1000,
    },
    ...
    "updates": {
        ...
        #     DBs) into <AGENTX_HOME>/state-snapshots/ before the update.
        ...
        #     jobs) safety net.
        #   full — the quick snapshot PLUS a full ``agentx backup``-style zip
        #     of AGENTX_HOME into <AGENTX_HOME>/backups/, restorable with
        #     ``agentx import``. Can add minutes on large homes. This is the
        ...
        "backup_keep": 5,
        # What `agentx update` does with uncommitted local changes to the
    ...
    "lsp": {
        ...
        #              ``<AGENTX_HOME>/lsp/bin/`` on first use.
    ...
    # `agentx tools`. These settings tune the backing Responses API call.
    "x_search": {
    ...
    # rather than storing them in ~/.agentx/.env.
    "secrets": {
        ...
        "bitwarden": {
            ...
            # in ~/.agentx/.env (or your shell) and never in config.yaml.
            ...
            # material under ~/.agentx/cache/. If a later startup cannot reach
            # Bitwarden due to NETWORK/TIMEOUT, AgentX may use this encrypted
            ...
            # ~/.agentx/bin/ on first use.  When False you must install
            ...
            # `agentx secrets bitwarden setup`.
            "server_url": "",
        },
        "onepassword": {
            ...
            # for headless auth.  Sourced from ~/.agentx/.env (or the shell)
    ...
    "computer_use": {
        ...
        # by default upstream. AgentX disables it for our users unless they
        # explicitly opt in here. When false (default), AgentX sets
        ...
        # cua-driver ≥ 0.6.x supports --no-overlay; AgentX also calls
    ...
    #
    # Configure with `agentx egress setup`.  Disabled by default — the rest of
    # AgentX works exactly as before with `enabled: false`.
    "proxy": {
        ...
        "tunnel_port": 9090,
        # Auto-download the pinned iron-proxy binary into ~/.agentx/bin/ on
    ...
    # AgentX Workmate Desktop (Electron app) launch options. These only affect
    # `agentx desktop`; they do not touch the CLI/gateway.
    "desktop": {
        ...
        # Bridged to the AGENTX_DESKTOP_DISABLE_GPU env var the Electron app reads.
        "disable_gpu": "auto",
...
OPTIONAL_ENV_VARS = {
    ...
    "VERTEX_CREDENTIALS_PATH": {
        "description": "Path to a Google Cloud service account JSON for Vertex AI (Gemini). "
                       ...
                       "credentials AgentX mints short-lived tokens from. Falls back to "
    ...
    "AZURE_FOUNDRY_BASE_URL": {
        "description": "Azure Foundry base URL (set via 'agentx model' for endpoint-specific config)",
    ...
    "TOOL_GATEWAY_USER_TOKEN": {
        "description": "Explicit Nous Subscriber access token for tool-gateway requests (optional; otherwise read from the AgentX auth store)",
    ...
    "PORCUPINE_ACCESS_KEY": {
        "description": "Picovoice access key for the Porcupine 'Hey AgentX' wake word engine (optional; openWakeWord is the free default)",
    ...
    "SLACK_ALLOWED_USERS": {
        "description": "Comma-separated Slack member IDs allowed to use AgentX, e.g. U01ABC2DEF3. Without this, Slack may connect but deny messages by default.",
    ...
    "MATRIX_DEVICE_ID": {
        "description": "Stable Matrix device ID for E2EE persistence across restarts (e.g. AGENTX_BOT)",
    ...
    "IRC_CHANNEL": {
        "description": "IRC channel to join (e.g. #agentx)",
        "prompt": "IRC channel",
        "url": None,
        "password": False,
        "category": "messaging",
    ...
    "IRC_NICKNAME": {
        "description": "Bot nickname on IRC (default: agentx-bot)",
        "prompt": "IRC nickname",
        "url": None,
        "password": False,
        "category": "messaging",
    ...
    "API_SERVER_MODEL_NAME": {
        "description": "Model name advertised on /v1/models. Defaults to the profile name (or 'agentx-agent' for the default profile). Useful for multi-user setups with OpenWebUI.",
    ...
    "GATEWAY_PROXY_URL": {
        "description": "URL of a remote AgentX API server to forward messages to (proxy mode). When set, the gateway handles platform I/O only — all agent work is delegated to the remote server. Use for Docker E2EE containers that relay to a host agent. Also configurable via gateway.proxy_url in config.yaml.",
        "prompt": "Remote AgentX API server URL (e.g. http://192.168.1.100:8642)",
        "url": None,
        "password": False,
        "category": "messaging",
        "advanced": True,
    ...
    "GATEWAY_PROXY_KEY": {
        "description": "Bearer token for authenticating with the remote AgentX API server (proxy mode). Must match the API_SERVER_KEY on the remote host.",
    ...
    # AGENTX_TOOL_PROGRESS_MODE is deprecated — tool progress is configured via
    # display.tool_progress in config.yaml (off|new|all|verbose|log). The
    # gateway still falls back to AGENTX_TOOL_PROGRESS_MODE for backward
