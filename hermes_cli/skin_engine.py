"""AgentX skin/theme engine — the theme SDK for every surface.

A data-driven skin system that lets users (and AgentX itself) customize the
visual appearance across the CLI, the TUI, and the desktop GUI from a single
file. Skins are defined as YAML files in ~/.agentx/skins/ or as built-in presets.
...
``resolve_skin`` / ``skin.changed``). A skin dropped in ~/.agentx/skins/ therefore
...
.. code-block:: yaml
    ...
    branding:
      agent_name: "AgentX Workmate"          # Banner title, status display
      welcome: "Welcome message"          # Shown at CLI startup
      goodbye: "Goodbye! ⬡"              # Shown on exit
      response_label: " ⬡ AgentX "       # Response box header label
...
.. code-block:: python
    ...
    print(skin.get_branding("agent_name"))  # "AgentX Workmate"

    set_active_skin("ares")               # Switch to built-in ares skin
    set_active_skin("mytheme")            # Switch to user skin from ~/.agentx/skins/
...
Drop a YAML file in ``~/.agentx/skins/<name>.yaml`` following the schema above.
...
class SkinConfig:
    ...
    banner_logo: str = ""    # Rich-markup ASCII art logo (replaces AGENTX_AGENT_LOGO)
    banner_hero: str = ""    # Rich-markup hero art (replaces AGENTX_CADUCEUS)
...
_BUILTIN_SKINS: Dict[str, Dict[str, Any]] = {
    "default": {
        "name": "default",
        "description": "Classic AgentX — gold and kawaii",
        # Dark-authored. Values match the TUI's DARK_THEME so the classic CLI
        # and the TUI render the same AgentX gold.
        "colors": {
        ...
        "branding": {
            ...
            "welcome": "Welcome to AgentX Workmate! Type your message or /help for commands.",
            "goodbye": "Goodbye! ⬡",
            "response_label": " ⬡ AgentX ",
            "prompt_symbol": "❯",
