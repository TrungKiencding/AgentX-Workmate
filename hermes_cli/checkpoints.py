"""`agentx checkpoints` CLI subcommand.

Gives users direct visibility and control over the filesystem checkpoint
store at ``~/.agentx/checkpoints/``.  Actions:

    agentx checkpoints               # same as `status`
    agentx checkpoints status        # total size, project count, breakdown
    agentx checkpoints list          # per-project checkpoint counts + workdir
    agentx checkpoints prune [opts]  # force a sweep (ignores the 24h marker)
    agentx checkpoints clear [-f]    # nuke the entire base (asks first)
    agentx checkpoints clear-legacy  # delete just the legacy-* archives

Examples::

    agentx checkpoints
    agentx checkpoints prune --retention-days 3 --max-size-mb 200
    agentx checkpoints clear -f
...
def cmd_status(args: argparse.Namespace) -> int:
    ...
    if legacy:
        ...
        print()
        print("Clear with: agentx checkpoints clear-legacy")
    return 0
...
def register_cli(parser: argparse.ArgumentParser) -> None:
    """Wire subcommands onto the ``agentx checkpoints`` parser."""
    parser.set_defaults(func=cmd_status)  # bare `agentx checkpoints` → status
