"""agentx memory setup|status — configure memory provider plugins.
...
def _provider_pip_dependencies(provider_name: str, declared: list) -> list:
    ...
    ``agentx memory setup`` — if the update-time refresh only reinstalled
...
# ---------------------------------------------------------------------------
# Curses-based interactive picker (same pattern as agentx tools)
# ---------------------------------------------------------------------------
...
def _install_dependencies(provider_name: str, *, force: bool = False) -> None:
    ...
    ranges. This is how ``agentx update`` heals the active memory provider
    ...
    # data volume (AGENTX_LAZY_INSTALL_TARGET). install_specs handles the
...
def cmd_setup_provider(provider_name: str) -> None:
    ...
    if not match:
        ...
        print("  Run 'agentx memory setup' to see available providers.\n")
        return
...
def cmd_setup(args) -> None:
    ...
    if not providers:
        ...
        print("  Install a plugin to ~/.agentx/plugins/ and try again.\n")
        return
...
def cmd_status(args) -> None:
    ...
    # canonical resolver (handles composite toolsets like agentx-cli).
    ...
    if provider_name:
        ...
        if provider:
            ...
            print(f"  Install the '{provider_name}' memory plugin to ~/.agentx/plugins/")
