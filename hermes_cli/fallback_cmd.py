"""
agentx fallback — manage the fallback provider chain.
...
Subcommands:
  agentx fallback [list]   Show the current fallback chain (default when no subcommand)
  agentx fallback add      Pick provider + model via the same picker as `agentx model`,
                           then append the selection to the chain
  agentx fallback remove   Pick an entry to delete from the chain
  agentx fallback clear    Remove all fallback entries

Storage: ``fallback_providers`` in ``~/.agentx/config.yaml`` (top-level, list of
...
def _restore_auth_active_provider(value: Any) -> None:
    ...
    try:
        ...
        # `agentx model` to fix it.  Don't fail the fallback add.
        pass
...
def cmd_fallback_list(args) -> None:  # noqa: ARG001
    ...
    if not chain:
        ...
        print()
        print("  Add one with:  agentx fallback add")
        print()
        return
    ...
    print("  Docs: https://github.com/TrungKiencding/AgentX-Workmate/blob/main/website/docs/user-guide/features/fallback-providers.md")
    print()
...
def cmd_fallback_add(args) -> None:
    """Launch the same picker as `agentx model`, then append the selection to the chain."""
    ...
    print("  `agentx model` — select the provider + model you want as a fallback.")
    print()
    ...
    print()
    print("  Run `agentx fallback list` to view, or `agentx fallback remove` to delete.")
...
def cmd_fallback(args) -> None:
    """Top-level dispatcher for ``agentx fallback [subcommand]``."""
