...
The provider list shown by ``agentx model`` (CLI/TUI) and the desktop Settings
...
the same universe ``agentx model`` renders (``CANONICAL_PROVIDERS``), joining:
...
``CANONICAL_PROVIDERS`` universe, i.e. exactly what ``agentx model`` shows.
"""
...
class ProviderDescriptor:
    ...
    order: int                     # CANONICAL_PROVIDERS index — mirrors `agentx model`
...
def provider_catalog() -> list[ProviderDescriptor]:
    """Return one descriptor per provider in the ``agentx model`` universe.
    ...
    # AgentX overlays carry auth_type for providers that have no registry/profile
    ...
    for order, entry in enumerate(CANONICAL_PROVIDERS):
        ...
        overlay = AGENTX_OVERLAYS.get(slug)

        # auth_type: registry is authoritative; fall back to profile, then the
        # AgentX overlay (e.g. moa → "virtual"), then api_key.
        auth_type = (
