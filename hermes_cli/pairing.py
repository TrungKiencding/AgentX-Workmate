...
Usage:
    agentx pairing list              # Show all pending + approved users
    agentx pairing approve <platform> <request-id|code>  # Approve a pairing request
    agentx pairing revoke <platform> <user_id> # Revoke user access
    agentx pairing clear-pending     # Clear all expired/pending codes
"""

def pairing_command(args):
    """Handle agentx pairing subcommands."""
    ...
    if action == "list":
        ...
        print("Usage: agentx pairing {list|approve|revoke|clear-pending}")
        print("Run 'agentx pairing --help' for details.")
...
def _cmd_list(store):
    ...
    if pending:
        ...
        print("\n  Approve with: agentx pairing approve <platform> <request-id>")
...
def _cmd_approve(store, platform: str, code: str):
    ...
    if result:
        ...
        print(
            ...
            "~/.agentx/platforms/pairing/_rate_limits.json\n".format(platform)
        )
    else:
        print(f"\n  Pairing request or code '{code}' not found or expired for platform '{platform}'.")
        print("  Run 'agentx pairing list' to see pending requests.\n")
