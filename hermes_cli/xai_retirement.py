...
Pure logic: walks an AgentX config dict, returns issues for any reference
to a retired xAI model. No I/O, no CLI dependencies — testable in isolation
and reusable from both `agentx doctor` and a future `agentx migrate xai`.
"""
...
class RetirementIssue:
    """A reference to a retired xAI model found in an AgentX config."""
...
def find_retired_xai_refs(config: Dict[str, Any]) -> List[RetirementIssue]:
    """Walk all model slots in an AgentX config and return retirement issues.
