"""``agentx skills`` subcommand parser.
...
def build_skills_parser(subparsers, *, cmd_skills: Callable) -> None:
    ...
    skills_update.add_argument(
        "--overwrite-local",
        action="store_true",
        help="Also update skills edited on this machine (the edited copy is backed up to skills/.hub/backups first)",
    )
    ...
    skills_reset = skills_subparsers.add_parser(
        ...
        description=(
            "Clear a bundled skill's entry from the sync manifest (~/.agentx/skills/.bundled_manifest) "
            "so future 'agentx update' runs stop marking it as user-modified. Pass --restore to also "
    ...
    skills_list_modified = skills_subparsers.add_parser(
        "list-modified",
        help="List bundled skills you've edited (which `agentx update` keeps)",
        description=(
            "Show the bundled skills whose local copy differs from the version last "
            "synced, i.e. the ones `agentx update` reports as user-modified and skips. "
            "Use `agentx skills diff <name>` to see changes and `agentx skills reset "
    ...
    skills_opt_out = skills_subparsers.add_parser(
        ...
        description=(
            ...
            "`agentx update`, and any direct sync stop seeding bundled skills "
    ...
    skills_opt_in = skills_subparsers.add_parser(
        ...
        description=(
            ...
            "again on the next `agentx update`. Pass --sync to re-seed now."
        ),
    )
