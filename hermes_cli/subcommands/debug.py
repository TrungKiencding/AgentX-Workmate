"""``agentx debug`` subcommand parser.
...
def build_debug_parser(subparsers, *, cmd_debug: Callable) -> None:
    ...
    debug_parser = subparsers.add_parser(
        ...
        description="Debug utilities for AgentX Workmate. Use 'agentx debug share' to "
        "upload a debug report (system info + recent logs) to a paste "
        "service and get a shareable URL.",
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog="""\
Examples:
    agentx debug share              Upload debug report (asks for confirmation)
    agentx debug share --yes        Skip confirmation (for scripts/CI)
    agentx debug share --lines 500  Include more log lines
    agentx debug share --expire 30  Keep paste for 30 days
    agentx debug share --local      Print report locally (no upload)
    agentx debug share --no-redact  Disable upload-time secret redaction
    agentx debug share --nous       Upload to Nous-internal storage (private)
    agentx debug delete <url>       Delete a previously uploaded paste
""",
    )
    ...
    delete_parser = debug_sub.add_parser(
        "delete",
        help="Delete a paste uploaded by 'agentx debug share'",
    )
