"""``agentx pause`` / ``agentx resume`` — the global emergency stop.

``agentx pause`` writes the ESTOP sentinel at ``$AGENTX_HOME/ESTOP``, which
halts cron dispatch, kanban dispatch, and new gateway turns on their next
check. In-flight work is never killed. ``agentx resume`` removes the
...
def cmd_pause(args: argparse.Namespace) -> int:
    ...
    state = get_state() or {}
    verb = "Still paused" if already else "AgentX paused"
    ...
    print(
        ...
        "    In-flight work keeps running. Run `agentx resume` to lift the pause."
    )
    return 0
...
def cmd_resume(args: argparse.Namespace) -> int:
    ...
    if disengage():
        print("▶️  AgentX resumed — dispatch picks up on the next tick.")
    else:
        print(f"AgentX is not paused (no sentinel at {sentinel_path()}).")
    return 0
...
def build_pause_parser(subparsers) -> None:
    ...
    pause_parser = subparsers.add_parser(
        ...
        description=(
            ...
            "`agentx resume`. In-flight work is never killed."
        ),
    )
    ...
    resume_parser = subparsers.add_parser(
        "resume",
        help="Lift the emergency stop set by `agentx pause`",
