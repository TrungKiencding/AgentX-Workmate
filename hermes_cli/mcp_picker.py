"""MCP picker — interactive `agentx mcp picker` (also the default `agentx mcp`).

Lists every catalog entry plus any custom MCP servers the user has added via
``agentx mcp add``, lets them pick one, and routes to install / enable /
disable / uninstall / configure-tools flows.

Mirrors the `agentx plugin` picker UX: arrow keys to navigate, ENTER on a row
...
def _enable_disable(name: str, *, enable: bool) -> None:
    ...
    print(color(
        ...
        "Start a new AgentX session for changes to take effect.",
        Colors.GREEN,
    ))
...
def _print_rows_text(rows: List[_Row]) -> None:
    ...
    as the default output of `agentx mcp catalog`."""
    if not rows:
        print()
    ...
    print()
    print(color(
        "  Install: agentx mcp install <name>    Picker: agentx mcp",
        Colors.DIM,
    ))

    # Surface manifest-version warnings so users know when their AgentX is
    ...
    if future:
        ...
        for name, _, msg in future:
            print(color(
                f"  ⚠ '{name}' requires a newer AgentX — run `agentx update` "
...
def show_catalog() -> None:
    """`agentx mcp catalog` — print the curated list + custom servers, no interaction."""
    _print_rows_text(_build_rows())


def run_picker() -> None:
    """`agentx mcp picker` (and default `agentx mcp`) — interactive selector.
...
def install_by_name(identifier: str) -> int:
    ...
    if entry is None:
        print(color(
            ...
            "Run `agentx mcp catalog` to see available entries.",
            Colors.RED,
        ))
        return 1
    try:
