...
try:
    ...
    # running `agentx dashboard` needs fastapi+uvicorn; lazy install keeps
...
WEB_DIST = Path(os.environ["AGENTX_WEB_DIST"]) if "AGENTX_WEB_DIST" in os.environ else Path(__file__).parent / "web_dist"
...
def _start_desktop_cron_ticker(stop_event: "threading.Event", interval: int = 60) -> None:
    ...
    The scheduler tick loop normally lives in ``agentx gateway run`` — but the
    desktop app spawns a ``agentx dashboard`` backend, not a gateway, so a cron
    ...
    real gateway on the same AGENTX_HOME — whichever process grabs the lock
    first wins the tick.
    """
...
app = FastAPI(title="AgentX Workmate", version=__version__, lifespan=_lifespan)
...
# AGENTX_DASHBOARD_SESSION_TOKEN so its main process can authenticate the
...
def _resolve_session_token() -> str:
    return os.environ.get("AGENTX_DASHBOARD_SESSION_TOKEN") or secrets.token_urlsafe(32)


_SESSION_TOKEN = _resolve_session_token()
_SESSION_HEADER_NAME = "X-Agentx-Session-Token"
...
def _require_token(request: Request) -> None:
    ...
    * **Loopback / ``--insecure`` mode** (``auth_required`` False): the
      ...
      back via ``X-Agentx-Session-Token`` (or the legacy ``Bearer`` header).
      Validate it here.
...
_SCHEMA_OVERRIDES: Dict[str, Dict[str, Any]] = {
    ...
    "proxy.enabled": {
        "type": "boolean",
        "description": (
            "Docker-only egress credential firewall. Requires `agentx egress setup` "
            "and `agentx egress start`; Modal/SSH/Daytona are not wired yet."
        ),
        "category": "security",
    },
    ...
    "updates.non_interactive_local_changes": {
        "type": "select",
        "description": (
            "When the chat app / gateway updates AgentX (no terminal prompt), "
    ...
    "updates.refresh_cua_driver": {
        "type": "boolean",
        "description": (
            "Refresh an already-installed cua-driver during agentx update. "
...
def _normalize_main_model_assignment(provider: str, model: str) -> tuple[str, str]:
    ...
    - The "Change" picker sends a real AgentX provider slug — fine.
    ...
    1. Vendor-name → AgentX-provider mapping: when the provider string is
       not a known AgentX provider/alias (e.g. ``moonshotai``, ``x-ai`` is
    ...
    # not be treated as a stray vendor prefix -- it isn't a known AgentX
...
def _count_status_active_sessions() -> int:
    ...
    while another AgentX process is writing to it.
    """
...
_MANAGED_FILES_ROOT_ENV = "AGENTX_DASHBOARD_FILES_ROOT"
...
# AGENTX_HOME itself, at which point every one of these basenames is a live
...
# without needing to resolve them relative to AGENTX_HOME.
...
def _is_sensitive_filename(name: str) -> bool:
    ...
    canonical AgentX credential-store basenames (see
...
def _is_sensitive_path(path: Path) -> bool:
    ...
    and needs no AGENTX_HOME resolution, so it blocks these trees wherever
...
def _default_hermes_root_is_opt_data() -> bool:
    raw = os.environ.get("AGENTX_HOME", "").strip()
    if not raw:
        return False
    try:
...
def _dashboard_local_update_managed_externally() -> bool:
    """Return true when the dashboard should not offer ``agentx update``.
    ...
    a container — e.g. the agentx-webui image sharing the AgentX source tree),
    the dashboard's ``agentx update`` button is the correct update path and
...
def _managed_files_policy(request: Request, *, create_root: bool = True) -> ManagedFilesPolicy:
    ...
    # to /opt/data only when the installation's AgentX root is actually /opt/data
    # (the container/hosted layout) or when AGENTX_DASHBOARD_FILES_ROOT is set.
    if _default_hermes_root_is_opt_data():
...
async def upload_chat_image(payload: ChatImageUpload, profile: Optional[str] = None):
    ...
    The dashboard /chat page runs AgentX inside an xterm.js PTY. Browser
    ...
    ``AGENTX_HOME/images/`` — the same directory ``clipboard.paste`` /
...
async def get_status(profile: Optional[str] = None):
    ...
    try:
        ...
        # AGENTX_HOME (see issue #69143). Plain /api/status keeps the exact
        ...
        # and which providers are registered so ``agentx status`` and the
        ...
        # ``gateway_mode`` are low-sensitivity PRODUCT surface — AgentX Cloud
...
async def get_system_stats():
    ...
    and non-sensitive (no env values, no paths beyond the agentx home root).
    """
...
# streamed to a per-action log file under ``~/.agentx/logs/<action>.log`` so
...
def _spawn_hermes_action(subcommand: List[str], name: str) -> subprocess.Popen:
    ...
    # _AGENTX_GATEWAY=1. Inheriting it makes a spawned `agentx gateway restart`
...
def _gateway_display_command(profile: Optional[str], verb: str) -> str:
    return " ".join(["agentx", *_gateway_subcommand(profile, verb)])
...
def _spawn_gateway_restart(profile: Optional[str] = None) -> Tuple[subprocess.Popen, bool]:
    """Spawn ``agentx gateway restart``, reusing an in-flight restart.
    ...
    concurrent ``agentx gateway restart`` children race each other on the
...
async def restart_gateway(profile: Optional[str] = None):
    """Kick off a ``agentx gateway restart`` in the background."""
    try:
...
async def gateway_drain(request: Request):
    ...
    active (no ``AGENTX_DASHBOARD_DRAIN_SECRET``), the route is NOT a token
...
async def update_hermes():
    ...
    if _dashboard_local_update_managed_externally():
        message = (
            "AgentX updates are managed outside this dashboard in "
            "containerized environments. The built-in local updater is "
            "disabled here."
        )
        _record_completed_action("agentx-update", message, exit_code=1)
        return {
            "ok": False,
            "pid": None,
            "name": "agentx-update",
    ...
    try:
        proc = _spawn_hermes_action(["update"], "agentx-update")
    except Exception as exc:
        _log.exception("Failed to spawn agentx update")
...
async def check_hermes_update(force: bool = False):
    ...
    Returns:
        ...
        current_version: installed AgentX version string
...
def _normalize_config_for_web(config: Dict[str, Any]) -> Dict[str, Any]:
    ...
    AgentX supports ``model`` as either a bare string (``"anthropic/claude-sonnet-4"``)
...
def get_recommended_default_model(provider: str = ""):
    ...
    Mirrors the model-curation `agentx model` does so GUI onboarding lands on a
...
async def set_model_assignment(body: ModelAssignment, profile: Optional[str] = None):
    ...
    Writes to ``~/.agentx/config.yaml`` — applies to **new** sessions only.
...
def _apply_model_assignment_sync(
    ...
    if scope == "main":
        ...
        # endpoint, mirroring the ``agentx model`` custom flow
...
def _catalog_provider_env_metadata() -> dict:
    ...
    (i.e. the ``agentx model`` universe). This is what lets the desktop Keys tab
    ...
    for d in provider_catalog():
        ...
        # Keys tab (otherwise Bedrock — a `agentx model` provider — would be
        ...
        # Vertex — a `agentx model` provider — would be invisible in the desktop
...
async def get_env_vars(profile: Optional[str] = None):
    ...
    def _row(var_name: str, info: dict, *, custom: bool = False) -> dict:
        ...
        return {
            ...
            # CLI `agentx model` picker uses (not desktop-only prefix guesses).
...
_PLATFORM_OVERRIDES: dict[str, dict[str, Any]] = {
    "telegram": {
        "name": "Telegram",
        "description": "Run AgentX from Telegram DMs, groups, and topics.",
    ...
    "discord": {
        "name": "Discord",
        "description": "Connect AgentX to Discord DMs, channels, and threads.",
    ...
    "slack": {
        "name": "Slack",
        "description": "Use AgentX from Slack via Socket Mode. Add allowed Slack member IDs so connected bots can respond.",
    ...
    "mattermost": {
        "name": "Mattermost",
        "description": "Connect AgentX to Mattermost channels and direct messages.",
    ...
    "matrix": {
        "name": "Matrix",
        "description": "Use AgentX in Matrix rooms and direct messages.",
    ...
    "whatsapp": {
        "name": "WhatsApp",
        "description": "Use AgentX through the bundled WhatsApp bridge with QR-based auth.",
    ...
    "homeassistant": {
        "name": "Home Assistant",
        "description": "Control your smart home from AgentX via Home Assistant.",
    ...
    "email": {
        "name": "Email",
        "description": "Talk to AgentX through an IMAP/SMTP mailbox.",
        "docs_url": "https://github.com/TrungKiencding/AgentX-Workmate/blob/main/website/docs/user-guide/messaging/index.md",
        "env_vars": (
            "EMAIL_ADDRESS",
            "EMAIL_PASSWORD",
            "EMAIL_IMAP_HOST",
    ...
    "dingtalk": {
        "name": "DingTalk",
        "description": "Connect AgentX to DingTalk groups (钉钉).",
    ...
    "feishu": {
        "name": "Feishu / Lark",
        "description": "Use AgentX inside Feishu / Lark.",
    ...
    "google_chat": {
        "name": "Google Chat",
        "description": "Connect AgentX to Google Chat via Cloud Pub/Sub.",
        "docs_url": "https://github.com/TrungKiencding/AgentX-Workmate/blob/main/website/docs/user-guide/messaging/google_chat.md",
    },
    "wecom": {
    ...
    "weixin": {
        ...
        "docs_url": "https://github.com/TrungKiencding/AgentX-Workmate/blob/main/website/docs/user-guide/messaging/weixin.md",
        "env_vars": ("WEIXIN_ACCOUNT_ID", "WEIXIN_TOKEN", "WEIXIN_BASE_URL"),
        "required_env": ("WEIXIN_ACCOUNT_ID", "WEIXIN_TOKEN"),
    },
    "bluebubbles": {
        "name": "BlueBubbles (iMessage)",
        "description": "Use AgentX through iMessage via a BlueBubbles server.",
    ...
    "qqbot": {
        "name": "QQ Bot",
        "description": "Connect AgentX to a QQ Bot from the QQ Open Platform.",
    ...
    "teams": {
        "description": "Connect AgentX to Microsoft Teams chats via the Bot Framework.",
        "docs_url": "https://github.com/TrungKiencding/AgentX-Workmate/blob/main/website/docs/user-guide/messaging/teams.md",
    },
    ...
    "irc": {
        "description": "Relay messages between an IRC channel (or DMs) and AgentX.",
        "docs_url": "https://github.com/TrungKiencding/AgentX-Workmate/blob/main/website/docs/user-guide/messaging/irc.md",
    },
    "line": {
        "description": "Use AgentX from LINE via the LINE Messaging API webhook.",
        "docs_url": "https://github.com/TrungKiencding/AgentX-Workmate/blob/main/website/docs/user-guide/messaging/line.md",
    },
    "ntfy": {
        "description": "Chat with AgentX over ntfy push topics (ntfy.sh or self-hosted).",
        "docs_url": "https://github.com/TrungKiencding/AgentX-Workmate/blob/main/website/docs/user-guide/messaging/ntfy.md",
    },
    "photon": {
        "description": "Use AgentX through iMessage via Photon's managed Spectrum platform.",
        "docs_url": "https://github.com/TrungKiencding/AgentX-Workmate/blob/main/website/docs/user-guide/messaging/photon.md",
    },
    "raft": {
        "description": "Join a Raft workspace as an external agent.",
        "docs_url": "https://github.com/TrungKiencding/AgentX-Workmate/blob/main/website/docs/user-guide/messaging/raft.md",
    },
    "simplex": {
        "description": "Talk to AgentX over SimpleX Chat via a local simplex-chat daemon.",
        "docs_url": "https://github.com/TrungKiencding/AgentX-Workmate/blob/main/website/docs/user-guide/messaging/simplex.md",
    },
    "yuanbao": {
        "name": "Yuanbao (元宝)",
        "description": "Connect AgentX to Tencent Yuanbao.",
        "docs_url": "",
        "required_env": (),
    },
    "api_server": {
        "name": "API server",
        "description": "Expose AgentX as an OpenAI-compatible HTTP API for tools like Open WebUI.",
        "docs_url": "https://github.com/TrungKiencding/AgentX-Workmate/blob/main/website/docs/user-guide/messaging/index.md",
        "env_vars": (
            "API_SERVER_ENABLED",
            "API_SERVER_KEY",
    ...
    "webhook": {
        ...
        "docs_url": "https://github.com/TrungKiencding/AgentX-Workmate/blob/main/website/docs/user-guide/messaging/webhooks.md",
    ...
    "msgraph_webhook": {
        ...
        "docs_url": "https://github.com/TrungKiencding/AgentX-Workmate/blob/main/website/docs/user-guide/messaging/msgraph-webhook.md",
        "required_env": (),
    },
    "whatsapp_cloud": {
        "name": "WhatsApp Cloud API",
        "description": "Use AgentX via Meta's hosted WhatsApp Cloud API (no local bridge).",
        "docs_url": "https://github.com/TrungKiencding/AgentX-Workmate/blob/main/website/docs/user-guide/messaging/whatsapp-cloud.md",
    },
    "relay": {
        "name": "Relay (experimental)",
        "description": "Generic relay adapter fronted by the AgentX Relay connector.",
        "docs_url": "",
        "required_env": (),
    },
}
...
_MESSAGING_ENV_FALLBACKS: dict[str, dict[str, Any]] = {
    ...
    "WEIXIN_ACCOUNT_ID": {
        "description": "iLink Bot account ID obtained through QR login in agentx gateway setup",
        "prompt": "iLink Bot account ID",
    },
    "WEIXIN_TOKEN": {
        "description": "iLink Bot token obtained through QR login in agentx gateway setup",
...
# hermes_cli/setup_hidden_env.py. Shared with the `agentx setup gateway`
...
def _messaging_platform_payload(
    ...
    # AGENTX_HOME contextvar override (#56986 / #69143), so the profile's
...
_TELEGRAM_ONBOARDING_DEFAULT_URL = "https://github.com/TrungKiencding/AgentX-Workmate"
...
async def start_telegram_onboarding(body: TelegramOnboardingStart):
    bot_name = (body.bot_name or "AgentX Workmate").strip() or "AgentX Workmate"
...
def _restart_gateway_after_telegram_onboarding(profile: Optional[str] = None) -> dict[str, Any]:
    ...
    saved token waits on a separate dashboard restart click, AgentX appears
...
async def get_messaging_platforms(profile: Optional[str] = None):
    ...
    # load_env() honors the AGENTX_HOME contextvar override; the gateway
...
def _multiplex_port_binding_conflict(
    ...
    *enabling* is blocked; disabling/clearing stays allowed so users can
    ...
    if not requested or requested.lower() == "current":
        ...
        # The dashboard's own profile. "custom" (an unrecognized AGENTX_HOME)
...
# the canonical ``agentx auth add <provider>`` command so the dashboard
...
def _anthropic_oauth_status() -> Dict[str, Any]:
    ...
    1. ``~/.agentx/.anthropic_oauth.json`` — AgentX-managed PKCE flow (what
    ...
    if hermes_creds and hermes_creds.get("accessToken"):
        return {
            "logged_in": True,
            "source": "hermes_pkce",
            "source_label": f"AgentX PKCE ({_get_hermes_oauth_file() if _get_hermes_oauth_file else None})",
...
def _claude_code_only_status() -> Dict[str, Any]:
    ...
    Claude Code subscription tokens are actively flowing into AgentX even
    when they also have a separate AgentX-managed PKCE login.
    """
    try:
...
def _copilot_acp_status() -> Dict[str, Any]:
    ...
    AgentX never claims a login state it can't verify.
    """
    return {
        "logged_in": False,
        "source": "copilot_cli",
...
_OAUTH_PROVIDER_CATALOG: tuple[Dict[str, Any], ...] = (
    {
        ...
        "name": "Nous Portal",
        "flow": "device_code",
        "cli_command": "agentx auth add nous",
    ...
    },
    {
        ...
        "flow": "device_code",
        "cli_command": "agentx auth add openai-codex",
    ...
    },
    {
        ...
        "flow": "external",
        "cli_command": "agentx auth add qwen-oauth",
    ...
    },
    {
        ...
        "flow": "device_code",
        "cli_command": "agentx auth add minimax-oauth",
    ...
    },
    {
        ...
        # 127.0.0.1 callback.
        "flow": "device_code",
        "cli_command": "agentx auth add xai-oauth",
        "docs_url": "https://github.com/TrungKiencding/AgentX-Workmate/blob/main/website/docs/guides/xai-grok-oauth.md",
    ...
    # usage credits on top of a Claude Max plan — see disclaimer in name).
    {
        ...
        "flow": "pkce",
        "cli_command": "agentx auth add anthropic",
...
def _oauth_provider_disconnect_command(provider: Dict[str, Any]) -> Optional[str]:
    ...
    External providers store their credentials outside AgentX, so the disconnect
    ...
    user sees exactly what executes, and AgentX then stops resolving the token.
...
def _oauth_provider_disconnect_hint(provider: Dict[str, Any], status: Dict[str, Any]) -> Optional[str]:
    ...
    if provider.get("flow") == "external":
        if _oauth_provider_disconnect_command(provider):
            ...
            return "Managed outside AgentX — run the disconnect command to remove it."
...
def _build_oauth_catalog() -> list[Dict[str, Any]]:
    ...
    MEMBERSHIP is the union of:
      ...
      2. every accounts-tab provider in the unified ``provider_catalog()`` (the
         ``agentx model`` universe) — so any OAuth/external provider added as a
    ...
    #    in lockstep with the `agentx model` universe (zero-edit for new plugins).
    try:
        from hermes_cli.provider_catalog import provider_catalog
        for d in provider_catalog():
            ...
            rows.append({
                ...
                "name": d.label,
                "flow": "external",
                "cli_command": f"agentx auth add {d.slug}",
...
async def list_oauth_providers(profile: Optional[str] = None):
    ...
    sync with the `agentx model` picker; _OAUTH_OVERRIDES supplies per-provider
...
async def disconnect_oauth_provider(
    ...
    with _profile_scope(profile):
        ...
        # Anthropic clears only the AgentX-managed PKCE file and auth-store entry.
...
#          → persists to ~/.agentx/.anthropic_oauth.json AND credential pool
...
# Guarded so agentx web still starts if anthropic_adapter is unavailable;
...
def _save_anthropic_oauth_creds(access_token: str, refresh_token: str, expires_at_ms: int) -> None:
    """Persist Anthropic PKCE creds to both AgentX file AND credential pool.

    Mirrors what auth_commands.add_command does so the dashboard flow leaves
    the system in the same state as ``agentx auth add anthropic``.
    """
    from agent.anthropic_adapter import _get_hermes_oauth_file
    oauth_file = _get_hermes_oauth_file()
    payload = {
...
async def _start_device_code_flow(
    ...
    if provider_id == "nous":
        ...
        portal_base_url = (
            os.getenv("AGENTX_PORTAL_BASE_URL")
...
def _xai_device_poller(session_id: str) -> None:
    ...
    try:
        ...
        with _profile_scope(_oauth_session_profile(session_id)):
            ...
            # Mirror `agentx auth add xai-oauth`: first credential may become
            ...
            # prior ``agentx auth remove xai-oauth`` (mirrors auth_add_command
            # and the ``agentx model`` re-login path in _login_xai_oauth).
...
def _codex_device_code_start_error(resp: Any) -> str:
    ...
    if "device" in lower and ("authori" in lower or "enable" in lower):
        message = (
            ...
            "account may need device-code authorization enabled before AgentX "
...
def _maybe_auto_archive_for_profile(profile: Optional[str]) -> None:
    ...
    The Desktop backend is spawned as ``agentx serve`` — it runs neither the
...
def _prune_sessions(body: SessionPrune):
    """Delete ended sessions matching filters (mirrors `agentx sessions prune`)."""
    has_window = (
...
def _cron_default_profile() -> str:
    ...
    A desktop pool backend runs one process per profile (AGENTX_HOME already
    ...
    fallback would write a non-default profile's job into ``~/.agentx``.
    Resolve the process's own profile instead. ``custom`` (an unrecognized
    AGENTX_HOME outside the profiles tree) has no profile-dir equivalent, so
...
def _cron_profile_home(profile: Optional[str]) -> Tuple[str, Path]:
    """Resolve a profile query value to (profile_name, AGENTX_HOME)."""
    from hermes_cli import profiles as profiles_mod
...
def _fire_cron_job_for_profile(profile: str, job_id: str) -> bool:
    ...
    Scope both cron storage and the runtime AgentX home so the job's store,
...
# servers managed here show up under `agentx mcp list` and vice versa.  Secrets
...
# spawn the real `agentx gateway <verb>` so behaviour matches the CLI exactly.
...
async def add_credential_pool_entry(body: CredentialPoolAdd):
    ...
    try:
        ...
        # (via DELETE below or `agentx auth remove`) can seed again.
        # Mirrors the `agentx auth add` behaviour in auth_commands.py.
...
async def remove_credential_pool_entry(provider: str, index: int):
    ...
    registry the CLI ``agentx auth remove`` uses: each source cleans up its
...
def _new_dashboard_backup_path() -> Path:
    ...
    return _dashboard_backup_dir() / f"agentx-backup-{stamp}-{secrets.token_hex(4)}.zip"
...
def _profile_cli_args(profile: Optional[str]) -> List[str]:
    ...
    Hub install/uninstall/update run in a fresh ``agentx`` subprocess, and
...
def _profile_setup_command(name: str) -> str:
    ...
    return "agentx setup" if name == "default" else f"{name} setup"
...
def _write_profile_model(profile_dir: Path, provider: str, model: str) -> None:
    ...
    context-local AGENTX_HOME override so the write lands in the target
...
def _write_profile_mcp_servers(profile_dir: Path, servers: List["MCPServerCreate"]) -> int:
    ...
    Scopes ``load_config``/``save_config`` to ``profile_dir`` via the
    context-local AGENTX_HOME override (same mechanism as
...
def _disable_unselected_skills(profile_dir: Path, keep: List[str]) -> int:
    ...
    install.) Scoped to the profile via the AGENTX_HOME override. Returns the
...
def _profile_scope(profile: Optional[str]):
    ...
    imported the modules before a AGENTX_HOME override, or under test
    isolation).
    """
    requested = (profile or "").strip()
...
# (which attach to cua-driver's OWN identity, com.trycua.driver — not AgentX,
...
#
# The endpoint spawns the same ``agentx --tui`` binary the CLI uses, behind
...
def _resolve_chat_argv(
    ...
    Default: whatever ``agentx --tui`` would run.  Tests monkeypatch this
    function to inject a tiny fake command (``cat``, ``sh -c 'printf …'``)
    so nothing has to build Node or the TUI bundle.

    Session resume is propagated via the ``AGENTX_TUI_RESUME`` env var —
    matching what ``hermes_cli.main._launch_tui`` does for the CLI path.
    Appending ``--resume <id>`` to argv doesn't work because ``ui-tui`` does
    not parse its argv.

    ``AGENTX_TUI_GATEWAY_URL`` is injected so the PTY child can attach to
    this process's in-memory ``tui_gateway`` instance instead of spawning
    its own Python gateway subprocess.

    `sidecar_url` (when set) is forwarded as ``AGENTX_TUI_SIDECAR_URL`` so
    the spawned ``tui_gateway.entry`` can mirror dispatcher emits to the
    dashboard's ``/api/pub`` endpoint (see :func:`pub_ws`).

    `active_session_file` (when set) is forwarded as
    ``AGENTX_TUI_ACTIVE_SESSION_FILE``. The TUI writes the current session id
    ...
    ``AGENTX_HOME`` at the profile dir in the child env. Every spawned
    ...
    — the same propagation ``agentx -p <name>`` performs. The in-process
    ``AGENTX_TUI_GATEWAY_URL`` attach is SKIPPED for scoped chats: the
    ...
    # AgentX TUI child: build via the single spawn-env factory (profile-home
    # contract applied; secrets kept — the spawned agent needs provider creds).
    # An explicit profile scope below still overrides AGENTX_HOME afterwards.
    from tools.environments.local import build_subprocess_env
    ...
    env.setdefault("AGENTX_TUI_DISABLE_MOUSE", "1")
    env.setdefault("AGENTX_TUI_INLINE", "1")
    ...
    env["AGENTX_TUI_DASHBOARD"] = "1"

    if profile_dir is not None:
        env["AGENTX_HOME"] = str(profile_dir)
    ...
    if resume:
        ...
        env["AGENTX_TUI_RESUME"] = resume

    if sidecar_url:
        env["AGENTX_TUI_SIDECAR_URL"] = sidecar_url
    ...
    # inherits the profile AGENTX_HOME set above.
    if profile_dir is None:
        if gateway_ws_url := _build_gateway_ws_url():
            env["AGENTX_TUI_GATEWAY_URL"] = gateway_ws_url
...
def _resolve_client_ws_host() -> Optional[str]:
    ...
    1. Explicit ``AGENTX_DASHBOARD_WS_HOST`` env var — wins always. Operators
    ...
    """
    explicit = os.environ.get("AGENTX_DASHBOARD_WS_HOST", "").strip()
    if explicit:
        return explicit
...
def _active_session_file_for_channel(app: "FastAPI", channel: str) -> Path:
    ...
    fd, raw_path = tempfile.mkstemp(prefix="agentx-pty-active-", suffix=".json")
    os.close(fd)
    path = Path(raw_path)
    files[channel] = path
    return path
...
# ---------------------------------------------------------------------------
# /api/console — safe AgentX Console command WebSocket.
#
# Unlike /api/pty, this endpoint never spawns a PTY, shell, or full AgentX CLI
        ...
        with _console_executor_lock:
            if _console_executor is None:
                _console_executor = concurrent.futures.ThreadPoolExecutor(
                    ...
                    thread_name_prefix="agentx-console",
                )
            ...
            if command_id == command_generation:
                ...
                await _console_send(
                    ...
                    send_lock,
                    {
                        ...
                        "id": command_id,
                        "message": (
                            "Command timed out. AgentX Console returned to the prompt."
                        ),
                        "command": line,
                    },
                )
...
async def pty_ws(ws: WebSocket) -> None:
    ...
    if not _PTY_BRIDGE_AVAILABLE:
        await ws.send_text(
            ...
            "\x1b[33mInstall AgentX inside WSL2 to use the dashboard's /chat "
    ...
    if raw_resume and env:
        registry_resume = env.get("AGENTX_TUI_RESUME") or raw_resume
...
# AGENTX_TUI_SIDECAR_URL set in /api/pty's PTY env) and writes every
...
def _render_active_theme_bootstrap_css() -> str:
    ...
    default AgentX Teal canvas before the SPA flips the configured user
    theme into place.
    ...
    try:
        ...
        for theme in _discover_user_themes():
            ...
            return (
                '<style id="agentx-theme-bootstrap">'
                ":root{"
...
def mount_spa(application: FastAPI):
    ...
    """
    # `agentx serve` is the headless backend: it must NEVER serve the browser
    ...
    _headless = os.environ.get("AGENTX_SERVE_HEADLESS") == "1"
    if _headless or not WEB_DIST.exists():
        _msg = (
            "Headless backend (agentx serve): web UI disabled — use "
            "`agentx dashboard` for the browser UI."
            if _headless
    ...
    def _serve_index(prefix: str = ""):
        ...
        ``__AGENTX_AUTH_REQUIRED__`` flag lets the SPA pick the right
        ...
        if gated:
            bootstrap_script = (
                f"<script>"
                f"window.__AGENTX_DASHBOARD_EMBEDDED_CHAT__={chat_js};"
                f'window.__AGENTX_BASE_PATH__="{prefix}";'
                f"window.__AGENTX_AUTH_REQUIRED__={gated_js};"
                f"</script>"
            )
        else:
            bootstrap_script = (
                f'<script>window.__AGENTX_SESSION_TOKEN__="{_SESSION_TOKEN}";'
                f"window.__AGENTX_DASHBOARD_EMBEDDED_CHAT__={chat_js};"
                f'window.__AGENTX_BASE_PATH__="{prefix}";'
                f"window.__AGENTX_AUTH_REQUIRED__={gated_js};"
                f"</script>"
        ...
        # (``AGENTX_HOME/dashboard-themes/<name>.yaml``), inject a minimal
        # critical-CSS block so the first paint uses the target palette.
        # Without this the SPA paints the default AgentX Teal canvas, then
...
_BUILTIN_DASHBOARD_THEMES = [
    {"name": "default",       "label": "AgentX Teal",         "description": "Classic dark teal — the canonical AgentX look"},
    {"name": "default-large", "label": "AgentX Teal (Large)", "description": "AgentX Teal with bigger fonts and roomier spacing"},
...
def _normalise_theme_definition(data: Dict[str, Any]) -> Optional[Dict[str, Any]]:
    ...
    # YAML in ~/.agentx/, same trust level as the config file itself.
...
def _discover_user_themes() -> list:
    """Scan ~/.agentx/dashboard-themes/*.yaml for user-created themes.
    ...
    live under the server's own ``AGENTX_HOME``.
    """
...
async def get_dashboard_themes():
    ...
    from `~/.agentx/dashboard-themes/*.yaml` ship with their full
...
def _discover_dashboard_plugins() -> list:
    ...
    1. User plugins:    ~/.agentx/plugins/<name>/dashboard/manifest.json
    2. Bundled plugins: <repo>/plugins/<name>/dashboard/manifest.json  (memory/, etc.)
    3. Project plugins: ./.agentx/plugins/  (only if AGENTX_ENABLE_PROJECT_PLUGINS)
    """
    plugins = []
    seen_names: set = set()
    ...
    # AGENTX_HOME override (e.g. embedded /chat under --open-profile).
    search_dirs = [
    ...
    if env_var_enabled("AGENTX_ENABLE_PROJECT_PLUGINS"):
        search_dirs.append((Path.cwd() / ".agentx" / "plugins", "project"))
            ...
            try:
                ...
                for tname in provides_tools:
                    ...
                    if cached_result is False:
                        auth_required = True
                        auth_command = f"agentx auth {name}"
                        break
            except Exception:
                pass
...
def _mount_plugin_api_routes():
    ...
    Project plugins (``./.agentx/plugins/``) ship with the CWD and are
    ...
    for plugin in _get_dashboard_plugins():
        ...
        if plugin.get("source") == "project":
            _log.warning(
                ...
                "~/.agentx/plugins/ if you trust it)",
...
def _write_dashboard_ready_file(actual_port: int) -> None:
    ...
    so Electron passes ``AGENTX_DESKTOP_READY_FILE`` and waits for this JSON.
    Normal CLI/dashboard launches still use the stdout READY line below.
    """
    target = os.environ.get("AGENTX_DESKTOP_READY_FILE")
    if not target:
        return
...
def start_server(
    ...
    build and no SPA mount (mount_spa() honours ``AGENTX_SERVE_HEADLESS``), so
    ...
    if app.state.auth_required:
        ...
        if not list_providers():
            ...
            # to register (e.g. missing AGENTX_DASHBOARD_OAUTH_CLIENT_ID).
            # Each provider plugin that ships with AgentX Workmate exposes a
            ...
            _fix_hint = (
                ...
                "  • OAuth: run `agentx dashboard register` (Nous Portal) or "
            ...
            try:
                ...
                if _has_creds and (set(_disabled) & _BASIC_AUTH_PLUGIN_KEYS):
                    _fix_hint = (
                        ...
                        "`agentx plugins enable basic`), then restart the "
                        "dashboard.\n\n"
                    ) + _fix_hint
            except Exception:
                pass
    ...
    # AGENTX_DASHBOARD_READY, open the browser, *then* serve.
    #
    ...
    # window.
    # dashboard.trusted_proxies: the bounded set of reverse-proxy peers whose
    # X-Forwarded-* headers uvicorn may honor (loopback is always trusted).
    try:
        _dash_cfg = (load_config() or {}).get("dashboard") or {}
    except Exception:
        _dash_cfg = {}
    ...
    async def _serve():
        ...
        with server.capture_signals():
            ...
            ready_token = "AGENTX_BACKEND_READY" if headless else "AGENTX_DASHBOARD_READY"
            print(f"{ready_token} port={actual_port}", flush=True)
            if headless:
                # No SPA, and the JSON-RPC/WS endpoints are auth-gated — don't
                # advertise a paste-and-connect URL, just announce the bind.
                print(f"  AgentX backend listening on {host}:{actual_port}")
            else:
                print(f"  AgentX Web UI → http://{host}:{actual_port}")
