"""
Status command for agentx CLI.

Shows the status of all AgentX Workmate components.
"""
...
def redact_key(key: str) -> str:
    ...
    "(not set)" placeholder in dim color to match ``agentx config``'s
...
def show_status(args):
    """Show status of all AgentX Workmate components."""
    deep = getattr(args, 'deep', False)

    print()
    print(color("┌─────────────────────────────────────────────────────────┐", Colors.CYAN))
    print(color("│                 ⬡ AgentX Workmate Status                  │", Colors.CYAN))
    print(color("└─────────────────────────────────────────────────────────┘", Colors.CYAN))
    ...
    try:
        ...
        # Read-only display: use the refresh-free snapshot so `agentx status`
    ...
    if nous_logged_in:
        ...
        nous_label = "not logged in (run: agentx portal)"
    print(
    ...
    print(
        ...
        f"{'logged in' if codex_logged_in else 'not logged in (run: agentx model)'}"
    )
    ...
    print(
        ...
        f"{'logged in' if minimax_logged_in else 'not logged in (run: agentx auth add minimax-oauth)'}"
    )
    ...
    print(
        ...
        f"{'logged in' if xai_oauth_logged_in else 'not logged in (run: agentx auth add xai-oauth)'}"
    )
    ...
    for pname, env_vars in apikey_providers.items():
        ...
        label = "configured" if configured else "not configured (run: agentx model)"
    ...
    if terminal_env == "ssh":
        ...
        sdk_label = "installed" if sdk_ok else "missing (install: pip install 'agentx-workmate[vercel]')"
    ...
    try:
        ...
        if snapshot.has_process_service_mismatch:
            ...
            print("  Start with:   agentx gateway")
    ...
    print(color("  Run 'agentx doctor' for detailed diagnostics", Colors.DIM))
    print(color("  Run 'agentx setup' to configure", Colors.DIM))
    print()
