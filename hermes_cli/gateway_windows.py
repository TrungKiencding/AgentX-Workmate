...
_TASK_NAME_DEFAULT = "AgentX_Gateway"
_TASK_DESCRIPTION = "AgentX Workmate Gateway - Messaging Platform Integration"
...
def _preserve_hermes_home_path(path: str | Path) -> str:
    """Render AgentX-owned paths under the configured AGENTX_HOME spelling.

    Windows installs may keep ``%LOCALAPPDATA%\\agentx`` as a symlink/junction to
...
def _current_profile_cli_args() -> list[str]:
    """Return CLI args that preserve the current AgentX profile."""
...
def _launch_elevated_install(
    ...
    *,
    ...
    old_start_now = os.environ.get("AGENTX_GATEWAY_INSTALL_START_NOW")
    old_start_on_login = os.environ.get("AGENTX_GATEWAY_INSTALL_START_ON_LOGIN")
    old_handoff = os.environ.get("AGENTX_GATEWAY_ELEVATED_HANDOFF")
    try:
        if start_now is not None:
            os.environ["AGENTX_GATEWAY_INSTALL_START_NOW"] = "1" if start_now else "0"
        if start_on_login is not None:
            os.environ["AGENTX_GATEWAY_INSTALL_START_ON_LOGIN"] = "1" if start_on_login else "0"
        os.environ["AGENTX_GATEWAY_ELEVATED_HANDOFF"] = "1"
        ...
        for key, old in (
            ("AGENTX_GATEWAY_INSTALL_START_NOW", old_start_now),
            ("AGENTX_GATEWAY_INSTALL_START_ON_LOGIN", old_start_on_login),
            ("AGENTX_GATEWAY_ELEVATED_HANDOFF", old_handoff),
        ):
            if old is None:
...
def get_task_name() -> str:
    ...
    Default profile: ``AgentX_Gateway``
    Named profile X: ``AgentX_Gateway_<X>``
    """
    _assert_windows()
...
def get_task_script_path() -> Path:
    ...
    Lives under ``%LOCALAPPDATA%\\agentx\\gateway-service\\<task_name>.cmd``
    (or ``<AGENTX_HOME>/gateway-service/<task_name>.cmd`` so per-profile
    AgentX installs stay self-contained).
    """
    _assert_windows()
    from hermes_cli.config import get_hermes_home
...
def _stable_gateway_working_dir(project_root: Path) -> str:
    ...
    Mirror the POSIX service invariant: anchor at ``AGENTX_HOME`` whenever it
    ...
    source checkout only if ``AGENTX_HOME`` cannot be used yet. Preserve the
...
def _build_gateway_cmd_script(
    ...
    The script:
      ...
      - exports AGENTX_HOME, PYTHONIOENCODING, VIRTUAL_ENV
    ...
    to that terminal like a normal foreground ``agentx gateway run``.
    ...
    lines.append(f'set "AGENTX_HOME={hermes_home}"')
    lines.append('set "PYTHONIOENCODING=utf-8"')
    lines.append('set "AGENTX_GATEWAY_DETACHED=1"')
...
def _build_gateway_vbs_script(
    ...
    lines = [
        ...
        f"env.Item({_quote_vbs_string('AGENTX_HOME')}) = {_quote_vbs_string(hermes_home)}",
        f"env.Item({_quote_vbs_string('PYTHONIOENCODING')}) = {_quote_vbs_string('utf-8')}",
        f"env.Item({_quote_vbs_string('AGENTX_GATEWAY_DETACHED')}) = {_quote_vbs_string('1')}",
...
def _install_scheduled_task(task_name: str, script_path: Path) -> tuple[bool, str]:
    ...
    Always recreate instead of ``/Change``. Older AgentX builds and failed
...
def windowless_gateway_restart_spec(
    ...
    supplies the stable cwd + env overlay (AGENTX_HOME, VIRTUAL_ENV,
    ...
    if hermes_home:
        env_overlay["AGENTX_HOME"] = hermes_home
    _prepend_pythonpath(
        env_overlay,
...
def _prompt_install_choices(
    ...
    env_start_now = _install_choice_from_env("AGENTX_GATEWAY_INSTALL_START_NOW")
    env_start_on_login = _install_choice_from_env("AGENTX_GATEWAY_INSTALL_START_ON_LOGIN")
...
def _install_startup_fallback(script_path: Path, start_now: bool, detail: str) -> None:
    ...
    # Re-running `agentx -p <profile> gateway install` must be safe.
    ...
    if running_pids:
        ...
        profile_arg = _profile_arg()
        start_cmd = f"agentx {profile_arg} gateway start" if profile_arg else "agentx gateway start"
    ...
    *,
    ...
    if not _is_running_as_admin() and not elevated_handoff:
        ...
        if prompt_yes_no("  Open the UAC prompt now?", False):
            if _launch_elevated_install(force=force, start_now=start_now, start_on_login=start_on_login):
                print("✓ Launched elevated AgentX gateway install prompt.")
                if start_now:
                    print("  Approve the Windows UAC prompt; the elevated install will start the gateway afterwards.")
                else:
                    print("  Approve the Windows UAC prompt, then run: agentx gateway status")
                return
    ...
    if ok:
        ...
        if start_now:
            ...
            print("  Start manually with: agentx gateway start")
        _print_next_steps()
        return
        ...
        if code == 0:
            ...
            if prompt_yes_no("  Open the UAC prompt now?", False):
                if _launch_elevated_uninstall():
                    print("✓ Launched elevated AgentX gateway uninstall prompt.")
                    print("  Approve the Windows UAC prompt, then run: agentx gateway status")
                    return
...
def restart() -> None:
    ...
    if not _wait_for_gateway_ready(timeout_s=15.0):
        raise RuntimeError(
            ...
            "Check logs/gateway.log and run `agentx gateway status`."
        )
