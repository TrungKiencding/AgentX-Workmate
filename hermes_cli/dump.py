"""
Dump command for agentx CLI.

Outputs a compact, plain-text summary of the user's AgentX setup
...
def _dotenv_key_names() -> set[str]:
    """Return the set of env-var names assigned a non-empty value in ~/.agentx/.env.
    ...
    exports. ``agentx debug share`` runs in a terminal, so ``os.getenv`` reflects
...
def _get_git_commit(project_root: Path) -> str:
    ...
    ``AGENTX_GIT_SHA`` build-arg (see ``hermes_cli/build_info.py``).
...
def _redact(value: str) -> str:
    ...
    ``agentx dump`` formats empty values as blank, not as ``"(not set)"``).
    """
...
def run_dump(args):
    ...
    for env_var, label in api_keys:
        ...
        # Set in this (shell) process but absent from ~/.agentx/.env: a managed
        ...
        # A credential added via `agentx auth add openrouter` lives in the
        # credential pool, not as an env var — surface it so the dump doesn't
        # misleadingly read "not set" while `agentx auth list` shows it (#42130).
    ...
    toolsets = config.get("toolsets", ["agentx-cli"])
