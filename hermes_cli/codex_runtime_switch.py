...
Toggles `model.openai_runtime` between "auto" (= chat_completions, AgentX'
...
def parse_args(arg_string: str) -> tuple[Optional[str], list[str]]:
    ...
    if raw in {"off", "default", "disable", "agentx"}:
        return "auto", []
...
def apply(
    ...
    *,
    ...
    if new_value == "codex_app_server":
        ...
        # Auto-migrate AgentX' MCP servers + Codex's installed curated
        # plugins into ~/.codex/config.toml so the spawned codex subprocess
        # sees the same tool surface AND can call back into AgentX for
        ...
        try:
            ...
            # Permissions + AgentX tool callback are always-on production
            ...
            if "hermes-tools" in mig_report.migrated:
                msg_lines.append(
                    "AgentX tool callback registered: codex can now use "
                ...
                msg_lines.append(
                    ...
                    "only on the default AgentX runtime — they need the "
                    "agent loop context.)"
                )
        ...
        msg_lines.append(
            ...
            "AgentX tools available via MCP callback)."
        )
        msg_lines.append(
        ...
        msg_lines.append("OpenAI/Codex turns will use the default AgentX runtime.")
