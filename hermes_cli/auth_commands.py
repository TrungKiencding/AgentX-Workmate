...
def auth_add_command(args) -> None:
    ...
    if provider == "nous":
        ...
        # <agentx-root>/shared/nous_auth.json (written by any previous
        # successful login), offer to import it instead of running the
        # full device-code flow. This makes `agentx --profile <name>
    ...
    if provider == "openai-codex":
        ...
        # latest login: a second ``agentx auth add openai-codex`` overwrote
    ...
    if provider == "xai-oauth":
        ...
        # The singleton round-trip collapsed every added account into the
        # latest login: a second ``agentx auth add xai-oauth`` overwrote the
    ...
    raise SystemExit(f"`agentx auth add {provider}` is not implemented for auth type {requested_type} yet.")
...
def auth_remove_command(args) -> None:
    ...
    # Unified removal dispatch.  Every credential source AgentX reads from
...
def auth_status_command(args) -> None:
    ...
    if not provider:
        raise SystemExit("Provider is required. Example: `agentx auth status spotify`.")
...
def _interactive_auth() -> None:
    """Interactive credential pool management when `agentx auth` is called bare."""
