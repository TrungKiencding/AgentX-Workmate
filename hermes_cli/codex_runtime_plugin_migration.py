"""Migrate AgentX' MCP server config and Codex's installed curated plugins
...
module:
  ...
  1. Reads AgentX' YAML and writes equivalent [mcp_servers.<name>]
...
What translates (MCP servers):
  AgentX mcp_servers.<n>.command/args/env  → codex stdio transport
  AgentX mcp_servers.<n>.url/headers       → codex streamable_http transport
  AgentX mcp_servers.<n>.timeout           → codex tool_timeout_sec
  AgentX mcp_servers.<n>.connect_timeout   → codex startup_timeout_sec

What does NOT translate (warned + skipped):
  AgentX-specific keys (sampling, etc.) — codex's MCP client has no
  equivalent. Listed in the per-server skipped[] field of the report.

What's NOT migrated (intentional):
  AGENTS.md — codex respects this file natively in its cwd. AgentX' own
...
class MigrationReport:
    ...
    def summary(self) -> str:
        ...
        if self.migrated:
            ...
            lines.append("No MCP servers found in AgentX config.")
...
# AgentX keys that codex's MCP schema doesn't support — dropped during
...
_KEYS_DROPPED_WITH_WARNING = {
    # AgentX' sampling subsection — codex MCP has no equivalent
    "sampling",
}
...
def _translate_one_server(
    ...
    """Translate one AgentX MCP server config to the codex inline-table dict
    ...
    if has_command:
        ...
        # AgentX' transport: sse hint is informational; codex auto-negotiates
    ...
    for key in hermes_cfg:
        ...
        elif key not in _KNOWN_AGENTX_KEYS:
            skipped.append(f"{key} (unknown AgentX key)")
...
def _format_toml_value(value: Any) -> str:
    ...
    if isinstance(value, str):
        ...
        # chars but env-var passthrough (AGENTX_HOME, PYTHONPATH) could
...
def render_codex_toml_section(
    ...
    if not servers and not plugins and not default_permission_profile:
        out.append("# (no MCP servers, plugins, or permissions configured by AgentX)")
...
def _insert_managed_block_at_top_level(user_text: str, managed_block: str) -> str:
    """Insert AgentX' managed Codex TOML block while keeping root keys root-scoped.
...
def _strip_unmanaged_plugin_tables(toml_text: str) -> str:
    ...
    directly (i.e. before AgentX' migrate has ever touched the file). When we
...
def _strip_existing_managed_block(toml_text: str) -> str:
    ...
    this code wrote so re-runs don't break configs from prior AgentX
    versions."""
    lines = toml_text.splitlines(keepends=True)
    out: list[str] = []
    in_managed = False
...
def _query_codex_plugins(
    ...
    try:
        with CodexAppServerClient(
            ...
            client.initialize(client_name="agentx-migration")
...
def _looks_like_test_tempdir(path: str) -> bool:
    ...
    what pytest's tempdir factory uses by default. If a AGENTX_HOME pointing
    ...
    ``~/.agentx`` symlink that happens to live under ``/private/var/folders``
...
def _build_hermes_tools_mcp_entry() -> dict:
    """Build the codex stdio-transport entry that launches AgentX' own
    ...
    env: dict[str, str] = {}
    # AGENTX_HOME passes through IF SET so the MCP subprocess sees the same
    ...
    # codex's subprocess to inherit whatever AGENTX_HOME its launcher sets
    ...
    # — that would override the launcher's AGENTX_HOME and pin the subprocess
    # to the wrong profile.
    #
    # The pytest-tempdir guard below catches the issue #26250 Bug C scenario:
    # a sibling test's monkeypatch.setenv("AGENTX_HOME", tmp_path) would
    ...
    hermes_home = os.environ.get("AGENTX_HOME") or ""
    if hermes_home and _looks_like_test_tempdir(hermes_home):
        hermes_home = ""
    if hermes_home:
        env["AGENTX_HOME"] = hermes_home
    # PYTHONPATH passes through so a worktree-launched agentx finds the
    ...
    env["AGENTX_QUIET"] = "1"
    env["AGENTX_REDACT_SECRETS"] = env.get("AGENTX_REDACT_SECRETS", "true")
...
def migrate(
    ...
    *,
    ...
    """Translate AgentX mcp_servers config + Codex curated plugins into
    ~/.codex/config.toml.

    Args:
        hermes_config: full ~/.agentx/config.yaml dict
        ...
        expose_hermes_tools: when True (default), register AgentX' own
            tool surface (web_search, browser_*, delegate_task, vision,
            memory, skills, etc.) as an MCP server in ~/.codex/config.toml
            so the codex subprocess can call back into AgentX for tools
    ...
    if not isinstance(hermes_servers, dict):
        report.errors.append(
            "mcp_servers in AgentX config is not a dict; cannot migrate."
        )
        return report
    ...
    # Inject AgentX' own tool surface as an MCP server so the spawned
    # codex subprocess can call back into AgentX for the tools codex
