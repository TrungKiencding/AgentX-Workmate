...
A system-level directory (default ``/etc/agentx``, root-owned and not
user-writable) supplies ``config.yaml`` and ``.env`` values that WIN over the
user's ``~/.agentx/config.yaml`` and ``~/.agentx/.env`` on a per-leaf-key basis.

This is DISTINCT from ``hermes_cli.config.is_managed()`` / ``AGENTX_MANAGED``,
...
_DEFAULT_MANAGED_DIR = Path("/etc/agentx")
...
def _under_pytest() -> bool:
    ...
    Used to ignore the system default ``/etc/agentx`` during tests so a real
    managed scope on a developer/CI box can't leak policy into the suite. Tests
    that exercise managed scope set ``AGENTX_MANAGED_DIR`` explicitly, which is
...
def get_managed_dir() -> Optional[Path]:
    ...
    Resolution (highest priority first):
      1. ``$AGENTX_MANAGED_DIR`` — deployment/bootstrap path override (IT-only;
         never persisted to any .env). Honored only when set to a non-empty value
         AND the directory exists.
      2. ``/etc/agentx`` — POSIX default, when it exists. Ignored under pytest so
    ...
    """
    override = os.environ.get("AGENTX_MANAGED_DIR", "").strip()
    if override:
        p = Path(override)
