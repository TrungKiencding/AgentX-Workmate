"""agentx hooks — inspect and manage shell-script hooks.

Usage::

    agentx hooks list
    agentx hooks test <event> [--for-tool X] [--payload-file F]
    agentx hooks revoke <command>
    agentx hooks doctor
...
def hooks_command(args) -> None:
    """Entry point for ``agentx hooks`` — dispatches to the requested action."""
    sub = getattr(args, "hooks_action", None)

    if not sub:
        print("Usage: agentx hooks {list|test|revoke|doctor}")
        print("Run 'agentx hooks --help' for details.")
        return
...
def _cmd_list(_args) -> None:
    ...
    if not specs and not outbound:
        ...
        print("See `agentx hooks --help` or")
                    ...
                    if entry and entry.get("approved_at"):
                        ...
                        if mtime_now and mtime_at and mtime_now > mtime_at:
                            print(
                                ...
                                f"run `agentx hooks doctor` to re-validate"
                            )
            print()
...
# stdin a script sees under `agentx hooks test` and `agentx hooks doctor`
...
def _print_parsed(result: Dict[str, Any]) -> None:
    ...
    if parsed:
        print(f"      parsed (AgentX wire shape): {json.dumps(parsed)}")
    else:
...
def _doctor_one(spec, shell_hooks) -> int:
    ...
    if entry and entry.get("script_mtime_at_approval"):
        ...
        if mtime_now and mtime_at and mtime_now > mtime_at:
            ...
            print(f"      ⚠ script modified since approval "
                  ...
                  f"then `agentx hooks revoke` + re-approve to refresh")
    ...
    # is already allowlisted.  Otherwise `agentx hooks doctor` would execute
    ...
    if not entry:
        print("      ℹ skipped JSON smoke test — not allowlisted yet. "
              ...
              "then re-run `agentx hooks doctor`.")
