...
Scope of the repair is deliberately narrow. AgentX only upgrades an npm that
lives inside its **own** managed Node tree (``$AGENTX_HOME/node``), installing
...
and their other projects; AgentX never modifies those. When the failing npm is
one of those foreign installs, AgentX instead provisions its own managed Node
...
def managed_npm_prefix(npm: str | os.PathLike[str] | None) -> Path | None:
    """Return the AgentX-managed Node root *npm* lives in, else ``None``.

    Symlinks are resolved first: an install links ``~/.local/bin/npm`` at
    ``$AGENTX_HOME/node/bin/npm``, which itself links into
...
def upgrade_managed_npm(
    ...
    *,
    ...
    ``prefix=~/.local`` into ``$AGENTX_HOME/node/etc/npmrc`` so that global
    ...
    if not quiet:
        print(
            f"→ Upgrading AgentX-managed npm to satisfy {npm_range}…",
            flush=True,
        )
    try:
...
def _print_manual_fix(npm: str, npm_range: str, actual: str | None) -> None:
    ...
    print(
        ...
        f"  Resolved npm: {npm}\n"
        "  AgentX could not provision its own Node.js runtime and never\n"
...
def _provision_managed_npm(npm_range: str | None, *, quiet: bool = False) -> str | None:
    """Provision an AgentX-managed Node tree and return a satisfying npm.

    Installs the managed tree under ``$AGENTX_HOME/node`` (reusing a healthy
    ...
    if not quiet:
        print(
            "→ Provisioning an AgentX-managed Node.js runtime "
...
def maybe_repair_npm_engine(
    ...
    *output* is the combined stdout/stderr of the npm command that just failed.
    ...
    the same *npm* after an in-place upgrade of an AgentX-managed install, or
    ...
    if prefix is not None:
        # AgentX owns this npm — upgrade it in place. Only an npm-range
