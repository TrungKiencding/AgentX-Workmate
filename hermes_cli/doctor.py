"""
Doctor command for agentx CLI.

Diagnoses issues with AgentX Workmate setup.
"""
...
AGENTX_HOME = get_hermes_home()
_DHH = display_hermes_home()  # user-facing display path (e.g. ~/.agentx or ~/.agentx/profiles/coder)

# Load environment variables from ~/.agentx/.env so API key checks work
_env_path = get_env_path()
load_hermes_dotenv(hermes_home=_env_path.parent, project_env=PROJECT_ROOT / ".env")

from hermes_cli.colors import Colors, color
from hermes_cli.models import _AGENTX_USER_AGENT
...
def _sqlite_upgrade_hint(install_method: str | None = None) -> str:
    ...
    if method == "docker":
        ...
        action = f"run `{command}`, then recreate all AgentX containers"
    elif method in {"nix", "nixos"}:
        action = recommended_update_command_for_method(method)
    else:
        action = "run `agentx update`"
    return (
...
def _has_provider_env_config(content: str) -> bool:
    """Return True when ~/.agentx/.env contains provider auth/base URL settings."""
...
def _doctor_tool_availability_detail(toolset: str) -> str:
    ...
    if toolset == "kanban" and not os.environ.get("AGENTX_KANBAN_TASK"):
...
_DEPRECATED_ENV_VARS: tuple[tuple[str, str], ...] = (
    ...
    # ignored. AGENTX_TOOL_PROGRESS_MODE is still read by the gateway as a
    # back-compat fallback but remains deprecated.
    ("AGENTX_TOOL_PROGRESS", "display.tool_progress in config.yaml — ignored/unsupported since config floor v12"),
    ("AGENTX_TOOL_PROGRESS_MODE", "display.tool_progress in config.yaml"),
...
def _check_version_consistency(issues: list[str]) -> None:
    ...
    other, leaving ``agentx --version`` reporting a stale version while
    ...
    if pyproject_version == init_version:
        ...
        _fail_and_issue(
            ...
            "Re-sync version files (e.g. run 'agentx update', or set "
...
def _check_s6_supervision(issues: list[str]) -> None:
    ...
    Reports:
      - Whether the main-agentx and dashboard static services are up
    ...
    for static in ("main-agentx", "dashboard"):
    ...
    if not profiles:
        check_info("No per-profile gateways registered yet — create one with `agentx profile create <name>`")
        return
...
def check_certificates(should_fix: bool = False, issues: "list | None" = None) -> None:
    ...
    if not should_fix:
        ...
        if issues is not None:
            issues.append(
                "Repair the CA bundle: run `agentx doctor --fix`, or "
...
def managed_scope_check() -> None:
    ...
    resolved from the AGENTX_MANAGED_DIR override (rather than the system
    ...
    if os.environ.get("AGENTX_MANAGED_DIR", "").strip():
        check_info(f"managed dir set via AGENTX_MANAGED_DIR={managed_dir}")
...
def run_doctor(args):
    ...
    # checks (like cronjob management) should see the same context as `agentx`.
    os.environ.setdefault("AGENTX_INTERACTIVE", "1")

    # Handle `agentx doctor --ack <id>` as a fast path. Persist the ack and
    ...
    if ack_target:
        ...
        if ack_advisory(ack_target):
            ...
            print(color(
                ...
                f"Check ~/.agentx/config.yaml is writable.",
                Colors.RED,
            ))
            sys.exit(1)
        return
    ...
    print(color("│                 🩺 AgentX Doctor                        │", Colors.CYAN))
    print(color("└─────────────────────────────────────────────────────────┘", Colors.CYAN))
        ...
        if fresh_hits:
            for hit in fresh_hits:
                ...
                manual_issues.append(
                    ...
                    f"`agentx doctor --ack {hit.advisory.id}`."
                )
    ...
    try:
        ...
        if is_sqlite_wal_reset_vulnerable():
            # Warn-only: AgentX already refuses to enable WAL on fresh DBs.
    ...
    managed_scope_check()
    # Check ~/.agentx/.env (primary location for user config)
    env_path = AGENTX_HOME / '.env'
    if env_path.exists():
        ...
        if _has_provider_env_config(content):
            ...
            issues.append("Run 'agentx setup' to configure API keys")
    else:
        ...
        if fallback_env.exists():
            ...
            if should_fix:
                ...
                check_info("Run 'agentx setup' to configure API keys")
                fixed_count += 1
            else:
                check_info("Run 'agentx setup' to create one")
                issues.append("Run 'agentx setup' to create .env")
    
    # Check ~/.agentx/config.yaml (primary) or project cli-config.yaml (fallback)
    config_path = AGENTX_HOME / 'config.yaml'
                ...
                if catalog_provider is None or (
                    ...
                    _fail_and_issue(
                        ...
                        (
                            ...
                            f"Fix: run 'agentx config set model.provider <valid_provider>'"
                        ),
                        issues,
                    )
                ...
                try:
                    ...
                    if not configured:
                        _fail_and_issue(
                            ...
                            "(check ~/.agentx/.env or run 'agentx setup')",
                            (
                                f"No credentials found for provider '{runtime_provider}'. "
                                f"Run 'agentx setup' or set the provider's API key in {_DHH}/.env, "
                                f"or switch providers with 'agentx config set model.provider <name>'"
                            ),
                            issues,
                        )
                except Exception:
    ...
    if config_path.exists():
            ...
            if current_ver < latest_ver:
                ...
                if should_fix:
                    try:
                        ...
                        issues.append("Run 'agentx setup' to migrate config")
                else:
                    issues.append("Run 'agentx doctor --fix' or 'agentx setup' to migrate config")
            else:
        ...
        try:
            ...
            if stale_root_keys:
                ...
                if should_fix:
                    ...
                    issues.append("Stale root-level provider/base_url in config.yaml — run 'agentx doctor --fix'")
        except Exception:
            pass

        # Detect stale AGENTX_MAX_ITERATIONS ghost in .env shadowing
        ...
        # bridge normally derives AGENTX_MAX_ITERATIONS from agent.max_turns
        ...
        try:
            ...
            env_ghost = load_env().get("AGENTX_MAX_ITERATIONS")
            drift = (
            ...
            if drift:
                check_warn(
                    f"AGENTX_MAX_ITERATIONS={env_ghost} in .env shadows "
                    f"agent.max_turns={cfg_max_turns} in config.yaml",
                    "(stale ghost from an earlier `agentx setup` run)",
                )
                if should_fix:
                    if remove_env_value("AGENTX_MAX_ITERATIONS"):
                        check_ok(
                            "Removed stale AGENTX_MAX_ITERATIONS from .env "
                            f"(config.yaml agent.max_turns={cfg_max_turns} is now authoritative)"
                        )
                        fixed_count += 1
                    else:
                        check_warn("Could not remove AGENTX_MAX_ITERATIONS from .env")
                        manual_issues.append(
                            "Manually delete the AGENTX_MAX_ITERATIONS line from "
                            f"{_DHH}/.env — config.yaml agent.max_turns is authoritative."
                        )
                else:
                    issues.append(
                        "Stale AGENTX_MAX_ITERATIONS in .env shadows config.yaml — "
                        "run 'agentx doctor --fix'"
                    )
        except Exception:
            pass
    ...
    try:
        ...
        if codex_status.get("logged_in"):
            ...
            # Native OAuth uses AgentX' own device-code flow — the Codex CLI is
    ...
    if soul_path.exists():
        ...
        check_warn(f"{_DHH}/SOUL.md not found", "(create it to give AgentX a custom personality)")
        if should_fix:
            soul_path.parent.mkdir(parents=True, exist_ok=True)
            soul_path.write_text(
                "# AgentX Workmate Persona\n\n"
                "<!-- Edit this file to customize how AgentX communicates. -->\n\n"
        ...
        try:
            ...
            if _write_reason is not None:
                ...
                if should_fix:
                    ...
                    issues.append(
                        "state.db FTS write corruption — run 'agentx doctor --fix' "
                        "(or 'agentx sessions repair') to rebuild the FTS index"
                    )
        except Exception as e:
            ...
            if is_malformed_db_error(e):
                ...
                if should_fix:
                    ...
                    issues.append(
                        "state.db schema malformed — run 'agentx doctor --fix' "
                        "(or 'agentx sessions repair') to recover hidden sessions"
                    )
            else:
        ...
        try:
            ...
            if wal_size > 50 * 1024 * 1024:  # 50 MB
                ...
                if should_fix:
                    ...
                    issues.append("Large WAL file — run 'agentx doctor --fix' to checkpoint")
    ...
    if sys.platform != "win32":
        ...
        for _venv_name in ("venv", ".venv"):
            _candidate = PROJECT_ROOT / _venv_name / "bin" / "agentx"
        ...
        _cmd_link = _cmd_link_dir / "agentx"

        if _venv_bin is None:
            check_warn(
                "Venv entry point not found",
                "(agentx not in venv/bin/ or .venv/bin/ — reinstall with pip install -e '.[all]')"
            )
            manual_issues.append(
            ...
            if _cmd_link.is_symlink():
                ...
                if _target == _expected:
                    check_ok(f"{_cmd_link_display}/agentx → correct target")
                else:
                    check_warn(
                        f"{_cmd_link_display}/agentx points to wrong target",
                        f"(→ {_target}, expected → {_expected})"
                    )
                    if should_fix:
                        _cmd_link.unlink()
                        _cmd_link.symlink_to(_venv_bin)
                        check_ok(f"Fixed symlink: {_cmd_link_display}/agentx → {_venv_bin}")
                        fixed_count += 1
                    else:
                        issues.append(f"Broken symlink at {_cmd_link_display}/agentx — run 'agentx doctor --fix'")
            elif _cmd_link.exists():
                # It's a regular file, not a symlink — possibly a wrapper script
                check_ok(f"{_cmd_link_display}/agentx exists (non-symlink)")
            else:
                check_fail(
                    f"{_cmd_link_display}/agentx not found",
                    "(agentx command may not work outside the venv)"
                )
                if should_fix:
                    _cmd_link_dir.mkdir(parents=True, exist_ok=True)
                    _cmd_link.symlink_to(_venv_bin)
                    check_ok(f"Created symlink: {_cmd_link_display}/agentx → {_venv_bin}")
                    fixed_count += 1
                    ...
                    issues.append(f"Missing {_cmd_link_display}/agentx symlink — run 'agentx doctor --fix'")
    ...
    if terminal_env == "vercel_sandbox":
        ...
        if importlib.util.find_spec("vercel") is not None:
            ...
            _fail_and_issue(
                ...
                "Install the Vercel optional dependency: pip install 'agentx-workmate[vercel]'",
                issues,
            )
    ...
    if _safe_which("node"):
        ...
        # `agentx acp --setup-browser` installs agent-browser into the
        # AgentX-managed node prefix, which isn't necessarily on PATH. Mirror
        ...
        _managed_ab = (
            _which_in(AGENTX_HOME / "node" / "bin")
            or _which_in(AGENTX_HOME / "node")
        )
        _legacy_ab = _which_in(AGENTX_HOME / "node_modules" / ".bin")
        if agent_browser_path.exists():
            ...
            # `agentx update` wiped node_modules (issue #48521).
            check_warn(
        ...
        if agent_browser_ok and not _is_termux():
            try:
                ...
                # to eagerly load in every `agentx doctor` invocation.
    ...
    if _npm_bin:
        ...
        # The WhatsApp bridge may live under a writable AGENTX_HOME mirror
            ...
            try:
                ...
                if total == 0:
                    ...
                    if audit_extra and audit_extra[0] == "--workspace":
                        ...
                        # tree — in that case it is an npm bug, not an AgentX one.
                        check_info(
            ...
            if r.status_code == 402:
                return _ConnectivityResult(
                    ...
                    ["OpenRouter account has insufficient credits. "
                     "Fix: run 'agentx config set model.provider <provider>' "
    ...
    def _probe_apikey_provider(pname, env_vars, default_url, base_env,
        ...
        try:
            ...
            # GOOGLE_API_KEY/GEMINI_API_KEY always shows red in ``agentx doctor``.
    ...
    # never the legitimate source for `agentx doctor`.
    ...
    try:
        ...
        if api_disabled:
            issues.append("Run 'agentx setup' to configure missing API keys for full tool access")
    except Exception as e:
        check_warn("Could not check tool availability", f"({e})")
    
    _section("Skills Hub")
    hub_dir = AGENTX_HOME / "skills" / ".hub"
    if hub_dir.exists():
        ...
        check_warn("Skills Hub directory not initialized", "(run: agentx skills list)")
    ...
    try:
        ...
        _mem_cfg_path = AGENTX_HOME / "config.yaml"
    ...
    if not _active_memory_provider:
        ...
        try:
            ...
            if not _honcho_cfg_path.exists():
                ...
                if hcfg.api_key or hcfg.base_url:
                    ...
                    check_warn("Honcho config not found", "run: agentx memory setup")
            elif not hcfg.enabled:
                check_info(f"Honcho disabled (set enabled: true in {_honcho_cfg_path} to activate)")
            elif not (hcfg.api_key or hcfg.base_url):
                _fail_and_issue(
                    "Honcho API key or base URL not set",
                    "run: agentx memory setup",
                    "No Honcho API key — run 'agentx memory setup'",
                    issues,
                )
            else:
        ...
        try:
            ...
            if mem0_key:
                ...
                _fail_and_issue(
                    ...
                    "(set MEM0_API_KEY in .env or run agentx memory setup)",
        ...
        try:
            ...
            if _provider and _provider.is_available():
                ...
                check_warn(f"{_active_memory_provider} configured but not available", "run: agentx memory status")
            else:
                check_warn(f"{_active_memory_provider} plugin not found", "run: agentx memory setup")
        except Exception as _e:
                ...
                for wrapper in wrapper_dir.iterdir():
                    ...
                    try:
                        ...
                        if "agentx -p" in content:
                            _m = _re.search(r"agentx -p (\S+)", content)
    ...
    if should_fix and fixed_count > 0:
        ...
        print()
        if not should_fix:
            print(color("  Tip: run 'agentx doctor --fix' to auto-fix what's possible.", Colors.DIM))
    else:
        print(color("─" * 60, Colors.GREEN))
