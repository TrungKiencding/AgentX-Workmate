"""Implementation of the ``agentx bundles`` CLI subcommand.
...
def _console() -> Console:
    # Bind to stderr so piping `agentx bundles list | grep …` doesn't
...
def _cmd_list(args) -> None:
    ...
    if not bundles:
        c.print(
            ...
            f"  agentx bundles create <name> --skill skill1 --skill skill2[/]\n"
...
def register_cli(subparser) -> None:
    """Build the ``agentx bundles`` argparse tree.
    ...
    p_create.add_argument(
        ...
        help="Human-readable description shown in /help and `agentx bundles list`",
    )
    p_create.add_argument(
...
def bundles_command(args) -> None:
    """Dispatch ``agentx bundles <subcommand>`` to the right handler."""
