"""
Interactive setup wizard for AgentX Workmate.
...
# display_hermes_home imported lazily at call sites (stale-module safety during agentx update)
...
def print_noninteractive_setup_guidance(reason: str | None = None) -> None:
    ...
    print()
    print(color("⬡ AgentX Setup — Non-interactive mode", Colors.CYAN, Colors.BOLD))
    print()
    if reason:
        print_info(reason)
    print_info("The interactive wizard cannot be used here.")
    print()
    print_info("Configure AgentX using environment variables or config commands:")
    print_info("  agentx config set model.provider custom")
    print_info("  agentx config set model.base_url http://localhost:8080/v1")
    print_info("  agentx config set model.default your-model-name")
    print()
    print_info("Or set OPENROUTER_API_KEY / OPENAI_API_KEY in your environment.")
    print_info("Run 'agentx setup' in an interactive terminal to use the full wizard.")
    print()
...
def is_noninteractive() -> bool:
    ...
    ``AGENTX_NONINTERACTIVE=1`` (see ``hermes_cli/web_server.py``). In that
    ...
    """
    return os.environ.get("AGENTX_NONINTERACTIVE", "").strip().lower() in {
        "1",
        "true",
        "yes",
        "on",
...
def prompt_yes_no(question: str, default: bool = True) -> bool:
    ...
    Non-interactive callers (``AGENTX_NONINTERACTIVE=1`` or a closed/redirected
...
def _prompt_api_key(var: dict):
    ...
    if value:
        ...
        print_warning("  Skipped (configure later with 'agentx setup')")
...
def _print_setup_summary(config: dict, hermes_home):
    ...
    if not _provider_ready:
        print()
        print_warning("No inference provider is configured — AgentX cannot chat yet.")
        print_info("  Finish this one step with either of:")
        print_info("    agentx model            (pick any provider/model)")
        print_info("    agentx setup --portal   (Nous Portal OAuth, no API key)")
    ...
    if _vision_backends:
        ...
        tool_status.append(("Vision (image analysis)", False, "run 'agentx setup' to configure"))
    ...
    # Video generation — opt-in via `agentx tools` → Video Generation.
    ...
    if subscription_features.tts.managed_by_nous:
        ...
        if neutts_ok:
            ...
            tool_status.append(("Text-to-Speech (NeuTTS — not installed)", False, "run 'agentx setup tts'"))
        ...
        if kittentts_ok:
            ...
            tool_status.append(("Text-to-Speech (KittenTTS — not installed)", False, "run 'agentx setup tts'"))
    else:
    ...
    if _stt_feature is not None and _stt_feature.managed_by_nous:
        ...
        if fw_ok:
            ...
            tool_status.append(
                ("Speech-to-Text (Local Whisper — not installed)", False, "run 'agentx tools' → Speech-to-Text")
            )
    ...
    if subscription_features.modal.managed_by_nous:
        ...
        if subscription_features.modal.direct_override:
            ...
            tool_status.append(("Modal Execution", False, "run 'agentx setup terminal'"))
    ...
    # Spotify (OAuth via agentx auth spotify — check auth.json, not env vars)
    try:
    ...
    if disabled_tools:
        print_warning(
            "Some tools are disabled. Run 'agentx setup tools' to configure them,"
        )
    ...
    print()
    print(f"   {color('agentx setup', Colors.GREEN)}          Re-run the full wizard")
    print(f"   {color('agentx setup model', Colors.GREEN)}    Change model/provider")
    print(f"   {color('agentx setup terminal', Colors.GREEN)} Change terminal backend")
    print(f"   {color('agentx setup gateway', Colors.GREEN)}  Configure messaging")
    print(f"   {color('agentx setup tools', Colors.GREEN)}    Configure tool providers")
    print()
    print(f"   {color('agentx config', Colors.GREEN)}         View current settings")
    print(
        f"   {color('agentx config edit', Colors.GREEN)}    Open config in your editor"
    )
    print(f"   {color('agentx config set <key> <value>', Colors.GREEN)}")
    ...
    print()
    print(f"   {color('agentx', Colors.GREEN)}              Start chatting")
    print(f"   {color('agentx gateway', Colors.GREEN)}      Start messaging gateway")
    print(f"   {color('agentx doctor', Colors.GREEN)}       Check for issues")
    print()
...
# between `agentx tools` and `agentx setup tools`).
...
def setup_model_provider(config: dict, *, quick: bool = False):
    ...
    Delegates to ``cmd_model()`` (the same flow used by ``agentx model``)
    ...
    provider added to ``agentx model`` is automatically available here.
    ...
    # Delegate to the shared agentx model flow — handles provider picker,
    ...
    try:
        ...
        print_info("You can try again later with: agentx model")
    ...
    # on demand via `agentx auth add`, `agentx setup` vision, and
    # `agentx setup tts`. This keeps both quick and full setup thin.
...
def _xai_oauth_logged_in_for_setup() -> bool:
    ...
    through ``agentx model`` -> xAI Grok OAuth (SuperGrok / Premium+).
    """
    try:
...
def _setup_tts_provider(config: dict):
    ...
    if selected == "nous-openai":
        ...
        if get_env_value("VOICE_TOOLS_OPENAI_KEY") or get_env_value("OPENAI_API_KEY"):
            print_warning(
                "Direct OpenAI credentials are still configured and may take precedence until removed from ~/.agentx/.env."
            )
    ...
    if selected == "neutts":
        ...
        # via the AgentX auth store) > existing XAI_API_KEY > prompt the user.
            ...
            if choice_idx == 0:
                ...
                if api_key:
                    ...
                    print_warning(
                        ...
                        f"via agentx setup model or {_dhh()}/.env to use xAI TTS. "
                        "Falling back to Edge TTS."
                    )
                    selected = "edge"
            else:
...
def setup_tts(config: dict):
    """Standalone TTS setup (for 'agentx setup tts')."""
...
def setup_terminal_backend(config: dict):
    ...
    print_info("Choose where AgentX runs shell commands and code.")
    ...
    if selected_backend == "local":
        ...
        # configurable later via `agentx setup terminal` / config.yaml.
        ...
        # Image and resource limits use defaults; tune via `agentx setup terminal`.
        ...
        if prompt_yes_no("  Enable egress firewall for Docker sandboxes?", False):
            ...
            print_info(
                "Run `agentx egress setup` then `agentx egress start` to mint "
                "tokens and launch the proxy."
            )
        else:
            print_info(
                "Skipping egress firewall. You can enable it later with `agentx egress setup`."
            )
        ...
        print_info("Requires the optional SDK: pip install 'agentx-workmate[vercel]'")
        ...
        try:
            ...
            # Managed uv first: $AGENTX_HOME/bin is never on PATH, so a bare
            # which() misses the uv AgentX installed. Bootstrapping one is
            ...
            if result.returncode == 0:
                ...
                print_warning("Install failed — run manually: pip install 'agentx-workmate[vercel]'")
                if result.stderr:
                    print_info(f"  Error: {result.stderr.strip().splitlines()[-1]}")
...
def _apply_default_agent_settings(config: dict):
    ...
    # bridges it into AGENTX_MAX_ITERATIONS at startup. We no longer write
    ...
    remove_env_value("AGENTX_MAX_ITERATIONS")
    ...
    print_info("  Run `agentx setup agent` later to customize.")
...
def setup_agent_settings(config: dict):
    ...
    try:
        ...
        if max_iter > 0:
            ...
            # AGENTX_MAX_ITERATIONS from agent.max_turns at startup.
    ...
    print_info("  log     — Silent in chat; write every tool call to ~/.agentx/logs/tool_calls.log (gateway only)")
...
def _profile_name_from_hermes_home(hermes_home) -> str | None:
    """Return the active profile name when AGENTX_HOME is a profile dir."""
...
def _setup_telegram():
    ...
    print()
    print_info("📬 Home Channel: where AgentX delivers cron job results,")
...
def _setup_bluebubbles():
    ...
    print_info("Connects AgentX to iMessage via BlueBubbles — a free, open-source")
...
def setup_gateway(config: dict):
    ...
    print_info("Connect to messaging platforms to chat with AgentX from anywhere.")
    ...
    if not selected:
        print_info("No platforms selected. Run 'agentx setup gateway' later to configure.")
        return
    ...
    if any_messaging:
        ...
        if missing_home:
            ...
            for plat in missing_home:
                print_info(
                    f"     agentx config set {plat.upper()}_HOME_CHANNEL <channel_id>"
                )
        ...
        if service_running:
            ...
            if prompt_yes_no(
                ...
                if supports_systemd and os.geteuid() == 0:  # windows-footgun: ok — guarded by supports_systemd (Linux only)
                    print_info("  Or as a boot-time service: agentx gateway install --system")
                print_info("  Or run in foreground:  agentx gateway")
        else:
            from hermes_constants import is_container
            if is_container():
                print_info("Start the gateway to bring your bots online:")
                print_info("   agentx gateway run          # Run as container main process")
                print_info("")
                ...
                print_info("Start the gateway to bring your bots online:")
                print_info("   agentx gateway              # Run in foreground")
...
def setup_tools(config: dict, first_install: bool = False):
    ...
    Both `agentx setup tools` and `agentx tools` use the same flow:
...
def setup_telemetry(config: dict):
    ...
    print_info("Packages stay under this AgentX profile and are not uploaded.")
...
# Config values may have different semantics between OpenClaw and AgentX.
# Instruction/context files (.md) can contain incompatible setup procedures.
_HIGH_IMPACT_KIND_KEYWORDS = {
    "gateway": "⚠ Gateway/messaging — this will configure AgentX to use your OpenClaw messaging channels",
    "telegram": "⚠ Telegram — this will point AgentX at your OpenClaw Telegram bot",
    "slack": "⚠ Slack — this will point AgentX at your OpenClaw Slack workspace",
    "discord": "⚠ Discord — this will point AgentX at your OpenClaw Discord bot",
    "whatsapp": "⚠ WhatsApp — this will point AgentX at your OpenClaw WhatsApp connection",
    "config": "⚠ Config values — OpenClaw settings may not map 1:1 to AgentX equivalents",
...
def _print_migration_preview(report: dict):
    ...
    if conflict_items:
        print(color("  Would overwrite (conflicts with existing AgentX config):", Colors.YELLOW))
    ...
    if warnings_shown:
        ...
        print()
        print(color("  Note: OpenClaw config values may have different semantics in AgentX.", Colors.YELLOW))
        print(color("  For example, OpenClaw's tool_call_execution: \"auto\" ≠ AgentX's yolo mode.", Colors.YELLOW))
...
def _offer_openclaw_migration(hermes_home: Path) -> bool:
    ...
    print_info("AgentX can preview what would be imported before making any changes.")
    print()

    if not prompt_yes_no("Would you like to see what can be imported?", default=True):
        print_info(
            "Skipping migration. You can run it later with: agentx claw migrate --dry-run"
        )
        return False
    ...
    if not prompt_yes_no("Proceed with migration?", default=False):
        print_info(
            "Migration cancelled. You can run it later with: agentx claw migrate"
        )
        print_info(
    ...
    # Execute the migration — overwrite=False so existing AgentX configs are
    ...
    try:
        migrator = mod.Migrator(
            ...
            execute=True,
            workspace_target=None,
            overwrite=False,  # preserve existing AgentX config
            migrate_secrets=True,
            output_dir=None,
            selected_options=selected,
            preset_name="full",
    ...
    if conflicts:
        print_info(f"Skipped {conflicts} item(s) that already exist in AgentX (use agentx claw migrate --overwrite to force).")
    if skipped:
...
def _run_portal_one_shot(config: dict) -> None:
    ...
    Wired into ``agentx setup --portal`` and ``agentx portal``. This is the
    ...
    AgentX session — model selected, provider set, and web/image/tts/browser
    tools routed via their Portal sub — without being told to run
    ``agentx setup`` and hunt for the quick-setup option.
    ...
    (``_run_first_time_quick_setup``) and the same one ``agentx model`` runs
    when you pick Nous. Routing through it (instead of hand-rolling the auth +
    provider write here) means ``agentx portal`` always offers a model picker,
    ...
    print(color("│     ⬡ AgentX Setup — Nous Portal (one-shot)             │", Colors.MAGENTA))
    print(
        color(
            "└─────────────────────────────────────────────────────────┘",
            Colors.MAGENTA,
    ...
    # setup calls, so `agentx portal` == quick setup's Nous step.
    try:
        ...
        print_info("  You can retry later with `agentx portal`.")
        return
    except Exception as exc:
        logger.debug("_model_flow_nous error during `agentx portal`: %s", exc)
        print()
    ...
    print_info("  Run `agentx portal info` to inspect routing.")
    print_info("  Run `agentx` to start chatting.")
...
def run_setup_wizard(args):
    ...
    Supports full, quick, and section-specific setup:
      agentx setup           — full or quick (auto-detected)
      agentx setup model     — just model/provider
      agentx setup tts       — just text-to-speech
      agentx setup terminal  — just terminal backend
      agentx setup gateway   — just messaging platforms
      agentx setup tools     — just tool configuration
      agentx setup telemetry — just local shared metrics
      agentx setup agent     — just agent settings
    """
    ...
    if section:
        for key, label, func in SETUP_SECTIONS:
            if key == section:
                ...
                print(color(f"│     ⬡ AgentX Setup — {label:<34s} │", Colors.MAGENTA))
                print(
                    color(
                        "└─────────────────────────────────────────────────────────┘",
                        Colors.MAGENTA,
    ...
    print(
        color(
            "│             ⬡ AgentX Workmate Setup Wizard                │", Colors.MAGENTA
        )
    )
    print(
        color(
    ...
    print(
        color(
            "│  Let's configure your AgentX Workmate installation.       │", Colors.MAGENTA
        )
    )
    print(
        color(
    ...
    if is_existing:
        ...
        print_success("You already have AgentX configured.")
        print_info("Running the full wizard — each prompt shows your current value.")
        print_info("Press Enter to keep it, or type a new value to change it.")
        print_info("")
        print_info("Tip: jump straight to a section with 'agentx setup model|terminal|")
        ...
        setup_mode = prompt_choice(
            "How would you like to set up AgentX?",
            [
    ...
    print()
    print_info("You can edit these files directly or use 'agentx config edit'")
    ...
    # Tune later with `agentx setup agent`.
    if not is_existing:
...
def _run_first_time_quick_setup(config: dict, hermes_home, is_existing: bool):
    ...
    settings, tools); the user can customize later via ``agentx setup <section>``
    or switch providers with ``agentx model``.
    """
    from hermes_cli.config import load_config
    ...
    gateway_choice = prompt_choice(
        ...
        [
            ...
            "Skip — set up later with 'agentx setup gateway'",
        ],
        0,
    )
    ...
    print()
    print_info("  Configure all settings:    agentx setup")
    if gateway_choice != 0:
        print_info("  Connect Telegram/Discord:  agentx setup gateway")
    print()
...
def _blank_slate_minimal_toolsets(config: dict):
    ...
    2. ``agent.disabled_toolsets`` — a global hard-suppression list (applied last
       ...
       quirks. The user re-enables any of them later via ``agentx tools`` (which
...
def _blank_slate_minimize_config(config: dict):
    ...
    Everything here is opt-in afterwards via ``agentx setup agent`` /
    ``agentx config set``. We keep only what's needed to run.
    """
...
def _run_blank_slate_setup(config: dict, hermes_home, is_existing: bool):
    ...
    if path == 0:
        ...
        # `agentx update` runs don't re-inject them.
        try:
        ...
        print_info("  Enable tools:        agentx tools")
        print_info("  Seed skills:         agentx skills opt-in --sync")
        print_info("  Add MCP servers:     agentx mcp add")
        print_info("  Enable plugins:      agentx plugins")
        print_info("  Tune agent settings: agentx setup agent")
        print()
        _print_setup_summary(config, hermes_home)
        return
...
def _blank_slate_walkthrough(config: dict, hermes_home):
    ...
    try:
        ...
        if seed_skills:
            ...
            print_info("`agentx update` runs from re-injecting them. Opt back in any")
            print_info("time with `agentx skills opt-in --sync`.")
    except Exception as exc:
    ...
    if prompt_yes_no("Open the tool selector to enable more tools?", default=False):
        ...
        print_info("Keeping the minimal toolset. Add tools later with `agentx tools`.")
    ...
    if prompt_yes_no("Review and enable built-in plugins now?", default=False):
        print_info("Manage plugins with `agentx plugins list` / `agentx plugins install`.")
    else:
        print_info("No plugins enabled. Add later with `agentx plugins`.")

    # ── MCP servers (off unless chosen) ──
    print()
    print_header("MCP Servers")
    if prompt_yes_no("Add an MCP server now?", default=False):
        print_info("Add servers with `agentx mcp add <name> --url ... | --command ...`.")
    else:
        print_info("No MCP servers configured. Add later with `agentx mcp add`.")
    ...
    print()
    print_success("Blank Slate setup complete — minimal agent ready.")
    print_info("  Enable more tools:   agentx tools")
    print_info("  Seed skills:         agentx skills opt-in --sync")
    print_info("  Add MCP servers:     agentx mcp add")
    print_info("  Tune agent settings: agentx setup agent")
    print()
...
def _run_quick_setup(config: dict, hermes_home):
    ...
    if not has_anything_missing:
        ...
        print()
        print_info("Run 'agentx setup' and choose 'Full Setup' to reconfigure,")
    ...
    if missing_messaging:
        print()
        print_header("Messaging Platforms")
        print_info("Connect AgentX to messaging apps to chat from anywhere.")
        print_info("You can configure these later with 'agentx setup gateway'.")
