...
def _messaging_conversation_rows(db, session_id: str) -> Optional[List[str]]:
    ...
    leaves the others visible as a ghost conversation. Synced copies on
    another device carry no session_key, so the messaging source is what
    ...
    if row.get("session_key") or source in MESSAGING_SESSION_SOURCE_VALUES:
        return db.get_compression_lineage(session_id)
    return None
...
async def search_sessions(
    ...
    try:
        ...
        try:
            ...
            # logs, or another AgentX surface. FTS can't find those unless the
...
async def bulk_delete_sessions_endpoint(body: BulkDeleteSessions):
    ...
    * A selected messaging-platform conversation takes its whole
      compression lineage with it, like ``DELETE /api/sessions/{id}``:
...
async def import_sessions_endpoint(request: Request):
    ...
    restores a whole AgentX backup archive, while this endpoint is scoped to
...
async def get_session_stats(profile: Optional[str] = None):
    """Session-store statistics for the Sessions page (mirrors `agentx sessions stats`).
...
async def delete_session_endpoint(session_id: str, profile: Optional[str] = None):
    ...
    def _delete():
        ...
        try:
            ...
            if conversation:
                db.delete_sessions(conversation, sessions_dir=sessions_dir)
            else:
