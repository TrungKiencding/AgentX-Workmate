...
async def get_toolset_config(name: str, profile: Optional[str] = None):
    ...
    Surfaces the same provider rows the CLI ``agentx tools`` picker shows
...
async def get_toolset_models(
    ...
    The GUI counterpart of the model picker `agentx tools` runs after a
...
async def select_toolset_provider(
    ...
    core extracted from the CLI configurator — so the GUI and ``agentx tools``
...
async def save_toolset_env(name: str, body: ToolsetEnvUpdate, profile: Optional[str] = None):
    ...
    Writes each ``key: value`` to ``~/.agentx/.env`` via ``save_env_value`` —
    the same store ``agentx tools`` writes when it prompts for keys. Keys are
...
async def run_toolset_post_setup(
    ...
    ``agentx tools post-setup <key>`` and the frontend tails the log via
    ...
    ``profile`` spawns the hook as ``agentx -p <profile> tools post-setup``.
    ...
    write per-profile state must see the same AGENTX_HOME the rest of the
...
async def grant_computer_use_permissions(profile: Optional[str] = None):
    """Spawn ``agentx computer-use permissions grant`` as a background action.
