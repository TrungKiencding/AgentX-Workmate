...
# `agentx tools` picker scope its entitlement gate to the selected backend, so a
...
def _has_agent_browser() -> bool:
    ...
    # AgentX-managed Node dirs (Windows installer / POSIX $AGENTX_HOME/node)
...
def _local_stt_backend_available() -> bool:
    ...
    """
    if get_env_value("AGENTX_LOCAL_STT_COMMAND"):
        return True
    try:
...
def get_nous_subscription_features(
    ...
    *,
    ...
    # Tool Gateway via `agentx model`, so direct credentials should NOT
    ...
    try:
        ...
        local_stt_available = bool(_HAS_FASTER_WHISPER) or bool(
            get_env_value("AGENTX_LOCAL_STT_COMMAND")
        )
    except Exception:
        local_stt_available = bool(get_env_value("AGENTX_LOCAL_STT_COMMAND"))
...
# ---------------------------------------------------------------------------
# Inline Nous Portal login for the Tool Gateway picker (`agentx tools`)
# ---------------------------------------------------------------------------
...
def ensure_nous_portal_access(
    *,
    ...
    Used by ``agentx tools`` when a user selects a Nous-managed Tool Gateway
    backend (e.g. "Firecrawl (Nous Portal)").  Unlike ``agentx model``'s Nous
    login, this:
