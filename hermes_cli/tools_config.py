"""
Unified tool configuration for AgentX Workmate.

`agentx tools` and `agentx setup tools` both enter this module.
Select a platform → toggle toolsets on/off → for newly enabled tools
that need API keys, run through provider-aware configuration.

Saves per-platform tool configuration to ~/.agentx/config.yaml under
...
def _post_setup_no_window_flags(*, streams_to_console: bool = False) -> int:
    ...
    ``agentx tools post-setup <key>`` child. On Windows, every console child
...
# They're still in _AGENTX_CORE_TOOLS (available at runtime if enabled),
# but the setup checklist won't pre-select them for first-time users.
#
# Video gen is off by default — it's a niche, paid, slow feature. Users
# who want it opt in via `agentx tools` → Video Generation, which walks
...
# `agentx tools` → X (Twitter) Search setup walks users through credential
# setup. The tool's check_fn means the schema still won't appear to the
# model if the credential later goes missing or expires.
_DEFAULT_OFF_TOOLSETS = {"homeassistant", "spotify", "discord", "discord_admin", "video", "video_gen", "x_search", "a2a"}


# Config-only capabilities: they appear in `agentx tools` for provider/API-key
...
# Platform-scoped toolsets: only appear in the `agentx tools` checklist for
...
def _get_effective_configurable_toolsets():
    ...
    Without the dedupe, ``agentx tools`` → "reconfigure existing" would
...
def _checklist_toolset_keys(platform: str) -> Set[str]:
    """Return the toolset keys the ``agentx tools`` checklist actually offers
    for ``platform``.
    ...
    so ``agentx tools`` never claims to add or remove a toolset the user was
...
TOOL_CATEGORIES = {
    ...
    "x_search": {
        ...
        "setup_note": (
            "AgentX routes X searches through xAI's built-in x_search "
        ...
        "providers": [
            {
                ...
                "env_vars": [
                    ...
                    # extra keys required. Set AGENTX_CUA_DRIVER_CMD to use a
        ...
        "providers": [
            {
                ...
                "env_vars": [
                    {"key": "AGENTX_LANGFUSE_PUBLIC_KEY", "prompt": "Langfuse public key (pk-lf-...)", "url": "https://cloud.langfuse.com"},
                    {"key": "AGENTX_LANGFUSE_SECRET_KEY", "prompt": "Langfuse secret key (sk-lf-...)", "url": "https://cloud.langfuse.com"},
                ],
                "post_setup": "langfuse",
            },
            {
                ...
                "env_vars": [
                    {"key": "AGENTX_LANGFUSE_PUBLIC_KEY", "prompt": "Langfuse public key (pk-lf-...)"},
                    {"key": "AGENTX_LANGFUSE_SECRET_KEY", "prompt": "Langfuse secret key (sk-lf-...)"},
                    {"key": "AGENTX_LANGFUSE_BASE_URL", "prompt": "Langfuse server URL (e.g. http://localhost:3000)", "default": "http://localhost:3000"},
                ],
                "post_setup": "langfuse",
            },
        ],
...
# provider+model picker like `agentx model` — NOT this single-key prompt, so
...
def _cua_driver_cmd() -> str:
    ...
    return os.environ.get("AGENTX_CUA_DRIVER_CMD", "").strip() or "cua-driver"
...
def _cua_driver_env() -> dict:
    """cua-driver child env with the AgentX telemetry policy applied.
...
def _pip_install(
    ...
    *,
    ...
    # Managed uv first: $AGENTX_HOME/bin is never on PATH, so a bare which()
    # misses the uv AgentX installed and prefers a system one when both exist.
...
def install_cua_driver(
    ...
    * ``upgrade=True`` — always re-run the installer (or call ``cua-driver
      update`` if the binary supports it). Used by ``agentx update`` and
      by ``agentx computer-use install --upgrade``.
    ...
    falling through to the full upstream installer. ``agentx update`` sets
    ...
    a further ~600s wait on Windows). ``agentx computer-use install
    ...
    ``agentx update`` already prints a contextual line before its update
    ...
    if system not in ("Darwin", "Windows", "Linux"):
        if upgrade:
            # Silent on unsupported platforms — `agentx update` calls this
    ...
    if not _cua_install_target_writable():
        ...
        _print_info(
            "    Run `agentx computer-use install --upgrade` from an admin account to update it."
        )
        return bool(binary)
    ...
    # caller: `agentx update` (require_confirmed_update=True) keeps the
    ...
    # `agentx computer-use install --upgrade` falls through and re-runs the
    # installer as before.
    ...
    if binary:
        ...
        if _state is None and require_confirmed_update:
            ...
            _print_info(
                "    Force a refresh with: agentx computer-use install --upgrade"
            )
            return True
...
def _run_cua_driver_installer(
    ...
    * Windows       → ``powershell -NoProfile -ExecutionPolicy Bypass -Command
    ...
    try:
        # When not verbose (e.g. `agentx update`'s refresh), capture the
        ...
        if verbose:
            ...
            # Preserve the full installer output. During `agentx update`,
            # sys.stdout is the mirroring _UpdateOutputStream whose `_log`
            # handle is ~/.agentx/logs/update.log — write straight to it so
        ...
        if result.returncode == 0 and shutil.which(driver_cmd):
            ...
            if verbose:
                ...
                if is_windows:
                    ...
                    _print_info("      System Settings > Privacy & Security > Accessibility")
                    _print_info("      System Settings > Privacy & Security > Screen Recording")
                    _print_info("    Both must allow the terminal / AgentX process.")
            return True
...
def _ensure_browser_use_cli(*, verbose_hints: bool = False) -> None:
    ...
    check — only the AgentX-managed ``$AGENTX_HOME/bin`` copy does.
    ``install_cli()`` short-circuits on the managed copy and otherwise
    provisions it, so resolution always lands on a binary AgentX installs
...
def _run_post_setup(post_setup_key: str):
    ...
    if post_setup_key in {"agent_browser", "browserbase"}:
        ...
        # Managed Node first — $AGENTX_HOME/node is not on PATH, so a bare
        # which() reports "no npm" on installs whose only Node is the one
        # AgentX installed for exactly this toolchain.
        ...
        if _running_in_docker():
            ...
            _print_info(
                "      docker pull ghcr.io/trungkiencding/agentx-workmate:latest"
            )
            return
        ...
        try:
            ...
            if result.returncode == 0:
                ...
                _print_info("    Change via stt.local.model in ~/.agentx/config.yaml")
            else:
        ...
        _print_info("    Switch voices by setting tts.piper.voice in ~/.agentx/config.yaml")
        ...
        # Run the full `agentx auth spotify` flow — if the user has no
        # client_id yet, this drops them into the interactive wizard
        # (opens the Spotify dashboard, prompts for client_id, persists
        # to ~/.agentx/.env), then continues straight into PKCE. If they
        ...
        try:
            ...
            _print_info("    Run manually: agentx auth spotify")
            return
        ...
        try:
            ...
            # toolset enable; they can retry with `agentx auth spotify`.
            _print_warning(f"    Spotify login did not complete: {exc}")
            _print_info("    Run later: agentx auth spotify")
        except Exception as exc:
        ...
        try:
            ...
            _print_info("    Run manually: agentx plugins enable observability/langfuse")
        _print_info("    Restart AgentX for tracing to take effect.")
        _print_info("    Verify: agentx plugins list")
        ...
        try:
            ...
            _print_info("    Run later: agentx auth add xai-oauth   (or set XAI_API_KEY)")
            return
        ...
        idx = prompt_choice(
            ...
            choices=[
                ...
                "Skip — configure later via `agentx auth add xai-oauth`",
            ],
            default=0,
        )
        if idx == 0:
            if _run_xai_oauth_login_from_setup():
                ...
                _print_warning(
                    ...
                    "Run later: agentx auth add xai-oauth"
                )
        elif idx == 1:
            ...
            if api_key:
                ...
                _print_warning(
                    "    No API key provided. Run later: agentx auth add xai-oauth"
                )
        else:
...
def valid_post_setup_keys() -> Set[str]:
    ...
    ``post_setup``). This is the allowlist the ``agentx tools post-setup``
...
def run_post_setup_command(args) -> int:
    """``agentx tools post-setup <key>`` — non-interactive post-setup runner.
    ...
    if not key:
        _print_error("Usage: agentx tools post-setup <key>")
        return 2
...
def _exempt_explicit_platform_native(
    ...
    the composite they chose (e.g. ``agentx-discord``, which contains those
...
#:
#: Saving ``agentx tools`` (or one toggle in the desktop Toolsets UI) replaces
...
#: has touched the picker, while everyone still on ``[agentx-cli]`` inherits it
...
def _enable_recently_shipped_toolsets(
    ...
    Either way of saying no outlives this: unchecking in ``agentx tools``
    ...
    default_ts = plat_info["default_toolset"] if plat_info else f"agentx-{platform}"
    ...
    for ts_key in sorted(_RECENTLY_SHIPPED_TOOLSETS):
        ...
        # narrow composites (agentx-acp, agentx-webhook) stay narrow.
...
def _get_platform_tools(
    ...
    *,
    ...
    # ``agentx-discord``) is an opt-in to the platform's native default-off
    ...
    # "agentx-cli" (which include all _AGENTX_CORE_TOOLS) cause disabled
    ...
    if has_explicit_config:
        ...
        # ``[agentx-cli, spotify]`` after enabling Spotify via ``agentx
        ...
        # (e.g. "agentx-cli") to individual tool names and reverse-mapping.
        all_tool_names = set()
        for ts_name in toolset_names:
        ...
        # through ``agentx tools`` to flip the toolset on. Only fires when
    ...
    # otherwise saving via `agentx tools` (which flips has_explicit_config
    # to True) silently drops them.
    _plat_info = PLATFORMS.get(platform)
    _default_ts = _plat_info["default_toolset"] if _plat_info else f"agentx-{platform}"
    ...
    skip |= {k for k in TOOLSETS if k.startswith("agentx-")}
    ...
    # shipped as a bundled plugin but user must opt in via `agentx tools`
    # so we don't ship 7 Spotify tool schemas to users who don't use it).
    # A plugin toolset is "known" for a platform once `agentx tools`
    ...
    if plugin_ts_keys:
        ...
        for pts in plugin_ts_keys:
            if pts in toolset_names:
                ...
                # New plugin not yet seen by agentx tools — default enabled
                enabled_toolsets.add(pts)
    ...
    # is invalid (e.g. a migration or hand-edit left `agentx` instead of
    # `agentx-cli`), resolve_toolset() returns [] for each and the platform ends
    ...
    # at runtime, not only during the next `agentx update`/`agentx doctor`.
    ...
    if isinstance(_explicit, list) and _explicit:
        ...
        if (
            ...
            logger.warning(
                ...
                "name(s): %s) - tools will be unavailable. Run `agentx tools` "
...
def _save_platform_tools(config: dict, platform: str, enabled_toolset_keys: Set[str]):
    ...
    # Also exclude platform default toolsets (agentx-cli, agentx-telegram, etc.)
    ...
    # Opening `agentx tools` is the user's opt-in to reconfigure tools, so treat
...
_POST_SETUP_INSTALLED: dict = {
    ...
    # who toggles the toolset on via `agentx tools` gets a silent no-op
...
def _agent_browser_installed() -> bool:
    ...
    # The install hook runs in a spawned ``agentx tools post-setup`` process,
...
def _configure_provider(
    ...
    *,
    ...
    # "enable all tools" prompt (that lives in `agentx model`).
    if managed_feature:
        from hermes_cli.nous_subscription import (
            MANAGED_FEATURE_COVERAGE_CATEGORY,
...
def _configure_vision_backend() -> None:
    ...
    provider + model — the same surface as ``agentx model`` — or point at a
...
def _configure_vision_provider_model(config: dict, vision_cfg: dict) -> None:
    ...
    substrate — so this picker lists exactly what the ``agentx model`` aux-task
    ...
    if not providers:
        _print_warning(
            ...
            "with `agentx model`, then re-run this."
        )
        return
...
def _reconfigure_simple_requirements(ts_key: str):
    ...
    if ts_key == "vision":
        ...
        # `agentx model`). Run it directly so reconfigure doesn't fall back to
...
def tools_command(args=None, first_install: bool = False, config: dict = None):
    """Entry point for `agentx tools` and `agentx setup tools`.
    ...
    print(color("  Guide: https://github.com/TrungKiencding/AgentX-Workmate/blob/main/website/docs/user-guide/features/tools.md", Colors.DIM))
    print()
    ...
    print(color("  Changes take effect on next 'agentx' or gateway restart.", Colors.DIM))
    print()
...
def _configure_mcp_tools_interactive(config: dict):
    ...
    for server_name, tools in server_tools.items():
        ...
        # tools.include across the codebase (catalog installs, agentx mcp
