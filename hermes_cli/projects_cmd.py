"""``agentx project`` CLI — manage first-class, multi-folder Projects.
...
convention. State lives in the per-profile ``$AGENTX_HOME/projects.db`` store
...
def build_parser(
    ...
    p_create.add_argument("name", help="Human name, e.g. 'AgentX Workmate'")
...
def projects_command(args: argparse.Namespace) -> int:
    """Entry point from ``agentx project …`` argparse dispatch."""
    ...
    if not action:
        ...
        if parser is not None:
            ...
            print(
                "usage: agentx project <action> [options]\n"
                "Run 'agentx project --help' for the full list.",
                file=sys.stderr,
            )
        return 0
...
def _cmd_list(args: argparse.Namespace) -> int:
    ...
    if not projs:
        print("No projects yet. Create one with `agentx project create <name>`.")
        return 0
    for p in projs:
...
def _cmd_set_primary(args, conn, proj) -> int:
    if not pdb.set_primary(conn, proj.id, args.path):
        print(
            ...
            f"add it first with `agentx project add-folder`.",
            file=sys.stderr,
        )
        return 1
