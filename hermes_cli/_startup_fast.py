...
def active_profile_may_override_home(hermes_root: str) -> bool:
    """Cheap probe: does an active non-default profile redirect AGENTX_HOME?"""
...
def _resolved_home() -> str:
    hermes_home = os.environ.get("AGENTX_HOME", "").strip()
    if hermes_home:
        return hermes_home
    return os.path.join(os.path.expanduser("~"), ".agentx")
...
def container_mode_may_be_active() -> bool:
    ...
    default_home = os.path.join(os.path.expanduser("~"), ".agentx")
...
def read_install_method() -> str | None:
    ...
    non-default profile might redirect AGENTX_HOME.
    """
...
def try_fast_version(argv: list[str] | None = None) -> bool:
    ...
    if is_termux and os.environ.get("AGENTX_TERMUX_DISABLE_FAST_CLI") == "1":
        return False
    if is_termux:
