"""
Backup and import commands for agentx CLI.

`agentx backup` creates a zip archive of the entire ~/.agentx/ directory
(excluding the agentx-agent repo and transient files).

`agentx import` restores from a backup zip, overlaying onto the current
AGENTX_HOME root.
"""
...
# ``agentx-agent`` is special-cased to root level only in ``_should_exclude``
# so that skill directories like ``skills/autonomous-ai-agents/agentx-agent/``
...
# AGENTX_HOME gets walked file-by-file, ballooning a backup to hundreds of
...
_EXCLUDED_DIRS = {
    "agentx-agent",     # the codebase repo — re-clone instead
    ...
    # Python dependency trees (plugin / MCP-server venvs under AGENTX_HOME) —
...
# File names that ``agentx import`` must never overwrite, matched by basename so
...
# Reserved archive subtree for provider state that lives OUTSIDE AGENTX_HOME
...
class BackupInProgressError(RuntimeError):
    """Raised when another process already owns the AgentX backup slot."""
            ...
            while True:
                try:
                    ...
                    if time.monotonic() >= deadline:
                        raise BackupInProgressError("another AgentX backup is already running")
                    time.sleep(0.05)
        else:
            import fcntl
...
def _collect_memory_provider_external_paths() -> List[Path]:
    ...
    outside AGENTX_HOME, resolved from config only (no network, no init).
...
def _should_exclude(rel_path: Path) -> bool:
    """Return True if *rel_path* (relative to agentx root) should be skipped."""
    parts = rel_path.parts

    for part in parts:
        if part not in _EXCLUDED_DIRS:
            continue
        # ``agentx-agent`` only matches at the root level (first component).
        # Nested directories with the same name — e.g.
        # ``skills/autonomous-ai-agents/agentx-agent/`` — must be preserved.
        if part == "agentx-agent" and part != parts[0]:
            continue
        return True
...
def _should_skip_backup_file(abs_path: Path, rel_path: Path, out_path: Path) -> bool:
    ...
    # write can copy data from outside AGENTX_HOME.
...
# to the user as a hung `agentx update` (#70553 follow-up). Sessions databases
...
def _run_backup_locked(args, hermes_root: Path) -> None:
    ...
    if args.output:
        ...
        if out_path.is_dir():
            ...
            out_path = out_path / f"agentx-backup-{stamp}.zip"
    else:
        stamp = datetime.now().strftime("%Y-%m-%d-%H%M%S")
        out_path = Path.home() / f"agentx-backup-{stamp}.zip"
    ...
    for dirpath, dirnames, filenames in os.walk(hermes_root, followlinks=False):
        ...
        # ``agentx-agent`` is only pruned at the root level; nested dirs
        # with the same name (e.g. in skills/) must be preserved.
        is_root = rel_dir == Path(".")
        orig_dirnames = dirnames[:]
        dirnames[:] = [
            d for d in dirnames
            if d not in _EXCLUDED_DIRS or (d == "agentx-agent" and not is_root)
        ]
    ...
    # outside AGENTX_HOME, so the walk above never sees it. Ask the active
    ...
    if not errors:
        print(f"\nRestore with: agentx import {out_path.name}")
...
def _detect_prefix(zf: zipfile.ZipFile) -> str:
    ...
    Some tools zip as `.agentx/config.yaml` instead of `config.yaml`.
...
def run_import(args) -> None:
    ...
    with zipfile.ZipFile(zip_path, "r") as zf:
        ...
        if (has_config or has_env) and not args.force:
            print()
            print("Warning: Target directory already has AgentX configuration.")
        ...
        for member in members:
            ...
            # location (e.g. ~/.honcho/config.json), NOT under AGENTX_HOME.
        ...
        if profiles_dir.is_dir():
            try:
                ...
                if any(profiles_dir.iterdir()):
                    ...
                    print("  Run: agentx profile list  (after installing agentx)")

        # Guidance
        print()
        if not (hermes_root / "agentx-agent").is_dir():
            print("Note: The agentx-agent codebase was not included in the backup.")
            print("  If this is a fresh install, run: agentx update")

        if restored_profiles:
            gw_profiles = [n for n, _ in restored_profiles]
            print("\nTo re-enable gateway services for profiles:")
            for pname in gw_profiles:
                print(f"  agentx -p {pname} gateway install")

        print("Done. Your AgentX configuration has been restored.")


# ---------------------------------------------------------------------------
# Quick state snapshots (used by /snapshot slash command and agentx backup --quick)
# ---------------------------------------------------------------------------

# Critical state files to include in quick snapshots (relative to AGENTX_HOME).
...
# — `agentx update` snapshots this set before pulling so approved-user lists
...
_QUICK_STATE_FILES = (
    ...
    # are at $AGENTX_HOME/<name> for the default/root profile; on non-root
    # profiles the real path is outside AGENTX_HOME and the entry is silently
...
def _create_quick_snapshot_locked(
    ...
    if failed_dbs:
        ...
        print(
            ...
            f"{root} and run: agentx snapshot list"
        )
        logger.error(
...
# Relative path of the cron job database inside AGENTX_HOME. Kept in sync with
...
def restore_cron_jobs_if_emptied(
    ...
    """Safety net for silent cron-job loss across ``agentx update``.
    ...
    Args:
        ...
        hermes_home: Override for the AgentX home directory (tests).
...
def run_quick_backup(args) -> None:
    """CLI entry point for agentx backup --quick."""
...
def create_pre_update_backup(
    ...
    """Create a full zip backup of AGENTX_HOME under ``backups/``.

    Mirrors :func:`run_backup` (same exclusion rules, same SQLite safe-copy)
    but writes to ``<AGENTX_HOME>/backups/pre-update-<timestamp>.zip`` and
...
# ---------------------------------------------------------------------------
# Pre-migration auto-backup (used by `agentx claw migrate`)
# ---------------------------------------------------------------------------
...
def create_pre_migration_backup(
    ...
    """Create a full zip backup of AGENTX_HOME under ``backups/`` before a
    ``agentx claw migrate`` apply.

    Shares implementation with :func:`create_pre_update_backup` via
    ``_write_full_zip_backup`` — same exclusions, same SQLite safe-copy,
    restorable with ``agentx import <archive>``.  Writes to
    ``<AGENTX_HOME>/backups/pre-migration-<timestamp>.zip`` and auto-prunes
    ...
    # Reuses the shared backups/ directory so `agentx import` and the
