...
on Japanese locales, etc.).  AgentX's banners, tool output feed, and slash
...
def _flip_console_code_page_to_utf8() -> None:
    ...
    is silent — if there's no attached console (e.g. AgentX is running
...
def configure_windows_stdio() -> bool:
    ...
    # Augment PATH with the AgentX-managed Git install directories so
    ...
    # already-running shells don't see the change — which means agentx
    ...
    # install without AgentX-managed PortableGit).
    ...
    # stdin is re-configured for completeness; AgentX's interactive
...
def _default_windows_editor() -> str:
    ...
    The prompt_toolkit buffer's ``open_in_editor`` and AgentX's
    ``agentx config edit`` both honour ``$EDITOR``.  Users who prefer a
    ...
    Set this before launching AgentX (User env var in Windows Settings, or
    export in a PowerShell profile) and AgentX picks it up automatically.
    """
    import shutil
...
def _augment_path_with_known_tools() -> None:
    """Prepend well-known AgentX-managed tool directories to os.environ['PATH'].
    ...
    *spawned* processes only — already-running shells (including the one the
    user invokes ``agentx`` from right after install) retain their old PATH.

    Any subprocess AgentX spawns — bash, ``rg``, ``grep``, ``npm`` — inherits
    ...
    Patch-up strategy: add the known AgentX-managed tool directories to our
