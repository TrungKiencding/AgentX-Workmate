"""
Top-level argparse construction for the agentx CLI.
...
_EPILOGUE = """
Examples:
    agentx                        Start interactive chat
    agentx chat -q "Hello"        Single query mode
    agentx --tui                  Launch the modern TUI (or set display.interface: tui)
    agentx --cli                  Force the classic REPL (overrides display.interface: tui)
    agentx -c                     Resume the most recent session
    agentx -c "my project"        Resume a session by name (latest in lineage)
    agentx --resume <session_id>  Resume a specific session by ID
    agentx setup                  Run setup wizard
    agentx logout                 Clear stored authentication
    agentx auth add <provider>    Add a pooled credential
    agentx auth list              List pooled credentials
    agentx auth remove <p> <t>    Remove pooled credential by index, id, or label
    agentx auth reset <provider>  Clear exhaustion status for a provider
    agentx model                  Select default model
    agentx fallback [list]        Show fallback provider chain
    agentx fallback add           Add a fallback provider (same picker as `agentx model`)
    agentx fallback remove        Remove a fallback provider from the chain
    agentx config                 View configuration
    ...
    agentx debug share             Upload debug report for support
    agentx console                Open the safe AgentX command console
    agentx update                 Update to latest version
    agentx dashboard              Start web UI dashboard (port 9119)
    agentx dashboard --stop       Stop running dashboard processes
    agentx dashboard --status     List running dashboard processes
...
def build_top_level_parser():
    ...
    parser = argparse.ArgumentParser(
        prog="agentx",
        description="AgentX Workmate - AI assistant with tool-calling capabilities",
    ...
    # Mirrors `agentx chat --model ... --provider ...` semantics.
    _inherited_flag(
        parser,
        "-m",
        "--model",
        ...
        help=(
            ...
            "Applies to -z/--oneshot and --tui. Also settable via AGENTX_INFERENCE_MODEL env var."
        ),
    )
    _inherited_flag(
        parser,
        ...
        help=(
            ...
            "under model.provider — use `agentx setup` or edit the file to change it."
        ),
    )
    _inherited_flag(
        parser,
    ...
    _inherited_flag(
        ...
        help=(
            ...
            "without a TTY prompt.  Equivalent to AGENTX_ACCEPT_HOOKS=1 or "
    ...
    _inherited_flag(
        ...
        action="store_true",
        default=False,
        help="Ignore ~/.agentx/config.yaml and fall back to built-in defaults (credentials in .env are still loaded)",
    )
    _inherited_flag(
        parser,
        "--ignore-rules",
    ...
    chat_parser = subparsers.add_parser(
        ...
        description="Start an interactive chat session with AgentX Workmate",
    )
    ...
    # top-level parser: when the user writes `agentx -m foo chat`, argparse
    ...
    _inherited_flag(
        ...
        help=(
            "Auto-approve any unseen shell hooks declared in config.yaml "
            "without a TTY prompt (see also AGENTX_ACCEPT_HOOKS env var and "
    ...
    _inherited_flag(
        ...
        action="store_true",
        default=argparse.SUPPRESS,
        help="Ignore ~/.agentx/config.yaml and fall back to built-in defaults (credentials in .env are still loaded). Useful for isolated CI runs, reproduction, and third-party integrations.",
    )
    _inherited_flag(
        chat_parser,
        "--ignore-rules",
    ...
    _inherited_flag(
        ...
        action="store_true",
        default=argparse.SUPPRESS,
        help="Troubleshooting mode: disable ALL customizations — user config, AGENTS.md/memory injection, plugins, and MCP servers (implies --ignore-user-config and --ignore-rules). Use to isolate whether a problem comes from your setup or from AgentX itself.",
    )
    chat_parser.add_argument(
        "--source",
        default=None,
