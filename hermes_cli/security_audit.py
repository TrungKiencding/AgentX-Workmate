...
2. Python deps declared by user-installed plugins under ``~/.agentx/plugins``
...
def _discover_plugins(hermes_home: Path) -> list[Component]:
    """Python deps declared by plugins under ``~/.agentx/plugins``.
...
def cmd_security_audit(args: argparse.Namespace) -> int:
    """Implementation of `agentx security audit`."""
