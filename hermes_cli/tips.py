...
TIPS = [
    ...
    "/title \"my project\" names your session — resume it later with /resume or agentx -c.",
    ...
    "Ctrl+Z suspends AgentX to the background — run fg in your shell to resume.",
    ...
    # --- CLI Flags ---
    "agentx -c resumes your most recent CLI session. agentx -c \"project name\" resumes by title.",
    "agentx -w creates an isolated git worktree — perfect for parallel agent workflows.",
    "agentx -w -q \"Fix issue #42\" combines worktree isolation with a one-shot query.",
    "agentx chat -t web,terminal enables only specific toolsets for a focused session.",
    "agentx chat -s github-pr-workflow preloads a skill at launch.",
    "agentx chat -q \"query\" runs a single non-interactive query and exits.",
    "agentx chat --max-turns 1000 overrides the default 500-iteration limit per turn.",
    "agentx chat --checkpoints enables filesystem snapshots before every destructive file change.",
    "agentx --yolo bypasses all dangerous command approval prompts for the entire session.",
    "agentx chat --source telegram tags the session for filtering in agentx sessions list.",
    "agentx -p work chat runs under a specific profile without changing your default.",

    # --- CLI Subcommands ---
    "agentx doctor --fix diagnoses and auto-repairs config and dependency issues.",
    "agentx dump outputs a compact setup summary — great for bug reports.",
    "agentx config set KEY VALUE auto-routes secrets to .env and everything else to config.yaml.",
    "agentx config edit opens config.yaml in your default editor.",
    "agentx config check scans for missing or stale configuration options.",
    "agentx sessions browse opens an interactive session picker with search.",
    "agentx sessions stats shows session counts by platform and database size.",
    "agentx sessions prune --older-than 30 cleans up old sessions.",
    "agentx skills search react --source skills-sh searches the skills.sh public directory.",
    "agentx skills check scans installed hub skills for upstream updates.",
    "agentx skills tap add myorg/skills-repo adds a custom GitHub skill source.",
    "agentx skills snapshot export setup.json exports your skill configuration for backup or sharing.",
    "agentx mcp add github --command npx adds MCP servers from the command line.",
    "agentx mcp serve runs AgentX itself as an MCP server for other agents.",
    "agentx auth add lets you add multiple API keys for credential pool rotation.",
    "agentx completion bash >> ~/.bashrc enables tab completion for all commands and profiles.",
    "agentx logs -f follows agent.log in real time. --level WARNING --since 1h filters output.",
    "agentx backup creates a zip backup of your entire AgentX home directory.",
    "agentx profile create coder creates an isolated profile that becomes its own command.",
    "agentx profile create work --clone copies your current config and keys to a new profile.",
    "agentx update syncs new bundled skills to ALL profiles automatically.",
    "agentx gateway install sets up AgentX as a system service (systemd/launchd).",
    "agentx memory setup lets you configure an external memory provider (Honcho, Mem0, etc.).",
    "agentx webhook subscribe creates event-driven webhook routes with HMAC validation.",
    "Save money: agentx tools disables unused tools, agentx skills config trims skills down.",
    "/reasoning low or /reasoning minimal cuts thinking depth below the default (medium) — faster, cheaper responses.",
    "agentx models routes vision, compression, and aux tasks to cheaper models — cuts background token cost 85%+ without downgrading your main chat model.",
    ...
    "Set worktree: true in config.yaml to always create a git worktree (same as agentx -w).",
    ...
    "AGENTX_TIMEZONE overrides the server timezone with any IANA timezone string.",
    ...
    # --- Tools & Capabilities ---
    "execute_code runs Python scripts that call AgentX tools programmatically — results stay out of context.",
    ...
    "Profile names become shell commands — 'agentx profile create coder' creates the 'coder' command.",
    "agentx profile export coder -o backup.tar.gz creates a portable profile archive.",
    ...
    "When exiting, AgentX prints a resume command with session ID and stats.",
    "agentx sessions export backup.jsonl exports all sessions for backup or analysis.",
    "agentx -r SESSION_ID resumes any specific past session by its ID.",
    ...
    "agentx skills install official/security/1password installs optional skills from the repo.",
    ...
    "The plan skill saves markdown plans under .agentx/plans/ in the active workspace.",

    # --- Cron & Scheduling ---
    "Cron jobs can attach skills: agentx cron add --skill blogwatcher \"Check for new posts\".",
    ...
    # --- Gateway & Messaging ---
    "AgentX runs on 21 messaging platforms: Telegram, Discord, Slack, WhatsApp, Signal, Matrix, IRC, Microsoft Teams, email, and more.",
    "agentx gateway install sets it up as a system service that starts on boot.",
    "DingTalk uses Stream Mode — no webhooks or public URL needed.",
    "BlueBubbles brings iMessage to AgentX via a local macOS server.",
    ...
    "Context files (.agentx.md, AGENTS.md) are security-scanned for prompt injection before loading.",
    ...
    "AgentX loads project context from .agentx.md, AGENTS.md, CLAUDE.md, or .cursorrules (first match).",
    ...
    # --- MCP ---
    "agentx mcp opens an interactive picker of Nous-approved MCPs you can install in one keystroke.",
    "agentx mcp catalog lists Nous-approved MCP servers shipped with the repo.",
    "agentx mcp install <name> installs a catalog entry, prompts for credentials, and lets you pick which of its tools to enable.",
    ...
    "MCP servers auto-generate toolsets at runtime — agentx tools can toggle them per platform.",
    ...
    "Checkpoints use shadow repos in ~/.agentx/checkpoints/ — your project's .git is never touched.",

    # --- Batch & Data ---
    "batch_runner.py processes hundreds of prompts in parallel for training data generation.",
    "agentx chat -Q enables quiet mode for programmatic use — suppresses banner and spinner.",
    ...
    "agentx plugins install owner/repo installs plugins directly from GitHub.",
    ...
    "agentx acp runs AgentX as an ACP server for VS Code, Zed, and JetBrains integration.",
    "Custom providers: save named endpoints in config.yaml under custom_providers.",
    "AGENTX_EPHEMERAL_SYSTEM_PROMPT injects a system prompt that's never persisted to history.",
    "credential_pool_strategies supports fill_first, round_robin, least_used, and random rotation.",
    "agentx auth add nous or agentx auth add openai-codex sets up OAuth-based providers.",
    ...
    "agentx status --deep runs deeper diagnostic checks across all components.",
    ...
    "Cron scripts live in ~/.agentx/scripts/ and run before the agent — perfect for data collection pipelines.",
    "prefill_messages_file in config.yaml injects few-shot examples into every API call, never saved to history.",
    "SOUL.md completely replaces the agent's default identity — rewrite it to make AgentX your own.",
    ...
    "Before a gateway session reset, AgentX auto-flushes important facts to memory in the background.",
    ...
    "The skills audit log at ~/.agentx/skills/.hub/audit.log tracks every install and removal operation.",
    ...
    "Profiles scope AgentX state via AGENTX_HOME; host tool subprocesses keep your real HOME unless terminal.home_mode is profile.",
    "AGENTX_HOME_MODE env var (octal, e.g. 0701) sets custom directory permissions for web server traversal.",
    "Container mode: place .container-mode in AGENTX_HOME and the host CLI auto-execs into the container.",
    "Ctrl+C has 5 priority tiers: cancel recording → cancel prompts → cancel picker → interrupt agent → exit.",
    "Every interrupt during an agent run is logged to ~/.agentx/interrupt_debug.log with timestamps.",
    ...
    "Profile names cannot collide with existing PATH binaries — 'agentx profile create ls' would be rejected.",
    "agentx profile create backup --clone-all copies everything (config, keys, SOUL.md, memories, skills, sessions).",
    ...
    "agentx claw migrate --dry-run previews OpenClaw migration without writing anything.",
    ...
    "AGENTX_DEV=1 bypasses container mode detection for local development.",
    "Each MCP server gets its own toolset (mcp-servername) that can be toggled independently via agentx tools.",
    "MCP ${ENV_VAR} placeholders in config are resolved at server spawn — including vars from ~/.agentx/.env.",
    "Skills from trusted repos (openai, anthropics, huggingface, NVIDIA) get a 'trusted' security level; community skills get extra scanning.",
    "The skills quarantine at ~/.agentx/skills/.hub/quarantine/ holds skills pending security review.",

    # --- Advanced Slash Commands ---
    '/steer <prompt> injects a note after the next tool call — nudge direction mid-task without interrupting.',
    '/goal <text> sets a standing Ralph-loop objective — AgentX auto-continues turn after turn until a judge says done.',
    '/snapshot create [label] saves a full state snapshot of AgentX config; /snapshot restore <id> reverts later.',
    ...
    '/busy queue|steer|interrupt controls what pressing Enter does while AgentX is working.',
    ...
    '/reload reloads ~/.agentx/.env into the running session — pick up new API keys without restarting.',
    ...
    "AGENTX_CRON_MAX_PARALLEL (default 4) caps how many cron jobs run per tick so bursts don't saturate your keys.",

    # --- Gateway Hooks ---
    'Gateway hooks live under ~/.agentx/hooks/<name>/ with HOOK.yaml + handler.py — handler must be named `handle`.',
    'Hook events include gateway:startup, session:start, agent:step, and command:* wildcard subscriptions.',
    'Drop a ~/.agentx/BOOT.md checklist and a gateway:startup hook runs it as a one-shot agent every boot.',

    # --- Curator ---
    'agentx curator run --dry-run previews what the curator would archive or consolidate without mutating anything.',
    "agentx curator pin <skill> hard-fences a skill against both auto-archival and the agent's skill_manage tool.",
    'agentx curator rollback restores skills from a pre-run snapshot — backups live under skills/.curator_backups/.',

    # --- Credential Pools & Routing ---
    'agentx auth reset <provider> clears all cooldowns and exhaustion flags on a credential pool.',
    ...
    # --- TUI & Dashboard ---
    'AGENTX_TUI_RESUME=1 auto-re-attaches to the most recent TUI session on launch — handy after SSH drops.',
    "AGENTX_TUI_THEME=light|dark|<hex> forces the TUI theme on terminals that don't set COLORFGBG.",
    ...
    'agentx dashboard launches a local web UI at 127.0.0.1:9119 — zero data leaves localhost.',
    'agentx dashboard embeds the full AgentX TUI in your browser via xterm.js and a WebSocket PTY.',
    'Drop a YAML in ~/.agentx/dashboard-themes/ with two palette colors to reskin the entire dashboard.',
    'Dashboard plugins are drop-in: manifest.json + JS bundle in ~/.agentx/dashboard-plugins/ — no npm build required.',
    ...
    'AGENTX_BACKGROUND_NOTIFICATIONS=result only pings when background tasks finish (vs all/error/off).',
    'AGENTX_WRITE_SAFE_ROOT restricts write_file/patch to directory prefixes; multiple paths via os.pathsep (: or ;).',
    'AGENTX_IGNORE_RULES skips auto-injection of AGENTS.md, SOUL.md, .cursorrules, memory, and preloaded skills.',
    'AGENTX_ACCEPT_HOOKS auto-approves unseen shell hooks declared in config.yaml without a TTY prompt.',
    ...
    # --- TTS ---
    'tts.provider: piper runs 44-language local TTS on CPU — voices auto-download to ~/.agentx/cache/piper-voices/.',
    ...
    '/reload-skills re-scans ~/.agentx/skills/ so drop-in skills appear without restarting the session.',
    ...
    'agentx -z "<prompt>" is the purest one-shot: final answer on stdout, nothing else — ideal for piping in scripts.',
    'agentx chat --pass-session-id injects the session ID into the system prompt so the agent can self-reference it.',
    'agentx chat --image path/to/pic.png attaches a local image to a single -q query without a separate upload step.',
    'agentx chat --ignore-user-config skips the active user config — reproducible bug reports and CI runs.',
    "agentx chat --source tool tags programmatic chats so they don't clutter agentx sessions list.",
    'agentx dump --show-keys includes redacted API key fingerprints for deeper support debugging.',
    'agentx sessions rename <ID> "new title" renames any past session; agentx sessions delete <ID> removes one.',
    'agentx import restores a full AgentX backup zip; session JSON/JSONL exports import from the dashboard Sessions page.',
    'agentx fallback manages the fallback_model chain interactively — no hand-editing config.yaml.',
    'agentx pairing rotates the DM pairing token — the first messager after rotation claims access to the bot.',
    'agentx setup walks first-time users through provider, keys, and platform wiring in one interactive flow.',
    'agentx status --deep runs the full health sweep across every component; plain agentx status is the quick view.',

    # --- Agent Behavior Env Vars ---
    'AGENTX_AGENT_TIMEOUT=0 disables the gateway inactivity kill for a running agent — use for long research runs.',
    'AGENTX_ENABLE_PROJECT_PLUGINS=1 auto-loads repo-local plugins from ./.agentx/plugins/ — trust-gated by design.',
    "AGENTX_DISABLE_FILE_STATE_GUARD=1 turns off the 'file changed since you read it' guard on patch and write_file.",
    'AGENTX_ALLOW_PRIVATE_URLS=true lets web tools hit localhost and private networks — off by default in gateway mode.',
    'AGENTX_OPTIONAL_SKILLS=name1,name2 auto-installs extra optional-catalog skills on first run per profile.',
    'AGENTX_BUNDLED_SKILLS points at a custom bundled-skill tree — used by Homebrew and Nix packaging.',
    'AGENTX_DUMP_REQUEST_STDOUT=1 dumps every API request payload to stdout instead of log files.',
    'AGENTX_OAUTH_TRACE=1 logs redacted OAuth token exchange and refresh attempts for debugging provider auth.',
    'AGENTX_STREAM_RETRIES (default 3) controls mid-stream reconnect attempts on transient network errors.',

    # --- Gateway Behavior Env Vars ---
    'AGENTX_GATEWAY_BUSY_ACK_ENABLED=false silences the ⚡/⏳/⏩ ack messages when a user messages a busy agent.',
    'AGENTX_AGENT_NOTIFY_INTERVAL (default 180s) sets how often the gateway pings with progress on long turns.',
    'AGENTX_RESTART_DRAIN_TIMEOUT (default 900s) caps how long /restart waits for in-flight runs before forcing.',
    'AGENTX_CHECKPOINT_TIMEOUT (default 30s) caps filesystem checkpoint creation — raise it on huge monorepos.',
    ...
    # --- Security ---
    'security.tirith_fail_open: false makes AgentX block commands when the tirith scanner itself errors out.',
    ...
    '--source tool chats are excluded from agentx sessions list by default — set --source explicitly to see them.',
    ...
    'Dashboard plugins are served from /dashboard-plugins/<name>/ — drop files into ~/.agentx/dashboard-plugins/.',
]
