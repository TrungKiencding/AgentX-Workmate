...
Add, remove, or reorder entries here — both `agentx setup` and
`agentx` provider-selection will pick up the change automatically.
"""
...
_AGENTX_USER_AGENT = f"agentx-cli/{_AGENTX_VERSION}"
...
def _codex_curated_models() -> list[str]:
    ...
    This keeps the gateway /model picker in sync with the CLI `agentx model`
...
# $AGENTX_HOME/models_dev_cache.json as of 2026-04-28. Whenever xAI renames
...
def _xai_merge_curated_extras(ids: list[str]) -> list[str]:
    """Append AgentX-curated xAI models that are missing from models.dev."""
    out = list(ids)
...
def _xai_curated_models() -> list[str]:
    ...
    Reads $AGENTX_HOME/models_dev_cache.json directly (no network) so this
    runs at import time without blocking. Falls back to ``_XAI_STATIC_FALLBACK``
    when the cache is empty or unreadable. AgentX refreshes the cache from
...
def union_with_portal_free_recommendations(
    ...
    *,
    ...
    older AgentX that doesn't ship that model in its hardcoded curated
...
def union_with_portal_paid_recommendations(
    ...
    *,
    ...
    picker even if the user is running an older AgentX that doesn't ship
...
def fetch_nous_recommended_models(
    ...
    *,
    ...
    (``$AGENTX_HOME/cache/nous_recommended_cache.json``) as last-known-good.
...
# this list:  agentx model, /model, list_authenticated_providers.
#
# Fields:
#   slug        — internal provider ID (used in config.yaml, --provider flag)
#   label       — short display name
#   tui_desc    — longer description for the `agentx model` interactive picker
# ---------------------------------------------------------------------------

class ProviderEntry(NamedTuple):
    slug: str
    label: str
    tui_desc: str   # detailed description for `agentx model` TUI
...
#
# Some vendors expose several AgentX provider slugs (one per endpoint /
# auth method: global API, China API, OAuth coding plan, ...). Listing every
# slug as a top-level row in the interactive `agentx model` / setup wizard /
...
def group_providers(slugs):
    ...
    DISPLAY ONLY. Used by every interactive picker (``agentx model``, the
...
# In-repo fallback for the model AgentX silently lands on when the user never
...
# ``agentx model``) uses the richer free/paid-tier-aware resolver — see
...
def get_default_model_for_provider(provider: str) -> str:
    ...
    model was ever selected (e.g. ``agentx auth add openai-codex`` without
    ``agentx model``, or a profile that sets ``provider`` with no ``model``).

    For most providers this is the first entry in ``_PROVIDER_MODELS`` — the
    same model the ``agentx model`` picker offers first. For metered aggregators
...
def _openrouter_model_supports_tools(item: Any) -> bool:
    ...
    agentx-agent is tool-calling-first — every provider path assumes the model
    ...
    *,
    ...
    for preferred_id in preferred_ids:
        ...
        if live_item is None:
            continue
        # Hide models that don't advertise tool-calling support — agentx-agent
        ...
        if preferred_id == silent_default:
            ...
            # picker shows which model AgentX lands on when none is selected.
            desc = "default"
        else:
...
def fetch_ai_gateway_pricing(
    ...
    *,
    ...
    """Fetch Vercel AI Gateway /v1/models and return agentx-shaped pricing.

    Vercel uses ``input`` / ``output`` field names; agentx's picker expects
...
def list_available_providers() -> list[dict[str, str]]:
    ...
    source of truth shared with ``agentx model``, ``/model``, etc.).
    """
...
def normalize_provider(provider: Optional[str]) -> str:
    """Normalize provider aliases to AgentX' canonical provider ids.
...
def model_supports_fast_mode(model_id: Optional[str]) -> bool:
    """Return whether AgentX should expose the /fast toggle for this model."""
...
def _resolve_copilot_catalog_api_key() -> str:
    ...
    Resolution order:
      ...
      2. ``read_credential_pool("copilot")`` — a token (typically a
         ...
         populated by ``agentx auth add copilot`` and by ``_seed_from_env``
         when the env var is set in ``~/.agentx/.env``.
...
#
# Cache strategy:
#   - One JSON file at $AGENTX_HOME/provider_models_cache.json
...
def _credential_fingerprint(provider: str) -> str:
    ...
    relevant tokens live in ``$AGENTX_HOME/auth.json`` and external
    ...
    # env var, so it must change the fingerprint too or `agentx config set
...
def _lmstudio_request_headers(api_key: Optional[str] = None) -> dict:
    ...
    headers = {"User-Agent": _AGENTX_USER_AGENT}
...
_COPILOT_MODEL_ALIASES = {
    ...
    # Dash-notation fallbacks: AgentX' default Claude IDs elsewhere use
...
def copilot_model_api_mode(
    ...
    *,
    ...
    # endpoint, not through AgentX' native Anthropic adapter. The live catalog may
...
def probe_api_models(
    ...
    tried: list[str] = []
    headers: dict[str, str] = {"User-Agent": _AGENTX_USER_AGENT}
    if urllib.parse.urlparse(normalized).hostname == "generativelanguage.googleapis.com":
        headers["X-Goog-Api-Client"] = f"agentx-agent/{_AGENTX_VERSION}"
...
_DEEPINFRA_MODELS_QUERY = "filter=true&sort_by=agentx"
    ...
    *,
    ...
    if normalized == "moa":
        try:
            ...
            return {
                ...
                "message": f"MoA preset `{requested}` was not found. Run `agentx moa list`.",
            }
        except Exception as exc:
            return {
                "accepted": False, "persist": False, "recognized": False,
    ...
    if normalized == "custom" or normalized.startswith("custom:"):
        ...
        message = (
            ...
            f"AgentX will still save `{requested}`, but the endpoint should expose `/models` for verification."
        )
        if api_mode == "anthropic_messages":
            message += (
        ...
        if catalog_models:
            ...
            return {
                ...
                "message": (
                    ...
                    f"{suggestion_text}"
                    "\n  MiniMax does not expose a /models endpoint, so AgentX cannot verify the model name."
    ...
    if api_models is not None:
        ...
        if _variant_base is not None and _variant_base in set(api_models):
            ...
            # fetch) so we only accept models AgentX actually ships.  (#46850)
            #
