...
Registry and ABC for every inference provider AgentX knows about.
...
`$AGENTX_HOME/plugins/model-providers/<name>/` (per-user overrides). The
...
under `$AGENTX_HOME/plugins/model-providers/` for a private plugin).
