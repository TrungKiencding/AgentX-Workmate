...
class PlatformEntry:
    ...
    # Env vars this platform needs (for ``agentx setup`` display).
    ...
    # built-ins).  Used by ``agentx gateway setup`` to auto-enable the
...
class PlatformRegistry:
    ...
    def __init__(self) -> None:
        ...
        # plugin-discovery time added several seconds to *every* `agentx`
        # invocation -- including plain `agentx chat`, which never touches any
        ...
        # `agentx setup`/`gateway status`, send_message).
    ...
    def _resolve_all(self) -> None:
        ...
        gateway startup, ``agentx setup``/``gateway status``, channel
