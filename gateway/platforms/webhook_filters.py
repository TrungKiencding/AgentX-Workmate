...
def _resolve_profile_path(path_value: Any) -> Optional[Path]:
    """Resolve a user path, mapping ~/.agentx to the active profile home."""
    ...
    if raw == "~/.agentx":
        return hermes_home
    if raw.startswith("~/.agentx/"):
        return hermes_home / raw.removeprefix("~/.agentx/")
...
def _resolve_script_path(script_value: Any) -> tuple[Optional[Path], Optional[str]]:
    """Resolve a route script under AGENTX_HOME/scripts."""
    ...
    if raw_text == "~/.agentx" or raw_text.startswith("~/.agentx/"):
