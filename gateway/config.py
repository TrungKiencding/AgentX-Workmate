...
class PlatformConfig:
    ...
    # "AgentX is thinking…"). Platforms with textless indicators (Discord,
...
class GatewayConfig:
    ...
    # gateway behaves exactly as before — single AGENTX_HOME, no profile stamping.
...
def load_gateway_config() -> GatewayConfig:
    ...
    2. ~/.agentx/config.yaml (primary user-facing config)
    3. ~/.agentx/gateway.json (legacy — provides defaults under config.yaml)
    4. Built-in defaults
    """
    ...
    try:
        ...
        if config_yaml_path.exists():
            ...
            # (e.g. via `agentx config set gateway.<key> ...`, which naturally
            ...
            if isinstance(gateway_section, dict):
                if "multiplex_profiles" in gateway_section and "multiplex_profiles" not in gw_data:
                    # gateway.multiplex_profiles written by `agentx config set gateway.multiplex_profiles true`
            ...
            if not isinstance(streaming_cfg, dict) and isinstance(gateway_section, dict):
                ...
                # ``agentx config set gateway.streaming.*``
