...
The PID file lives at ``{AGENTX_HOME}/gateway.pid``.  AGENTX_HOME defaults to
``~/.agentx`` but can be overridden via the environment variable.  This means
separate AGENTX_HOME directories naturally get separate PID files — a property
...
def _get_process_hermes_home() -> Path:
    """Return the process-level AGENTX_HOME, skipping context-local overrides.
    ...
    ``get_hermes_home()`` honors ``_AGENTX_HOME_OVERRIDE`` contextvar used for
...
def _canonical_hermes_home(path: Path | str) -> Path:
    """Return a stable absolute AGENTX_HOME path for persisted identity data."""
    return Path(path).expanduser().resolve(strict=False)


def _same_hermes_home(left: Path | str, right: Path | str) -> bool:
    """Compare AGENTX_HOME paths with the host platform's case semantics."""
...
def _get_pid_path() -> Path:
    """Return the path to the gateway PID file, respecting AGENTX_HOME."""
...
def _get_lock_dir() -> Path:
    ...
    override = os.getenv("AGENTX_GATEWAY_LOCK_DIR")
    if override:
        return Path(override)
    state_home = Path(os.getenv("XDG_STATE_HOME", Path.home() / ".local" / "state"))
    return state_home / "agentx" / _LOCKS_DIRNAME
...
# Reject epoch values before 2000-01-01T00:00:00Z: nothing in AgentX' lifetime
...
def _gateway_command_subcommand(command: str | None) -> str | None:
    """Return the AgentX gateway lifecycle subcommand from a command line.
    ...
    (``"C:\\Program Files\\...\\agentx-gateway.exe"``) survive, and strips
    ``--profile``/``-p`` selectors from anywhere in argv -- AgentX's
    ...
    for token in tokens:
        ...
        if basename in ("agentx-gateway", "agentx-gateway.exe"):
            return "run"
    ...
    has_gateway_entry = (
        ...
        or any(t.rsplit("/", 1)[-1] in ("agentx", "agentx.exe") for t in tokens)
    )
    ...
    for i, token in enumerate(filtered):
        if token != "gateway":
            continue
        if i + 1 >= len(filtered):
            return "run"  # bare `agentx gateway` defaults to `run`
        return filtered[i + 1]
    return None
...
def looks_like_gateway_runtime_command_line(command: str | None) -> bool:
    ...
    use this broader matcher only when validating AgentX-owned runtime records
...
def _looks_like_gateway_process(pid: int) -> bool:
    """Return True when the live PID still looks like the AgentX gateway."""
...
def _profile_name_for_home(profile_home: Path) -> Optional[str]:
    """Return the profile id a AGENTX_HOME directory represents, or None.

    A named profile's home is ``<root>/profiles/<name>`` (immediate parent is
    ``profiles``).  The root/default home (``~/.agentx`` or ``$AGENTX_HOME``)
...
def _command_line_belongs_to_profile(command: str, profile_home: Path) -> bool:
    ...
    explicit ``AGENTX_HOME=<path>``) on its argv; the default/root gateway runs
    bare with no profile flag.
    """
    # Normalize separators before the substring match: on Windows,
    # str(Path) renders backslashes while a AGENTX_HOME= value on the argv
    ...
    # a non-matching explicit AGENTX_HOME= on the argv. AGENTX_HOME is usually
...
def _build_pid_record() -> dict:
    return {
        ...
        # AGENTX_HOME-local.  Persist the owning gateway's process home so an
...
def write_runtime_status(
    *,
    ...
    if served_profiles is not _UNSET:
        ...
        # for a single-profile gateway. Lets `agentx status` show per-profile
...
def read_runtime_status(path: Optional[Path] = None) -> Optional[dict[str, Any]]:
    ...
    can do so without mutating ``AGENTX_HOME`` in-process.  Defaults to
...
def get_runtime_status_running_pid(
    ...
    *,
    ...
    AGENTX_HOME.  Pass it when validating *another* profile's state file (the
...
def acquire_scoped_lock(scope: str, identity: str, metadata: Optional[dict[str, Any]] = None) -> tuple[bool, Optional[dict[str, Any]]]:
    ...
    at once (e.g. the same Telegram bot token across different AGENTX_HOME dirs).
    """
...
def _consume_pid_marker_for_self(
    ...
    *,
    ...
    # TARGET home.  That permits a deliberate cross-AGENTX_HOME --replace while
    ...
    # ``agentx gateway stop`` on Windows would be misclassified as an
...
def write_takeover_marker(
    ...
    *,
    ...
    gateway's AGENTX_HOME rather than the replacer's.  Same-home callers omit
...
def _validated_scoped_lock_gateway_owner(
    ...
    the lock record, the target AGENTX_HOME's gateway PID record, and the live
