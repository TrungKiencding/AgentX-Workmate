...
messages arrive at the same time the old code did:
    ...
    os.environ["AGENTX_SESSION_THREAD_ID"] = str(context.source.thread_id)
...
``os.getenv("AGENTX_SESSION_*", ...)`` calls.  Existing tool code only
needs to replace the import + call site:

    # before
    import os
    platform = os.getenv("AGENTX_SESSION_PLATFORM", "")

    # after
    from gateway.session_context import get_session_env
    platform = get_session_env("AGENTX_SESSION_PLATFORM", "")
"""
...
_SESSION_PLATFORM: ContextVar = ContextVar("AGENTX_SESSION_PLATFORM", default=_UNSET)
_SESSION_SOURCE: ContextVar = ContextVar("AGENTX_SESSION_SOURCE", default=_UNSET)
_SESSION_CHAT_ID: ContextVar = ContextVar("AGENTX_SESSION_CHAT_ID", default=_UNSET)
_SESSION_CHAT_TYPE: ContextVar = ContextVar("AGENTX_SESSION_CHAT_TYPE", default=_UNSET)
_SESSION_CHAT_NAME: ContextVar = ContextVar("AGENTX_SESSION_CHAT_NAME", default=_UNSET)
_SESSION_THREAD_ID: ContextVar = ContextVar("AGENTX_SESSION_THREAD_ID", default=_UNSET)
_SESSION_USER_ID: ContextVar = ContextVar("AGENTX_SESSION_USER_ID", default=_UNSET)
_SESSION_USER_NAME: ContextVar = ContextVar("AGENTX_SESSION_USER_NAME", default=_UNSET)
_SESSION_KEY: ContextVar = ContextVar("AGENTX_SESSION_KEY", default=_UNSET)
_SESSION_ID: ContextVar = ContextVar("AGENTX_SESSION_ID", default=_UNSET)
# In-process UI session/window id for multi-session desktop/TUI hosts. This is
# intentionally separate from AGENTX_SESSION_ID: the latter is the durable
...
_SESSION_UI_SESSION_ID: ContextVar = ContextVar("AGENTX_UI_SESSION_ID", default=_UNSET)
# ID of the message that triggered the current turn. Used as a reply anchor
# so background-process notifications stay inside the originating Telegram
# private-chat topic (those lanes route only with thread id + reply anchor).
_SESSION_MESSAGE_ID: ContextVar = ContextVar("AGENTX_SESSION_MESSAGE_ID", default=_UNSET)

_SESSION_PROFILE: ContextVar = ContextVar("AGENTX_SESSION_PROFILE", default=_UNSET)
...
_CRON_SESSION: ContextVar = ContextVar("AGENTX_CRON_SESSION", default=_UNSET)
...
_SESSION_ASYNC_DELIVERY: ContextVar = ContextVar("AGENTX_SESSION_ASYNC_DELIVERY", default=_UNSET)

# Cron auto-delivery vars — set per-job in run_job() so concurrent jobs
# don't clobber each other's delivery targets.
_CRON_AUTO_DELIVER_PLATFORM: ContextVar = ContextVar("AGENTX_CRON_AUTO_DELIVER_PLATFORM", default=_UNSET)
_CRON_AUTO_DELIVER_CHAT_ID: ContextVar = ContextVar("AGENTX_CRON_AUTO_DELIVER_CHAT_ID", default=_UNSET)
_CRON_AUTO_DELIVER_THREAD_ID: ContextVar = ContextVar("AGENTX_CRON_AUTO_DELIVER_THREAD_ID", default=_UNSET)

_VAR_MAP = {
    "AGENTX_SESSION_PLATFORM": _SESSION_PLATFORM,
    "AGENTX_SESSION_SOURCE": _SESSION_SOURCE,
    "AGENTX_SESSION_CHAT_ID": _SESSION_CHAT_ID,
    "AGENTX_SESSION_CHAT_TYPE": _SESSION_CHAT_TYPE,
    "AGENTX_SESSION_CHAT_NAME": _SESSION_CHAT_NAME,
    "AGENTX_SESSION_THREAD_ID": _SESSION_THREAD_ID,
    "AGENTX_SESSION_USER_ID": _SESSION_USER_ID,
    "AGENTX_SESSION_USER_NAME": _SESSION_USER_NAME,
    "AGENTX_SESSION_KEY": _SESSION_KEY,
    "AGENTX_SESSION_ID": _SESSION_ID,
    "AGENTX_UI_SESSION_ID": _SESSION_UI_SESSION_ID,
    "AGENTX_SESSION_MESSAGE_ID": _SESSION_MESSAGE_ID,
    "AGENTX_SESSION_PROFILE": _SESSION_PROFILE,
    "AGENTX_CRON_SESSION": _CRON_SESSION,
    "AGENTX_CRON_AUTO_DELIVER_PLATFORM": _CRON_AUTO_DELIVER_PLATFORM,
    "AGENTX_CRON_AUTO_DELIVER_CHAT_ID": _CRON_AUTO_DELIVER_CHAT_ID,
    "AGENTX_CRON_AUTO_DELIVER_THREAD_ID": _CRON_AUTO_DELIVER_THREAD_ID,
}


def set_current_session_id(session_id: str) -> None:
    """Synchronize ``AGENTX_SESSION_ID`` across ContextVar and ``os.environ``.
    ...
    ``get_session_env("AGENTX_SESSION_ID")`` with an ``os.environ`` fallback,
    ...
    ``AGENTX_SESSION_ID`` for the rest of the process — leaking the child id
    ...
    os.environ["AGENTX_SESSION_ID"] = session_id
...
def set_session_vars(
    ...
    ``os.environ["AGENTX_CRON_SESSION"]`` fallback, ``"1"`` marks a cron job,
...
def reset_session_vars() -> None:
    ...
    *A's* ``AGENTX_SESSION_*`` identity via the subprocess-env bridge.  The
    ...
    ``AGENTX_SESSION_*`` env var read via :func:`get_session_env`), so it is
...
def get_session_env(name: str, default: str = "") -> str:
    """Read a session context variable by its legacy ``AGENTX_SESSION_*`` name.

    Drop-in replacement for ``os.getenv("AGENTX_SESSION_*", default)``.
...
# value (``telegram``) to AGENTX_SESSION_PLATFORM, while the CLI, TUI, and
# desktop bind AGENTX_SESSION_SOURCE (``cli``, ``tui``, ``desktop``) and leave
...
# AGENTX_SESSION_PLATFORM but have no attachment channel behind them.
...
def session_is_messaging_surface() -> bool:
    ...
    Resolves ``AGENTX_PLATFORM``, then the session platform, then the session
    source, and reports messaging when any of them names a surface outside
    :data:`NON_MESSAGING_SESSION_SURFACES`.
    """
    import os

    platform = os.getenv("AGENTX_PLATFORM") or get_session_env("AGENTX_SESSION_PLATFORM", "")
