...
Defenses 1 and 2 (the ``_AGENTX_GATEWAY`` guard on ``agentx gateway
...
raw ``terminal("launchctl kickstart -k gui/<uid>/ai.agentx.gateway")``,
...
State lives in ``<AGENTX_HOME>/gateway/restart_loop.json`` so it is
