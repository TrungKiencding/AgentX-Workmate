...
Uses a file-based lock (~/.agentx/cron/.tick.lock) so only one tick
...
# Without this, standalone invocations (e.g. after `agentx update` reloads
...
def _resolve_cron_enabled_toolsets(job: dict, cfg: dict) -> list[str] | None:
    ...
    2. Per-platform ``agentx tools`` config for the ``cron`` platform.
...
def _interpreter_shutting_down(exc: Optional[BaseException] = None) -> bool:
    ...
    ``agentx update`` / ``agentx gateway stop`` / systemd restart, or an
...
def _get_hermes_home() -> Path:
    """Resolve AgentX home dynamically while preserving test monkeypatch hooks.

    Cron is per-profile by design (#4707): the in-process ticker runs inside a
    profile-scoped gateway, so resolving the active AGENTX_HOME at call time
...
def _open_continuable_cron_thread(
    ...
    thread_name = f"AgentX — {task_name}"
    try:
...
def _get_script_timeout() -> int:
    ...
    env_value = os.getenv("AGENTX_CRON_SCRIPT_TIMEOUT", "").strip()
    if env_value:
        try:
            ...
            logger.warning("Invalid AGENTX_CRON_SCRIPT_TIMEOUT=%r; using config/default", env_value)
...
def _run_job_script(
    ...
    Scripts must reside within AGENTX_HOME/scripts/.  Both relative and
    ...
    * anything else — run with the current Python interpreter
    ...
    provider credentials and other AgentX-managed secrets are not inherited
    (SECURITY.md §2.3), matching terminal and MCP child processes.

    Args:
        script_path: Path to the script.  Relative paths are resolved
            against AGENTX_HOME/scripts/.  Absolute and ~-prefixed paths
    ...
    # escape — scripts MUST reside within AGENTX_HOME/scripts/.
    try:
...
def run_job(
    ...
    #
    # Bounded with its own timeout (separate from AGENTX_CRON_TIMEOUT, which
    ...
    try:
        ...
        _raw_env_timeout = os.getenv("AGENTX_CRON_SESSION_DB_TIMEOUT", "").strip()
        if _raw_env_timeout:
            try:
                _session_db_timeout = float(_raw_env_timeout)
            except (ValueError, TypeError):
                logger.warning(
                    "Invalid AGENTX_CRON_SESSION_DB_TIMEOUT=%r; using config/default",
                    _raw_env_timeout,
                )
    ...
    # gateway message. Do not seed AGENTX_SESSION_* contextvars from the
    ...
    #     (notify_on_complete / watch_patterns) reads AGENTX_SESSION_PLATFORM
    #     and AGENTX_SESSION_CHAT_ID to populate watcher_platform / chat_id,
    #     which would route completion notifications to the origin chat
    #     instead of via AGENTX_CRON_AUTO_DELIVER_* below.
    #   - tools/tts_tool.py: picks Opus vs MP3 based on
    #     AGENTX_SESSION_PLATFORM == "telegram".
    ...
    #     send_message gate read AGENTX_SESSION_PLATFORM.
    # Cron output delivery itself reads job["origin"] directly via
    # _resolve_origin(job) and the AGENTX_CRON_AUTO_DELIVER_* vars set
    # below, so clearing AGENTX_SESSION_* here does not affect delivery.
    ...
    _ctx_tokens = set_session_vars(
        ...
        # AGENTX_SESSION_* routing keys just below, so an async delegation's
        ...
        # os.environ AGENTX_SESSION_KEY, which risks routing a cron subagent's output
    ...
    _cron_delivery_vars = (
        "AGENTX_CRON_AUTO_DELIVER_PLATFORM",
        "AGENTX_CRON_AUTO_DELIVER_CHAT_ID",
        "AGENTX_CRON_AUTO_DELIVER_THREAD_ID",
    )
    ...
    _cron_session_var = _VAR_MAP["AGENTX_CRON_SESSION"]
    ...
    try:
        ...
        # recorded this AGENTX_HOME in _APPLIED_HOMES, so a naive reload would
        ...
        if delivery_target:
            _VAR_MAP["AGENTX_CRON_AUTO_DELIVER_PLATFORM"].set(delivery_target["platform"])
            _VAR_MAP["AGENTX_CRON_AUTO_DELIVER_CHAT_ID"].set(str(delivery_target["chat_id"]))
            _VAR_MAP["AGENTX_CRON_AUTO_DELIVER_THREAD_ID"].set(
                ""
        ...
        # cron-fleet default) > AGENTX_MODEL env > config.yaml ``model:``
        ...
        prefill_file = (
            os.getenv("AGENTX_PREFILL_MESSAGES_FILE", "")
        ...
        try:
            # Do not inject AGENTX_INFERENCE_PROVIDER here. resolve_runtime_provider()
        ...
        agent = AIAgent(
            ...
            # AGENTX_HOME. When a workdir is configured, also inject project
        ...
        # override via AGENTX_CRON_TIMEOUT env var.  0 = unlimited.
        #
        # Uses the agent's built-in activity tracker (updated by
        # _touch_activity() on every tool call, API call, and stream delta).
        _raw_cron_timeout = os.getenv("AGENTX_CRON_TIMEOUT", "").strip()
        if _raw_cron_timeout:
            try:
                _cron_timeout = float(_raw_cron_timeout)
            except (ValueError, TypeError):
                logger.warning(
                    "Invalid AGENTX_CRON_TIMEOUT=%r; using default 600s",
                    _raw_cron_timeout,
                )
                _cron_timeout = 600.0
        else:
...
def tick(
    ...
    *,
    ...
    try:
        ...
        # scheduler STARTUP. A one-shot `agentx cron run` that claimed a job
        ...
        # Set AGENTX_CRON_MAX_PARALLEL=1 to restore old serial behaviour.
        _max_workers: Optional[int] = None
        try:
            _env_par = os.getenv("AGENTX_CRON_MAX_PARALLEL", "").strip()
            if _env_par:
                _max_workers = int(_env_par) or None
        except (ValueError, TypeError):
            logger.warning("Invalid AGENTX_CRON_MAX_PARALLEL value; defaulting to unbounded")
