"""ACP session manager — maps ACP sessions to AgentX AIAgent instances.

Sessions are persisted to the shared SessionDB (``~/.agentx/state.db``) so they
...
def _translate_acp_cwd(cwd: str) -> str:
    """Translate Windows ACP cwd values when AgentX itself is running in WSL.

    Windows ACP clients can launch ``agentx acp`` inside WSL while still sending
...
def _register_task_cwd(task_id: str, cwd: str) -> None:
    ...
    Zed can launch AgentX from a Windows workspace while the ACP process runs
...
def _expand_acp_enabled_toolsets(
    ...
    expanded: List[str] = []
    for name in list(toolsets or ["agentx-acp"]):
...
@dataclass
class SessionState:
    """Tracks per-session state for an ACP-managed AgentX agent."""
...
class SessionManager:
    """Thread-safe manager for ACP sessions backed by AgentX AIAgent instances.
    ...
    def __init__(self, agent_factory=None, db=None):
        ...
        Args:
            agent_factory: Optional callable that creates an AIAgent-like object.
                           ...
                           using the current AgentX runtime provider configuration.
            db:            Optional SessionDB instance. When omitted, the default
                           SessionDB (``~/.agentx/state.db``) is lazily created.
        """
    ...
    def _get_db(self):
        ...
        Note: we resolve ``AGENTX_HOME`` dynamically rather than relying on
    ...
    def _make_agent(
        ...
        *,
        ...
        # editor/session cwd instead of the AgentX daemon's process cwd.
