"""ACP permission bridging for AgentX dangerous-command approvals."""
...
# Maps ACP permission option ids to AgentX approval result strings.
...
def _build_permission_options(
    *, allow_permanent: bool, smart_denied: bool = False,
    ...
    """Return ACP options that match AgentX approval semantics."""
    ...
    if not smart_denied:
        options.append(PermissionOption(
            ...
            # hint while keeping AgentX semantics in the option id.
            kind="allow_always",
...
def _map_outcome_to_hermes(outcome: object, *, allowed_option_ids: set[str]) -> str:
    """Map an ACP permission outcome into AgentX approval strings."""
...
def make_approval_callback(
    ...
    """
    Return an AgentX-compatible approval callback that bridges to ACP.
