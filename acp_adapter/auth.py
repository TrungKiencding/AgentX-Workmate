"""ACP auth helpers — detect and advertise AgentX authentication methods."""
...
TERMINAL_SETUP_AUTH_METHOD_ID = "agentx-setup"


def detect_provider() -> Optional[str]:
    """Resolve the active AgentX runtime provider, or None if unavailable.
...
def has_provider() -> bool:
    """Return True if AgentX can resolve any runtime provider credentials."""
    return detect_provider() is not None


def build_auth_methods() -> list[Any]:
    """Return registry-compatible ACP auth methods for AgentX.
    ...
    not have AgentX provider credentials configured yet, so AgentX always
    ...
    if provider:
        methods.append(
            AuthMethodAgent(
                ...
                description=(
                    "Authenticate AgentX using the currently configured "
    ...
    methods.append(
        TerminalAuthMethod(
            ...
            name="Configure AgentX provider",
            description=(
                "Open AgentX' interactive model/provider setup in a terminal. "
                "Use this when AgentX has not been configured on this machine yet."
            ),
            type="terminal",
            args=["--setup"],
        )
