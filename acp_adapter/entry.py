...
try:
    ...
    # shadowing AgentX's own modules — ``agentx acp`` can be started from any
...
def _load_env() -> None:
    """Load .env from AGENTX_HOME (default ``~/.agentx``)."""
...
def _parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        prog="agentx-acp",
        description="Run AgentX Workmate as an ACP stdio server.",
    )
    parser.add_argument("--version", action="store_true", help="Print AgentX version and exit")
    parser.add_argument(
        "--check",
        action="store_true",
    ...
    parser.add_argument(
        "--setup",
        action="store_true",
        help="Run interactive AgentX provider/model setup for ACP terminal auth",
    )
    parser.add_argument(
        "--setup-browser",
        action="store_true",
        help="Install agent-browser + Playwright Chromium into ~/.agentx/node/ "
...
def _run_setup() -> None:
    ...
    old_argv = sys.argv[:]
    try:
        sys.argv = [old_argv[0] if old_argv else "agentx", "model"]
        hermes_main()
    finally:
        sys.argv = old_argv
...
def main(argv: list[str] | None = None) -> None:
    ...
    if os.environ.get("AGENTX_ACP_SKIP_CONFIGURED_MCP", "").strip() != "1":
        try:
