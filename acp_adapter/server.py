"""ACP agent server — exposes AgentX Workmate via the Agent Client Protocol."""
...
try:
    from hermes_cli import __version__ as AGENTX_VERSION
except Exception:
    AGENTX_VERSION = "0.0.0"
...
def _path_from_file_uri(uri: str) -> Path | None:
    ...
    /mnt/<drive>/... so AgentX running in WSL can read it.
    """
    raw = (uri or "").strip()
    if not raw:
        return None
    ...
    if path is None:
        return [{
            ...
            "text": _format_resource_text(
                ...
                name=name,
                title=title,
                body="[Resource link only; AgentX cannot read non-file ACP resource URIs directly.]",
            ),
        }]
...
def _content_blocks_to_openai_user_content(
    ...
    """Convert ACP prompt blocks into an AgentX/OpenAI-compatible user content payload."""
...
class HermesACPAgent(acp.Agent):
    """ACP Agent implementation wrapping AgentX AIAgent."""
    ...
    _ADVERTISED_COMMANDS = (
        ...
        {
            "name": "version",
            "description": "Show AgentX version",
        },
    )
    ...
    def _session_modes(self, state: SessionState) -> SessionModeState:
        ...
        modes, which coexist with the model picker, so AgentX maps edit approval
    ...
    def _build_model_state(self, state: SessionState) -> SessionModelState | None:
        ...
        The shared AgentX inventory is also used by ``agentx model``, the TUI,
    ...
    def _build_usage_update(state: SessionState) -> UsageUpdate | None:
        ...
        the current request pressure.  AgentX estimates ``used`` from the same
    ...
    async def _send_session_info_update(
        ...
        *,
        ...
        """Send ACP native session metadata after AgentX changes it.

        When the internal AgentX head rotated (e.g. compression-driven session
    ...
    async def _register_session_mcp_servers(
        ...
        try:
            ...
            enabled_toolsets = _expand_acp_enabled_toolsets(
                getattr(state.agent, "enabled_toolsets", None) or ["agentx-acp"],
    ...
    async def initialize(
        ...
        **kwargs: Any,
        ...
        return InitializeResponse(
            ...
            agent_info=Implementation(name="agentx-agent", version=AGENTX_VERSION),
    ...
    async def authenticate(self, method_id: str, **kwargs: Any) -> AuthenticateResponse | None:
        ...
        # AgentX' threat model (ACP is stdio-only, local-trust), but poor
        ...
        if normalized_method == TERMINAL_SETUP_AUTH_METHOD_ID:
            # Terminal auth launches AgentX setup/model selection out-of-band.
    ...
    async def _replay_session_history(self, state: SessionState) -> None:
        ...
        notifications. Merely restoring server-side state makes AgentX
    ...
    async def prompt(
        ...
        **kwargs: Any,
        ...
        """Run AgentX on the user's prompt and stream events back to the editor."""
        state = self.session_manager.get_session(session_id)
        if state is None:
        ...
        # ACP thought panes should not receive AgentX' local kawaii waiting/status
        ...
        # os.environ["AGENTX_INTERACTIVE"], so concurrent executor workers can't
        ...
        # callback shape — not the gateway-queue AGENTX_EXEC_ASK path,
        ...
        def _run_agent() -> dict:
            ...
            # Bind AGENTX_SESSION_KEY for this session so per-session caches
            ...
            try:
                ...
                # Without it the prompt advertises the global AgentX workspace
                # while the tools are rooted at the client's project, so the
                # model emits absolute paths under ~/.agentx/workspace and the
                # edit silently lands outside the editor's workspace.
                # cron_session="" explicitly marks this as a non-cron context,
                # masking any leaked process-global AGENTX_CRON_SESSION (#37968).
            ...
            previous_session_id = os.environ.get("AGENTX_SESSION_ID")
            ...
            try:
                ...
                # Restore AGENTX_SESSION_ID symmetrically.
                if previous_session_id is None:
                    os.environ.pop("AGENTX_SESSION_ID", None)
                else:
                    os.environ["AGENTX_SESSION_ID"] = previous_session_id
                if approval_cb:
                    try:
                        from tools import terminal_tool as _terminal_tool
        ...
        try:
            # Snapshot the internal AgentX DB session id before the turn so we
            ...
            # stomp on each other's ContextVar writes (AGENTX_SESSION_KEY in
        ...
        # AgentX' local "waiting for model response" interrupt status is metadata,
    ...
    def _handle_slash_command(self, text: str, state: SessionState) -> str | None:
        ...
        # an unpinned handler bakes the AgentX install tree into the session's
    ...
    def _cmd_version(self, args: str, state: SessionState) -> str:
        return f"AgentX Workmate v{AGENTX_VERSION}"
    ...
    async def set_config_option(
        ...
        """Accept ACP config option updates even when AgentX has no typed ACP config surface yet."""
        state = self.session_manager.get_session(session_id)
        if state is None:
