"""Centralized logging setup for AgentX Workmate.

Provides a single ``setup_logging()`` entry point that both the CLI and
gateway call early in their startup path.  All log files live under
``~/.agentx/logs/`` (profile-aware via ``get_hermes_home()``).
...
# essentially always in AgentX (TUI, gateway, ``hy_memory`` server, MCP
...
# Used by _ComponentFilter and exposed for ``agentx logs --component``.
COMPONENT_PREFIXES = {
...
def setup_logging(
    *,
    ...
    """Configure the AgentX logging subsystem.
    ...
    *force* is ``True``.
    ...
    ----------
    hermes_home
        Override for the AgentX home directory.  Falls back to
...
class _ManagedRotatingFileHandler(RotatingFileHandler):
    ...
    1.  In managed mode (NixOS), the stateDir uses setgid (2770) so new files
        inherit the agentx group. However, both ``_open()`` (initial creation)
...
# the module header): when several AgentX processes log to the same file, an
