...
React + Ink terminal UI for AgentX. TypeScript owns the screen. Python owns sessions, tools, model calls, and most command logic.
...
Interpreter resolution order is: `AGENTX_PYTHON` → `PYTHON` → `$VIRTUAL_ENV/bin/python` → `./.venv/bin/python` → `./venv/bin/python` → `python3` (or `python` on Windows).
...
- `src/app/setupHandoff.ts` — launches external `agentx setup`, suspends Ink while it runs, opens a new session on success
...
- Input history is stored in `~/.agentx/.hermes_history` or under `AGENTX_HOME`.
...
### Setup (`setup.ts`)
`/setup` — launches external `agentx setup` wizard, suspends Ink while it runs
...
ui-tui/
  ...
  src/
    ...
    app/
      ...
      setupHandoff.ts               launches external agentx setup, suspends Ink while it runs
    ...
    lib/
      ...
      parentLog.ts               append-only log to ~/.agentx/tui-parent.log
