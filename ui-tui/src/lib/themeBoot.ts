 ...
 * `agentx-boot-background` / `agentx-boot-color-scheme` localStorage keys.
 *
...
// Profile-aware: the Python launcher exports AGENTX_HOME (set by
// _apply_profile_override) before spawning the TUI. Falling back to
// ~/.agentx matches get_hermes_home()'s default.
const bootFilePath = () => join(process.env.AGENTX_HOME ?? join(homedir(), '.agentx'), 'tui-theme-boot.json')

// Never touch the user's real ~/.agentx from test runs (the TS suite has no
// AGENTX_HOME isolation fixture).
...
export interface BootSeedResult {
  ...
  /** True when the cached config pin was seeded into AGENTX_TUI_THEME. */
  seededPin: boolean
}
...
export function seedBootEnvironment(boot: BootTheme | null, env: NodeJS.ProcessEnv): BootSeedResult {
  ...
  if (!boot || env.AGENTX_TUI_THEME || env.AGENTX_TUI_LIGHT) {
    return result
  }
  ...
  if (boot.mode) {
    env.AGENTX_TUI_THEME = boot.mode
  ...
  if (
    ...
    !env.AGENTX_TUI_BACKGROUND
  ) {
    env.AGENTX_TUI_BACKGROUND = boot.background
...
export function invalidateBootBackground(env: NodeJS.ProcessEnv = process.env): boolean {
  if (!seeded.seededBackground || env.AGENTX_TUI_BACKGROUND !== seeded.seededBackground) {
    return false
  }

  delete env.AGENTX_TUI_BACKGROUND
...
/** True when this boot replayed a cached config pin into AGENTX_TUI_THEME.
