...
export interface HeapDumpResult {
  ...
  // the heavy snapshot because AGENTX_AUTO_HEAPDUMP was not enabled (#21767).
  suppressed?: boolean
  success: boolean
}
...
export async function performHeapDump(trigger: MemoryTrigger = 'manual'): Promise<HeapDumpResult> {
  try {
    ...
    const dir = process.env.AGENTX_HEAPDUMP_DIR?.trim() || join(homedir() || tmpdir(), '.agentx', 'heapdumps')
    ...
    const autoEnabled = /^(?:1|true|yes|on)$/i.test((process.env.AGENTX_AUTO_HEAPDUMP ?? '').trim())
...
async function pruneHeapdumps(dir: string): Promise<void> {
  const raw = process.env.AGENTX_HEAPDUMP_MAX_BYTES?.trim()
