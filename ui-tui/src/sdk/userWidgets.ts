...
import { Box, Text } from '@agentx/ink'
...
/**
 * User widget apps — AgentX authors its own TUI widgets, mirroring the
 * Python plugin contract: drop `<name>.mjs` into `$AGENTX_HOME/tui-widgets/`,
 ...
 * React.createElement.
 *
 * Trust model matches `~/.agentx/plugins/`: files under AGENTX_HOME execute
...
const widgetsDir = () => join(process.env.AGENTX_HOME?.trim() || join(homedir(), '.agentx'), 'tui-widgets')
 ...
 *  every change, so a widget AgentX writes appears within ~a second — no
...
export function watchUserWidgets(dir = widgetsDir()): void {
  ...
  if (!attach()) {
    ...
    // user (or AgentX) ever writes must hot-load too — a 10s poll here read
