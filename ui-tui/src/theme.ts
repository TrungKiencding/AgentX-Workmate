import type { SkinBranding, SkinColors } from '@agentx/shared/skin'
...
// Mirrors @agentx/ink's colorize.ts. Keep local: app code compiles from
// ui-tui/src, while @agentx/ink is bundled separately from packages/.
...
export const DARK_SEEDS: ThemeSeeds = {
  accent: '#FFBF00',
  // The classic AgentX navy surfaces are IDENTITY, not derivation drift —
...
// The classic light-mode AgentX look was never hand-authored: for years the
 ...
 *  answer when known (cached in AGENTX_TUI_BACKGROUND), else the mode's
 *  assumed pole. */
function referenceBackground(isLight: boolean, env: NodeJS.ProcessEnv = process.env): string {
  const cached = (env.AGENTX_TUI_BACKGROUND ?? '').trim()
...
// Terminal. Explicit AGENTX_TUI_THEME / COLORFGBG signals above still win,
...
// `AGENTX_TUI_BACKGROUND` is intentionally generic so a future OSC11
...
//
//   1. `AGENTX_TUI_LIGHT` boolean — `1`/`true`/`yes`/`on` → light;
//      `0`/`false`/`no`/`off` → dark.  Either explicit value wins
//      regardless of any later signal.
//   2. `AGENTX_TUI_THEME` named override — `light` / `dark` win over
//      every signal below.
//   3. `AGENTX_TUI_BACKGROUND` hex hint (3- or 6-digit) — luminance
...
//
// Anything we can't decide stays dark — the default AgentX palette
// is the dark one.
export function detectLightMode(
  ...
  const lightFlag = (env.AGENTX_TUI_LIGHT ?? '').trim().toLowerCase()
  ...
  const themeFlag = (env.AGENTX_TUI_THEME ?? '').trim().toLowerCase()
  ...
  const bgHint = backgroundLuminance(env.AGENTX_TUI_BACKGROUND ?? '')
 ...
 * the OSC-11 background answer cached into AGENTX_TUI_BACKGROUND after
...
export function fromSkin(
  ...
  // itself into AGENTX_TUI_BACKGROUND. See #applySkin / syncThemeToTerminalBackground).
  ...
  //    base seeds for this polarity. The base's IDENTITY FILLS (AgentX navy
  ...
  // the derived value already carries the identity seeds (e.g. AgentX navy).
  const activeRow =
