...
``<agentx home>/plugins/<name>/``. That tree is the plugin *install* dir —
``agentx plugins remove`` deletes it and ``agentx plugins update`` git-pulls
...
``<agentx home>/plugin-data/<name>/``, owned by the user, untouched by
...
through ``agent.secret_scope`` / ``.env`` like everywhere else in AgentX.
...
# Mirrors the plugin-name shape `agentx plugins install` accepts. Anything
...
def plugin_data_dir(name: str) -> Path:
    ...
    ``<agentx home>/plugin-data/<name>/`` — survives plugin update and
