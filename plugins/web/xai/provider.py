...
``{title, url, description, position}`` rows every other AgentX web
provider produces.
...
prefers AgentX-managed xAI Grok OAuth (via ``agentx auth``) and falls back
to ``XAI_API_KEY`` (resolved through ``~/.agentx/.env``, then
``os.environ``).
"""
...
class XAIWebSearchProvider(WebSearchProvider):
    ...
    def is_available(self) -> bool:
        ...
        every ``agentx tools`` repaint and at tool-registration time.
    ...
    def search(self, query: str, limit: int = 5) -> Dict[str, Any]:
        ...
        if not api_key:
            return {
                "success": False,
                "error": (
                    "No xAI credentials found. Run `agentx auth` to sign in with "
