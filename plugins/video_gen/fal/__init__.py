    ...
    try:
        ...
        if status is not None and 400 <= status < 500:
            raise ValueError(
                ...
                f"  • Pick a different model via `agentx tools` → Video Generation."
            ) from exc
        raise
        ...
        if not _check_fal_video_available():
            ...
            return error_response(
                error=(
                    ...
                    "(run `agentx tools` → Video Generation → FAL to configure) "
                    "or sign in to Nous (`agentx setup`) for managed gateway access."
                ),
                error_type="auth_required",
                provider="fal",
                prompt=prompt,
            ...
            if not endpoint:
                return error_response(
                    error=(
                        ...
                        f"via `agentx tools` → Video Generation."
                    ),
