...
own pages inject it via ``window.__AGENTX_SESSION_TOKEN__`` so logged-in
...
This means ``agentx dashboard --host 0.0.0.0`` is safe to run on a LAN:
...
def get_board(
    ...
    through to the active board (``AGENTX_KANBAN_BOARD`` env → on-disk
    ...
    try:
        ...
        if task and task.status == "ready" and task.assignee:
            try:
                ...
                # backend can run under a different AGENTX_HOME than the
...
class UpdateTaskBody(BaseModel):
    ...
    # transitions to 'done'. Dashboard parity with ``agentx kanban
...
def list_diagnostics(
    ...
    AND for ``agentx kanban diagnostics`` which shells to this
...
def reclaim_task_endpoint(
    ...
    ``agentx kanban reclaim <task_id> --reason ...``.
    """
    board = _resolve_board(board)
    conn = _conn(board=board)
    try:
...
def specify_task_endpoint(
    ...
    it to ``todo``. Maps 1:1 to ``agentx kanban specify <task_id>``.
    ...
    # AGENTX_KANBAN_BOARD env var — this endpoint runs in FastAPI's
...
def reassign_task_endpoint(
    ...
    Maps 1:1 to ``agentx kanban reassign <task_id> <profile> [--reclaim]``.
    """
    board = _resolve_board(board)
    conn = _conn(board=board)
    try:
...
@router.get("/config")
def get_config():
    """Return kanban dashboard preferences from ~/.agentx/config.yaml.
...
def _active_profile_name() -> str:
    """Return the current AgentX profile name for notify-sub ownership."""
    try:
...
def get_assignees(board: Optional[str] = Query(None)):
    ...
    Returns the union of ``~/.agentx/profiles/*`` on disk and every
...
def model_options():
    ...
    dropdown can never offer a model/provider pair the rest of AgentX
...
def list_kanban_projects():
    """List first-class AgentX projects for board scoping.
...
def auto_describe_profile(profile_name: str, payload: DescribeAutoBody):
    ...
    Maps 1:1 to ``agentx profile describe <name> --auto``. Non-OK
...
def decompose_task_endpoint(
    ...
    1:1 to ``agentx kanban decompose <task_id>``.
    ...
    # AGENTX_KANBAN_BOARD env var would let concurrent requests for
...
def set_orchestration_settings(payload: OrchestrationSettingsBody):
    """Update the kanban orchestration knobs in ~/.agentx/config.yaml.
