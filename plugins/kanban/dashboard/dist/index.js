/**
 * AgentX Kanban — Dashboard Plugin
 *
 * Board view for the multi-agent collaboration board backed by
 * ~/.agentx/kanban.db. Calls the plugin's backend at /api/plugins/kanban/
 * and tails task_events over a WebSocket for live updates.
 *
 * Plain IIFE, no build step. Uses window.__AGENTX_PLUGIN_SDK__ for React +
...
(function () {
  ...
  const SDK = window.__AGENTX_PLUGIN_SDK__;
  if (!SDK) return;
  ...
  const COLUMN_DOT = {
    triage: "agentx-kanban-dot-triage",
    todo: "agentx-kanban-dot-todo",
    ready: "agentx-kanban-dot-ready",
    running: "agentx-kanban-dot-running",
    blocked: "agentx-kanban-dot-blocked",
    done: "agentx-kanban-dot-done",
    archived: "agentx-kanban-dot-archived",
  };
  ...
  const MIME_TASK = "text/x-agentx-task";
  ...
  const DOCS_URL = "https://github.com/TrungKiencding/AgentX-Workmate/blob/main/website/docs/user-guide/features/kanban.md";
  const DOCS_TUTORIAL_URL = "https://github.com/TrungKiencding/AgentX-Workmate/blob/main/website/docs/user-guide/features/kanban-tutorial.md";
  ...
  const LS_BOARD_KEY = "agentx.kanban.selectedBoard";
  ...
  function renderMarkdown(src) {
    ...
    html = html.replace(/\u0000CODE(\d+)\u0000/g, (_m, i) =>
      `<pre class="agentx-kanban-md-code"><code>${escapeHtml(blocks[Number(i)])}</code></pre>`,
    );
    return html;
  }
  ...
  function sanitizeMarkdownAttrs(tag, attrs) {
    ...
    if (tag === "pre" && /\sclass=(["'])agentx-kanban-md-code\1/i.test(attrs)) {
      return ' class="agentx-kanban-md-code"';
    }
    return "";
  }
  ...
  function MarkdownBlock(props) {
    ...
    if (!enabled) {
      return h("pre", { className: "agentx-kanban-pre" }, props.source || "");
    }
    return h("div", {
      className: "agentx-kanban-md",
  ...
  // standard `drop` event and our `agentx-kanban:drop` event.
  // -------------------------------------------------------------------------
  ...
  function attachTouchDrag(el, taskId) {
    ...
    function onDown(e) {
      ...
      proxy.classList.add("agentx-kanban-touch-proxy");
      ...
      function move(ev) {
        ...
        if (target !== lastTarget) {
          if (lastTarget) lastTarget.classList.remove("agentx-kanban-column--drop");
          if (target) target.classList.add("agentx-kanban-column--drop");
          lastTarget = target;
        }
      }
      function up() {
        ...
        if (lastTarget) {
          lastTarget.classList.remove("agentx-kanban-column--drop");
          const status = lastTarget.getAttribute("data-kanban-column");
          const isTrash = lastTarget.hasAttribute("data-kanban-trash");
          if (isTrash) {
            lastTarget.dispatchEvent(new CustomEvent("agentx-kanban:delete", {
              detail: { taskId },
              bubbles: true,
            }));
          } else if (status) {
            lastTarget.dispatchEvent(new CustomEvent("agentx-kanban:drop", {
  ...
  function KanbanPage() {
    ...
    useEffect(function () {
      ...
      function openWs() {
        ...
        // loopback. Reading window.__AGENTX_SESSION_TOKEN__ directly (the old
   ...
   const deleteTask = useCallback(function (taskId) {
    ...
    return h(ErrorBoundary, null,
      h("div", { className: "agentx-kanban flex flex-col gap-4" },
        h(BoardSwitcher, {
          board: board,
          boardList: boardList,
  ...
  function AttentionStrip(props) {
    ...
    return h("div", {
      className: cn(
        "agentx-kanban-attention",
        "agentx-kanban-attention--" + topSev,
      ),
    },
      h("div", { className: "agentx-kanban-attention-bar" },
        h("span", { className: "agentx-kanban-attention-icon" },
          topSev === "critical" ? "!!!" : topSev === "error" ? "!!" : "⚠"),
        h("span", { className: "agentx-kanban-attention-text" },
          diagTasks.length === 1
      ...
      expanded
        ? h("div", { className: "agentx-kanban-attention-list" },
            diagTasks.map(function (task) {
              ...
              return h("div", {
                key: task.id,
                className: cn(
                  "agentx-kanban-attention-row",
                  "agentx-kanban-attention-row--" + sev,
                ),
              },
                h("span", { className: "agentx-kanban-attention-row-sev" },
                  sev === "critical" ? "!!!" : sev === "error" ? "!!" : "⚠"),
                h("span", { className: "agentx-kanban-attention-row-id" }, task.id),
                h("span", { className: "agentx-kanban-attention-row-title" },
                  task.title || tx(t, "untitled", "(untitled)")),
                h("span", { className: "agentx-kanban-attention-row-meta" },
                  task.assignee ? "@" + task.assignee : tx(t, "unassigned", "unassigned"),
                  " \u00b7 ",
                  kinds.length > 0 ? kinds.join(", ") : tx(t, "diagnostic", "diagnostic"),
                ),
                h("button", {
                  className: "agentx-kanban-attention-row-btn",
  ...
  function DiagnosticActionButton(props) {
    ...
    const cls = cn(
      "agentx-kanban-diag-action-btn",
      action.suggested ? "agentx-kanban-diag-action-btn--suggested" : "",
    );
    ...
    return h("span", { className: cls + " agentx-kanban-diag-action-btn--unknown" },
      label);
  }
  ...
  function DiagnosticCard(props) {
    ...
    const execAction = function (action) {
      ...
      if (action.kind === "comment") {
        ...
        const ta = document.querySelector(".agentx-kanban-drawer-comment-row input, .agentx-kanban-drawer-comment-row textarea");
        if (ta) {
    ...
    const sevClass = "agentx-kanban-diag--" + (diag.severity || "warning");
    return h("div", { className: cn("agentx-kanban-diag", sevClass) },
      h("div", { className: "agentx-kanban-diag-header" },
        h("span", { className: "agentx-kanban-diag-sev" },
          diag.severity === "critical" ? "!!!" :
          diag.severity === "error" ? "!!" : "\u26a0"),
        h("span", { className: "agentx-kanban-diag-title" },
          diag.title),
      ),
      h("div", { className: "agentx-kanban-diag-detail" },
        diag.detail),
      diag.data && Object.keys(diag.data).length > 0
        ? h("div", { className: "agentx-kanban-diag-data" },
            Object.keys(diag.data).map(function (k) {
              const v = diag.data[k];
              if (Array.isArray(v) && v.length > 0 && typeof v[0] === "string" &&
                  v[0].indexOf("t_") === 0) {
                // Task-id list — render as chips.
                return h("div", { key: k, className: "agentx-kanban-diag-data-row" },
                  h("span", { className: "agentx-kanban-diag-data-key" }, k + ":"),
                  v.map(function (x) {
                    return h("code", {
                      key: x, className: "agentx-kanban-event-phantom-chip",
                    }, x);
                  }),
                );
              }
              return h("div", { key: k, className: "agentx-kanban-diag-data-row" },
                h("span", { className: "agentx-kanban-diag-data-key" }, k + ":"),
                h("span", { className: "agentx-kanban-diag-data-val" },
      ...
      reassignAction
        ? h("div", { className: "agentx-kanban-diag-reassign-row" },
            h("span", { className: "agentx-kanban-diag-reassign-label" },
              tx(t, "reassignTo", "Reassign to:")),
            h("select", {
              className: "agentx-kanban-recovery-select",
        ...
        : null,
      h("div", { className: "agentx-kanban-diag-actions" },
      ...
      msg
        ? h("div", {
            className: cn(
              "agentx-kanban-diag-msg",
              msg.ok ? "agentx-kanban-diag-msg--ok" : "agentx-kanban-diag-msg--err",
            ),
          }, msg.text)
        : null,
    );
  ...
  function DiagnosticsSection(props) {
    ...
    return h("div", { className: "agentx-kanban-section" },
      h("div", { className: "agentx-kanban-section-head-row" },
        h("span", { className: "agentx-kanban-section-head" },
          hasOpenDiags
            ? h("span", { className: "agentx-kanban-section-head-warning" },
                `\u26a0 ${tx(t, "diagnostics", "Diagnostics")} (${diags.length})`)
            : tx(t, "diagnostics", "Diagnostics"),
        ),
        h("button", {
          className: "agentx-kanban-section-toggle",
          onClick: function () { setOpen(function (x) { return !x; }); },
          type: "button",
        }, open ? tx(t, "hide", "Hide") : tx(t, "show", "Show")),
      ),
      open
        ? h("div", { className: "agentx-kanban-diag-list" },
  ...
  function DocsLink() {
    return h("a", {
      ...
      className: "agentx-kanban-docs-link",
      title: "Open AgentX Kanban docs in a new tab",
      "aria-label": "AgentX Kanban documentation",
    }, "?");
  }
  ...
  function OrchestrationPanel() {
    ...
    // each triage card (or runs `agentx kanban decompose <id>`) and tasks
    ...
    return h(Card, { className: "p-3" },
      h(CardContent, { className: "p-2 flex flex-col gap-3" },
        ...
        msg ? h("div", {
          className: msg.ok ? "agentx-kanban-msg-ok" : "agentx-kanban-msg-err",
        }, msg.text) : null,
  ...
  function BoardSwitcher(props) {
    ...
    if (!shouldShow) {
      return h("div", {
        className: "agentx-kanban-boardswitcher-compact",
    ...
    return h("div", { className: "agentx-kanban-boardswitcher" },
      h("div", { className: "agentx-kanban-boardswitcher-inner" },
  ...
  function NewBoardDialog(props) {
    ...
    return h("div", {
      className: "agentx-kanban-dialog-backdrop",
      onClick: function (e) { if (e.target === e.currentTarget) props.onCancel(); },
    },
      h("form", {
        className: "agentx-kanban-dialog",
        onSubmit: onSubmit,
      },
        h("div", { className: "agentx-kanban-dialog-title" },
        ...
        h("div", { className: "agentx-kanban-dialog-actions" },
          h(Button, {
            type: "button",
  ...
  function BoardToolbar(props) {
    ...
    return h("div", { className: "flex flex-wrap items-end gap-3" },
      ...
      h("div", { className: "flex flex-col gap-1",
                 title: "Filter by assigned AgentX profile. Profiles are the named agent identities that claim and work on tasks." },
  ...
  function BulkActionBar(props) {
    ...
    return h("div", { className: "agentx-kanban-bulk" },
      h("span", { className: "agentx-kanban-bulk-count" },
      ...
      h("div", { className: "agentx-kanban-bulk-priority",
      ...
      h("div", { className: "agentx-kanban-bulk-reassign",
                 title: "Reassign selected tasks to a different AgentX profile. Pick a profile (or unassign) and click Apply." },
        h(Select, Object.assign({
          value: assignee,
          className: "h-7 text-xs",
      ...
      h("label", { className: "agentx-kanban-bulk-reclaim-first", title: "Reclaim any active claims before reassigning" },
        h(Checkbox, {
  ...
  function TrashDropZone(props) {
    ...
    useEffect(function () {
      ...
      el.addEventListener("agentx-kanban:delete", onTouchDelete);
      return function () { el.removeEventListener("agentx-kanban:delete", onTouchDelete); };
    }, [props.onDelete]);
    ...
    return h("div", {
      ...
      className: cn(
        "agentx-kanban-trash",
        dragOver ? "agentx-kanban-trash--drop" : "",
        props.draggingTaskId ? "agentx-kanban-trash--active" : "",
      ),
      onDragOver: handleDragOver,
      onDragLeave: handleDragLeave,
      onDrop: handleDrop,
    },
      h("span", { className: "agentx-kanban-trash-icon" }, "🗑️"),
      h("span", { className: "agentx-kanban-trash-label" },
  ...
  function BoardColumns(props) {
    ...
    const isPanBlockedTarget = useCallback(function (target) {
      ...
      if (target.closest && target.closest(".agentx-kanban-card")) return true;
      if (target.closest && target.closest(".agentx-kanban-column-add")) return true;
      if (target.closest && target.closest(".agentx-kanban-col-check")) return true;
    ...
    const stopPan = useCallback(function () {
      ...
      if (el) {
        ...
        el.classList.remove("agentx-kanban-columns--panning");
        el.style.userSelect = "";
      }
    ...
    const handleMouseDown = useCallback(function (e) {
      ...
      setIsPanning(true);
      el.classList.add("agentx-kanban-columns--panning");
    ...
    const handleDragStart = useCallback(function (e) {
      const card = e.target.closest && e.target.closest(".agentx-kanban-card");
      if (!card) return;
    ...
    return h("div", {
      ref: columnsRef,
      className: cn(
        "agentx-kanban-columns",
        isScrollable ? "agentx-kanban-columns--scrollable" : "",
        isPanning ? "agentx-kanban-columns--panning" : "",
      ),
      onDragStart: handleDragStart,
      onDragEnd: handleDragEnd,
  ...
  function Column(props) {
    ...
    useEffect(function () {
      ...
      el.addEventListener("agentx-kanban:drop", onTouchDrop);
      return function () { el.removeEventListener("agentx-kanban:drop", onTouchDrop); };
    ...
    return h("div", {
      ...
      className: cn(
        "agentx-kanban-column",
        dragOver ? "agentx-kanban-column--drop" : "",
      ),
      onDragOver: handleDragOver,
      onDragLeave: handleDragLeave,
      onDrop: handleDrop,
    },
      h("div", { className: "agentx-kanban-column-header",
                 title: colHelp || "" },
        h(Checkbox, {
          className: "agentx-kanban-col-check",
        ...
        h("span", { className: cn("agentx-kanban-dot", COLUMN_DOT[props.column.name]) }),
        h("span", { className: "agentx-kanban-column-label" },
          colLabel || props.column.name),
        h("span", { className: "agentx-kanban-column-count",
                    title: `${props.column.tasks.length} task${props.column.tasks.length === 1 ? "" : "s"} in this column` },
          props.column.tasks.length),
        h("button", {
          type: "button",
          className: "agentx-kanban-column-add",
          title: tx(t, "createTask", "Create task in this column"),
          onClick: function () { setShowCreate(function (v) { return !v; }); },
        }, showCreate ? "×" : "+"),
      ),
      h("div", { className: "agentx-kanban-column-sub" },
        colHelp || ""),
      ...
      h("div", { className: "agentx-kanban-column-body" },
        props.column.tasks.length === 0
          ? h("div", { className: "agentx-kanban-empty" }, tx(t, "noTasks", "— no tasks —"))
          : lanes
            ? lanes.map(function (lane) {
                return h("div", { key: lane.assignee, className: "agentx-kanban-lane" },
                  h("div", { className: "agentx-kanban-lane-head" },
                    h("span", { className: "agentx-kanban-lane-name" }, lane.assignee),
                    h("span", { className: "agentx-kanban-lane-count" }, lane.tasks.length),
                  ),
  ...
  function stalenessClass(task) {
    ...
    if (age >= tier.red)   return "agentx-kanban-card--stale-red";
    if (age >= tier.amber) return "agentx-kanban-card--stale-amber";
    return "";
  }
  ...
  function TaskCard(props) {
    ...
    const handleDragStart = function (e) {
      ...
      const selectedCards = document.querySelectorAll(".agentx-kanban-card--selected");
      if (selectedCards.length > 1 && props.selected) {
        const ghost = document.createElement("div");
        ghost.className = "agentx-kanban-drag-ghost";
    ...
    return h("div", {
      ...
      className: cn(
        "agentx-kanban-card",
        props.selected ? "agentx-kanban-card--selected" : "",
        props.failed ? "agentx-kanban-card--failed" : "",
        props.draggingSource ? "agentx-kanban-card--dragging-source" : "",
        stalenessClass(t),
      ),
      draggable: true,
      tabIndex: 0,
      ...
      h(Card, null,
        h(CardContent, { className: "agentx-kanban-card-content" },
          h("div", { className: "agentx-kanban-card-row" },
            h("label", {
              className: "agentx-kanban-card-check-wrap",
              title: tx(i18n, "selectForBulk", "Select for bulk actions"),
              onClick: function (e) { e.stopPropagation(); },
            },
              h(Checkbox, {
                className: "agentx-kanban-card-check",
            ...
            h("span", { className: "agentx-kanban-card-id",
                        title: `Task id: ${t.id}. Use this id with kanban_show, /kanban show, or agentx kanban show.` }, t.id),
            t.warnings && t.warnings.count > 0
              ? h("span", {
                  className: cn(
                    "agentx-kanban-warning-badge",
                    "agentx-kanban-warning-badge--" + (t.warnings.highest_severity || "warning"),
                  ),
                  title: (
            ...
            needsAssignee
              ? h(Badge, {
                  variant: "outline",
                  className: "agentx-kanban-needs-assignee",
          ...
          h("div", { className: "agentx-kanban-card-title" },
            t.title || tx(i18n, "untitled", "(untitled)")),
          h("div", { className: "agentx-kanban-card-row agentx-kanban-card-meta" },
            t.assignee
              ? h("span", { className: "agentx-kanban-assignee",
                            title: `Assigned to AgentX profile @${t.assignee}` }, "@", t.assignee)
              : h("span", { className: "agentx-kanban-unassigned",
                            title: needsAssignee
            ...
            t.comment_count > 0
              ? h("span", { className: "agentx-kanban-count",
            ...
            t.link_counts && (t.link_counts.parents + t.link_counts.children) > 0
              ...
              : null,
            h("span", { className: "agentx-kanban-ago",
  ...
  // centered modal reusing the agentx-kanban-dialog chrome so the form is
  ...
  function InlineCreate(props) {
    ...
    return h("div", {
      ...
      h("form", {
        className: "agentx-kanban-dialog agentx-kanban-create-dialog",
            ...
            h("div", { className: "flex flex-col gap-1 flex-1" },
              ...
              h(Input, {
                ...
                title: props.columnName === "triage"
                  ? "AgentX profile that will spec this task (default: the dispatcher's configured specifier). Leave blank to let the dispatcher pick."
                  : "AgentX profile to assign. Leave blank and the dispatcher will pick from available profiles when the task is Ready.",
  ...
  function TaskDrawer(props) {
    ...
    return h("div", { className: "agentx-kanban-drawer-shade", onClick: props.onClose },
      h("div", {
        className: "agentx-kanban-drawer",
        onClick: function (e) { e.stopPropagation(); },
      },
        h("div", { className: "agentx-kanban-drawer-head" },
          h("span", { className: "text-xs text-muted-foreground" }, props.taskId),
          h("button", {
            type: "button",
            onClick: props.onClose,
            className: "agentx-kanban-drawer-close",
        ...
        data ? h("div", { className: "agentx-kanban-drawer-comment-foot" },
          h("div", {
            className: "agentx-kanban-comment-hint text-xs text-muted-foreground",
          ...
          h("div", { className: "agentx-kanban-drawer-comment-row" },
            h(Input, {
              value: newComment,
  ...
  function AttachmentsSection(props) {
    ...
    return h("div", { className: "agentx-kanban-section" },
      h("div", { className: "agentx-kanban-section-head" },
      ...
      atts.length === 0
        ...
        : atts.map(function (a) {
            return h("div", {
              ...
              h("button", {
                type: "button",
                className: "agentx-kanban-attachment-link truncate",
                title: a.filename,
  ...
  function TaskDetail(props) {
    ...
    return h("div", { className: "agentx-kanban-drawer-body" },
      h("div", { className: "agentx-kanban-drawer-title" },
        h("span", { className: cn("agentx-kanban-dot", COLUMN_DOT[t.status]) }),
        props.editing
          ? h(TitleEditor, {
              initial: t.title || "",
          ...
          : h("span", {
              className: "agentx-kanban-drawer-title-text",
              title: tx(i18n, "clickToEdit", "Click to edit"),
              onClick: function () { props.setEditing(true); },
            }, t.title || tx(i18n, "untitled", "(untitled)")),
      ),
      h("div", { className: "agentx-kanban-drawer-meta" },
      ...
      (function () {
        ...
        if (finalResult) {
          ...
          return h("div", { className: "agentx-kanban-section" },
            h("div", { className: "agentx-kanban-section-head" }, label),
            h(MarkdownBlock, { source: finalResult, enabled: props.renderMarkdown }),
          );
        }
        if (isDone && isParent) {
          return h("div", { className: "agentx-kanban-section" },
            h("div", { className: "agentx-kanban-section-head" }, tx(i18n, "result", "Result")),
            h("div", { className: "agentx-kanban-done-no-result agentx-kanban-done-parent-note" },
        ...
        if (isDone) {
          return h("div", { className: "agentx-kanban-section" },
            h("div", { className: "agentx-kanban-section-head" }, tx(i18n, "result", "Result")),
            h("div", { className: "agentx-kanban-done-no-result" },
      ...
      childResults.length > 0 ? h("div", { className: "agentx-kanban-section" },
        h("div", { className: "agentx-kanban-section-head" },
        ...
        childResults.map(function (child) {
          ...
          return h("div", { key: child.id, className: "agentx-kanban-comment" },
            h("div", { className: "agentx-kanban-comment-head" },
              h("span", { className: "agentx-kanban-comment-author" },
                `${child.id} · ${child.title || tx(i18n, "untitled", "(untitled)")}`),
              h(Badge, { variant: "outline" }, child.status),
              h("button", {
                type: "button",
                className: "agentx-kanban-diag-action-btn",
      ...
      h("div", { className: "agentx-kanban-section" },
        h("div", { className: "agentx-kanban-section-head" },
        ...
        comments.length === 0
          ...
          : comments.map(function (c) {
              return h("div", { key: c.id, className: "agentx-kanban-comment" },
                h("div", { className: "agentx-kanban-comment-head" },
                  h("span", { className: "agentx-kanban-comment-author" }, c.author || "anon"),
                  h("span", { className: "agentx-kanban-comment-ago" },
      ...
      h("div", { className: "agentx-kanban-section" },
        ...
        events.slice().reverse().slice(0, 20).map(function (e) {
          ...
          return h("div", {
            ...
            className: cn(
              "agentx-kanban-event",
              isDiag ? "agentx-kanban-event--hallucination" : "",
            ),
          },
            isDiag
              ? h("div", { className: "agentx-kanban-event-header" },
                  h("span", { className: "agentx-kanban-event-warning-icon" }, "⚠"),
                  h("span", { className: "agentx-kanban-event-warning-label" },
                    getDiagnosticEventLabel(i18n, e.kind) || e.kind),
                  h("span", { className: "agentx-kanban-event-ago" },
                    timeAgo ? timeAgo(e.created_at) : ""),
                )
              : h("div", { className: "agentx-kanban-event-header-plain" },
                  h("span", { className: "agentx-kanban-event-kind" }, e.kind),
                  h("span", { className: "agentx-kanban-event-ago" },
                    timeAgo ? timeAgo(e.created_at) : ""),
                ),
            isDiag && phantoms.length > 0
              ? h("div", { className: "agentx-kanban-event-phantom-row" },
                  h("span", { className: "agentx-kanban-event-phantom-label" },
                    tx(i18n, "phantomIds", "Phantom ids:")),
                  phantoms.map(function (pid) {
                    return h("code", {
                      key: pid,
                      className: "agentx-kanban-event-phantom-chip",
                    }, pid);
                  }),
                )
              : null,
            e.payload && !isDiag
              ? h("code", { className: "agentx-kanban-event-payload" },
    ...
    return h("div", { className: "agentx-kanban-section" },
      h("div", { className: "agentx-kanban-section-head-row" },
        ...
        !showAll
          ? h("button", {
              type: "button",
              onClick: function () { setExpanded(true); },
              className: "agentx-kanban-edit-link",
      ...
      visible.map(function (r) {
        const outcomeClass = r.ended_at
          ? `agentx-kanban-run--${r.outcome || r.status || "ended"}`
          : "agentx-kanban-run--active";
        return h("div", { key: r.id, className: cn("agentx-kanban-run", outcomeClass) },
          h("div", { className: "agentx-kanban-run-head" },
            h("span", { className: "agentx-kanban-run-outcome" },
              r.ended_at ? (r.outcome || r.status || tx(t, "ended", "ended")) : tx(t, "active", "active")),
            h("span", { className: "agentx-kanban-run-profile" },
              r.profile ? `@${r.profile}` : tx(t, "noProfile", "(no profile)")),
            h("span", { className: "agentx-kanban-run-elapsed" }, fmtElapsed(r)),
            h("span", { className: "agentx-kanban-run-ago" },
              timeAgo ? timeAgo(r.started_at) : ""),
          ),
          r.summary
            ? h("div", { className: "agentx-kanban-run-summary" }, r.summary)
            : null,
          r.error
            ? h("div", { className: "agentx-kanban-run-error" }, r.error)
            : null,
          (r.metadata && Object.keys(r.metadata).length > 0)
            ? (function () {
                ...
                return h("details", {
                    className: "agentx-kanban-run-meta-block",
                    open: !collapsed,
                  },
                  h("summary", { className: "agentx-kanban-run-meta-label" }, "Metadata"),
                  h("code", { className: "agentx-kanban-run-meta" }, json),
                );
              })()
            : null,
        );
  ...
  function WorkerLogSection(props) {
    ...
    if (state.loading) {
      ...
      body = h("pre", { className: "agentx-kanban-pre agentx-kanban-log" },
        data.content || "(empty)");
    }
  ...
  function MetaRow(props) {
    return h("div", { className: "agentx-kanban-meta-row" },
      h("span", { className: "agentx-kanban-meta-label" }, props.label),
      h("span", { className: "agentx-kanban-meta-value" }, props.value),
    );
  }
  ...
  function TitleEditor(props) {
    ...
    return h("div", { className: "agentx-kanban-edit-row" },
      h(Input, {
  ...
  function AssigneeEditor(props) {
    ...
    if (!editing) {
      return h("div", { className: "agentx-kanban-meta-row" },
        h("span", { className: "agentx-kanban-meta-label" }, tx(t, "assignee", "Assignee")),
        h("span", {
          className: "agentx-kanban-meta-value agentx-kanban-editable",
  ...
  function PriorityEditor(props) {
    ...
    if (!editing) {
      return h("div", { className: "agentx-kanban-meta-row" },
        h("span", { className: "agentx-kanban-meta-label" }, tx(t, "priority", "Priority")),
        h("span", {
          className: "agentx-kanban-meta-value agentx-kanban-editable",
          onClick: function () { setEditing(true); },
  ...
  function ModelEditor(props) {
    ...
    if (!editing) {
      return h("div", { className: "agentx-kanban-meta-row" },
        h("span", { className: "agentx-kanban-meta-label" }, tx(t, "model", "Model")),
        h("span", {
          className: cn(
            "agentx-kanban-meta-value agentx-kanban-editable",
    ...
    return h("div", { className: "agentx-kanban-meta-row" },
      h("span", { className: "agentx-kanban-meta-label" }, tx(t, "model", "Model")),
      loading
        ? h("span", { className: "agentx-kanban-meta-value text-muted-foreground" },
  ...
  function BodyEditor(props) {
    ...
    return h("div", { className: "agentx-kanban-section" },
      h("div", { className: "agentx-kanban-section-head-row" },
        h("span", { className: "agentx-kanban-section-head" }, tx(t, "description", "Description")),
        editing
      ...
      editing
        ? h("textarea", {
            className: "agentx-kanban-textarea",
            value: v,
            rows: 8,
            onChange: function (e) { setV(e.target.value); },
          })
  ...
  function DependencyEditor(props) {
    ...
    return h("div", { className: "agentx-kanban-section" },
      h("div", { className: "agentx-kanban-section-head" }, tx(t, "dependencies", "Dependencies")),
      h("div", { className: "agentx-kanban-deps-row" },
        h("span", { className: "agentx-kanban-deps-label" }, tx(t, "parents", "Parents:")),
        h("div", { className: "agentx-kanban-deps-chips" },
          (links.parents || []).length === 0
            ? h("span", { className: "agentx-kanban-deps-empty" }, tx(t, "none", "none"))
            : (links.parents || []).map(function (id) {
                return h("span", { key: id, className: "agentx-kanban-dep-chip" },
                  id,
                  h("button", {
                    type: "button",
                    className: "agentx-kanban-dep-chip-x",
      ...
      h("div", { className: "agentx-kanban-deps-row" },
        h("span", { className: "agentx-kanban-deps-label" }, tx(t, "children", "Children:")),
        h("div", { className: "agentx-kanban-deps-chips" },
  ...
  function StatusActions(props) {
    ...
    return h("div", null,
      h("div", { className: "agentx-kanban-actions" },
        specifyButton,
        decomposeButton,
      ...
      specifyMsg ? h("div", {
        className: specifyMsg.ok
          ? "agentx-kanban-msg-ok"
          : "agentx-kanban-msg-err",
  ...
  function HomeSubsSection(props) {
    ...
    return h("div", { className: "agentx-kanban-section" },
      ...
      h("div", { className: "agentx-kanban-home-subs" },
        channels.map(function (hc) {
          ...
          return h(Button, {
            ...
            className: hc.subscribed
              ? "agentx-kanban-home-sub agentx-kanban-home-sub--on"
              : "agentx-kanban-home-sub",
          }, label);
        })
      )
    );
  ...
  if (window.__AGENTX_PLUGINS__ && typeof window.__AGENTX_PLUGINS__.register === "function") {
    window.__AGENTX_PLUGINS__.register("kanban", KanbanPage);
  }
})();
