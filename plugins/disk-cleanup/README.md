...
Scoped strictly to `$AGENTX_HOME` and `/tmp/agentx-*`.
...
| Hook | Behaviour |
|---|---|
| `post_tool_call` | When `write_file` / `terminal` / `patch` creates a file matching `test_*`, `tmp_*`, or `*.test.*` inside `AGENTX_HOME`, track it silently as `test` / `temp` / `cron-output`. |
...
- `is_safe_path()` rejects anything outside `AGENTX_HOME` or `/tmp/agentx-*`
- Windows mounts (`/mnt/c` etc.) are rejected
- The state directory `$AGENTX_HOME/disk-cleanup/` is itself excluded
- `$AGENTX_HOME/logs/`, `memories/`, `sessions/`, `skills/`, `plugins/`,
