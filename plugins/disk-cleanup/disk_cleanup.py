"""disk_cleanup — ephemeral file cleanup for AgentX Workmate.
...
Rules:
  ...
  - empty dirs    → always delete (under AGENTX_HOME)
...
try:
    ...
    def get_hermes_home() -> Path:  # type: ignore[no-redef]
        ...
        return Path(val).resolve() if val else (Path.home() / ".agentx").resolve()
...
def get_state_dir() -> Path:
    """State dir — separate from ``$AGENTX_HOME/logs/``."""
...
def get_log_file() -> Path:
    """Audit log — intentionally NOT under ``$AGENTX_HOME/logs/``."""
...
_EMPTY_DIR_PROTECTED_TOP_LEVEL = frozenset({
    ...
    "agentx-agent", "backups", "profiles", ".worktrees",
...
# Paths under $AGENTX_HOME that must NEVER be deleted by quick(),
...
def _is_protected_cron_path(p: Path) -> bool:
    ...
    history at once.
    """
    # Lazily build the set once per process so AGENTX_HOME is resolved
    # exactly once.
...
def track(path_str: str, category: str, silent: bool = False) -> bool:
    ...
    if not is_safe_path(path):
        _log(f"REJECT: {path} (outside AGENTX_HOME)")
        return False
...
def quick() -> Dict[str, Any]:
    ...
    # Remove empty dirs under AGENTX_HOME, but never recurse into known
    # durable state trees.  Some installs place the AgentX checkout, venv,
    # and desktop build under AGENTX_HOME; a full rglob over that tree can
