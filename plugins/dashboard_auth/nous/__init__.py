...
Configuration surfaces (env wins over config.yaml when set non-empty):
  ...
  per-deploy values don't need to bake into ``config.yaml``:
      ...
      AGENTX_DASHBOARD_OAUTH_CLIENT_ID  — shape ``agent:{agent_instance_id}``
      AGENTX_DASHBOARD_PORTAL_URL       — defaults to
...
Skip reasons:
  ...
  ("Set AGENTX_DASHBOARD_OAUTH_CLIENT_ID …") instead of the bare "no
...
# Production Portal URL. Override via AGENTX_DASHBOARD_PORTAL_URL for
...
class NousDashboardAuthProvider(DashboardAuthProvider):
    ...
    def _verify_jwt(self, access_token: str) -> Dict[str, Any]:
        ...
        try:
            ...
            # between AGENTX_DASHBOARD_PORTAL_URL / AGENTX_DASHBOARD_OAUTH_CLIENT_ID
...
def _resolve_client_id() -> str:
    ...
    Order:
      1. ``AGENTX_DASHBOARD_OAUTH_CLIENT_ID`` env var (when non-empty
    ...
    """
    env = os.environ.get("AGENTX_DASHBOARD_OAUTH_CLIENT_ID", "").strip()
    if env:
        return env
...
def _resolve_portal_url() -> str:
    ...
    Order:
      1. ``AGENTX_DASHBOARD_PORTAL_URL`` env var (non-empty after strip).
      2. ``dashboard.oauth.portal_url`` in ``config.yaml``.
      3. :data:`_DEFAULT_PORTAL_URL` (production Portal).
    """
    env = os.environ.get("AGENTX_DASHBOARD_PORTAL_URL", "").strip()
    if env:
        return env
    cfg_value = str(
...
def register(ctx) -> None:
    ...
    configured (either via ``AGENTX_DASHBOARD_OAUTH_CLIENT_ID`` env var
    ...
    can surface "Set AGENTX_DASHBOARD_OAUTH_CLIENT_ID …" instead of the
    ...
    if not client_id:
        LAST_SKIP_REASON = (
            ...
            "'agent:{instance_id}') when it deploys an AgentX Workmate "
    ...
    if not client_id.startswith("agent:"):
        LAST_SKIP_REASON = (
            f"AGENTX_DASHBOARD_OAUTH_CLIENT_ID={client_id!r} doesn't match "
