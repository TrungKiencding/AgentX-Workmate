...
three virtual model IDs so the ``agentx tools`` model picker and the
    ...
    def generate(
        ...
        **kwargs: Any,
        ...
        if not api_key:
            return error_response(
                error=(
                    "OPENAI_API_KEY not set. Run `agentx tools` → Image "
                    "Generation → OpenAI to configure, or `agentx setup` "
                    "to add the key."
                ),
