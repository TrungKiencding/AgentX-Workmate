...
``https://api.deepinfra.com/v1/openai/models?filter=true&sort_by=agentx``
...
models show up in ``agentx tools`` automatically. No model ids are
hardcoded in this file — if a model is retired upstream, it disappears
from agentx the next time the catalog is fetched, no patch required.
    ...
    def generate(
        ...
        **kwargs: Any,
        ...
        if not api_key:
            return error_response(
                error=(
                    "DEEPINFRA_API_KEY not set. Run `agentx tools` → Image "
                    "Generation → DeepInfra to configure, or `agentx setup` "
                    "to add the key."
                ),
