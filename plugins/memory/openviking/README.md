...
- OpenViking server running and reachable from AgentX

OpenViking 0.2.10 or newer is recommended. For backward compatibility,
AgentX can identify older servers that expose the legacy status-only health
...
connection values into AgentX, or create a minimal `ovcli.conf` when one does
not exist.
...
OpenViking's server config is separate from AgentX:
...
AgentX-side provider config is read from environment variables in the active
profile's `.env`:
...
| `OPENVIKING_AGENT` | `agentx` | AgentX peer ID in OpenViking, used for peer-scoped memories |

When `OPENVIKING_API_KEY` is set, AgentX lets OpenViking derive account/user
identity from the key. In local or trusted deployments without an API key,
AgentX sends `OPENVIKING_ACCOUNT` and `OPENVIKING_USER` as identity headers.
...
AgentX built-in `memory` tool additions are mirrored to OpenViking after the
local memory operation succeeds:

| AgentX action | OpenViking operation |
|---------------|----------------------|
| `add` | `content/write` with `mode=create` under the configured peer memory namespace |

Built-in `replace` and `remove` operations are not mirrored because AgentX
