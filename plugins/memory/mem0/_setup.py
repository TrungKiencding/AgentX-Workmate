...
def _setup_platform(hermes_home: str, config: dict, flags: dict[str, str]) -> None:
    ...
    # docs tell self-hosted users to put MEM0_HOST in ~/.agentx/.env. Warn
    ...
    if os.environ.get("MEM0_HOST", "").strip():
        print(
            ...
            "remove it from ~/.agentx/.env (or unset it) or AgentX will keep "
...
def _setup_oss(hermes_home: str, config: dict, flags: dict[str, str]) -> None:
    ...
    if env_writes:
        _write_env(Path(hermes_home) / ".env", env_writes)
    _save_mem0_json(hermes_home, {"mode": "oss", "user_id": user_id, "agent_id": "agentx", "oss": oss_config})
...
def post_setup(hermes_home: str, config: dict) -> None:
    """Entry point called by agentx memory setup framework.
