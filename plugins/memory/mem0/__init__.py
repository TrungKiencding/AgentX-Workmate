...
Configuration
-------------
Secret (lives in $AGENTX_HOME/.env or the environment):
...
Behavioral settings (live in $AGENTX_HOME/mem0.json, set via `agentx memory
setup`):
...
def _load_config() -> dict:
    """Load config from env vars, with $AGENTX_HOME/mem0.json overrides.
    ...
    config = {
        ...
        "agent_id": os.environ.get("MEM0_AGENT_ID", "agentx"),
        "oss": {},
    }
...
class Mem0MemoryProvider(MemoryProvider):
    ...
    def save_config(self, values, hermes_home):
        """Write config to $AGENTX_HOME/mem0.json."""
        import json
    ...
    def initialize(self, session_id: str, **kwargs) -> None:
        ...
        #   1. Operator-configured MEM0_USER_ID (env or $AGENTX_HOME/mem0.json) —
        ...
        self._agent_id = self._config.get("agent_id", "agentx")
