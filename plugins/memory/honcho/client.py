...
Resolution order for config file:
  1. $AGENTX_HOME/honcho.json  (instance-local, enables isolated AgentX instances)
...
def resolve_active_host() -> str:
    """Derive the Honcho host key from the active AgentX profile.
    ...
    """
    explicit = os.environ.get("AGENTX_HONCHO_HOST", "").strip()
    if explicit:
        return explicit
    ...
    # but applying it before profile resolution makes every named AgentX
    # profile share that same host.  Keep named profiles isolated; only the
    # default AgentX profile may opt into the config's default host.
...
def resolve_config_path() -> Path:
    ...
    Resolution order:
      1. $AGENTX_HOME/honcho.json      (profile-local, if it exists)
      2. ~/.agentx/honcho.json          (default profile — shared host blocks live here)
...
class HonchoClientConfig:
    ...
    def from_global_config(
        ...
        Resolution: $AGENTX_HOME/honcho.json -> ~/.honcho/config.json -> env vars.
        When host is None, derives it from the active AgentX profile.
        """
        resolved_host = host or resolve_active_host()
        ...
        # baseUrl / base_url keys below are the AgentX-specific spelling.
    ...
    def resolve_session_name(
        ...
        Resolution order:
          ...
          2. per-session strategy — AgentX session_id ({timestamp}_{hex}); authoritative,
             so a generated title never remaps a live conversation
          3. Manual directory override from sessions map
          4. AgentX session title (from /title command; non-per-session)
...
def get_honcho_client(config: HonchoClientConfig | None = None) -> Honcho:
    ...
    if not config.api_key and not config.base_url:
        raise ValueError(
            ...
            "then run 'agentx honcho setup' or set HONCHO_API_KEY. "
    ...
    def _build() -> "Honcho":
        ...
        try:
            ...
            raise ImportError(
                ...
                "(or run `agentx honcho setup` to configure)."
            )
