"""agentx-memory-store — holographic memory plugin using MemoryProvider interface.
...
Config in $AGENTX_HOME/config.yaml (profile-scoped):
  plugins:
    agentx-memory-store:
      db_path: $AGENTX_HOME/memory_store.db   # omit to use the default
      auto_extract: false
      default_trust: 0.5
...
def _load_plugin_config() -> dict:
    try:
        ...
        return cfg_get(all_config, "plugins", "agentx-memory-store", default={}) or {}
    except Exception:
        return {}
...
class HolographicMemoryProvider(MemoryProvider):
    ...
    def save_config(self, values, hermes_home):
        """Write config to config.yaml under plugins.agentx-memory-store."""
        ...
        try:
            ...
            existing["plugins"]["agentx-memory-store"] = values
    ...
    def initialize(self, session_id: str, **kwargs) -> None:
        ...
        # Expand $AGENTX_HOME in user-supplied paths so config values like
        # "$AGENTX_HOME/memory_store.db" or "~/.agentx/memory_store.db" both
        # resolve to the active profile's directory.
        if isinstance(db_path, str):
            db_path = db_path.replace("$AGENTX_HOME", _hermes_home)
            db_path = db_path.replace("${AGENTX_HOME}", _hermes_home)
