...
- Hosted: API key from [app.supermemory.ai/integrations?connect=agentx](http://app.supermemory.ai/integrations?connect=agentx)
...
Before running `agentx memory setup`, add the local endpoint to
...
Then run `agentx memory setup` and enter the local server's API key. Configuring
the endpoint first ensures the setup connection probe also stays local.

## Config

Config file: `$AGENTX_HOME/supermemory.json`
...
`https://api.supermemory.ai`. AgentX resolves it once and uses the same endpoint
...
All Supermemory API calls send `x-sm-source: agentx`, and document writes stamp
`metadata.sm_source: agentx`. This is a **functional routing key, not telemetry**:
it groups AgentX-written memories into a dedicated "AgentX" Space in the
...
Use `{identity}` in the `container_tag` to scope memories per AgentX profile:
