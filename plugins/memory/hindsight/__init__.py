...
Config via environment variables:
  ...
  HINDSIGHT_BANK_ID                — memory bank identifier (default: agentx)
...
Or via $AGENTX_HOME/hindsight/config.json (profile-scoped), falling back to
...
def _check_local_runtime() -> tuple[bool, str | None]:
    ...
    so AgentX can degrade gracefully instead of repeatedly trying to start
    ...
    healthy and ``agentx memory status`` would stay green while the daemon
...
def _load_config() -> dict:
    ...
    Resolution order:
      1. $AGENTX_HOME/hindsight/config.json  (profile-scoped)
    ...
    return {
        ...
        "banks": {
            "agentx": {
                "bankId": os.environ.get("HINDSIGHT_BANK_ID", "agentx"),
...
def _embedded_profile_name(config: dict[str, Any]) -> str:
    """Return the Hindsight embedded profile name for this AgentX config."""
    profile = config.get("profile", "agentx")
    return str(profile or "agentx")
...
def _load_simple_env(path) -> dict[str, str]:
    ...
    values: dict[str, str] = {}
    # utf-8-sig, not plain utf-8: this is also used on the AgentX .env during
...
def _resolve_bank_id_template(template: str, fallback: str, **placeholders: str) -> str:
    ...
    Supported placeholders (each is sanitized before substitution):
      {profile}   — active AgentX profile name (from agent_identity)
      {workspace} — AgentX workspace name (from agent_workspace)
      {platform}  — "cli", "telegram", "discord", etc.
      {user}      — platform user id (gateway sessions)
      {session}   — current session id

    Missing/empty placeholders are rendered as the empty string and then
    collapsed — e.g. ``agentx-{user}`` with no user becomes ``agentx``.
...
class HindsightMemoryProvider(MemoryProvider):
    ...
    def __init__(self):
        ...
        self._retain_context = "conversation between AgentX Workmate and the User"
    ...
    def save_config(self, values, hermes_home):
        """Write config to $AGENTX_HOME/hindsight/config.json."""
        import json
        from pathlib import Path
    ...
    def post_setup(self, hermes_home: str, config: dict) -> None:
        ...
        provider_config.setdefault("bank_id", "agentx")
    ...
    def get_config_schema(self):
        return [
            ...
            {"key": "bank_id", "description": "Memory bank name (static fallback when bank_id_template is unset)", "default": "agentx"},
            {"key": "bank_id_template", "description": "Optional template to derive bank_id dynamically. Placeholders: {profile}, {workspace}, {platform}, {user}, {session}. Example: agentx-{profile}", "default": ""},
            ...
            {"key": "retain_context", "description": "Context label for retained memories", "default": "conversation between AgentX Workmate and the User"},
        ...
        if self._client is None:
            if self._mode == "local_embedded":
                ...
                logger.debug("Creating HindsightEmbedded client (profile=%s, provider=%s)",
                             self._config.get("profile", "agentx"), llm_provider)
                kwargs = dict(
                    profile=self._config.get("profile", "agentx"),
    ...
    def initialize(self, session_id: str, **kwargs) -> None:
        ...
        try:
            ...
            if Version(installed) < Version(_MIN_CLIENT_VERSION):
                ...
                # durable data-volume target instead of /opt/agentx (NS-605).
                from tools.lazy_deps import install_specs
        ...
        banks = cfg_get(self._config, "banks", "agentx", default={})
        static_bank_id = self._config.get("bank_id") or banks.get("bankId", "agentx")
        ...
        self._retain_context = self._config.get("retain_context", "conversation between AgentX Workmate and the User")
        ...
        if self._mode == "local_embedded":
            ...
            if hasattr(os, "geteuid") and os.geteuid() == 0:
                msg = (
                    ...
                    "memory daemon. Run AgentX as a non-root user, or switch "
                    "to cloud / local_external mode via 'agentx memory setup'."
                )
                logger.warning(msg)
                # Surface to the terminal too — a daemon that never starts
                # would otherwise fail silently and the user would only see
                # AgentX get sluggish. (issue #13125)
                try:
    ...
    def on_session_switch(
        ...
        **kwargs,
        ...
        document. See agentx-agent#6672.
        ...
        if self._client is not None:
            try:
                if self._mode == "local_embedded":
                    ...
                    # When AgentX created/used that client on the shared async loop,
