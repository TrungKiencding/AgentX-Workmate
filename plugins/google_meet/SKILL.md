...
agentx meet install                 # pip deps + Chromium (transcribe only)
agentx meet install --realtime      # + pulseaudio-utils / brew blackhole+ffmpeg
agentx meet auth                    # optional; skips guest-lobby wait
agentx meet setup                   # preflight checks
```

`agentx meet install --realtime` prompts before running `sudo apt-get` (Linux)
...
#   Then set OPENAI_API_KEY or AGENTX_MEET_REALTIME_KEY in ~/.agentx/.env
```
...
pip install playwright websockets && python -m playwright install chromium
agentx plugins enable google_meet
agentx meet node run --display-name my-mac    # persistent server
# copy the printed token

# on the gateway:
agentx meet node approve my-mac ws://<mac-ip>:18789 <token>
agentx meet node ping my-mac                   # confirm reachable
```

Run `agentx meet setup` to preflight local prereqs.

## Flow

1. **Join** — call `meet_join(url=..., mode=..., node=...)`. Returns immediately.
2. **Announce yourself** — no auto-consent. Say (in whatever channel the user is watching): "An AgentX agent bot is in this call taking notes."
...
- Guest mode sits in the lobby until a host admits. Warn the user; `agentx meet auth` avoids this.
- **Lobby timeout**: if the host doesn't admit the bot within 5 minutes (configurable via `AGENTX_MEET_LOBBY_TIMEOUT` env), the bot leaves and `meet_status` reports `leaveReason: "lobby_timeout"`.
...
Local:
```
$AGENTX_HOME/workspace/meetings/<meeting-id>/transcript.txt
```
...
- Remote nodes use bearer-token auth; tokens are generated on the node (32 hex chars, persisted in `$AGENTX_HOME/workspace/meetings/node_token.json`) and must be copied to the gateway via `agentx meet node approve`.
