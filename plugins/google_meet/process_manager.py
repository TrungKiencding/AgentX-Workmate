...
session-scoped state file under ``$AGENTX_HOME/workspace/meetings/.active.json``
...
# File + directory layout (under $AGENTX_HOME):
#
#   workspace/meetings/
...
def start(
    ...
    *,
    ...
    guest_name: str = "AgentX Workmate",
    ...
    If a bot is already running for this agentx install, leave it first —
    ...
    env = os.environ.copy()
    env["AGENTX_MEET_URL"] = url
    env["AGENTX_MEET_OUT_DIR"] = str(out)
    env["AGENTX_MEET_GUEST_NAME"] = guest_name
    if headed:
        env["AGENTX_MEET_HEADED"] = "1"
    if auth_state:
        env["AGENTX_MEET_AUTH_STATE"] = auth_state
    if duration:
        env["AGENTX_MEET_DURATION"] = duration
    # v2: realtime mode + passthroughs. The bot defaults to transcribe
    # mode if AGENTX_MEET_MODE isn't set, matching v1 behavior.
    if mode:
        env["AGENTX_MEET_MODE"] = mode
    if realtime_model:
        env["AGENTX_MEET_REALTIME_MODEL"] = realtime_model
    if realtime_voice:
        env["AGENTX_MEET_REALTIME_VOICE"] = realtime_voice
    if realtime_instructions:
        env["AGENTX_MEET_REALTIME_INSTRUCTIONS"] = realtime_instructions
    ...
    # meet_bot checks AGENTX_MEET_REALTIME_KEY before OPENAI_API_KEY.
    if not realtime_api_key:
        try:
            from agent.secret_scope import get_secret

            realtime_api_key = (
                get_secret("AGENTX_MEET_REALTIME_KEY")
                or get_secret("OPENAI_API_KEY")
            )
        except ImportError:  # pragma: no cover — secret_scope is in-repo
            pass
    if realtime_api_key:
        env["AGENTX_MEET_REALTIME_KEY"] = realtime_api_key
