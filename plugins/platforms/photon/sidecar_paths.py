...
under an immutable ``/opt/agentx`` (read-only for the agentx user), which
...
4. Source dir read-only and deps missing or stale → mirror the sidecar
   source files to ``$AGENTX_HOME/photon/sidecar`` (the durable data volume,
