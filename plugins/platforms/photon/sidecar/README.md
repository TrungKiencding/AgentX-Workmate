...
Small Node helper that bridges AgentX Workmate to Photon's Spectrum SDK
(`spectrum-ts`).  AgentX is Python; Photon has no public HTTP
...
- exposes a loopback-only HTTP control channel for the Python adapter
  to push send/typing requests (auth via `X-Agentx-Sidecar-Token`)
- drains the inbound message stream so `spectrum-ts` keeps its
  reconnect/heartbeat machinery alive and AgentX can receive inbound messages
...
The AgentX plugin's `agentx photon setup` command runs `npm install`
here automatically.
...
`Space.send(...)` API. AgentX is Python, so replies go through this sidecar
