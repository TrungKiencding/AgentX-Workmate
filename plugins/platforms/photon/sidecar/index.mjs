// AgentX Workmate — Photon Spectrum sidecar
//
...
//
// Inbound  (gRPC -> AgentX): the SDK's `app.messages` async iterator is a
...
//   gateway connects.
// Outbound (AgentX -> gRPC): `/send` drives `space.send(...)`; `/typing`
//   sends the documented `typing("start" | "stop")` content builder.
//
// Protocol (all requests require `X-Agentx-Sidecar-Token: ${TOKEN}`):
...
//                          default off — toggle with `agentx photon telemetry`)
  ...
  streamRestartTimer = setTimeout(() => {
    ...
    console.error(
      `photon-sidecar: upstream stream degraded for ${degradedForMs}ms; ` +
        "exiting so AgentX can restart the Photon adapter"
    );
    process.exit(75);
...
// instead of a cryptic module-resolution error during import. Apply AgentX'
...
function inboundStreamErrorMessage(e) {
  ...
  // iMessage internal error. Local AgentX allowlists cannot cause or fix this:
  ...
  if (
    ...
    out +=
      ...
      "this is upstream of AgentX, so inbound iMessages may not be delivered " +
...
const server = http.createServer(async (req, res) => {
  if (!tokenOk(req.headers["x-agentx-sidecar-token"])) {
  ...
  try {
    ...
    if (req.url === "/send-attachment") {
      ...
      // overrides only when AgentX supplied them so a known-good
