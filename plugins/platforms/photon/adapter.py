"""
Photon Spectrum (iMessage) platform adapter for AgentX Workmate.
...
#
# Out-of-process senders (cron subprocesses, `agentx send`, the dashboard)
...
def _runtime_record_path() -> Path:
    # get_hermes_home() honors profile overrides — never hardcode ~/.agentx.
...
# this module (agentx status, test collection, plugin discovery).
...
# artifacts so AgentX sees the link once, not a follow-up "(attachment)" prompt.
...
def sidecar_deps_installed() -> bool:
    ...
    _start_sidecar(), and `agentx photon status` so all three agree on
...
def check_requirements() -> bool:
    ...
    if not sidecar_deps_installed():
        ...
        # user has no CLI to run `agentx photon setup`, so the connect path
        # must self-heal). Otherwise keep returning False so
        # `agentx setup` / status surface the missing-deps state.
        ...
        # (load_gateway_config, agentx status, GET /api/status polling) —
        ...
        if npm_error:
            logger.debug(
                ...
                "(last npm error: %s) — run: agentx photon setup",
                _sidecar_dir(),
                npm_error,
            )
        else:
            logger.debug(
                "photon: spectrum-ts not installed at %s — run: agentx photon setup",
                _sidecar_dir(),
            )
        return False
    return True
...
def _sidecar_deps_stale() -> bool:
    ...
    `agentx update` rewrites ``package-lock.json`` when the spectrum-ts pin is
...
def _reinstall_sidecar_deps() -> None:
    ...
    Mirrors ``agentx photon install-sidecar``: ``npm ci`` for an exact,
...
def _richlink_candidate(text: str) -> Optional[str]:
    ...
    markdown/text path so AgentX does not drop labels or rewrite intent.
    """
...
class PhotonAdapter(BasePlatformAdapter):
    ...
    def _compile_mention_patterns(raw: Any) -> "list[re.Pattern]":
        ...
        list, or comma/newline-separated), or None (use AgentX defaults).
    ...
    async def _inbound_loop(self) -> None:
        ...
        headers = {"X-Agentx-Sidecar-Token": self._sidecar_token}
        backoff = 1.0
    ...
    def _pid_is_sidecar(pid: int) -> bool:
        ...
        try:
            ...
            return False
        # Checkout-agnostic: any AgentX checkout's sidecar entry point.
    ...
    async def _start_sidecar(self) -> None:
        if not sidecar_deps_installed():
            ...
            # `agentx photon setup`, so the connect path must be able to
            ...
            if not sidecar_deps_installed():
                raise RuntimeError(
                    ...
                    f"Run: cd {_sidecar_dir()} && npm ci   (or `agentx photon setup`)"
                )
        # A `agentx update` that bumps the spectrum-ts pin rewrites
            ...
            while time.time() < deadline:
                ...
                try:
                    ...
                    if resp.status_code == 200:
                        ...
                        # (cron, `agentx send`) can reach this sidecar
...
async def _standalone_send(
    ...
    *,
    ...
    if not token:
        ...
        # the gateway process env otherwise, so cron/`agentx send` would be
    ...
    headers = {"X-Agentx-Sidecar-Token": token}
...
def register(ctx) -> None:
    """Called by the AgentX plugin loader at startup."""
    # Local import to avoid argparse work at module load; reused for both the
    # gateway-setup hook and the `agentx photon` CLI command below.
    ...
    ctx.register_platform(
        ...
        install_hint=(
            "Run: agentx photon setup  (logs in via device flow, creates a "
            "Spectrum project, links your phone number, installs the "
            "spectrum-ts sidecar)."
        ),
        # Surfaces Photon in `agentx gateway setup` alongside every other
    ...
    # Register CLI subcommands — `agentx photon ...`
