...
This plugin connects AgentX Workmate to iMessage (and other Spectrum
...
TypeScript-only, AgentX runs it inside a small supervised Node sidecar and
...
There is no separate `login` command; like every other AgentX channel,
...
Run `agentx photon status` to see what's configured.

## Credentials

Runtime SDK credentials live in `~/.agentx/.env` (the same place every other
...
Management metadata lives in `~/.agentx/auth.json` under `credential_pool`:
...
| `PHOTON_TELEMETRY`        | false                      | Spectrum SDK telemetry — toggle with `agentx photon telemetry on\|off` (restart the gateway to apply) |
...
- **Inbound attachments and voice notes are downloaded.** The sidecar reads
  ...
  something arrived. If Spectrum emits a `richlink` content object, AgentX
  ...
  `.pluginPayloadAttachment` images immediately after the URL; AgentX coalesces
...
- **Native polls are supported.** AgentX posts poll content through
...
6. Verify end-to-end: `agentx photon status`, a DM and a group roundtrip,
