"""ntfy platform adapter (AgentX plugin).
...
an AgentX dependency.

This adapter ships as an AgentX platform plugin under
``plugins/platforms/ntfy/``. The AgentX plugin loader scans the
    ...
    platforms:
      ntfy:
        ...
        extra:
          ...
          topic: "agentx-in"              # subscribe topic (incoming)
          publish_topic: "agentx-out"     # optional — defaults to topic
...
_ECHO_TAG = "agentx-agent"  # tag added to outgoing messages for echo-loop prevention
...
async def _standalone_send(
    ...
    *,
    ...
    ``agentx cron`` running standalone). Without this hook,
...
def register(ctx) -> None:
    ...
    ctx.register_platform(
        ...
        install_hint="pip install httpx   # already an AgentX dependency",
        # Env-driven auto-configuration: seeds PlatformConfig.extra so
        # env-only setups show up in `agentx gateway status` without
