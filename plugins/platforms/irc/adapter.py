"""
IRC Platform Adapter for AgentX Workmate.

A plugin-based gateway adapter that connects to an IRC server and relays
messages to/from the AgentX agent.  Zero external dependencies — uses
...
class IRCAdapter(BasePlatformAdapter):
    ...
    def __init__(self, config, **kwargs):
        ...
        try:
            ...
            self.port = 6697
        self.nickname = os.getenv("IRC_NICKNAME") or extra.get("nickname", "agentx-bot")
    ...
    async def connect(self, *, is_reconnect: bool = False) -> bool:
        ...
        await self._send_raw(f"USER {self.nickname} 0 * :AgentX Workmate")
    ...
    async def disconnect(self) -> None:
        ...
        if self._writer and not self._writer.is_closing():
            try:
                await self._send_raw("QUIT :AgentX Workmate shutting down")
    ...
    async def _handle_line(self, raw: str) -> None:
        ...
        if command == "433":
            # Retry with incrementing suffix: agentx_, agentx_1, agentx_2...
...
def check_requirements() -> bool:
    ...
    # agentx setup / requirements check path.
...
def interactive_setup() -> None:
    """Interactive `agentx gateway setup` flow for the IRC platform.
    ...
    print_info("Connect AgentX to an IRC network. Uses Python stdlib — no extra packages needed.")
    ...
    channel = prompt(
        "Channel to join (e.g. #agentx — comma-separate for multiple)",
    ...
    print_info("Restart the gateway for changes to take effect: agentx gateway restart")
...
async def _standalone_send(
    ...
    *,
    ...
    nickname = os.getenv("IRC_NICKNAME") or extra.get("nickname", "agentx-bot")
    ...
    nick_base = nickname.rstrip("_0123456789-")[:24] or "agentx-bot"
    ...
    try:
        ...
        await _raw(f"USER {standalone_nick} 0 * :AgentX Workmate (cron)")
