...
def _discord_ready_timeout_seconds() -> float:
    ...
    raw = os.getenv("AGENTX_GATEWAY_PLATFORM_CONNECT_TIMEOUT", "").strip()
    if raw:
        try:
            return max(0.0, float(raw))
        except ValueError:
            logger.warning(
                "Ignoring invalid AGENTX_GATEWAY_PLATFORM_CONNECT_TIMEOUT=%r",
                raw,
            )
    return 30.0
...
def _read_dm_role_auth_guild() -> Optional[int]:
    ...
    policy, ``~/.agentx/.env`` is for secrets only, and this is a
...
class DiscordAdapter(BasePlatformAdapter):
    ...
    def __init__(self, config: PlatformConfig):
        ...
        self._text_batch_delay_seconds = env_float("AGENTX_DISCORD_TEXT_BATCH_DELAY_SECONDS", 0.6)
        self._text_batch_split_delay_seconds = env_float("AGENTX_DISCORD_TEXT_BATCH_SPLIT_DELAY_SECONDS", 2.0)
        ...
        self._liveness_interval_seconds = self._finite_positive_config_float(
            ...
            15.0,
            env_key="AGENTX_DISCORD_LIVENESS_INTERVAL_SECONDS",
        )
        self._liveness_failure_threshold = self._config_int(
            "websocket_liveness_failure_threshold",
            2,
            env_key="AGENTX_DISCORD_LIVENESS_FAILURE_THRESHOLD",
        )
    ...
    def _handle_bot_task_done(self, task: asyncio.Task) -> None:
        ...
        marked running, the Discord websocket is dead while the AgentX gateway
    ...
    async def cancel_background_tasks(self) -> None:
        ...
        disconnect budget (``AGENTX_GATEWAY_ADAPTER_DISCONNECT_TIMEOUT``, default
    ...
    def _text_batch_flush_deadline_seconds(self) -> float:
        ...
        raw = os.getenv("AGENTX_GATEWAY_ADAPTER_DISCONNECT_TIMEOUT", "").strip()
        if raw:
            try:
                parsed = float(raw)
                if parsed > 0:
    ...
    async def _run_missed_message_backfill(self) -> None:
        ...
        that lack a substantive non-outage AgentX response. Emoji-only acks are
    ...
    async def _should_backfill_discord_message(self, message: Any) -> bool:
        """Return True when a recent Discord message still needs AgentX work."""
    ...
    def _is_down_notice_content(self, content: str) -> bool:
        """Recognize only explicit AgentX/gateway outage notices."""
        text = (content or "").lower()
        subject = r"(?:agentx|the agent|agent|the gateway|gateway|bmo)"
    ...
    def _canonicalize_app_command_payload(self, payload: Dict[str, Any]) -> Dict[str, Any]:
        """Reduce command payloads to the semantic fields AgentX manages."""
    ...
    def _is_pairing_approved_user(self, user_id: str) -> bool:
        """True when the Discord user has an explicit AgentX pairing grant."""
    ...
    def _register_slash_commands(self) -> None:
        ...
        @tree.command(name="reset", description="Reset your AgentX session")
        ...
        @tree.command(name="status", description="Show AgentX session status")
        ...
        @tree.command(name="stop", description="Stop the running AgentX agent")
        ...
        @tree.command(name="reload-skills", description="Re-scan ~/.agentx/skills/ for new or removed skills")
        ...
        @tree.command(name="update", description="Update AgentX Workmate to the latest version")
        async def slash_update(interaction: discord.Interaction):
            await self._run_simple_slash(interaction, "/update", "Update initiated~")

        @tree.command(name="restart", description="Gracefully restart the AgentX gateway")
    ...
    async def _handle_thread_create_slash(
        ...
        # If a message was provided, kick off a new AgentX session in the thread
    ...
    async def _create_thread(
        ...
        *,
        ...
        try:
            ...
            try:
                seed_content = starter_message or f"\U0001f9f5 Thread created by AgentX: **{name}**"
    ...
    def _derive_auto_thread_name(self, content: str) -> str:
        ...
        AgentX has an LLM-generated session title and can safely rename
        ...
        thread_name = content[:80] if content else "AgentX"
        if len(content) > 80:
            ...
            try:
                ...
                try:
                    seed_msg = await message.channel.send(
                        f"\U0001f9f5 Thread created by AgentX: **{thread_name}**"
                    )
                    thread = await seed_msg.create_thread(
                        name=thread_name,
    ...
    async def rename_thread(
        ...
        *,
        ...
        if edit is None:
            return False
        try:
            await edit(name=cleaned, reason="AgentX semantic session title")
            logger.info(
    ...
    async def create_handoff_thread(
        ...
        try:
            ...
            if send is None:
                return None
            seed_msg = await send(f"\U0001f9f5 AgentX handoff: **{thread_name}**")
            thread = await seed_msg.create_thread(
                name=thread_name,
                auto_archive_duration=1440,
                reason=reason,
    ...
    async def send_exec_approval(
        ...
        try:
            ...
            prompt_prefix = (
                ...
                "Do you want AgentX to run this command?\n\n"
                "**Requested command:**\n```bash\n"
            )
            if smart_denied:
    ...
    async def send_clarify(
        ...
        try:
            ...
            content = self._self_contained_prompt_content(
                "❓ **AgentX needs your input**", str(question or "").strip(),
                tail=clarify_tail,
            )
    ...
    async def send_update_prompt(
        ...
        Used by the gateway ``/update`` watcher when ``agentx update --gateway``
                ...
                if thread:
                    ...
                    try:
                        await message.channel.send(
                            "⚠️ AgentX could not create a Discord thread for "
...
def _component_check_auth(
    ...
    the pairing store (``agentx pairing approve``).
    ...
    # so users approved via ``agentx pairing approve`` can interact with
...
def _define_discord_view_classes() -> None:
    ...
    class UpdatePromptView(discord.ui.View):
        """Interactive Yes/No buttons for ``agentx update`` prompts.
...
# is not in this process (e.g. ``agentx cron`` running standalone) and no live
...
def _apply_yaml_config(yaml_cfg: dict, discord_cfg: dict) -> dict | None:
    ...
    _websocket_liveness_keys = (
        (
            ...
            "AGENTX_DISCORD_LIVENESS_INTERVAL_SECONDS",
        ),
        (
            "websocket_liveness_failure_threshold",
            "liveness_failure_threshold",
            "AGENTX_DISCORD_LIVENESS_FAILURE_THRESHOLD",
        ),
...
def register(ctx) -> None:
    """Plugin entry point — called by the AgentX plugin system."""
    ctx.register_platform(
        ...
        install_hint="Run `agentx setup` to install Discord support.",
