...
class TelegramAdapter(BasePlatformAdapter):
    ...
    def __init__(self, config: PlatformConfig):
        ...
        self._media_batch_delay_seconds = env_float("AGENTX_TELEGRAM_MEDIA_BATCH_DELAY_SECONDS", 0.8)
        ...
        self._text_batch_delay_seconds = self._env_float_clamped(
            "AGENTX_TELEGRAM_TEXT_BATCH_DELAY_SECONDS",
            0.3,
            min_value=0.08,
            max_value=2.0,
        )
        self._text_batch_split_delay_seconds = self._env_float_clamped(
            "AGENTX_TELEGRAM_TEXT_BATCH_SPLIT_DELAY_SECONDS",
            1.0,
    ...
    def _thread_kwargs_for_send(
        ...
        metadata. AgentX-created private-chat topic lanes are marked with
    ...
    # older PTB/clients. Streaming edits stay on AgentX' existing MarkdownV2
    ...
    def _has_telegram_desktop_details_math_crash_shape(self, content: str) -> bool:
        ...
        AgentX must skip rich delivery up front and use the legacy MarkdownV2
    ...
    async def _handle_polling_conflict(self, error: Exception) -> None:
        ...
        # has been killed (e.g. during `agentx update` or `--replace` handoffs)
        ...
        message = (
            ...
            "To recover: ensure no other AgentX or OpenClaw instance is running "
            "with this token, then restart the gateway with 'agentx gateway restart'."
    ...
    async def _run_post_connect_housekeeping(self) -> None:
        ...
        try:
            ...
            try:
                ...
                # payload size limit (~4KB total).  AgentX defaults to 60 to
    ...
    async def connect(self, *, is_reconnect: bool = False) -> bool:
        ...
        try:
            ...
            # readable by the AgentX process (shared mount, same machine, etc.).
            ...
            request_kwargs = {
                "connection_pool_size": _env_int("AGENTX_TELEGRAM_HTTP_POOL_SIZE", 512),
                "pool_timeout": _env_float("AGENTX_TELEGRAM_HTTP_POOL_TIMEOUT", 8.0),
                "connect_timeout": _env_float("AGENTX_TELEGRAM_HTTP_CONNECT_TIMEOUT", 10.0),
                "read_timeout": _env_float("AGENTX_TELEGRAM_HTTP_READ_TIMEOUT", 20.0),
                "write_timeout": _env_float("AGENTX_TELEGRAM_HTTP_WRITE_TIMEOUT", 20.0),
            ...
            disable_fallback = (os.getenv("AGENTX_TELEGRAM_DISABLE_FALLBACK_IPS", "").strip().lower() in {"1", "true", "yes", "on"})
            ...
            _max_connect = 8
            _init_timeout = _env_float("AGENTX_TELEGRAM_INIT_TIMEOUT", 30.0)
                ...
                try:
                    ...
                    if asyncio.get_running_loop().time() >= _total_deadline:
                        raise OSError(
                            ...
                            f"or set AGENTX_TELEGRAM_HTTP_CONNECT_TIMEOUT / "
                            f"AGENTX_TELEGRAM_INIT_TIMEOUT to a lower value."
                        )
                    logger.warning(
                    ...
                    if _attempt < _max_connect - 1:
                        ...
                        raise OSError(
                            ...
                            f"or set AGENTX_TELEGRAM_HTTP_CONNECT_TIMEOUT to a lower value."
                        )
    ...
    def _build_provider_keyboard(self, providers: list, page: int = 0) -> tuple:
        ...
        CLI ``agentx model`` picker via the shared ``group_providers`` fold,
    ...
    # Scripts live in ~/.agentx/scripts/gmail-triage/. `arg` from the callback
    ...
    def _message_mentions_bot(self, message: Message) -> bool:
        ...
        # raw substring matches like "foo@agentx_bot.example" are not mentions
    ...
    def _explicit_bot_mentions_exclude_self(self, message: Message) -> bool:
        ...
        Telegram groups can contain several AgentX bot profiles. A message like
    ...
    def _is_own_message(self, message: Message) -> bool:
        ...
        counted as incoming unread messages in the AgentX inbox.
        """
        if not self._bot:
            return False
    ...
    def _should_process_message(self, message: Message, *, is_command: bool = False) -> bool:
        ...
        # unread in the AgentX inbox (#52363).
        #
...
def _resolve_notifications_mode() -> str:
    ...
    mode = os.getenv("AGENTX_TELEGRAM_NOTIFICATIONS", "")
    if not mode:
        try:
...
def register(ctx) -> None:
    ...
    ctx.register_platform(
        ...
        install_hint="Run `agentx setup` to install Telegram support.",
