...
class SlackAdapter(BasePlatformAdapter):
        ...
        try:
            ...
            if "im:history" in granted and "mpim:history" not in granted:
                ...
                logger.warning(
                    ...
                    "Regenerating the app from `agentx slack` produces a "
    ...
    async def connect(self, *, is_reconnect: bool = False) -> bool:
        ...
        if not raw_token:
            logger.error(
                ...
                "error; set SLACK_BOT_TOKEN via `agentx gateway setup` "
                "or in the active profile's ~/.agentx/.env file, then restart "
                "the gateway.",
            )
            self._set_fatal_error(
                "missing_slack_bot_token",
                "SLACK_BOT_TOKEN not configured. Use `agentx gateway setup` "
        ...
        if not app_token:
            logger.error(
                ...
                "error; set SLACK_APP_TOKEN via `agentx gateway setup` "
                "or in the active profile's ~/.agentx/.env file, then restart "
                "the gateway.",
            )
            self._set_fatal_error(
                "missing_slack_app_token",
                "SLACK_APP_TOKEN not configured. Use `agentx gateway setup` "
                "or add it to your active profile's ~/.agentx/.env file, "
                "then restart the gateway.",
                retryable=False,
            )
        ...
        try:
            ...
            # AgentX has no listener for (e.g. user_change,
            ...
            async def handle_unhandled_event(event, body, logger):
                logger.debug(
                    ...
                    "registered; subscribed events not handled by AgentX can "
                    "be removed from the Slack app manifest via "
                    "`agentx slack manifest`)",
                    (event or {}).get(
                        "type",
            ...
            # /model work directly without /agentx prefix). A single regex
            ...
            # manifest (see `agentx slack manifest`). In Socket Mode, Slack
            ...
            if _slash_names:
                ...
                _slash_pattern = _re.compile(r"^/agentx$")
            ...
            if _allow_bots_cfg != "none":
                logger.info(
                    ...
                    "'agentx slack manifest' if unsure), and (b) the other bot's "
    ...
    async def create_handoff_thread(
        ...
        try:
            ...
            if client is None:
                return None
            seed_text = (
                f":thread: AgentX handoff — *{(name or 'session').strip()[:80]}*"
            )
    ...
    def _dm_top_level_threads_as_sessions(self) -> bool:
        ...
        own AgentX session — matching the per-thread behavior channels already
    ...
    async def _handle_app_context_changed(
        ...
        # AgentX owns. Do not write it into _channel_team: channel IDs can be
    ...
    async def _handle_slack_message(
        ...
        # the shape produced by peer AgentX agents in Socket Mode on some
    ...
    async def _handle_slash_command(self, command: dict) -> None:
        ...
        The legacy ``/agentx <subcommand> [args]`` form is preserved for
        backward compatibility with older workspace manifests and for users
        who want a single entry point for free-form questions (``/agentx
        ...
        if slash_name in {"agentx", ""}:
            # Legacy /agentx <subcommand> [args] routing + free-form questions.
            ...
            # ``text.split()`` returns ``[]`` (e.g. user sends ``/agentx   ``).
        ...
        # questions via "/agentx <question>" must produce public replies so
...
def interactive_setup() -> None:
    ...
    def _write_slack_manifest_and_instruct() -> None:
        ...
        try:
            ...
            manifest = _build_full_manifest(
                bot_name="AgentX",
                bot_description="Your AgentX agent on Slack",
            )
            ...
            print_info(
                "   Re-run `agentx slack manifest --write` anytime to refresh after "
                "AgentX adds new commands."
            )
        except Exception as e:
    ...
    if existing:
        ...
        if not prompt_yes_no("Reconfigure Slack?", False):
            ...
            if prompt_yes_no(
                ...
                "list? (recommended after `agentx update`)",
                True,
            ):
    ...
    print()
    print_info("   Full guide: https://github.com/TrungKiencding/AgentX-Workmate/blob/main/website/docs/user-guide/messaging/slack.md")
    print()
...
def register(ctx) -> None:
    ...
    ctx.register_platform(
        ...
        install_hint="Run `agentx setup` to install Slack support.",
