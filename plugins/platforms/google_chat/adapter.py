...
# ``model_tools`` import time, so ``agentx status``, ``agentx chat``, etc.
...
#     fresh "AgentX is thinking…" card during the small window between
...
def _mime_for_message_type(mime: str) -> MessageType:
    """Map a MIME string to an agentx MessageType.
...
class GoogleChatAdapter(BasePlatformAdapter):
    ...
    def __init__(self, config: PlatformConfig):
        ...
        # Persisted to ${AGENTX_HOME}/google_chat_thread_counts.json so
        ...
        try:
            ...
            _hermes_home = _Path.home() / ".agentx"
        ...
        # they don't sit in the chat forever as "AgentX is thinking…".
    ...
    def _bot_id_cache_path(self) -> _Path:
        ...
        base = os.getenv("AGENTX_HOME", str(_Path.home() / ".agentx"))
    ...
    async def _dispatch_message(self, msg: Dict[str, Any], envelope: Dict[str, Any]) -> None:
        ...
        try:
            ...
            if text.startswith("/setup-files") and event.source is not None:
                ...
                # ${AGENTX_HOME}/google_chat_user_tokens/<sanitized>.json
    ...
    async def _build_message_event(
        ...
        """Parse a Chat API message into an agentx MessageEvent."""
        space = envelope.get("space") or msg.get("space") or {}
    ...
    async def send_typing(self, chat_id: str, metadata: Any = None) -> None:
        """Post a visible 'AgentX is thinking…' marker message.
        ...
        "AgentX is thinking…" stuck in chat forever, plus one card that
    ...
    async def stop_typing(self, chat_id: str) -> None:
        ...
        Google Chat has no separate typing API: the "AgentX is thinking…"
    ...
    async def on_processing_complete(
        ...
        see "AgentX is thinking…" stuck forever.
        """
    ...
    async def _consume_typing_card_with_text(
        ...
        creating a fresh "AgentX is thinking…" card during any subsequent
...
def interactive_setup() -> None:
    """Walk the user through Google Chat configuration via ``agentx setup``.
    ...
    prompt for env vars, persist them to ``~/.agentx/.env`` so the next
    ...
    print()
    print_success("Google Chat configuration saved to ~/.agentx/.env")
    print_info("Restart the gateway: agentx gateway restart")
...
def register(ctx) -> None:
    ...
    ctx.register_platform(
        ...
        install_hint="Run `agentx setup` to install Google Chat support.",
        ...
        # the "AgentX is thinking..." marker patches and edit overhead.
        ...
        platform_hint=(
            ...
            "are generating a response, a 'AgentX is thinking…' marker message "
