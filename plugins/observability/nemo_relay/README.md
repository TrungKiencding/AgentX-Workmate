...
Optional AgentX observability plugin that configures exporters and maps
AgentX-specific observer hooks to NeMo Relay marks and ATIF state. AgentX core
...
not replace AgentX Workmate's planner, tools, memory, model provider routing, or
CLI UX. AgentX core emits NeMo Relay lifecycle events for provider and tool
...
With this plugin enabled, AgentX Workmate can:

- Export the Relay scopes and LLM/tool lifecycles emitted by AgentX core.
- Add AgentX session, turn, approval, and subagent mark events.
...
The `AGENTX_NEMO_RELAY_*` environment variables below only configure an
already-enabled plugin. They do not enable plugin discovery by themselves.

For isolated test homes, enable the plugin in the same `AGENTX_HOME` that the
agent run will use:
...
`AGENTX_HOME`, so local E2E tests should omit that flag unless the test harness
loads `observability/nemo_relay` explicitly another way.

`AGENTX_HOME` is the AgentX profile/config home used by both
`agentx plugins enable ...` and the later `agentx chat ...` run. If unset,
AgentX uses the user's default home, usually `~/.agentx`. For isolated smoke
...
For source checkouts, make sure the `agentx` command you run is built from the
checkout that contains this plugin. A globally installed older CLI will not see
new bundled plugins from your working tree.

```bash
uv sync
uv run agentx plugins enable observability/nemo_relay
uv run agentx chat --query 'Reply exactly ok' --provider custom --model qwen3.6:35b
```
...
wheel, AgentX installs its supported NeMo Relay runtime as a normal dependency:
...
behavior; it does not control AgentX shared client metrics.

## Export Configuration

The plugin can configure exporters directly from `AGENTX_NEMO_RELAY_*`
...
export AGENTX_NEMO_RELAY_ATOF_OUTPUT_DIRECTORY=.nemo-relay/atof
export AGENTX_NEMO_RELAY_ATIF_ENABLED=1
export AGENTX_NEMO_RELAY_ATIF_OUTPUT_DIRECTORY=.nemo-relay/atif
```

Optional overrides:

- `AGENTX_NEMO_RELAY_ATOF_FILENAME`
- `AGENTX_NEMO_RELAY_ATOF_MODE` (`append` or `overwrite`)
- `AGENTX_NEMO_RELAY_ATIF_FILENAME_TEMPLATE`
- `AGENTX_NEMO_RELAY_ATIF_AGENT_NAME`
- `AGENTX_NEMO_RELAY_ATIF_AGENT_VERSION`
- `AGENTX_NEMO_RELAY_ATIF_MODEL_NAME`
- `AGENTX_NEMO_RELAY_ATIF_SUBAGENT_EXPORT_MODE` (`embedded` by default; set `all` to also write standalone child files)

### NeMo Relay Component Config

To initialize NeMo Relay from a component config, create a `plugins.toml` file
and point AgentX at it:

```bash
export AGENTX_NEMO_RELAY_PLUGINS_TOML=.nemo-relay/plugins.toml
```
...
agent_name = "AgentX Workmate"
agent_version = "local"
```

When `AGENTX_NEMO_RELAY_PLUGINS_TOML` is set and initializes successfully, NeMo
Relay owns exporter lifecycle through that config. The direct
`AGENTX_NEMO_RELAY_ATOF_*` fallback setup is skipped. If the same
`plugins.toml` observability config enables `atif`, the direct
`AGENTX_NEMO_RELAY_ATIF_*` fallback setup is also skipped so AgentX does not
double-export trajectories on teardown. If `plugins.toml` initialization fails,
AgentX keeps the direct env-var fallbacks active for that run.

AgentX core routes provider and tool execution through NeMo Relay managed APIs
...
AgentX uses the dynamic-plugin activation API available in NeMo Relay 0.6 and
later. Configure native or worker plugins with AgentX-owned
...
with this AgentX-owned section. The gateway combines those records with
separate lifecycle state for enablement, trust policy, and worker environments;
the Python binding does not yet expose that resolver. AgentX rejects
...
AgentX activates these plugins before registering its managed LLM and tool
...
For the full generic AgentX middleware contract, see
...
This run starts a parent AgentX session, delegates to a child subagent, has the
...
export AGENTX_NEMO_RELAY_ATIF_SUBAGENT_EXPORT_MODE=all
...
```json
{
  ...
  "session_id": "docs-parent-session",
  "agent": {"name": "AgentX Workmate E2E", "version": "docs-example", "model_name": "qwen3.6:35b"},
  "steps": [
    {
      "source": "agent",
...
message. AgentX dispatches the read-only tools as one batch, and NeMo Relay
...
- AgentX sessions map to `agent` scopes.
- AgentX core managed provider calls map to `llm` scope start/end events.
- AgentX core managed tool calls map to `tool` scope start/end events.
...
AgentX core owns the LLM and tool boundaries and enters NeMo Relay managed
execution while an AgentX-managed Relay consumer is active. With no shared
metrics subscriber or explicitly configured Relay plugin, AgentX calls the
...
```text
AgentX provider call
  -> nemo_relay.llm.execute(...)
    -> AgentX provider adapter callback(...)

AgentX tool call
  -> nemo_relay.tools.execute(...)
    -> AgentX authorization and dispatch callback(...)
```
...
middleware for a local AgentX run. This path requires a NeMo Relay runtime that
