"""Shared constants for AgentX Workmate.
...
# branding is stdlib-only and imports nothing from this project, so it is safe
# to pull in from a module that everything else imports.
from branding import CONFIG_DIR_POSIX, CONFIG_DIR_WINDOWS


_profile_fallback_warned: bool = False
_UNSET = object()
_AGENTX_HOME_OVERRIDE: ContextVar[str | object] = ContextVar(
    "_AGENTX_HOME_OVERRIDE", default=_UNSET
)
...
def set_hermes_home_override(path: str | Path | None) -> Token:
    """Set a context-local AgentX home override and return its reset token.

    This is for in-process, per-task scoping.  It deliberately does not mutate
    ``os.environ`` because that is shared by every thread in the process.
    """
    value: str | object = _UNSET if path is None else str(path)
    return _AGENTX_HOME_OVERRIDE.set(value)


def reset_hermes_home_override(token: Token) -> None:
    """Restore the previous context-local AgentX home override."""
    _AGENTX_HOME_OVERRIDE.reset(token)


def get_hermes_home_override() -> str | None:
    """Return the active context-local AgentX home override, if any."""
    override = _AGENTX_HOME_OVERRIDE.get()
...
def _get_platform_default_hermes_home() -> Path:
    ...
    The directory *names* come from :mod:`branding` rather than being spelled
    here, so the product config directory is renamed in exactly one place.
    """
    if sys.platform == "win32":
        local_appdata = os.environ.get("LOCALAPPDATA", "").strip()
        base = Path(local_appdata) if local_appdata else Path.home() / "AppData" / "Local"
        return base / CONFIG_DIR_WINDOWS
    return Path.home() / CONFIG_DIR_POSIX


def _hermes_home_from_env() -> Path:
    """Resolve AGENTX_HOME from the process environment only.

    Reads the ``AGENTX_HOME`` env var, falling back to the platform-native
...
def _warn_profile_fallback_once() -> None:
    ...
    Guard: if a non-default profile is sticky-active but ``AGENTX_HOME`` is
    ...
    if active and active != "default":
        ...
        msg = (
            f"[AGENTX_HOME fallback] AGENTX_HOME is unset but active "
            f"profile is {active!r}. Falling back to {fallback_home}, which "
            f"is the DEFAULT profile — not {active!r}. Any data this "
            f"process writes will land in the wrong profile. The "
            f"subprocess spawner should pass AGENTX_HOME explicitly "
            f"(see issue #18594)."
        )
        try:
...
def get_hermes_home() -> Path:
    """Return the AgentX home directory (default: platform-native path).

    Resolution order: context-local override (see
    :func:`set_hermes_home_override`) → ``AGENTX_HOME`` env var → the
    platform-native default.  This is the single source of truth — all other
    copies should import this.

    When ``AGENTX_HOME`` is unset but an ``active_profile`` file indicates
    ...
    if not os.environ.get("AGENTX_HOME", "").strip():
...
def get_process_hermes_home() -> Path:
    """Return the AgentX home for the running process, ignoring task overrides.
    ...
    process ``AGENTX_HOME`` env var (falling back to the platform default),
...
def get_default_hermes_root() -> Path:
    ...
    In Docker or custom deployments where ``AGENTX_HOME`` points outside
    ``~/.agentx`` (e.g. ``/opt/data``), returns ``AGENTX_HOME`` directly
    — that IS the root.
    ...
    env_home = os.environ.get("AGENTX_HOME", "")
    if not env_home:
        return native_home
...
def get_optional_skills_dir(default: Path | None = None) -> Path:
    ...
    tree and expose it via ``AGENTX_OPTIONAL_SKILLS``.
    """
    override = os.getenv("AGENTX_OPTIONAL_SKILLS", "").strip()
    if override:
        return Path(override)
...
def get_optional_mcps_dir(default: Path | None = None) -> Path:
    ...
    package tree and expose it via ``AGENTX_OPTIONAL_MCPS``.
    """
    override = os.getenv("AGENTX_OPTIONAL_MCPS", "").strip()
    if override:
        return Path(override)
    if default is not None:
        return default
...
def get_bundled_skills_dir(default: Path | None = None) -> Path:
    ...
    Resolution order:
        1. ``AGENTX_BUNDLED_SKILLS`` env var (Nix wrapper / explicit override)
        2. Caller-supplied ``default`` (typically the source-checkout path)
        3. ``<AGENTX_HOME>/skills`` last-resort
    """
    override = os.getenv("AGENTX_BUNDLED_SKILLS", "").strip()
    if override:
        return Path(override)
    if default is not None:
        return default
...
def get_hermes_dir(
    ...
    *,
    ...
    Args:
        new_subpath: Preferred path relative to AGENTX_HOME (e.g. ``"cache/images"``).
        old_name: Legacy path relative to AGENTX_HOME (e.g. ``"image_cache"``).
        home: Optional explicit AgentX home. Profile-aware callers that manage
            more than one home in the same process use this instead of
            temporarily mutating the process or context-local AGENTX_HOME.
...
def iter_hermes_node_dirs(home: Path | None = None) -> list[Path]:
    """Return AgentX-managed Node.js directories in preferred lookup order.

    Windows installs from ``scripts/install.ps1`` unpack portable Node directly
    into ``%LOCALAPPDATA%\\agentx\\node``. POSIX installs use
    ``$AGENTX_HOME/node/bin``. Include both shapes on every platform so mixed
...
_AGENTX_NODE_TARGET_MAJOR = int(os.environ.get("AGENTX_NODE_TARGET_MAJOR", "22"))
...
# 0700 breaks the service account traversal in Docker (UID 10000). See #25821, #93050.
...
def node_tool_runnable(path: str | None) -> bool:
    ...
    AgentX-managed Node trees live under ``$AGENTX_HOME/node`` (or a profile's
    ``AGENTX_HOME``). A partial upgrade or interrupted install can leave
    ...
    used to trust file presence alone, so ``agentx update`` would pick that
...
def hermes_managed_node_tree_present(home: Path | None = None) -> bool:
    """Return True when any AgentX-managed node/npm/npx shim exists on disk."""
    names = set()
...
def _heal_managed_node_windows() -> bool:
    """Redownload the portable Node zip into ``%AGENTX_HOME%\\node`` on Windows."""
    import re
    import tempfile
    ...
    index_url = f"https://nodejs.org/dist/latest-v{_AGENTX_NODE_TARGET_MAJOR}.x/"
    try:
    ...
    match = re.search(
        rf"node-v{_AGENTX_NODE_TARGET_MAJOR}\.\d+\.\d+-win-{node_arch}\.zip",
        index_html,
    )
    if not match:
        return False
...
def _bootstrap_managed_node_posix() -> bool:
    """Install a fresh managed Node under ``$AGENTX_HOME/node`` on POSIX.
    ...
    ``AGENTX_NODE_SKIP_LINKS=1`` so the user's own node/npm on PATH is not
    ...
    try:
        result = subprocess.run(
            ...
            env={
                **os.environ,
                "AGENTX_HOME": str(get_hermes_home()),
...
def bootstrap_hermes_managed_node() -> str | None:
    ...
    AgentX never modifies a toolchain it does not own, so instead it provisions
    its own tree under ``$AGENTX_HOME/node`` (the same tree a fresh install
...
def heal_hermes_managed_node() -> bool:
    """Redownload AgentX-managed Node when the tree exists but is broken.
    ...
    try:
        result = subprocess.run(
            ...
            env={**os.environ, "AGENTX_HOME": str(get_hermes_home())},
            capture_output=True,
            timeout=300,
            check=False,
        )
...
def _managed_node_tree_outdated(home: Path | None = None) -> bool:
    ...
    ``latest-v{_AGENTX_NODE_TARGET_MAJOR}.x`` — so existing users are upgraded
    ...
    for directory in iter_hermes_node_dirs(home):
        for name in _candidate_node_command_names("node"):
            ...
            return major < _AGENTX_NODE_TARGET_MAJOR
    return False
...
def find_node_executable_on_path(command: str) -> str | None:
    ...
    directly, so prefer the launchable variants explicitly for AgentX-owned
    subprocesses.
    """
    if sys.platform != "win32":
...
def find_node_executable(command: str) -> str | None:
    """Resolve a Node.js command, preferring healthy AgentX-managed installs.

    This is for AgentX-owned subprocesses that should not be broken by a bad,
...
def with_hermes_node_path(env: dict[str, str] | None = None) -> dict[str, str]:
    """Return *env* with AgentX-managed Node directories prepended to PATH."""
...
def agent_browser_runnable(path: str | None) -> bool:
    ...
    next ``agentx update`` — leaving a **dangling symlink** that ``which`` still
...
def display_hermes_home() -> str:
    """Return a user-friendly display string for the current AGENTX_HOME.
...
def secure_parent_dir(path: Path) -> None:
    ...
    if parent == Path("/") or len(parent.parts) < 3:
        return
    # Refuse the install tree root. chmodding it 0700 breaks the service
    # account's traversal in Docker (UID 10000) and any other install where
    # the runtime user doesn't own the install dir. See #25821, #93050.
...
def _profile_home_path(env: dict[str, str] | None = None) -> str | None:
    """Return ``{AGENTX_HOME}/home`` when the profile-home directory exists."""
    hermes_home = get_hermes_home_override() or (env or {}).get("AGENTX_HOME") or os.getenv("AGENTX_HOME")
    if not hermes_home:
        return None
...
def _iter_real_home_candidates(env: dict[str, str] | None = None) -> list[str]:
    ...
    env = env or {}
    candidates: list[str] = []
    explicit = str(env.get("AGENTX_REAL_HOME") or os.getenv("AGENTX_REAL_HOME", "")).strip()
    if explicit:
...
def get_real_home(env: dict[str, str] | None = None) -> str:
    """Return the OS user's real home directory, avoiding AgentX profile HOME.

    ``AGENTX_HOME`` scopes AgentX state. ``HOME`` is reserved for the OS/user
    account and the many external CLIs that store credentials under ``~``.
    If a parent process is already running with ``HOME={AGENTX_HOME}/home``,
...
def get_subprocess_home(env: dict[str, str] | None = None) -> str | None:
    ...
    * ``auto`` (default): host installs keep the real user HOME; containers use
      ``{AGENTX_HOME}/home`` for persistent state. If a host parent already has
      HOME pointed at the profile home, repair subprocesses back to real HOME.
    * ``real``: always prefer the real OS-user HOME.
    * ``profile``: use ``{AGENTX_HOME}/home`` when it exists, preserving the
...
def apply_subprocess_home_env(env: dict[str, str]) -> None:
    """Apply AgentX' subprocess HOME contract to *env* in-place."""
    real_home = get_real_home(env)
    if real_home:
        env["AGENTX_REAL_HOME"] = real_home
...
def translate_cwd_for_wsl_backend(cwd: str) -> str:
    """Normalize a cross-boundary cwd when AgentX itself runs inside WSL.
...
def get_config_path() -> Path:
    """Return the path to ``config.yaml`` under AGENTX_HOME.
...
def get_skills_dir() -> Path:
    """Return the path to the skills directory under AGENTX_HOME."""
    return get_hermes_home() / "skills"



def get_env_path() -> Path:
    """Return the path to the ``.env`` file under AGENTX_HOME."""
...
# Top-level packages/modules that ship as part of AgentX itself. An ImportError
...
def is_first_party_module(name: str | None) -> bool:
    """True when *name* is a module that ships with AgentX.
...
def partial_update_hint(exc: BaseException) -> list[str]:
    ...
    rather than their config, is the problem — and `agentx update` is exactly
