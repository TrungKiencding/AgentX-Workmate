...
# 5. Symlinks the 'agentx' CLI command into a user-facing bin dir
...
echo ""
echo -e "${CYAN}⬡ AgentX Workmate Setup${NC}"
echo ""
...
if is_termux; then
    ...
    if [ -n "$UV_CMD" ]; then
        ...
        _uv_log="$(mktemp 2>/dev/null || echo "/tmp/agentx-uv-install.$$.log")"
...
# ============================================================================
# PATH setup — symlink agentx into a user-facing bin dir
# ============================================================================

echo -e "${CYAN}→${NC} Setting up agentx command..."

AGENTX_BIN="$SCRIPT_DIR/venv/bin/agentx"
COMMAND_LINK_DIR="$(get_command_link_dir)"
COMMAND_LINK_DISPLAY_DIR="$(get_command_link_display_dir)"
mkdir -p "$COMMAND_LINK_DIR"
ln -sf "$AGENTX_BIN" "$COMMAND_LINK_DIR/agentx"
echo -e "${GREEN}✓${NC} Symlinked agentx → $COMMAND_LINK_DISPLAY_DIR/agentx"
...
# ============================================================================
# Seed bundled skills into ~/.agentx/skills/
# ============================================================================

AGENTX_SKILLS_DIR="${AGENTX_HOME:-$HOME/.agentx}/skills"
mkdir -p "$AGENTX_SKILLS_DIR"

echo ""
echo "Syncing bundled skills to ~/.agentx/skills/ ..."
if "$SCRIPT_DIR/venv/bin/python" "$SCRIPT_DIR/tools/skills_sync.py" 2>/dev/null; then
    echo -e "${GREEN}✓${NC} Skills synced"
else
    # Fallback: copy if sync script fails (missing deps, etc.)
    if [ -d "$SCRIPT_DIR/skills" ]; then
        cp -rn "$SCRIPT_DIR/skills/"* "$AGENTX_SKILLS_DIR/" 2>/dev/null || true
...
echo "Other commands:"
echo "  agentx status        # Check configuration"
if is_termux; then
    echo "  agentx gateway       # Run gateway in foreground"
else
    echo "  agentx gateway install # Install gateway service (messaging + cron)"
fi
echo "  agentx cron list     # View scheduled jobs"
echo "  agentx doctor        # Diagnose issues"
echo ""
