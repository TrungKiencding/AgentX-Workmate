#!/usr/bin/env python3
"""
SQLite State Store for AgentX Workmate.
...
def _workspace_key_clause(key: str) -> Tuple[str, List[str]]:
    ...
    Used by ``agentx -c``/``--resume`` to continue the most recent session in
...
def _default_db_path() -> Path:
    ...
    freezes the developer's real ``~/.agentx`` even when a test fixture later
    redirects ``AGENTX_HOME`` — importing this module during collection was
    ...
    2. Otherwise resolve ``get_hermes_home()`` fresh so a runtime
       ``AGENTX_HOME`` redirect takes effect regardless of import order.
    """
...
def _wal_reset_repair_hint() -> str:
    ...
    ``agentx update`` can actually do for this install (#75153).
    """
    try:
        ...
        if method in {"git", "unknown"}:
            return f"AgentX-managed installs can repair the embedded runtime with `{cmd}`"
        if method == "docker":
    ...
    return (
        ...
        "(or backports 3.50.7 / 3.44.6) and restart AgentX"
    )
...
def _log_wal_reset_bug_once(
    ...
    *,
    ...
    # Check whether this is an AgentX-managed install (uv-managed venv)
    ...
    logger.warning(
        ...
        "%s. See `agentx doctor`. This warning fires once per "
...
def preflight_db_writability(
    ...
    *,
    ...
    - **Repairs** permissions with ``chmod u+rw`` when the file lives inside
      the AgentX home tree (``get_hermes_home()``) — the safe repair scope:
      AgentX owns those files, and the OS makes ``chmod`` fail on files the
    ...
    def _ensure_writable(p: Path, *, is_dir: bool = False) -> None:
        ...
        raise sqlite3.OperationalError(
            ...
            f"user. AgentX needs read-write access to open the database. "
...
# (``~/.agentx/lib/libfts5_cjk.so``, built by ``native/fts5_cjk/build.sh``).
...
# next ``agentx sessions optimize-storage`` on a capable host).
#
...
def fts5_cjk_so_path() -> Path:
    ...
    env = os.getenv("AGENTX_FTS5_CJK_SO")
    if env:
...
def _cjk_fts_config_enabled() -> bool:
    ...
    return os.getenv("AGENTX_CJK_FTS", "1").strip().lower() not in (
        "0", "false", "off", "no",
    )
    ...
    try:
        ...
        if not acquired:
            ...
            logger.error(
                ...
                "state-snapshots via `agentx snapshot list` / "
                "`agentx snapshot restore <id>`.",
                path,
            )
            return None
...
class SessionDB(
    ...
    # With multiple agentx processes (gateway + CLI sessions + worktree agents)
    ...
    # legitimately held for multi-second stretches by sibling AgentX
    ...
    # `agentx update` leaves mixed-version processes sharing the DB until
        ...
        try:
            ...
            if (
                ...
                msg = (
                    ...
                    f"Restore from {snaps} via `agentx snapshot list` / "
                    f"`agentx snapshot restore <id>` if available. "
            ...
            # NOTE: the v23 FTS optimization is OPT-IN (`agentx db optimize`),
    ...
    def _warn_fts5_unavailable(self, exc: sqlite3.OperationalError) -> None:
        ...
        logger.warning(
            ...
            "disabled. Run `agentx update` to rebuild the venv with a "
            ...
            if cjk_present:
                ...
                if live:
                    ...
                    logger.warning(
                        ...
                        "run `agentx sessions optimize-storage` on a host "
        ...
        while True:
            try:
                ...
                if "locked" in err_msg or "busy" in err_msg:
                    ...
                    raise sqlite3.OperationalError(
                        f"database is locked (another AgentX process held the "
    ...
    #
    # `optimize_fts_storage()` (the `agentx sessions optimize-storage`
    ...
    # ── Opt-in v23 FTS storage optimization (`agentx sessions optimize-storage`) ──
    #
    ...
    def set_session_yolo(self, session_id: str, enabled: bool) -> None:
        ...
        ``--yolo`` launch — survives ``agentx --resume`` into a fresh
    ...
    def search_sessions(
        ...
        ``agentx -c``/``--resume`` so the "last" session is the last one in
    ...
    def delete_session_if_empty(
        ...
        and ``agentx sessions list`` output. (Pattern ported from
    ...
    def retag_kanban_worker_sessions(self, workspaces_root: str) -> int:
        ...
        Workers used to spawn without ``AGENTX_SESSION_SOURCE``, so their runs
    ...
    def apply_telegram_topic_migration(self) -> None:
        ...
        reconciliation. Operators must be able to upgrade AgentX, keep the old
    ...
    def bind_telegram_topic(
        ...
        *,
        ...
        """Bind one Telegram DM topic thread to one AgentX session.

        An AgentX session may only be linked to one Telegram topic in MVP.
    ...
    def is_telegram_session_linked_to_topic(self, *, session_id: str) -> bool:
        """Return True if an AgentX session is already bound to any Telegram DM topic.
